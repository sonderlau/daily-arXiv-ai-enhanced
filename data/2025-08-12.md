...
Motivation: 解决传统MEF模型未能充分利用高精度但分散的预测成员的问题，提升ENSO长期预测的可靠性。

Method: 构建无向图表示集合成员相似性（基于RMSE和相关性），通过社区检测选择20个最优成员进行平均预测。

Result: 该方法提高了预测技能，尤其在长期复合情景下表现更稳定，且具有模型无关性。

Conclusion: 图分析方法优化了集合预测，为其他大规模集合模型提供了通用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 为公众提供黑色暴雨的早期预警具有重要应用价值，能显著影响生活。

Method: 利用天气雷达的三维风场数据，分析强对流的触发因素，并评估AI模型的预测能力。

Result: 研究发现四次暴雨的可预测性不同，AI模型能提前一周成功预测两次主要暴雨。

Conclusion: 地表辐合、中层波动和急流是暴雨发生的重要机制，未来研究需进一步探讨黑色暴雨的预测方法。

Abstract: Four episodes of black rainstorms, the highest tier of heavy rain according
to the rainstorm warning system in Hong Kong, occurred within eight days from
29 July to 5 August 2025, breaking the record for torrential rain in the
territory. Providing early alerts to the public about these black rainstorms
has considerable application value and has a substantial impact on their lives.
This initial review of the four black rainstorm cases focuses on the
predictability of the events from the synoptic/mesoscale to the micro/storm
scale. A newly available tool for three-dimensional wind fields retrieved from
weather radars in the region was used to analyse the triggering factors for the
development and maintenance of intense convection. Given the current level of
technology, the four episodes of heavy rain had different degrees of
predictability. Global artificial intelligence (AI) models were found to be
skilful in capturing two major rainstorms, including one week before they
occurred. Surface convergence, mid-tropospheric waves, and jets were important
mechanisms in heavy rainfall occurrence. Future research directions on these
black rainstorms have been briefly discussed.
...
<details>
  <summary>Details</summary>
Motivation: 探索储层计算机在无噪声系统中的预测性能及其超参数的影响，为实际应用提供理论支持。

Method: 通过调整正则化系数、储层大小和谱半径等超参数，结合Lorenz方程预测性能，分析VPT与这些参数的关系。

Result: 在无噪声系统中，RC的VPT可超过30个Lyapunov时间，且VPT可通过前几步预测误差高效预测。

Conclusion: 无噪声系统中，RC的预测性能优异，但需注意数值求解器的选择，避免VPT超过有效基准时间（VGTT）。
...
<details>
  <summary>Details</summary>
Motivation: 现有脉冲图神经网络主要在欧几里得空间运行，依赖固定几何假设，难以建模复杂图结构（如层次和循环）。GSG旨在克服这些限制。

Method: GSG包含三个关键组件：黎曼嵌入层、流形脉冲层和流形学习目标，通过黎曼SGD训练，无需时间反向传播。

Result: 实验表明，GSG在准确性、鲁棒性和能效上优于欧几里得SNN和基于流形的GNN。

Conclusion: GSG为曲率感知、高能效的图学习建立了新范式。
...
<details>
  <summary>Details</summary>
Motivation: 光伏模型参数识别因非线性、多模态和高维度特性而具有挑战性，传统DBO算法易陷入早熟收敛。

Method: 结合分数阶微积分增强搜索记忆性，使用分数阶逻辑混沌映射初始化种群，并引入混沌扰动机制避免局部最优。

Result: 在CEC2017基准和光伏参数识别问题上，MFO-DBO在准确性、鲁棒性和收敛速度上均优于现有算法。

Conclusion: MFO-DBO算法显著提升了光伏参数识别的性能，并在探索与开发之间保持了良好平衡。
...
<details>
  <summary>Details</summary>
Motivation: 现有MOEA多关注目标空间优化，忽视了决策空间多样性，而这对决策者提供广泛选择至关重要。

Method: 在MOEA的选择机制中直接集成基于汉明距离的均匀性度量。

Result: 在饮食问题的多目标优化中，显著提高了决策空间多样性，同时保持目标空间性能。
...
<details>
  <summary>Details</summary>
Motivation: 探索在非确定性、部分可观测的游戏环境中，结合深度学习和进化优化的混合学习策略的有效性。

Method: 使用LSTM网络评估棋盘状态，通过CMA-ES优化决策策略。

Result: 模型在模拟中表现优于随机和规则代理，性能接近PPO基线。

Conclusion: 混合学习策略为复杂随机游戏提供了计算高效的替代方案，并展示了在自适应决策和战略AI开发中的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 解决复杂问题需要高效算法，但代理模型辅助的元启发式算法在能量效率和准确性方面仍需深入研究。

Method: 研究了不同版本的粒子群优化算法（包括预训练和再训练的神经网络代理模型），分析其处理器和内存的能量消耗及代理模型准确性。

Result: 研究揭示了代理模型辅助算法的能量效率和准确性特征，为更全面的算法评估提供了初步方法。

Conclusion: 这是首次从能量和准确性角度评估代理模型辅助算法的研究，为未来更全面的优化和学习技术评估奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 探索DGCA模型在形态发生和储层生成中的应用潜力，为功能性和适应性形态发生建模奠定基础。

Method: 训练DGCA生成储层，分为任务驱动（NARMA任务）和任务无关（储层指标）两种目标。

Result: DGCA能生成多样化的生命样结构，有效解决基准任务，性能优于传统储层。

Conclusion: DGCA为生成可塑性储层和建模适应性形态发生提供了新方向。
...
Result: Med-GRIM在医疗VQA中实现高效准确，接近大模型性能但成本更低。

Conclusion: BIND和Med-GRIM为医疗VQA提供高效解决方案，并开源数据集DermaGraph支持研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的肖像动画方法主要关注唇同步或静态情感转换，忽视了动态风格（如头部运动），且双U-Net架构计算开销大。

Method: DiTalker采用Style-Emotion Encoding Module分离身份特定风格和身份无关情感特征，并通过Audio-Style Fusion Module解耦音频与说话风格。

Result: 实验表明DiTalker在唇同步和说话风格控制上优于现有方法。

Conclusion: DiTalker通过模块化设计和优化约束，实现了高质量的肖像动画，解决了动态风格和计算效率问题。
...
<details>
  <summary>Details</summary>
Motivation: 社交媒体上针对肌肉畸形行为的危害内容（如pro-bigorexia）难以通过传统文本检测系统识别，亟需多模态方法解决。

Method: 开发BigTokDetect框架，基于专家标注的多模态数据集BigTok（2200+视频），结合视觉语言模型进行多模态分类。

Result: 模型在主要类别分类上达到82.9%准确率，子类别检测为69%，多模态融合比纯文本方法提升5-10%性能。

Conclusion: 研究为心理健康领域的危害内容检测提供了新基准和工具，展示了多模态方法的优势。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有半监督学习方法在息肉分割中因忽略结构特性而导致的泛化能力不足问题。

Method: FPGM通过两阶段过程：学习域不变频率先验，并对未标记图像进行频谱扰动，以对齐振幅谱并保留相位信息。

Result: 在六个公共数据集上验证，FPGM在零样本泛化能力上表现优异，Dice分数提升超过10%。

Conclusion: FPGM为临床部署提供了强大的息肉分割解决方案，显著提升了跨域鲁棒性。
...
Method: 通过提示LMM验证视觉模型预测，分析视觉-语言连接器的行为，并测试训练无关的连接器在细粒度识别任务中的效果。

Result: 验证了LMMs通过文本概念映射和常识推理提升性能，且依赖紧凑的文本表示而非原始视觉特征。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统需要高精度和实时性的全景驾驶感知，现有方法在多任务联合处理时存在负迁移和手动设计结构的问题。

Method: 提出RMT-PPAD模型，采用门控适配器融合共享和任务特定特征，设计自适应分割解码器自动学习多尺度特征权重，并解决车道线分割标签不一致问题。

Result: 在BDD100K数据集上，目标检测mAP50达84.9%，召回率95.4%；可行驶区域分割mIoU为92.6%；车道线分割IoU为56.8%，准确率84.7%；推理速度32.6 FPS。

Conclusion: RMT-PPAD在性能和实时性上均达到SOTA，实际场景中表现稳定，代码和模型已开源。
...
<details>
  <summary>Details</summary>
Motivation: 现有POPE基准在评估LVLMs的幻觉问题时效果减弱，因其忽略了图像特定信息且仅限制于负面对象类别。

Method: HOPE利用CLIP选择高预测可能性的负面对象作为干扰项，并通过配对真实对象与虚假描述构建误导性干扰项。

Result: HOPE导致各种LVLMs的精度下降9%至23%，显著优于POPE。

Conclusion: HOPE为评估LVLMs的幻觉免疫性提供了更严格的基准。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据集（如ImageNet、COCO、PASCAL VOC）专注于日常物体，缺乏非商业领域中的信号稀疏性。MobilTelesco填补了这一空白。

Method: 使用MobilTelesco数据集对多种检测模型进行基准测试。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在复杂多实例场景（如人-人遮挡、人-物纠缠）中表现不佳，主要原因是数据集限制和缺乏空间解耦能力。

Method: 提出了Multi-Layer Diffusion (MILD)策略，结合Human Morphology Guidance和Spatially-Modulated Attention，分层生成实例和背景。

Result: MILD在复杂人像擦除任务中优于现有方法。

Conclusion: MILD通过分层解耦和形态学引导，显著提升了复杂场景下的人像擦除效果。
...
<details>
  <summary>Details</summary>
Motivation: 在缺乏3D真实标注的情况下，探索如何仅利用多视角RGB图像实现准确的3D语义场景图估计。

Method: 设计语义掩码引导特征聚合以减少背景噪声，并引入邻居节点信息增强特征鲁棒性；利用统计先验优化节点和边的预测。

Result: 实验表明，该方法在仅使用多视角图像作为输入的情况下优于现有方法。

Conclusion: 通过语义和空间信息增强以及统计先验的利用，实现了高效且鲁棒的3D语义场景图估计。
...
<details>
  <summary>Details</summary>
Motivation: 解决深度神经网络训练资源密集、适应不同用户需求时需多模型版本的低效问题。

Method: 提出NNObfuscator机制，通过实时调整单一模型性能，支持分层访问控制。

Result: 实验验证了NNObfuscator在图像分类、语义分割等任务中的有效性，单一模型可适应多需求。

Conclusion: NNObfuscator提高了资源利用率，支持可持续AI部署商业模式。
...
Result: 模型在新数据集上表现更公平，整体准确率和跨数据集泛化能力提升。

Conclusion: 研究提供了一个公平性更强的深度伪造数据集和模型流程，为未来研究奠定基础。
...
<details>
  <summary>Details</summary>
Motivation: 当前评测方法依赖交互式模拟环境或真实场景，成本高且难以扩展，亟需高效统一的评测方案。

Method: 开发StaticEmbodiedBench，覆盖42个场景和8个核心维度，提供简单接口支持可扩展评测。

Result: 评测了19个VLMs和11个VLAs，建立了首个统一的静态榜单，并公开200个样本以推动研究。

Conclusion: StaticEmbodiedBench为具身智能评测提供了高效、可扩展的解决方案，并推动了领域发展。
...
Method: 采用多级负面反馈驱动的视觉细化范式，核心代理包括设计师和顾问，通过分层视觉语言模型反馈逐步优化。

Result: 实验表明StyleTailor在个性化设计和推荐方面优于基线方法，建立了时尚系统的新基准。

Conclusion: StyleTailor通过闭环反馈机制显著提升了时尚搭配的个性化和精确性。
...
<details>
  <summary>Details</summary>
Motivation: 对象检测数据集中的标签错误（如缺失标签、错误分类或定位不准确）会影响训练和评估结果，但目前缺乏系统性的大规模修正方法。

Method: REC✓D框架结合现有错误检测器和众包微任务，通过多标注者独立验证候选边界框，并聚合反馈以提高标签质量。

Result: 在KITTI数据集的“行人”类中，REC✓D发现原始标注中至少24%的错误，并生成了高质量修正标注。

Conclusion: REC✓D框架显著提升了标签修正效率，但现有方法仍可能遗漏大量错误，需进一步研究。发布的基准数据集将推动相关研究。
...
<details>
  <summary>Details</summary>
Motivation: 临床实践中部署深度学习模型常需多模态数据，但推理时并非所有模态都可用，因此需要一种方法利用训练时的额外模态提升模型性能。

Method: 提出MMPKD训练策略，使用文本和表格元数据作为教师模型，将知识蒸馏到视觉Transformer学生模型中。

Result: MMPKD提升了注意力图在输入图像中定位ROI的零样本能力，但效果不跨领域。
...
<details>
  <summary>Details</summary>
Motivation: 多模态情感识别任务中，现有模型缺乏心理学意义的先验指导多模态对齐，VEGA旨在填补这一空白。

Method: 利用CLIP的图像编码器构建情感特定的视觉锚点，结合随机锚点采样策略和双分支自蒸馏架构。

Result: 在IEMOCAP和MELD数据集上实现了最先进的性能。

Conclusion: VEGA机制通过心理学理论和视觉语义的引入，显著提升了多模态情感识别的效果。
...
<details>
  <summary>Details</summary>
Motivation: 结合脑成像数据和临床报告，利用多模态信息提升临床诊断效果，解决如何有效关联客观成像数据与主观文本报告的挑战。

Method: 将脑子网络视为成像数据的标记，与临床报告中的词语标记对齐，构建共享跨模态潜在空间。
...
Result: Surformer v1达到99.4%准确率，推理时间0.77毫秒，优于多模态CNN。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要集中在文本和图像上的仇恨言论检测，视频领域的隐性仇恨言论检测尚未充分探索。
...
Method: 基于视觉语言大模型（LVLMs），提出ContextGuard-LVLM框架，结合多阶段上下文推理和强化/对抗学习。

Result: 在多个数据集上显著优于零样本基线（如InstructBLIP和LLaVA 1.5），尤其在复杂逻辑推理和上下文理解任务中表现突出。

Conclusion: ContextGuard-LVLM在检测上下文一致性方面高效且鲁棒，与人类专家判断高度一致。
...
### [32] [VL-MedGuide: A Visual-Linguistic Large Model for Intelligent and Explainable Skin Disease Auxiliary Diagnosis](https://arxiv.org/abs/2508.06624)
...
Result: 在Derm7pt数据集上，VL-MedGuide在疾病诊断和概念检测上均达到最优性能。

Conclusion: VL-MedGuide通过提供透明且可操作的诊断解释，提升了AI在皮肤病临床实践中的实用性。
...
Conclusion: 联合学习框架提升了全局最优性，生成效果更优。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在处理复杂镜面反射和高光时存在模糊或优化不稳定的问题，需要更灵活的建模方式。

Method: 通过动态系数分解，将外观分解为静态神经基础和动态系数，由系数网络根据视图和光照生成，最终通过动态辐射积分器合成辐射。

Result: 在多个挑战性基准测试中，该方法能生成比现有技术更清晰、更真实的镜面高光。

Conclusion: 动态系数分解范式为神经场景表示中的复杂外观建模提供了灵活有效的方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖准确的关键帧索引，但实际中获取准确索引困难且存在误差，限制了微表情识别的实际应用。

Method: CausalNet采用完整序列输入，通过Causal Motion Position Learning Module（CMPLM）定位肌肉运动区域，并利用Causal Attention Block（CAB）学习肌肉收缩与放松的因果关系。

Result: 实验表明，CausalNet在不同噪声水平的关键帧索引下均表现鲁棒，并在标准基准测试中超越现有最优方法。

Conclusion: CausalNet为微表情识别提供了一种鲁棒且高效的解决方案，适用于实际应用场景。
...
<details>
  <summary>Details</summary>
Motivation: 探索在自回归（AR）图像模型中应用生成水印的可行性，以解决现有方法在图像扰动下检测能力下降的问题。

Method: 提出了两种基于视觉令牌聚类的水印方法：1）无训练方法，使用聚类查找表；2）微调VAE编码器直接从扰动图像预测令牌聚类。

Result: 实验表明，聚类级水印提高了抗干扰性和再生攻击的鲁棒性，同时保持图像质量，且检测能力优于基线方法。

Conclusion: 聚类级水印在自回归图像模型中表现优异，具有快速验证和高效检测的特点。
...
<details>
  <summary>Details</summary>
Motivation: 现代视觉系统依赖冗余的视觉输入，而人类能轻松理解稀疏的线描。这表明结构而非外观是高效视觉理解的关键。

Method: 提出线描作为预训练模态，并通过实验验证其在分类、检测和分割任务中的效果。还提出无监督方法“学习绘制”。

Result: 线描预训练模型具有更强的形状偏好、更集中的注意力和更高的数据效率，且内在维度更低。知识蒸馏效果也更好。
...
<details>
  <summary>Details</summary>
Motivation: 抑郁症的早期检测对治疗至关重要，但传统方法依赖主观评估，而社交媒体数据提供了新的可能性。

Method: MMFformer利用Transformer网络提取视频的空间特征和音频的时间动态，通过融合策略整合多模态数据。

Result: 在两个大规模数据集上，MMFformer的F1-Score分别提升了13.92%和7.74%，优于现有方法。

Conclusion: MMFformer在多模态抑郁症检测中表现出色，为早期诊断提供了有效工具。
...
<details>
  <summary>Details</summary>
Motivation: 计算机生成全息术（CGH）是一种有前景的方法，但需要高效的算法和优化技术来提升生成质量和速度。

Method: 使用点云和MRI数据重建体积对象，通过交替投影、随机梯度下降和拟牛顿法生成相位全息图和复全息图。

Result: 比较了不同算法的重建性能（MSE、RMSE、PSNR），并通过2D中值滤波优化了性能指标。

Conclusion: 提出的框架在CGH生成中表现出色，优化算法和滤波技术显著提升了全息图的质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到图像和图像到视频生成模型虽然能提供丰富的语义先验，但难以捕捉物理真实性和运动动态性，而真实视频可以提供物理基础的几何和运动线索。论文旨在探索如何利用真实视频的运动先验生成物理一致的4D内容。

Method: 论文提出了Restage4D，一种几何保持的视频条件4D重演方法。该方法通过视频倒带训练策略，将真实基础视频和合成驱动视频通过共享运动表示连接，并结合遮挡感知的刚性损失和去遮挡回溯机制，提升运动下的结构和几何一致性。

Result: 在DAVIS和PointOdyssey数据集上的实验表明，Restage4D在几何一致性、运动质量和3D跟踪性能上均有提升，能够在新颖运动下保持可变形结构并自动纠正生成模型引入的错误。

Conclusion: Restage4D展示了视频先验在4D重演任务中的潜力，能够生成物理一致的4D内容，同时纠正生成模型的错误。
...
<details>
  <summary>Details</summary>
Motivation: 传统侵入性组织采样方法无法捕捉肿瘤的空间异质性，且现有深度学习模型因标注数据稀缺而受限。
...
Method: 使用计算机图形渲染技术构建VOccl3D数据集，包含多样化的真实遮挡场景，并微调CLIFF和BEDLAM-CLIFF方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在交通事故理解中主要关注粗粒度图像或视频级理解，难以处理细粒度视觉细节或局部场景，限制了在复杂事故场景中的应用。

Method: 提出SafePLUG框架，结合像素级理解和时序定位，支持任意形状视觉提示的区域感知问答和基于语言指令的像素级分割。

Result: 实验表明，SafePLUG在区域问答、像素级分割、时序事件定位和事故事件理解等任务中表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 解决术中超声图像因噪声、伪影和与术前MRI/CT扫描对齐不佳而难以解释的问题。

Method: 通过机器学习将MRI 3D扫描转换为声阻抗体积，使用射线追踪模拟超声束传播，并通过稀疏线性系统捕捉多次内部反射，最终生成B模式图像。

Result: 在ReMIND数据集上验证了DiffUS能够从脑MRI数据生成解剖学上准确的超声图像。

Conclusion: DiffUS为下游应用（如切片到体积配准和体积重建）提供了基于梯度的优化能力。
...
Result: 在CT和MRI数据集上，边界定位显著优于基线方法，并提升下游任务（如分割、配准）性能。
...
<details>
  <summary>Details</summary>
Motivation: 皮肤镜图像中黑色素瘤的精确分割对自动化癌症筛查和临床决策至关重要，但现有方法难以处理纹理、颜色变化及成像伪影。

Method: 采用ResNet启发的双分辨率架构，结合边界感知残差连接和通道注意力模块，并引入轻量级伪影抑制块和多任务训练目标。

Result: 在公共皮肤镜基准测试中，该方法显著改善了边界贴合度和临床相关分割指标，优于标准编码器-解码器基线。

Conclusion: 该方法为自动化黑色素瘤评估系统提供了实用的构建模块，无需复杂后处理或预训练协议。
...
Method: 利用稀疏标注（如中心线、点标记或短划线）通过随机游走标签传播生成密集监督，结合图像驱动的血管特征和连续性先验，联合训练CNN分割器。

Result: 在临床数据集上表现优于传统方法，生成更完整的血管图并提供校准的不确定性。

Conclusion: 该方法显著降低标注负担，同时保留临床相关的血管拓扑结构。
...
### [49] [Hybrid Machine Learning Framework for Predicting Geometric Deviations from 3D Surface Metrology](https://arxiv.org/abs/2508.06845)
*Hamidreza Samadi,Md Manjurul Ahsan,Shivakumar Raman*

Main category: cs.CV

TL;DR: 该研究提出了一种结合高分辨率3D扫描和混合机器学习框架的方法，用于预测制造组件的几何偏差，显著提高了预测精度。
...
Result: 预测精度达到0.012 mm（95%置信水平），比传统统计过程控制方法提高了73%，并揭示了制造参数与几何偏差之间的隐藏相关性。

Conclusion: 该方法在自动化质量控制、预测性维护和设计优化方面具有潜力，并为未来预测建模研究提供了数据集基础。
...
### [50] [AGIC: Attention-Guided Image Captioning to Improve Caption Relevance](https://arxiv.org/abs/2508.06853)
...
Result: AGIC在多个评估指标上表现优异，推理速度更快。
...
### [51] [A Joint Sparse Self-Representation Learning Method for Multiview Clustering](https://arxiv.org/abs/2508.06857)
...
<details>
  <summary>Details</summary>
Motivation: 多视图聚类旨在利用不同视图的一致和互补信息进行样本分组，现有方法在提取局部信息和保证收敛性方面存在不足。

Method: 提出基于基数约束的稀疏自表示学习模型，结合低秩约束揭示全局结构，开发交替二次惩罚方法解决非凸非光滑模型的收敛问题。

Result: 在六个标准数据集上的实验表明，所提模型和方法优于八种现有算法。

Conclusion: 所提模型和方法在多视图聚类中表现出优越性能，尤其在提取局部信息和保证收敛性方面。
...
### [52] [VSI: Visual Subtitle Integration for Keyframe Selection to enhance Long Video Understanding](https://arxiv.org/abs/2508.06869)
*Jianxiang He,Shaoguang Wang,Weiyu Guo,Meisheng Hong,Jungang Li,Yijie Xu,Ziyang Chen,Hui Xiong*

Main category: cs.CV

TL;DR: VSI是一种多模态关键帧搜索方法，通过整合字幕、时间戳和场景边界，显著提升了长视频理解和问答任务的准确性。


<details>
  <summary>Details</summary>
Motivation: 长视频理解因数据规模庞大而具有挑战性，现有方法因多模态对齐不足和无法捕捉复杂时间语义信息而效果有限。

Method: 提出VSI方法，通过双流搜索机制（视频搜索流和字幕匹配流）整合视觉和文本信息，提升关键帧搜索精度。

Result: 在LongVideoBench上，VSI的关键帧定位准确率达到40.00%，视频问答任务准确率达到68.48%，显著超越基线方法。
...
### [53] [NS-FPN: Improving Infrared Small Target Detection and Segmentation from Noise Suppression Perspective](https://arxiv.org/abs/2508.06878)
*Maoxun Yuan,Duanni Meng,Ziteng Xi,Tianyi Zhao,Shiji Zhao,Yimian Dai,Xingxing Wei*

Main category: cs.CV

TL;DR: 提出了一种基于频域分析的噪声抑制特征金字塔网络（NS-FPN），通过低频引导特征净化（LFP）模块和螺旋感知特征采样（SFS）模块，显著减少了红外小目标检测中的误报问题。


<details>
  <summary>Details</summary>
Motivation: 红外小目标检测因目标暗淡、形状模糊及背景干扰严重而具有挑战性，现有CNN方法虽提升了特征表示但增加了误报。

Method: NS-FPN整合了LFP模块（通过净化高频成分抑制噪声）和SFS模块（螺旋采样融合目标相关特征），结构轻量且易集成。

Result: 在公开数据集上，NS-FPN显著减少误报并优于现有方法。
...
### [54] [BASIC: Boosting Visual Alignment with Intrinsic Refined Embeddings in Multimodal Large Language Models](https://arxiv.org/abs/2508.06895)
...
### [55] [Advancements in Chinese font generation since deep learning era: A survey](https://arxiv.org/abs/2508.06900)
*Weiran Chen,Guiqian Zhu,Ying Li,Yi Ji,Chunping Liu*

Main category: cs.CV

TL;DR: 本文综述了基于深度学习的汉字字体生成方法，包括研究背景、文献选择方法、相关基础知识、现有方法分类及未来挑战。
...
Method: 通过文献综述，将现有方法分为多参考样本和少参考样本两类，并分析其优缺点。

Result: 总结了代表性方法及其局限性，提出了未来研究方向。

Conclusion: 本文为研究者提供了有价值的参考，并指出了字体生成领域的挑战与未来方向。
...
### [56] [eMotions: A Large-Scale Dataset and Audio-Visual Fusion Network for Emotion Analysis in Short-form Videos](https://arxiv.org/abs/2508.06902)
...
<details>
  <summary>Details</summary>
Motivation: 短视频（SVs）的多模态复杂性为情感分析带来新挑战，现有数据稀缺，需高质量数据集和方法。

Method: 提出eMotions数据集和多阶段标注流程，设计AV-CANet网络，结合视频Transformer和局部-全局融合模块，使用EP-CE Loss优化。

Result: 在多个数据集上验证AV-CANet有效性，提供广泛研究见解，并通过消融实验验证关键组件。

Conclusion: AV-CANet和eMotions数据集为短视频情感分析提供了有效工具和基准，推动未来研究。
...
### [57] [A Simple yet Powerful Instance-Aware Prompting Framework for Training-free Camouflaged Object Segmentation](https://arxiv.org/abs/2508.06904)
*Chao Yin,Jide Li,Xiaoqiang Li*

Main category: cs.CV

TL;DR: 提出了一种无需训练的实例感知提示框架（IAPF），通过多模态大语言模型生成图像特定标签，结合实例级边界框和点提示，显著提升了伪装对象分割的性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于训练的伪装对象分割方法在标注稀疏时性能下降，而训练免费方法通常只能生成粗粒度的语义掩码，无法有效处理多实例场景。

Method: IAPF框架包括文本提示生成器、实例掩码生成器和自一致性投票机制，通过精细的实例级提示生成候选掩码并选择最一致的结果。

Result: 在标准COS基准测试中，IAPF显著优于现有训练免费方法。

Conclusion: IAPF是一种简单但强大的训练免费方法，能够有效生成精细的实例掩码，提升伪装对象分割的性能。
...
### [58] [MultiRef: Controllable Image Generation with Multiple Visual References](https://arxiv.org/abs/2508.06905)
*Ruoxi Chen,Dongping Chen,Siyuan Wu,Sinan Wang,Shiyun Lang,Petr Sushko,Gaoyang Jiang,Yao Wan,Ranjay Krishna*

Main category: cs.CV

TL;DR: 论文提出MultiRef-bench评估框架，用于多视觉参考图像生成任务，并构建数据集MultiRef。实验显示现有模型在多参考条件下表现不佳，为开发更灵活的创意工具提供方向。


<details>
  <summary>Details</summary>
Motivation: 当前图像生成框架主要依赖单源输入，而设计师通常从多视觉参考中获取灵感，因此需要研究多参考图像生成任务。

Method: 提出MultiRef-bench评估框架和数据集MultiRef，包含合成和真实样本，并通过RefBlend数据引擎生成合成样本。

Result: 实验表明，即使是先进模型（如OmniGen）在多参考条件下表现有限，合成样本准确率为66.6%，真实样本为79.0%。

Conclusion: 研究为开发能有效整合多视觉参考的创意工具提供了重要方向，数据集已公开。
...
### [59] [MMReID-Bench: Unleashing the Power of MLLMs for Effective and Versatile Person Re-identification](https://arxiv.org/abs/2508.06908)
*Jinhao Li,Zijian Chen,Lirong Deng,Changbo Wang,Guangtao Zhai*

Main category: cs.CV

TL;DR: 论文提出首个多任务多模态行人重识别基准MMReID-Bench，利用多模态大语言模型（MLLMs）提升跨模态理解能力，实验显示其有效性，但在处理热成像和红外数据时仍有局限。


<details>
  <summary>Details</summary>
Motivation: 传统行人重识别模型在多模态数据上泛化能力差，而现有方法未充分利用MLLMs的推理和跨模态理解能力。

Method: 引入MMReID-Bench基准，包含20,710个多模态查询和库图像，覆盖10种任务，评估MLLMs在行人重识别中的表现。

Result: 实验证明MLLMs在多模态行人重识别中表现优异，但在热成像和红外数据上仍有不足。

Conclusion: MMReID-Bench有望推动开发更鲁棒的多模态基础模型，提升行人重识别性能。
...
### [60] [Talk2Image: A Multi-Agent System for Multi-Turn Image Generation and Editing](https://arxiv.org/abs/2508.06916)
*Shichao Ma,Yunhe Guo,Jiahao Su,Qihe Huang,Zhengyang Zhou,Yang Wang*

Main category: cs.CV

TL;DR: Talk2Image是一个多代理系统，用于多轮对话中的交互式图像生成和编辑，通过意图解析、任务分解和反馈驱动优化，显著提升了可控性、连贯性和用户满意度。
...
Method: Talk2Image整合了三个关键组件：从对话历史中解析意图、通过专业代理分解和执行任务、基于多视角评估机制的反馈驱动优化。
...
### [61] [AR-GRPO: Training Autoregressive Image Generation Models via Reinforcement Learning](https://arxiv.org/abs/2508.06924)
*Shihao Yuan,Yahui Liu,Yang Yue,Jingyuan Zhang,Wangmeng Zuo,Qi Wang,Fuzheng Zhang,Guorui Zhou*

Main category: cs.CV

TL;DR: AR-GRPO是一种将在线强化学习（RL）训练集成到自回归（AR）图像生成模型中的方法，通过精心设计的奖励函数提升生成图像的质量和人类偏好。


<details>
  <summary>Details</summary>
Motivation: 受强化学习在大型语言模型（LLM）中成功的启发，研究如何将其应用于自回归图像生成模型以提升生成质量。

Method: 采用Group Relative Policy Optimization（GRPO）算法，通过多维度奖励函数（如感知质量、真实感和语义保真度）优化自回归模型输出。

Result: 在类条件和文本条件图像生成任务中，RL增强框架显著提升了图像质量和人类偏好，各项评估指标均有改善。
...
### [62] [CannyEdit: Selective Canny Control and Dual-Prompt Guidance for Training-Free Image Editing](https://arxiv.org/abs/2508.06937)
*Weiyan Xie,Han Gao,Didan Deng,Kaican Li,April Hua Liu,Yongxiang Huang,Nevin L. Zhang*

Main category: cs.CV

TL;DR: CannyEdit是一种无需训练的区域图像编辑框架，通过选择性Canny控制和双提示引导，平衡文本遵从性和上下文保真度。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以在编辑区域中平衡文本遵从性、未编辑区域的上下文保真度以及编辑的无缝集成。

Method: 采用选择性Canny控制（保留未编辑区域的细节）和双提示引导（结合局部和全局提示）。

Result: 在真实图像编辑任务中，CannyEdit在文本遵从性和上下文保真度的平衡上优于现有方法，用户研究显示其编辑结果更不易被识别为AI生成。

Conclusion: CannyEdit通过创新方法解决了区域图像编辑的挑战，显著提升了编辑质量和自然度。
...
### [63] [SLRTP2025 Sign Language Production Challenge: Methodology, Results, and Future Work](https://arxiv.org/abs/2508.06951)
...
<details>
  <summary>Details</summary>
Motivation: 由于缺乏标准化的评估指标，手语生成领域的系统难以进行有效比较，因此设计了这一挑战赛以推动领域发展。

Method: 挑战赛使用RWTH-PHOENIX-Weather-2014T数据集，评估从文本到姿势序列（T2P）的翻译架构，并引入标准化评估网络。

Result: 33名参与者提交了231个解决方案，最佳团队的BLEU-1得分为31.40，DTW-MJE为0.0574。

Conclusion: 挑战赛为手语生成领域提供了标准化评估基准，未来研究可在此基础上进行比较。
...
### [64] [Beyond Frequency: Seeing Subtle Cues Through the Lens of Spatial Decomposition for Fine-Grained Visual Classification](https://arxiv.org/abs/2508.06959)
...
<details>
  <summary>Details</summary>
Motivation: 解决细粒度视觉分类（FGVC）中固定频率域方法缺乏适应性和动态调整能力的问题。
...
Conclusion: SCOPE通过自适应多尺度融合，显著提升了细粒度分类的灵活性和性能。
...
### [65] [Adversarial Video Promotion Against Text-to-Video Retrieval](https://arxiv.org/abs/2508.06964)
*Qiwei Tian,Chenhao Lin,Zhengyu Zhao,Qian Li,Shuai Liu,Chao Shen*

Main category: cs.CV

TL;DR: 本文提出了首个针对文本到视频检索（T2VR）的攻击方法ViPro，旨在通过对抗性手段提升视频排名，并提出了Modal Refinement（MoRe）方法以增强黑盒迁移性。实验表明ViPro在多种场景下显著优于基线方法。
...
Method: 提出ViPro攻击方法，通过对抗性手段提升视频排名；进一步提出MoRe方法，捕捉视觉与文本模态的细粒度交互以增强黑盒迁移性。

Result: ViPro在白盒、灰盒和黑盒设置下分别平均超过基线方法30%、10%和4%。实验覆盖多种场景和数据集。

Conclusion: ViPro揭示了T2VR中一个被忽视的漏洞，提供了攻击的上下界分析，并为潜在防御措施提供了见解。
...
### [66] [Evaluating Fisheye-Compatible 3D Gaussian Splatting Methods on Real Images Beyond 180 Degree Field of View](https://arxiv.org/abs/2508.06968)
*Ulas Gunes,Matias Turkulainen,Juho Kannala,Esa Rahtu*

Main category: cs.CV

TL;DR: 首次评估了基于鱼眼镜头的3D高斯泼溅方法Fisheye-GS和3DGUT在超过180度视场的真实图像上的表现，分析了极端失真下的处理能力，并提出了一种基于深度的初始化策略。


<details>
  <summary>Details</summary>
Motivation: 研究鱼眼镜头在极端失真条件下的3D重建效果，解决传统SfM初始化在高失真场景中的失败问题。

Method: 通过不同视场（200度、160度、120度）评估Fisheye-GS和3DGUT的性能，并提出基于UniK3D预测的深度初始化策略。

Result: Fisheye-GS在160度视场表现最佳，3DGUT在200度视场保持稳定；UniK3D策略在复杂场景下表现优异。

Conclusion: 鱼眼3DGS方法在稀疏且高失真图像输入下具有实际可行性。
...
### [67] [WeatherDiffusion: Weather-Guided Diffusion Model for Forward and Inverse Rendering](https://arxiv.org/abs/2508.06982)
*Yixin Zhu,Zuoliang Zhu,Miloš Hašan,Jian Yang,Jin Xie,Beibei Wang*

Main category: cs.CV

TL;DR: WeatherDiffusion是一个基于扩散模型的框架，用于自动驾驶场景中的正向和逆向渲染，支持多种天气和光照条件下的可控编辑。
...
Method: 提出WeatherDiffusion框架，利用文本引导的预测本征图实现高质量逆向渲染，并引入本征图感知注意力机制（MAA）。

Result: WeatherDiffusion在多个基准测试中优于现有方法，并在下游任务（如目标检测和图像分割）中表现出显著优势。

Conclusion: WeatherDiffusion为自动驾驶场景的正向和逆向渲染提供了高效且可控的解决方案，增强了在复杂天气条件下的鲁棒性。
...
### [68] [TADoc: Robust Time-Aware Document Image Dewarping](https://arxiv.org/abs/2508.06988)
*Fangmin Zhao,Weichao Zeng,Zhenhang Li,Dongbao Yang,Yu Zhou*

Main category: cs.CV

TL;DR: 论文提出了一种动态建模的文档图像去扭曲方法TADoc，并引入新评价指标DLS，实验证明其优于现有方法。
...
Method: 将任务重新建模为动态过程，设计轻量级框架TADoc，并提出新评价指标DLS。
...
### [69] [OctreeNCA: Single-Pass 184 MP Segmentation on Consumer Hardware](https://arxiv.org/abs/2508.06993)
*Nick Lemke,John Kalkhof,Niklas Babendererde,Anirban Mukhopadhyay*

Main category: cs.CV

TL;DR: 论文提出OctreeNCA，通过八叉树数据结构扩展NCA的邻域定义，解决高分辨率图像和视频分割中的VRAM瓶颈问题，显著减少资源消耗并提升速度。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割（如MRI、病理切片或手术视频）需要全局上下文，但现有模型（如UNet或Vision Transformers）因VRAM限制需分块处理，影响全局一致性和速度。NCA虽轻量但缺乏全局知识。

Method: 提出OctreeNCA，利用八叉树数据结构扩展NCA的邻域定义，实现全局知识高效传递，并开发CUDA实现的NCA推理函数以优化VRAM和速度。

Result: OctreeNCA在高分辨率图像和视频分割中VRAM占用比UNet减少90%，可一次性处理184兆像素病理切片或1分钟手术视频。

Conclusion: OctreeNCA通过改进NCA的邻域定义和优化实现，显著提升了高分辨率医学图像分割的效率和全局一致性。
...
### [70] [S2-UniSeg: Fast Universal Agglomerative Pooling for Scalable Segment Anything without Supervision](https://arxiv.org/abs/2508.06995)
...
Conclusion: S2-UniSeg在效率和性能上均有显著提升，适用于大规模数据集。
...
### [71] [HiMat: DiT-based Ultra-High Resolution SVBRDF Generation](https://arxiv.org/abs/2508.07011)
...
Method: 引入HiMat框架和CrossStitch模块，通过局部操作捕获多图依赖关系，保持DiT主干不变。

Result: 实验证明HiMat能生成结构一致且高频细节丰富的4K SVBRDF，并可推广至其他任务如本征分解。
...
TL;DR: TerraMAE是一种专为高光谱图像（HSI）设计的新型自监督编码框架，通过自适应通道分组和增强的重建损失函数，显著提升了空间-光谱信息的表示能力，并在多个地理空间任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 高光谱卫星图像包含丰富的光谱信息，但现有的自监督方法（如Masked Autoencoders）难以有效利用200+波段的高光谱数据中的复杂空间-光谱相关性。

Method: TerraMAE采用自适应通道分组策略（基于反射率统计特性）和增强的重建损失函数（结合空间和光谱质量指标），以学习更具代表性的空间-光谱嵌入。

Result: TerraMAE在高保真图像重建中表现出卓越的空间-光谱信息保留能力，并在作物识别、土地覆盖分类和土壤质地预测等下游任务中取得优异性能。

Conclusion: TerraMAE为高光谱图像提供了一种高效的自监督学习框架，显著提升了地理空间分析的性能。
...
TL;DR: DocRefine是一种创新的多智能体框架，用于科学PDF文档的智能理解、内容优化和自动摘要，通过自然语言指令驱动，显著提升了语义一致性和视觉保真度。
...
Method: DocRefine采用六种协作智能体的多智能体系统，结合先进的大模型（如GPT-4o），实现文档的布局分析、内容理解、指令分解、内容优化、摘要生成和一致性验证。

Result: 在DocEditBench数据集上，DocRefine在语义一致性（86.7%）、布局保真度（93.9%）和指令遵循率（85.0%）上均优于现有方法。
...
TL;DR: FOCUS-Med提出了一种结合空间和结构图的注意力感知内窥镜息肉分割方法，通过双图卷积网络和自注意力机制提升分割精度，并在公开基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 内窥镜图像中息肉分割对早期结直肠癌检测至关重要，但存在低对比度、高光和边界模糊等挑战。
...
Conclusion: FOCUS-Med展示了AI辅助结肠镜检查的有效性和临床潜力。
...
Method: TeSO将3D场景表示为八叉树组织的立方体边界曲面元，每个曲面元关联一个纹理贴图，并通过压缩方案高效编码几何和纹理。
...
### [77] [ForeSight: Multi-View Streaming Joint Object Detection and Trajectory Forecasting](https://arxiv.org/abs/2508.07089)
*Sandro Papais,Letian Wang,Brian Cheong,Steven L. Waslander*

Main category: cs.CV

TL;DR: ForeSight是一个用于自动驾驶车辆3D感知的联合检测与预测框架，通过多任务流式双向学习实现信息共享，无需显式对象关联，性能优于现有方法。
...
Method: 采用多任务流式双向学习框架，结合检测与预测查询内存，通过预测感知检测变换器和流式预测变换器提升时空一致性。

Result: 在nuScenes数据集上，EPA达54.9%，优于之前方法9.3%，同时在mAP和minADE上表现最佳。

Conclusion: ForeSight通过联合学习和无跟踪设计，显著提升了3D感知性能，为自动驾驶提供了高效解决方案。
...
TL;DR: 提出了一种名为HyComm的混合协作3D检测系统，通过自适应整合紧凑的感知输出和丰富的原始观测信息，优化了检测性能与通信带宽的权衡。
...
Result: HyComm在DAIR-V2X和OPV2V数据集上表现优异，通信量减少2006倍以上，性能优于Where2comm。

Conclusion: HyComm通过自适应消息选择和标准化数据格式，实现了高效的协作3D检测，适用于不同代理配置。
...
TL;DR: AugLift通过稀疏增强2D关键点输入（加入置信度和深度估计），显著提升3D人体姿态估计的泛化性能，无需额外数据或传感器。


<details>
  <summary>Details</summary>
Motivation: 现有基于提升的3D姿态估计方法在新数据集和真实场景中泛化能力不足。

Method: AugLift在标准2D关键点坐标基础上，稀疏地加入关键点检测置信度和深度估计，利用预训练模型计算这些信号。

Result: 在四个数据集上的实验表明，AugLift平均提升跨数据集性能10.1%，同时提升分布内性能4.0%。
...
TL;DR: 生成图像模型在自然和医学影像中取得显著进展，但合成图像的保真度和临床效用仍存疑。本研究评估了GANs和DMs在合成胸片中的表现，发现DMs整体更真实，而GANs在某些条件下更准确。
...
Result: DMs生成的图像整体更真实，但GANs在特定条件（如ECS缺失）下更准确。研究还揭示了放射科医生检测合成图像的视觉线索。
...
TL;DR: 提出了一种名为CMAMRNet的网络，通过全面的掩码引导和多尺度特征提取，解决了壁画数字修复中掩码一致性和修复质量的问题。


<details>
  <summary>Details</summary>
Motivation: 壁画作为文化遗产面临环境与人为破坏，现有学习方法在掩码引导和修复质量上存在不足。

Method: CMAMRNet包含Mask-Aware Up/Down-Sampler (MAUDS)和Co-Feature Aggregator (CFA)，分别实现跨分辨率掩码敏感性和多尺度特征提取。

Result: 在基准数据集上表现优于现有方法，有效保留了壁画的结构完整性和艺术细节。

Conclusion: CMAMRNet为壁画数字修复提供了高效解决方案，代码已开源。
...
TL;DR: 提出了一种名为DPAL的新型蒸馏预训练框架，通过动态模式解码器和多级对齐目标，高效训练轻量级人本视觉模型，使其从大型模型中学习泛化能力。


<details>
  <summary>Details</summary>
Motivation: 解决大型人本视觉模型依赖大规模预训练数据和复杂架构的局限性，提升轻量级模型的实用性。

Method: 设计动态模式解码器（D-PaDe）作为动态专家混合模型，并结合全局、局部和实例关系三级对齐目标。

Result: 在15个数据集上验证了DPAL的有效性，轻量级模型（5M参数）表现接近大型模型（84M/307M），并显著优于其他蒸馏方法。

Conclusion: DPAL成功实现了轻量级模型的高效训练和泛化能力，适用于多样化的人本视觉任务。
...
TL;DR: 提出了一种基于扩散模型的行人轨迹预测框架，结合短期和长期意图建模，提高了预测准确性。


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散模型的行人轨迹预测方法缺乏对行人意图的显式语义建模，可能导致行为误解和预测精度下降。

Method: 通过残差极坐标表示建模短期意图，利用可学习的基于令牌的终点预测器估计长期意图，并结合自适应指导和残差噪声预测器优化扩散过程。
...
TL;DR: 提出了一种名为SketchAnimator的模型，通过三个阶段（外观学习、运动学习和视频先验蒸馏）为静态草图添加动态效果，使其能够模仿参考视频的运动。


<details>
  <summary>Details</summary>
Motivation: 草图动画化是一个耗时且需要专业技能的过程，对业余用户来说具有挑战性。本文旨在简化这一过程，使非专业人士也能轻松为草图添加动态效果。

Method: 模型分为三个阶段：1）外观学习，利用LoRA将草图外观信息整合到预训练的T2V模型中；2）运动学习，从参考视频中提取运动动态；3）视频先验蒸馏，使用SDS更新贝塞尔曲线参数以生成动态草图视频。

Result: 模型生成的草图视频既保留了原始草图的外观，又模仿了参考视频的动态运动，且在一键运动定制任务中表现优于其他方法。

Conclusion: SketchAnimator为草图动画化提供了一种高效且用户友好的解决方案，尤其适合非专业人士使用。
...
TL;DR: 提出了一种名为CoopDiff的解耦扩散框架，通过分离建模人和物体的运动，利用接触点作为共享锚点，提升3D人-物交互预测的准确性。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常忽略人和物体运动模式的差异，导致预测效果不佳。

Method: 使用两个独立分支分别建模人和物体的运动，并通过接触点一致性约束实现协同预测。

Result: 在BEHAVE和Human-object Interaction数据集上表现优于现有方法。

Conclusion: CoopDiff通过解耦建模和接触点一致性约束，显著提升了人-物交互预测的准确性。
...
### [86] [Lightweight Multi-Scale Feature Extraction with Fully Connected LMF Layer for Salient Object Detection](https://arxiv.org/abs/2508.07170)
*Yunpeng Shi,Lei Chen,Xiaolu Shen,Yanju Guo*

Main category: cs.CV

TL;DR: 本文提出了一种轻量级多尺度特征提取层（LMF层）和基于此的LMFNet网络，用于显著目标检测，显著减少了参数量同时保持高性能。


<details>
  <summary>Details</summary>
Motivation: 在轻量级网络中实现多尺度特征提取仍具挑战性，需平衡效率与性能。

Method: 采用深度可分离空洞卷积构建LMF层，并集成多个LMF层形成LMFNet。

Result: LMFNet在五个基准数据集上达到或接近最优性能，仅需0.81M参数。

Conclusion: LMFNet解决了轻量级网络中的多尺度学习问题，展示了在图像处理任务中的广泛应用潜力。
...
### [87] [EventRR: Event Referential Reasoning for Referring Video Object Segmentation](https://arxiv.org/abs/2508.07171)
*Huihui Xu,Jiashi Lin,Haoyu Chen,Junjun He,Lei Zhu*

Main category: cs.CV

TL;DR: 论文提出EventRR框架，通过解耦视频对象分割为对象总结和引用推理两部分，利用Referential Event Graph（REG）和Temporal Concept-Role Reasoning（TCRR）提升性能。
...
Method: EventRR框架分为对象总结和引用推理两部分：总结阶段通过瓶颈令牌聚合全局跨模态时间上下文；推理阶段构建REG图，通过TCRR从叶子节点到根节点累积引用分数。

Result: 在四个基准数据集上，EventRR在定量和定性上均优于现有RVOS方法。

Conclusion: EventRR通过结构化语义推理显著提升了视频对象分割的性能，代码已开源。
...
### [88] [OpenHAIV: A Framework Towards Practical Open-World Learning](https://arxiv.org/abs/2508.07270)
...
Conclusion: DGN通过深度引导显著提升了图像修复质量，新数据集进一步支持了方法的有效性和鲁棒性。
...
TL;DR: 提出了一种统一的特征可视化框架，适用于CNN和ViT，扩展了中间层的可视化，并研究了激活最大化在生成对抗样本中的应用。
...
Method: 采用激活最大化（AM）方法，扩展特征可视化至中间层，并探索其在生成对抗样本中的应用。

Result: 实验证明该方法在CNN和ViT中均有效，具有普适性和解释价值。

Conclusion: 该框架为DNN的特征表示和决策边界提供了更深入的理解，揭示了潜在的脆弱性。
...
### [91] [Unsupervised Real-World Super-Resolution via Rectified Flow Degradation Modelling](https://arxiv.org/abs/2508.07214)
*Hongyang Zhou,Xiaobin Zhu,Liuling Chen,Junyi He,Jingyan Qin,Xu-Cheng Yin,Zhang xiaoxing*

Main category: cs.CV

TL;DR: 提出了一种基于修正流的无监督真实世界超分辨率方法，通过生成具有真实退化的LR-HR训练对，显著提升了现有超分辨率方法在真实场景中的性能。


<details>
  <summary>Details</summary>
Motivation: 真实世界超分辨率面临复杂未知的退化分布问题，现有方法因合成数据与真实数据的域差距难以泛化。

Method: 提出了修正流退化模块（RFDM）和傅里叶先验引导退化模块（FGDM），分别通过连续可逆的退化轨迹建模和傅里叶相位信息来生成更真实的LR图像。

Result: 在真实世界数据集上的实验表明，该方法显著提升了现有超分辨率方法的性能。

Conclusion: 通过生成具有真实退化的训练对，该方法有效解决了真实世界超分辨率的挑战。
...
### [92] [DragonFruitQualityNet: A Lightweight Convolutional Neural Network for Real-Time Dragon Fruit Quality Inspection on Mobile Devices](https://arxiv.org/abs/2508.07306)
*Md Zahurul Haquea,Yeahyea Sarker,Muhammed Farhan Sadique Mahi,Syed Jubayer Jaman,Md Robiul Islam*

Main category: cs.CV

TL;DR: 该研究提出了一种轻量级CNN模型DragonFruitQualityNet，用于火龙果实时质量检测，准确率达93.98%，并嵌入移动应用以支持农民使用。
...
Method: 构建包含13,789张图像的数据集，训练轻量级CNN模型，并开发移动应用实现实时检测。
...
### [93] [Bridging Semantic Logic Gaps: A Cognition-Inspired Multimodal Boundary-Preserving Network for Image Manipulation Localization](https://arxiv.org/abs/2508.07216)
*Songlin Li,Zhiqing Guo,Yuanman Li,Zeyu Li,Yunfeng Diao,Gaobo Yang,Liejun Wang*

Main category: cs.CV

TL;DR: 提出了一种基于认知启发的多模态边界保持网络（CMB-Net），通过大语言模型分析图像篡改区域并生成文本信息，结合图像-文本交互模块和恢复边缘解码器，显著提升了图像篡改定位性能。


<details>
  <summary>Details</summary>
Motivation: 现有图像篡改定位模型主要依赖视觉线索，忽略了内容特征的语义逻辑关系。篡改技术破坏了内容特征的内部关系，但留下了语义线索，这为定位提供了机会。

Method: CMB-Net利用大语言模型生成文本信息补充视觉语义缺失，设计了图像-文本中心模糊模块（ITCAM）量化文本与图像特征的模糊性，图像-文本交互模块（ITIM）对齐视觉与文本特征，以及恢复边缘解码器（RED）保留边界信息。

Result: 实验表明，CMB-Net在图像篡改定位任务中优于大多数现有模型。

Conclusion: CMB-Net通过结合语义分析和多模态交互，显著提升了篡改定位的准确性，为未来研究提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 多模态持续学习任务（如视觉和语言）需要模型不仅避免灾难性遗忘，还需处理跨模态交互的挑战，现有方法对此关注不足。

Method: 开发了MCITlib代码库，实现了8种代表性算法，并在2个基准上进行了系统评估。

Result: MCITlib为多模态持续学习研究提供了工具支持，并将持续更新以反映领域进展。

Conclusion: MCITlib的发布将推动多模态持续学习领域的研究，代码库已开源。
...
### [95] [Generic Calibration: Pose Ambiguity/Linear Solution and Parametric-hybrid Pipeline](https://arxiv.org/abs/2508.07217)
*Yuqi Han,Qi Cai,Yuanxin Wu*

Main category: cs.CV

TL;DR: 论文提出了一种混合标定方法，结合通用和参数化模型，解决了通用标定中的姿态模糊问题，并提升了标定精度。


<details>
  <summary>Details</summary>
Motivation: 离线相机标定方法中，参数化模型依赖用户经验，通用模型复杂且无法提供传统内参，且存在姿态模糊问题。

Method: 提出线性求解器和非线性优化解决姿态模糊，引入全局优化的混合标定方法结合通用和参数化模型。

Result: 仿真和实验表明，混合方法在不同镜头类型和噪声下表现优异。
...
### [96] [Freeze and Reveal: Exposing Modality Bias in Vision-Language Models](https://arxiv.org/abs/2508.07432)
...
TL;DR: 论文分析了视觉语言模型中的性别偏见来源，并提出两种去偏方法（CDA和DAUDoS），实验表明视觉编码器（CLIP）和文本编码器（PaliGemma2）分别是主要偏见来源。
...
Method: 使用反事实数据增强（CDA）和基于新指标（Degree of Stereotypicality）的DAUDoS方法进行去偏。

Result: CDA减少性别差距6%，DAUDoS减少3%且仅需三分之一数据；两种方法均提升性别识别准确率3%。

Conclusion: 明确了视觉和文本编码器的偏见主导性，为未来多模态系统的针对性去偏提供了方向。
...
### [97] [Landmark Guided Visual Feature Extractor for Visual Speech Recognition with Limited Resource](https://arxiv.org/abs/2508.07233)
*Lei Yang,Junshan Jin,Mingyuan Zhang,Yi He,Bofan Chen,Shilin Wang*

Main category: cs.CV

TL;DR: 提出了一种基于面部标志的视觉特征提取器，通过多图卷积网络和多级唇动态融合框架，提升有限数据下的视觉语音识别性能。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法易受视觉干扰（如光照、皮肤纹理）影响，且需要大量数据和计算资源。本文旨在减少用户特定特征的影响，提升小数据下的性能。

Method: 使用面部标志作为辅助信息，设计时空多图卷积网络提取标志特征，并引入多级唇动态融合框架结合原始视频帧的视觉特征。

Result: 实验表明，该方法在有限数据下表现良好，并提高了对未见说话者的识别准确率。

Conclusion: 提出的方法有效减少了视觉干扰的影响，提升了小数据场景下的视觉语音识别性能。
...
### [98] [From Field to Drone: Domain Drift Tolerant Automated Multi-Species and Damage Plant Semantic Segmentation for Herbicide Trials](https://arxiv.org/abs/2508.07514)
...
TL;DR: 论文提出了一种改进的分割模型，结合自监督视觉模型和植物分类学的层次推理，显著提升了物种识别和损害分类的准确性，并在跨设备和跨地域测试中表现出鲁棒性。
...
Method: 结合通用自监督视觉模型和植物分类学的层次推理，使用多国多年份的数据集进行训练和测试。

Result: 模型在物种识别（F1-score: 0.52到0.85）和损害分类（F1-score: 0.28到0.44）上显著优于先前方法，并在跨设备测试中保持较强性能。
...
### [99] [ASM-UNet: Adaptive Scan Mamba Integrating Group Commonalities and Individual Variations for Fine-Grained Segmentation](https://arxiv.org/abs/2508.07237)
...
<details>
  <summary>Details</summary>
Motivation: 现有粗粒度分割方法在细粒度分割中表现不佳，且固定扫描顺序限制了适应性。

Method: 提出ASM-UNet，结合群体共性和个体差异生成自适应扫描分数。

Result: 在ACDC、Synapse和BTMS数据集上表现优异。

Conclusion: ASM-UNet在细粒度和粗粒度分割任务中均表现出色。
...
### [100] [A DICOM Image De-identification Algorithm in the MIDI-B Challenge](https://arxiv.org/abs/2508.07538)
*Hongzhu Jiang,Sihan Xie,Zhiyu Wan*

Main category: cs.CV

TL;DR: 论文介绍了医学图像去标识化的重要性，提出了MIDI-B挑战赛的背景和方法，并展示了其算法的优异表现。


<details>
  <summary>Details</summary>
Motivation: 医学图像共享需符合隐私法规（如HIPAA、DICOM标准），去标识化是保护患者隐私的关键。

Method: 采用像素掩码、日期偏移、哈希、文本识别与替换等方法处理DICOM图像。

Result: 算法在MIDI-B挑战赛中正确执行99.92%的任务，排名第二。

Conclusion: 当前方法存在局限，未来需进一步优化以提升去标识化效果。
...
### [101] [Consistent and Controllable Image Animation with Motion Linear Diffusion Transformers](https://arxiv.org/abs/2508.07246)
...
Conclusion: MiraMo为图像动画提供了高效、高质量的解决方案，并适用于多种任务。
...
### [102] [ShoulderShot: Generating Over-the-Shoulder Dialogue Videos](https://arxiv.org/abs/2508.07597)
...
<details>
  <summary>Details</summary>
Motivation: 对话视频在影视作品中至关重要，但现有研究对其生成技术探索不足，面临角色一致性、空间连续性和计算预算限制等挑战。
...
### [103] [SUIT: Spatial-Spectral Union-Intersection Interaction Network for Hyperspectral Object Tracking](https://arxiv.org/abs/2508.07250)
*Fengchao Xiong,Zhenxing Wu,Sen Jia,Yuntao Qian*

Main category: cs.CV

TL;DR: 论文提出了一种新的高光谱视频跟踪方法，通过Transformer和集合论的包含-排除原理建模光谱交互，并结合光谱损失提升性能。
...
Method: 使用Transformer建立模板与搜索区域的频带间长程空间关系，利用包含-排除原理建模光谱交互，并引入光谱损失优化训练。

Result: 实验表明，该方法在跟踪性能上达到最优。

Conclusion: 通过光谱交互的建模和优化，显著提升了高光谱视频跟踪的鲁棒性和准确性。
...
### [104] [SOFA: Deep Learning Framework for Simulating and Optimizing Atrial Fibrillation Ablation](https://arxiv.org/abs/2508.07621)
*Yunsung Chung,Chanho Lim,Ghassan Bidaoui,Christian Massad,Nassir Marrouche,Jihun Hamm*

Main category: cs.CV

TL;DR: SOFA是一种深度学习框架，通过模拟消融效果、预测房颤复发风险并优化消融参数，个性化治疗房颤。


<details>
  <summary>Details</summary>
Motivation: 房颤消融手术效果差异大，难以评估和优化。研究旨在预测复发风险并优化消融策略。

Method: SOFA利用多模态多视图生成器，模拟消融后疤痕形成图像，预测复发风险，并通过优化消融参数降低风险。

Result: SOFA能准确合成消融后图像，优化方案使模型预测的复发风险降低22.18%。

Conclusion: SOFA首次整合模拟、预测和优化，为个性化房颤消融提供了新工具。
...
### [105] [Understanding Dynamic Scenes in Ego Centric 4D Point Clouds](https://arxiv.org/abs/2508.07251)
*Junsheng Huang,Shengyu Hao,Bocheng Hu,Gaoang Wang*

Main category: cs.CV

TL;DR: 论文提出了EgoDynamic4D，一个用于动态4D场景理解的问答基准，包含丰富的标注和任务驱动的评估协议，并提出了一个端到端的时空推理框架。
...
Method: 提出了EgoDynamic4D基准，包含RGB-D视频、相机位姿、实例掩码和4D边界框，设计了12个动态QA任务。提出了一个端到端的时空推理框架，结合动态和静态场景信息。

Result: 实验表明，提出的方法在EgoDynamic4D上优于基线，验证了多模态时间建模的有效性。
...
### [106] [TAR-TVG: Enhancing VLMs with Timestamp Anchor-Constrained Reasoning for Temporal Video Grounding](https://arxiv.org/abs/2508.07683)
...
Result: 实验表明，该模型在性能上达到最优，同时生成可解释、可验证的推理链和逐步优化的时间估计。

Conclusion: TAR-TVG通过显式监督和逐步优化的推理过程，显著提升了视频片段定位的准确性和可解释性。
...
### [107] [Small-Large Collaboration: Training-efficient Concept Personalization for Large VLM using a Meta Personalized Small VLM](https://arxiv.org/abs/2508.07260)
*Sihan Yang,Huitong Ji,Shaolin Lu,Jiayi Chen,Binxiao Xu,Ming Lu,Yuanxing Zhang,Wenhui Dong,Wentao Zhang*

Main category: cs.CV

TL;DR: 提出了一种名为Small-Large Collaboration (SLC)的协作框架，通过小型VLM生成个性化信息，大型VLM整合信息以实现高效个性化应用。


<details>
  <summary>Details</summary>
Motivation: 大型VLM训练成本高且访问受限，小型VLM虽易个性化但推理能力不足，需一种平衡方案。

Method: SLC框架中，小型VLM负责生成个性化信息，大型VLM整合信息并避免幻觉问题，采用测试时反射策略。

Result: 实验证明SLC框架在多种基准测试和大型VLM中有效，支持开源和闭源模型。
...
### [108] [DoorDet: Semi-Automated Multi-Class Door Detection Dataset via Object Detection and Large Language Models](https://arxiv.org/abs/2508.07714)
*Licheng Zhang,Bach Le,Naveed Akhtar,Tuan Ngo*

Main category: cs.CV

TL;DR: 提出了一种半自动化流程，结合目标检测模型和大型语言模型（LLM），以低成本构建多类别门检测数据集。


<details>
  <summary>Details</summary>
Motivation: 现有公开数据集稀缺，需要高效方法构建细粒度多类别门检测数据集，用于建筑合规检查和室内场景理解。

Method: 使用深度学习目标检测模型统一检测门，再通过LLM分类，最后人工验证确保数据质量。

Result: 显著降低了标注成本，生成了适用于神经网络模型在平面图分析中基准测试的数据集。
...
### [109] [UniSVG: A Unified Dataset for Vector Graphic Understanding and Generation with Multimodal Large Language Models](https://arxiv.org/abs/2508.07766)
*Jinke Li,Jiarui Yu,Chenxing Wei,Hande Dong,Qiang Lin,Liangjing Yang,Zhicai Wang,Yanbin Hao*

Main category: cs.CV

TL;DR: 论文提出UniSVG数据集，用于训练多模态大语言模型（MLLMs）处理SVG的理解与生成任务，提升性能并超越GPT-4V。


<details>
  <summary>Details</summary>
Motivation: AI在SVG理解与生成（U&G）方面面临高精度和多模态处理的挑战，需要统一的数据集和模型支持。

Method: 构建包含525k数据项的UniSVG数据集，用于MLLMs训练和评估，支持文本提示和视觉参考的SVG生成与理解。

Result: UniSVG显著提升开源MLLMs在SVG U&G任务中的性能，优于GPT-4V等闭源模型。

Conclusion: UniSVG为SVG领域提供了首个综合性数据集，展示了MLLMs在SVG任务中的潜力。
...
### [110] [Architectural Co-Design for Zero-Shot Anomaly Detection: Decoupling Representation and Dynamically Fusing Features in CLIP](https://arxiv.org/abs/2508.07819)
*Ke Ma,Jun Long,Hongxiao Fei,Liujie Hua,Yueyi Luo*

Main category: cs.CV

TL;DR: 提出一种架构协同设计框架，通过卷积低秩适配器和动态融合网关改进视觉语言模型在零样本异常检测中的表现。
...
Method: 采用卷积低秩适配器（Conv-LoRA）注入局部归纳偏置，并引入动态融合网关（DFG）自适应调整文本提示，实现双向融合。
...
### [111] [SynMatch: Rethinking Consistency in Medical Image Segmentation with Sparse Annotations](https://arxiv.org/abs/2508.07298)
...
<details>
  <summary>Details</summary>
Motivation: 解决医学图像分割中标签稀缺问题，避免伪标签与未标记图像不一致带来的性能限制。

Method: 提出SynMatch框架，通过提取纹理和形状特征合成图像，生成一致的图像-伪标签对，无需额外训练参数。

Result: 在多种医学图像分割任务中表现优异，尤其在BSL设置下，显著优于现有方法。

Conclusion: SynMatch通过合成图像匹配伪标签，有效提升了半监督、弱监督和极弱监督学习下的分割性能。
...
### [112] [Deep Space Weather Model: Long-Range Solar Flare Prediction from Multi-Wavelength Images](https://arxiv.org/abs/2508.07847)
*Shunya Nagashima,Komei Sugiura*

Main category: cs.CV

TL;DR: Deep SWM模型通过多深度状态空间模型和稀疏掩码自编码器，解决了太阳耀斑预测中的长时空依赖问题，并在FlareBench基准上表现优于基线方法和人类专家。
...
Result: 在FlareBench基准上，Deep SWM在性能和可靠性上优于基线方法和人类专家。
...
### [113] [BEVANet: Bilateral Efficient Visual Attention Network for Real-Time Semantic Segmentation](https://arxiv.org/abs/2508.07300)
*Ping-Mao Huang,I-Tien Chao,Ping-Chia Huang,Jia-Wei Liao,Yung-Yu Chuang*

Main category: cs.CV

TL;DR: BEVANet提出了一种高效的双边视觉注意力网络，通过大核注意力机制和动态选择机制，实现了实时语义分割的高性能。
...
Method: 采用大核注意力（LKA）、稀疏分解大可分核注意力（SDLSKA）、综合核选择（CKS）机制和深度大核金字塔池化模块（DLKPPM），结合双边架构和边界引导自适应融合（BGAF）。

Result: BEVANet在Cityscapes数据集上达到81.0% mIoU（预训练后），实时运行速度为33 FPS。

Conclusion: BEVANet通过创新的注意力机制和动态适应策略，实现了实时语义分割的先进性能。
...
### [114] [Selective Contrastive Learning for Weakly Supervised Affordance Grounding](https://arxiv.org/abs/2508.07877)
*WonJun Moon,Hyun Seok Seong,Jae-Pil Heo*

Main category: cs.CV

TL;DR: 提出了一种选择性原型和像素对比目标的方法，通过多视角学习来提升弱监督功能接地（WSAG）的性能，解决了传统方法依赖分类而忽视功能相关部分的问题。


<details>
  <summary>Details</summary>
Motivation: 人类能够从第三人称演示中直观地理解功能部分，而无需像素级标注。现有模型因依赖分类而容易关注与功能无关的类特定模式，因此需要一种更有效的方法来学习功能相关线索。

Method: 利用CLIP在多视角图像中找到动作相关对象，并通过交叉参考互补视角的对象，挖掘精确的部分级功能线索。引入选择性原型和像素对比目标，自适应地学习功能和对象级别的线索。

Result: 实验结果表明，该方法能够有效地将激活从无关区域转移到有意义的功能线索上，提升了功能接地的准确性。

Conclusion: 通过多视角学习和对比目标，该方法显著提升了弱监督功能接地的性能，代码已开源。
...
### [115] [NeeCo: Image Synthesis of Novel Instrument States Based on Dynamic and Deformable 3D Gaussian Reconstruction](https://arxiv.org/abs/2508.07897)
*Tianle Zeng,Junlei Hu,Gerardo Loza Galindo,Sharib Ali,Duygu Sarikaya,Pietro Valdastri,Dominic Jones*

Main category: cs.CV

TL;DR: 论文提出了一种动态高斯点渲染技术，用于解决手术图像数据集稀缺问题，生成高质量合成图像，提升模型性能。
...
Method: 提出动态高斯模型表示手术场景，结合动态训练调整策略和自动标注方法，生成合成数据。

Result: 生成的照片级合成图像在PSNR上表现最佳（29.87），模型性能提升15%。

Conclusion: 动态高斯点渲染技术有效解决了数据稀缺问题，显著提升了手术自动化模型的性能。
...
### [116] [MobileViCLIP: An Efficient Video-Text Model for Mobile Devices](https://arxiv.org/abs/2508.07312)
*Min Yang,Zihan Jia,Zhilin Dai,Sheng Guo,Limin Wang*

Main category: cs.CV

TL;DR: MobileViCLIP是一种高效的视频文本模型，通过引入时间结构重参数化技术，在移动设备上实现快速推理和强大零样本能力。
...
Result: MobileViCLIP-Small在移动设备上的推理速度显著优于InternVideo2，零样本检索性能接近或优于InternVideo2。

Conclusion: MobileViCLIP为移动设备提供了一种高效且性能优越的视频文本模型解决方案。
...
### [117] [DocR1: Evidence Page-Guided GRPO for Multi-Page Document Understanding](https://arxiv.org/abs/2508.07313)
...
<details>
  <summary>Details</summary>
Motivation: 多页文档理解对多模态大语言模型（MLLMs）提出了挑战，需要细粒度视觉理解和跨页多跳推理，而现有强化学习方法在此领域尚未充分探索。

Method: 提出EviGRPO框架，结合证据感知奖励机制和两阶段标注流程，构建了EviBench训练集和ArxivFullQA评估基准。
...
### [118] [Omni-Effects: Unified and Spatially-Controllable Visual Effects Generation](https://arxiv.org/abs/2508.07981)
*Fangyuan Mao,Aiming Hao,Jintao Chen,Dongxia Liu,Xiaokun Feng,Jiashu Zhu,Meiqi Wu,Chubin Chen,Jiahong Wu,Xiangxiang Chu*

Main category: cs.CV

TL;DR: Omni-Effects提出了一种统一框架，通过LoRA-MoE和SAP技术实现多VFX的空间可控生成，解决了现有方法的单效果限制和干扰问题。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成模型受限于单效果训练，无法满足多效果空间可控的需求，阻碍了复杂VFX的应用。

Method: 采用LoRA-MoE整合多效果，利用SAP实现空间控制，并引入IIF模块防止信号干扰。

Result: 实验表明，Omni-Effects能精确控制多效果的空间分布和类别。

Conclusion: Omni-Effects为多VFX生成提供了高效、可控的解决方案。
...
### [119] [RORPCap: Retrieval-based Objects and Relations Prompt for Image Captioning](https://arxiv.org/abs/2508.07318)
*Jinjing Gu,Tianbao Qin,Yuanyuan Pu,Zhengpeng Zhao*

Main category: cs.CV

TL;DR: RORPCap是一种基于检索的图像描述生成方法，通过提取对象和关系词并结合提示模板，快速生成高质量的图像描述，显著减少训练时间和成本。


<details>
  <summary>Details</summary>
Motivation: 解决传统图像描述方法中冗余检测信息、GCN构建困难和训练成本高的问题。

Method: 使用对象和关系提取模型提取关键词，结合提示模板和Mamba网络映射图像嵌入，最终通过GPT-2生成描述。

Result: 在MS-COCO数据集上仅需2.6小时训练，CIDEr和SPICE分数分别为120.5%和22.0%。

Conclusion: RORPCap在性能和效率上优于传统方法，是图像描述任务的潜在替代方案。
...
### [120] [Investigating the Design Space of Visual Grounding in Multimodal Large Language Model](https://arxiv.org/abs/2508.08066)
*Weitai Kang,Weiming Zhuang,Zhizhong Li,Yan Yan,Lingjuan Lyu*

Main category: cs.CV

TL;DR: 该论文系统研究了多模态大语言模型（MLLMs）中视觉接地（VG）任务的设计选择，通过LLaVA-1.5模型验证了不同设计对性能的影响，最终提升了VG任务的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在微调MLLMs用于VG任务时设计选择分散，缺乏系统性验证，因此需要全面研究这些设计对性能的影响。

Method: 使用LLaVA-1.5模型，研究不同视觉接地范式和数据设计，进行消融实验以优化微调。

Result: 研究发现优化后的设计显著提升了VG任务性能，在RefCOCO/+/g数据集上分别提高了5.6%、6.9%和7.0%。

Conclusion: 通过系统性研究设计选择，论文为MLLMs在VG任务中的性能提升提供了有效方法，并验证了其广泛适用性。
...
### [121] [Planner-Refiner: Dynamic Space-Time Refinement for Vision-Language Alignment in Videos](https://arxiv.org/abs/2508.07330)
*Tuyen Tran,Thao Minh Le,Quang-Hung Le,Truyen Tran*

Main category: cs.CV

TL;DR: Planner-Refiner框架通过迭代优化视觉元素的时空表示，结合语言指导，减少语义鸿沟，提升视频与语言对齐任务的效果。


<details>
  <summary>Details</summary>
Motivation: 解决视频与语言对齐中的复杂语言、动态实体交互及语义鸿沟问题。
...
Conclusion: Planner-Refiner框架在处理复杂语言提示时具有显著优势。
...
### [122] [CoAR: Concept Injection into Autoregressive Models for Personalized Text-to-Image Generation](https://arxiv.org/abs/2508.07341)
*Fangtai Wu,Mushui Liu,Weijie He,Wanggui He,Hao Jiang,Zhao Wang,Yunlong Yu*

Main category: cs.CV

TL;DR: CoAR是一种新颖的框架，用于在保持预训练参数冻结的情况下，向统一自回归模型注入主题概念，显著提高了计算和内存效率。
...
Method: 采用层间多模态上下文学习策略，仅用极少量参数学习有效的特定主题表示，并引入正则化防止过拟合和语言漂移。

Result: CoAR在主题驱动个性化和风格个性化方面表现优异，同时显著提升计算和内存效率，仅调整不到0.05%的参数。

Conclusion: CoAR在保持预训练模型性能的同时，实现了高效的主题定制化生成，为统一自回归模型的应用提供了新方向。
...
### [123] [Hyperspectral Imaging](https://arxiv.org/abs/2508.08107)
...
TL;DR: 该论文全面综述了高光谱成像（HSI）的原理、技术、分析方法及应用，并探讨了当前挑战与未来发展方向。


<details>
  <summary>Details</summary>
Motivation: HSI作为一种先进的传感技术，能够同时捕捉空间和光谱信息，为多领域提供非侵入性、无标记的分析能力。本文旨在系统介绍HSI的各个方面，促进其跨学科应用。

Method: 从物理原理、传感器架构到数据采集、校准和校正，总结了HSI的数据结构和分析方法，包括降维、分类、光谱解混及深度学习等。

Result: HSI在地球观测、精准农业、生物医学等领域展现出揭示亚视觉特征的潜力，但面临硬件限制、数据复杂性等挑战。

Conclusion: 未来HSI将向实时、嵌入式系统发展，结合自监督学习和基础模型，有望成为跨学科的通用平台，推动科学和社会的变革。
...
### [124] [SODiff: Semantic-Oriented Diffusion Model for JPEG Compression Artifacts Removal](https://arxiv.org/abs/2508.07346)
...
<details>
  <summary>Details</summary>
Motivation: JPEG高压缩比会引入严重的视觉伪影，现有深度学习方法难以恢复复杂纹理细节，导致输出过于平滑。

Method: 提出SODiff模型，结合语义对齐的图像提示提取器（SAIPE）和质量因子感知时间预测器，优化扩散过程。

Result: 实验表明，SODiff在视觉质量和定量指标上均优于现有领先方法。

Conclusion: SODiff通过语义导向和自适应时间预测，有效解决了JPEG伪影问题，提升了图像恢复质量。
...
### [125] [GRASPTrack: Geometry-Reasoned Association via Segmentation and Projection for Multi-Object Tracking](https://arxiv.org/abs/2508.08117)
...
Method: GRASPTrack将单目深度估计和实例分割整合到标准跟踪流程中，生成3D点云，并通过体素化实现精确的3D IoU空间关联。还引入了深度感知自适应噪声补偿和深度增强的观测中心动量。

Result: 在MOT17、MOT20和DanceTrack基准测试中表现出色，显著提升了复杂场景下的跟踪鲁棒性。

Conclusion: GRASPTrack通过深度感知和3D几何推理，有效提升了多目标跟踪在复杂场景中的性能。
...
### [126] [GS4Buildings: Prior-Guided Gaussian Splatting for 3D Building Reconstruction](https://arxiv.org/abs/2508.07355)
*Qilin Zhang,Olaf Wysocki,Boris Jutzi*

Main category: cs.CV

TL;DR: GS4Buildings是一种基于语义3D建筑模型的高斯泼溅方法，用于改进大规模复杂城市场景中的建筑表面重建。
...
Result: 实验显示，GS4Buildings提高了重建完整性20.5%，几何精度32.8%，并减少了71.8%的高斯基元。

Conclusion: 语义建筑模型的整合为高斯泼溅重建在智能城市和数字孪生等实际应用中提供了潜力。
...
### [127] [MedReasoner: Reinforcement Learning Drives Reasoning Grounding from Clinical Thought to Pixel-Level Precision](https://arxiv.org/abs/2508.08177)
*Zhonghao Yan,Muxi Diao,Yuxuan Yang,Jiayuan Xu,Kaizhou Zhang,Ruoyan Jing,Lele Yang,Yanxi Liu,Kongming Liang,Zhanyu Ma*

Main category: cs.CV

TL;DR: 论文提出了一种新的医学视觉语言任务UMRG，并发布了包含14K样本的数据集U-MRG-14K。同时，提出了MedReasoner框架，通过强化学习优化推理模块，结合冻结的分割专家，实现了高性能的医学图像定位。


<details>
  <summary>Details</summary>
Motivation: 当前医学图像定位方法依赖显式空间提示，难以处理临床实践中的隐式查询。

Method: 提出UMRG任务和U-MRG-14K数据集，设计MedReasoner框架，分离推理与分割模块，通过强化学习优化推理。
...
### [128] [Training and Inference within 1 Second -- Tackle Cross-Sensor Degradation of Real-World Pansharpening with Efficient Residual Feature Tailoring](https://arxiv.org/abs/2508.07369)
...
<details>
  <summary>Details</summary>
Motivation: 解决深度学习全色锐化模型在跨传感器数据上泛化能力差的问题，避免传统方法的高耗时或额外数据需求。

Method: 通过模块化分解模型，识别关键接口并集成特征裁剪器，利用物理感知的无监督损失高效训练，采用分块并行处理提升效率。

Result: 在跨传感器场景下性能显著提升，训练和推理时间极短（如512x512x8图像仅需0.2秒），比零样本方法快100倍以上。

Conclusion: 该方法在质量和效率上均达到最优，为跨传感器全色锐化提供了高效解决方案。
...
### [129] [DIP-GS: Deep Image Prior For Gaussian Splatting Sparse View Recovery](https://arxiv.org/abs/2508.07372)
*Rajaei Khatib,Raja Giryes*

Main category: cs.CV

TL;DR: DIP-GS结合Deep Image Prior与3D高斯泼溅技术，提升稀疏视图下的3D场景重建效果。
...
Method: 采用Deep Image Prior，以粗到细的方式优化3D高斯泼溅表示，无需预训练模型。

Result: 在稀疏视图重建任务中达到SOTA水平。

Conclusion: DIP-GS在稀疏视图场景下表现出色，具有实际应用潜力。
...
### [130] [OMGSR: You Only Need One Mid-timestep Guidance for Real-World Image Super-Resolution](https://arxiv.org/abs/2508.08227)
*Zhiqiang Wu,Zhaomang Sun,Tong Zhou,Bingtao Fu,Ji Cong,Yitong Dong,Huaqi Zhang,Xuan Tang,Mingsong Chen,Xian Wei*

Main category: cs.CV

TL;DR: OMGSR是一种基于DDPM/FM的一步式真实图像超分辨率框架，通过在中时间步注入低质量图像潜在分布，并优化潜在分布差距，显著提升了生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有一步式真实图像超分辨率模型在初始时间步注入低质量图像潜在分布，但其与高斯噪声潜在分布存在差距，限制了生成先验的有效利用。

Method: 提出OMGSR框架，在中时间步注入低质量图像潜在分布，引入潜在分布细化损失和重叠分块LPIPS/GAN损失以消除棋盘伪影。

Result: OMGSR-S/F在512分辨率下表现优异，OMGSR-F在所有参考指标中占据绝对优势，1k分辨率下细节生成效果出色。

Conclusion: OMGSR框架显著提升了一步式真实图像超分辨率的性能，尤其在细节生成和高分辨率扩展方面表现突出。
...
### [131] [LET-US: Long Event-Text Understanding of Scenes](https://arxiv.org/abs/2508.07401)
...
<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在处理事件流时效率低下或仅能处理短序列，LET-US旨在解决这一问题，实现长事件流与文本的高效对齐和理解。
...
### [132] [Cut2Next: Generating Next Shot via In-Context Tuning](https://arxiv.org/abs/2508.08244)
*Jingwen He,Hongbo Liu,Jiajun Li,Ziqi Huang,Yu Qiao,Wanli Ouyang,Ziwei Liu*

Main category: cs.CV

TL;DR: 论文提出Next Shot Generation (NSG)框架Cut2Next，通过Diffusion Transformer和Hierarchical Multi-Prompting策略生成符合专业剪辑模式的高质量后续镜头，提升叙事流畅性和电影连续性。


<details>
  <summary>Details</summary>
Motivation: 当前方法多注重基础视觉一致性，忽视了推动叙事的剪辑模式（如正反打镜头、插入镜头），导致输出缺乏叙事复杂性和电影完整性。

Method: 采用Diffusion Transformer (DiT)和Hierarchical Multi-Prompting策略，结合Relational Prompts和Individual Prompts，并通过Context-Aware Condition Injection (CACI)和Hierarchical Attention Mask (HAM)整合信号。
...
### [133] [ForensicsSAM: Toward Robust and Unified Image Forgery Detection and Localization Resisting to Adversarial Attack](https://arxiv.org/abs/2508.07402)
*Rongxuan Peng,Shunquan Tan,Chenqi Kong,Anwei Luo,Alex C. Kot,Jiwu Huang*

Main category: cs.CV

TL;DR: 论文提出ForensicsSAM，一种具有内置对抗鲁棒性的统一IFDL框架，通过注入伪造和对抗专家增强模型性能，并在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有PEFT方法在适应大型视觉基础模型时忽视了对对抗攻击的脆弱性，导致性能下降。

Method: 1. 注入伪造专家增强图像编码器；2. 设计轻量级对抗检测器；3. 注入对抗专家修正特征偏移。

Result: ForensicsSAM在对抗攻击下表现优异，同时在图像级伪造检测和像素级伪造定位中达到SOTA性能。

Conclusion: ForensicsSAM通过创新的专家注入和检测机制，显著提升了对抗鲁棒性和任务性能。
...
### [134] [CharacterShot: Controllable and Consistent 4D Character Animation](https://arxiv.org/abs/2508.07409)
...
<details>
  <summary>Details</summary>
Motivation: 解决从单张图像和2D姿势序列生成高质量4D角色动画的挑战，提升设计师的创作效率。

Method: 结合2D动画模型、双注意力模块、相机先验和4D高斯溅射优化，实现从2D到3D的提升。
...
### [135] [CLUE: Leveraging Low-Rank Adaptation to Capture Latent Uncovered Evidence for Image Forgery Localization](https://arxiv.org/abs/2508.07413)
...
### [136] [Levarging Learning Bias for Noisy Anomaly Detection](https://arxiv.org/abs/2508.07441)
*Yuxin Zhang,Yunkang Cao,Yuqi Cheng,Yihan Sun,Weiming Shen*

Main category: cs.CV

TL;DR: 论文提出了一种两阶段框架，利用模型的学习偏置来解决训练数据中可能包含未标记异常的全无监督图像异常检测问题。
...
Method: 通过两阶段框架：第一阶段利用学习偏置划分训练集并过滤净化数据，第二阶段在净化数据上训练最终检测器。

Result: 在Real-IAD基准测试中表现出优异的异常检测和定位性能，且对噪声具有鲁棒性。

Conclusion: 该框架模型无关，适用于多种无监督骨干网络，为实际场景中的不完美训练数据提供了实用解决方案。
...
### [137] [Health Care Waste Classification Using Deep Learning Aligned with Nepal's Bin Color Guidelines](https://arxiv.org/abs/2508.07450)
*Suman Kunwar,Prabesh Rai*

Main category: cs.CV

TL;DR: 研究比较了多种医疗废物分类模型，发现YOLOv5-s准确率最高（95.06%），但推理速度略慢于YOLOv8-n。EfficientNet-B0准确率93.22%，但耗时最长。最终部署了YOLOv5-s模型，并建议进一步优化数据。


<details>
  <summary>Details</summary>
Motivation: 尼泊尔医疗设施增加导致医疗废物管理挑战加剧，不当处理会引发污染和传染病传播。

Method: 使用Stratified K-fold技术（5折）对比ResNeXt-50、EfficientNet-B0、MobileNetV3-S、YOLOv8-n和YOLOv5-s模型。

Result: YOLOv5-s准确率最高（95.06%），EfficientNet-B0耗时最长。ANOVA验证统计显著性后部署了YOLOv5-s。

Conclusion: YOLOv5-s表现最佳，已部署至网络供公众使用，建议进一步优化数据和本地化。
...
### [138] [AURA: A Fine-Grained Benchmark and Decomposed Metric for Audio-Visual Reasoning](https://arxiv.org/abs/2508.07470)
*Siminfar Samakoush Galougah,Rishie Raj,Sanjoy Chowdhury,Sayan Nag,Ramani Duraiswami*

Main category: cs.CV

TL;DR: AURA是一个新的音频-视觉基准测试，旨在评估跨模态推理能力，填补现有测试仅关注答案准确性的不足。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试忽视推理过程，无法区分真正理解与通过错误推理得出的正确答案。

Method: AURA包含六个认知领域的问题，要求模型基于音频和视频构建逻辑路径，并提出AuraScore评估推理保真度。

Result: 评估显示，尽管模型在某些任务上准确率高达92%，但推理保真度低于45%，表明逻辑缺陷。

Conclusion: AURA揭示了模型推理能力的不足，为更稳健的多模态评估提供了方向。
...
### [139] [Novel View Synthesis with Gaussian Splatting: Impact on Photogrammetry Model Accuracy and Resolution](https://arxiv.org/abs/2508.07483)
*Pranav Chougule*

Main category: cs.CV

TL;DR: 论文比较了摄影测量与高斯泼溅技术在3D模型重建和视图合成中的表现，通过多种指标评估性能，并展示了高斯泼溅在生成高质量新视图方面的潜力。


<details>
  <summary>Details</summary>
Motivation: 研究旨在比较摄影测量与高斯泼溅技术在3D重建和视图合成中的效果，探索高斯泼溅在提升摄影测量模型质量方面的潜力。

Method: 创建真实场景数据集，分别用摄影测量和高斯泼溅构建3D模型，并通过SSIM、PSNR、LPIPS和lp/mm分辨率等指标评估性能。改进高斯泼溅库以支持Blender环境中的新视图渲染。

Result: 高斯泼溅能生成高质量新视图，并提升摄影测量模型的3D重建效果。比较分析揭示了两种方法的优缺点。

Conclusion: 高斯泼溅在视图合成和3D重建中具有潜力，为扩展现实（XR）、摄影测量和自动驾驶模拟提供了有价值的信息。
...
### [140] [VisR-Bench: An Empirical Study on Visual Retrieval-Augmented Generation for Multilingual Long Document Understanding](https://arxiv.org/abs/2508.07493)
...
Conclusion: VisR-Bench填补了多语言视觉检索的空白，揭示了当前模型的局限性。
...
### [141] [FormCoach: Lift Smarter, Not Harder](https://arxiv.org/abs/2508.07501)
*Xiaoye Zuo,Nikos Athanasiou,Ginger Delmas,Yiming Huang,Xingyu Fu,Lingjie Liu*

Main category: cs.CV

TL;DR: FormCoach是一个基于视觉语言模型（VLM）的AI健身教练，通过摄像头实时检测和纠正用户动作，填补了家庭健身中缺乏专业指导的空白。
...
Method: 利用视觉语言模型（VLM）分析用户动作，并通过Web界面提供实时反馈，同时基于1,700个专家标注的视频对进行模型评估。

Result: 基准测试显示AI与人类教练水平仍有差距，但数据集和评估管道的发布推动了AI驱动健身研究。

Conclusion: FormCoach通过人机协作的方式，为AI在健身领域的应用开辟了新方向。
...
### [142] [Exploring Multimodal Diffusion Transformers for Enhanced Prompt-based Image Editing](https://arxiv.org/abs/2508.07519)
*Joonghyuk Shin,Alchan Hwang,Yujin Kim,Daneul Kim,Jaesik Park*

Main category: cs.CV

TL;DR: 论文分析了基于Transformer的多模态扩散模型（MM-DiT）的注意力机制，提出了一种新的基于提示的图像编辑方法，适用于全局到局部编辑。


<details>
  <summary>Details</summary>
Motivation: 传统U-Net架构已被Transformer扩散模型取代，MM-DiT成为主流。现有编辑技术面临新架构的挑战，需系统分析其注意力机制。

Method: 通过分解注意力矩阵为四个块，揭示其特性，提出适用于MM-DiT的提示编辑方法。

Result: 方法支持全局到局部编辑，适用于多种MM-DiT变体，包括少步模型。

Conclusion: 研究填补了U-Net与新兴架构间的技术空白，为MM-DiT行为模式提供了深入见解。
...
### [143] [Enhancing Reliability of Medical Image Diagnosis through Top-rank Learning with Rejection Module](https://arxiv.org/abs/2508.07528)
*Xiaotong Ji,Ryoma Bise,Seiichi Uchida*

Main category: cs.CV

TL;DR: 提出一种结合拒绝模块的top-rank学习方法，用于医学图像处理，以解决噪声标签和类别模糊实例的问题，提高诊断准确性。
...
Method: 提出一种新方法，通过集成拒绝模块与top-rank损失共同优化，识别并减少异常值的影响。

Result: 在医学数据集上的实验验证表明，该方法能有效检测和减少异常值，提高诊断的可靠性和准确性。

Conclusion: 结合拒绝模块的top-rank学习方法能显著提升医学图像诊断的准确性和可靠性。
...
### [144] [Enhanced Generative Structure Prior for Chinese Text Image Super-resolution](https://arxiv.org/abs/2508.07537)
*Xiaoming Li,Wangmeng Zuo,Chen Change Loy*

Main category: cs.CV

TL;DR: 提出了一种基于结构先验的高质量中文文本图像超分辨率框架，利用StyleGAN生成能力恢复低分辨率字符的精确笔画。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要针对英文文本，对复杂脚本如中文关注不足，需解决字符结构多样性和字体风格的问题。

Method: 结合结构先验和StyleGAN，通过代码本机制限制生成空间，生成高分辨率结构先验以指导恢复。

Result: 实验表明该方法能准确恢复低分辨率中文文本的清晰笔画，适用于不规则布局的真实场景。

Conclusion: 提出的结构先验框架为中文文本图像超分辨率提供了有效解决方案，代码和模型已开源。
...
### [145] [Domain Generalization of Pathological Image Segmentation by Patch-Level and WSI-Level Contrastive Learning](https://arxiv.org/abs/2508.07539)
...
TL;DR: 提出一种针对病理图像中域偏移的泛化方法，通过聚类非肿瘤区域的WSI特征并利用对比学习减少特征差异。
...
Method: 采用两阶段对比学习（WSI级别和patch级别）以减少不同聚类间的特征差异。

Result: 有效减少了病理图像中的域偏移问题。

Conclusion: 该方法为病理图像域泛化提供了一种实用且高效的解决方案。
...
### [146] [CoT-Pose: Chain-of-Thought Reasoning for 3D Pose Generation from Abstract Prompts](https://arxiv.org/abs/2508.07540)
*Junuk Cha,Jihyeon Kim*

Main category: cs.CV

TL;DR: 论文提出了一种结合CoT推理的3D人体姿势生成框架，解决了现有模型依赖低层次提示的问题，并通过数据合成管道生成训练数据。


<details>
  <summary>Details</summary>
Motivation: 现有文本到姿势生成模型依赖低层次提示，而人类倾向于用高层次语言描述动作，导致实际应用中的不匹配。

Method: 引入CoT推理框架，将抽象提示转化为准确3D姿势，并提出自动生成训练数据的数据合成管道。

Result: 实验表明，CoT-Pose模型能从抽象文本输入生成合理且语义对齐的姿势。

Conclusion: 该工作强调了高层次理解在姿势生成中的重要性，并为推理增强方法开辟了新方向。
...
### [147] [Commentary Generation for Soccer Highlights](https://arxiv.org/abs/2508.07543)
*Chidaksh Ravuru*

Main category: cs.CV

TL;DR: 本文扩展了MatchVoice模型，用于生成足球集锦的实时解说，使用GOAL数据集，并通过实验评估了不同训练配置和硬件限制的影响。


<details>
  <summary>Details</summary>
Motivation: 解决现有系统（如SoccerNet-Caption）在视频内容与解说之间细粒度对齐方面的不足，MatchVoice通过粗粒度和细粒度对齐技术改进时间同步。

Method: 扩展MatchVoice模型，使用GOAL数据集（专注于短片段而非整场比赛），并进行实验以复现MatchTime结果，评估不同训练配置和硬件限制。

Result: MatchVoice展示了良好的泛化能力，但需要整合更广泛的视频-语言领域技术以进一步提升性能。

Conclusion: 尽管MatchVoice表现良好，但未来工作需结合其他领域技术以优化性能。
...
### [148] [Adaptive Pseudo Label Selection for Individual Unlabeled Data by Positive and Unlabeled Learning](https://arxiv.org/abs/2508.07548)
*Takehiro Yamane,Itaru Tsuge,Susumu Saito,Ryoma Bise*

Main category: cs.CV

TL;DR: 提出一种基于PU学习的医学图像分割伪标签方法，通过单图像学习选择有效伪标签。
...
Method: 引入PU学习，仅使用正样本和未标记数据进行二分类，为每张未标记图像选择伪标签。

Result: 实验证明该方法有效。

Conclusion: 提出的PU学习方法能有效选择伪标签，适用于医学图像分割。
...
### [149] [Decoupled Functional Evaluation of Autonomous Driving Models via Feature Map Quality Scoring](https://arxiv.org/abs/2508.07552)
*Ludan Zhang,Sihan Wang,Yuqi Dai,Shuofei Qiao,Lei He*

Main category: cs.CV

TL;DR: 本文提出了一种基于特征图收敛分数（FMCS）的独立评估方法，通过双粒度动态加权评分系统（DG-DWSS）和CLIP-FMQE-Net网络，实现了对自动驾驶功能模块生成的特征图质量的实时评估，并在NuScenes数据集上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 端到端模型在自动驾驶感知与规划中成为主流，但缺乏对中间功能模块的显式监督信号，导致机制不透明且可解释性有限，传统方法难以独立评估和训练这些模块。

Method: 提出基于特征图-真值表示相似性的评估框架，构建DG-DWSS评分系统，开发CLIP-FMQE-Net网络，结合特征-真值编码器和质量分数预测头，实现实时质量分析。

Result: 在NuScenes数据集上，集成评估模块后，3D目标检测性能提升3.89%（NDS指标），验证了方法的有效性。

Conclusion: 该方法显著提升了特征表示质量和整体模型性能，为自动驾驶功能模块的独立评估与训练提供了新思路。
...
### [150] [Splat4D: Diffusion-Enhanced 4D Gaussian Splatting for Temporally and Spatially Consistent Content Creation](https://arxiv.org/abs/2508.07557)
...
### [151] [Adaptive Cache Enhancement for Test-Time Adaptation of Vision-Language Models](https://arxiv.org/abs/2508.07570)
*Khanh-Binh Nguyen,Phuoc-Nguyen Bui,Hyunseung Choo,Duc Thanh Nguyen*

Main category: cs.CV

TL;DR: ACE框架通过动态缓存和自适应决策边界，提升了视觉语言模型在分布偏移下的测试时适应性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在零样本泛化中表现优异，但在分布偏移和缺乏标注数据时性能下降。现有缓存方法因不可靠的置信度指标和刚性决策边界而受限。

Method: 提出ACE框架，通过动态、类别特定的阈值选择高置信度或低熵样本构建缓存，并利用指数移动平均和探索增强更新优化决策边界。

Result: 在15个基准数据集上，ACE在分布偏移场景中表现最优，优于现有测试时适应方法。
...
### [152] [Exploiting Layer Normalization Fine-tuning in Visual Transformer Foundation Models for Classification](https://arxiv.org/abs/2508.07577)
*Zhaorui Tan,Tan Pan,Kaizhu Huang,Weimiao Yu,Kai Yao,Chen Jiang,Qiufeng Wang,Anh Nguyen,Xin Guo,Yuan Cheng,Xi Yang*

Main category: cs.CV

TL;DR: 本文研究了Vision Transformers中LayerNorm在数据稀缺和域转移下的微调动态，提出了Fine-tuning Shift Ratio（FSR）和基于标量λ的重新缩放机制，以优化LayerNorm的微调效果。


<details>
  <summary>Details</summary>
Motivation: 探索LayerNorm在Vision Transformers中的微调动态，特别是在数据稀缺和域转移情况下的表现，填补相关研究空白。

Method: 提出了FSR量化目标域样本的代表性，并设计了一个基于λ的重新缩放机制和循环框架来优化LayerNorm微调。

Result: 实验表明，OOD任务通常FSR较低且λ较高，尤其是在数据稀缺时；病理数据微调更接近ID设置。
...
### [153] [GAPNet: A Lightweight Framework for Image and Video Salient Object Detection via Granularity-Aware Paradigm](https://arxiv.org/abs/2508.07585)
*Yu-Huan Wu,Wei Liu,Zi-Xuan Zhu,Zizhou Wang,Yong Liu,Liangli Zhen*

Main category: cs.CV

TL;DR: GAPNet是一种基于粒度感知范式的轻量级网络，用于图像和视频显著性目标检测（SOD），通过多尺度解码器输出和高效特征融合模块，实现了高性能且低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有SOD模型依赖重型骨干网络，计算成本高，限制了在边缘设备等实际场景中的应用。GAPNet旨在解决这一问题。

Method: 采用粒度感知连接，结合粒度金字塔卷积（GPC）和跨尺度注意力（CSA）模块，高效融合多尺度特征，并通过自注意力模块学习全局信息。

Result: 在轻量级图像和视频SOD模型中，GAPNet实现了新的最优性能。

Conclusion: GAPNet通过优化特征利用和语义解释，显著提升了轻量级SOD模型的性能，适用于实际应用场景。
...
### [154] [Voice Pathology Detection Using Phonation](https://arxiv.org/abs/2508.07587)
*Sri Raksha Siva,Nived Suthahar,Prakash Boominathan,Uma Ranjan*

Main category: cs.CV

TL;DR: 提出了一种基于机器学习的非侵入性声音病理检测框架，利用声学特征和RNN分类，支持早期诊断。


<details>
  <summary>Details</summary>
Motivation: 声音障碍严重影响生活质量，传统诊断方法侵入性强且主观，需非侵入性解决方案。

Method: 分析声学特征（如MFCCs、chroma等），使用RNN（LSTM和注意力机制）分类，数据增强提升泛化能力。

Result: 框架为非侵入性自动化诊断工具，支持早期声音病理检测。

Conclusion: 该框架有望改善患者预后，推动AI驱动的医疗发展。
...
### [155] [From Prediction to Explanation: Multimodal, Explainable, and Interactive Deepfake Detection Framework for Non-Expert Users](https://arxiv.org/abs/2508.07596)
*Shahroz Tariq,Simon S. Woo,Priyanka Singh,Irena Irmalasari,Saakshi Gupta,Dev Gupta*

Main category: cs.CV

TL;DR: DF-P2E是一个多模态框架，通过视觉、语义和叙事层解释，使深度伪造检测可解释且易用。


<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测系统缺乏透明度和可解释性，限制了其在实际决策中的实用性。

Method: DF-P2E包含三个模块：基于Grad-CAM的分类器、视觉字幕生成模块和叙事优化模块。

Result: 在DF40数据集上表现出色，检测性能优越且解释质量高。

Conclusion: 该框架为可解释的深度伪造检测提供了可扩展方案，推动了可信赖AI系统的发展。
...
### [156] [LaVieID: Local Autoregressive Diffusion Transformers for Identity-Preserving Video Creation](https://arxiv.org/abs/2508.07603)
...
Method: 引入局部路由器显式表示细粒度局部面部结构，并结合时间自回归模块增强帧间身份一致性。

Result: LaVieID能够生成高保真个性化视频，并实现最先进的性能。

Conclusion: LaVieID通过局部和时间的优化设计，显著提升了身份保持视频生成的效果。
...
### [157] [X2Edit: Revisiting Arbitrary-Instruction Image Editing through Self-Constructed Data and Task-Aware Representation Learning](https://arxiv.org/abs/2508.07607)
...
<details>
  <summary>Details</summary>
Motivation: 现有开源数据集和编辑模块在多样性和兼容性上存在不足，需要更高质量的数据集和更高效的训练方法。

Method: 构建了X2Edit数据集（370万高质量数据），设计了任务感知的MoE-LoRA训练方法，并引入对比学习。

Result: 模型编辑性能优异，数据集优于现有开源数据集。

Conclusion: X2Edit数据集和训练方法为图像编辑任务提供了高质量资源和高效解决方案。
...
### [158] [An Iterative Reconstruction Method for Dental Cone-Beam Computed Tomography with a Truncated Field of View](https://arxiv.org/abs/2508.07618)
*Hyoung Suk Park,Kiwan Jeon*

Main category: cs.CV

TL;DR: 提出一种两阶段方法，通过隐式神经表示（INR）生成先验图像并校正截断误差，以改善牙科CBCT的图像质量。


<details>
  <summary>Details</summary>
Motivation: 小型探测器导致的截断视场（FOV）问题在迭代重建中会累积误差，降低图像质量。

Method: 第一阶段使用INR生成扩展区域的先验图像，第二阶段利用校正后的投影数据进行常规迭代重建。

Result: 数值结果表明，该方法有效抑制了截断伪影，提升了图像质量。

Conclusion: 两阶段方法通过INR和迭代重建的结合，显著改善了牙科CBCT的截断问题。
...
### [159] [Enhancing Egocentric Object Detection in Static Environments using Graph-based Spatial Anomaly Detection and Correction](https://arxiv.org/abs/2508.07624)
*Vishakha Lall,Yisi Liu*

Main category: cs.CV

TL;DR: 提出了一种基于图神经网络的后期处理框架，通过建模物体间的空间关系来纠正检测异常，显著提升了检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有目标检测模型未能有效利用静态环境中物体空间布局的一致性，导致检测结果不一致或错误。

Method: 使用图神经网络（GNN）建模物体间的空间关系，训练数据为人工标注数据，用于纠正异常检测结果。

Result: 实验表明，该方法显著提升了检测性能，mAP@50最高提升4%。

Conclusion: 利用环境空间结构可以显著提升目标检测系统的可靠性。
...
### [160] [A Trustworthy Method for Multimodal Emotion Recognition](https://arxiv.org/abs/2508.07625)
*Junxiao Xue,Xiaozhen Liu,Jie Wang,Xuecheng Wu,Bin Wu*

Main category: cs.CV

TL;DR: 提出了一种基于不确定性估计的信任情感识别方法（TER），通过多模态融合和置信度计算提高预测的可靠性，并在噪声和异常数据下表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有情感识别方法依赖复杂深度模型，但忽略了预测的可靠性，特别是在噪声和异常数据下。

Method: TER利用不确定性估计计算预测的置信度，结合多模态结果输出可信预测，并引入信任精度和信任召回等新评估标准。

Result: TER在Music-video数据集上达到82.40%的准确率，在IEMOCAP和Music-video上的信任F1分数分别为0.7511和0.9035。

Conclusion: TER通过置信度模块提升了模型的可靠性和鲁棒性，在性能和信任度上均优于现有方法。
...
### [161] [AR-VRM: Imitating Human Motions for Visual Robot Manipulation with Analogical Reasoning](https://arxiv.org/abs/2508.07626)
*Dejie Yang,Zijing Zhao,Yang Liu*

Main category: cs.CV

TL;DR: AR-VRM通过模仿人类手部关键点，从大规模人类动作视频中学习，解决了机器人数据不足的问题，并在CALVIN基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有方法利用网络数据或隐式训练，泛化能力有限，AR-VRM旨在通过显式模仿人类动作提升机器人操作的泛化能力。

Method: 提出基于关键点的视觉语言模型预训练方案，学习人类动作知识并预测手部关键点；在机器人数据微调时，通过类比推理映射人类手部关键点到机器人组件。

Result: 在CALVIN基准测试和真实实验中表现领先，尤其在少样本场景下显著优于现有方法。

Conclusion: 显式模仿人类动作在数据稀缺时有效，AR-VRM为视觉机器人操作提供了新思路。
...
### [162] [LaRender: Training-Free Occlusion Control in Image Generation via Latent Rendering](https://arxiv.org/abs/2508.07647)
*Xiaohang Zhan,Dingming Liu*

Main category: cs.CV

TL;DR: 提出了一种无需训练的图像生成算法，通过体积渲染原理精确控制图像中对象的遮挡关系。
...
Result: 在遮挡准确性上显著优于现有方法，并能调整对象透明度、密度等实现多种效果。

Conclusion: 该方法无需重新训练，基于物理原理实现了精确遮挡控制，扩展了图像生成的应用场景。
...
### [163] [Collaborative Learning of Scattering and Deep Features for SAR Target Recognition with Noisy Labels](https://arxiv.org/abs/2508.07656)
*Yimin Fu,Zhunga Liu,Dongxiu Guo,Longfei Wang*

Main category: cs.CV

TL;DR: 提出了一种结合散射特征和深度特征的协作学习方法（CLSDF），用于处理SAR自动目标识别中的噪声标签问题，通过多模型特征融合和半监督学习提升性能。


<details>
  <summary>Details</summary>
Motivation: 由于SAR数据标注需要专业知识，噪声标签不可避免，导致自动目标识别性能下降。现有方法主要针对图像数据，但SAR数据的非直观特性使其难以实现噪声鲁棒学习。

Method: 设计了多模型特征融合框架，将散射特征和深度特征结合；通过GMM建模损失分布区分干净和噪声标签样本；采用半监督学习和联合分布对齐策略。

Result: 在MSTAR数据集上的实验表明，该方法在不同噪声条件下均能实现最先进的性能。

Conclusion: CLSDF方法有效解决了SAR数据噪声标签问题，显著提升了自动目标识别的鲁棒性和准确性。
...
### [164] [Undress to Redress: A Training-Free Framework for Virtual Try-On](https://arxiv.org/abs/2508.07680)
...
<details>
  <summary>Details</summary>
Motivation: 现有虚拟试衣方法在长袖转短袖场景中因皮肤恢复不准确而效果不佳，UR-VTON旨在解决这一问题。

Method: 采用“脱衣-穿衣”两步机制，结合动态分类器自由指导和结构细化器优化细节。

Result: 实验表明UR-VTON在细节保留和图像质量上优于现有方法。

Conclusion: UR-VTON为虚拟试衣提供了一种高效且无需训练的解决方案，适用于复杂场景。
...
### [165] [Make Your MoVe: Make Your 3D Contents by Adapting Multi-View Diffusion Models to External Editing](https://arxiv.org/abs/2508.07700)
*Weitao Wang,Haoran Xu,Jun Meng,Haoqian Wang*

Main category: cs.CV

TL;DR: 提出了一种无需调参、即插即用的方法，用于在3D生成中保持编辑内容与原始几何的对齐。
...
### [166] [Multi-view Normal and Distance Guidance Gaussian Splatting for Surface Reconstruction](https://arxiv.org/abs/2508.07701)
*Bo Jia,Yanan Guo,Ying Chang,Benkui Zhang,Ying Xie,Kangning Du,Lin Cao*

Main category: cs.CV

TL;DR: 3D高斯泼溅（3DGS）在多视角场景中通过距离和法线约束优化表面重建，解决了单视角投影中的偏差问题。


<details>
  <summary>Details</summary>
Motivation: 单视角投影中高斯法线对齐可能导致多视角切换时的几何偏差，需解决多视角场景中的距离和全局匹配问题。

Method: 设计了多视角法线和距离引导的高斯泼溅方法，包括距离重投影正则化模块和法线增强模块，约束深度图并对齐3D法线。

Result: 实验表明，该方法在定量和定性评估中优于基线，显著提升了3DGS的表面重建能力。

Conclusion: 提出的方法有效解决了多视角场景中的几何偏差问题，实现了高精度重建。
...
### [167] [A Registration-Based Star-Shape Segmentation Model and Fast Algorithms](https://arxiv.org/abs/2508.07721)
...
Conclusion: 提出的模型在星形分割任务中表现出色，能够适应多种场景需求。
...
### [168] [Enhancing Small-Scale Dataset Expansion with Triplet-Connection-based Sample Re-Weighting](https://arxiv.org/abs/2508.07723)
*Ting Xiang,Changjian Chen,Zhuo Tang,Qifeng Zhang,Fei Lyu,Li Yang,Jiapeng Zhang,Kenli Li*

Main category: cs.CV

TL;DR: 论文提出了一种基于三元组连接的样本重加权方法TriReWeight，用于解决生成数据增强中噪声图像的问题，并在理论和实验上验证了其有效性。
...
Method: 提出TriReWeight方法，通过三元组连接对生成图像进行重加权，理论分析三种监督方式。

Result: 实验表明TriReWeight在六个自然图像数据集和三个医学数据集上分别平均提升7.9%和3.4%，且兼容多种生成数据增强方法。

Conclusion: TriReWeight是一种有效的样本重加权方法，能显著提升生成数据增强的性能，且理论最优。
...
### [169] [Grouped Speculative Decoding for Autoregressive Image Generation](https://arxiv.org/abs/2508.07747)
...
<details>
  <summary>Details</summary>
Motivation: 自回归图像模型虽然生成能力强，但推理时间长，限制了实际应用。传统Speculative Decoding方法未能充分利用图像令牌的冗余性和多样性。

Method: 提出动态GSD策略，评估视觉有效令牌簇而非单一目标令牌，克服静态聚类无效的问题。

Result: 实验表明GSD平均加速3.7倍，且无需额外训练。
...
### [170] [Comparison Reveals Commonality: Customized Image Generation through Contrastive Inversion](https://arxiv.org/abs/2508.07755)
*Minseo Kim,Minchan Kwon,Dongyeun Lee,Yunho Jeon,Junmo Kim*

Main category: cs.CV

TL;DR: 提出了一种无需额外信息的对比反转方法，通过对比学习提取图像中的共同概念，并在生成和编辑中表现优异。
...
Method: 通过对比学习训练目标标记和图像辅助文本标记，应用解耦交叉注意力微调以提高概念保真度。

Result: 实验表明，该方法在概念表示和编辑方面均表现优异，优于现有技术。

Conclusion: 对比反转方法能有效提取共同概念，无需额外信息，生成质量高。
...
### [171] [Correspondence as Video: Test-Time Adaption on SAM2 for Reference Segmentation in the Wild](https://arxiv.org/abs/2508.07759)
*Haoran Wang,Zekun Li,Jian Zhang,Lei Qi,Yinghuan Shi*

Main category: cs.CV

TL;DR: CAV-SAM提出了一种轻量级方法，通过将参考-目标图像对的对应关系视为伪视频，利用SAM2的iVOS能力适应下游任务，性能提升超过5%。


<details>
  <summary>Details</summary>
Motivation: 现有参考分割方法依赖元学习，数据与计算成本高，而SAM在真实下游任务中表现有限。

Method: 将参考-目标图像对表示为伪视频，利用SAM2的iVOS能力，结合DBST和TTGA模块进行轻量级适配。

Result: 在广泛使用的数据集上，分割性能提升超过5%。

Conclusion: CAV-SAM为轻量级适应视觉模型提供了新方向，显著提升了性能。
...
### [172] [Dream4D: Lifting Camera-Controlled I2V towards Spatiotemporally Consistent 4D Generation](https://arxiv.org/abs/2508.07769)
*Xiaoyan Liu,Kangrui Li,Jiaxin Liu*

Main category: cs.CV

TL;DR: Dream4D提出了一种结合可控视频生成与神经4D重建的新框架，用于合成时空一致的4D内容，显著提升了生成质量。
...
Method: 采用两阶段架构：首先通过少样本学习从单图像预测最佳相机轨迹，然后通过姿态条件扩散过程生成几何一致的多视角序列，最终转换为持久4D表示。

Result: Dream4D首次结合了视频扩散模型的丰富时间先验和重建模型的几何感知能力，在生成质量（如mPSNR、mSSIM）上优于现有方法。

Conclusion: Dream4D通过协同可控视频生成与神经4D重建，显著提升了4D内容的合成质量，为复杂场景的动态建模提供了新思路。
...
### [173] [Prototype-Guided Curriculum Learning for Zero-Shot Learning](https://arxiv.org/abs/2508.07771)
*Lei Wang,Shiming Chen,Guo-Sen Xie,Ziming Hong,Chaojian Yu,Qinmu Peng,Xinge You*

Main category: cs.CV

TL;DR: 论文提出了一种原型引导的课程学习框架（CLZSL），通过PCL模块减少实例级不匹配，通过PUP模块动态更新类级语义原型，以提升零样本学习中的知识迁移效果。


<details>
  <summary>Details</summary>
Motivation: 现有嵌入方法中，手动定义的语义原型可能因实例级不匹配和类级不精确性引入噪声监督，影响视觉-语义映射的准确性。

Method: CLZSL框架包含PCL模块（通过课程学习减少实例级不匹配）和PUP模块（动态更新类级语义原型）。

Result: 在AWA2、SUN和CUB数据集上的实验验证了方法的有效性。

Conclusion: CLZSL通过减少噪声监督，显著提升了零样本学习中知识迁移的效果。
...
### [174] [Forecasting Continuous Non-Conservative Dynamical Systems in SO(3)](https://arxiv.org/abs/2508.07775)
*Lennart Bastian,Mohammad Rashed,Nassir Navab,Tolga Birdal*

Main category: cs.CV

TL;DR: 提出了一种基于神经控制微分方程和SO(3) Savitzky-Golay路径的方法，用于在3D旋转流形上建模噪声姿态估计的轨迹，解决了非保守力和噪声观测下的鲁棒性问题。


<details>
  <summary>Details</summary>
Motivation: 解决SO(3)外推中的挑战，如未知惯性矩、非保守运动学及稀疏噪声观测下的状态估计问题。

Method: 利用神经控制微分方程和SO(3) Savitzky-Golay路径，建模3D旋转流形上的轨迹，不依赖能量或动量守恒假设。

Result: 模型在仿真和实际场景中表现出鲁棒的外推能力，适用于复杂非惯性系统。

Conclusion: 该方法无需物理参数先验，易于集成到现有流程中，适用于未知动态的轨迹预测。
...
### [175] [GaitSnippet: Gait Recognition Beyond Unordered Sets and Ordered Sequences](https://arxiv.org/abs/2508.07782)
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于集合或序列的步态识别方法在捕捉短范围和长范围时间依赖关系上存在不足，需要一种更全面的方法。

Method: 提出将步态分解为动作片段（snippet），通过随机选择连续片段中的帧表示动作，并结合片段采样和建模技术。
...
### [176] [Boosting Active Defense Persistence: A Two-Stage Defense Framework Combining Interruption and Poisoning Against Deepfake](https://arxiv.org/abs/2508.07795)
*Hongrui Zheng,Yuezun Li,Liejun Wang,Yunfeng Diao,Zhiqing Guo*

Main category: cs.CV

TL;DR: 论文提出了一种双阶段防御框架（TSDF），通过双功能对抗扰动直接扭曲伪造结果并破坏攻击者的数据准备过程，以提升主动防御的持久性。
...
Method: 设计了强度分离机制，利用双功能对抗扰动实现直接扭曲伪造结果和破坏攻击者数据准备的双重作用。

Result: 实验表明，TSDF框架具有强大的双重防御能力，显著提升了防御的持久性。
...
### [177] [Power Battery Detection](https://arxiv.org/abs/2508.07797)
...
TL;DR: 该论文提出了一种新的任务——动力电池检测（PBD），旨在从工业X射线图像中定位阴极和阳极板的密集端点，以进行质量检测。作者发布了首个大规模基准数据集PBD5K，并提出了一种智能标注流程和模型MDCNeXt，结合多维度结构线索和状态空间模块，显著提升了检测性能。


<details>
  <summary>Details</summary>
Motivation: 动力电池内部结构缺陷可能引发严重安全问题，传统视觉算法在密集排列、低对比度和成像伪影等问题上表现不佳，手动检测效率低且易出错。因此，作者提出PBD任务并开发相关解决方案。

Method: 作者构建了PBD5K数据集，包含5,000张X射线图像，并提出智能标注流程。模型MDCNeXt结合点、线和计数信息，并引入两个状态空间模块：提示过滤模块和密度感知重排序模块。此外，还提出距离自适应掩码生成策略。

Result: MDCNeXt在PBD任务中表现出色，能够有效区分电池板并抑制视觉干扰。PBD5K数据集为后续研究提供了基准。

Conclusion: 该论文为动力电池检测任务提供了首个大规模数据集和高效模型，推动了该领域的研究。代码和数据集将公开。
...
### [178] [MambaTrans: Multimodal Fusion Image Translation via Large Language Model Priors for Downstream Visual Tasks](https://arxiv.org/abs/2508.07803)
*Yushen Xu,Xiaosong Li,Zhenyu Kuang,Xiaoqi Cheng,Haishu Tan,Huafeng Li*

Main category: cs.CV

TL;DR: 提出MambaTrans，一种多模态融合图像模态翻译器，用于适配下游任务模型。


<details>
  <summary>Details</summary>
Motivation: 解决多模态融合图像与可见图像像素分布差异导致下游任务性能下降的问题。

Method: 利用多模态大语言模型描述和语义分割掩码作为输入，结合Multi-Model State Space Block增强视觉能力。

Result: 实验表明MambaTrans有效提升下游任务性能。

Conclusion: MambaTrans在不调整预训练模型参数的情况下，显著改善了多模态图像在下游任务中的表现。
...
### [179] [Pose-RFT: Enhancing MLLMs for 3D Pose Generation via Hybrid Action Reinforcement Fine-Tuning](https://arxiv.org/abs/2508.07804)
*Bao Li,Xiaomei Zhang,Miao Xu,Zhaoxin Fan,Xiangyu Zhu,Zhen Lei*

Main category: cs.CV

TL;DR: Pose-RFT是一个基于强化学习的框架，用于优化多模态大语言模型（MLLMs）在3D人体姿态生成任务中的表现，通过混合动作强化学习算法HyGRPO，结合离散语言预测和连续姿态生成，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于监督学习的MLLMs在3D姿态生成任务中难以建模模糊性和任务对齐，Pose-RFT旨在通过强化学习解决这些问题。

Method: 提出Pose-RFT框架，采用混合动作强化学习算法HyGRPO，结合任务特定的奖励函数，优化离散和连续动作的联合优化。

Result: 在多个姿态生成基准测试中，Pose-RFT显著优于现有方法，验证了其有效性。

Conclusion: Pose-RFT通过强化学习优化3D姿态生成任务，展示了混合动作强化学习的潜力。
...
### [180] [DiTVR: Zero-Shot Diffusion Transformer for Video Restoration](https://arxiv.org/abs/2508.07811)
*Sicheng Gao,Nancy Mehta,Zongwei Wu,Radu Timofte*

Main category: cs.CV

TL;DR: DiTVR是一种零样本视频恢复框架，结合扩散变换器和轨迹感知注意力，通过光流轨迹对齐令牌，提升时间一致性。
...
Method: DiTVR采用扩散变换器与轨迹感知注意力，结合光流轨迹对齐令牌，并通过小波引导的流一致性采样器注入数据一致性。

Result: DiTVR在视频恢复基准测试中达到零样本最优，表现出优异的时间一致性和细节保留能力。

Conclusion: DiTVR通过创新的注意力机制和采样策略，解决了视频恢复中的时间一致性和细节保留问题。
...
### [181] [Semi-supervised Multiscale Matching for SAR-Optical Image](https://arxiv.org/abs/2508.07812)
*Jingze Gai,Changchun Li*

Main category: cs.CV

TL;DR: 提出了一种半监督的SAR-光学图像匹配方法S2M2-SAR，结合少量标注数据和大量未标注数据，通过伪标签和跨模态特征增强提升匹配性能。


<details>
  <summary>Details</summary>
Motivation: 现有SAR-光学图像匹配方法依赖像素级标注，耗时且难以获取足够标注数据，因此设计半监督方法以利用未标注数据。

Method: 结合深度和浅层匹配结果生成伪标签相似性热图，并引入无监督的跨模态特征增强模块，通过统计独立性分离模态共享和特定特征。

Result: S2M2-SAR在基准数据集上表现优于现有半监督方法，并与全监督SOTA方法竞争。

Conclusion: S2M2-SAR高效实用，为SAR-光学图像匹配提供了一种有效的半监督解决方案。
...
### [182] [Segmenting and Understanding: Region-aware Semantic Attention for Fine-grained Image Quality Assessment with Large Language Models](https://arxiv.org/abs/2508.07818)
*Chenyue Song,Chen Hui,Haiqi Zhu,Feng Jiang,Yachun Mi,Wei Zhang,Shaohui Liu*

Main category: cs.CV

TL;DR: 提出了一种名为RSFIQA的细粒度图像质量评估模型，通过整合区域级失真信息和多模态大语言模型（MLLM）来感知多维质量差异，并引入区域感知语义注意力机制（RSA）提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有无参考图像质量评估（NR-IQA）方法要么关注全局表示导致对语义显著区域的洞察有限，要么采用均匀区域特征加权减弱了对局部质量变化的敏感性。

Method: 利用Segment Anything Model（SAM）动态分割图像为非重叠语义区域，结合MLLM提取描述性内容并感知多维失真，引入RSA机制聚合局部区域的细粒度表示生成全局注意力图。

Result: 在多个基准数据集上表现出鲁棒性和有效性，实现了竞争性的质量预测性能。

Conclusion: RSFIQA模型通过区域级失真感知和RSA机制，显著提升了图像质量评估的准确性和适应性。
...
### [183] [MIMIC: Multimodal Inversion for Model Interpretation and Conceptualization](https://arxiv.org/abs/2508.07833)
*Animesh Jain,Alexandros Stergiou*

Main category: cs.CV

TL;DR: MIMIC框架通过合成视觉概念来可视化VLM的内部表示，提升透明度和信任。


<details>
  <summary>Details</summary>
Motivation: 解决VLM因复杂架构导致的透明度不足和信任问题。

Method: 结合VLM的反演和特征对齐目标，加入空间对齐、图像平滑和语义真实性的正则化。

Result: 通过定量和定性评估，展示了MIMIC在视觉质量和语义文本指标上的表现。
...
### [184] [Effortless Vision-Language Model Specialization in Histopathology without Annotation](https://arxiv.org/abs/2508.07835)
*Jingna Qiu,Nishanth Jain,Jonas Ammeling,Marc Aubreville,Katharina Breininger*

Main category: cs.CV

TL;DR: 该论文研究了通过无标注的领域相关图像-文本对继续预训练视觉语言模型（VLMs），以提升其在组织病理学任务中的零样本和小样本性能。


<details>
  <summary>Details</summary>
Motivation: 尽管通用视觉语言模型（如CONCH和QuiltNet）在组织病理学中表现出色，但其通用设计可能导致在特定任务中表现不佳。传统的有监督微调需要手动标注数据，限制了适应性。

Method: 通过从现有数据库中提取领域和任务相关的图像-文本对，对VLMs进行无标注的继续预训练。

Result: 实验表明，这种无标注的继续预训练显著提升了零样本和小样本性能，且在大规模训练时能达到与有监督小样本方法相当的效果。

Conclusion: 无标注的继续预训练是一种高效、任务无关且无需手动标注的VLM适应方法，为组织病理学任务提供了新途径。
...
### [185] [CBDES MoE: Hierarchically Decoupled Mixture-of-Experts for Functional Modules in Autonomous Driving](https://arxiv.org/abs/2508.07838)
*Qi Xiang,Kunsong Shi,Zhigui Lin,Lei He*

Main category: cs.CV

TL;DR: 提出了一种名为CBDES MoE的分层解耦混合专家架构，用于解决多模态BEV感知系统的局限性，并在3D目标检测中表现优于单专家模型。


<details>
  <summary>Details</summary>
Motivation: 现有BEV感知系统在多模态融合中存在输入适应性差、建模能力有限和泛化性能不足的问题。

Method: 采用分层解耦的混合专家架构（CBDES MoE），结合轻量级自注意力路由机制（SAR），实现动态专家路径选择和高效推理。

Result: 在nuScenes数据集上，CBDES MoE在mAP和NDS指标上分别比最强单专家模型提高了1.6和4.1个百分点。

Conclusion: CBDES MoE通过模块化设计提升了BEV感知系统的性能，展示了其在自动驾驶领域的实用优势。
...
TL;DR: 论文通过图卷积网络处理Ge表面的电子显微镜图像，提取拓扑特征并分析，发现辐照角度对形态影响大于辐照通量。


<details>
  <summary>Details</summary>
Motivation: 研究Ge表面微结构的形态变化与辐照参数（角度和通量）的关系。

Method: 使用图卷积网络嵌入骨架图，通过主成分分析和Davies-Bouldin指数评估聚类分离性。

Result: 辐照角度对Ge表面形态的影响显著大于辐照通量。

Conclusion: 辐照角度是影响Ge表面形态的关键因素。
...
TL;DR: 提出了一种基于2D TAP网络的无标记3D组织追踪方法，结合两个CoTracker模型实现立体内窥镜图像的3D运动估计，实验验证了其在手术场景中的准确性。
...
Conclusion: 基于TAP的模型在复杂手术场景中具有高精度的无标记3D追踪潜力。
...
### [188] [Being-M0.5: A Real-Time Controllable Vision-Language-Motion Model](https://arxiv.org/abs/2508.07863)
*Bin Cao,Sipeng Zheng,Ye Wang,Lujie Xia,Qianshan Wei,Qin Jin,Jing Liu,Zongqing Lu*

Main category: cs.CV

TL;DR: 论文提出了一种实时可控的视觉语言运动模型Being-M0.5，解决了现有模型在可控性方面的五大瓶颈，并基于大规模数据集HuMo100M实现了高性能运动生成。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言运动模型（VLMMs）在可控性方面存在显著不足，包括对多样化指令的响应不足、姿态初始化能力有限、长序列性能差、对未见场景处理不足以及缺乏对身体部位的细粒度控制。

Method: 基于HuMo100M数据集（包含500万运动序列和1亿多任务指令实例），提出了一种新颖的部分感知残差量化技术，用于运动标记化，实现细粒度控制。

Result: Being-M0.5在多个运动生成任务中表现优异，实验验证了其高性能和实时能力。

Conclusion: HuMo100M和Being-M0.5显著推动了运动生成技术的发展，为实际应用提供了重要工具和设计指导。
...
### [189] [CATP: Contextually Adaptive Token Pruning for Efficient and Enhanced Multimodal In-Context Learning](https://arxiv.org/abs/2508.07871)
*Yanshu Li,Jianjiang Yang,Zhennan Shen,Ligong Han,Haoyan Xu,Ruixiang Tang*

Main category: cs.CV

TL;DR: 论文提出了一种针对多模态上下文学习的训练无关的令牌剪枝方法（CATP），通过两阶段渐进剪枝减少冗余图像令牌，提高效率且性能略有提升。


<details>
  <summary>Details</summary>
Motivation: 现有图像令牌剪枝方法主要针对单图像任务，忽视了多模态上下文学习中更大的冗余和效率需求，导致性能下降。

Method: 提出CATP方法，通过两阶段渐进剪枝处理复杂的跨模态交互，减少冗余令牌。

Result: 在移除77.8%图像令牌后，CATP在四个LVLMs和八个基准测试中平均性能提升0.6%，推理延迟平均降低10.78%。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常为每种退化类型设计专用网络模块或参数，导致参数开销大且忽略任务间关联性。

Method: 采用两阶段训练范式（预训练和提示调优），利用任务感知增强提示和低秩分解捕捉任务通用与特定特征，并通过对比约束优化任务对齐。

Result: 实验表明，该方法仅用2.75M参数即可在不同修复任务上实现优异性能。

Conclusion: 所提框架通过增强提示和参数优化，高效解决了多任务图像修复问题。
...
### [191] [Stand-In: A Lightweight and Plug-and-Play Identity Control for Video Generation](https://arxiv.org/abs/2508.07901)
*Bowen Xue,Qixin Yan,Wenjing Wang,Hao Liu,Chen Li*

Main category: cs.CV

TL;DR: 提出了一种轻量级、即插即用的视频生成框架Stand-In，用于身份保留，仅需少量参数和训练数据，性能优于全参数训练方法。
...
Result: 仅增加约1%参数，视频质量和身份保留效果优异，优于全参数训练方法，并可无缝集成其他任务。
...
### [192] [CTC Transcription Alignment of the Bullinger Letters: Automatic Improvement of Annotation Quality](https://arxiv.org/abs/2508.07904)
...
### [193] [Generative Video Matting](https://arxiv.org/abs/2508.07905)
...
Method: 1. 利用大规模合成和伪标注数据集进行预训练；2. 开发可扩展的合成数据生成流程；3. 提出基于视频扩散模型的新方法，确保时间一致性。

Result: 在三个基准数据集上表现优异，展示了强大的泛化能力和时间一致性。

Conclusion: 该方法通过预训练和视频扩散模型先验，显著提升了视频抠图的性能，适用于多样化的真实场景。
...
### [194] [Mem4D: Decoupling Static and Dynamic Memory for Dynamic Scene Reconstruction](https://arxiv.org/abs/2508.07908)
...
<details>
  <summary>Details</summary>
Motivation: 动态场景的单目视频密集几何重建存在内存需求困境，现有方法在静态结构的长期稳定性和动态运动的高保真细节保留之间存在冲突。

Method: Mem4D采用双内存架构：瞬态动态内存（TDM）捕获高频动态细节，持久结构内存（PSM）保留长期静态信息。
...
### [195] [RSVLM-QA: A Benchmark Dataset for Remote Sensing Vision Language Model-based Question Answering](https://arxiv.org/abs/2508.07918)
*Xing Zi,Jinghao Xiao,Yunxiao Shi,Xian Tao,Jun Li,Ali Braytee,Mukesh Prasad*

Main category: cs.CV

TL;DR: 论文介绍了RSVLM-QA数据集，一个用于遥感视觉问答（VQA）的大规模、内容丰富的数据集，旨在解决现有数据集在标注丰富性、问题多样性和推理能力评估方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有遥感VQA数据集在标注丰富性、问题多样性和推理能力评估方面存在局限，需要更全面的数据集来推动研究。

Method: 通过整合多个遥感数据集，利用GPT-4.1自动生成详细标注和复杂问题，并开发专门流程生成计数问答对。

Result: RSVLM-QA包含13,820张图像和162,373个VQA对，统计分析和基准实验显示其标注深度和广度优于现有数据集。
...
### [196] [Safeguarding Generative AI Applications in Preclinical Imaging through Hybrid Anomaly Detection](https://arxiv.org/abs/2508.07923)
*Jakub Binda,Valentina Paneta,Vasileios Eleftheriadis,Hongkyou Chung,Panagiotis Papadimitroulas,Neo Christopher Chung*

Main category: cs.CV

TL;DR: 提出了一种混合异常检测框架，用于增强核医学中生成式AI的可靠性和合规性。


<details>
  <summary>Details</summary>
Motivation: 核医学中的生成式AI具有巨大潜力，但需要解决模型行为异常的高风险问题。
...
### [197] [TAG: A Simple Yet Effective Temporal-Aware Approach for Zero-Shot Video Temporal Grounding](https://arxiv.org/abs/2508.07925)
*Jin-Seop Lee,SungJoon Lee,Jaehan Ahn,YunSeok Choi,Jee-Hyong Lee*

Main category: cs.CV

TL;DR: 论文提出了一种名为TAG的零样本视频时间定位方法，通过时间池化、时间相干聚类和相似性调整，解决了现有方法的语义碎片化和相似性分布偏差问题。


<details>
  <summary>Details</summary>
Motivation: 现有零样本视频时间定位方法存在语义碎片化和相似性分布偏差问题，且依赖昂贵的大型语言模型（LLMs）。

Method: 提出TAG方法，结合时间池化、时间相干聚类和相似性调整，无需训练即可捕捉视频时间上下文并修正相似性分布。

Result: 在Charades-STA和ActivityNet Captions基准数据集上取得最先进结果，且不依赖LLMs。
...
### [198] [VOIDFace: A Privacy-Preserving Multi-Network Face Recognition With Enhanced Security](https://arxiv.org/abs/2508.07960)
...
Result: 在VGGFace2数据集上验证，VOIDFace提供数据控制、安全和隐私，同时保持性能。

Conclusion: VOIDFace提升了面部识别训练中的隐私、安全和效率，赋予用户数据控制权。
...
### [199] [TrackOR: Towards Personalized Intelligent Operating Rooms Through Robust Tracking](https://arxiv.org/abs/2508.07968)
*Tony Danjun Wang,Christian Heiliger,Nassir Navab,Lennart Bastian*

Main category: cs.CV

TL;DR: TrackOR框架通过3D几何特征实现手术室中长期多人跟踪和重识别，提升关联准确率11%，并支持离线轨迹恢复，为个性化智能系统提供基础。


<details>
  <summary>Details</summary>
Motivation: 为手术团队提供智能支持，改善患者结果，需解决手术室中长期多人跟踪和身份一致性问题。

Method: 提出TrackOR框架，利用3D几何特征进行在线跟踪和离线轨迹恢复。

Result: TrackOR在关联准确率上优于基线11%，并能生成分析就绪的轨迹数据。

Conclusion: 通过3D几何信息实现持久身份跟踪，为手术室个性化智能系统奠定基础，并支持团队效率和安全性的提升。
...
### [200] [The Escalator Problem: Identifying Implicit Motion Blindness in AI for Accessibility](https://arxiv.org/abs/2508.07989)
*Xiantao Zhang*

Main category: cs.CV

TL;DR: 多模态大语言模型（MLLMs）在盲人和视障（BVI）社区中具有巨大潜力，但存在‘电梯问题’——无法感知电梯运行方向，揭示了‘隐式运动盲区’的深层限制。


<details>
  <summary>Details</summary>
Motivation: 揭示当前模型在动态环境中感知连续运动的不足，强调其对用户信任的影响。

Method: 通过‘电梯问题’作为典型案例，分析帧采样范式在视频理解中的局限性。

Result: 指出模型在低信号连续运动感知上的失败，呼吁关注物理感知和用户需求。

Conclusion: 呼吁范式转变，从语义识别转向物理感知，并开发以用户为中心的新基准。
...
### [201] [Prompt-Guided Relational Reasoning for Social Behavior Understanding with Vision Foundation Models](https://arxiv.org/abs/2508.07996)
*Thinesh Thiyakesan Ponbagavathi,Chengzheng Yang,Alina Roitberg*

Main category: cs.CV

TL;DR: 论文提出ProGraD方法，通过可学习的群体提示和轻量级GroupContext Transformer改进群体活动检测，在复杂多群体场景中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有视觉基础模型（VFMs）虽提供优秀特征，但因其以对象为中心的训练数据，对群体动态建模不足，直接替换CNN骨干网络效果有限。

Method: 引入ProGraD方法，包括1）可学习的群体提示引导VFM注意力，2）轻量级GroupContext Transformer推断参与者-群体关联和集体行为。
...
### [202] [Sample-aware RandAugment: Search-free Automatic Data Augmentation for Effective Image Recognition](https://arxiv.org/abs/2508.08004)
*Anqi Xiao,Weichen Yu,Hongyuan Yu*

Main category: cs.CV

TL;DR: 提出了一种名为SRA的自动数据增强方法，解决了传统AutoDA方法搜索耗时和性能不足的问题，通过动态调整增强策略，在ImageNet上取得了78.31%的Top-1准确率。


<details>
  <summary>Details</summary>
Motivation: 主流AutoDA方法存在搜索过程耗时或性能不足的问题，限制了实际应用。

Method: 提出Sample-aware RandAugment (SRA)，结合启发式评分模块和不对称增强策略，动态调整增强策略。

Result: 在ImageNet上达到78.31%的Top-1准确率，兼容现有增强流程，泛化能力强。
...
### [203] [Mitigating Biases in Surgical Operating Rooms with Geometry](https://arxiv.org/abs/2508.08028)
*Tony Danjun Wang,Tobias Czempiel,Nassir Navab,Lennart Bastian*

Main category: cs.CV

TL;DR: 论文探讨了深度学习模型在手术室（OR）中因学习虚假相关性而导致的偏差问题，提出通过3D点云序列编码人员信息来解决这一问题。


<details>
  <summary>Details</summary>
Motivation: 手术室中标准化服装掩盖了识别特征，导致模型依赖无关视觉线索（如鞋子、眼镜），影响任务准确性。

Method: 采用梯度显著性分析揭示CNN模型的偏差，并提出基于3D点云序列的方法，分离身份相关的形状和运动模式。

Result: 实验显示，RGB模型在临床环境中性能下降12%，而几何方法能捕捉更有意义的生物特征。

Conclusion: 几何表示能更有效地建模手术室中的人员，为开发鲁棒智能辅助系统提供了方向。
...
### [204] [TRIDE: A Text-assisted Radar-Image weather-aware fusion network for Depth Estimation](https://arxiv.org/abs/2508.08038)
*Huawei Sun,Zixu Wang,Hao Feng,Julius Ott,Lorenzo Servadei,Robert Wille*

Main category: cs.CV

TL;DR: 该论文提出了一种结合雷达和相机数据的深度估计方法TRIDE，通过引入天气感知融合块和文本特征提取技术，显著提升了深度估计的准确性。


<details>
  <summary>Details</summary>
Motivation: 解决现有雷达-相机融合方法未考虑天气条件的问题，并探索语言描述在多模态深度估计中的应用。

Method: 提出TRIDE算法，结合雷达点信息和文本特征提取，并引入天气感知融合块动态调整雷达权重。

Result: 在nuScenes数据集上，MAE和RMSE分别提升了12.87%和9.08%。

Conclusion: TRIDE通过多模态融合和天气适应机制，显著提升了深度估计性能。
...
### [205] [S^2VG: 3D Stereoscopic and Spatial Video Generation via Denoising Frame Matrix](https://arxiv.org/abs/2508.08048)
...
Method: 通过深度估计将单目视频映射到预定义视角，采用帧矩阵修复框架合成缺失内容，并开发双更新方案提升修复质量。

Result: 实验表明，该方法在多种生成模型（如Sora、Lumiere等）上显著优于现有方法。

Conclusion: 该方法无需额外训练即可生成高质量的3D视频，适用于立体对或4D高斯优化的空间视频合成。
...
### [206] [PrIINeR: Towards Prior-Informed Implicit Neural Representations for Accelerated MRI](https://arxiv.org/abs/2508.08058)
...
Method: PrIINeR结合群体级知识和实例优化，通过双数据一致性约束，将k空间数据与先验信息对齐。

Result: 在NYU fastMRI数据集上，PrIINeR优于现有INR和部分学习型方法，显著提升结构保留和保真度，减少伪影。
...
### [207] [Information Bottleneck-based Causal Attention for Multi-label Medical Image Recognition](https://arxiv.org/abs/2508.08069)
...
<details>
  <summary>Details</summary>
Motivation: 当前方法在学习类特定特征时容易关注无关特征，导致解释性差。

Method: 提出结构因果模型（SCM）和IBCA，结合高斯混合多标签空间注意力和对比增强因果干预。

Result: 在Endo和MuReD数据集上，IBCA在多项指标上显著优于其他方法。

Conclusion: IBCA能有效学习类特定注意力，提升多标签分类的准确性和可解释性。
...
### [208] [ME-TST+: Micro-expression Analysis via Temporal State Transition with ROI Relationship Awareness](https://arxiv.org/abs/2508.08082)
*Zizheng Guo,Bochao Zou,Junbao Zhuo,Huimin Ma*

Main category: cs.CV

TL;DR: 论文提出ME-TST和ME-TST+两种基于状态空间模型的架构，通过时间状态转移机制改进微表情分析，结合多粒度ROI建模和慢快Mamba框架，显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 传统滑动窗口分类方法存在固定窗口长度和硬分类的局限性，且忽略了微表情检测与识别之间的内在联系。

Method: 采用时间状态转移机制替代窗口级分类，引入多粒度ROI建模和慢快Mamba框架，提出特征和结果层面的协同策略。
...
### [209] [Matrix-3D: Omnidirectional Explorable 3D World Generation](https://arxiv.org/abs/2508.08086)
...
Method: 训练轨迹引导的全景视频扩散模型，结合场景网格渲染，提出前馈和优化两种3D重建方法，并使用Matrix-Pano数据集进行训练。

Result: 实验表明，Matrix-3D在全景视频生成和3D世界生成中达到最先进性能。

Conclusion: Matrix-3D框架通过全景表示和条件视频生成，显著提升了3D世界生成的覆盖范围和质量。
...
### [210] [MDD-Net: Multimodal Depression Detection through Mutual Transformer](https://arxiv.org/abs/2508.08093)
*Md Rezwanul Haque,Md. Milon Islam,S M Taslim Uddin Raju,Hamdi Altaheri,Lobna Nassar,Fakhri Karray*
...
<details>
  <summary>Details</summary>
Motivation: 抑郁症严重影响个体身心健康，社交媒体数据的易获取性使其成为心理健康研究的理想资源。

Method: MDD-Net包含四个核心模块：声学特征提取、视觉特征提取、互变压器（用于特征关联与融合）和检测层。实验基于D-Vlog数据集进行。

Result: 实验结果显示，MDD-Net的F1-Score比现有技术提升了17.37%，性能显著优于现有方法。

Conclusion: MDD-Net在多模态抑郁症检测中表现出色，为心理健康研究提供了高效工具。
...
<details>
  <summary>Details</summary>
Motivation: 植物根系结构复杂且缺乏纹理信息，传统2D研究难以满足3D表型分析需求。
...
TL;DR: TBAC-UniImage是一种新型多模态理解与生成统一模型，通过深度整合预训练扩散模型和多模态大语言模型（MLLM），解决了现有方法的浅层连接和计算成本高的问题。
...
Method: 采用MLLM的多个不同层表示作为扩散模型的生成条件，将预训练生成器视为梯子，接收MLLM理解过程的多层次指导。

Result: TBAC-UniImage实现了更深层次、更细粒度的理解与生成统一。

Conclusion: 该方法为多模态理解与生成提供了一种高效且深度整合的新范式。
...
### [213] [Follow-Your-Shape: Shape-Aware Image Editing via Trajectory-Guided Region Control](https://arxiv.org/abs/2508.08134)
*Zeqian Long,Mingzhe Zheng,Kunyu Feng,Xinhua Zhang,Hongyu Liu,Harry Yang,Linfeng Zhang,Qifeng Chen,Yue Ma*

Main category: cs.CV

TL;DR: 论文提出Follow-Your-Shape框架，通过Trajectory Divergence Map（TDM）和Scheduled KV Injection机制，实现无需训练和掩码的精确形状编辑，同时保持非目标内容不变。


<details>
  <summary>Details</summary>
Motivation: 现有流式图像编辑模型在涉及大规模形状变换的复杂场景中表现不佳，容易改变非目标区域或无法实现预期形状变化。

Method: 通过比较反转和去噪路径的token-wise速度差异计算TDM，定位可编辑区域，并结合Scheduled KV Injection机制实现稳定编辑。

Result: 实验表明，该方法在形状替换任务中表现出卓越的编辑能力和视觉保真度。

Conclusion: Follow-Your-Shape框架在无需训练和掩码的情况下，实现了高精度和可控的形状编辑，并通过ReShapeBench验证了其有效性。
...
### [214] [FantasyStyle: Controllable Stylized Distillation for 3D Gaussian Splatting](https://arxiv.org/abs/2508.08136)
...
<details>
  <summary>Details</summary>
Motivation: 当前3DGS风格迁移方法存在多视图不一致导致风格冲突，以及VGG特征难以分离风格与内容的问题。

Method: 提出Multi-View Frequency Consistency增强视图一致性，Controllable Stylized Distillation抑制内容泄漏，并优化3D高斯分布。

Result: 实验表明，FantasyStyle在多种场景和风格下均优于现有方法，具有更高的风格化质量和视觉真实感。

Conclusion: FantasyStyle通过创新组件解决了现有挑战，显著提升了3D风格迁移的效果。
...
### [215] [Pindrop it! Audio and Visual Deepfake Countermeasures for Robust Detection and Fine Grained-Localization](https://arxiv.org/abs/2508.08141)
...
Method: 提交了深度伪造视频分类和定位的方法，参加了ACM 1M Deepfakes Detection Challenge。

Result: 在时间定位任务中表现最佳，分类任务中排名前四。

Conclusion: 论文提出的方法在深度伪造检测任务中表现出色，尤其在局部修改的检测上具有优势。
...
### [216] [Integrating Task-Specific and Universal Adapters for Pre-Trained Model-based Class-Incremental Learning](https://arxiv.org/abs/2508.08165)
*Yan Wang,Da-Wei Zhou,Han-Jia Ye*

Main category: cs.CV

TL;DR: 论文提出了一种结合任务特定和通用适配器（TUNA）的方法，用于解决类增量学习中的模块选择问题和共享知识不足的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有基于预训练模型的类增量学习方法通常冻结预训练网络，并通过轻量级模块（如适配器）适应增量任务，但模块选择错误和任务特定模块忽视共享知识会导致性能下降。

Method: 通过训练任务特定适配器捕捉关键特征，并引入基于熵的选择机制；同时利用适配器融合策略构建通用适配器，编码跨任务的共享特征。
...
### [217] [ReconDreamer-RL: Enhancing Reinforcement Learning via Diffusion-based Scene Reconstruction](https://arxiv.org/abs/2508.08170)
*Chaojun Ni,Guosheng Zhao,Xiaofeng Wang,Zheng Zhu,Wenkang Qin,Xinze Chen,Guanghong Jia,Guan Huang,Wenjun Mei*

Main category: cs.CV

TL;DR: ReconDreamer-RL通过结合视频扩散先验和物理建模，缩小了自动驾驶训练中的仿真与现实差距，并引入动态对抗代理和表亲轨迹生成器以覆盖更多极端场景。


<details>
  <summary>Details</summary>
Motivation: 现有仿真环境与现实条件差异大，且受限于训练数据分布，难以生成高质量传感器数据用于新轨迹或极端场景。

Method: 提出ReconDreamer-RL框架，结合视频扩散先验和物理建模的ReconSimulator，动态对抗代理（DAA）生成极端场景，表亲轨迹生成器（CTG）解决数据分布偏差。
...
### [218] [CD-TVD: Contrastive Diffusion for 3D Super-Resolution with Scarce High-Resolution Time-Varying Data](https://arxiv.org/abs/2508.08173)
...
Method: 结合对比学习和改进扩散模型，通过预训练学习退化模式和细节特征，仅需一个高分辨率时间步进行微调。
...
### [219] [3D Human Mesh Estimation from Single View RGBD](https://arxiv.org/abs/2508.08178)
...
### [220] [PP-Motion: Physical-Perceptual Fidelity Evaluation for Human Motion Generation](https://arxiv.org/abs/2508.08179)
*Sihan Zhao,Zixuan Wang,Tianyu Luan,Jia Jia,Wentao Zhu,Jiebo Luo,Junsong Yuan,Nan Xi*

Main category: cs.CV

TL;DR: 论文提出了一种物理标注方法和数据驱动指标PP-Motion，用于评估人体运动的物理和感知保真度，解决了现有方法在物理可行性与人类感知之间的差距问题。


<details>
  <summary>Details</summary>
Motivation: 人体运动生成在多个领域有广泛应用，但现有评估方法在物理可行性与人类感知之间存在差距，且主观标注不够精细。

Method: 引入物理标注方法，计算运动与物理法则的最小修改量，提出PP-Motion指标，结合皮尔逊相关损失和人类感知损失进行训练。

Result: 实验表明，PP-Motion在物理法则和人类感知保真度上均优于现有方法。

Conclusion: PP-Motion为运动保真度评估提供了更客观和精细的解决方案。
...
### [221] [THAT: Token-wise High-frequency Augmentation Transformer for Hyperspectral Pansharpening](https://arxiv.org/abs/2508.08183)
*Hongkun Jin,Hongcheng Jiang,Zejun Zhang,Yuan Zhang,Jia Fu,Tingfeng Li,Kai Luo*

Main category: cs.CV

TL;DR: 论文提出了一种名为THAT的Transformer框架，通过增强高频特征表示和令牌选择，解决了现有方法在冗余令牌和多尺度特征建模上的不足，提升了高光谱图像融合的性能。
...
Method: 提出THAT框架，包含Pivotal Token Selective Attention（PTSA）以减少冗余令牌，以及Multi-level Variance-aware Feed-forward Network（MVFN）以增强高频细节学习。
...
### [222] [KARMA: Efficient Structural Defect Segmentation via Kolmogorov-Arnold Representation Learning](https://arxiv.org/abs/2508.08186)
...
<details>
  <summary>Details</summary>
Motivation: 解决基础设施缺陷语义分割中因外观多变、成像条件恶劣和类别不平衡带来的挑战，同时减少参数数量以适应实时系统。

Method: 采用TiKAN模块、优化的特征金字塔结构和静态-动态原型机制，通过一维函数组合替代传统卷积。

Result: 在基准数据集上表现优异，参数减少97%（0.959M vs. 31.04M），推理速度适合实时部署。

Conclusion: KARMA为基础设施自动检测提供了高效且准确的解决方案。
...
### [223] [Reinforcement Learning in Vision: A Survey](https://arxiv.org/abs/2508.08189)
*Weijia Wu,Chen Gao,Joya Chen,Kevin Qinghong Lin,Qingwei Meng,Yiming Zhang,Yuke Qiu,Hong Zhou,Mike Zheng Shou*

Main category: cs.CV

TL;DR: 这篇综述总结了视觉强化学习（RL）领域的最新进展，包括问题定义、策略优化方法、代表性工作分类、算法设计、奖励工程、评估协议及未来挑战。


<details>
  <summary>Details</summary>
Motivation: 为研究者和从业者提供一个关于视觉RL领域的全面概述，并指出未来研究方向。

Method: 通过分类200多篇代表性工作，分析算法设计、奖励工程和基准测试进展，总结趋势如课程驱动训练和统一奖励建模。

Result: 总结了视觉RL的四大主题支柱（多模态大语言模型、视觉生成、统一模型框架、视觉-语言-动作模型）及其相关进展。

Conclusion: 视觉RL领域发展迅速，但仍面临样本效率、泛化性和安全部署等挑战，未来研究需关注这些方向。
...
### [224] [Spatial-ORMLLM: Improve Spatial Relation Understanding in the Operating Room with Multimodal Large Language Model](https://arxiv.org/abs/2508.08199)
*Peiqi He,Zhenhao Zhang,Yixiang Zhang,Xiongjun Zhao,Shaoliang Peng*

Main category: cs.CV

TL;DR: 提出Spatial-ORMLLM模型，仅用RGB数据实现手术室3D空间推理，解决多模态数据获取难和2D数据细节不足的问题。


<details>
  <summary>Details</summary>
Motivation: 手术室精确空间建模对临床任务至关重要，但现有方法依赖多模态数据且忽视3D能力。

Method: 结合2D输入与3D空间知识，通过Spatial-Enhanced Feature Fusion Block和端到端MLLM框架实现3D推理。
...
### [225] [SAGOnline: Segment Any Gaussians Online](https://arxiv.org/abs/2508.08219)
*Wentao Sun,Quanyun Wu,Hanqing Xu,Kyle Gao,Zhengsen Xu,Yiping Chen,Dedong Zhang,Lingfei Ma,John S. Zelek,Jonathan Li*

Main category: cs.CV

TL;DR: SAGOnline是一个轻量级、零样本的实时3D分割框架，通过视频基础模型和GPU加速算法，解决了3D高斯场景中的分割与跟踪问题，性能显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前3D高斯场景分割方法存在计算成本高、空间推理能力有限和无法同时跟踪多对象的问题，需要一种高效且一致的解决方案。

Method: SAGOnline采用两种创新方法：1）基于视频基础模型（如SAM2）的视图一致2D掩码传播；2）GPU加速的3D掩码生成与高斯级实例标注算法。

Result: 在NVOS和Spin-NeRF基准测试中，SAGOnline分别达到92.7%和95.2%的mIoU，推理速度比现有方法快15-1500倍（27毫秒/帧）。

Conclusion: SAGOnline实现了实时3D场景理解，为AR/VR和机器人应用提供了实用工具。
...
### [226] [Learning User Preferences for Image Generation Model](https://arxiv.org/abs/2508.08220)
*Wenyi Mo,Ying Ba,Tianyu Zhang,Yalong Bai,Biye Li*

Main category: cs.CV

TL;DR: 提出了一种基于多模态大语言模型的方法，通过对比偏好损失和偏好标记来学习个性化用户偏好，显著提升了偏好预测的准确性。
...
Method: 采用多模态大语言模型，引入对比偏好损失和可学习的偏好标记，以区分用户喜好并捕捉群体兴趣表征。

Result: 实验表明，该方法在偏好预测准确性上优于其他方法，能有效识别相似审美倾向的用户，并为图像生成提供更精准的指导。

Conclusion: 该方法通过动态捕捉用户偏好，显著提升了预测精度，为个性化推荐和图像生成提供了新思路。
...
### [227] [StableAvatar: Infinite-Length Audio-Driven Avatar Video Generation](https://arxiv.org/abs/2508.08248)
*Shuyuan Tu,Yueming Pan,Yinming Huang,Xintong Han,Zhen Xing,Qi Dai,Chong Luo,Zuxuan Wu,Yu-Gang Jiang*

Main category: cs.CV

TL;DR: StableAvatar是一个端到端的视频扩散变换器，能够生成无限长度的高质量视频，解决了现有模型在音频同步和身份一致性上的问题。


<details>
  <summary>Details</summary>
Motivation: 当前音频驱动的虚拟化身视频生成模型难以生成长视频且保持自然的音频同步和身份一致性。

Method: StableAvatar通过定制的训练和推理模块，结合时间步感知音频适配器和音频原生引导机制，解决了音频建模问题，并引入动态加权滑动窗口策略增强视频平滑度。

Result: 实验证明StableAvatar在质量和数量上均优于现有方法。

Conclusion: StableAvatar通过创新的音频建模和推理策略，实现了高质量、无限长度的视频生成。
...
### [228] [ReferSplat: Referring Segmentation in 3D Gaussian Splatting](https://arxiv.org/abs/2508.08252)
...
<details>
  <summary>Details</summary>
Motivation: 推动具身AI发展，解决3D场景中基于语言描述的目标分割问题，尤其是遮挡或不可见物体的识别。

Method: 提出ReferSplat框架，显式建模3D高斯点与自然语言表达的空间关系。

Result: ReferSplat在新任务R3DGS和3D开放词汇分割基准上达到最优性能。
...
### [229] [Learning an Implicit Physics Model for Image-based Fluid Simulation](https://arxiv.org/abs/2508.08254)
...
<details>
  <summary>Details</summary>
Motivation: 人类能够从静态图像想象4D场景（运动与3D几何），现有方法常违反物理规律，导致动画不真实。

Method: 使用物理约束的神经网络预测表面点运动，结合Navier-Stokes方程，并通过3D高斯特征捕捉外观。

Result: 实验表明，该方法能生成物理合理的动画，性能显著优于现有方法。

Conclusion: 该方法成功模拟了人类对4D场景的想象能力，为流体动画生成提供了新思路。
...
### [230] [Solving Pasur Using GPU-Accelerated Counterfactual Regret Minimization](https://arxiv.org/abs/2508.06559)
*Sina Baghal*

Main category: cs.AI

TL;DR: 本文介绍了一个基于CUDA加速的Pasur纸牌游戏计算框架，通过高效内存管理和CFR算法计算近纳什均衡。


<details>
  <summary>Details</summary>
Motivation: 解决Pasur游戏规则复杂和游戏树庞大的挑战，为类似多轮决策问题提供通用框架。

Method: 使用PyTorch CUDA张量处理规则，分解游戏树为状态和继承分数，采用逐轮反向训练策略。

Result: 构建了包含超过10^9节点的完整游戏树，并通过大规模自对局估计牌组公平值。

Conclusion: 该框架可扩展至其他多轮决策问题，如回合制策略游戏或金融市场序列交易。
...
### [231] [Operationalizing Serendipity: Multi-Agent AI Workflows for Enhanced Materials Characterization with Theory-in-the-Loop](https://arxiv.org/abs/2508.06569)
...
<details>
  <summary>Details</summary>
Motivation: 现代自主实验室虽然高效，但可能忽略意外发现，SciLink旨在填补这一空白，促进科学探索中的意外发现。

Method: 采用混合AI策略，结合机器学习模型和大型语言模型，将原始数据转化为可验证的科学主张，并评估其新颖性。

Result: SciLink在多种研究场景中表现出色，能够整合实时专家指导并提议后续实验。

Conclusion: SciLink不仅提高效率，还主动培育意外发现的环境，弥合自动化实验与开放式科学探索之间的差距。
...
### [232] [IRL-VLA: Training an Vision-Language-Action Policy via Reward World Model](https://arxiv.org/abs/2508.06571)
...
TL;DR: 本文提出IRL-VLA框架，通过三阶段方法结合模仿学习和逆强化学习，解决自动驾驶中VLA模型的闭环训练问题，并在NAVSIM v2和CVPR2025挑战赛中取得优异表现。
...
Result: 在NAVSIM v2和CVPR2025挑战赛中表现优异。
...
### [233] [CountQA: How Well Do MLLMs Count in the Wild?](https://arxiv.org/abs/2508.06585)
*Jayant Sravan Tamarapalli,Rynaa Grover,Nilay Pande,Sahiti Yerramilli*

Main category: cs.AI

TL;DR: 论文提出CountQA基准测试，用于评估多模态大语言模型（MLLMs）在复杂场景中的物体计数能力，发现当前模型表现不佳。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在物体计数能力上存在严重不足，限制了其实际应用的可靠性，而现有基准测试未能充分评估这一能力。

Method: 引入CountQA基准测试，包含1,500多个真实世界高密度、杂乱和遮挡场景的问题-答案对，评估15种主流MLLMs。

Result: 表现最佳的模型准确率仅为42.9%，且随着物体数量增加性能下降。

Conclusion: CountQA为改进MLLMs的计数能力提供了诊断工具，推动模型在描述流畅的同时具备数值和空间感知能力。
...
### [234] [Formal Concept Analysis: a Structural Framework for Variability Extraction and Analysis](https://arxiv.org/abs/2508.06668)
*Jessie Galasso*
...
<details>
  <summary>Details</summary>
Motivation: FCA在知识表示和发现中具有潜力，但其数学基础使得如何利用其属性进行变异性分析并不直观。

Method: 通过筛选FCA框架中对变异性分析至关重要的属性，并解释其在概念结构中的应用。

Result: 明确了FCA中可用于变异性分析的关键属性及其解读方法。

Conclusion: 本文为FCA在变异性分析中的应用提供了实用指导，填补了理论与实际应用之间的部分鸿沟。
...
### [235] [Zero-Shot Cellular Trajectory Map Matching](https://arxiv.org/abs/2508.06674)
*Weijie Shi,Yue Cui,Hao Chen,Jiaming Li,Mengze Li,Jia Zhu,Jiajie Xu,Xiaofang Zhou*

Main category: cs.AI

TL;DR: 论文提出了一种基于像素的轨迹校准方法，用于零样本蜂窝轨迹地图匹配（CTMM），通过迁移地理空间知识校准轨迹，并结合高斯混合模型和时空感知模块提升性能。


<details>
  <summary>Details</summary>
Motivation: 当前CTMM方法依赖区域特定数据，难以适应未探索区域，需开发无需额外训练的零样本方法。

Method: 提出像素轨迹校准助手，结合高斯混合模型和时空感知模块，优化路径查找算法。
...
### [236] [Probabilistic Circuits for Knowledge Graph Completion with Reduced Rule Sets](https://arxiv.org/abs/2508.06706)
...
<details>
  <summary>Details</summary>
Motivation: 规则方法在知识图谱补全中可解释性强，但需要大量规则才能达到高性能，这会降低可解释性。

Method: 从训练数据中发现规则上下文，并利用概率电路学习这些上下文的概率分布，以减少规则数量。

Result: 规则数量减少70-96%，性能优于基线31倍，保留91%的峰值性能。

Conclusion: 方法在8个基准数据集上验证有效，为基于规则的推理提供了新思路。
...
### [237] [GLIDR: Graph-Like Inductive Logic Programming with Differentiable Reasoning](https://arxiv.org/abs/2508.06716)
*Blair Johnson,Clayton Kerce,Faramarz Fekri*

Main category: cs.AI

TL;DR: GLIDR是一种可微分的归纳逻辑编程方法，通过更灵活的规则结构和消息传递算法提升知识图谱任务的性能。
...
Method: GLIDR采用可微分消息传递算法，支持分支和循环等复杂规则结构。

Result: GLIDR在知识图谱补全任务中显著优于现有方法，且能与其他模态数据结合优化。

Conclusion: GLIDR提供了高性能、鲁棒性强的规则学习方法，并支持端到端优化。
...
### [238] [ParBalans: Parallel Multi-Armed Bandits-based Adaptive Large Neighborhood Search](https://arxiv.org/abs/2508.06736)
*Alican Yilmaz,Junyang Cai,Serdar Kadioglu,Bistra Dilkina*

Main category: cs.AI

TL;DR: ParBalans扩展了Balans，通过并行化提升混合整数规划问题的求解效率，性能媲美商业求解器Gurobi。


<details>
  <summary>Details</summary>
Motivation: 混合整数规划问题计算资源需求高，并行化是加速求解的关键策略。Balans的并行潜力尚未充分探索。

Method: 提出ParBalans，结合求解器和算法级并行化，优化Balans的性能。

Result: 实验显示ParBalans在复杂问题上表现优异，与Gurobi相当。

Conclusion: ParBalans有效提升了并行化能力，为大规模MIP问题提供了高效解决方案。
...
### [239] [Topology Generation of UAV Covert Communication Networks: A Graph Diffusion Approach with Incentive Mechanism](https://arxiv.org/abs/2508.06746)
*Xin Tang,Qian Chen,Fengshun Li,Youchun Gong,Yinqiu Liu,Wen Tian,Shaowen Qin,Xiaohuan Li*

Main category: cs.AI

TL;DR: 提出了一种结合图扩散策略优化（GDPO）和Stackelberg博弈（SG）激励机制的自组织无人机网络框架，以解决动态移动性和暴露风险带来的挑战。
...
Conclusion: 该框架能有效应对无人机网络的动态性和隐蔽通信需求。
...
### [240] [Pushing the Envelope of LLM Inference on AI-PC](https://arxiv.org/abs/2508.06753)
*Evangelos Georganas,Dhiraj Kalamkar,Alexander Heinecke*

Main category: cs.AI

TL;DR: 本文介绍了针对超低比特LLM模型（1/1.58/2比特）的高效推理运行时优化，通过设计优化的微内核并集成到PyTorch-TPP框架中，显著提升了推理性能。


<details>
  <summary>Details</summary>
Motivation: 超低比特LLM模型在资源受限环境中具有潜力，但现有推理运行时的计算效率尚未充分探索，因此需要优化以实现高效部署。

Method: 设计并实现了针对现代CPU优化的1比特和2比特微内核，将其集成到PyTorch-TPP框架中，进行端到端推理测试。

Result: 优化后的运行时在2比特模型上比当前SOTA运行时bitnet.cpp快2.2倍，比16比特模型推理快7倍。

Conclusion: 优化后的运行时推动了AI PC和边缘设备上的LLM推理效率，为超低比特LLM模型的高效部署铺平了道路。
...
### [241] [A Fuzzy Logic Prompting Framework for Large Language Models in Adaptive and Uncertain Tasks](https://arxiv.org/abs/2508.06754)
*Vanessa Figueiredo*
...
Conclusion: 该框架不仅适用于教育领域，还可扩展至其他交互密集型任务，为不确定或动态环境中的LLM行为提供可复用方法。
...
### [242] [Natural Language-Driven Viewpoint Navigation for Volume Exploration via Semantic Block Representation](https://arxiv.org/abs/2508.06823)
*Xuan Zhao,Jun Tao*

Main category: cs.AI

TL;DR: 提出了一种基于自然语言交互的框架，用于增强体数据探索，通过CLIP Score和强化学习优化视角选择。


<details>
  <summary>Details</summary>
Motivation: 体数据探索对科学数据集解释至关重要，但缺乏领域专业知识的用户难以选择最佳视角。

Method: 框架将体数据块编码以区分结构，结合CLIP Score提供语义信息，并通过强化学习搜索符合用户意图的视角。

Result: 方法自动化视角选择，提高了体数据导航效率并增强了复杂科学现象的可解释性。

Conclusion: 该框架通过自然语言交互和语义指导，显著改善了体数据探索的效率和用户体验。
...
### [243] [Remote Sensing Image Intelligent Interpretation with the Language-Centered Perspective: Principles, Methods and Challenges](https://arxiv.org/abs/2508.06832)
*Haifeng Li,Wang Guo,Haiyang Wu,Mengwei Wu,Jipeng Zhang,Qing Zhu,Yu Liu,Xin Huang,Chao Tao*

Main category: cs.AI

TL;DR: 论文主张从视觉为中心的遥感图像解释转向以语言为中心的框架，利用大语言模型（LLMs）作为认知中心，整合感知、任务、知识和行动空间，实现统一理解、推理和决策。


<details>
  <summary>Details</summary>
Motivation: 现有视觉为中心的模型在多模态推理、语义抽象和交互决策方面存在局限，缺乏统一的理论框架解释语言的认知作用。

Method: 提出基于全局工作空间理论（GWT）的语言为中心框架，将LLMs作为认知中心，整合多模态表示、知识关联及推理决策。

Result: 构建了全局工作空间驱动的解释机制，总结了语言为中心解决方案如何应对核心挑战。

Conclusion: 为下一代遥感解释系统提供概念基础，并规划了认知驱动智能地理空间分析的路线图。
...
### [244] [Multi-level Advantage Credit Assignment for Cooperative Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2508.06836)
...
<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习（MARL）中，信用分配是一个关键挑战，需要评估每个智能体对共享奖励的贡献。任务多样性导致智能体协作方式不同，奖励可能分配给不同且重叠的智能体子集。

Method: 论文提出MACA方法，通过多级优势函数进行显式反事实推理，捕捉个体、联合和相关动作的贡献。使用注意力框架识别智能体关系并构建多级优势指导策略学习。

Result: 在Starcraft v1&v2任务上的实验表明，MACA在复杂信用分配场景中表现优异。

Conclusion: MACA通过多级优势信用分配，有效解决了多智能体协作中的信用分配问题，适用于多样化任务。
...
### [245] [MDK12-Bench: A Comprehensive Evaluation of Multimodal Large Language Models on Multidisciplinary Exams](https://arxiv.org/abs/2508.06851)
...
TL;DR: MDK12-Bench是一个多学科大规模基准测试，用于评估多模态大语言模型（MLLMs）的智能表现，覆盖六个学科、141K实例和6,225个知识点。
...
Method: 提出MDK12-Bench基准和动态评估框架，引入视觉、文本和问题形式的变化，并评估知识驱动的推理能力。

Result: 发现当前MLLMs在多个方面存在局限性，为提升模型鲁棒性和可解释性提供指导。
...
### [246] [MeteorPred: A Meteorological Multimodal Large Model and Dataset for Severe Weather Event Prediction](https://arxiv.org/abs/2508.06859)
*Shuo Tang,Jian Xu,Jiadong Zhang,Yi Chen,Qizhao Jin,Lingdong Shen,Chenglin Liu,Shiming Xiang*

Main category: cs.AI

TL;DR: 论文提出了一种基于多模态大模型（MMLM）的端到端AI天气预测系统，解决了现有方法在样本稀缺、数据对齐和多模态处理上的挑战，并通过实验验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 当前严重天气预警系统依赖人工专家解释，存在主观性和操作负担。AI技术的发展为自动化预测提供了可能，但面临样本稀缺、数据对齐和多模态处理等挑战。

Method: 构建了大规模多模态数据集MP-Bench，并开发了MMLM模型，通过自适应融合模块处理4D气象数据，实现动态特征提取与整合。

Result: 实验表明MMLM在多种任务中表现优异，验证了其在严重天气理解和自动化预测中的有效性。

Conclusion: MMLM为AI驱动的自动化天气预测系统迈出了关键一步，数据集和代码将公开。
...
### [247] [Pushdown Reward Machines for Reinforcement Learning](https://arxiv.org/abs/2508.06894)
*Giovanni Varricchione,Toryn Q. Klassen,Natasha Alechina,Mehdi Dastani,Brian Logan,Sheila A. McIlraith*

Main category: cs.AI

TL;DR: 本文提出了推下奖励机（pdRMs），基于确定性下推自动机扩展了奖励机（RMs），能够识别和奖励确定性上下文无关语言表示的行为，提高了表达能力。


<details>
  <summary>Details</summary>
Motivation: 奖励机（RMs）在强化学习中提高了样本效率，但其表达能力有限，仅能处理正则语言。本文旨在扩展RMs以支持更复杂的确定性上下文无关语言。

Method: 提出了推下奖励机（pdRMs），并设计了两种策略：一种能访问整个堆栈，另一种仅能访问堆栈顶部的k个符号。提出了检查两种策略最优性的方法。

Result: 理论证明了pdRMs的表达能力，并提供了空间复杂度分析。实验表明pdRMs能有效训练代理完成确定性上下文无关语言表示的任务。
...
### [248] [GDBA Revisited: Unleashing the Power of Guided Local Search for Distributed Constraint Optimization](https://arxiv.org/abs/2508.06899)
*Yanchen Deng,Xinrun Wang,Bo An*

Main category: cs.AI

TL;DR: 论文提出了一种名为DGLS的新方法，用于改进分布式约束优化问题（DCOPs）的局部搜索算法，解决了GDBA的局限性，并在实验中表现出优越性能。


<details>
  <summary>Details</summary>
Motivation: GDBA在解决DCOPs时容易陷入局部最优，且性能提升有限。论文旨在通过分析GDBA的不足并提出改进方法，提升算法的性能。

Method: 提出DGLS框架，包括自适应违反条件、惩罚蒸发机制和同步惩罚更新方案，以解决GDBA的三大问题。

Result: DGLS在标准基准测试中显著优于现有基线方法，尤其在结构化问题上表现突出。

Conclusion: DGLS通过改进GDBA的不足，显著提升了DCOPs的求解性能，具有理论和实际应用价值。
...
### [249] [Automated Formalization via Conceptual Retrieval-Augmented LLMs](https://arxiv.org/abs/2508.06931)
...
<details>
  <summary>Details</summary>
Motivation: 解决交互式定理证明器（ITPs）手动形式化的劳动密集性和专家知识需求，以及自动形式化中的模型幻觉和语义鸿沟问题。

Method: 提出CRAMF框架，通过从Mathlib4构建概念定义知识库，结合上下文查询增强和双通道混合检索策略，提升形式化定义的检索精度。

Result: 在miniF2F、ProofNet和AdvancedMath基准测试中，CRAMF显著提升了翻译准确性，最高相对改进达62.1%，平均为29.9%。

Conclusion: CRAMF能够无缝集成到基于LLM的自动形式化器中，有效提升形式化准确性和效率。
...
### [250] [Intrinsic Explainability of Multimodal Learning for Crop Yield Prediction](https://arxiv.org/abs/2508.06939)
*Hiba Najjar,Deepak Pathak,Marlon Nuske,Andreas Dengel*

Main category: cs.AI

TL;DR: 该研究利用基于Transformer的多模态学习模型，结合自注意力机制，提出两种特征归因方法（AR和GA）和一种模态归因方法（WMA），用于解释作物产量预测任务，并验证其优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 多模态学习在农业中的应用需要复杂的架构，但模型可解释性常被忽视。研究旨在利用Transformer的固有可解释性，提升作物产量预测模型的可解释性。

Method: 使用四种输入模态（多光谱卫星、天气时间序列、地形高程图和土壤属性），基于自注意力机制提出AR、GA和WMA方法，并与Shapley值采样（SVS）对比。

Result: Transformer模型在子田和田块级别分别比卷积和循环网络高0.10和0.04的R2分数；AR在时间归因上更稳健；WMA揭示了模态归因的差异模式。

Conclusion: 基于Transformer的模型在性能和可解释性上均优于传统方法，AR和WMA为多模态学习提供了有效的解释工具。
...
### [251] [Large Language Models Do Not Simulate Human Psychology](https://arxiv.org/abs/2508.06950)
*Sarah Schröder,Thekla Morgenroth,Ulrike Kuhl,Valerie Vaquet,Benjamin Paaßen*

Main category: cs.AI

TL;DR: 论文警告不要用大语言模型（LLMs）替代人类参与者进行心理学研究，并通过理论和实证证据表明LLMs无法模拟人类心理学。


<details>
  <summary>Details</summary>
Motivation: 探讨LLMs是否能模拟人类心理学并替代人类参与者，以解决心理学研究中的潜在问题。

Method: 通过概念论证和实证研究，分析LLMs在心理学研究中的表现，尤其是对细微语义变化的反应差异。

Result: LLMs对语义变化的反应与人类不一致，且不同模型间差异显著，表明其不可靠。

Conclusion: LLMs无法模拟人类心理学，心理学研究应将其视为需验证的工具，而非替代人类参与者。
...
### [252] [DatasetResearch: Benchmarking Agent Systems for Demand-Driven Dataset Discovery](https://arxiv.org/abs/2508.06960)
...
<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的快速发展，数据可用性成为AI开发的瓶颈，许多有价值的数据集分散在不同平台。论文旨在探索AI代理是否能超越传统搜索，自主发现满足用户需求的任何数据集。

Method: 论文引入DatasetResearch基准，评估AI代理在208个真实需求中的表现，采用三维评估框架分析搜索代理和合成代理的能力。

Result: 结果显示，即使是先进的深度研究系统，在DatasetResearch-pro子集上的得分仅为22%，表明当前技术与完美数据集发现之间存在巨大差距。

Conclusion: 论文为数据集发现代理建立了首个严格基准，揭示了搜索代理和合成代理在不同任务中的优势与局限性，为下一代自改进AI系统奠定了基础。
...
### [253] [MASteer: Multi-Agent Adaptive Steer Strategy for End-to-End LLM Trustworthiness Repair](https://arxiv.org/abs/2508.06963)
*Changqing Li,Tianlin Li,Xiaohan Zhang,Aishan Liu,Li Pan*

Main category: cs.AI

TL;DR: MASteer是一个基于表示工程的端到端框架，用于修复大语言模型的可信性问题，通过自动化生成样本和自适应策略选择，显著提升性能。
...
Method: MASteer结合AutoTester（多智能体生成样本）和AutoRepairer（自适应策略选择），实现自动化可信性修复。
...
### [254] [DSperse: A Framework for Targeted Verification in Zero-Knowledge Machine Learning](https://arxiv.org/abs/2508.06972)
...
<details>
  <summary>Details</summary>
Motivation: 在分布式零知识机器学习的背景下，DSperse旨在避免全模型电路化的高成本和僵化性，通过验证关键子计算实现信任最小化。

Method: DSperse采用模块化设计，支持对推理管道的部分或全部进行验证（称为“切片”），并通过审计、复制或经济激励确保全局一致性。

Result: 实验评估了多种证明系统，展示了切片和非切片配置下的内存使用、运行时间和电路行为。

Conclusion: DSperse通过灵活的验证边界设计，支持可扩展的、针对性的验证策略，适用于多样化部署需求。
...
### [255] [Simulating Biological Intelligence: Active Inference with Experiment-Informed Generative Model](https://arxiv.org/abs/2508.06980)
*Aswin Paul,Moein Khajehnejad,Forough Habibollahi,Brett J. Kagan,Adeel Razi*

Main category: cs.AI

TL;DR: 本文提出了一种基于主动推理的框架，用于建模自主代理的决策过程，结合生物神经元网络的实验数据，展示了记忆学习和预测规划在智能决策中的作用。


<details>
  <summary>Details</summary>
Motivation: 随着人工智能的快速发展，理解自主代理的有目的行为对开发安全高效系统至关重要。生物神经元网络可能提供更高效率和可解释性。

Method: 采用主动推理理论，结合实验数据构建生成模型，模拟游戏环境中的决策过程。
...
### [256] [Efficient and Reliable Hitting-Set Computations for the Implicit Hitting Set Approach](https://arxiv.org/abs/2508.07015)
*Hannes Ihalainen,Dieter Vandesande,André Schidler,Jeremias Berg,Bart Bogaerts,Matti Järvisalo*

Main category: cs.AI

TL;DR: 论文探讨了隐式命中集（IHS）方法中替代优化技术的可行性，比较了伪布尔推理和随机局部搜索，发现商业整数规划求解器效率高但存在数值不稳定问题，而基于伪布尔推理的精确计算可提供正确性证明。


<details>
  <summary>Details</summary>
Motivation: 研究隐式命中集方法中优化技术的替代方案，以解决商业整数规划求解器可能导致的数值不稳定问题，并提供正确性证明。

Method: 采用伪布尔推理和随机局部搜索作为替代优化技术，并与商业整数规划求解器进行比较。

Result: 商业整数规划求解器效率最高但存在数值不稳定问题；基于伪布尔推理的精确计算在效率上可与之竞争，并能提供正确性证明。

Conclusion: 伪布尔推理可作为隐式命中集计算的有效替代方案，尤其适用于需要正确性证明的场景。
...
### [257] [MultiMedEdit: A Scenario-Aware Benchmark for Evaluating Knowledge Editing in Medical VQA](https://arxiv.org/abs/2508.07022)
*Shengtao Wen,Haodong Chen,Yadong Wang,Zhongying Pan,Xiang Chen,Yu Tian,Bo Qian,Dong Liang,Sheng-Jun Huang*

Main category: cs.AI

TL;DR: MultiMedEdit是首个针对临床多模态任务的知识编辑（KE）基准，揭示了当前方法在复杂临床工作流中的局限性，并提供了未来开发临床鲁棒性KE技术的基础。


<details>
  <summary>Details</summary>
Motivation: 解决多模态医疗场景中知识编辑的空白，支持安全且可解释的临床决策。

Method: 提出MultiMedEdit基准，涵盖理解和推理任务类型，定义三维度量标准（可靠性、通用性和局部性），支持跨范式比较。

Result: 当前方法在泛化和长尾推理方面表现不佳，尤其在复杂临床工作流中。效率分析揭示了实际部署中的权衡。

Conclusion: MultiMedEdit不仅揭示了当前方法的局限性，还为未来开发临床鲁棒性KE技术奠定了基础。
...
### [258] [K-Dense Analyst: Towards Fully Automated Scientific Analysis](https://arxiv.org/abs/2508.07043)
*Orion Li,Vinayak Agarwal,Summer Zhou,Ashwin Gopinath,Timothy Kassis*

Main category: cs.AI

TL;DR: K-Dense Analyst是一个分层多代理系统，通过双循环架构实现自主生物信息学分析，性能超越现有最强语言模型。
...
Method: 采用分层多代理系统，通过双循环架构将复杂目标分解为可执行、可验证的任务，结合规划与验证执行。

Result: 在BixBench基准测试中，K-Dense Analyst准确率达29.2%，比最佳语言模型（GPT-5）高6.3个百分点。

Conclusion: 自主科学推理需要专门构建的系统，而不仅仅是增强的语言模型，K-Dense Analyst为此提供了重要进展。
...
### [259] [Towards Safer AI Moderation: Evaluating LLM Moderators Through a Unified Benchmark Dataset and Advocating a Human-First Approach](https://arxiv.org/abs/2508.07063)
...
Conclusion: 研究揭示了LLMs在内容审核中的关键不足，强调需要更多样化和代表性的数据以及人类参与，以提高模型的鲁棒性和可解释性。
...
### [260] [Designing a Feedback-Driven Decision Support System for Dynamic Student Intervention](https://arxiv.org/abs/2508.07107)
...
<details>
  <summary>Details</summary>
Motivation: 现有机器学习模型在教育领域多为静态，无法适应新数据（如干预后结果），需动态调整以提高预测效果。

Method: 结合LightGBM回归器和增量训练，支持实时更新模型；采用Flask构建交互界面，集成SHAP增强可解释性。

Result: 实验显示，模型重训练后RMSE降低10.7%，干预学生预测分数持续上调。

Conclusion: 该系统将静态预测转为自优化系统，推动教育分析向人本、数据驱动和响应式AI发展。
...
### [261] [Multi-Dimensional Summarization Agents with Context-Aware Reasoning over Enterprise Tables](https://arxiv.org/abs/2508.07186)
*Amit Dhanda*

Main category: cs.AI

TL;DR: 提出了一种基于多智能体的大型语言模型框架，用于跨维度总结结构化企业数据，显著优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 传统表格到文本模型无法有效处理层次结构和上下文感知变化，而这对商业报告任务至关重要。

Method: 采用多智能体流水线，包括数据切片、方差检测、上下文构建和基于LLM的生成。

Result: 框架在数据忠实度（83%）、显著变化覆盖率和决策关键洞察相关性（4.4/5）上表现优异。

Conclusion: 该方法在复杂商业场景中（如收入与单位量权衡）表现突出，显著提升了总结质量。
...
### [262] [EndoAgent: A Memory-Guided Reflective Agent for Intelligent Endoscopic Vision-to-Decision Reasoning](https://arxiv.org/abs/2508.07292)
*Yi Tang,Kaini Wang,Yang Chen,Guangquan Zhou*

Main category: cs.AI

TL;DR: EndoAgent是一种基于双记忆设计的AI代理，用于内窥镜图像分析，通过短期行动跟踪和长期经验学习提升推理能力，优于现有模型。
...
Result: 实验表明EndoAgent在视觉理解和语言生成任务上优于通用和医学多模态模型。
...
### [263] [Hallucination as a Computational Boundary: A Hierarchy of Inevitability and the Oracle Escape](https://arxiv.org/abs/2508.07334)
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型的幻觉现象是其可靠部署的核心障碍，本文旨在从理论和实践上解决这一问题。

Method: 构建'计算必要性层次结构'，证明幻觉的不可避免性；提出RAG作为预言机模型和持续学习的'内化预言机'机制。

Result: 证明了幻觉的边界，并提出了两种有效解决方案。

Conclusion: 通过形式化理论和实践路径，为大语言模型的可靠部署提供了新思路。
...
### [264] [Rethinking Domain-Specific LLM Benchmark Construction: A Comprehensiveness-Compactness Approach](https://arxiv.org/abs/2508.07353)
*Rubing Chen,Jiaxin Wu,Jian Wang,Xulu Zhang,Wenqi Fan,Chenghua Lin,Xiao-Yong Wei,Qing Li*

Main category: cs.AI

TL;DR: 论文提出了一种基于全面性-紧凑性（Comp-Comp）原则的迭代基准框架，挑战了现有领域特定基准主要依赖规模扩展的现状。


<details>
  <summary>Details</summary>
Motivation: 现有领域特定基准过于依赖规模扩展，忽略了语料和问答集设计对模型精确度和召回率的影响。

Method: 提出Comp-Comp框架，通过全面性确保语义召回，紧凑性提升精确度，指导语料和问答集构建。

Result: 案例研究中创建了XUBench，一个大规模、全面的封闭领域基准。
...
### [265] [Pentest-R1: Towards Autonomous Penetration Testing Reasoning Optimized via Two-Stage Reinforcement Learning](https://arxiv.org/abs/2508.07382)
...
Conclusion: 两阶段训练的协同效应是Pentest-R1成功的关键，为LLM在渗透测试中的应用提供了新思路。
...
### [266] [Invert4TVG: A Temporal Video Grounding Framework with Inversion Tasks for Enhanced Action Understanding](https://arxiv.org/abs/2508.07388)
*Zhaoyu Chen,Hongnan Lin,Yongwei Nie,Fei Ma,Xuemiao Xu,Fei Yu,Chengjiang Long*

Main category: cs.AI

TL;DR: 论文提出Invert4TVG框架，通过三个反转任务增强视频片段定位和动作理解，无需额外数据，显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法过度优化时间IoU，牺牲了语义动作理解，影响TVG的鲁棒性。

Method: 引入三个反转任务：动词补全、动作识别和视频描述，结合强化学习框架优化定位和语义。
...
### [267] [Generative AI for Strategic Plan Development](https://arxiv.org/abs/2508.07405)
*Jesse Ponnock*

Main category: cs.AI

TL;DR: 论文提出了一种利用生成式人工智能（GAI）为大型政府组织制定战略计划的模块化模型，并评估了BERTopic和非负矩阵分解（NMF）在主题建模中的表现。结果显示，这些技术能生成与战略计划中愿景元素相似的主题，其中BERTopic表现更优。


<details>
  <summary>Details</summary>
Motivation: 随着生成式人工智能（GAI）和大语言模型（LLM）的突破，越来越多的专业服务可以通过AI增强。本文旨在探索如何利用GAI为政府组织开发战略计划，并评估相关技术的有效性。

Method: 通过训练BERTopic和NMF模型，利用政府问责局（GAO）的大量报告进行主题建模，生成与战略计划中愿景元素相似的主题，并比较两者的表现。

Result: 结果显示，BERTopic和NMF均能生成与愿景元素100%相似的主题，其中BERTopic表现更优，超过一半的相关主题达到“中等”或“强”相关性。

Conclusion: GAI支持的战略计划开发对数十亿美元的行业具有重要影响，并帮助联邦政府满足关键公共利益需求。未来工作将聚焦于该概念的实践应用及其他模块的可行性。
...
### [268] [A Comprehensive Survey of Self-Evolving AI Agents: A New Paradigm Bridging Foundation Models and Lifelong Agentic Systems](https://arxiv.org/abs/2508.07407)
...
TL;DR: 该论文综述了自进化AI代理系统的技术，提出了统一框架并分类讨论了不同策略，同时探讨了领域专用方法及伦理问题。


<details>
  <summary>Details</summary>
Motivation: 现有AI代理系统依赖静态配置，难以适应动态环境，因此研究自进化技术以提升适应性。

Method: 提出统一框架，分析系统输入、代理系统、环境和优化器，并分类讨论自进化技术及领域专用策略。

Result: 系统总结了自进化代理技术，为开发更自适应、自主的系统奠定基础。

Conclusion: 自进化AI代理是未来发展方向，需关注评估、安全与伦理问题。
...
### [269] [Grounding Natural Language for Multi-agent Decision-Making with Multi-agentic LLMs](https://arxiv.org/abs/2508.07466)
*Dom Huh,Prasant Mohapatra*

Main category: cs.AI

TL;DR: 论文提出了一种多智能体大型语言模型（LLM）的系统框架，通过整合多智能体决策算法，提升LLM在协作和推理中的能力。


<details>
  <summary>Details</summary>
Motivation: 语言是协作和推理的基础，建立共同语言有助于清晰沟通和策略协调。本文旨在扩展LLM在多智能体环境中的应用。

Method: 提出系统框架，包括高级提示工程、有效记忆架构、多模态信息处理和微调对齐策略。

Result: 通过经典游戏场景的消融实验验证了设计选择的有效性。

Conclusion: 整合LLM与多智能体决策算法，为协作和推理提供了新方法。
...
### [270] [CP-Agent: Agentic Constraint Programming](https://arxiv.org/abs/2508.07468)
*Stefan Szeider*

Main category: cs.AI

TL;DR: 提出了一种基于纯代理策略的新方法，通过Python编码代理和精心设计的提示，成功解决了CP-Bench基准集中的所有101个问题。


<details>
  <summary>Details</summary>
Motivation: 解决将自然语言问题描述转化为形式化约束模型的挑战，避免传统固定流程方法的局限性。

Method: 使用基于ReAct原则的Python编码代理，通过IPython内核实现状态化代码执行和迭代开发，依赖提示注入领域知识。
...
### [271] [Democratizing Diplomacy: A Harness for Evaluating Any Large Language Model on Full-Press Diplomacy](https://arxiv.org/abs/2508.07485)
*Alexander Duffy,Samuel J Paech,Ishana Shastri,Elizabeth Karpinski,Baptiste Alloui-Cros,Tyler Marques,Matthew Lyle Olson*

Main category: cs.AI

TL;DR: 提出了首个无需微调或专门训练的评估工具，使本地大型语言模型（LLM）能玩完整版《外交》游戏。
...
Result: 实验表明，较大模型表现最佳，但较小模型也能胜任；还提出了关键状态分析协议，用于深入分析游戏关键时刻。

Conclusion: 该工具通过消除微调需求，普及了LLM战略推理能力的评估，并揭示了这些能力如何自然地从广泛使用的LLM中涌现。
...
### [272] [MCPToolBench++: A Large Scale AI Agent Model Context Protocol MCP Tool Use Benchmark](https://arxiv.org/abs/2508.07575)
*Shiqing Fan,Xichen Ding,Liang Zhang,Linjian Mo*

Main category: cs.AI

TL;DR: 论文提出MCPToolBench++，一个用于评估LLMs调用MCP工具性能的大规模多领域基准，解决了现有评估中的数据集不足、响应格式多样性和工具调用成功率不稳定等问题。


<details>
  <summary>Details</summary>
Motivation: 现有评估LLMs和AI Agents使用MCP工具的能力存在数据集不全面、响应格式多样、工具调用成功率不稳定等问题，亟需标准化解决方案。

Method: 提出MCPToolBench++基准，基于来自MCP市场和GitHub社区的4k多个MCP服务器，覆盖40多个类别，包含单步和多步工具调用数据集。

Result: 评估了具有代理能力的SOTA LLMs在该基准上的表现，并报告了结果。

Conclusion: MCPToolBench++为评估LLMs的MCP工具调用能力提供了有效的解决方案，填补了现有研究的空白。
...
### [273] [Optimization of Private Semantic Communication Performance: An Uncooperative Covert Communication Method](https://arxiv.org/abs/2508.07586)
*Wenjing Zhang,Ye Hu,Tao Luo,Zhilong Zhang,Mingzhe Chen*

Main category: cs.AI

TL;DR: 提出了一种新型隐蔽语义通信框架，通过友好干扰器和时间槽优化策略保护语义信息传输，避免被攻击者窃取。


<details>
  <summary>Details</summary>
Motivation: 研究如何在攻击者试图窃取语义信息的情况下，保护图像数据的语义传输隐私和质量。

Method: 部署友好干扰器干扰攻击者，并采用优先采样辅助的双延迟深度确定性策略梯度算法优化传输策略。

Result: 相比传统强化学习方法，隐私保护效果提升77.8%，语义信息传输质量提升14.3%。

Conclusion: 所提算法能有效提升语义通信的隐私性和传输质量，无需干扰器与服务器间的通信。
...
### [274] [HGMF: A Hierarchical Gaussian Mixture Framework for Scalable Tool Invocation within the Model Context Protocol](https://arxiv.org/abs/2508.07602)
...
<details>
  <summary>Details</summary>
Motivation: 解决LLM在大规模层次化工具库中选择工具时因上下文限制和噪声导致的低准确性和高计算成本问题。

Method: 提出HGMF框架，分两阶段：1) 用GMM聚类服务器并过滤；2) 对选定服务器的工具进行相同操作，生成高相关性候选集。

Result: 实验表明HGMF显著提升工具选择准确性并降低推理延迟。

Conclusion: HGMF是一种可扩展且高效的工具调用方法，适用于大规模工具库。
...
### [275] [ThinkTuning: Instilling Cognitive Reflections without Distillation](https://arxiv.org/abs/2508.07616)
*Aswin RRV,Jacob Dineen,Divij Handa,Md Nayem Uddin,Mihir Parmar,Chitta Baral,Ben Zhou*

Main category: cs.AI

TL;DR: ThinkTuning是一种基于GRPO的交互式训练方法，通过教师模型的反馈提升学生模型的推理能力，实验显示在多任务基准上有显著提升。


<details>
  <summary>Details</summary>
Motivation: 研究发现强化学习（RL）无法真正赋予模型新的推理能力，仅能激发已有行为。因此，提出如何让不具备思考能力的模型发展出这种能力的问题。

Method: 采用ThinkTuning方法，通过教师模型对学生模型的输出提供反馈，逐步引导其改进推理能力，类似于课堂中的师生互动。

Result: 实验结果显示，ThinkTuning在多个基准测试中平均提升3.85%，尤其在MATH-500、AIME和GPQA-Diamond任务上分别提升2.08%、2.23%和3.99%。

Conclusion: ThinkTuning通过教师模型的反馈有效提升了学生模型的推理能力，为模型自我改进提供了新思路。
...
### [276] [Multimodal AI Systems for Enhanced Laying Hen Welfare Assessment and Productivity Optimization](https://arxiv.org/abs/2508.07628)
*Daniel Essien,Suresh Neethirajan*

Main category: cs.AI

TL;DR: 论文提出利用多模态AI技术改进家禽福利监测，通过中间融合策略平衡性能与鲁棒性，并引入新评估工具和部署框架以解决实际应用中的挑战。


<details>
  <summary>Details</summary>
Motivation: 传统家禽福利监测依赖主观人工观察和单一传感器数据，无法全面反映现代农场中蛋鸡的多维度福利需求，亟需数据驱动的智能监测系统。

Method: 采用多模态AI技术，整合视觉、声音、环境和生理数据，提出中间融合策略，并引入域转移评分（DTS）和数据可靠性指数（DRI）作为评估工具。

Result: 中间融合策略在真实农场条件下表现最佳，提出的DTS和DRI工具能有效评估模型适应性和数据质量。

Conclusion: 多模态AI为家禽福利监测提供了从被动单模态到主动精准系统的转型基础，结合了生产效率和科学伦理。
...
### [277] [Breaking Down and Building Up: Mixture of Skill-Based Vision-and-Language Navigation Agents](https://arxiv.org/abs/2508.07642)
*Tianyi Ma,Yue Zhang,Zehao Wang,Parisa Kordjamshidi*

Main category: cs.AI

TL;DR: SkillNav是一个模块化框架，通过将导航任务分解为可解释的原子技能，并动态选择最适合的代理，显著提升了Vision-and-Language Navigation（VLN）的性能和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有VLN方法在复杂空间和时间推理场景中泛化能力不足，尤其是在未见过的环境中。

Method: 提出SkillNav框架，将导航任务分解为原子技能（如垂直移动、区域识别等），并引入基于VLM的路由器动态选择代理。

Result: 在R2R和GSA-R2R基准测试中达到最新最优性能，展示了强大的泛化能力。
...
### [278] [Disentangling Multiplex Spatial-Temporal Transition Graph Representation Learning for Socially Enhanced POI Recommendation](https://arxiv.org/abs/2508.07649)
*Jie Li,Haoye Dong,Zhengyang Wu,Zetao Zheng,Mingrong Lin*

Main category: cs.AI

TL;DR: DiMuST模型通过解耦多时空转换图的表示学习，提升了POI推荐的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法单独建模时空转换，导致关键节点表示不一致，引入冗余信息，降低模型性能。

Method: 提出DiMuST模型，使用解耦变分多图自动编码器（DAE），分离共享和私有分布，并通过PoE机制融合共享特征，对比约束去噪私有特征。
...
### [279] [1-2-3 Check: Enhancing Contextual Privacy in LLM via Multi-Agent Reasoning](https://arxiv.org/abs/2508.07667)
*Wenkai Li,Liwen Sun,Zhenxiang Guan,Xuhui Zhou,Maarten Sap*

Main category: cs.AI

TL;DR: 论文提出了一种多智能体框架，通过分解隐私推理任务来减少隐私信息泄露，实验表明其优于单智能体基线。
...
### [280] [EMPATHIA: Multi-Faceted Human-AI Collaboration for Refugee Integration](https://arxiv.org/abs/2508.07671)
*Mohamed Rayan Barhdadi,Mehmet Tuncel,Erchin Serpedin,Hasan Kurban*

Main category: cs.AI

TL;DR: EMPATHIA是一个多智能体框架，旨在解决难民整合中的文化、情感和伦理问题，通过模块化设计实现透明决策。


<details>
  <summary>Details</summary>
Motivation: 当前AI在难民整合中仅关注就业等狭隘目标，忽视了文化、情感和伦理等长期成功的关键维度。

Method: 基于Kegan的建构发展理论，EMPATHIA分为SEED、RISE和THRIVE三个模块，采用多智能体透明决策架构。

Result: 在UN Kakuma数据集上验证，达到87.4%的收敛率，并在五个接收国实现可解释的评估。

Conclusion: EMPATHIA通过平衡多元价值系统，为AI驱动的分配任务提供了可推广的框架，同时增强而非取代人类专业知识。
...
### [281] [Ethics2vec: aligning automatic agents and human preferences](https://arxiv.org/abs/2508.07673)
*Gianluca Bontempi*

Main category: cs.AI

TL;DR: 论文提出Ethics2Vec方法，通过向量化AI决策策略来评估其与人类价值观的对齐，解决伦理价值难以量化的问题。


<details>
  <summary>Details</summary>
Motivation: AI行为中隐含的伦理价值难以从人类角度理解，尤其是涉及不可比较的价值观时（如生命价值与治疗成本）。

Method: 扩展Anything2vec方法，将AI决策策略映射为多元向量表示，用于比较和评估与人类价值观的对齐。

Result: 提出了Ethics2Vec方法，适用于二元决策和自动控制场景（如自动驾驶）。

Conclusion: Ethics2Vec为量化伦理价值提供了可行路径，有助于AI与人类价值观的对齐。
...
### [282] [Symmetry-Aware Transformer Training for Automated Planning](https://arxiv.org/abs/2508.07743)
*Markus Fritzsche,Elliot Gestrin,Jendrik Seipp*

Main category: cs.AI

TL;DR: 论文提出了一种新的对比学习目标，使Transformer能够感知对称性，从而弥补其在自动规划领域的局限性。


<details>
  <summary>Details</summary>
Motivation: Transformer在自动规划领域应用受限，主要由于问题对称性导致组合爆炸，现有方法如PlanGPT难以从等效表示中学习。

Method: 提出了一种对称感知的对比学习目标，并结合架构改进，训练Transformer用于规划生成或启发式预测。
...
### [283] [Best-Effort Policies for Robust Markov Decision Processes](https://arxiv.org/abs/2508.07790)
*Alessandro Abate,Thom Badings,Giuseppe De Giacomo,Francesco Fabiano*

Main category: cs.AI

TL;DR: 论文研究了鲁棒马尔可夫决策过程（RMDPs）中多最优鲁棒策略的问题，提出了一种基于博弈论中优势与最佳努力概念的ORBE策略选择标准，并证明了其存在性和计算可行性。
...
Method: 提出ORBE策略选择标准，结合最坏情况和非对抗性情况下的表现，并设计算法计算ORBE策略。

Result: 证明了ORBE策略的存在性，分析了其结构，并通过数值实验验证了方法的可行性。

Conclusion: ORBE策略为RMDPs中的最优鲁棒策略提供了一种有原则的决策依据，计算开销小且实用。
...
### [284] [KIRETT: Knowledge-Graph-Based Smart Treatment Assistant for Intelligent Rescue Operations](https://arxiv.org/abs/2508.07834)
*Mubaris Nadeem,Johannes Zenkert,Lisa Bender,Christian Weber,Madjid Fathi*

Main category: cs.AI

TL;DR: 本文提出了一种基于知识图谱的智能系统，用于辅助急救人员在紧急情况下提供个性化医疗建议。


<details>
  <summary>Details</summary>
Motivation: 全球救援需求增加，急救人员需快速评估患者状况并提供优化治疗，但缺乏实时知识支持。

Method: 采用知识图谱作为核心知识表示，结合人工智能预识别技术，提供智能治疗建议。

Result: 系统能为急救人员提供实时的、基于AI的情境预识别和治疗推荐。

Conclusion: 知识图谱和AI技术的结合显著提升了急救效率和治疗质量。
...
### [285] [\(X\)-evolve: Solution space evolution powered by large language models](https://arxiv.org/abs/2508.07932)
*Yi Zhai,Zhiqiang Wei,Ruohan Li,Keyu Pan,Shuo Liu,Lu Zhang,Jianmin Ji,Wuyang Zhang,Yu Zhang,Yanyong Zhang*

Main category: cs.AI

TL;DR: 论文提出了一种名为X-evolve的新方法，通过进化解空间而非单个解，显著减少了LLM调用次数，并在多个复杂优化问题上取得了突破性成果。


<details>
  <summary>Details</summary>
Motivation: 当前结合LLM和EA的方法通常进化单个解，导致LLM调用成本高，限制了效率和可扩展性。

Method: X-evolve通过LLM生成可调程序，定义解空间，并利用基于分数的搜索算法高效探索，减少LLM调用。

Result: 在三个优化问题上取得突破：发现更大的cap set部分解、提高Shannon容量下界、生成优于标准策略的启发式算法。

Conclusion: X-evolve通过进化解空间显著提升了搜索效率，适用于高维复杂问题。
...
### [286] [Deep Reinforcement Learning with anticipatory reward in LSTM for Collision Avoidance of Mobile Robots](https://arxiv.org/abs/2508.07941)
...
<details>
  <summary>Details</summary>
Motivation: 在无通信和标识的受限环境中，减少机器人间的碰撞并提高稳定性。
...
### [287] [FEAT: A Multi-Agent Forensic AI System with Domain-Adapted Large Language Model for Automated Cause-of-Death Analysis](https://arxiv.org/abs/2508.07950)
...
TL;DR: FEAT是一个多智能体AI框架，用于自动化和标准化法医死亡调查，通过领域适应的大语言模型提高效率和一致性。


<details>
  <summary>Details</summary>
Motivation: 解决法医死亡调查中的劳动力短缺和诊断变异性问题，特别是在高负荷系统中如中国的法医基础设施。

Method: FEAT结合了任务分解的中央规划器、证据分析的专业本地求解器、迭代优化的记忆与反思模块，以及结论合成的全局求解器。

Result: 在多样化的中国案例中，FEAT优于现有AI系统，表现出强大的泛化能力和专家一致性。

Conclusion: FEAT是首个基于LLM的法医AI系统，结合AI效率和人类监督，提升法医服务的可靠性和可及性。
...
### [288] [Interpreting Fedspeak with Confidence: A LLM-Based Uncertainty-Aware Framework Guided by Monetary Policy Transmission Paths](https://arxiv.org/abs/2508.08001)
*Rui Yao,Qi Chai,Jinhai Yao,Siyuan Li,Junhao Chen,Qi Zhang,Hao Wang*

Main category: cs.AI

TL;DR: 本文提出了一种基于LLM的框架，用于解析美联储的Fedspeak并分类其货币政策立场，结合领域特定推理和动态不确定性解码模块，显著提升了分类准确性和模型可靠性。


<details>
  <summary>Details</summary>
Motivation: Fedspeak作为美联储的政策信号工具，对金融市场和经济预测具有重要影响，但其隐含的复杂语义和策略立场需要自动化解析。

Method: 采用LLM框架，结合货币政策传导机制的领域特定推理，并引入动态不确定性解码模块评估预测置信度。

Result: 实验结果显示，该框架在政策立场分析任务中达到最先进性能，且感知不确定性与模型错误率显著正相关。

Conclusion: 该框架有效提升了Fedspeak的解析能力，为金融预测和政策分析提供了可靠工具。
...
### [289] [Fitting Description Logic Ontologies to ABox and Query Examples](https://arxiv.org/abs/2508.08007)
*Maurice Funk,Marvin Grosser,Carsten Lutz*

Main category: cs.AI

TL;DR: 研究基于本体介导查询的拟合问题，确定是否存在满足正负例子的本体，并分析计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 解决在给定正负例子下，如何找到满足条件本体的拟合问题，为实际应用提供理论基础。

Method: 使用描述逻辑ALC和ALCI作为本体语言，涵盖多种查询语言（AQs、CQs、UCQs），并分析拟合问题的计算复杂度。

Result: 发现拟合问题对AQs和完整CQs是CONP复杂度，对CQs和UCQs是2EXPTIME完全复杂度。

Conclusion: 研究为不同查询语言下的本体拟合问题提供了明确的复杂度界限，适用于ALC和ALCI。
...
### [290] [AdaptFlow: Adaptive Workflow Optimization via Meta-Learning](https://arxiv.org/abs/2508.08053)
...
TL;DR: AdaptFlow是一个基于自然语言的元学习框架，通过双层优化方案学习通用工作流初始化，实现快速子任务适应，在多个任务中表现优异。
...
Method: 采用双层优化方案：内循环通过LLM生成的反馈优化子任务工作流，外循环更新共享初始化以跨任务表现良好。

Result: 在问答、代码生成和数学推理任务中，AdaptFlow表现优于基线方法，实现最先进结果。

Conclusion: AdaptFlow通过语言引导的修改实现通用化，适应性强，适用于多样化任务。
...
### [291] [FNBT: Full Negation Belief Transformation for Open-World Information Fusion Based on Dempster-Shafer Theory of Evidence](https://arxiv.org/abs/2508.08075)
*Meishen He,Wenjun Ma,Jiao Wang,Huijun Yue,Xiaoma Fan*

Main category: cs.AI

TL;DR: 提出了一种基于Dempster-Shafer理论的开放世界信息融合方法FNBT，解决了异构框架下的信息融合问题。
...
Method: 引入标准判断是否为开放世界任务，扩展框架以容纳异构元素，采用全否定机制转换质量函数。

Result: 理论满足质量函数不变性、遗传性和本质冲突消除；实验在分类任务中表现优异，解决了Zadeh反例。
...
### [292] [TeamMedAgents: Enhancing Medical Decision-Making of LLMs Through Structured Teamwork](https://arxiv.org/abs/2508.08115)
...
<details>
  <summary>Details</summary>
Motivation: 将人类团队合作的理论应用于AI多智能体系统，以提升医疗决策的准确性和协作效率。

Method: 基于Salas的'Big Five'模型，实现六个团队合作组件（如团队领导力、共享心智模型等），并在八个医疗基准数据集上评估。

Result: 在7/8的数据集上表现提升，通过消融实验揭示了不同任务和领域的最优团队配置。
...
### [293] [BlindGuard: Safeguarding LLM-based Multi-Agent Systems under Unknown Attacks](https://arxiv.org/abs/2508.08127)
...
<details>
  <summary>Details</summary>
Motivation: 现有监督防御方法依赖标记数据，实际应用中不切实际。BlindGuard旨在提供一种通用且实用的防御方案。

Method: 通过分层代理编码器捕捉个体、邻域和全局交互模式，并结合噪声注入和对比学习的检测器。

Result: 实验表明BlindGuard能有效检测多种攻击类型，并优于监督基线方法。

Conclusion: BlindGuard是一种高效且通用的无监督防御方法，适用于多智能体系统。
...
### [294] [From Natural Language to Solver-Ready Power System Optimization: An LLM-Assisted, Validation-in-the-Loop Framework](https://arxiv.org/abs/2508.08147)
*Yunkai Hu,Tianqiao Zhao,Meng Yue*

Main category: cs.AI

TL;DR: 提出了一种基于大语言模型（LLM）的代理，将电力系统优化场景的自然语言描述转换为可求解的数学公式，并通过验证和修复生成可靠解。


<details>
  <summary>Details</summary>
Motivation: 直接使用LLM生成解常导致不可行或次优结果，因其缺乏数值精度和约束处理能力。结合AI与传统优化框架，可提升决策效率。

Method: 集成领域感知提示与LLM，通过系统验证和迭代修复确保可行性，生成可求解的数学模型。

Result: 以机组组合问题为例，代理生成了最优或接近最优的调度方案及目标成本，验证了方法的可靠性。
...
### [295] [Transfer Learning with EfficientNet for Accurate Leukemia Cell Classification](https://arxiv.org/abs/2508.06535)
*Faisal Ahmed*

Main category: eess.IV

TL;DR: 研究通过迁移学习和数据增强技术，使用EfficientNet-B3模型在急性淋巴细胞白血病（ALL）分类中取得最佳性能。


<details>
  <summary>Details</summary>
Motivation: 急性淋巴细胞白血病的准确分类对早期诊断和治疗至关重要，但数据不平衡问题影响模型性能。

Method: 采用预训练的CNN模型（如ResNet50、ResNet101和EfficientNet变体），并通过数据增强平衡数据集。
...
### [296] [LWT-ARTERY-LABEL: A Lightweight Framework for Automated Coronary Artery Identification](https://arxiv.org/abs/2508.06874)
*Shisheng Zhang,Ramtin Gharleghi,Sonit Singh,Daniel Moses,Dona Adikari,Arcot Sowmya,Susann Beier*

Main category: eess.IV

TL;DR: 提出了一种结合解剖学知识和规则拓扑约束的轻量级方法，用于冠状动脉自动标记，性能优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 冠状动脉疾病是全球主要死因，CTCA是重要诊断工具，但手动分析耗时且复杂，现有自动标记方法存在不足。

Method: 整合解剖学知识和规则拓扑约束，提出轻量级方法。
...
### [297] [Fusion-Based Brain Tumor Classification Using Deep Learning and Explainable AI, and Rule-Based Reasoning](https://arxiv.org/abs/2508.06891)
*Melika Filvantorkaman,Mohsen Piri,Maral Filvan Torkaman,Ashkan Zabihi,Hamidreza Moradi*

Main category: eess.IV

TL;DR: 该研究提出了一种基于MobileNetV2和DenseNet121的集成深度学习框架，结合软投票策略和可解释AI模块，用于脑肿瘤分类，性能优于单一模型，并得到临床验证。
...
Method: 采用MobileNetV2和DenseNet121的集成框架，结合软投票策略，并引入Grad-CAM++和临床决策规则覆盖（CDRO）增强可解释性。
...
### [298] [Spatio-Temporal Conditional Diffusion Models for Forecasting Future Multiple Sclerosis Lesion Masks Conditioned on Treatments](https://arxiv.org/abs/2508.07006)
*Gian Mario Favero,Ge Ya Luo,Nima Fathi,Justin Szeto,Douglas L. Arnold,Brennan Nichyporuk,Chris Pal,Tal Arbel*

Main category: eess.IV

TL;DR: 提出了一种基于多模态数据的治疗感知时空扩散模型，用于预测多发性硬化症（MS）患者的未来病灶演变。


<details>
  <summary>Details</summary>
Motivation: 图像驱动的个性化医疗在异质性疾病（如MS）中具有潜力，但需要准确的病灶预测模型。

Method: 采用体素空间方法，结合MRI和治疗信息，预测未来时间点的病灶演变。

Result: 模型能准确预测六种不同治疗下患者的未来病灶，并展示了在临床任务中的实用性。

Conclusion: 该模型为MS的数据驱动预后提供了有力工具。
...
### [299] [Trustworthy Medical Imaging with Large Language Models: A Study of Hallucinations Across Modalities](https://arxiv.org/abs/2508.07031)
...
<details>
  <summary>Details</summary>
Motivation: LLMs在医学影像任务中常产生幻觉（错误但自信的输出），可能误导临床决策，因此需要系统研究其成因和影响。

Method: 研究分析了图像到文本（生成报告）和文本到图像（生成影像）任务中的幻觉现象，使用专家标准评估错误类型。

Result: 发现幻觉在解释性和生成性任务中均存在常见模式，揭示了模型架构和训练数据等因素的影响。
...
### [300] [3DGS-VBench: A Comprehensive Video Quality Evaluation Benchmark for 3DGS Compression](https://arxiv.org/abs/2508.07038)
*Yuke Xing,William Gordon,Qi Yang,Kaifa Yang,Jiarui Wang,Yiling Xu*

Main category: eess.IV

TL;DR: 3DGS-VBench是一个大规模视频质量评估数据集和基准测试，用于评估3D高斯溅射（3DGS）压缩算法的存储效率和视觉质量。
...
Method: 建立了包含660个压缩3DGS模型和视频序列的数据集，涵盖6种SOTA压缩算法，并通过50名参与者标注MOS分数。

Result: 数据集可靠，并评估了6种压缩算法和15种质量评估指标。
...
### [301] [SAGCNet: Spatial-Aware Graph Completion Network for Missing Slice Imputation in Population CMR Imaging](https://arxiv.org/abs/2508.07041)
*Junkai Liu,Nay Aung,Theodoros N. Arvanitis,Stefan K. Piechnik,Joao A C Lima,Steffen E. Petersen,Le Zhang*

Main category: eess.IV

TL;DR: 提出了一种名为SAGCNet的方法，用于解决MRI中缺失切片的合成问题，通过图结构和空间适配器提升性能。


<details>
  <summary>Details</summary>
Motivation: MRI中缺失或不可用的切片影响诊断准确性，现有方法难以建模3D数据的局部和全局依赖关系。

Method: SAGCNet包含两个创新模块：体积切片图补全模块和体积空间适配器组件，分别建模切片间关系和3D空间上下文。

Result: 在心脏MRI数据集上，SAGCNet在定量和定性上均优于现有方法，且在数据有限时仍表现优异。

Conclusion: SAGCNet有效解决了MRI缺失切片合成问题，为临床实践提供了更可靠的诊断工具。
...
### [302] [Large-scale Multi-sequence Pretraining for Generalizable MRI Analysis in Versatile Clinical Applications](https://arxiv.org/abs/2508.07165)
...
<details>
  <summary>Details</summary>
Motivation: 多序列MRI的异质性限制了深度学习模型的泛化能力，影响了临床实用性。

Method: 收集了336,476个体积MRI扫描，构建了最大的多器官多序列MRI预训练语料库，并提出了一种新的预训练范式。

Result: 在44个下游任务中，PRISM在39个任务中表现最佳，显著优于未预训练模型和现有基础模型。

Conclusion: PRISM为多序列MRI分析提供了可扩展的框架，增强了AI在放射学中的转化潜力。
...
### [303] [HaDM-ST: Histology-Assisted Differential Modeling for Spatial Transcriptomics Generation](https://arxiv.org/abs/2508.07225)
...
Method: HaDM-ST包括三个模块：(i) 语义蒸馏网络从H&E图像提取预测线索；(ii) 空间对齐模块实现与低分辨率ST的像素级对应；(iii) 通道感知对抗学习器用于基因级精细建模。

Result: 在200个基因的实验中，HaDM-ST在多种组织和物种中表现优于现有方法，提升了高分辨率ST预测的空间保真度和基因级一致性。

Conclusion: HaDM-ST通过结合H&E图像和低分辨率ST，显著提高了空间转录组学的分辨率和准确性。
...
### [304] [Towards Human-AI Collaboration System for the Detection of Invasive Ductal Carcinoma in Histopathology Images](https://arxiv.org/abs/2508.07875)
*Shuo Han,Ahmed Karam Eldaly,Solomon Sunday Oyelere*

Main category: eess.IV

TL;DR: 提出了一种人机交互的深度学习系统，结合EfficientNetV2S模型和专家反馈，提高浸润性导管癌（IDC）检测的准确性。


<details>
  <summary>Details</summary>
Motivation: 早期准确诊断IDC对提高患者生存率至关重要，人工智能与医学专家结合有望提升检测效率和精度。

Method: 采用人机交互（HITL）系统，先由EfficientNetV2S模型初步诊断，专家修正后反馈至模型，形成迭代优化。
...
### [305] [Diffusing the Blind Spot: Uterine MRI Synthesis with Diffusion Models](https://arxiv.org/abs/2508.07903)
*Johanna P. Müller,Anika Knupfer,Pedro Blöss,Edoardo Berardi Vittur,Bernhard Kainz,Jana Hutter*

Main category: eess.IV

TL;DR: 提出了一种新的扩散模型框架，用于生成高保真、解剖学准确的女性盆腔MRI图像，解决了现有模型在妇科影像中的不足。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在生成女性盆腔图像时解剖学准确性不足，且妇科影像数据稀缺和隐私问题限制了应用。
...
Conclusion: 该框架为妇科影像提供了有价值的合成数据资源，支持可重复研究和公平AI发展。
...
### [306] [RedDino: A foundation model for red blood cell analysis](https://arxiv.org/abs/2508.08180)
...
Method: 采用DINOv2自监督学习框架的RBC特定版本，训练于125万张RBC图像。

Result: 在RBC形状分类任务中表现优异，特征表示和泛化能力强大。
...
### [307] [DiffVC-OSD: One-Step Diffusion-based Perceptual Neural Video Compression Framework](https://arxiv.org/abs/2508.07682)
*Wenzhuo Ma,Zhenzhong Chen*

Main category: eess.IV

TL;DR: DiffVC-OSD是一种基于单步扩散的感知神经视频压缩框架，通过直接输入重建的潜在表示到单步扩散模型中，提升感知质量，同时利用时间上下文适配器和端到端微调策略优化性能。实验表明其解码速度提升20倍，比特率降低86.92%。


<details>
  <summary>Details</summary>
Motivation: 传统多步扩散方法效率低，DiffVC-OSD旨在通过单步扩散提升感知质量和计算效率。

Method: 提出单步扩散模型，设计时间上下文适配器编码多级特征，并采用端到端微调策略。

Result: 实现了最先进的感知压缩性能，解码速度快20倍，比特率降低86.92%。

Conclusion: DiffVC-OSD在视频压缩中显著提升了效率和性能，为单步扩散模型的应用提供了新方向。
...
### [308] [Anatomy-Aware Low-Dose CT Denoising via Pretrained Vision Models and Semantic-Guided Contrastive Learning](https://arxiv.org/abs/2508.07788)
...
Result: 在多个低剂量CT去噪数据集上，ALDEN实现了最先进的性能，显著减少过平滑问题，并在下游多器官分割任务中验证了解剖感知能力。

Conclusion: ALDEN通过解剖感知机制提升了低剂量CT去噪的准确性和解剖结构保留能力，为医学影像分析提供了更可靠的解决方案。
...
### [309] [A Physics-Driven Neural Network with Parameter Embedding for Generating Quantitative MR Maps from Weighted Images](https://arxiv.org/abs/2508.08123)
...
<details>
  <summary>Details</summary>
Motivation: 提高临床加权MRI定量图像合成的准确性和泛化性，以加速qMRI并增强其临床实用性。
