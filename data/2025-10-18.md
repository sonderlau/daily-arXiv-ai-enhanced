...
Motivation: 脉冲神经网络(SNN)作为神经形态计算的有前景方法，在能效和实时资源受限处理方面具有显著优势，但设计最优SNN架构面临复杂性和硬件约束的挑战。

Method: 首先概述SNN的操作原理和与传统ANN的关键区别，然后简要介绍ANN架构搜索的现状，最后重点调查SNN特定的架构搜索方法。

Result: 提供了SNN架构搜索领域的全面概览，强调了硬件/软件协同设计方法在实现SNN全部潜力中的重要性。

Conclusion: 硬件/软件协同设计对于释放神经形态计算的全部能力至关重要，本调查旨在为该领域研究人员提供有价值的资源。
...
<details>
  <summary>Details</summary>
Motivation: 现有监督模型依赖大量标注数据且对未见食物类别泛化能力有限，需要克服这些挑战以实现更好的食物图像分类。

Method: 集成视觉语言模型和大语言模型，使用对象感知令牌捕获细粒度视觉属性，交互式推理代理动态解释上下文线索来优化预测，通过多轮视觉-文本对话实现多智能体协作推理。

Result: 在多个公共食物数据集上的实验表明，MultiFoodChat相比现有无监督和少样本方法实现了更优的识别准确性和可解释性。

Conclusion: 该框架展示了作为智能食物质量检测和分析新范式的潜力，无需额外训练或手动标注即可灵活理解复杂食物场景。
...
<details>
  <summary>Details</summary>
Motivation: 子宫内膜异位症具有多样的视觉表现，在不同体内位置出现，这使得其识别非常困难且容易出错，特别是对于非专科医生。该系统旨在为妇科医生提供辅助诊断工具

Method: 训练一个系统来分割深色子宫内膜植入物，能够分析腹腔镜手术视频，用多色覆盖层标注识别区域，并显示检测摘要以改善视频浏览

Result: 系统能够成功识别和分割子宫内膜异位症的深色植入物区域，在腹腔镜手术视频中提供可视化标注

Conclusion: 该系统为妇科医生在诊断和治疗子宫内膜异位症时提供了有价值的辅助工具，通过自动识别和标注植入物区域来提高诊断准确性和效率
...
Method: 集成YOLO目标检测模型与多种视觉语言模型（LLaVA、ChatGPT、Gemini），在标记和未标记的遥感数据以及图像退化场景下进行性能评估。

Result: 在飞机检测和计数方面，平均MAE提升了48.46%，在原始和退化场景下都表现优异；CLIPScore提升了6.17%，表明对遥感图像的全面理解能力增强。
...
<details>
  <summary>Details</summary>
Motivation: 前列腺癌中的筛状形态是预后不良的组织学特征，但目前存在报告不足和观察者间差异大的问题，需要开发AI系统来提高检测准确性。

Method: 使用EfficientNetV2-S编码器和多实例学习的深度学习模型，在640个前列腺穿刺活检切片上进行训练，并在多个队列中进行内部和外部验证。

Result: 模型在内部验证中AUC为0.97，Cohen's kappa为0.81；在外部验证中AUC为0.90，Cohen's kappa为0.55。在评估中模型表现优于所有九位病理学家。
...
<details>
  <summary>Details</summary>
Motivation: 现有对抗净化方法主要针对加性扰动设计，对非加性扰动（如模糊、遮挡、扭曲）效果不佳，需要扩展框架来处理这类现实世界中常见的扰动。

Method: 建立对抗图像的生成过程，通过似然最大化方法分离底层干净图像和扰动参数。

Result: 在GTSRB和CIFAR-10数据集上的实验表明，NAPPure显著提升了图像分类模型对非加性扰动的鲁棒性。

Conclusion: NAPPure框架成功扩展了对抗净化的适用范围，能够有效处理非加性对抗扰动，提升了模型在现实场景中的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 解决长视频语言模型在处理超出上下文窗口的密集视频token和保留长期序列信息方面的困难，以及传统RAG方法在长视频中面临的时间依赖关系破坏和无关信息包含等问题。
...
Conclusion: Vgent框架通过图结构表示和中间推理机制，有效提升了长视频语言模型的理解能力，在多个基准测试中表现出显著优势。
...
Method: 使用预训练模型提取高维嵌入，然后学习统一的原型序列来锚定关键动作阶段，构建共享的紧凑1D表示。

Result: 在多样化数据集上提高了同步准确性、效率和鲁棒性，特别是在细粒度帧检索和阶段分类任务中表现优异。

Conclusion: TPL是首个解决多个生成AI视频同步问题的方法，在复杂视频同步场景中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法存在几何不一致、幻觉问题导致身份特征退化，或使用合成数据训练无法捕捉高频细节如皮肤皱纹和细发，限制了真实感。

Method: 引入两个关键贡献：(1) 生成式规范化模块处理多个非结构化视图为标准化表示；(2) 基于transformer的模型，使用从真实人物圆顶捕捉构建的高斯溅射虚拟形象大规模数据集训练。

Result: 提出的"捕捉、规范化、溅射"流程能从非结构化照片生成具有引人注目的真实感和稳健身份保持的静态半身虚拟形象。

Conclusion: 该方法在保持身份特征的同时实现了超真实3D虚拟形象的创建，解决了现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现代显微镜生成的大型2D和3D数据集使得现有计算方法在可扩展性、效率和与现代科学计算工作流集成方面存在局限。现有生物图像分析工具缺乏API、不支持GPU加速、3D图像处理能力有限，且对计算密集型工作流的互操作性差。

Method: cubic通过扩展广泛使用的SciPy和scikit-image API，集成CuPy和RAPIDS cuCIM的GPU加速替代方案。其API是设备无关的，当数据在设备上时自动调度到GPU执行，否则在CPU上执行，无缝加速各种图像处理例程。

Result: 通过基准测试单个操作和复现现有的去卷积和分割流程，cubic实现了显著的加速，同时保持了算法保真度。

Conclusion: cubic为可扩展、可重复的生物图像分析建立了坚实基础，能够与更广泛的Python科学计算生态系统集成，包括其他GPU加速方法，支持交互式探索和自动化高通量分析工作流。
...
<details>
  <summary>Details</summary>
Motivation: 为了解决视频生成中角色一致性和相机控制的问题，推动视频生成在虚拟制作中的集成应用。

Method: 使用4D高斯泼溅技术重新渲染体积捕捉表演，结合视频重光照模型获得光照变化，在开源视频扩散模型上微调，支持多主体生成的联合训练和噪声混合方法。

Result: 实验显示视频质量提升，个性化精度更高，相机控制和光照适应性增强，实现了更好的虚拟制作集成。

Conclusion: 该框架显著提升了视频生成在虚拟制作中的应用能力，为多视角角色一致性和3D相机控制提供了有效解决方案。
...
Conclusion: 联合建模Big Five和HEXACO可以改善多模态人类行为的认知，为外显人格特质识别提供了有效方法。
...
Result: 在GenImage基准上平均准确率达98.9%（提升11.9%），跨生成器泛化能力强，从GAN到Diffusion准确率超98.2%，从Diffusion到GAN超99.2%，误差提取速度比现有方法快近百倍。
...
<details>
  <summary>Details</summary>
Motivation: 传统深度伪造检测方法依赖手动设计的音素-视位对齐阈值、基本帧级一致性检查或单模态检测策略，无法有效识别由GAN、扩散模型和神经渲染技术生成的现代深度伪造内容，这些先进技术会产生几乎完美的单帧但存在细微时间差异。

Method: 开发了PIA多模态框架，整合了音素序列、唇部几何数据和先进的面部身份嵌入，通过分析语言、动态面部运动和面部识别线索来识别跨多个互补模态的不一致性。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于帧的OCC系统存在比特率低、处理负载高的问题，而现有事件基OCC系统使用的传统调制方案未能充分利用事件视觉传感器的独特特性。

Method: 提出了事件间隔调制(EIM)方案，通过调制事件间间隔来编码信息；对EVS参数进行优化调谐，确定EIM的最大调制阶数，并进行传输实验验证。

Result: 在室内环境中成功实现了28kbps@10米和8.4kbps@50米的传输，创下了事件基OCC系统的比特率新纪录。

Conclusion: EIM调制方案能够充分利用事件视觉传感器的特性，显著提升光学相机通信系统的传输性能，为高速、低延迟的可见光通信提供了新的技术路径。
...
Result: 在剑桥测试集上仅需10分钟训练就能获得高质量渲染结果，在保持高精度的同时显著降低成本。

Conclusion: MACE框架为大规模场景应用提供了高效的解决方案，实现了计算成本降低和精度提升的双重优势。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像到视频生成模型在人物面部占比小、表情和动作变化大时难以保持身份一致性，而人类对身份变化高度敏感，这是一个关键但未被充分探索的挑战。

Method: 使用强化学习优化扩散模型，通过人脸身份评分器提供奖励信号，在采样链的最后步骤反向传播奖励信号以获得更丰富的梯度反馈，并提出新的面部评分机制和KL散度正则化。

Result: 在Wan 2.2 I2V模型和内部I2V模型上的广泛实验证明了该方法的有效性。

Conclusion: IPRO框架能够有效增强图像到视频生成中的身份保持能力，无需引入辅助模块或改变模型架构。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法如VACE和Phantom在特定主题视频生成方面有进展，但在多人物动态互动中难以保持身份一致性，这对多角色一致身份至关重要。
...
<details>
  <summary>Details</summary>
Motivation: 现有跨注意力机制存在二次复杂度问题，且缺乏显式匹配约束，难以处理高分辨率图像的跨视图匹配。

Method: 提出MatchAttention机制，使用BilinearSoftmax实现连续可微的滑动窗口注意力采样，相对位置通过残差连接在层间迭代更新。设计MatchDecoder作为核心组件，并引入门控跨MatchAttention和一致性约束损失来处理遮挡问题。
...
Conclusion: 所提出的基于事件视觉传感器的解调方案能够有效提升光学相机通信系统在长距离高速率条件下的可靠性和鲁棒性。
...
Method: 集成2D计算机视觉技术（凸滤波和DINO等基础模型的语义特征监督），利用2D分割先验和高维特征嵌入来指导高斯泼溅的密集化和细化。
...
Method: 提出因果推断框架，通过前门原则设计转换策略，明确结合中介变量和观察到的组织切片，利用语义特征同时减轻混杂因素的影响。

Result: 在CAMELYON17数据集和私有组织病理学数据集上验证，在未见域上获得一致的性能提升，相比现有基线方法实现了高达7%的性能改进。

Conclusion: 因果推断是解决组织病理学图像分析中域偏移问题的有力工具，展示了在医学图像分析中考虑因果关系的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在多模态任务中表现良好，但仍容易产生幻觉，过度依赖单一模态或记忆训练数据而缺乏视觉接地。

Method: 训练免费的三层对比解码加水印：1) 选择解码层中的成熟层和业余层；2) 使用水印相关问题识别视觉接地层；3) 应用三层对比解码生成最终输出。

Result: 在POPE、MME和AMBER等基准测试中达到最先进性能，显著减少LVLMs的幻觉并生成更视觉接地的响应。

Conclusion: 该方法能有效减少大型视觉语言模型的幻觉问题，提高输出的视觉接地性，且无需额外训练。
...
Method: MID-StyleGAN结合扩散模型和GAN的优势，采用多领域架构，在真实眼部图像和不同演示攻击领域之间进行转换，使用针对眼部数据的自适应损失函数保持领域一致性。

Result: 在LivDet2020数据集上，1%误检率下的真检测率从93.41%提升到98.72%，显著提高了演示攻击检测系统的性能。

Conclusion: MID-StyleGAN为虹膜和眼部生物识别中的数据稀缺问题提供了可扩展的解决方案，通过生成高质量合成数据有效增强演示攻击检测能力。
...
Method: 引入可学习的模块化任务查询(MTQs)和视觉对齐层(VALs)，在多个视觉基础模型的监督下激活特定视觉信号，并通过令牌网关掩码(TGM)协调不同视觉基础模型之间的表示冲突。

Result: 大量实验表明，VaCo显著提升了不同MLLMs在各种基准测试上的性能，展示了其在视觉理解方面的卓越能力。

Conclusion: VaCo通过视觉中心激活和协调机制，成功统一了MLLMs中文本和视觉输出的优化，显著增强了模型的视觉理解能力。
...
Result: 在ScanNet和3DMatch数据集上超越了之前的自监督配准方法，甚至优于一些较老的监督方法，且组件集成到现有方法中也表现出有效性。

Conclusion: 该方法通过循环一致性关键点和GRU姿态块有效提升了RGB-D配准性能，证明了自监督方法在几何推理任务中的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型在细粒度空间感知能力上存在不足，如生成详细区域描述和精确定位物体方面表现不佳，且无法满足用户对细粒度空间理解的需求。

Method: SPR方法通过语义和定位评分全面评估模型生成描述的质量，将最佳精炼描述与最低分初始描述配对进行直接偏好优化，从而增强与视觉输入的细粒度对齐。

Result: 在标准参考和定位基准测试上的广泛实验表明，SPR能够以最小的训练开销有效提升多模态大语言模型的空间理解能力。

Conclusion: SPR方法通过空间偏好奖励机制成功解决了多模态大语言模型在细粒度空间感知方面的局限性，显著提升了模型的空间理解性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有Mamba模型在脑肿瘤分割中存在计算开销大和跨不同数据分区鲁棒性不足的问题，需要更高效和可靠的解决方案。

Method: 使用空间填充曲线进行3D到1D特征映射以保持空间局部性，提出门控融合模块自适应整合前后向上下文，并引入量化块离散化特征以提高鲁棒性。

Result: 在BraTS2023测试集上，全肿瘤、肿瘤核心和增强肿瘤的Dice系数分别提高0.10%、1.75%和0.93%；在五个系统折上，肿瘤核心和增强肿瘤平均Dice增益分别为0.86%和1.45%，同时效率提升15倍。

Conclusion: DRBD-Mamba模型在保持高分割精度的同时显著提高了计算效率，展现出优于现有方法的鲁棒性和计算优势。
...
<details>
  <summary>Details</summary>
Motivation: 主板缺陷检测对高产量电子制造可靠性至关重要，但现有研究主要针对裸板或线路级缺陷，组装级全主板检测仍未被充分探索。

Method: 提出BoardVision框架，在MiracleFactory数据集上基准测试YOLOv7和Faster R-CNN，设计轻量级集成方法CTV Voter通过可解释规则平衡精度和召回率，评估在锐度、亮度和方向变化等真实扰动下的鲁棒性。

Result: YOLOv7在精度方面表现优异但在召回率方面表现不足，Faster R-CNN则相反；CTV Voter集成方法有效平衡了精度和召回率；在真实扰动下存在稳定性挑战。

Conclusion: 该研究展示了计算机视觉技术如何从基准测试结果过渡到组装级主板制造的实际质量保证应用，并发布了可部署的GUI驱动检测工具。
...
TL;DR: 提出DCMIL模型，通过双课程对比多示例学习处理全切片图像，无需密集标注即可预测癌症预后，在12种癌症类型上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 计算病理学面临全切片图像计算量大、密集标注稀缺的挑战，现有方法常忽略多放大倍数下的细粒度信息和肿瘤微环境变化。

Method: 使用易到难的渐进表示学习模型DCMIL，结合双课程对比多示例学习，直接处理千兆像素级全切片图像进行预后预测。

Result: 在12种癌症类型（5,954患者，1,254万图块）上验证，DCMIL优于标准预后模型，能识别细粒度预后相关区域并提供不确定性估计。

Conclusion: DCMIL能有效处理全切片图像，识别形态学差异，有潜力产生新的生物学见解，代码已开源。
...
Method: 设计了统一的帧内/帧间编码框架，使用单一模型自适应执行编码；提出同时双帧压缩设计，利用前向和后向的帧间冗余。

Result: 相比DCVC-RT平均减少10.7%的BD-rate，提供更稳定的帧级码率和质量，同时保持实时编码/解码性能。

Conclusion: 该框架有效解决了神经视频压缩的关键问题，在压缩效率和稳定性方面均有显著提升，且保持了实时性能。
...
Method: 使用核范数正则化促进集中在背景的结构化扰动，并采用自适应乐观指数梯度方法进行高效优化。
...
<details>
  <summary>Details</summary>
Motivation: 监督方法需要大量体素级标注数据且仅限于已知病理，而无监督深度生成模型仅需健康数据训练，能识别与学习到的正常脑结构偏差的异常，特别适用于罕见或异质性疾病。

Method: 采用PRISMA指导的范围综述方法，系统梳理了无监督深度生成模型在脑MRI和CT异常检测中的应用，涵盖肿瘤、中风、多发性硬化等多种病理。

Result: 生成模型在大病灶检测中表现良好，在识别更细微异常方面也取得进展，能生成可解释的伪健康重建图像，在标注数据稀缺情况下特别有价值。
...
Method: 使用迭代剪枝策略，通过多轮移除低幅值权重，同时将剩余权重重置为原始初始化，发现高性能的稀疏子网络。

Result: 在去雨、去雾和去噪任务的基准数据集上，MIR-L仅保留10%的可训练参数就能保持高图像恢复性能。

Conclusion: 提出的压缩策略能有效发现多任务图像恢复模型中的高性能稀疏子网络，在保持性能的同时大幅减少参数量。
...
Method: 利用Sentinel-2 L2A时间序列数据，为每个多边形定义的牧场边界，使用4-10月影像进行二元预测（放牧/未放牧），训练CNN-LSTM集成模型处理多时相反射特征。

Result: 在五个验证分割上平均F1分数达到77%，放牧牧场的召回率达到90%。如果检查员每年最多只能访问4%的场地，优先检查模型预测为未放牧的场地，确认的未放牧场地数量比随机检查多17.2倍。
...
<details>
  <summary>Details</summary>
Motivation: Vision Mamba在图像分类中表现出优于Vision Transformers的线性扩展特性和比传统CNN更少的可训练参数，这为三维多孔介质渗透率预测提供了更高效的计算方案。

Method: 使用Vision Mamba作为主干网络构建神经网络，与ViT和CNN模型在渗透率预测的多个方面进行性能比较，并通过消融研究评估各组件对准确性的影响。

Result: 在实际应用中验证了Vision Mamba相比ViT和CNN在三维多孔介质渗透率预测中的计算效率和内存优势。
...
<details>
  <summary>Details</summary>
Motivation: 手术器械缺陷会严重影响无菌性、机械完整性和患者安全，而传统的人工检测存在人为错误和不一致性问题，需要自动化解决方案。

Method: 采用YOLOv8模型，在包含102,876张高分辨率图像的数据集上进行训练，涵盖11种器械类型和5种主要缺陷类别，并使用对比度增强预处理技术。

Result: SurgScan达到99.3%的最高准确率，实时推理速度为每张图像4.2-5.8毫秒，统计分析显示对比度增强预处理显著提升缺陷检测性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决文本到图像生成中训练-推理不匹配问题：训练时噪声位于提示特定的潜在空间子集，而推理时从提示无关的高斯先验采样噪声，导致生成图像与提示对齐不佳。

Method: 设计噪声投影器，基于提示嵌入将噪声映射到提示感知的对应物；首先采样噪声并通过视觉语言模型获得token级反馈，将信号蒸馏到奖励模型中，最后通过准直接偏好优化优化噪声投影器。

Result: 实验表明，提示感知的噪声投影改善了各种提示下的文本-图像对齐度，推理成本小，用单次前向传播替代多样本选择。

Conclusion: 提出的噪声投影框架有效解决了训练-推理不匹配问题，无需参考图像或手工先验，显著提高了文本到图像生成的对齐质量。
...
Conclusion: PaddleOCR-VL在性能、多语言支持和资源效率方面表现出色，非常适合在实际场景中部署应用。
...
<details>
  <summary>Details</summary>
Motivation: 解决牙科放射影像解读中专业人员短缺的问题，克服现有牙科AI系统单模态、任务特定设计和对标注数据依赖的限制，提升在多样化临床场景中的泛化能力。

Method: 基于Vision Transformer架构开发2D和3D版本的DentVFM模型，使用自监督学习在DentVista数据集（约160万张多模态牙科影像）上训练，并引入DentBench综合基准进行评估。

Result: DentVFM在疾病诊断、治疗分析、生物标志物识别和解剖标志点检测分割等任务中表现出强大的泛化能力，显著优于监督、自监督和弱监督基线方法，在跨模态诊断中甚至比经验丰富的牙医更可靠。

Conclusion: DentVFM为牙科AI设立了新范式，提供了可扩展、适应性强且标注高效的模型，有望改善智能牙科医疗并填补全球口腔医疗的关键空白。
...
<details>
  <summary>Details</summary>
Motivation: 医学图像（如MR扫描）在不同成像站点间存在域偏移，这会降低机器学习性能。现有方法虽然有效但缺乏可解释性，这在医疗应用中至关重要。

Method: 使用两个编码器分别提取域不变特征和域特定特征，通过对抗训练和图像重建（将两个特征的重构相加）来确保协调性和信息保留。

Result: 在图像重建、疾病分类和域识别任务中达到与现有方法相当或更好的性能，并能可视化域无关和域特定脑部特征。

Conclusion: PL-SE-ADA在实现域协调的同时提供了高可解释性，为医疗图像分析提供了更可靠和透明的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 深度学习内部表示不透明，难以解释视觉信息处理过程，而经典视觉描述符对人类直观可理解。探索现代学习能否从这些经典线索中受益。

Method: 提出VisualSplit框架，将图像显式分解为解耦的经典描述符，通过重建驱动的预训练方案学习每个视觉描述符的本质，同时保持可解释性。

Result: 通过显式分解视觉属性，该方法在图像生成和编辑等高级视觉任务中有效促进属性控制，超越了传统的分类和分割任务。

Conclusion: 这种新的视觉理解学习方法展现了有效性，为可解释的视觉表示学习提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 现有参数高效微调(PEFT)方法仅进行单步调整，对于特征高度纠缠的复杂数据集效果不足，需要多步调整能力实现更精确的跨模态对齐。

Method: 提出FMA方法：1) 使用固定耦合策略确保类别对应；2) 噪声增强策略缓解数据稀缺问题；3) 早停求解器提高效率和精度。

Result: 在多个基准测试和骨干网络上取得显著性能提升，特别是在具有挑战性的数据集上表现突出。
...
TL;DR: 本文提出了一种无需训练的场景去情境化(SDeC)方法，通过编辑提示嵌入来抑制文本到图像生成中主体与场景的自然相关性，从而解决身份偏移问题。


<details>
  <summary>Details</summary>
Motivation: 现有的文本到图像生成方法在保持主体身份一致性方面存在身份偏移问题，且通常需要预先知道所有目标场景，这在现实中不切实际。

Method: 提出SDeC方法，通过量化SVD方向稳定性来自适应重新加权特征值，抑制ID提示嵌入中的潜在场景-ID相关性，实现场景去情境化。

Result: 实验证明SDeC显著增强了身份保持能力，同时保持了场景多样性。

Conclusion: SDeC是一种高效、无需训练且灵活的解决方案，适用于现实世界应用，无需预先知道所有目标场景。
...
TL;DR: 提出ESTP-Bench基准和ESTP-F1指标，用于评估AI在流式视频中主动理解、预测和响应事件的能力，并开发了一个包含数据引擎、多阶段训练策略和主动动态压缩技术的技术流程。


<details>
  <summary>Details</summary>
Motivation: 开发能够在人类环境中主动理解、预测和响应事件的AI系统，超越单纯观察，实现同步感知和推理。

Method: 提出包含数据引擎、多阶段训练策略和主动动态压缩技术的综合技术流程，用于处理流式视频输入下的主动问答任务。

Result: 所提模型在多个在线和离线基准测试中优于多个基线方法，有效解决了主动一致性、及时响应和同步效率等关键属性。

Conclusion: 该工作为AI在流式视频环境中的主动感知和推理能力提供了新的评估框架和技术解决方案，展示了在复杂动态场景中的有效应用。
...
Method: 提出三层次优化：算法层面的启发式密度控制、系统层面的相似性高斯采样与合并、映射层面的重排序内存访问策略。
...
TL;DR: CALM-Net是一种基于LiDAR点云的车辆重识别多分支神经网络，通过整合边缘卷积、点注意力和曲率嵌入来学习区分性特征，在nuScenes数据集上相比最强基线提升了约1.97%的平均重识别准确率。
...
Method: 采用多分支架构，整合边缘卷积、点注意力和曲率嵌入机制，通过曲率嵌入表征点云中的局部表面变化。
...
TL;DR: 提出了一种用于像素级关键点理解的框架，包含点描述器和点定位器两个组件，通过自然语言实现精确的关键点定位。
...
Method: 提出两个组件：1) 实例线索 - 将稀疏的用户可编辑提示转换为密集的2.5D运动场；2) 密集RoPE - 在token空间中保持线索显著性，配合RGB+辅助图联合预测。

Result: 该方法减少了深度模糊性，稳定了优化过程，提高了时间一致性。

Conclusion: STANCE框架有效解决了视频生成中的运动连贯性问题，无需复杂的逐帧轨迹脚本。
...
<details>
  <summary>Details</summary>
Motivation: 现有动物分类模型如SpeciesNet使用保守的汇总策略，导致许多动物仅被标记在高级分类水平而非物种级别。
...
TL;DR: 评估了使用自监督视觉变换器的零样本方法在野生动物图像分类中的表现，比较了不同架构和聚类方法，发现DINOv2结合UMAP和GMM能达到88.6%的准确率，并已部署到生产环境中。


<details>
  <summary>Details</summary>
Motivation: 相机陷阱产生大量野生动物图像，但许多数据集包含现有分类器无法识别的物种，需要零样本方法来处理未标记的野生动物图像。
...
TL;DR: 提出Wiki-PRF三阶段方法，通过处理、检索和过滤阶段改进知识库视觉问答任务，使用强化学习训练视觉语言模型，在基准数据集上实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有的检索增强生成方法在知识库视觉问答任务中面临多模态查询质量差和检索结果相关性不足的挑战。

Method: 三阶段方法：处理阶段调用视觉工具提取精确多模态信息；检索阶段整合视觉和文本特征进行多模态知识检索；过滤阶段进行相关性过滤和结果浓缩。使用强化学习训练视觉语言模型。

Result: 在E-VQA和InfoSeek基准数据集上分别取得36.0和42.8的显著改进，达到最先进性能。

Conclusion: Wiki-PRF方法通过三阶段处理和强化学习训练，有效提升了知识库视觉问答任务的性能，解决了多模态查询质量和检索结果相关性的问题。
...
TL;DR: Shot2Tactic-Caption是一个羽毛球视频字幕生成框架，能够同时生成描述单个动作的击球级字幕和描述战术执行的战术级字幕。


<details>
  <summary>Details</summary>
Motivation: 羽毛球战术理解不仅需要解释单个动作，还需要理解战术如何随时间动态执行。现有方法缺乏对战术层面动态执行的描述能力。

Method: 采用双分支设计，包含视觉编码器、时空Transformer编码器和基于Transformer的解码器。引入战术单元检测器识别有效战术单元、类型和状态，并使用击球级提示引导机制。

Result: 实验结果表明该框架能有效生成击球和战术字幕。消融研究显示基于ResNet50的时空编码器性能最优，击球级提示结构能产生更连贯准确的战术字幕。

Conclusion: 提出的Shot2Tactic-Caption框架能够有效实现羽毛球视频的语义和时序多尺度字幕生成，特别是在战术层面的动态执行描述方面表现出色。
...
TL;DR: 提出Efficient Video Sampling (EVS)方法，通过识别和剪枝时间上静态的视觉块来减少视频处理的token冗余，实现更快的推理速度和更长的输入序列。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在处理密集视频帧序列时面临二次计算成本问题，长视频常超出语言模型的token预算，导致上下文限制和延迟问题。

Method: EVS是一种即插即用的方法，通过识别和剪枝时间上静态的视觉块来减少token冗余，保持位置身份，无需架构修改或重新训练。

Result: EVS显著减少token数量同时保持语义保真度，将大型语言模型的首次token时间减少高达4倍，且准确率损失最小。结合随机剪枝率的上训练阶段，模型对不同程度的压缩具有鲁棒性。

Conclusion: EVS持续改善效率-准确率权衡，在不牺牲质量的情况下实现可扩展的视频语言理解。
...
Method: 基于预训练的SSL编码器，仅微调语义令牌嵌入，与使用流匹配目标的生成解码器联合训练，添加余弦相似度损失保持潜在空间平滑性。

Result: 在类别条件ImageNet生成上取得竞争性结果，在MS-COCO上以极有限训练预算达到竞争性零样本性能。

Conclusion: 微调的SSL表示可作为紧凑有效的潜在空间，用于高效生成建模。
...
TL;DR: SteeringTTA是一个无需模型更新或源数据的测试时自适应框架，通过Feynman-Kac引导和扩散过程来适应分布偏移，在ImageNet-C上表现优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有的基于扩散的测试时自适应方法依赖梯度引导，限制了在不同失真类型间的探索和泛化能力。

Method: 提出SteeringTTA框架，将Feynman-Kac引导应用于基于扩散的输入自适应，使用伪标签驱动的奖励机制，维护多个粒子轨迹并通过累积top-K概率和熵调度来平衡探索和置信度。

Result: 在ImageNet-C数据集上，SteeringTTA始终优于不进行任何模型更新或使用源数据的基线方法。

Conclusion: SteeringTTA通过引导扩散过程有效提升了模型在分布偏移下的鲁棒性，无需模型更新或源数据即可实现性能提升。
...
TL;DR: 提出了一种低成本预训练策略，通过上下文学习从未配对视频剪辑中学习视频编辑能力，显著提升指令视频编辑的性能。


<details>
  <summary>Details</summary>
Motivation: 指令式图像编辑发展迅速，但视频编辑由于构建大规模配对视频编辑数据集成本高昂且复杂，研究较少。

Method: 基于HunyuanVideoT2V框架，先在约100万真实视频剪辑上预训练学习基本编辑概念，然后在少于15万高质量配对编辑数据上微调。

Result: 实验表明该方法在指令对齐和视觉保真度上均优于现有方法，编辑指令遵循度提升12%，编辑质量提升15%。

Conclusion: 该策略有效解决了视频编辑数据集构建难题，为指令式视频编辑提供了可行的低成本解决方案。
...
<details>
  <summary>Details</summary>
Motivation: MAE预训练在低标签场景下表现优异但计算成本高昂，难以在工业环境中实际应用

Method: 在MAE预训练的编码器中选择性应用DBP优化方法，迭代减少各层的输入相关性

Result: DBP-MAE将训练时间减少21.1%，碳排放降低21.4%，分割mIoU提升1.1个百分点，在工业数据上同样有效
...
TL;DR: EuroMineNet是首个基于Sentinel-2多光谱影像的综合性多时相采矿足迹测绘基准数据集，涵盖欧盟133个采矿点2015-2024年的年度观测数据，支持采矿足迹测绘和跨时相变化检测任务。
...
Method: 基于Sentinel-2多光谱影像构建包含133个欧盟采矿点的多时相数据集，提供专家验证的年度标注，支持采矿足迹测绘和跨时相变化检测，并引入新的Change-Aware Temporal IoU评估指标。

Result: 对20个最先进的深度学习模型进行基准测试表明，GeoAI方法能有效识别长期环境变化，但在检测对及时缓解至关重要的短期动态方面仍面临挑战。
...
TL;DR: 提出了WeCKD网络，通过链式知识蒸馏框架解决传统知识蒸馏中的知识退化、监督效率低和数据依赖问题，在有限数据场景下实现高效学习。


<details>
  <summary>Details</summary>
Motivation: 传统知识蒸馏依赖强教师模型或大量标注数据，在真实世界有限数据场景下效果受限，需要解决知识退化、监督效率低和数据依赖问题。

Method: 采用弱监督链式知识蒸馏框架，构建渐进式蒸馏链，每个模型不仅从前驱学习，还精炼知识后传递给后继模型，每个模型仅使用部分数据集训练。

Result: 在四个耳镜成像数据集上超越现有监督方法，在其他两个数据集上验证了跨模态泛化能力，相比单骨干网络在有限数据下获得最高+23%的累积准确率提升。

Conclusion: WeCKD通过链式知识蒸馏有效降低了数据依赖，在有限监督下实现了高效学习，具有实际应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于多模态大语言模型的视频问答系统在视频时序定位和推理方面存在明显不足，这限制了实际视频理解系统的开发。

Method: 提出了两个新颖的视觉工具：即插即用的进度条集成工具和高效高亮工具，并引入了结合视频和文本跨模态推理的视觉时序思维链过程。

Result: 在Qwen2VL-7B和GPT4o基准测试中，在视频时序定位和基于推理的问答任务上取得了显著的性能提升。
...
TL;DR: 提出了一种半自动的鲸鱼检测方法，使用统计异常检测来标记空间异常点，结合网页标注界面让专家快速标注，大幅减少了需要专家检查的面积。


<details>
  <summary>Details</summary>
Motivation: 传统鲸鱼监测方法昂贵且难以扩展，而现有的卫星图像自动检测方法面临缺乏标注数据、图像质量变化和构建机器学习管道成本高等挑战。

Method: 使用统计异常检测方法标记空间异常点作为可能的鲸鱼检测，配合网页标注界面让专家快速验证，无需依赖标注训练数据。

Result: 在三个基准场景上实现了90.3%到96.4%的召回率，将专家需要检查的面积从超过1000平方公里减少到不到2平方公里，减少了99.8%。
...
TL;DR: 提出了一种利用学习图像先验来恢复压缩质量损失的3D高斯泼溅压缩框架，通过恢复网络建模压缩伪影，并引入粗渲染残差作为边信息，提升率失真性能。


<details>
  <summary>Details</summary>
Motivation: 现有的3D高斯泼溅压缩方法虽然显著减少了存储开销，但缺乏学习先验限制了率失真权衡的进一步改进，需要解决压缩引起的质量退化问题。

Method: 基于初始压缩的高斯，构建恢复网络在图像空间建模压缩伪影，引入粗渲染残差作为边信息，通过恢复图像的监督来优化压缩高斯。

Result: 实验验证了框架的有效性，在率失真性能上表现优越，渲染质量优于最先进的3D高斯压缩方法，同时显著减少存储需求。

Conclusion: 该框架兼容现有的高斯压缩方法，能够产生高度紧凑的表示并提升渲染性能，为3D高斯泼溅压缩提供了有效的解决方案。
...
### [65] [Camera Movement Classification in Historical Footage: A Comparative Study of Deep Video Models](https://arxiv.org/abs/2510.14713)
*Tingyu Lin,Armin Dadras,Florian Kleber,Robert Sablatnig*

Main category: cs.CV

TL;DR: 本文首次系统评估了深度视频相机运动分类模型在历史档案影片上的表现，发现Video Swin Transformer在HISTORIAN数据集上达到80.25%准确率，显示了现有模型在低质量视频上的适应潜力。
...
TL;DR: DEXTER是一个无需数据的框架，使用扩散模型和大型语言模型生成视觉分类器的全局文本解释，通过优化文本提示合成激活目标分类器的类条件图像，从而揭示分类器的决策模式和偏见。
...
Method: 使用扩散模型优化文本提示来合成类条件图像，这些图像能强烈激活目标分类器，然后利用大型语言模型基于合成样本生成详细的自然语言报告。

Result: 在ImageNet、Waterbirds、CelebA和FairFaces上的实验表明，DEXTER在全局模型解释和类级偏差报告方面优于现有方法，定量和定性评估（包括用户研究）证实其输出准确且可解释。

Conclusion: DEXTER能够在无需训练数据或真实标签的情况下，通过自然语言有效揭示视觉分类器的内部机制和偏见，为模型可解释性提供了灵活且强大的解决方案。
...
TL;DR: 提出基于无条件扩散模型的方法来重建火星表面地形，填补卫星高度图中的缺失值，相比传统方法在重建精度和感知相似度方面表现更优。


<details>
  <summary>Details</summary>
Motivation: 火星地形数据中存在大量缺失值，传统插值方法难以保持几何一致性，而地球上的条件方法无法应用于火星，需要开发专门的无条件重建方法。

Method: 使用无条件扩散模型，在12000个火星高度图增强数据集上训练，采用非均匀缩放策略捕捉多尺度地形特征，最终固定为128x128分辨率。

Result: 在1000个样本的评估集上，相比逆距离加权、克里金法和纳维-斯托克斯算法，该方法在RMSE指标上提升4-15%，在LPIPS感知相似度指标上提升29-81%。

Conclusion: 基于无条件扩散模型的方法能有效重建火星表面地形，填补数据缺失，在精度和感知质量上均优于传统方法。
...
Result: 在SSD300框架中集成CFSAM，在PASCAL VOC上达到78.6% mAP（基线75.5%），在COCO上达到52.1% mAP（基线43.1%），优于现有注意力模块，且加速训练收敛
...
### [69] [Morphology-Aware Prognostic model for Five-Year Survival Prediction in Colorectal Cancer from H&E Whole Slide Images](https://arxiv.org/abs/2510.14800)
...
<details>
  <summary>Details</summary>
Motivation: 现有的计算病理学基础模型往往忽视器官特异性关键形态模式，而这些模式代表了影响肿瘤行为、治疗反应和患者预后的不同生物学过程。

Method: PRISM模型整合了每个独特形态内的连续变异谱，基于424名III期结直肠癌患者的874万张组织学图像进行训练，反映恶性转化是通过渐进进化过程而非突然表型转变的原理。

Result: PRISM在5年总生存期预测上表现优异（AUC=0.70，准确率68.37%，HR=3.34），比现有CRC特异性方法高15%，比AI基础模型高约23%准确率，且在不同亚组中表现稳定。

Conclusion: PRISM模型成功捕捉了结直肠癌的形态多样性，提供了优越的预后预测性能，证明了整合连续形态变异谱在癌症预后建模中的价值。
...
TL;DR: 提出了ImageNet-F基准，用于模拟现实世界中混合粒度的标注，并开发了free-grain学习方法，通过视觉语言模型和半监督学习提升层次分类性能。
...
Result: 提出的方法和强基线在混合监督下显著提高了层次分类性能。
...
### [71] [Scaling Artificial Intelligence for Multi-Tumor Early Detection with More Reports, Fewer Masks](https://arxiv.org/abs/2510.14803)
...
TL;DR: R-Super通过利用医学报告训练AI进行肿瘤分割，大幅减少对人工绘制肿瘤掩模的依赖，在多种肿瘤类型上达到甚至超越放射科医生的检测性能。


<details>
  <summary>Details</summary>
Motivation: 早期肿瘤检测至关重要，但传统AI训练需要大量人工绘制的肿瘤掩模，成本高昂且耗时。临床实践中丰富的医学报告信息未被充分利用。

Method: 引入R-Super方法，训练AI根据医学报告描述来分割肿瘤，利用101,654份医学报告进行训练，减少对723个手工掩模的依赖。

Result: 仅使用报告训练的AI模型性能与使用掩模训练的相当；结合报告和掩模后，敏感性提升13%，特异性提升8%，在7种肿瘤类型中有5种超越放射科医生。

Conclusion: 挑战了大规模人工肿瘤掩模创建不可或缺的传统观念，为多种肿瘤类型的早期检测提供了可扩展且易获取的路径。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视觉语言任务中表现出色，但其在面部识别方面的潜力尚未充分探索，特别是开源MLLMs需要在标准基准上与现有面部识别模型进行比较评估。
...
### [73] [LightQANet: Quantized and Adaptive Feature Learning for Low-Light Image Enhancement](https://arxiv.org/abs/2510.14753)
*Xu Wu,Zhihui Lai,Xianxu Hou,Jie Zhou,Ya-nan Zhang,Linlin Shen*

Main category: cs.CV

TL;DR: 提出了LightQANet框架，通过量化特征学习和自适应提示机制来解决低光图像增强中的纹理恢复、颜色一致性和伪影问题。
...
Method: 设计了Light Quantization Module (LQM)来显式提取和量化光照相关因素，以及Light-Aware Prompt Module (LAPM)将光照先验编码为可学习提示来动态指导特征学习。

Result: 在多个低光数据集上的广泛实验表明，该方法实现了最先进的性能，在各种挑战性光照场景下都获得了优异的定性和定量结果。

Conclusion: LightQANet通过量化特征学习和自适应提示机制，能够在不同光照条件下实现一致且鲁棒的图像质量增强。
...
### [74] [MaskCaptioner : Learning to Jointly Segment and Caption Object Trajectories in Videos](https://arxiv.org/abs/2510.14904)
*Gabriel Fiastre,Antoine Yang,Cordelia Schmid*

Main category: cs.CV

TL;DR: 提出了MaskCaptioner模型，通过扩展LVIS和LV-VIS数据集生成合成字幕，实现端到端的密集视频目标检测、跟踪和描述。


<details>
  <summary>Details</summary>
Motivation: 由于密集视频目标描述任务复杂且人工标注成本高，现有方法采用分离训练策略导致性能不佳。

Method: 利用先进视觉语言模型生成时空定位实体的字幕，扩展LVIS和LV-VIS数据集为LVISCap和LV-VISCap，训练端到端的MaskCaptioner模型。

Result: 在LVISCap和LV-VISCap上预训练后，MaskCaptioner在VidSTG、VLN和BenSMOT三个基准测试中达到最先进水平。

Conclusion: 通过合成字幕扩展数据集和端到端训练，显著提升了密集视频目标描述任务的性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型在复杂运动合成方面存在困难，生成的运动会不够自然、平滑和上下文一致，限制了实际应用

Method: 引入RealDPO对齐范式，使用真实世界视频作为正样本进行偏好学习，通过对比真实视频与错误模型输出来实现迭代自校正

Result: 实验表明RealDPO在视频质量、文本对齐和运动真实感方面显著优于最先进模型和现有偏好优化技术

Conclusion: RealDPO通过真实世界数据驱动的偏好学习有效提升了复杂运动合成的质量
...
### [76] [MoCom: Motion-based Inter-MAV Visual Communication Using Event Vision and Spiking Neural Networks](https://arxiv.org/abs/2510.14770)
*Zhang Nengbo,Hann Woei Ho,Ye Zhou*

Main category: cs.CV

TL;DR: 提出了一种基于视觉运动信号的微型飞行器蜂群通信框架，模仿蜜蜂摇摆舞，使用事件相机和脉冲神经网络进行低功耗通信解码


<details>
  <summary>Details</summary>
Motivation: 解决微型飞行器蜂群在频谱拥塞、干扰和高功耗环境下的可靠通信问题，受蜜蜂摇摆舞的启发

Method: 使用四种运动基元（垂直、水平、左-上-右、左-下-右）作为控制符号，通过事件相机捕获运动信号，采用事件帧分割模型和轻量级脉冲神经网络进行解码

Result: 实验验证了框架的有效性，实现了准确解码和低功耗，在受限环境中表现出良好的通信性能

Conclusion: 该视觉通信框架为微型飞行器蜂群在受限环境中提供了一种节能高效的通信替代方案
...
TL;DR: C4D是一个从单目视频恢复4D动态场景的框架，通过引入时间对应关系扩展现有的3D重建方法，能够同时估计动态几何和相机姿态。


<details>
  <summary>Details</summary>
Motivation: 现有的点图式3D重建方法在静态场景中表现良好，但直接应用于动态场景会导致不准确结果，因为运动物体违反了多视角几何约束。

Method: 除了预测点图外，C4D捕获两种对应关系：短期光流和长期点跟踪。训练动态感知点跟踪器提供移动性信息，估计运动掩码分离移动元素和静态背景，并引入动态场景优化目标。

Result: 实验表明该框架实现了完整的4D恢复，在深度估计、相机姿态估计和点跟踪等多个下游任务中表现出强大性能。

Conclusion: C4D通过时间对应关系成功将3D重建扩展到4D，能够有效处理动态场景中的几何重建问题。
...
### [78] [CoT-PL: Visual Chain-of-Thought Reasoning Meets Pseudo-Labeling for Open-Vocabulary Object Detection](https://arxiv.org/abs/2510.14792)
*Hojun Choi,Youngsun Lim,Jaeyo Shin,Hyunjung Shim*

Main category: cs.CV

TL;DR: CoT-PL是一个新的开放词汇目标检测框架，通过结构化视觉思维链推理改进伪标签生成过程，在拥挤或遮挡场景中显著提升新类别检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇目标检测方法过度依赖直接图像-文本匹配，忽略了理解语义复杂场景所需的中间推理步骤，导致在拥挤或遮挡场景中鲁棒性不足。

Method: 将目标理解分解为三个可解释步骤：区域感知、零样本类别识别和背景定位，并引入对比背景学习来促进目标与背景的特征解耦。

Result: 在拥挤和遮挡场景中，新类别伪标签质量分别比之前最佳方法提升103.4%和168.4%；在开放词汇COCO上AP50提升7.7，在LVIS上新类别mask AP提升2.9。

Conclusion: CoT-PL通过结构化思维链推理和对比背景学习的集成管道，显著提升了开放词汇目标检测在复杂场景中的鲁棒性和性能，创造了新的最先进水平。
...
### [79] [WithAnyone: Towards Controllable and ID Consistent Image Generation](https://arxiv.org/abs/2510.14975)
...
TL;DR: 提出了WithAnyone模型，通过构建MultiID-2M数据集和对比身份损失来缓解身份一致生成中的复制粘贴问题，在保持身份相似性的同时提高生成多样性。


<details>
  <summary>Details</summary>
Motivation: 现有身份一致生成模型依赖重建训练，导致复制粘贴问题——直接复制参考人脸而非在不同姿态、表情或光照下保持身份一致性，这限制了生成的可控性和表达能力。

Method: 构建MultiID-2M大规模配对数据集；引入评估复制粘贴伪影和身份保真度-变化权衡的基准；提出带有对比身份损失的新训练范式。

Result: WithAnyone模型显著减少复制粘贴伪影，提高姿态和表情的可控性，保持强感知质量。用户研究验证了高身份保真度和表达性可控生成。

Conclusion: 通过数据集构建、基准制定和对比损失训练，成功缓解了身份一致生成中的复制粘贴问题，实现了身份保真度与生成多样性的更好平衡。
...
Conclusion: Terra通过原生3D表示和架构实现精确的多视角一致性，支持单次生成过程从任意视角灵活渲染，并通过点潜在空间中的渐进生成实现可探索世界建模。
...
TL;DR: 本文提出了NEO原生视觉语言模型系列，通过设计三个核心原则来解决原生VLM与模块化VLM的根本区别问题，并在仅使用3.9亿图像-文本对的情况下实现了与顶级模块化模型相媲美的性能。


<details>
  <summary>Details</summary>
Motivation: 解决原生视觉语言模型面临的两个关键挑战：明确原生VLM与模块化VLM的根本区别和限制，以及如何使原生VLM研究更加普及和民主化，从而加速该领域的发展。

Method: 提出了构建原生VLM的三个指导原则：(i)在共享语义空间中有效对齐像素和词表示；(ii)无缝整合先前分离的视觉和语言模块优势；(iii)内在地体现支持统一视觉语言编码、对齐和推理的各种跨模态特性。基于这些原则开发了NEO模型系列。

Result: NEO模型在仅使用3.9亿图像-文本对的情况下，能够从零开始高效发展视觉感知，并在各种真实场景中与顶级模块化模型相竞争。模型缓解了密集单体模型内部的视觉-语言冲突。

Conclusion: NEO为可扩展且强大的原生VLM奠定了基石，提供了一套丰富的可重用组件，促进了成本效益高且可扩展的生态系统发展。
...
### [82] [Unifying Environment Perception and Route Choice Modeling for Trajectory Representation Learning](https://arxiv.org/abs/2510.14819)
...
Method: PRTraj包含环境感知模块和路径选择编码器，前者从周围POI分布中捕获多粒度环境语义，后者将路段转换建模为决策序列来捕获路径选择行为。
...
<details>
  <summary>Details</summary>
Motivation: 现有预训练2D图像编辑模型在多视角图像编辑时缺乏跨视角一致性，而基于显式3D表示的方法存在优化过程冗长和稀疏视角下不稳定的问题

Method: 采用耦合扩散采样技术，同时从多视角图像分布和2D编辑图像分布中采样两个轨迹，通过耦合项强制生成图像的多视角一致性

Result: 在三个不同的多视角图像编辑任务上验证了该框架的有效性和通用性，适用于各种模型架构

Conclusion: 该方法作为多视角一致编辑的通用解决方案具有巨大潜力
...
TL;DR: 提出了一种新的分数位量化方法SHORT，通过渐进式降低模型精度从32位到4位，在优化过程中利用分数位来保持生成质量，在多种扩散模型上实现了比标准QAT更好的质量。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的生成模型虽然性能强大，但由于模型容量大无法在智能手机上部署。量化方法虽然能提高计算效率，但激进量化会损害模型质量。

Method: 提出分数位量化方法SHORT，渐进式降低模型精度（从32位到4位），在优化过程中利用分数位来维持生成质量。

Result: 在多种扩散模型（SD3.5-Medium、Sana、Pixart、FLUX.1-schnell）上实现了改进的质量，比标准QAT的FiD低4-7%。成功在三星S25U手机上部署运行Sana模型。

Conclusion: SHORT方法能够在保持生成质量的同时实现高效的模型量化，使大型生成模型能够在移动设备上部署运行。
...
TL;DR: 该论文介绍了AbdomenAtlas 2.0数据集，包含10,135个CT扫描和15,130个肿瘤实例，用于训练AI进行六种器官的肿瘤分割，相比现有公共数据集在性能上有显著提升。


<details>
  <summary>Details</summary>
Motivation: 由于缺乏大规模、体素级标注的肿瘤数据集，且这些数据集创建困难且需要医学专家，限制了AI在肿瘤分割方面的性能。

Method: 基于JHH数据集的教训，创建了AbdomenAtlas 2.0数据集，包含10,135个CT扫描，由23名放射科专家手动标注六种器官的15,130个肿瘤实例，并使用合成数据来提升数据扩展效率。

Result: AbdomenAtlas 2.0在分布内测试中DSC提升7%，在分布外测试中提升16%，显著优于现有公共数据集。

Conclusion: 合成数据可以显著提升数据扩展效率，AbdomenAtlas 2.0为训练AI进行多器官肿瘤分割提供了强大的基础，并展示了在有限真实数据下通过合成数据实现高性能的潜力。
...
TL;DR: QDepth-VLA通过添加深度预测任务增强VLA模型的空间感知能力，使用量化深度令牌学习几何感知表示，在仿真和真实任务中表现出色。
...
Conclusion: 通过深度预测任务增强VLA模型是提升空间感知和推理能力的有效方法，QDepth-VLA框架在精细操作任务中表现优异。
...
TL;DR: 提出ImagerySearch方法，通过提示引导的自适应测试时搜索策略，动态调整推理搜索空间和奖励函数，以提升视频生成模型在想象力场景中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型在现实场景表现优秀，但在想象力场景中性能显著下降，这些场景涉及罕见共现概念和长距离语义关系，超出了训练分布范围。

Method: ImagerySearch是一种提示引导的自适应测试时搜索策略，根据提示中的语义关系动态调整推理搜索空间和奖励函数。同时提出了LDT-Bench基准测试来评估长距离语义提示的生成能力。

Result: 实验表明ImagerySearch在LDT-Bench上持续优于强基线视频生成方法和现有测试时缩放方法，在VBench上也取得了有竞争力的改进。

Conclusion: ImagerySearch方法能有效提升视频生成模型在挑战性想象力场景中的表现，LDT-Bench将为未来想象力视频生成研究提供支持。
...
TL;DR: 提出一个深度学习框架，既能对皮肤病变进行分类，又能量化ABCDE特征评分，模拟病变随时间演变过程，提高黑色素瘤早期检测的可解释性。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习模型对皮肤病变分析缺乏可解释性，无法解释ABCDE分类法中的人类可理解特征。需要将机器学习诊断与临床相关标准联系起来。

Method: 使用深度学习框架同时分类皮肤病变并量化ABCD特征评分，模拟E特征的演变过程，在潜在空间中可视化ABCD特征轨迹。实验使用HAM10000数据集。
...
TL;DR: 该论文提出了一种结合多种视觉模态的方法，使用预训练专家模型和程序化组合，在原始视频上构建全自动数据管道，并利用PHG-MAE模型有效处理多模态数据。


<details>
  <summary>Details</summary>
Motivation: 现实世界本质上是多模态的，但传统机器学习模型多为单模态，最近趋势是双模态学习。为了真正理解世界，需要整合所有独立的模态。

Method: 使用预训练专家模型和程序化组合，在原始视频上构建全自动数据管道，利用PHG-MAE模型处理多模态数据，并进行高效蒸馏。

Result: 经过蒸馏的低参数模型（<1M）在性能上可与约300M参数的模型竞争，并实现了在消费级硬件上的实时语义分割和近实时深度估计。

Conclusion: 该方法展示了多模态学习在低参数模型中的有效性，为在资源受限设备上部署多模态应用提供了可行方案。
...
TL;DR: 提出了Free-Form HOI生成方法，通过细粒度意图控制生成多样化、物理合理的手-物体交互，超越传统的抓握模式。


<details>
  <summary>Details</summary>
Motivation: 现有HOI生成研究局限于固定抓握模式，受物理先验约束，无法捕捉日常交互的多样性。需要支持推、戳、旋转等自由形式交互。

Method: 构建WildO2数据集，包含4.4k个独特交互；提出TOUCH三阶段框架，基于多级扩散模型，利用显式接触建模和物理约束细化。

Result: 能够生成可控、多样化且物理合理的手部交互，代表日常活动中的真实交互模式。

Conclusion: 该方法成功扩展了HOI从抓握到自由形式交互，通过细粒度语义控制实现了更真实多样的手-物体交互生成。
...
TL;DR: BADAS是一个基于真实世界行车记录仪碰撞数据集训练的碰撞预测模型系列，专门针对自车视角评估设计，在多个基准测试中实现了最先进的性能表现。


<details>
  <summary>Details</summary>
Motivation: 现有碰撞预测方法无法区分自车威胁和其他车辆事故，导致实际部署中产生过多误报。
...
Conclusion: BADAS通过专门的自我中心评估基准和重新标注的数据集，显著提升了碰撞预测的准确性和实用性，促进了自我中心碰撞预测研究的发展。
...
Method: 提出了改进的MMDiT块和参考注意力模块，通过参数重用和零初始化线性投影，在减少计算成本的同时稳定地注入控制信息。
...
TL;DR: 提出nlg2choice方法解决MLLMs在细粒度视觉分类中多选项MCQ评估问题，通过两阶段方法先开放式回答再约束解码预测最佳选项，在检索任务中使用早停机制提高效率


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注语言任务或5选项以内的MCQ，而细粒度视觉分类需要处理数百至数千个高度相关的选项，且不清楚如何将LLM选择提取扩展到检索问题

Method: nlg2choice两阶段方法：先让MLLM回答开放式问题，然后使用纯文本约束解码预测最可能的选项；在检索设置中使用早停机制计算约束响应的概率

Result: 在7个细粒度视觉数据集上的分类和检索评估中表现优于现有方法，且性能在不同自然语言任务实现方式下保持稳定

Conclusion: nlg2choice方法有效解决了MLLMs在细粒度视觉分类中的多选项MCQ评估挑战，在分类和检索任务中均表现优异
...
TL;DR: 提出基于多模态大语言模型的新颖视频异常检测框架，通过分析对象活动和交互的文本描述来检测复杂异常，并提供可解释性
...
Method: 使用MLLM分析不同时刻对象对的视觉输入，生成活动和交互的文本描述，作为视频中对象活动的高层表示，在测试时与正常训练视频的文本描述进行比较

Result: 在基准数据集上的广泛实验表明，该方法不仅能有效检测基于交互的复杂异常，在无交互异常的数据集上也达到了最先进性能

Conclusion: 该方法为视频异常检测提供了可解释性，并能与多种传统VAD方法结合以进一步增强其可解释性
...
TL;DR: 3DScenePrompt是一个视频生成框架，通过双时空条件和3D场景记忆实现长视频生成、精确相机控制和场景一致性保持。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常基于单张图像或短片段生成视频，难以保持长视频的场景一致性和精确相机控制，特别是在跨越时间边界时动态元素会错误保留。

Method: 采用双时空条件机制：时间相邻帧保证运动连续性，空间相邻内容保证场景一致性。引入3D场景记忆存储从整个输入视频提取的静态几何信息，通过动态SLAM和动态掩码策略分离静态场景和动态元素。

Result: 在场景一致性、相机可控性和生成质量方面显著优于现有方法，能够保持长距离空间连贯性和精确相机控制，同时不牺牲计算效率或运动真实性。
...
TL;DR: 提出了一种连续掩码自回归运动变换器，通过门控线性注意力和RMSNorm模块增强多模态人体运动生成，在文本到运动、语音到手势、音乐到舞蹈等任务上优于现有方法。
...
Method: 开发连续掩码自回归运动变换器，采用因果注意力考虑人体运动的序列特性，引入门控线性注意力和RMSNorm模块关注关键动作并抑制异常运动或多模态异质分布带来的不稳定性，使用DiT结构扩散条件信息，通过AdaLN和交叉注意力融合文本、语音、音乐信号。

Result: 实验结果表明，该框架在所有模态（文本到运动、语音到手势、音乐到舞蹈）上都优于先前的方法。

Conclusion: 提出的连续掩码自回归运动变换器能够有效处理多模态人体运动生成任务，在多个基准测试中表现出优越性能。
...
TL;DR: MathCanvas是一个赋予统一大型多模态模型内在视觉思维链能力的框架，通过视觉操作预训练和战略视觉辅助推理微调，显著提升数学几何问题的解决能力。


<details>
  <summary>Details</summary>
Motivation: 现有视觉思维链方法受限于外部工具的僵化性，无法生成高保真、时机恰当的可视化图表来支持复杂数学问题求解。

Method: 分两阶段：视觉操作阶段预训练模型掌握图表生成和编辑；战略视觉辅助推理阶段微调模型学习何时及如何利用视觉辅助。

Result: BAGEL-Canvas模型在MathCanvas-Bench上相比强基线LMMs实现86%相对提升，在其他公开数学基准上表现出优秀泛化能力。

Conclusion: 该工作提供了完整的工具包（框架、数据集和基准），解锁了LMMs中复杂、类人的视觉辅助推理能力。
...
TL;DR: 提出了一种集成令牌注意力的扩散模型，用于降水临近预报，在保持计算效率的同时显著提升了长程时空依赖建模能力。


<details>
  <summary>Details</summary>
Motivation: 解决现有扩散模型在降水临近预报中的可扩展性问题：潜在空间方法需要额外训练自编码器，而像素空间方法计算成本高且缺乏注意力机制，难以建模长程时空依赖。

Method: 在U-Net扩散模型和时空编码器中集成令牌注意力机制，动态捕捉多尺度空间交互和时间演化，无需单独潜在模块。

Result: 在多个数据集上的实验表明，该方法显著优于现有最先进方法，在复杂降水预报场景中展现出优越的局部保真度、泛化性和鲁棒性。

Conclusion: 提出的令牌注意力集成扩散模型有效解决了降水临近预报中的计算效率和建模能力平衡问题，为复杂气象预测提供了高效解决方案。
...
TL;DR: 提出了ChangingGrounding基准和Mem-ChangingGrounder方法，用于在动态变化场景中进行3D视觉定位，通过记忆驱动的方法减少重复扫描需求。


<details>
  <summary>Details</summary>
Motivation: 现有3D视觉定位方法假设场景点云是重建且最新的，这在实际应用中需要昂贵的重复扫描，阻碍了部署。需要将3DVG重新定义为主动的、记忆驱动的问题。

Method: 提出Mem-ChangingGrounder方法：通过跨模态检索识别查询对象类型，检索相关记忆指导行动，高效探索目标场景，执行多视角扫描，并将融合证据投影以获得精确边界框。
...
TL;DR: Ponimator是一个基于近距离交互姿态的通用交互动画框架，使用两个条件扩散模型从交互姿态生成动态运动序列，支持图像基交互动画、反应动画和文本到交互合成等任务。


<details>
  <summary>Details</summary>
Motivation: 近距离人-人交互姿态蕴含丰富的交互动态上下文信息，人类能基于行为先验从这些姿态推断上下文并预测可能的过去和未来动态。
...
TL;DR: 提出无需配对数据的新训练范式，通过展开扩散模型并利用视觉语言模型反馈直接优化，在少步设置下达到与监督训练方法相当的性能


<details>
  <summary>Details</summary>
Motivation: 现有图像编辑模型依赖大规模输入-目标配对数据进行监督微调，但自然配对大尺度收集困难，而合成训练对会传播预训练模型的缺陷

Method: 直接优化少步扩散模型：训练时展开模型，利用视觉语言模型评估编辑是否遵循指令并保留未改变内容，提供端到端优化梯度；结合分布匹配损失确保视觉保真度

Result: 在标准基准测试中，无需任何配对数据，在少步设置下与使用大量监督配对数据训练的扩散模型性能相当；使用相同VLM作为奖励模型时，优于Flow-GRPO等基于强化学习的方法

Conclusion: 该方法成功消除了对配对数据的依赖，通过直接优化和VLM反馈实现了高质量的图像编辑，为图像编辑模型训练提供了新范式
...
TL;DR: CodeEvolve是一个开源进化编码代理，结合大型语言模型和遗传算法解决复杂计算问题，在多个数学基准测试中超越了DeepMind的AlphaEvolve。
...
<details>
  <summary>Details</summary>
Motivation: 传统方法依赖预定义的有效性框架，但这些框架可能不可用或不充分。需要一种新方法来确定模型有效性，特别是在指导决策过程时。

Method: DOTechnique方法通过评估替代模型与高保真模型的决策一致性来确定模型有效性，结合领域约束和符号推理来缩小搜索空间，提高计算效率。

Result: 以高速公路变道系统为例，展示了DOTechnique能够发现仿真模型的有效区域，证明了该技术的潜力。

Conclusion: DOTechnique能够通过决策者上下文支持寻找模型有效性，为模型验证提供了新视角。
...
TL;DR: 该论文提出了一种融合视觉信息（特别是演示文稿幻灯片）的多模态自动语音识别系统，用于改善科学演示场景下的语音识别性能，特别是在领域特定术语识别方面。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的ASR系统主要依赖声学信息而忽略了多模态上下文。视觉信息对于消歧和适应至关重要，特别是在科学演示场景中，演示文稿幻灯片包含重要的上下文信息。

Method: 首先创建多模态演示基准，包括领域特定术语的自动分析；然后探索用多模态信息增强语音模型的方法；通过数据增强方法解决缺乏配套幻灯片数据集的问题；最后使用增强数据集训练模型。

Result: 训练出的模型在所有词汇上的词错误率相对降低了约34%，在领域特定术语上的词错误率相对降低了约35%，相比基线模型有显著提升。
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在探索大型语言模型是否像人类一样容易产生因果幻觉，特别是在经典的认知科学范式——列联判断任务中。

Method: 构建了包含1000个零列联场景的医疗数据集，让LLMs评估潜在原因的效力，测试其在缺乏足够证据时的因果推理能力。
...
TL;DR: GammaZero提出了一种基于动作中心图表示的学习框架，用于在部分可观测马尔可夫决策过程中指导规划。该方法使用统一的图表示实现跨问题规模的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法需要领域特定的神经网络架构且难以扩展，GammaZero旨在解决这些问题，通过学习小规模问题上的结构模式来泛化到更大规模的问题实例。

Method: 将信念状态系统性地转换为动作中心图，使用图神经网络和编码器架构从专家演示中学习价值函数和策略，然后将学习到的启发式方法应用于引导大规模问题上的蒙特卡洛树搜索。

Result: 在标准POMDP基准测试中，GammaZero在相同规模问题上与BetaZero性能相当，同时能够零样本泛化到训练时未见过的2-4倍大规模问题，保持解质量的同时减少搜索需求。

Conclusion: GammaZero通过动作中心图表示框架成功实现了在POMDP中跨问题规模的泛化能力，为可扩展的规划学习提供了有效解决方案。
...
TL;DR: 提出一种替代性监管方法：要求大型AI实验室发布小型开放模拟模型，这些模型是从其最大专有模型中蒸馏训练而来的缩小版本，既能确保AI安全又能促进创新。
...
Method: 强制要求大型AI实验室发布小型开放模拟模型，这些模型采用类似方式训练并从其最大专有模型中蒸馏而来，作为公共代理供广泛参与安全验证和可解释性研究。

Result: 小型模拟模型使更广泛的研究社区能够直接研究和创新，显著减少监管负担并加速安全进展，同时成本最小化。

Conclusion: 这种监管方法能够缓解安全与创新的权衡，通过更深入理解模型来实现更多的安全与创新，并支持机器学习基础研究的原则。
...
TL;DR: 该论文提出了一种基于多目标马尔可夫决策过程的共识声明生成框架，通过社会选择理论保证公平性，包括两种方法：随机生成策略保证核心稳定性，以及搜索算法最大化平等主义福利。
...
Method: 将任务建模为多目标、令牌级马尔可夫决策过程，每个目标对应一个代理的偏好。提出两种基于社会选择理论的方法：保证处于事前核心的随机生成策略，以及使用搜索算法最大化平等主义福利的单一声明生成方法。

Result: 实验表明，使用语言模型实例化代理策略时，基于平等主义目标的搜索生成的共识声明相比基线方法（包括Habermas Machine）在最低代理对齐方面有改进。
...
TL;DR: 提出STEMS框架，一种用于协调建筑能源管理的安全约束多智能体强化学习方法，通过空间-时间图表示学习和控制屏障函数实现安全保证，显著降低能耗、排放和安全违规。


<details>
  <summary>Details</summary>
Motivation: 解决多建筑能源系统中空间-时间依赖关系利用不足、缺乏严格安全保证和系统复杂性三大挑战，实现碳减排目标、提升居住者舒适度和降低能源成本。

Method: STEMS框架包含两个核心组件：(1) GCN-Transformer融合架构的空间-时间图表示学习框架，捕捉建筑间关系和时间模式；(2) 集成控制屏障函数的安全约束多智能体强化学习算法，提供数学安全保证。

Result: 在真实建筑数据集上的实验显示，STEMS相比现有方法实现21%成本降低、18%排放减少，安全违规从35.1%大幅降至5.6%，同时仅0.13的不适比例保持最优舒适度。

Conclusion: STEMS框架在多建筑能源管理中表现出卓越性能，在极端天气条件下保持强鲁棒性，并在不同建筑类型中保持有效性，为协调建筑能源管理提供了安全可靠的解决方案。
...
TL;DR: 提出一个建模框架来解决多智能体AI系统中的语义鸿沟问题，包含主机智能体模型和任务生命周期模型，定义了31个形式化属性来确保系统的正确性、可靠性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前多智能体AI系统的通信协议碎片化，导致语义鸿沟，无法进行严格的系统属性分析，存在架构错位和可被利用的协调问题风险。

Method: 引入由两个基础模型组成的建模框架：主机智能体模型（负责与用户交互、任务分解和协调执行）和任务生命周期模型（详细描述子任务从创建到完成的状态转换）。

Result: 定义了17个主机智能体属性和14个任务生命周期属性，涵盖活性、安全性、完整性和公平性类别，使用时序逻辑表达，支持形式化验证。

Conclusion: 这是第一个严格基础的、领域无关的框架，用于系统分析、设计和部署正确、可靠、鲁棒的多智能体AI系统。
...
<details>
  <summary>Details</summary>
Motivation: 文化遗产面临气候变化加速退化，传统单模态监测（视觉检查或环境传感器）无法捕捉环境压力与材料退化间的复杂相互作用。

Method: 采用改进的PerceiverIO架构，包含简化编码器（64D潜空间）防止小数据集过拟合，以及自适应Barlow Twins损失函数促进模态互补性。

Result: 模型准确率76.9%，比标准多模态架构提升43%，比单模态传感器（61.5%）和图像（46.2%）表现更好。系统超参数研究发现最佳相关性目标τ=0.3。
...
TL;DR: 论文探讨了强化学习在商业游戏中的应用挑战，提出将强化学习与传统行为树结合的方法，并通过AMD Schola插件在复杂3D环境中验证了该方法的可行性。


<details>
  <summary>Details</summary>
Motivation: 尽管强化学习研究进展迅速，但在商业游戏中的应用仍然缓慢。论文旨在解决游戏AI社区在使用RL驱动的NPC时面临的常见挑战，并探索RL与行为树的结合点。

Method: 使用AMD Schola插件在虚幻引擎中训练RL智能体，创建多任务NPC，在受《最后生还者》启发的复杂3D环境中展示联合训练RL模型与行为树的方法。

Result: 验证了RL与行为树结合方法的可行性，展示了在复杂环境中训练多任务NPC的各种技能。

Conclusion: RL与行为树的交集是一个值得进一步探索的关键节点，虽然已有研究提出，但实际应用仍然罕见，本文证明了该方法的实际可行性。
...
TL;DR: JEDA是一个用于临床订单检索的双编码器系统，通过联合嵌入直接和上下文对话来实时触发临床订单检索，无需LLM重写，减少延迟和不稳定性。
...
Method: 基于PubMedBERT初始化的双编码器，使用重复安全的对比目标进行微调，通过受限LLM指导将不同意图表达对齐到共享订单概念，支持无查询模式编码滚动对话窗口。

Result: 在实践中部署显示大幅性能提升，显著优于基础编码器和最新开源嵌入模型，无查询模式对噪声具有弹性。
...
Method: 使用基础模型从自然语言规范自动生成奖励机器；为每个自动机状态关联语言嵌入以实现任务间泛化；利用奖励机器的结构化形式化方法进行有效任务分解。

Result: 在多样化挑战性环境中验证了ARM-FM的有效性，展示了零样本泛化能力，证明该方法能够自动构建有效的奖励函数。

Conclusion: ARM-FM通过结合基础模型和奖励机器，实现了从自然语言到结构化奖励函数的自动转换，解决了强化学习中奖励函数设计的核心挑战，具有良好的泛化性能。
...
Result: 识别了数据质量、临床可靠性、工作流程整合和治理四个主要维度的障碍和促进因素，强调了现实世界转化中的相互依赖关系。
...
TL;DR: 提出了在线骚扰代理基准，包括多轮骚扰对话数据集、多代理模拟、三种越狱攻击方法和混合评估框架。研究发现越狱调优显著提高了骚扰攻击成功率，降低了拒绝率，并揭示了封闭源和开源模型的不同升级轨迹。
...
Method: 创建了包含多轮骚扰对话数据集的在线骚扰代理基准，采用基于重复博弈论的多代理模拟，开发了针对记忆、规划和微调的三种越狱攻击方法，并使用混合方法评估框架。

Result: 越狱调优使骚扰攻击成功率大幅提升（Llama从57-64%升至96-97%，Gemini从98%升至99%），拒绝率降至1-2%。最常见的毒性行为是侮辱（84-88%）和谩骂（81-85%）。封闭源模型显示出显著脆弱性。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准在评估深度研究系统时存在不足，无法满足用户中心、动态性、明确性和多维度搜索等关键原则。

Method: 构建了包含100个专家策划任务的LiveResearchBench基准，并开发了DeepEval评估套件，涵盖内容和报告层面的质量评估。

Result: 对17个前沿深度研究系统进行了全面评估，揭示了当前系统的优势、常见失败模式和关键系统组件需求。

Conclusion: LiveResearchBench和DeepEval为深度研究系统提供了严格的评估基础，有助于推动可靠、有洞察力的深度研究发展。
...
TL;DR: 提出了Agentic Self-Learning (ASL)框架，通过生成式奖励模型和多角色协同进化实现无人工标注数据的智能体自学习，在搜索任务中表现出持续改进能力。
...
Result: ASL在搜索任务中实现稳定轮次增益，超越强基线方法，在零标注数据条件下持续改进，表明具有优越的样本效率和鲁棒性。
...
TL;DR: 提出了MorphoBench基准测试，用于评估大型模型的推理能力，能够根据模型能力自适应调整问题难度，包含1300+测试问题。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试评估大型模型推理能力时存在范围有限、缺乏灵活性、无法适应模型推理能力演进的问题。
...
TL;DR: GuardSpace是一个用于在微调过程中保持大语言模型安全对齐的框架，通过安全敏感子空间和有害抵抗零空间来保护预训练的安全行为。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在微调过程中容易丧失预训练的安全对齐能力，即使使用良性数据或低秩适配也会导致有害响应。

Method: 使用协方差预处理奇异值分解将预训练权重分解为安全相关和安全无关组件，从安全无关组件初始化低秩适配器并冻结安全相关组件；构建零空间投影器限制适配器更新对有害提示的安全输出。

Result: 在多个下游任务上的实验表明，GuardSpace优于现有方法。对于在GSM8K上微调的Llama-2-7B-Chat，平均有害分数从14.4%降至3.6%，准确率从26.0%提升至28.0%。

Conclusion: GuardSpace框架能有效在微调过程中保持大语言模型的安全对齐，同时提升任务性能。
...
<details>
  <summary>Details</summary>
Motivation: LLM驱动的多智能体系统能够自动化繁琐的用户任务，但引入了新的风险，包括错位、恶意方攻击、智能体被破坏或用户数据被盗。

Method: 重新利用早期的多智能体系统黑板设计，创建模块化、可配置的测试平台，识别关键攻击向量并实现三个协作多智能体场景和四种代表性攻击。
...
Method: MASC采用两种互补设计：(1) 下一执行重构，从查询和交互历史预测下一步的嵌入以捕捉因果一致性；(2) 原型引导增强，学习正常步骤嵌入的原型先验，在稀疏上下文下稳定重构和异常评分。检测到异常时触发纠正智能体。

Result: 在Who&When基准测试中，MASC持续优于所有基线方法，步骤级错误检测AUC-ROC提升高达8.47%；在不同多智能体框架中都能带来一致的端到端性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 现有AI服务多为被动响应，需要用户明确指令。真正智能的助手应能预见用户需求并在适当时机主动提供帮助
...
TL;DR: 提出IP-Merging方法，通过识别多模态大语言模型和数学大语言模型中的推理相关参数，将其投影到MLLM子空间进行融合，无需调优即可提升MLLM的数学推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型的数学推理能力落后于纯文本大语言模型，希望探索能否在不进行调优的情况下，让MLLM直接从现成的数学LLM中吸收推理能力。

Method: IP-Merging方法：1）识别MLLM和数学LLM中的推理相关参数；2）将这些参数投影到MLLM的子空间；3）在子空间中进行参数融合。这是一个无需调优的直接参数调整方法。

Result: 大量实验表明，IP-Merging方法能够在不损害MLLM其他能力的前提下，直接从数学LLM中增强MLLM的数学推理能力。

Conclusion: IP-Merging是一种有效的免调优方法，能够成功地将数学推理能力从纯文本LLM迁移到多模态LLM中，填补了两种模型参数空间之间的差距。
...
Method: 提出分层架构：高层推理模型进行规划，低层动作模型执行操作。将多步决策重构为单步子目标序列，提出前瞻优势函数，利用低层模型执行反馈指导高层优化，缓解长时任务中的路径爆炸问题。

Result: 在Android-in-the-Wild基准测试中达到87.9%的任务成功率，显著优于提示式方法(17.7%)、监督学习方法(54.5%)和强化学习方法(71.9%)。在ScreenSpot-v2上表现出有竞争力的零样本泛化能力。

Conclusion: Hi-Agent通过分层设计和联合优化策略，在移动设备控制任务中实现了最先进的性能，并展现出良好的泛化能力和可扩展性。
...
TL;DR: 提出了IMAGINE框架，将多智能体系统的推理规划能力整合到单一紧凑模型中，显著提升了复杂推理任务的性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在复杂推理和规划任务中表现不佳，多智能体系统虽然能改善集体推理但存在推理成本高、延迟长和端到端训练困难的问题。

Method: IMAGINE框架通过端到端训练将多智能体系统的能力整合到单一模型中，使小规模模型获得结构化推理和规划能力。

Result: 使用Qwen3-8B-Instruct作为基础模型，在TravelPlanner基准测试中达到82.7%的最终通过率，远超DeepSeek-R1-671B的40%。

Conclusion: IMAGINE框架成功地将多智能体系统的优势整合到单一模型中，显著提升了推理规划能力，同时保持了较小的模型规模。
...
Method: 提出一种转换方法，通过消除派生谓词的负出现，将包含负出现的公理集转换为符合PDDL标准限制的形式。

Result: 证明了包含派生谓词负出现的公理集与符合PDDL限制的公理集在表达能力上是等价的，都能表达最小不动点逻辑的查询。

Conclusion: 通过提出的转换方法，可以消除PDDL公理中派生谓词的负出现，从而统一文献中的不同处理方法，同时保持相同的表达能力。
...
TL;DR: Helmsman是一个多智能体系统，能够从高级用户规范自动合成联邦学习系统，通过交互式规划、模块化代码生成和自主评估三个协作阶段，生成的解决方案优于手工设计的基准。


<details>
  <summary>Details</summary>
Motivation: 联邦学习在分散数据上训练模型具有强大潜力，但设计和部署稳健系统的复杂性成为关键瓶颈，导致脆弱、定制化的解决方案。

Method: Helmsman通过三个协作阶段实现：交互式人机循环规划制定研究计划、监督智能体团队进行模块化代码生成、在沙盒模拟环境中进行自主评估和优化的闭环。

Result: 大量实验表明，该方法生成的解决方案与手工设计的基准相比具有竞争力，且通常更优。

Conclusion: 这项工作代表了向复杂分散AI系统自动化工程迈出的重要一步。
...
TL;DR: JSPLIT是一个基于分类学的框架，通过层次化组织工具并基于用户提示选择最相关工具，有效解决MCP工具使用中的提示膨胀问题，显著减少提示大小同时保持代理性能。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统发展，用户期望从简单的文本交互转向需要LLM与外部工具交互的复杂代理系统。MCP等标准使代理能够访问工具，但随着工具数量增加，提示变得冗长，导致高令牌成本、延迟增加和任务成功率下降。

Method: JSPLIT将工具组织成层次化分类学结构，使用用户提示基于查询和分类学结构识别并仅包含最相关工具，包括分类学设计、工具选择算法和评估数据集。

Result: JSPLIT显著减少了提示大小，且未显著影响代理响应能力。当可用工具数量大幅增加时，JSPLIT甚至提高了代理的工具选择准确性，在降低成本的同时改善高复杂度环境中的任务成功率。

Conclusion: JSPLIT框架有效解决了MCP工具使用中的提示膨胀问题，通过分类学驱动的方法在减少提示大小的同时保持甚至提升代理性能，特别适用于工具数量大幅增长的复杂代理环境。
...
TL;DR: 神经符号AI中存在的推理捷径问题综述：当概念未被直接监督时，模型可能通过错误地基础化概念来获得高标签准确率，这会损害模型的可解释性、分布外性能及可靠性。本文系统介绍了推理捷径的成因、后果、理论特征及应对方法。


<details>
  <summary>Details</summary>
Motivation: 神经符号AI旨在开发符合先验知识约束的深度神经网络，是实现可靠可信AI的重要途径。然而，当概念未被直接监督时，模型可能受到推理捷径的影响，即通过错误地基础化概念来获得高准确率，这会损害模型的可解释性、分布外性能和可靠性。

Method: 本文提供了推理捷径的温和介绍，直观讨论了其成因和后果，回顾并阐明了该现象的理论特征，详细介绍了处理推理捷径的方法，包括缓解和认知策略，并分析了各种方法的优缺点。

Result: 通过将高级材料重新表述为易于理解的形式，本文提供了关于推理捷径的统一视角，降低了解决该问题的入门门槛。

Conclusion: 本文希望通过对推理捷径的系统综述，为开发可靠的神经符号AI和可信AI模型做出贡献。
...
TL;DR: 本文研究如何将预训练LLM代理转变为能够自主生成任务、积累知识并追求长期目标的开放式实体，探讨了其潜力与当前局限。


<details>
  <summary>Details</summary>
Motivation: 随着LLM代理能力的提升，研究其是否能从单纯的问题解决工具转变为能够自主规划、设计任务并追求模糊长期目标的独立实体。

Method: 采用开放式实验设置，增强预训练LLM代理的能力，使其能够生成自己的任务、积累知识并与环境广泛互动，进行定性研究。

Result: 代理能够可靠执行复杂多步骤指令、跨运行存储和重用信息、提出并解决自己的任务，但对提示设计敏感、易重复生成任务、无法形成自我表征。

Conclusion: 研究显示了将预训练LLM适配到开放式的潜力与当前限制，指出了未来在内存管理、生产性探索和追求抽象长期目标方面的训练方向。
...
TL;DR: 本文提出了ColorBench，一种用于评估移动代理在复杂长任务中性能的图结构基准框架，解决了现有评估方法只能验证单一预设路径的问题。


<details>
  <summary>Details</summary>
Motivation: 现有移动代理评估标准存在局限性：离线静态基准只能验证单一"黄金路径"，而在线动态测试受限于真实设备的复杂性和不可重现性，无法全面评估代理能力。

Method: 通过建模真实设备交互中观察到的有限状态，实现动态行为的静态模拟，开发了专注于复杂长任务的ColorBench基准，支持多有效解评估、子任务完成率统计和原子级能力分析。

Result: ColorBench包含175个任务（74个单应用，101个跨应用），平均长度超过13步，每个任务至少包含两条正确路径和若干典型错误路径，实现了准动态交互。

Conclusion: 通过在不同基线模型上评估ColorBench，发现了现有模型的局限性，并基于实验结果提出了改进方向和可行的技术路径，以增强代理在复杂长任务上的性能。
...
TL;DR: 本文提出了Rose-Frame框架，用于诊断人类与AI交互中的认知和认识论偏差。该框架包含三个维度：地图与领土（表征与现实）、直觉与理性（快速与慢速思维）、冲突与确认（批判性测试与相互验证）。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然流畅且具有情感共鸣，但基于统计预测而非有根据的推理，存在幻觉风险。它们实现了系统1认知的大规模操作化：快速、联想且具有说服力，但缺乏反思或证伪。

Method: 引入Rose-Frame三维诊断框架：(i)地图与领土区分现实表征与现实本身；(ii)直觉与理性基于双过程理论分离快速情感判断与慢速反思思维；(iii)冲突与确认检验观点是通过分歧批判性测试还是仅通过相互验证强化。

Result: Rose-Frame不试图用更多数据或规则修复LLM，而是提供一个反思工具，使模型局限性和用户假设可见，实现更透明和批判性意识的AI部署。

Conclusion: 将对齐重新定义为认知治理：无论是人类还是人工智能的直觉，都必须受到人类理性的治理。只有通过嵌入反思性、可证伪的监督，才能使机器流畅性与人类理解保持一致。
...
TL;DR: 该论文对2021-2025年荷兰公共卫生机器学习研究中的算法偏见进行了系统文献综述，开发了RABAT评估工具，发现普遍存在公平性框架缺失等问题，并提出了ACAR四阶段公平性框架。


<details>
  <summary>Details</summary>
Motivation: 机器学习在公共卫生领域有巨大潜力，但如果不系统关注算法偏见，可能会无意中加剧现有的健康不平等。需要评估当前研究中算法偏见的识别、讨论和报告情况。

Method: 开发了RABAT评估工具，整合了Cochrane偏见风险、PROBAST和微软负责任AI检查表等框架，对35篇同行评审研究进行了系统分析。

Result: 分析显示普遍存在差距：虽然数据抽样和缺失数据处理有良好记录，但大多数研究缺乏明确的公平性框架、亚组分析和潜在危害的透明讨论。

Conclusion: 提出了ACAR四阶段公平性框架（意识、概念化、应用、报告），为公共卫生ML从业者提供可操作建议，确保算法创新促进而非破坏健康公平。
...
Method: 集成路径规划模型预测逻辑关系链，图执行器遍历TITAN本体检索事实答案和支持证据，基于MITRE构建类型化双向图。

Result: 创建了包含88209个示例的TITAN数据集，实证评估显示模型能生成语法有效且语义连贯的推理路径，可在底层图上确定性执行。

Conclusion: TITAN框架成功实现了自然语言威胁查询到结构化知识图谱的可执行推理，为网络安全威胁情报分析提供了新方法。
...
TL;DR: 提出NAEL（非人类中心主义伦理逻辑）框架，基于主动推理和符号推理为AI代理构建伦理系统，强调伦理行为是智能系统在动态多智能体环境中最小化全局预期自由能量的涌现属性。


<details>
  <summary>Details</summary>
Motivation: 解决现有AI伦理模型局限于人类中心主义的问题，让AI代理能在不确定环境中发展情境敏感、自适应和关系性的伦理行为，无需预设人类道德直觉。

Method: 采用神经符号架构，结合主动推理和符号推理，使代理能够评估行为在不确定环境中的伦理后果。通过案例研究展示伦理资源分配中的动态平衡。

Result: NAEL框架能够实现自我保存、认知学习和集体福利之间的动态平衡，克服了传统伦理模型的局限性。

Conclusion: NAEL为非人类中心主义的AI伦理提供了可行的理论基础和实现路径，使AI系统能够发展出适应性的伦理行为模式。
...
<details>
  <summary>Details</summary>
Motivation: COUP方法虽然具有理论保证，但实际性能不如启发式配置方法。本文旨在弥合这一差距，使理论基础的实用算法配置方法在实践中有竞争力。

Method: 提出了一系列对COUP的改进措施，这些改进在不降低理论保证的前提下提升了实证性能，并通过实验验证了其有效性。

Result: 改进后的COUP方法在实证性能上达到了与广泛使用的启发式配置方法相当的水平，同时保持了理论性能保证。

Conclusion: 通过系统改进，成功将理论基础的实用算法配置方法提升到了实用水平，为算法配置提供了既有理论保证又有良好性能的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有模型融合方法中，任务向量包含任务无关的冗余信息，导致融合模型性能显著下降。现有方法随机丢弃参数元素，缺乏知识感知且存在随机性。

Method: 从每个任务采样训练样本，通过对应微调模型获取线性层前的协方差矩阵，执行上下文导向的奇异值分解，突出与目标知识最相关的权重分量，将微调模型权重分解为任务相关和冗余成分，通过剪枝冗余成分净化任务向量。引入谱秩分配策略优化归一化激活剪枝误差。

Result: PAVE作为即插即用方案，适用于各种基于任务向量的融合方法，在多种融合方法、任务和模型架构中均表现出有效性。

Conclusion: PAVE方法能够有效净化任务向量中的冗余信息，显著提升模型融合性能，且具有良好的通用性。
...
TL;DR: CoAST是一个认知对齐的时空大语言模型框架，用于下一个兴趣点推荐，通过自然语言接口整合世界知识、时空轨迹模式和用户信息，提升推荐性能。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型主要基于非结构化文本预训练，缺乏对结构化地理实体和序列移动模式的理解，且工业级POI推荐需要结合世界知识和人类认知对齐来提升用户体验。

Method: CoAST包含两个阶段：(1) 推荐知识获取：在去敏化的用户时空轨迹数据上继续预训练；(2) 认知对齐：通过监督微调和强化学习将认知判断与人类偏好对齐。

Result: 在多个真实数据集上的离线实验和AMAP App首页"猜你去哪"功能的在线实验证明了CoAST的有效性。

Conclusion: CoAST框架能够有效整合世界知识和认知对齐，显著提升下一个兴趣点推荐的性能。
...
Conclusion: 揭示了将推理扩展技术应用于结构化输出的关键原则：由于结构化函数调用生成的不可恢复特性，需要"更多探索但保留更少"。
...
TL;DR: 提出SimKO方法解决RLVR训练中的过度集中问题，通过不对称设计提升探索能力，改善pass@K性能


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法存在系统性偏向利用而非探索的问题，表现为pass@1提升但pass@K下降

Method: 分析训练动态发现概率集中效应，提出SimKO方法：对已验证正确响应提升top-K候选概率，对错误响应惩罚top-1候选

Result: 在多个数学和逻辑推理基准测试中，SimKO能持续提高各种K值的pass@K性能

Conclusion: SimKO通过缓解过度集中问题有效提升RLVR的探索能力，为改进RLVR提供了简单有效的方法
...
TL;DR: Datalake Agent是一个基于LLM的智能代理系统，通过交互式循环选择性请求必要信息，显著减少NL2SQL任务中的token使用量，降低成本同时保持性能。
...
Method: 采用交互式循环和推理框架，让LLM选择性请求必要信息来解决表问答任务，而非一次性提供所有元信息。

Result: 在23个数据库和100个表问答任务上的评估显示，token使用量减少高达87%，成本显著降低，同时保持竞争力性能。
...
TL;DR: 提出了RoboGPT-R1，一个两阶段微调框架，通过监督学习和强化学习相结合的方式提升具身智能体在长视野操作任务中的推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型和视觉语言模型在规划任务中面临常识和推理能力受限的问题，特别是在复杂真实环境中的长视野操作任务中表现不佳。

Method: 采用两阶段微调框架：监督训练阶段通过专家序列获取基础知识，强化学习阶段解决模型在视觉空间理解和推理方面的不足，设计了基于规则的奖励函数来考虑长期性能和动作约束。

Result: 在Qwen2.5-VL-3B上训练的推理模型在EmbodiedBench基准测试中显著优于GPT-4o-mini 21.33%，并超过其他在Qwen2.5-VL-7B上训练的工作20.33%。

Conclusion: 两阶段微调框架有效提升了具身智能体的推理能力，证明了小模型通过适当训练方法可以超越更大规模模型在具身规划任务中的表现。
...
Method: 引入Instruction Boosting作为后生成方法，并创建SCALEDIF基准测试，包含最多10条指令的数据样本，同时开发定量冲突评分工具。

Result: Instruction Boosting显著提高了指令遵循率，同时发现性能下降与指令间的紧张和冲突程度相关。

Conclusion: Instruction Boosting是提高LLM指令遵循可靠性的有效方法，冲突评分工具可为开发者提供关于额外提示指令对模型性能影响的反馈。
...
TL;DR: 提出了一种紧凑的形式理论来描述和衡量基于领域先验指导的LLM辅助迭代搜索，通过模糊关系算子表示智能体，引入覆盖生成函数来衡量可达性难度。
...
Method: 将智能体表示为输入输出的模糊关系算子，通过安全包络约束智能体；使用单参数加权所有可达路径得到覆盖生成函数，从而衡量可达性难度；提供可测试的推断并通过多数投票实例验证。

Result: 该理论提供了可操作的语言和工具来测量智能体及其搜索空间，为LLM构建的迭代搜索提供了系统的形式化描述。

Conclusion: 提出的形式理论为LLM辅助的迭代搜索提供了系统化的描述框架和测量工具，能够更好地理解和优化基于领域先验的搜索过程。
...
TL;DR: LabOS是首个将计算推理与物理实验结合的AI共同科学家系统，通过多模态感知、自进化代理和XR技术实现人机协作，将实验室转变为智能协作环境。


<details>
  <summary>Details</summary>
Motivation: 现代科学发展需要思想与行动的结合，当前AI主要停留在计算设计层面，缺乏与物理实验的实时交互能力。

Method: 通过连接多模型AI代理、智能眼镜和人机协作，让AI能够看到科学家所见、理解实验背景并实时协助执行。

Result: 在癌症免疫治疗靶点发现和干细胞工程等应用中，LabOS展示了AI从计算设计到实际参与实验的能力。

Conclusion: LabOS证明了AI可以超越计算设计，真正参与实验过程，使人类和机器发现共同进化，将实验室转变为智能协作环境。
...
TL;DR: 提出了Gatekeeper协议，一个领域无关的框架，通过让AI代理先在低保真潜在状态表示上操作，按需请求高保真上下文，来解决LLM上下文窗口限制和状态不同步问题。


<details>
  <summary>Details</summary>
Motivation: LLM作为自主代理部署时，受到有限上下文窗口和状态不同步的约束，导致不可靠输出、不可预测行为和低效资源使用，特别是在与代码库和文档等结构化知识系统交互时。

Method: 引入Gatekeeper协议，要求代理首先在低保真的潜在状态表示上进行操作和推理，然后按需策略性地请求高保真上下文。所有交互通过统一的JSON格式进行中介，作为声明式、状态同步的协议。

Result: 在软件开发的参考实现Sage中，该方法显著提高了代理可靠性，通过最小化token消耗改善了计算效率，并实现了与复杂系统的可扩展交互。

Conclusion: Gatekeeper协议为在任何结构化知识领域构建更稳健、可预测和接地的AI代理创建了基础方法学。
...
<details>
  <summary>Details</summary>
Motivation: 企业智能平台需要集成来自多个第三方供应商的日志，但供应商文档在测试时往往不可用、不匹配、格式混乱或不完整，导致模式映射困难。

Method: 使用强化学习代理：1)识别模糊的字段映射尝试；2)生成针对性网络搜索查询收集外部证据；3)应用基于置信度的奖励迭代优化映射。

Result: 在将Microsoft Defender for Endpoint日志转换为通用模式的任务中，映射准确率从56.4%(仅LLM)提升到72.73%(RAG)再到93.94%(100次迭代后)，同时将需要专家审查的低置信度映射减少了85%。

Conclusion: 该方法提供了一种基于证据的透明解决方案，为更鲁棒、可问责、可扩展、高效、灵活、适应性强和协作的解决方案铺平了道路。
...
Conclusion: 预算感知的判别式验证不仅是自一致性的免费升级，而且是比昂贵生成式技术更有效和高效的替代方案。
...
TL;DR: 该论文系统探索了使用EEG、语音和文本进行多模态抑郁症检测，比较了手工特征与预训练嵌入、不同神经编码器、单模态/双模态/三模态配置以及融合策略，发现三模态组合能提升检测性能，预训练嵌入优于手工特征。


<details>
  <summary>Details</summary>
Motivation: 现有抑郁症自动检测研究范围有限，缺乏系统性的特征比较和一致的评估协议，需要填补这些空白来推进多模态抑郁症检测研究。

Method: 系统探索EEG、语音和文本的特征表示和建模策略，评估手工特征与预训练嵌入，比较不同神经编码器、单模态/双模态/三模态配置，分析融合策略，并应用一致的主体独立分割进行稳健可复现的基准测试。

Result: 结果显示：(i) EEG、语音和文本多模态组合能增强多模态检测；(ii) 预训练嵌入优于手工特征；(iii) 精心设计的三模态模型达到最先进的性能。

Conclusion: 该研究为未来多模态抑郁症检测研究奠定了基础，证明了多模态方法的有效性。
...
TL;DR: 该论文将康德的《纯粹理性批判》重新解释为反馈稳定性理论，提出复合不稳定性指数(H-Risk)来衡量推理系统的稳定性，发现LLM的脆弱内部动态与错误校准和幻觉相关。


<details>
  <summary>Details</summary>
Motivation: 将康德的理性批判理论与反馈控制理论相结合，为诊断和减少推理系统中的过度自信提供理论框架。

Method: 提出复合不稳定性指数H-Risk，结合频谱裕度、条件数、时间敏感性和创新放大等指标，在线性高斯模拟和LLM中进行实验验证。

Result: 在模拟中，高H-Risk预测过度自信错误；在LLM中，脆弱内部动态与错误校准和幻觉相关，批判式提示对校准和幻觉有混合效果。

Conclusion: 建立了康德自我限制理论与反馈控制之间的结构性桥梁，为诊断和选择性减少推理系统中的过度自信提供了原则性视角。
...
TL;DR: GroundedPRM是一个基于树引导和保真度的自动过程监督框架，通过蒙特卡洛树搜索构建结构化推理路径，使用外部工具验证中间步骤，融合工具验证和MCTS反馈的混合奖励机制，在少量数据上实现高质量的过程级推理。


<details>
  <summary>Details</summary>
Motivation: 现有的过程奖励模型面临三大挑战：噪声奖励、低事实保真度以及与步骤级推理目标的不对齐。这些挑战源于缺乏可扩展的高质量标注、人类标注成本高、LLM自评估易产生幻觉，以及蒙特卡洛估计导致的信用分配错误。

Method: 1. 使用蒙特卡洛树搜索构建结构化推理路径以减少奖励噪声；2. 通过外部工具验证每个中间步骤，提供基于执行的正确性信号；3. 设计混合奖励聚合机制，融合工具验证和MCTS反馈；4. 将奖励信号格式化为理由增强的生成结构。

Result: 仅使用4万个自动标注样本（最佳PRM自动监督训练数据的10%），在ProcessBench上实现高达26%的相对性能提升。在奖励引导的贪婪搜索中，甚至优于使用人类标注监督训练的PRM。

Conclusion: GroundedPRM提供了一个可扩展且可验证的路径，实现高质量的过程级推理，解决了现有PRM方法的核心局限性，为多步推理的监督学习提供了有效解决方案。
...
TL;DR: 本文研究了大型语言模型在组合式机器设计中的能力，开发了BesiegeField测试平台来评估LLMs在物理模拟环境中构建功能性机器的表现，并探索了强化学习作为改进途径。
...
Method: 开发BesiegeField测试平台，基于Besiege游戏构建，支持部件组装、物理模拟和奖励驱动评估；对最先进的LLMs进行基准测试，并探索强化学习微调作为改进方法。

Result: 当前开源模型在空间推理、策略组装和指令遵循等关键能力上表现不足，但通过强化学习微调可以改善性能。

Conclusion: 机器设计任务位于语言、物理推理和机器设计的交叉点，存在开放挑战，强化学习是提升LLMs在该领域能力的可行路径。
...
<details>
  <summary>Details</summary>
Motivation: 现有的虚假内容检测方法通常只针对单一类型（人工制作或AI生成），但在真实场景中内容类型未知，限制了这些专门化系统的有效性。

Method: 构建了包含12.7万样本的OmniFake数据集，提出UMFDet框架，使用VLM骨干网络，结合类别感知的MoE适配器和归因链式思维机制来捕捉类别特定线索。
