...
Motivation: 理解pyroCb火灾风暴中燃料水分的作用及其与火灾的相互作用机制。

Method: 使用精细尺度云模型与火灾模型结合燃烧物理模拟pyroCb及其火灾的协同演化。

Result: 燃料水分是能量汇而非源，SAFIR机制在弱高层风条件下加剧火灾。

Conclusion: 燃料水分抑制pyroCb发展，SAFIR机制为理解pyroCb行为提供新框架。
...
<details>
  <summary>Details</summary>
Motivation: 气溶胶-云相互作用是地球系统中最不确定的气候强迫因素，研究其在气候变化中的表现对理解气候系统至关重要。

Method: 利用大涡模拟（LES）对东北太平洋的层积云-积云转变（SCT）进行3天模拟，通过扰动海洋边界层（MBL）中的气溶胶浓度，评估当前和双倍CO2气候下的气溶胶-云相互作用。

Result: 双倍CO2气候下，气溶胶对云的影响（如Twomey效应和LWP调整）被抑制，LWP调整对全球变暖更敏感。气溶胶引起的云分数变化对云辐射效应变化贡献最大。

Conclusion: 气溶胶增加的冷却效应在双倍CO2气候下减弱，研究结果有助于理解海洋云增亮（MCB）在气候变化中的冷却潜力。

Abstract: Because aerosol-cloud interactions are the most uncertain climate forcing in
the Earth system, it is important to better understand the aerosol-cloud
interactions, especially how they will change with climate. This study carries
out large eddy simulations (LES) of a 3-day stratocumulus-to-cumulus transition
(SCT) along an airmass-following trajectory in the Northeast Pacific Ocean. By
perturbing aerosol concentrations within the marine boundary layer (MBL) in the
SCT simulations, we evaluate aerosol-cloud interactions in both the present day
as well as in a double-CO2 climate. We find that aerosol-induced cloud changes,
including first (Twomey effect) and second adjustments of cloud fraction and
liquid water path (LWP) indirect effects of aerosols, tend to be inhibited in a
double-CO2 climate. The LWP adjustment is more sensitive to global warming than
the Twomey effect. By decomposing the aerosol-induced cloud radiative effect
change ($\Delta$CRE), we find that the aerosol-induced cloud fraction change
($\Delta$CF) shows the largest contribution to $\Delta$CRE in our simulations.
Aerosol-induced droplet number concentration change ($\Delta$Nc) shows a
cooling effect, while the LWP change shows a warming effect that is consistent
with aerosol-induced cloud thinning. Overall, the cooling effect associated
with increased aerosol concentrations will weaken in a double-CO2 climate. Our
results can also help to understand and predict the cooling potential of marine
cloud brightening (MCB) in a changing climate.
...
<details>
  <summary>Details</summary>
Motivation: 理解pyroCb火灾风暴的耦合火-大气相互作用及其行为机制，以改进预测。

Method: 使用精细尺度云模型与火灾模型结合燃烧物理模拟pyroCb及其火灾的共演化。

Result: 燃料水分是能量汇而非水分源，SAFIR机制在弱高层风条件下加剧火灾。

Conclusion: 燃料水分抑制pyroCb，SAFIR机制揭示了火灾快速增强的条件，为预测提供新视角。
...
Method: 利用大涡模拟（LES）模拟东北太平洋地区3天的层积云到积云转变（SCT），通过扰动海洋边界层（MBL）中的气溶胶浓度，评估当前和双倍CO2气候下的气溶胶-云相互作用。

Result: 双倍CO2气候下，气溶胶对云的影响（如Twomey效应和LWP调整）被抑制，LWP调整对全球变暖更敏感。气溶胶引起的云分数变化对云辐射效应变化贡献最大。

Conclusion: 全球变暖会减弱气溶胶增加的冷却效应，研究结果有助于理解海洋云增亮（MCB）在气候变化中的冷却潜力。
...
<details>
  <summary>Details</summary>
Motivation: 交叉操作在遗传算法中虽能生成新解，但其效果受限于种群多样性与交叉操作本身的矛盾。本文旨在解决这一矛盾。

Method: 提出DiPEC范式，设计DEGA算法，通过保留多样性来优化交叉操作的效果。

Result: (2+1)-DEGA在LeadingOnes问题上仅需O(n^{5/3}log^{2/3}n)次评估，优于传统算法的Θ(n^2)次。实验验证了其有效性。

Conclusion: DiPEC范式具有潜力，值得进一步系统研究。
...
<details>
  <summary>Details</summary>
Motivation: 确定算法运行次数对实验设计和结果可靠性至关重要，但传统方法可能效率低下或资源浪费。

Method: 结合概率理论和鲁棒性检查，动态调整运行次数，并在执行过程中在线更新。

Result: 在多个算法组合和基准测试中，准确率达82%-95%，运行次数减少约50%，且不影响优化结果。

Conclusion: 该方法显著提高了基准测试效率，同时减少了能源消耗，有助于可持续计算。
...
<details>
  <summary>Details</summary>
Motivation: 探索景观特征如何影响多目标组合优化算法的性能，为特定算法和景观提供优化建议。

Method: 使用C-PLOS-net模型提取特征，测试PLS、GSEMO和NSGA-II算法在rmnk-landscapes上的表现，采用分辨率和超体积指标。

Result: 揭示了特定景观下影响算法性能的特征组合，为不同算法和景观提供了定制化分析。

Conclusion: 研究深化了对景观特征重要性的理解，为特定算法和景观的性能优化提供了依据。
...
<details>
  <summary>Details</summary>
Motivation: 针对元启发式算法领域缺乏区分度的问题，研究旨在通过统计测试评估算法的搜索行为。

Method: 使用交叉匹配统计测试比较多元分布，分析了MEALPY库中的114种算法。

Result: 研究结果用于识别具有相似搜索行为的算法。

Conclusion: 统计测试有助于区分算法的创新性，为元启发式算法的评估提供了新方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统遗传算法中，交叉操作在利用多样性时反而会减少多样性，导致其效果降低。本文旨在解决这一矛盾。

Method: 提出了DiPEC范式，并基于此设计了Diversity Exploitation Genetic Algorithm (DEGA)。

Result: 理论证明和实验表明，(2+1)-DEGA在LeadingOnes问题上仅需O(n^{5/3}log^{2/3} n)次评估即可找到最优解，优于传统遗传算法的Θ(n^2)次评估。

Conclusion: DiPEC范式在多个基准测试中表现优异，值得进一步系统研究。
...
<details>
  <summary>Details</summary>
Motivation: 确定算法运行次数对实验时间和结果可靠性至关重要，但传统方法可能效率低下或资源浪费。

Method: 结合概率理论和鲁棒性检查，动态调整运行次数，适用于在线执行。

Result: 在多个算法组合和基准测试中，准确率达82%-95%，运行次数减少约50%。

Conclusion: 该方法提高了基准测试效率，减少能耗，有助于可持续计算。
...
<details>
  <summary>Details</summary>
Motivation: 研究景观特征如何预测多目标组合优化算法的性能，以提升算法选择和设计。

Method: 使用C-PLOS-net模型提取特征，测试PLS、GSEMO和NSGA-II在rmnk-landscape上的性能。

Result: 揭示了特定景观和算法下影响性能的特征组合。

Conclusion: 为特定rmnk-landscape和算法提供了特征重要性的深入见解。
...
Conclusion: 统计测试有助于区分算法的搜索行为，为元启发式算法的创新性评估提供新方法。
...
### [13] [Geometry-aware 4D Video Generation for Robot Manipulation](https://arxiv.org/abs/2507.01099)
...
### [47] [Representation Entanglement for Generation:Training Diffusion Transformers Is Much Easier Than You Think](https://arxiv.org/abs/2507.01467)
...
### [51] [Depth Anything at Any Condition](https://arxiv.org/abs/2507.01634)
...
### [52] [What Really Matters for Robust Multi-Sensor HD Map Construction?](https://arxiv.org/abs/2507.01484)
...
### [53] [Autoregressive Image Generation with Linear Complexity: A Spatial-Aware Decay Perspective](https://arxiv.org/abs/2507.01652)
...
### [54] [AVC-DPO: Aligned Video Captioning via Direct Preference Optimization](https://arxiv.org/abs/2507.01492)
...
### [55] [ECCV 2024 W-CODA: 1st Workshop on Multimodal Perception and Comprehension of Corner Cases in Autonomous Driving](https://arxiv.org/abs/2507.01735)
...
### [56] [ReFlex: Text-Guided Editing of Real Images in Rectified Flow via Mid-Step Feature Extraction and Attention Adaptation](https://arxiv.org/abs/2507.01496)
...
### [57] [Are Vision Transformer Representations Semantically Meaningful? A Case Study in Medical Imaging](https://arxiv.org/abs/2507.01788)
...
### [58] [Mamba Guided Boundary Prior Matters: A New Perspective for Generalized Polyp Segmentation](https://arxiv.org/abs/2507.01509)
...
### [59] [Exploring Pose-based Sign Language Translation: Ablation Studies and Attention Insights](https://arxiv.org/abs/2507.01532)
...
### [60] [TrackingMiM: Efficient Mamba-in-Mamba Serialization for Real-time UAV Object Tracking](https://arxiv.org/abs/2507.01535)
...
### [61] [A Multi-Centric Anthropomorphic 3D CT Phantom-Based Benchmark Dataset for Harmonization](https://arxiv.org/abs/2507.01539)
...
### [62] [Interpolation-Based Event Visual Data Filtering Algorithms](https://arxiv.org/abs/2507.01557)
...
### [63] [How Well Does GPT-4o Understand Vision? Evaluating Multimodal Foundation Models on Standard Computer Vision Tasks](https://arxiv.org/abs/2507.01955)
...
### [64] [A Gift from the Integration of Discriminative and Diffusion-based Generative Learning: Boundary Refinement Remote Sensing Semantic Segmentation](https://arxiv.org/abs/2507.01573)
...
### [65] [Locality-aware Parallel Decoding for Efficient Autoregressive Image Generation](https://arxiv.org/abs/2507.01957)
...
### [66] [SketchColour: Channel Concat Guided DiT-based Sketch-to-Colour Pipeline for 2D Animation](https://arxiv.org/abs/2507.01586)
...
### [67] [Towards Controllable Real Image Denoising with Camera Parameters](https://arxiv.org/abs/2507.01587)
...
### [68] [DepthSync: Diffusion Guidance-Based Depth Synchronization for Scale- and Geometry-Consistent Video Depth Estimation](https://arxiv.org/abs/2507.01603)
...
### [69] [Perception-Oriented Latent Coding for High-Performance Compressed Domain Semantic Inference](https://arxiv.org/abs/2507.01608)
...
### [70] [SAILViT: Towards Robust and Generalizable Visual Backbones for MLLMs via Gradual Feature Refinement](https://arxiv.org/abs/2507.01643)
...
### [71] [RobuSTereo: Robust Zero-Shot Stereo Matching under Adverse Weather](https://arxiv.org/abs/2507.01653)
...
### [72] [SPoT: Subpixel Placement of Tokens in Vision Transformers](https://arxiv.org/abs/2507.01654)
...
### [73] [What does really matter in image goal navigation?](https://arxiv.org/abs/2507.01667)
...
### [76] [Using Wavelet Domain Fingerprints to Improve Source Camera Identification](https://arxiv.org/abs/2507.01712)
...
### [77] [Soft Self-labeling and Potts Relaxations for Weakly-Supervised Segmentation](https://arxiv.org/abs/2507.01721)
...
### [78] [When Does Pruning Benefit Vision Representations?](https://arxiv.org/abs/2507.01722)
...
### [79] [HOI-Dyn: Learning Interaction Dynamics for Human-Object Motion Diffusion](https://arxiv.org/abs/2507.01737)
...
### [80] [DeRIS: Decoupling Perception and Cognition for Enhanced Referring Image Segmentation through Loopback Synergy](https://arxiv.org/abs/2507.01738)
...
### [81] [Calibrated Self-supervised Vision Transformers Improve Intracranial Arterial Calcification Segmentation from Clinical CT Head Scans](https://arxiv.org/abs/2507.01744)
...
### [82] [SSL4SAR: Self-Supervised Learning for Glacier Calving Front Extraction from SAR Imagery](https://arxiv.org/abs/2507.01747)
...
### [83] [Rethinking Discrete Tokens: Treating Them as Conditions for Continuous Autoregressive Image Synthesis](https://arxiv.org/abs/2507.01756)
...
### [88] [Modulate and Reconstruct: Learning Hyperspectral Imaging from Misaligned Smartphone Views](https://arxiv.org/abs/2507.01835)
...
### [89] [MobileIE: An Extremely Lightweight and Effective ConvNet for Real-Time Image Enhancement on Mobile Devices](https://arxiv.org/abs/2507.01838)
...
### [90] [Future Slot Prediction for Unsupervised Object Discovery in Surgical Video](https://arxiv.org/abs/2507.01882)
...
### [91] [Self-Reinforcing Prototype Evolution with Dual-Knowledge Cooperation for Semi-Supervised Lifelong Person Re-Identification](https://arxiv.org/abs/2507.01884)
...
### [92] [Reasoning to Edit: Hypothetical Instruction-Based Image Editing with Visual Reasoning](https://arxiv.org/abs/2507.01908)
...
### [93] [Modality Agnostic, patient-specific digital twins modeling temporally varying digestive motion](https://arxiv.org/abs/2507.01909)
...
### [94] [3D Reconstruction and Information Fusion between Dormant and Canopy Seasons in Commercial Orchards Using Deep Learning and Fast GICP](https://arxiv.org/abs/2507.01912)
...
### [95] [IC-Custom: Diverse Image Customization via In-Context Learning](https://arxiv.org/abs/2507.01926)
...
### [96] [evMLP: An Efficient Event-Driven MLP Architecture for Vision](https://arxiv.org/abs/2507.01927)
*Zhentan Zheng*
...
### [97] [CI-VID: A Coherent Interleaved Text-Video Dataset](https://arxiv.org/abs/2507.01938)
...
### [98] [LongAnimation: Long Animation Generation with Dynamic Global-Local Memory](https://arxiv.org/abs/2507.01945)
...
### [99] [Kwai Keye-VL Technical Report](https://arxiv.org/abs/2507.01949)
...
### [100] [FreeMorph: Tuning-Free Generalized Image Morphing with Diffusion Model](https://arxiv.org/abs/2507.01953)
...
### [101] [Geometry-aware 4D Video Generation for Robot Manipulation](https://arxiv.org/abs/2507.01099)
*Zeyi Liu,Shuang Li,Eric Cousineau,Siyuan Feng,Benjamin Burchfiel,Shuran Song*
...
Conclusion: 该方法在多视角一致性和机器人任务中表现出色，为动态场景预测提供了新思路。
...
### [102] [Landslide Detection and Mapping Using Deep Learning Across Multi-Source Satellite Data and Geographic Regions](https://arxiv.org/abs/2507.01123)
*Rahul A. Burange,Harsh K. Shinde,Omkar Mutyalwar*
...
### [103] [cp_measure: API-first feature extraction for image-based profiling workflows](https://arxiv.org/abs/2507.01163)
*Alán F. Muñoz,Tim Treis,Alexandr A. Kalinin,Shatavisha Dasgupta,Fabian Theis,Anne E. Carpenter,Shantanu Singh*
...
<details>
  <summary>Details</summary>
Motivation: 传统生物图像分析工具（如CellProfiler）在自动化、可重复性方面存在障碍，阻碍了机器学习在计算生物学中的应用。
...
### [104] [Rapid Salient Object Detection with Difference Convolutional Neural Networks](https://arxiv.org/abs/2507.01182)
*Zhuo Su,Li Liu,Matthias Müller,Jiehua Zhang,Diana Wofk,Ming-Ming Cheng,Matti Pietikäinen*

Main category: cs.CV

TL;DR: 论文提出了一种高效网络设计SDNet和STDNet，用于资源受限设备上的实时显著目标检测（SOD），通过结合传统SOD方法和现代CNN，显著提升了效率和准确性。
...
Method: 结合传统SOD的对比线索和现代CNN，提出Pixel Difference Convolutions (PDCs)和Difference Convolution Reparameterization (DCR)策略，以及SpatioTemporal Difference Convolution (STDC)用于视频SOD。

Result: SDNet和STDNet在Jetson Orin设备上分别以46 FPS和150 FPS运行，参数少于1M，速度和准确性均优于现有轻量级模型。
...
### [105] [Robust Brain Tumor Segmentation with Incomplete MRI Modalities Using Hölder Divergence and Mutual Information-Enhanced Knowledge Transfer](https://arxiv.org/abs/2507.01254)
*Runze Cheng,Xihang Qiu,Ming Li,Ye Zhang,Chun Li,Fei Yu*

Main category: cs.CV

TL;DR: 提出了一种基于单模态并行处理的框架，通过Holder散度和互信息处理缺失模态的脑肿瘤分割问题，显著提升准确性。
...
Conclusion: 该框架通过动态调整和损失函数优化，有效解决了模态缺失问题，提升了分割准确性。
...
### [106] [AIGVE-MACS: Unified Multi-Aspect Commenting and Scoring Model for AI-Generated Video Evaluation](https://arxiv.org/abs/2507.01255)
*Xiao Liu,Jiawei Zhang*

Main category: cs.CV

TL;DR: 论文提出AIGVE-MACS模型，通过多角度语言评论和数值评分评估AI生成视频，显著提升解释性和人类评估一致性。
...
Method: 引入AIGVE-MACS模型，结合视觉语言模型、加权损失和动态帧采样策略，利用大规模基准AIGVE-BENCH 2进行训练。

Result: AIGVE-MACS在评分相关性和评论质量上表现最佳，显著优于GPT-4o和VideoScore，并通过多智能体框架提升视频生成质量53.5%。

Conclusion: AIGVE-MACS为AI生成视频提供了全面、人类对齐的评估新范式，并公开了基准和模型。
...
### [107] [Advancements in Weed Mapping: A Systematic Review](https://arxiv.org/abs/2507.01269)
*Mohammad Jahanbakht,Alex Olsen,Ross Marchant,Emilie Fillols,Mostafa Rahimi Azghadi*

Main category: cs.CV

TL;DR: 本文综述了杂草绘图领域的最新进展，包括数据采集、处理技术和绘图工具，填补了该领域缺乏全面文献综述的空白。


<details>
  <summary>Details</summary>
Motivation: 杂草绘图对精准管理至关重要，但缺乏全面的文献综述和结构化分析，限制了该领域的发展。

Method: 通过系统分析数据采集（传感器和平台技术）、数据处理（标注和建模）和绘图技术（时空分析和决策支持工具），遵循PRISMA指南进行文献综述。

Result: 综述提供了杂草绘图领域的全面理解，为未来研究和可持续杂草管理系统的发展奠定了基础。

Conclusion: 本文填补了杂草绘图领域的文献空白，为高效、可扩展和可持续的杂草管理提供了指导。
...
### [108] [DiffusionLight-Turbo: Accelerated Light Probes for Free via Single-Pass Chrome Ball Inpainting](https://arxiv.org/abs/2507.01305)
*Worameth Chinchuthakun,Pakkapon Phongthawee,Amit Raj,Varun Jampani,Pramook Khungurn,Supasorn Suwajanakorn*

Main category: cs.CV

TL;DR: 提出了一种通过将任务重新定义为镀铬球修复问题，从单张低动态范围（LDR）图像估计光照的简单有效技术。
...
Method: 利用预训练的扩散模型Stable Diffusion XL，通过迭代修复生成稳定的低频光照先验，并结合Exposure LoRA生成HDR光探针。

Result: 实验结果表明，该方法在多样化场景中生成逼真的光照估计，并表现出优异的泛化能力。

Conclusion: DiffusionLight-Turbo在保持质量的同时显著提升了速度，适用于实际应用。
...
### [109] [Frequency Domain-Based Diffusion Model for Unpaired Image Dehazing](https://arxiv.org/abs/2507.01275)
*Chengxu Liu,Lu Qi,Jinshan Pan,Xueming Qian,Ming-Hsuan Yang*

Main category: cs.CV

TL;DR: 论文提出了一种基于频域的扩散模型（\ours），用于无配对图像去雾，通过振幅残差编码器和相位校正模块提升效果。


<details>
  <summary>Details</summary>
Motivation: 现有基于对比学习的方法忽略了雾相关特性在频域的表现，且引入了无关内容信息。

Method: 提出频域扩散模型，利用振幅残差编码器（ARE）补偿振幅差异，并通过相位校正模块（PCM）消除伪影。

Result: 实验表明，该方法在合成和真实数据集上优于现有技术。

Conclusion: 频域扩散模型有效解决了无配对去雾问题，并显著提升了性能。
...
### [110] [Tile and Slide : A New Framework for Scaling NeRF from Local to Global 3D Earth Observation](https://arxiv.org/abs/2507.01631)
*Camille Billouard,Dawa Derksen,Alexandre Constantin,Bruno Vallet*
...
<details>
  <summary>Details</summary>
Motivation: 传统NeRF方法因内存限制仅适用于小场景，而大规模场景需要分块处理，但现有方法存在重建误差问题。

Method: 提出Snake-NeRF框架，采用分块策略、图像裁剪和3D瓦片渐进策略，避免边缘重建误差。

Result: 实验表明，Snake-NeRF能在单GPU上线性处理大规模卫星图像，且不损失重建质量。
...
### [111] [Learning an Ensemble Token from Task-driven Priors in Facial Analysis](https://arxiv.org/abs/2507.01290)
*Sunyong Seo,Semin Kim,Jongha Lee*

Main category: cs.CV

TL;DR: 论文提出ET-Fuser方法，通过注意力机制结合预训练模型的任务先验，学习统一的特征表示，提升面部分析任务的性能。
...
Method: 利用预训练模型的注意力机制生成集成令牌，共享预训练编码器的互信息，实现高效的特征统一。

Result: 实验表明，该方法在多种面部分析任务中显著提升了特征表示效果。

Conclusion: ET-Fuser通过集成令牌和注意力机制，高效地实现了特征统一，为面部分析任务提供了新思路。
...
### [112] [Physics-informed Ground Reaction Dynamics from Human Motion Capture](https://arxiv.org/abs/2507.01340)
*Cuong Le,Huy-Phuong Le,Duc Le,Minh-Thien Duong,Van-Binh Nguyen,My-Ha Le*
...
Result: 在GroundLink数据集上，新方法在反作用力估计精度和根轨迹模拟精度上均优于基线模型。

Conclusion: 物理约束的引入显著提高了动态估计的准确性，扩展了方法的实际应用场景。
...
### [113] [Learning Camera-Agnostic White-Balance Preferences](https://arxiv.org/abs/2507.01342)
*Luxi Zhao,Mahmoud Afifi,Michael S. Brown*

Main category: cs.CV

TL;DR: 论文提出了一种轻量级方法，通过学习后照明估计映射，将中性白平衡校正转换为美学偏好校正，实现跨相机美学一致性。
...
Method: 通过学习一个后照明估计映射，在中性白平衡模块后应用，将中性校正转换为美学偏好校正。

Result: 模型仅含约500参数，运行时间为0.024毫秒，在771张智能手机图像数据集上达到最优性能。

Conclusion: 该方法实现了跨相机美学一致性，且计算和内存开销极小，兼容现有跨相机AWB技术。
...
### [114] [Learning from Random Subspace Exploration: Generalized Test-Time Augmentation with Self-supervised Distillation](https://arxiv.org/abs/2507.01347)
*Andrei Jelea,Ahmed Nabil Belbachir,Marius Leordeanu*
...
Method: 通过随机扰动PCA子空间投影形成集成，并利用自监督学习优化单模型。

Result: 在多种任务和数据集上验证了GTTA的通用性和有效性，特别是在低能见度水下视频中的三文鱼分割任务。

Conclusion: GTTA是一种通用且高效的测试时间增强方法，适用于广泛任务，且计算成本低。
...
### [115] [Long-Tailed Distribution-Aware Router For Mixture-of-Experts in Large Vision-Language Model](https://arxiv.org/abs/2507.01351)
*Chaoxiang Cai,Longrong Yang,Kaibing Chen,Fan Yang,Xi Li*

Main category: cs.CV

TL;DR: 论文提出了一种长尾分布感知路由器（LTDR），用于视觉语言混合专家模型（MoE），解决了模态特定路由和视觉尾部令牌专家激活的问题。


<details>
  <summary>Details</summary>
Motivation: 现有MoE框架在视觉语言模型中依赖负载平衡机制，忽视了视觉和语言之间的分布差异，导致性能不足。
...
### [116] [3D Gaussian Splatting Driven Multi-View Robust Physical Adversarial Camouflage Generation](https://arxiv.org/abs/2507.01367)
*Tianrui Lou,Xiaojun Jia,Siyuan Liang,Jiawei Liang,Ming Zhang,Yanjun Xiao,Xiaochun Cao*

Main category: cs.CV

TL;DR: 论文提出了一种基于3D高斯溅射（3DGS）的物理攻击框架PGA，通过快速精确的重建和照片级渲染能力，解决了现有伪装攻击方法在多视角和复杂环境中的不足。


<details>
  <summary>Details</summary>
Motivation: 物理对抗攻击揭示了深度神经网络的脆弱性，尤其在自动驾驶等安全关键场景中构成威胁。现有伪装攻击方法依赖耗时且不准确的模拟环境，且在多视角和复杂环境中效果不佳。

Method: PGA框架利用3DGS进行快速重建和渲染，通过防止高斯间的相互遮挡和自遮挡，以及采用最小-最大优化调整背景，提升对抗效果和跨视角鲁棒性。

Result: 大量实验验证了PGA的有效性和优越性。

Conclusion: PGA在多视角和复杂物理环境中表现出更强的对抗效果和鲁棒性，优于现有方法。
...
### [117] [Activation Reward Models for Few-Shot Model Alignment](https://arxiv.org/abs/2507.01368)
*Tianning Chai,Chancharik Mitra,Brandon Huang,Gautam Rajendrakumar Gare,Zhiqiu Lin,Assaf Arbelle,Leonid Karlinsky,Rogerio Feris,Trevor Darrell,Deva Ramanan,Roei Herzig*

Main category: cs.CV

TL;DR: 论文提出了一种新的少样本奖励建模方法——激活奖励模型（Activation RMs），通过激活导向构建对齐的奖励信号，无需额外微调，优于现有方法，并在新基准PreferenceHack上表现优异。


<details>
  <summary>Details</summary>
Motivation: 对齐大型语言模型（LLMs）和多模态模型（LMMs）的生成输出与人类偏好是提升模型实际应用质量的关键，但传统奖励建模方法难以适应新偏好。

Method: 提出激活奖励模型（Activation RMs），利用激活导向和少量监督构建奖励信号，无需额外模型微调。

Result: Activation RMs在标准奖励建模基准上优于现有少样本方法，并在新基准PreferenceHack上超越GPT-4o。

Conclusion: Activation RMs为奖励建模提供了一种高效、适应性强的解决方案，尤其在安全关键应用中表现突出。
...
### [118] [Active Measurement: Efficient Estimation at Scale](https://arxiv.org/abs/2507.01372)
*Max Hamilton,Jinlin Lai,Wenlong Zhao,Subhransu Maji,Daniel Sheldon*

Main category: cs.CV

TL;DR: 提出了一种名为“主动测量”的人机协作AI框架，通过重要性采样和蒙特卡洛估计提高科学测量的精度和效率。
...
Method: 结合AI预测和人工标注，使用重要性采样优化样本选择，并通过迭代改进模型和估计。

Result: 主动测量在多个任务中降低了估计误差，且对AI模型的准确性要求较低。

Conclusion: 该方法为科学测量提供了一种高效且可靠的解决方案，尤其适用于AI模型不完美的情况。
...
### [119] [MUG: Pseudo Labeling Augmented Audio-Visual Mamba Network for Audio-Visual Video Parsing](https://arxiv.org/abs/2507.01384)
*Langyu Wang,Bingke Zhu,Yingying Chen,Yiyuan Zhang,Ming Tang,Jinqiao Wang*
...
Method: 使用伪标签增强数据，并通过音频-视觉Mamba网络处理特征，以增强段级事件组合解析能力并排除模态噪声。
...
### [120] [FixTalk: Taming Identity Leakage for High-Quality Talking Head Generation in Extreme Cases](https://arxiv.org/abs/2507.01390)
*Shuai Tan,Bill Gong,Bin Ji,Ye Pan*
...
Conclusion: FixTalk通过创新设计解决了身份泄漏和渲染伪影问题，提升了说话头生成效果。
...
### [121] [Coherent Online Road Topology Estimation and Reasoning with Standard-Definition Maps](https://arxiv.org/abs/2507.01397)
*Khanh Son Pham,Christian Witte,Jens Behley,Johannes Betz,Cyrill Stachniss*

Main category: cs.CV

TL;DR: 提出了一种基于标准地图（SD）信息的方法，用于在线预测高精地图（HD）元素，包括车道段、拓扑和道路边界，并通过混合编码和去噪技术提升性能。


<details>
  <summary>Details</summary>
Motivation: 解决高精地图在线构建的复杂性和一致性问题，减少对现有高精地图的依赖。

Method: 利用标准地图（SD）作为先验信息，设计网络架构结合混合车道段编码和去噪技术，并引入时间一致性处理。

Result: 实验表明，该方法显著优于现有方法，验证了建模方案的有效性。

Conclusion: 提出的方法能够高效、一致地在线构建高精地图，为自动驾驶提供更可靠的解决方案。
...
### [122] [Medical-Knowledge Driven Multiple Instance Learning for Classifying Severe Abdominal Anomalies on Prenatal Ultrasound](https://arxiv.org/abs/2507.01401)
*Huanwen Liang,Jingxian Xu,Yuanji Zhang,Yuhao Huang,Yuhan Zhang,Xin Yang,Ran Li,Xuedong Deng,Yanjun Liu,Guowei Tao,Yun Wu,Sheng Zhao,Xinru Gao,Dong Ni*
...
<details>
  <summary>Details</summary>
Motivation: 胎儿腹部畸形是严重的先天性异常，需准确诊断以指导妊娠管理并降低死亡率。现有AI研究多关注图像级分类，而病例级诊断研究较少。

Method: 采用MIL框架，结合MoAE模块加权不同注意力头，MFS模块基于医学知识选择特征，PPL模块增强MFS。

Result: 在包含2,419个病例、24,748张图像的数据集上验证，性能优于现有方法。
...
### [123] [DocShaDiffusion: Diffusion Model in Latent Space for Document Image Shadow Removal](https://arxiv.org/abs/2507.01422)
*Wenjie Liu,Bingshu Wang,Ze Wang,C. L. Philip Chen*

Main category: cs.CV

TL;DR: 本文提出了一种名为DocShaDiffusion的潜在空间扩散模型，用于文档图像阴影去除，并设计了阴影软掩模生成模块（SSGM）和阴影掩模感知引导扩散模块（SMGDM），以解决彩色阴影问题。此外，还提出了阴影鲁棒感知特征损失和合成数据集SDCSRD，实验验证了方法的优越性。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常忽略彩色阴影，且仅适用于恒定颜色背景的阴影去除，本文旨在解决这一问题。

Method: 设计了潜在空间扩散模型DocShaDiffusion，结合SSGM生成阴影掩模和SMGDM引导扩散过程，并使用阴影鲁棒感知特征损失优化细节保留。

Result: 在三个公共数据集上的实验表明，该方法优于现有技术。

Conclusion: DocShaDiffusion在文档图像阴影去除任务中表现出色，尤其是对彩色阴影的处理，代码和数据集将公开。
...
### [124] [CaptionSmiths: Flexibly Controlling Language Pattern in Image Captioning](https://arxiv.org/abs/2507.01409)
*Kuniaki Saito,Donghyun Kim,Kwanyong Park,Atsushi Hashimoto,Yoshitaka Ushiku*

Main category: cs.CV

TL;DR: 提出了一种名为CaptionSmiths的新方法，通过量化标题的长度、描述性和单词独特性，实现单一模型对多样化语言模式的灵活控制。


<details>
  <summary>Details</summary>
Motivation: 现有的图像描述模型难以精细控制生成标题的属性，因为它们未在训练中将这些属性作为条件，且无法平滑过渡语言模式。

Method: 通过将标题的三个属性量化为连续标量值，并通过端点向量插值表示条件，实现语言模式的灵活切换。

Result: 实验表明，CaptionSmiths能平滑改变输出标题的属性，并在词汇对齐上优于基线模型，例如将标题长度控制误差降低了506%。

Conclusion: CaptionSmiths提供了一种无需人工标注的方法，实现了对生成标题属性的灵活控制，适用于多样化应用。
...
<details>
  <summary>Details</summary>
Motivation: 解决计算机视觉中未见物体实例分割的通用性问题，避免重新训练的需求。

Method: 利用Grounded-SAM 2生成物体提议和分割掩码，结合DINOv2的零样本能力生成嵌入，通过相似性评分匹配物体。

Result: 在BOP 2023挑战的七个核心数据集上，NOCTIS表现优于其他RGB和RGB-D方法。

Conclusion: NOCTIS提供了一种简单而强大的方法，适用于未见物体的实例分割任务。
...
### [126] [Gradient Short-Circuit: Efficient Out-of-Distribution Detection via Feature Intervention](https://arxiv.org/abs/2507.01417)
*Jiawei Gu,Ziyue Qiao,Zechao Li*
...
<details>
  <summary>Details</summary>
Motivation: 在开放世界中，OOD检测对深度学习模型的安全部署至关重要。研究发现，ID样本的梯度方向一致，而OOD样本的梯度方向混乱，这启发了新方法的提出。

Method: 提出了一种推理阶段的技术，通过短路异常梯度来降低OOD样本的置信度，并引入局部一阶近似避免二次前向传播。

Result: 在标准OOD基准测试中，该方法显著提升了检测性能，且计算轻量，适用于实际应用。
...
### [127] [Crop Pest Classification Using Deep Learning Techniques: A Review](https://arxiv.org/abs/2507.01494)
*Muhammad Hassam Ejaz,Muhammad Bilal,Usman Habib*

Main category: cs.CV

TL;DR: 综述分析了2018至2025年间37项研究，探讨了基于AI的害虫分类方法，包括CNN、ViT和混合模型，总结了技术挑战和未来方向。
...
Result: 研究发现早期研究多依赖CNN，近期转向混合和Transformer模型，但仍面临数据集不平衡、小害虫检测难等问题。

Conclusion: 综述提供了该领域的结构化概述，指出了关键挑战和未来发展方向。
...
### [128] [DiffMark: Diffusion-based Robust Watermark Against Deepfakes](https://arxiv.org/abs/2507.01428)
*Chen Sun,Haiyang Sun,Zhiqing Guo,Yunfeng Diao,Liejun Wang,Dan Ma,Gaobo Yang,Keqin Li*
...
<details>
  <summary>Details</summary>
Motivation: 现有水印方法对Deepfake操作的鲁棒性不足，扩散模型在图像生成中的优异表现提供了新思路。

Method: 修改扩散模型的训练和采样方案，引入时间步依赖的面部条件权重和跨信息融合模块（CIF），并集成冻结自编码器模拟Deepfake操作。

Result: 实验证明DiffMark对典型Deepfake操作具有显著鲁棒性。

Conclusion: DiffMark为Deepfake环境下的水印技术提供了有效解决方案。
...
### [129] [Integrating Traditional and Deep Learning Methods to Detect Tree Crowns in Satellite Images](https://arxiv.org/abs/2507.01502)
*Ozan Durgut,Beril Kallfelz-Sirmacek,Cem Unsalan*
...
Conclusion: 提出的规则方法增强了树冠检测的准确性和鲁棒性，为未来研究提供了改进方向。
...
### [130] [TurboReg: TurboClique for Robust and Efficient Point Cloud Registration](https://arxiv.org/abs/2507.01439)
*Shaocheng Yan,Pengcheng Shi,Zhenjun Zhao,Kaixin Wang,Kuang Cao,Ji Wu,Jiayuan Li*
...
### [131] [Following the Clues: Experiments on Person Re-ID using Cross-Modal Intelligence](https://arxiv.org/abs/2507.01504)
*Robert Aufschläger,Youssef Shoeb,Azarm Nowzad,Michael Heigl,Fabian Bally,Martin Schramm*

Main category: cs.CV

TL;DR: 论文提出cRID框架，结合视觉语言模型和图注意力网络，检测行人图像中的PII并提升Re-ID性能。


<details>
  <summary>Details</summary>
Motivation: 街景数据作为开放数据对自动驾驶和AI研究至关重要，但存在隐私风险，尤其是行人PII问题。

Method: 结合大型视觉语言模型、图注意力网络和表示学习，检测文本可描述的PII线索。

Result: 实验显示在跨数据集Re-ID任务中性能提升，如Market-1501到CUHK03-np。

Conclusion: cRID框架能有效检测语义PII并提升Re-ID实用性。
...
### [132] [OoDDINO:A Multi-level Framework for Anomaly Segmentation on Complex Road Scenes](https://arxiv.org/abs/2507.01455)
*Yuxing Liu,Ji Zhang,Zhou Xuchuan,Jingzhong Xiao,Huimin Yang,Jiaxin Zhong*

Main category: cs.CV

TL;DR: OoDDINO是一个多级异常分割框架，通过粗到细的检测策略解决现有像素级方法的局限性，结合不确定性引导的检测模型和像素级分割模型，显著提升异常分割性能。
...
Method: 提出两阶段级联架构：1）正交不确定性感知融合策略（OUAFS）整合多不确定性指标；2）自适应双阈值网络（ADT-Net）动态生成区域特定阈值。

Result: 在两个基准数据集上的实验验证了OoDDINO的优越性和兼容性，显著优于现有方法。

Conclusion: OoDDINO通过多级策略有效解决异常分割中的碎片化和阈值适应性问题，具有广泛兼容性和实际应用潜力。
...
### [133] [Autonomous AI Surveillance: Multimodal Deep Learning for Cognitive and Behavioral Monitoring](https://arxiv.org/abs/2507.01590)
*Ameer Hamza,Zuhaib Hussain But,Umar Arif,Samiya,M. Abdullah Asad,Muhammad Naeem*

Main category: cs.CV

TL;DR: 提出了一种多模态课堂监控系统，通过检测学生困倦、手机使用和人脸识别来评估注意力，结合YOLOv8和LResNet Occ FC等技术，实现了高精度的实时监测。
...
Method: 使用YOLOv8检测手机和睡眠行为，LResNet Occ FC进行人脸识别，结合MTCNN和YOLO进行身体追踪，系统基于PHP和ESP32-CAM硬件实现。

Result: 睡眠检测mAP@50达97.42%，人脸识别验证准确率86.45%，手机检测mAP@50为85.89%。

Conclusion: 该系统为教育环境提供了一种高效、可扩展的多模态监控解决方案。
...
### [134] [Survivability of Backdoor Attacks on Unconstrained Face Recognition Systems](https://arxiv.org/abs/2507.01607)
*Quentin Le Roux,Yannick Teglia,Teddy Furon,Philippe Loubet-Moundi,Eric Bourbao*

Main category: cs.CV

TL;DR: 本文首次系统研究了深度学习人脸识别系统中的后门攻击，展示了两种针对人脸检测任务的后门攻击，并验证了特征提取器的脆弱性，提出了防御建议。


<details>
  <summary>Details</summary>
Motivation: 深度学习人脸识别系统在现实场景中的安全性问题尚未被充分研究，尤其是后门攻击的潜在威胁。

Method: 通过探索两种后门攻击（人脸生成和关键点偏移），并结合20种系统配置和15种攻击案例进行验证。

Result: 研究表明，单一后门攻击可绕过整个系统功能，特征提取器也易受攻击。

Conclusion: 提出了针对后门攻击的最佳实践和防御措施，为相关方提供参考。
...
### [135] [Representation Entanglement for Generation:Training Diffusion Transformers Is Much Easier Than You Think](https://arxiv.org/abs/2507.01467)
*Ge Wu,Shen Zhang,Ruijing Shi,Shanghua Gao,Zhenyuan Chen,Lei Wang,Zhaowei Chen,Hongcheng Gao,Yao Tang,Jian Yang,Ming-Ming Cheng,Xiang Li*
...
<details>
  <summary>Details</summary>
Motivation: 现有方法（如REPA）在训练过程中利用外部视觉表示，但在推理阶段未充分利用判别性表示。REG旨在通过表示纠缠更全面地利用这些表示。

Method: REG将低层图像潜在表示与预训练基础模型的单一高层类别标记纠缠，直接从纯噪声中生成连贯的图像-类别对。

Result: 在ImageNet 256×256上，SiT-XL/2 + REG实现了63倍和23倍于SiT-XL/2和SiT-XL/2 + REPA的训练加速，且仅需400K次迭代即可超越4M次迭代的SiT-XL/2 + REPA。

Conclusion: REG通过表示纠缠显著提升了生成质量和训练效率，且推理开销极小。
...
### [136] [Prompt Guidance and Human Proximal Perception for HOT Prediction with Regional Joint Loss](https://arxiv.org/abs/2507.01630)
*Yuxiao Wang,Yu Lei,Zhenao Wei,Weiying Xue,Xinyu Jiang,Nan Zhuang,Qi Liu*

Main category: cs.CV

TL;DR: 论文提出了一种名为P3HOT的框架，结合提示引导和人类近端感知，用于改进人-物接触（HOT）检测任务，解决了现有模型在区域分割和类别一致性上的问题。


<details>
  <summary>Details</summary>
Motivation: 当前HOT检测模型局限于单一图像类型，导致过度分割和类别一致性不足，需要一种更有效的解决方案。

Method: P3HOT框架结合语义驱动的提示机制和人类近端感知机制，动态感知关键深度范围，并使用新的Regional Joint Loss和评价指标AD-Acc。

Result: 实验表明，P3HOT在两个基准数据集上四项指标均达到最优，尤其在HOT-Annotated数据集上显著提升。

Conclusion: P3HOT通过结合提示和深度感知，显著提升了HOT检测的性能和鲁棒性。
...
### [137] [Optimizing Methane Detection On Board Satellites: Speed, Accuracy, and Low-Power Solutions for Resource-Constrained Hardware](https://arxiv.org/abs/2507.01472)
*Jonáš Herec,Vít Růžička,Rado Pitoňák*

Main category: cs.CV

TL;DR: 论文提出了一种加速甲烷检测的方法，通过高效低功耗算法（如Mag1c-SAS和CEM）结合机器学习模型（U-Net、LinkNet），实现了在资源有限的硬件上快速检测甲烷泄漏。


<details>
  <summary>Details</summary>
Motivation: 甲烷是一种强效温室气体，早期通过高光谱卫星图像检测其泄漏有助于减缓气候变化。现有任务多为手动操作，效率低下，而传统甲烷增强方法计算成本高，不适合资源有限的机载硬件。

Method: 测试了快速目标检测方法（ACE、CEM），并提出Mag1c-SAS（Mag1c的快速变体），结合机器学习模型（U-Net、LinkNet）评估其性能。还提出了三种波段选择策略。

Result: Mag1c-SAS和CEM在检测强甲烷羽流时表现出色，计算效率分别比原始Mag1c快约100倍和230倍。其中一种波段选择策略在减少通道数的同时保持了准确性。

Conclusion: 该研究为未来在资源有限的硬件上实现机载甲烷检测奠定了基础，提高了数据及时性。代码、数据和模型已开源。
...
### [138] [Active Control Points-based 6DoF Pose Tracking for Industrial Metal Objects](https://arxiv.org/abs/2507.01478)
*Chentao Shen,Ding Pan,Mingyu Mei,Zaixing He,Xinyue Zhao*
...
Method: 使用图像控制点主动生成边缘特征进行优化，而非基于6DoF姿态的渲染，并引入最优控制点回归方法提升鲁棒性。

Result: 方法在数据集评估和真实任务中均表现有效，为工业金属物体的实时跟踪提供了可行方案。

Conclusion: 提出的方法成功解决了工业金属物体姿态跟踪的难题，代码已开源。
...
### [139] [Depth Anything at Any Condition](https://arxiv.org/abs/2507.01634)
*Boyuan Sun,Modi Jin,Bowen Yin,Qibin Hou*
...
Result: 实验证明DepthAnything-AC在多种基准测试（包括恶劣天气、合成失真和通用场景）中具有零样本能力。

Conclusion: DepthAnything-AC在复杂环境下表现出色，为单目深度估计提供了更通用的解决方案。
...
### [140] [What Really Matters for Robust Multi-Sensor HD Map Construction?](https://arxiv.org/abs/2507.01484)
*Xiaoshuai Hao,Yuting Zhao,Yuheng Ji,Luanyuan Dai,Peng Hao,Dingzhe Li,Shuai Cheng,Rong Yin*

Main category: cs.CV

TL;DR: 本文探讨了如何通过数据增强、新型多模态融合模块和模态丢弃训练策略，提升高清地图构建中多模态融合方法的鲁棒性，并在NuScenes数据集上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 现有高清地图构建方法多关注模型精度，而忽视了鲁棒性，这对实际应用至关重要。

Method: 提出三种关键组件：数据增强、新型多模态融合模块和模态丢弃训练策略。

Result: 实验表明，所提方法显著提升了基线方法的鲁棒性，并在NuScenes数据集上达到最先进性能。

Conclusion: 研究为开发更鲁棒可靠的高清地图构建模型提供了重要见解，推动了其在现实自动驾驶场景中的应用。
...
### [141] [Autoregressive Image Generation with Linear Complexity: A Spatial-Aware Decay Perspective](https://arxiv.org/abs/2507.01652)
*Yuxin Mao,Zhen Qin,Jinxing Zhou,Hui Deng,Xuyang Shen,Bin Fan,Jing Zhang,Yiran Zhong,Yuchao Dai*

Main category: cs.CV

TL;DR: 提出了一种名为LASAD的新型注意力机制，通过保留2D空间关系解决线性注意力在图像生成中的性能问题，并基于此构建了高效且高质量的图像生成模型LASADGen。


<details>
  <summary>Details</summary>
Motivation: 现有自回归模型依赖Transformer架构，存在计算复杂度和内存开销大的问题，而线性注意力机制在图像生成中因无法捕捉长程依赖导致质量下降。

Method: 提出Linear Attention with Spatial-Aware Decay (LASAD)，通过基于真实2D空间位置计算位置相关衰减因子，保留空间关系。

Result: 在ImageNet上，LASADGen实现了高效的线性复杂度计算，同时达到了最先进的图像生成性能。
...
### [142] [AVC-DPO: Aligned Video Captioning via Direct Preference Optimization](https://arxiv.org/abs/2507.01492)
*Jiyang Tang,Hengyi Li,Yifan Du,Wayne Xin Zhao*

Main category: cs.CV

TL;DR: AVC-DPO是一种后训练框架，通过偏好对齐提升视频多模态大语言模型（video MLLMs）的标题生成能力，专注于时空动态和空间信息，并在VDC基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有视频MLLMs在生成标题时难以根据人类偏好调整焦点，因此需要一种方法来实现偏好对齐。

Method: 设计增强提示，针对时空动态和空间信息，利用基础模型在不同提示条件下的响应进行偏好感知训练和对齐。

Result: 在LOVE@CVPR'25 Workshop Track 1A的VDC基准测试中取得第一名，表现优异。

Conclusion: AVC-DPO通过偏好对齐显著提升了视频MLLMs的标题生成能力，满足了人类偏好需求。
...
### [143] [ECCV 2024 W-CODA: 1st Workshop on Multimodal Perception and Comprehension of Corner Cases in Autonomous Driving](https://arxiv.org/abs/2507.01735)
*Kai Chen,Ruiyuan Gao,Lanqing Hong,Hang Xu,Xu Jia,Holger Caesar,Dengxin Dai,Bingbing Liu,Dzmitry Tsishkou,Songcen Xu,Chunjing Xu,Qiang Xu,Huchuan Lu,Dit-Yan Yeung*
...
Result: 收集研究论文并举办挑战赛，推动自动驾驶技术的进步。
...
### [144] [ReFlex: Text-Guided Editing of Real Images in Rectified Flow via Mid-Step Feature Extraction and Attention Adaptation](https://arxiv.org/abs/2507.01496)
*Jimyeong Kim,Jungwon Park,Yeji Song,Nojun Kwak,Wonjong Rhee*

Main category: cs.CV

TL;DR: 提出了一种基于ReFlow的实时图像编辑方法，通过分析多模态Transformer块的中间表示，利用中步潜在特征提升编辑效果。


<details>
  <summary>Details</summary>
Motivation: ReFlow在图像质量和文本对齐上优于扩散模型，但实时图像编辑仍具挑战性。

Method: 分析多模态Transformer块的中间表示，提取关键特征，利用中步潜在特征优化编辑。

Result: 在多个基准测试中优于现有方法，用户评价显示强烈偏好。

Conclusion: 该方法无需训练、无需用户提供掩码，且无需源提示即可应用，性能优越。
...
### [145] [Are Vision Transformer Representations Semantically Meaningful? A Case Study in Medical Imaging](https://arxiv.org/abs/2507.01788)
*Montasir Shams,Chashi Mahiul Islam,Shaeke Salman,Phat Tran,Xiuwen Liu*

Main category: cs.CV

TL;DR: Vision transformers (ViTs)在医学影像任务中表现出色，但其表示缺乏语义意义且对微小变化敏感，导致分类结果不可靠。


<details>
  <summary>Details</summary>
Motivation: 研究ViTs在医学影像分类中的表示是否具有语义意义，揭示其潜在脆弱性。

Method: 使用基于投影梯度的算法分析ViTs的表示。

Result: ViTs的表示对微小变化敏感，可能导致分类准确率下降60%以上。

Conclusion: ViTs的表示缺乏语义意义，对其在安全关键系统中的部署提出挑战。
...
### [146] [Mamba Guided Boundary Prior Matters: A New Perspective for Generalized Polyp Segmentation](https://arxiv.org/abs/2507.01509)
*Tapas K. Dutta,Snehashis Majhi,Deepak Ranjan Nayak,Debesh Jha*
...
Conclusion: SAM-MaGuP为息肉分割领域设定了新标准，解决了弱边界问题并提升了性能。
...
### [147] [Exploring Pose-based Sign Language Translation: Ablation Studies and Attention Insights](https://arxiv.org/abs/2507.01532)
*Tomas Zelezny,Jakub Straka,Vaclav Javorek,Ondrej Valach,Marek Hruz,Ivan Gruber*

Main category: cs.CV

TL;DR: 本文研究了基于姿态的数据预处理技术（归一化、插值和增强）对手语翻译（SLT）性能的影响，使用改进的T5编码器-解码器模型，结果表明这些技术能显著提升模型的鲁棒性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 探索姿态数据预处理技术如何提升手语翻译系统的性能，以解决传统孤立识别方法的局限性。

Method: 采用改进的T5编码器-解码器模型处理姿态表示，并在YouTubeASL和How2Sign数据集上进行消融实验，分析不同预处理策略的效果。

Result: 适当的归一化、插值和增强技术能显著提高翻译准确性和模型泛化能力，同时发现添加专用寄存器标记可进一步优化性能。

Conclusion: 姿态数据预处理技术对手语翻译性能有重要影响，未来可通过优化预处理策略进一步提升模型表现。
...
### [148] [TrackingMiM: Efficient Mamba-in-Mamba Serialization for Real-time UAV Object Tracking](https://arxiv.org/abs/2507.01535)
*Bingxi Liu,Calvin Chen,Junhao Li,Guyang Yu,Haoqian Song,Xuchen Liu,Jinqiang Cui,Hong Zhang*

Main category: cs.CV

TL;DR: 论文提出了一种基于Mamba模型的TrackingMiM架构，解决了Vision Transformer在无人机跟踪任务中的计算效率问题，实现了高性能和实时处理。


<details>
  <summary>Details</summary>
Motivation: Vision Transformer在无人机跟踪任务中因二次复杂度问题难以实时处理数据，而现有的Mamba方法未考虑时间连续性。

Method: 提出TrackingMiM架构，采用嵌套Mamba扫描机制，独立处理时空一致的图像块标记，并将模板帧编码为查询标记。
...
### [149] [A Multi-Centric Anthropomorphic 3D CT Phantom-Based Benchmark Dataset for Harmonization](https://arxiv.org/abs/2507.01539)
*Mohammadreza Amirian,Michael Bach,Oscar Jimenez-del-Toro,Christoph Aberle,Roger Schaer,Vincent Andrearczyk,Jean-Félix Maestrati,Maria Martin Asiain,Kyriakos Flouris,Markus Obmann,Clarisse Dromain,Benoît Dufour,Pierre-Alexandre Alois Poletti,Hendrik von Tengg-Kobligk,Rolf Hügli,Martin Kretzschmar,Hatem Alkadhi,Ender Konukoglu,Henning Müller,Bram Stieltjes,Adrien Depeursinge*

Main category: cs.CV

TL;DR: 本文介绍了一个开源基准数据集，用于促进AI在CT分析中的泛化能力研究，并提供了基线方法和结果。


<details>
  <summary>Details</summary>
Motivation: AI在医学CT分析中因数据分布变化（如扫描仪、重建技术或剂量差异）而泛化能力差，需要开发AI协调技术来解决这一问题。

Method: 使用幻影扫描构建数据集，包含1378个图像系列，来自4个制造商的13台扫描仪，并提供了评估图像和特征稳定性的方法。

Result: 提出了一个开源数据集和基线方法，用于评估AI协调技术的性能，特别是在肝脏组织分类任务中。

Conclusion: 该数据集和方法为AI协调技术的发展提供了重要资源，有助于解决医学CT分析中的数据分布偏移问题。
...
### [150] [Interpolation-Based Event Visual Data Filtering Algorithms](https://arxiv.org/abs/2507.01557)
*Marcin Kowlaczyk,Tomasz Kryjak*
...
### [151] [How Well Does GPT-4o Understand Vision? Evaluating Multimodal Foundation Models on Standard Computer Vision Tasks](https://arxiv.org/abs/2507.01955)
*Rahul Ramachandran,Ali Garjani,Roman Bachmann,Andrei Atanov,Oğuzhan Fatih Kar,Amir Zamir*
...
Method: 通过将标准视觉任务转化为文本提示和API兼容的任务，建立标准化评估框架。

Result: 模型在语义任务上表现较好，但在几何任务上较弱；GPT-4o在非推理模型中表现最佳。

Conclusion: 多模态基础模型在视觉任务中表现出潜力，但仍需改进，尤其是在几何任务和提示敏感性方面。
...
### [152] [A Gift from the Integration of Discriminative and Diffusion-based Generative Learning: Boundary Refinement Remote Sensing Semantic Segmentation](https://arxiv.org/abs/2507.01573)
*Hao Wang,Keyan Hu,Xin Guo,Haifeng Li,Chao Tao*

Main category: cs.CV

TL;DR: 论文提出了一种结合判别式学习和生成式学习的方法（IDGBR），用于改进遥感图像语义分割的边界精度。
...
### [153] [Locality-aware Parallel Decoding for Efficient Autoregressive Image Generation](https://arxiv.org/abs/2507.01957)
*Zhuoyang Zhang,Luke J. Huang,Chengyue Wu,Shang Yang,Kelly Peng,Yao Lu,Song Han*
...
Method: 引入灵活并行自回归建模和局部感知生成顺序，支持任意生成顺序和高并行度。
...
### [154] [SketchColour: Channel Concat Guided DiT-based Sketch-to-Colour Pipeline for 2D Animation](https://arxiv.org/abs/2507.01586)
*Bryan Constantine Sadihin,Michael Hua Wang,Shei Pern Chua,Hang Su*

Main category: cs.CV

TL;DR: SketchColour是一种基于扩散变换器（DiT）的2D动画草图着色方法，通过轻量级适配器和LoRA微调，显著减少参数和GPU内存使用，优于现有方法。
...
Result: 在SAKUGA数据集上表现优于现有视频着色方法，训练数据仅为竞争模型的一半，生成时间连贯的动画且伪影少。

Conclusion: SketchColour为2D动画着色提供了高效、高质量的解决方案，显著降低资源消耗。
...
### [155] [Towards Controllable Real Image Denoising with Camera Parameters](https://arxiv.org/abs/2507.01587)
*Youngjin Oh,Junhyeong Kwon,Keuntek Lee,Nam Ik Cho*
...
### [156] [DepthSync: Diffusion Guidance-Based Depth Synchronization for Scale- and Geometry-Consistent Video Depth Estimation](https://arxiv.org/abs/2507.01603)
*Yue-Jiang Dong,Wang Zhao,Jiale Xu,Ying Shan,Song-Hai Zhang*
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在处理长视频时存在尺度差异累积和几何不一致的问题，仅依赖2D扩散先验忽略了3D几何结构。

Method: 引入尺度引导和几何引导，协同优化去噪过程以实现一致的深度预测。

Result: 实验表明DepthSync在多个数据集上显著提升了深度估计的尺度和几何一致性。
...
### [157] [Perception-Oriented Latent Coding for High-Performance Compressed Domain Semantic Inference](https://arxiv.org/abs/2507.01608)
*Xu Zhang,Ming Lu,Yan Chen,Zhan Ma*
...
Result: POLC在保持生成图像编码性能的同时，显著提升视觉任务性能，且微调开销低。
...
### [158] [SAILViT: Towards Robust and Generalizable Visual Backbones for MLLMs via Gradual Feature Refinement](https://arxiv.org/abs/2507.01643)
*Weijie Yin,Dingkang Yang,Hongyuan Dong,Zijian Kang,Jiacong Wang,Xiao Liang,Chao Feng,Jiao Ran*

Main category: cs.CV

TL;DR: SAILViT是一种逐步特征学习增强的ViT，用于解决ViT与LLMs直接联合训练时的参数冲突和模态语义差距问题，提升MLLMs在多模态交互中的性能。
...
Conclusion: SAILViT有效解决了ViT与LLMs联合训练的挑战，为MLLMs在多模态任务中的性能提升提供了新方案。
...
### [159] [RobuSTereo: Robust Zero-Shot Stereo Matching under Adverse Weather](https://arxiv.org/abs/2507.01653)
*Yuran Wang,Yingping Liang,Yutao Hu,Ying Fu*

Main category: cs.CV

TL;DR: 论文提出RobuSTereo框架，通过扩散模拟和数据增强提升立体匹配模型在恶劣天气下的零样本泛化能力。
...
Method: 1. 提出扩散模拟管道生成恶劣天气下的高质量立体数据；2. 设计结合ConvNet和去噪Transformer的鲁棒特征编码器。
...
### [160] [SPoT: Subpixel Placement of Tokens in Vision Transformers](https://arxiv.org/abs/2507.01654)
*Martine Hjelkrem-Tan,Marius Aasan,Gabriel Y. Arteaga,Adín Ramírez Rivera*

Main category: cs.CV

TL;DR: SPoT是一种新的视觉Transformer标记化策略，通过连续放置标记来避免网格限制，显著提升性能并减少推理时所需的标记数量。


<details>
  <summary>Details</summary>
Motivation: 标准标记化方法将特征限制在离散的网格中，阻碍了模型在稀疏场景中的潜力，需要妥协。

Method: 提出Subpixel Placement of Tokens (SPoT)，利用连续标记位置和Oracle引导搜索优化标记位置。
...
### [161] [What does really matter in image goal navigation?](https://arxiv.org/abs/2507.01667)
*Gianluca Monaci,Philippe Weinzaepfel,Christian Wolf*

Main category: cs.CV

TL;DR: 研究探讨了通过端到端强化学习训练解决图像目标导航任务的可行性，分析了架构选择的影响，并揭示了模拟器设置对方法成功的影响。


<details>
  <summary>Details</summary>
Motivation: 验证是否可以通过端到端强化学习训练实现图像目标导航，避免依赖专用图像匹配或预训练模块。

Method: 通过大规模研究分析不同架构选择（如延迟融合、通道堆叠、空间到深度投影和交叉注意力）对导航任务中相对姿态估计器的影响。

Result: 发现模拟器设置对方法成功有影响，但能力可部分迁移到更现实场景，导航性能与相对姿态估计性能相关。
...
### [162] [Facial Emotion Learning with Text-Guided Multiview Fusion via Vision-Language Model for 3D/4D Facial Expression Recognition](https://arxiv.org/abs/2507.01673)
*Muzammil Behzad*
...
Result: 模型在多个基准测试中达到最先进水平，并在4D微表情识别中表现出色。
...
### [163] [Component Adaptive Clustering for Generalized Category Discovery](https://arxiv.org/abs/2507.01711)
*Mingfu Yan,Jiancheng Huang,Yifan Liu,Shifeng Chen*
...
<details>
  <summary>Details</summary>
Motivation: 传统方法依赖预定义类别数量的刚性假设，无法处理真实数据的复杂性和变异性。
...
### [164] [Using Wavelet Domain Fingerprints to Improve Source Camera Identification](https://arxiv.org/abs/2507.01712)
*Xinle Tian,Matthew Nunes,Emiko Dupont,Shaunagh Downing,Freddie Lichtenstein,Matt Burns*
...
<details>
  <summary>Details</summary>
Motivation: 传统的小波去噪方法在提取传感器模式噪声（SPN）时需要将指纹构建为图像并反演，过程复杂且效率低。

Method: 提出小波域指纹的概念，直接在频域进行指纹提取和比较，省去了反演步骤。

Result: 实验表明，该方法不仅检测精度更高，还能显著提升处理速度。

Conclusion: 小波域指纹方法简化了流程，提高了效率和准确性，适用于图像取证和源识别。
...
### [165] [Soft Self-labeling and Potts Relaxations for Weakly-Supervised Segmentation](https://arxiv.org/abs/2507.01721)
*Zhongwen Zhang,Yuri Boykov*
...
Method: 通过优化CRF/Potts损失的松弛形式，引入软伪标签，并提出连续子问题求解器。

Result: 软自标记方法在标准架构下显著提升性能，优于复杂系统和全监督方法。
...
### [166] [When Does Pruning Benefit Vision Representations?](https://arxiv.org/abs/2507.01722)
*Enrico Cassano,Riccardo Renzulli,Andrea Bragagnolo,Marco Grangetto*

Main category: cs.CV

TL;DR: 该论文研究了剪枝对视觉模型在可解释性、无监督对象发现和人类感知对齐三个维度的影响，发现稀疏模型存在性能最优的“甜点”，但这些效果高度依赖网络架构和参数规模。


<details>
  <summary>Details</summary>
Motivation: 剪枝常用于降低深度学习模型的复杂度，但其对可解释性和表示学习的影响尚不明确。本文旨在探索剪枝如何影响视觉模型在这三个关键维度的表现。

Method: 分析了不同视觉网络架构在不同稀疏度下对特征归因可解释性方法的影响，并探讨剪枝是否促进更简洁的结构化表示，以及是否增强模型与人类感知的对齐。

Result: 研究发现稀疏模型在可解释性、下游泛化和人类对齐方面存在“甜点”，但这些效果因网络架构和参数规模而异。
...
### [167] [HOI-Dyn: Learning Interaction Dynamics for Human-Object Motion Diffusion](https://arxiv.org/abs/2507.01737)
*Lin Wu,Zhixiang Chen,Jianglin Lan*

Main category: cs.CV

TL;DR: HOI-Dyn框架通过驱动-响应系统生成3D人-物交互，使用轻量级Transformer模型预测物体响应，并通过动态损失优化一致性。


<details>
  <summary>Details</summary>
Motivation: 现有方法将人和物体运动独立处理，导致物理上不合理且因果不一致的行为，因此需要一种更有效的建模方法。
...
### [168] [DeRIS: Decoupling Perception and Cognition for Enhanced Referring Image Segmentation through Loopback Synergy](https://arxiv.org/abs/2507.01738)
*Ming Dai,Wenxuan Cheng,Jiang-jiang Liu,Sen Yang,Wenxiao Cai,Yanpeng Sun,Wankou Yang*

Main category: cs.CV

TL;DR: DeRIS框架通过将Referring Image Segmentation（RIS）分解为感知和认知两个模块，系统性分析了现有RIS框架的瓶颈，并提出Loopback Synergy机制增强模块协同，同时引入数据增强解决长尾分布问题。


<details>
  <summary>Details</summary>
Motivation: 现有RIS研究多关注视觉-语言交互和细粒度定位，但对框架基础瓶颈的系统性分析不足。

Method: 提出DeRIS框架，分解RIS为感知和认知模块，引入Loopback Synergy机制增强协同，并采用非参照样本转换数据增强。

Result: 发现当前模型的主要瓶颈是多模态认知能力不足，DeRIS在非和多参照场景中表现优异。

Conclusion: DeRIS通过模块化分析和协同机制提升了RIS性能，具有广泛适用性。
...
### [169] [Calibrated Self-supervised Vision Transformers Improve Intracranial Arterial Calcification Segmentation from Clinical CT Head Scans](https://arxiv.org/abs/2507.01744)
*Benjamin Jin,Grant Mair,Joanna M. Wardlaw,Maria del C. Valdés Hernández*

Main category: cs.CV

TL;DR: 本文研究了基于Vision Transformers (ViTs)和MAE框架的3D医学图像分割方法，首次将其应用于颅内动脉钙化(IAC)分割，并展示了其优于传统方法的性能。


<details>
  <summary>Details</summary>
Motivation: 3D ViTs在医学图像分割中表现不佳，但因其高效的自我监督训练能力，适合处理无需昂贵标注的大规模医学影像数据。IAC作为神经血管疾病的生物标志物，其自动化量化有助于大规模风险评估。

Method: 采用MAE框架预训练ViTs，并在IST-3临床试验的异构数据上进行微调，用于IAC分割。研究了ViTs的关键设计选择，如低patch大小和插值上采样。

Result: 1) 自我监督ViT比监督nnU-Net基线高3.2 Dice分数；2) 低patch大小和插值上采样对ViTs效果更好；3) ViTs提高了对高切片厚度的鲁棒性，临床风险分类提升46%。

Conclusion: MAE预训练的ViTs在IAC分割中表现优异，具有临床应用的潜力，尤其是在处理异构数据和高切片厚度时。
...
### [170] [SSL4SAR: Self-Supervised Learning for Glacier Calving Front Extraction from SAR Imagery](https://arxiv.org/abs/2507.01747)
*Nora Gourmelon,Marcel Dreier,Martin Mayr,Thorsten Seehaus,Dakota Pyles,Matthias Braun,Andreas Maier,Vincent Christlein*
...
<details>
  <summary>Details</summary>
Motivation: 冰川冰量损失加剧，需高精度全年监测，但现有基于ImageNet预训练的模型因领域差异表现不佳。
...
Conclusion: 该技术能精确监测冰川崩解前沿的季节性变化。
...
### [171] [Rethinking Discrete Tokens: Treating Them as Conditions for Continuous Autoregressive Image Synthesis](https://arxiv.org/abs/2507.01756)
*Peng Zheng,Junke Wang,Yi Chang,Yizhou Yu,Rui Ma,Zuxuan Wu*
...
Result: 在ImageNet 256×256生成任务上，DisCon取得了1.38的gFID分数，优于现有自回归方法。
...
### [172] [Boosting Adversarial Transferability Against Defenses via Multi-Scale Transformation](https://arxiv.org/abs/2507.01791)
*Zihong Guo,Chen Wan,Yayin Zheng,Hailing Kuang,Xiaohai Lu*
...
Conclusion: SGP是一种可扩展性强且易于集成的方法，显著增强了对抗攻击的可迁移性。
...
### [173] [FreeLoRA: Enabling Training-Free LoRA Fusion for Autoregressive Multi-Subject Personalization](https://arxiv.org/abs/2507.01792)
*Peng Zheng,Ye Wang,Rui Ma,Zuxuan Wu*
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在多主体个性化生成时需复杂调整或联合优化，FreeLoRA旨在简化这一过程。
...
### [174] [HCNQA: Enhancing 3D VQA with Hierarchical Concentration Narrowing Supervision](https://arxiv.org/abs/2507.01800)
*Shengli Zhou,Jianuo Zhu,Qilin Huang,Fangjing Wang,Yanfu Zhang,Feng Zheng*

Main category: cs.CV

TL;DR: 论文提出了一种名为HCNQA的3D VQA模型，通过分层监督方法解决现有模型推理路径缺乏监督的问题，提升了性能。


<details>
  <summary>Details</summary>
Motivation: 现有的3D VQA模型仅监督最终输出，可能导致模型通过浅层捷径学习，缺乏对推理路径的监督。此外，慢思考方法存在思考不足的问题。

Method: 提出HCNQA模型，采用分层浓度缩小监督方法，模拟人类从广泛区域逐步聚焦到特定对象的推理过程，通过分层监督指导模型完成三个阶段。
...
### [175] [AMD: Adaptive Momentum and Decoupled Contrastive Learning Framework for Robust Long-Tail Trajectory Prediction](https://arxiv.org/abs/2507.01801)
*Bin Rao,Haicheng Liao,Yanchen Guan,Chengyue Wang,Bonan Wang,Jiaxun Zhang,Zhenning Li*
...
Result: 在nuScenes和ETH/UCY数据集上，AMD在长尾轨迹预测和整体预测准确性上均表现优异。

Conclusion: AMD框架有效提升了模型对复杂和罕见轨迹的识别能力，适用于自动驾驶中的长尾数据分布问题。
...
### [176] [Modulate and Reconstruct: Learning Hyperspectral Imaging from Misaligned Smartphone Views](https://arxiv.org/abs/2507.01835)
*Daniil Reutsky,Daniil Vladimirov,Yasin Mamedov,Georgy Perevozchikov,Nancy Mehta,Egor Ershov,Radu Timofte*

Main category: cs.CV

TL;DR: 提出了一种基于多图像的超光谱重建（MI-HSR）框架，利用三摄像头智能手机系统，显著提升了重建精度。


<details>
  <summary>Details</summary>
Motivation: 解决单RGB图像在超光谱重建中因光谱信息丢失导致的精度不足问题。

Method: 采用三摄像头智能手机系统，其中两个镜头配备特定光谱滤波器，结合理论和实证分析，提出MI-HSR框架。

Result: 在新建数据集Doomer上，模型比现有方法提升了30%的光谱估计精度。
...
### [177] [MobileIE: An Extremely Lightweight and Effective ConvNet for Real-Time Image Enhancement on Mobile Devices](https://arxiv.org/abs/2507.01838)
*Hailong Yan,Ao Li,Xiangtao Zhang,Zhe Liu,Zenglin Shi,Ce Zhu,Le Zhang*

Main category: cs.CV

TL;DR: 提出了一种极轻量级的CNN框架，仅约4K参数，实现了实时图像增强（IE）推理，最高达1,100 FPS，并在速度和性能之间取得最佳平衡。
...
Result: 首次实现高达1,100 FPS的实时IE推理，同时保持竞争力的图像质量。

Conclusion: 该框架在多个IE任务中实现了速度和性能的最佳平衡，代码已开源。
...
### [178] [Future Slot Prediction for Unsupervised Object Discovery in Surgical Video](https://arxiv.org/abs/2507.01882)
*Guiqiu Liao,Matjaz Jogan,Marcel Hussing,Edward Zhang,Eric Eaton,Daniel A. Hashimoto*
...
### [179] [Self-Reinforcing Prototype Evolution with Dual-Knowledge Cooperation for Semi-Supervised Lifelong Person Re-Identification](https://arxiv.org/abs/2507.01884)
*Kunlun Xu,Fan Zhuo,Jiangmeng Li,Xu Zou,Jiahuan Zhou*

Main category: cs.CV

TL;DR: 论文提出了一种名为SPRED的框架，用于解决半监督终身行人重识别（Semi-LReID）问题，通过动态原型引导的伪标签生成和新旧知识协同净化，提升未标记数据的利用率。


<details>
  <summary>Details</summary>
Motivation: 现实场景中标注资源有限，现有LReID方法在未标记数据利用时性能下降严重，亟需解决半监督LReID问题。

Method: SPRED框架结合动态原型生成高质量伪标签，并通过双知识协作净化噪声伪标签，形成自增强循环。

Result: 在Semi-LReID基准测试中，SPRED取得了最先进的性能。

Conclusion: SPRED通过自增强循环和双知识协作，显著提升了半监督终身行人重识别的性能。
...
### [180] [Reasoning to Edit: Hypothetical Instruction-Based Image Editing with Visual Reasoning](https://arxiv.org/abs/2507.01908)
*Qingdong He,Xueqin Chen,Chaoyi Wang,Yanjie Pan,Xiaobin Hu,Zhenye Gan,Yabiao Wang,Chengjie Wang,Xiangtai Li,Jiangning Zhang*
...
Method: 提出Reason50K数据集（50K样本）和ReasonBrain框架，结合MLLM和扩散模型，引入FRCE模块和CME增强器。

Result: ReasonBrain在推理场景中优于现有基线，并在传统IIE任务中表现出强零样本泛化能力。
...
### [181] [Modality Agnostic, patient-specific digital twins modeling temporally varying digestive motion](https://arxiv.org/abs/2507.01909)
*Jorge Tapias Gomez,Nishant Nadkarni,Lando S. Bosma,Jue Jiang,Ergys D. Subashi,William P. Segars,James M. Balter,Mert R Sabuncu,Neelam Tyagi,Harini Veeraraghavan*
...
<details>
  <summary>Details</summary>
Motivation: 临床实施DIR需要基于体素的空间精度指标，如手动标记的解剖标志，但对于高度移动的GI器官来说，这些方法难以实现。因此，需要一种更有效的方法来评估DIR的准确性。

Method: 通过半自动化管道，利用已发表的GI运动模型，从静态3D患者扫描生成21个运动阶段的4D序列，模拟消化GI运动。使用这些DT评估六种DIR方法的性能，包括目标配准误差、Dice相似系数和95% Hausdorff距离。

Result: 生成的DT能够模拟真实的GI运动，运动幅度和Jacobian行列式与真实患者数据相似。该方法可提取详细的DIR性能指标，并验证剂量映射的准确性。
...
### [182] [3D Reconstruction and Information Fusion between Dormant and Canopy Seasons in Commercial Orchards Using Deep Learning and Fast GICP](https://arxiv.org/abs/2507.01912)
*Ranjan Sapkota,Zhichao Meng,Martin Churuvija,Xiaoqiang Du,Zenghong Ma,Manoj Karkee*
...
<details>
  <summary>Details</summary>
Motivation: 果园自动化中，茂密的树冠严重遮挡了树干和树枝，限制了机器视觉系统的能力。休眠季节树冠结构更开放，可见性更高。

Method: 使用YOLOv9-Seg进行实例分割，Kinect Fusion进行3D重建，Fast GICP进行模型对齐，融合休眠期和生长期的RGB-D图像。

Result: YOLOv9-Seg在休眠季节数据集上的MSE为0.0047，mAP@50为0.78；Kinect Fusion重建的树干直径RMSE为5.23 mm；Fast GICP的最小适应分数为0.00197。

Conclusion: 该框架通过多季节数据融合，提高了机器人系统对树冠结构的建模精度，优化了修剪和疏果等自动化操作。
...
### [183] [IC-Custom: Diverse Image Customization via In-Context Learning](https://arxiv.org/abs/2507.01926)
*Yaowei Li,Xiaoyu Li,Zhaoyang Zhang,Yuxuan Bian,Gan Liu,Xinyuan Li,Jiale Xu,Wenbo Hu,Yating Liu,Lingen Li,Jing Cai,Yuexian Zou,Yancheng He,Ying Shan*
...
Result: 在身份一致性、和谐性和文本对齐等指标上，IC-Custom比现有方法表现更好，训练参数仅占原模型的0.4%。

Conclusion: IC-Custom为工业应用提供了高效的图像定制解决方案，具有广泛的应用潜力。
...
### [184] [evMLP: An Efficient Event-Driven MLP Architecture for Vision](https://arxiv.org/abs/2507.01927)
*Zhentan Zheng*

Main category: cs.CV

TL;DR: 提出了evMLP模型，通过事件驱动的局部更新机制，选择性处理图像或特征图中的变化区域，减少冗余计算，在图像分类和视频处理中表现优异。
...
Method: 提出evMLP模型，结合事件驱动的局部更新机制，仅处理连续帧间发生变化的区域（定义为“事件”）。

Result: 在ImageNet图像分类中达到与先进模型相当的精度；在视频数据集中显著降低计算成本，同时保持输出一致性。
...
### [185] [CI-VID: A Coherent Interleaved Text-Video Dataset](https://arxiv.org/abs/2507.01938)
*Yiming Ju,Jijin Hu,Zhengxiong Luo,Haoge Deng,hanyu Zhao,Li Du,Chengwei Wu,Donglin Hao,Xinlong Wang,Tengfei Pan*

Main category: cs.CV

TL;DR: CI-VID数据集解决了现有文本到视频（T2V）生成数据集中孤立文本-视频对的问题，支持连贯多场景视频序列的生成。
...
Method: 引入CI-VID数据集，包含34万样本，每个样本包含连贯的视频剪辑序列和文本描述，支持文本和视频到视频（TV2V）生成。

Result: 实验表明，基于CI-VID训练的模型在生成视频序列时显著提高了准确性和内容一致性。

Conclusion: CI-VID数据集为生成具有平滑视觉过渡和时间连贯性的故事驱动内容提供了高质量支持。
...
### [186] [LongAnimation: Long Animation Generation with Dynamic Global-Local Memory](https://arxiv.org/abs/2507.01945)
*Nan Chen,Mengqi Huang,Yihao Meng,Zhendong Mao*

Main category: cs.CV

TL;DR: 提出了一种名为LongAnimation的新框架，用于解决长动画着色中的长期颜色一致性问题，通过动态全局-局部范式实现。
...
Result: 在短期（14帧）和长期（平均500帧）动画上实验验证了LongAnimation在保持颜色一致性上的有效性。
...
### [187] [Kwai Keye-VL Technical Report](https://arxiv.org/abs/2507.01949)
*Kwai Keye Team,Biao Yang,Bin Wen,Changyi Liu,Chenglong Chu,Chengru Song,Chongling Rao,Chuan Yi,Da Li,Dunju Zang,Fan Yang,Guorui Zhou,Hao Peng,Haojie Ding,Jiaming Huang,Jiangxia Cao,Jiankang Chen,Jingyun Hua,Jin Ouyang,Kaibing Chen,Kaiyu Jiang,Kaiyu Tang,Kun Gai,Shengnan Zhang,Siyang Mao,Sui Huang,Tianke Zhang,Tingting Gao,Wei Chen,Wei Yuan,Xiangyu Wu,Xiao Hu,Xingyu Lu,Yang Zhou,Yi-Fan Zhang,Yiping Yang,Yulong Chen,Zhenhua Wu,Zhenyu Li,Zhixin Ling,Ziming Li,Dehua Ma,Di Xu,Haixuan Gao,Hang Li,Jiawei Guo,Jing Wang,Lejian Ren,Muhao Wei,Qianqian Wang,Qigen Hu,Shiyao Wang,Tao Yu,Xinchen Luo,Yan Li,Yiming Liang,Yuhang Hu,Zeyi Lu,Zhuoran Yang,Zixing Zhang*

Main category: cs.CV

TL;DR: Kwai Keye-VL是一个80亿参数的多模态基础模型，专注于短视频理解，同时保持通用视觉语言能力。通过大规模数据集和创新训练方法，实现了领先性能。


<details>
  <summary>Details</summary>
Motivation: 解决多模态大语言模型在动态、信息密集的短视频理解上的不足，满足当今数字媒体的需求。

Method: 采用四阶段预训练和两阶段后训练，包括创新的五模式“冷启动”数据混合和强化学习。

Result: 在公共视频基准测试中达到最先进水平，并在通用图像任务中保持竞争力。

Conclusion: Kwai Keye-VL在短视频理解方面表现卓越，并通过新基准KC-MMBench验证了其优势。
...
### [188] [FreeMorph: Tuning-Free Generalized Image Morphing with Diffusion Model](https://arxiv.org/abs/2507.01953)
*Yukang Cao,Chenyang Si,Jinghao Wang,Ziwei Liu*
...
Method: FreeMorph通过1）基于输入的显式引导修改自注意力模块，2）混合输入图像的自注意力模块实现可控过渡。

Result: FreeMorph在图像变形任务中表现优异，速度提升10~50倍，达到新的最佳水平。

Conclusion: FreeMorph为无需调优的图像变形提供了高效且高质量的解决方案。
...
### [189] [Geometry-aware 4D Video Generation for Robot Manipulation](https://arxiv.org/abs/2507.01099)
*Zeyi Liu,Shuang Li,Eric Cousineau,Siyuan Feng,Benjamin Burchfiel,Shuran Song*

Main category: cs.CV

TL;DR: 提出了一种4D视频生成模型，通过跨视角点图对齐监督，确保多视角3D一致性，无需相机姿态输入即可预测未来视频序列。
...
Method: 利用跨视角点图对齐监督训练模型，学习共享3D场景表示，基于RGB-D观测预测未来视频序列。

Result: 在模拟和真实机器人数据集上生成更稳定、空间对齐的预测视频，并能恢复机器人末端执行器轨迹。

Conclusion: 该方法支持机器人操作和适应新视角，具有实际应用潜力。
...
### [190] [Landslide Detection and Mapping Using Deep Learning Across Multi-Source Satellite Data and Geographic Regions](https://arxiv.org/abs/2507.01123)
*Rahul A. Burange,Harsh K. Shinde,Omkar Mutyalwar*
...
Conclusion: 深度学习与多源遥感数据结合在构建稳健、可扩展和可转移的滑坡预测模型中具有巨大潜力。
...
### [191] [cp_measure: API-first feature extraction for image-based profiling workflows](https://arxiv.org/abs/2507.01163)
*Alán F. Muñoz,Tim Treis,Alexandr A. Kalinin,Shatavisha Dasgupta,Fabian Theis,Anne E. Carpenter,Shantanu Singh*
...
<details>
  <summary>Details</summary>
Motivation: 解决当前工具在自动化、可重复分析和机器学习工作流中的障碍。
...
### [192] [Rapid Salient Object Detection with Difference Convolutional Neural Networks](https://arxiv.org/abs/2507.01182)
*Zhuo Su,Li Liu,Matthias Müller,Jiehua Zhang,Diana Wofk,Ming-Ming Cheng,Matti Pietikäinen*
...
<details>
  <summary>Details</summary>
Motivation: 解决现有SOD模型计算成本高的问题，使其适用于资源受限设备并实现实时性能。

Method: 结合传统SOD方法与现代CNN，使用PDC编码特征对比，并通过DCR策略嵌入标准卷积以提升效率。针对视频SOD，引入时空差异卷积（STDC）。

Result: 模型SDNet和STDNet在效率和准确性上取得显著平衡，在Jetson Orin设备上分别以46 FPS和150 FPS运行，速度和准确性均优于其他轻量级模型。
...
### [193] [Robust Brain Tumor Segmentation with Incomplete MRI Modalities Using Hölder Divergence and Mutual Information-Enhanced Knowledge Transfer](https://arxiv.org/abs/2507.01254)
*Runze Cheng,Xihang Qiu,Ming Li,Ye Zhang,Chun Li,Fei Yu*
...
Method: 利用Holder散度和互信息，设计了一种动态调整网络参数的并行处理框架。

Result: 在BraTS 2018和BraTS 2020数据集上表现出优于现有方法的性能。

Conclusion: 该框架在模态缺失情况下仍能保持高分割准确性，具有实际应用潜力。
...
### [194] [AIGVE-MACS: Unified Multi-Aspect Commenting and Scoring Model for AI-Generated Video Evaluation](https://arxiv.org/abs/2507.01255)
*Xiao Liu,Jiawei Zhang*
...
Conclusion: AIGVE-MACS为AI生成视频评估提供了全面、人类对齐的新范式，相关资源已开源。
...
### [195] [Advancements in Weed Mapping: A Systematic Review](https://arxiv.org/abs/2507.01269)
*Mohammad Jahanbakht,Alex Olsen,Ross Marchant,Emilie Fillols,Mostafa Rahimi Azghadi*

Main category: cs.CV

TL;DR: 本文综述了杂草测绘的最新方法，包括数据获取、处理技术和测绘工具，填补了该领域文献的空白。
...
Method: 通过PRISMA指南系统分析文献，涵盖数据获取（传感器和平台技术）、数据处理（标注和建模）及测绘技术（时空分析和决策支持工具）。
...
### [196] [DiffusionLight-Turbo: Accelerated Light Probes for Free via Single-Pass Chrome Ball Inpainting](https://arxiv.org/abs/2507.01305)
*Worameth Chinchuthakun,Pakkapon Phongthawee,Amit Raj,Varun Jampani,Pramook Khungurn,Supasorn Suwajanakorn*

Main category: cs.CV

TL;DR: 论文提出了一种通过将任务重新定义为铬球修复问题，从单张低动态范围（LDR）图像估计光照的简单有效技术。利用预训练的扩散模型Stable Diffusion XL，克服了现有方法依赖有限HDR全景数据集的泛化问题。通过迭代修复生成稳定的低频光照先验，并结合曝光LoRA生成HDR光探针。进一步提出DiffusionLight-Turbo，将运行时间从30分钟缩短至30秒。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖有限的HDR全景数据集，泛化能力不足。论文旨在通过扩散模型解决这一问题，并提升光照估计的效率和准确性。

Method: 提出DiffusionLight，利用迭代修复生成稳定的低频光照先验；结合曝光LoRA生成HDR光探针；进一步提出DiffusionLight-Turbo，通过训练Turbo LoRA直接预测迭代过程的平均铬球，实现60倍加速。

Result: 实验结果表明，该方法在多样化场景中生成逼真的光照估计，并在实际场景中表现出优越的泛化能力。

Conclusion: 论文提出的技术简单有效，显著提升了光照估计的效率和泛化能力，DiffusionLight-Turbo进一步实现了快速推理。
...
### [197] [Frequency Domain-Based Diffusion Model for Unpaired Image Dehazing](https://arxiv.org/abs/2507.01275)
*Chengxu Liu,Lu Qi,Jinshan Pan,Xueming Qian,Ming-Hsuan Yang*

Main category: cs.CV

TL;DR: 本文提出了一种基于频域的扩散模型（\ours），用于无配对图像去雾，通过振幅残差编码器和相位校正模块，显著提升了去雾效果。
...
Method: 提出频域扩散模型，利用振幅残差编码器（ARE）补偿振幅差距，并通过相位校正模块（PCM）优化相位谱。
...
### [198] [Tile and Slide : A New Framework for Scaling NeRF from Local to Global 3D Earth Observation](https://arxiv.org/abs/2507.01631)
*Camille Billouard,Dawa Derksen,Alexandre Constantin,Bruno Vallet*

Main category: cs.CV

TL;DR: Snake-NeRF框架通过分块处理大场景，解决了传统NeRF方法内存占用高的问题，实现了单设备高效处理。


<details>
  <summary>Details</summary>
Motivation: 传统NeRF方法因内存限制无法处理大场景，需要分块处理。

Method: 提出Snake-NeRF框架，采用分块策略和重叠裁剪图像，引入2×2 3D瓦片渐进策略和分段采样器。

Result: 实验表明，该方法能线性处理大卫星图像，单GPU运行且质量无损失。

Conclusion: Snake-NeRF有效扩展了NeRF的应用范围，适用于大场景3D重建。
...
### [199] [Learning an Ensemble Token from Task-driven Priors in Facial Analysis](https://arxiv.org/abs/2507.01290)
*Sunyong Seo,Semin Kim,Jongha Lee*
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在单任务学习中缺乏统一的特征表示，ET-Fuser旨在通过集成令牌和预训练模型的先验知识解决这一问题。

Method: 提出了一种基于自注意力机制的集成令牌学习方法，利用预训练编码器的先验知识生成共享互信息的令牌。

Result: 实验表明，该方法在多种面部分析任务中显著提升了特征表示，且计算成本极低。

Conclusion: ET-Fuser通过集成令牌和注意力机制，有效统一了特征表示，为面部分析任务提供了高效且性能优越的解决方案。
...
### [200] [Physics-informed Ground Reaction Dynamics from Human Motion Capture](https://arxiv.org/abs/2507.01340)
*Cuong Le,Huy-Phuong Le,Duc Le,Minh-Thien Duong,Van-Binh Nguyen,My-Ha Le*

Main category: cs.CV

TL;DR: 提出了一种基于物理约束的方法，直接从动作捕捉数据估计地面反作用力，结合欧拉积分和PD算法，提高了估计精度。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖实验室专用设备（如力板）收集数据，限制了人体动力学学习的应用范围。

Method: 利用物理定律和计算模拟作为约束，结合欧拉积分和PD算法，从动作捕捉数据中估计地面反作用力。
...
### [201] [Learning Camera-Agnostic White-Balance Preferences](https://arxiv.org/abs/2507.01342)
*Luxi Zhao,Mahmoud Afifi,Michael S. Brown*

Main category: cs.CV

TL;DR: 论文提出了一种轻量级方法，通过学习后照明估计映射，将中性白平衡转换为美学偏好，实现跨相机一致的颜色渲染。


<details>
  <summary>Details</summary>
Motivation: 商业自动白平衡（AWB）系统通常追求美学偏好而非中性颜色校正，且现有学习型方法难以跨相机传感器泛化。

Method: 提出一种后照明估计映射，将中性白平衡转换为美学偏好的颜色校正，适用于任何中性AWB模块。

Result: 模型仅含约500参数，运行时间为0.024毫秒，在771张智能手机图像数据集上表现优异。

Conclusion: 该方法实现了跨相机美学一致性，计算开销低，兼容现有技术。
...
### [202] [Learning from Random Subspace Exploration: Generalized Test-Time Augmentation with Self-supervised Distillation](https://arxiv.org/abs/2507.01347)
*Andrei Jelea,Ahmed Nabil Belbachir,Marius Leordeanu*
...
<details>
  <summary>Details</summary>
Motivation: 现有测试时间增强方法缺乏通用性，GTTA旨在提供一种适用于多种任务且高效的解决方案。

Method: GTTA通过随机扰动PCA子空间投影生成多个测试样本，形成鲁棒集成，并引入自监督学习阶段优化模型。

Result: GTTA在多种任务和数据集上表现优异，包括图像分类、分割、语音识别和房价预测，并在特定任务（如低能见度水下视频中的三文鱼分割）中验证了有效性。
...
### [203] [Long-Tailed Distribution-Aware Router For Mixture-of-Experts in Large Vision-Language Model](https://arxiv.org/abs/2507.01351)
*Chaoxiang Cai,Longrong Yang,Kaibing Chen,Fan Yang,Xi Li*

Main category: cs.CV

TL;DR: 提出了一种长尾分布感知路由器（LTDR），用于视觉语言模型的专家混合（MoE）框架，解决了模态特定路由和视觉尾部令牌的专家激活问题。


<details>
  <summary>Details</summary>
Motivation: 现有MoE框架在视觉语言模型中忽略了视觉和语言分布的差异，导致路由策略不匹配。

Method: 提出LTDR，包括模态特定路由策略和针对视觉尾部令牌的专家激活增强策略。

Result: 在多个基准测试中验证了方法的有效性。

Conclusion: LTDR显著提升了视觉语言模型的性能，特别是在处理视觉尾部令牌时。
...
### [204] [3D Gaussian Splatting Driven Multi-View Robust Physical Adversarial Camouflage Generation](https://arxiv.org/abs/2507.01367)
*Tianrui Lou,Xiaojun Jia,Siyuan Liang,Jiawei Liang,Ming Zhang,Yanjun Xiao,Xiaochun Cao*

Main category: cs.CV

TL;DR: 提出了一种基于3D高斯散射（3DGS）的物理攻击框架PGA，解决了现有方法在多视角和复杂环境中的对抗效果和鲁棒性问题。


<details>
  <summary>Details</summary>
Motivation: 物理对抗攻击在安全关键场景（如自动驾驶）中暴露深度神经网络的脆弱性，现有方法依赖耗时且不准确的模拟环境，且在多视角和复杂环境中效果不佳。

Method: 利用3DGS快速精确重建和逼真渲染能力，通过防止高斯间的遮挡和采用min-max优化调整背景，提升对抗效果和跨视角鲁棒性。

Result: 实验验证了PGA在对抗效果和鲁棒性上的优越性。

Conclusion: PGA为物理对抗攻击提供了高效、鲁棒的解决方案，适用于复杂和多视角环境。
...
### [205] [Activation Reward Models for Few-Shot Model Alignment](https://arxiv.org/abs/2507.01368)
*Tianning Chai,Chancharik Mitra,Brandon Huang,Gautam Rajendrakumar Gare,Zhiqiu Lin,Assaf Arbelle,Leonid Karlinsky,Rogerio Feris,Trevor Darrell,Deva Ramanan,Roei Herzig*

Main category: cs.CV

TL;DR: 提出了一种新的少样本奖励建模方法（Activation RMs），通过激活导向构建对齐的奖励信号，无需额外微调，性能优于现有方法，并在安全关键应用中有效减少奖励黑客行为。


<details>
  <summary>Details</summary>
Motivation: 对齐大型语言模型（LLMs）和多模态模型（LMMs）到人类偏好是提升生成输出质量的核心挑战，传统奖励建模难以适应新偏好。

Method: 利用激活导向构建奖励信号（Activation RMs），仅需少量监督且无需额外模型微调。

Result: Activation RMs在标准奖励建模基准上优于现有少样本方法，并在新提出的PreferenceHack基准中表现最佳，甚至超越GPT-4o。

Conclusion: Activation RMs为少样本奖励建模提供了高效且安全的方法，适用于实际应用。
...
### [206] [Active Measurement: Efficient Estimation at Scale](https://arxiv.org/abs/2507.01372)
*Max Hamilton,Jinlin Lai,Wenlong Zhao,Subhransu Maji,Daniel Sheldon*
...
Method: 结合AI模型预测和人类标注，采用重要性采样策略，迭代优化模型并生成无偏的蒙特卡洛估计。

Result: 主动测量框架在多个测量任务中显著降低了估计误差，且对AI模型的准确性要求较低。

Conclusion: 主动测量是一种高效且可靠的科学测量方法，适用于需要高精度统计保证的场景。
...
### [207] [MUG: Pseudo Labeling Augmented Audio-Visual Mamba Network for Audio-Visual Video Parsing](https://arxiv.org/abs/2507.01384)
*Langyu Wang,Bingke Zhu,Yingying Chen,Yiyuan Zhang,Ming Tang,Jinqiao Wang*
...
### [208] [FixTalk: Taming Identity Leakage for High-Quality Talking Head Generation in Extreme Cases](https://arxiv.org/abs/2507.01390)
*Shuai Tan,Bill Gong,Bin Ji,Ye Pan*
...
### [209] [Coherent Online Road Topology Estimation and Reasoning with Standard-Definition Maps](https://arxiv.org/abs/2507.01397)
*Khanh Son Pham,Christian Witte,Jens Behley,Johannes Betz,Cyrill Stachniss*

Main category: cs.CV

TL;DR: 论文提出了一种利用标准地图（SD）信息预测车道段及其拓扑和道路边界的方法，通过混合编码和去噪技术提升性能，实验表明其优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 高精地图（HD）对自动驾驶至关重要，但在线构建HD地图仍具挑战性，需要统一建模复杂的道路拓扑。

Method: 提出了一种网络架构，利用SD地图的先前信息，结合混合车道段编码和去噪技术，并引入时间一致性。

Result: 实验证明该方法大幅优于现有方法。

Conclusion: 通过利用SD地图信息和改进的建模方案，实现了更优的车道和道路边界预测。
...
### [210] [Medical-Knowledge Driven Multiple Instance Learning for Classifying Severe Abdominal Anomalies on Prenatal Ultrasound](https://arxiv.org/abs/2507.01401)
*Huanwen Liang,Jingxian Xu,Yuanji Zhang,Yuhao Huang,Yuhan Zhang,Xin Yang,Ran Li,Xuedong Deng,Yanjun Liu,Guowei Tao,Yun Wu,Sheng Zhao,Xinru Gao,Dong Ni*

Main category: cs.CV

TL;DR: 提出了一种基于多实例学习（MIL）的方法，用于胎儿腹部异常的产前超声分类，无需标准平面定位，性能优于现有技术。


<details>
  <summary>Details</summary>
Motivation: 胎儿腹部畸形是严重的先天性异常，需要准确诊断以指导妊娠管理并降低死亡率。现有AI研究多关注图像级分类，缺乏案例级诊断。

Method: 采用混合注意力专家模块（MoAE）对不同平面加权；提出医学知识驱动的特征选择模块（MFS）进行案例级自监督图像标记选择；提出基于提示的原型学习（PPL）增强MFS。
...
### [211] [DocShaDiffusion: Diffusion Model in Latent Space for Document Image Shadow Removal](https://arxiv.org/abs/2507.01422)
*Wenjie Liu,Bingshu Wang,Ze Wang,C. L. Philip Chen*

Main category: cs.CV

TL;DR: 本文提出了一种名为DocShaDiffusion的潜在空间扩散模型，用于文档图像阴影去除，结合阴影软掩模生成模块（SSGM）和阴影掩模引导扩散模块（SMGDM），有效处理彩色阴影问题。
...
Conclusion: 提出的方法在文档图像阴影去除任务中表现优异，代码和数据集将公开。
...
### [212] [CaptionSmiths: Flexibly Controlling Language Pattern in Image Captioning](https://arxiv.org/abs/2507.01409)
*Kuniaki Saito,Donghyun Kim,Kwanyong Park,Atsushi Hashimoto,Yoshitaka Ushiku*
...
<details>
  <summary>Details</summary>
Motivation: 现有图像字幕模型难以精细控制生成字幕的属性，如长度和描述性，因训练时未考虑这些条件且无法平滑切换语言模式。

Method: 量化字幕的三种属性（长度、描述性、词汇独特性）为连续标量值，通过端点向量插值实现条件控制。

Result: 模型能平滑调整输出字幕属性，词汇对齐优于基线，如长度控制误差降低506%。

Conclusion: CaptionSmiths方法有效实现单一模型对多样化语言模式的控制，提升生成字幕的灵活性和质量。
...
### [213] [NOCTIS: Novel Object Cyclic Threshold based Instance Segmentation](https://arxiv.org/abs/2507.01463)
*Max Gandyra,Alessandro Santonicola,Michael Beetz*

Main category: cs.CV

TL;DR: NOCTIS是一种新型实例分割框架，利用Grounded-SAM 2和DINOv2实现无需重新训练的高效分割，在BOP 2023挑战中表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决无需重新训练即可泛化到各种新对象的实例分割问题。

Method: 结合Grounded-SAM 2生成对象提议和分割掩码，利用DINOv2的零样本能力生成图像嵌入，通过相似性评分匹配对象。

Result: 在BOP 2023挑战的七个核心数据集上表现优于现有RGB和RGB-D方法。

Conclusion: NOCTIS框架在无需额外训练的情况下实现了高效的新对象实例分割。
...
### [214] [Gradient Short-Circuit: Efficient Out-of-Distribution Detection via Feature Intervention](https://arxiv.org/abs/2507.01417)
*Jiawei Gu,Ziyue Qiao,Zechao Li*

Main category: cs.CV

TL;DR: 本文提出了一种基于梯度方向一致性的OOD检测方法，通过短路异常梯度提升检测效果，且无需额外计算开销。


<details>
  <summary>Details</summary>
Motivation: 在开放世界中，OOD检测对模型安全部署至关重要。研究发现，ID样本的梯度方向一致，而OOD样本的梯度方向混乱，这启发了本文方法。

Method: 提出一种推理阶段技术，短路异常梯度以降低OOD置信度，同时保持ID分类性能，并引入局部一阶近似避免额外计算。

Result: 在标准OOD基准测试中表现显著提升，方法轻量且易于集成到现有推理流程。
...
### [215] [Crop Pest Classification Using Deep Learning Techniques: A Review](https://arxiv.org/abs/2507.01494)
*Muhammad Hassam Ejaz,Muhammad Bilal,Usman Habib*

Main category: cs.CV

TL;DR: 本文回顾了2018至2025年间37项关于AI害虫分类的研究，总结了基于深度学习的害虫检测方法的发展趋势、挑战及未来方向。


<details>
  <summary>Details</summary>
Motivation: 传统害虫监测方法效率低、难以扩展，AI技术（如CNN、ViT和混合模型）为自动化害虫检测提供了高效解决方案。

Method: 分析37项研究，按作物类型、害虫种类、模型架构、数据集使用和技术挑战分类，对比早期CNN与新兴混合/Transformer模型的性能。

Result: 研究发现混合和Transformer模型在精度和上下文理解上优于传统CNN，但仍面临数据集不平衡、小害虫检测难、泛化性差和边缘设备部署等挑战。

Conclusion: AI害虫监测系统前景广阔，但需解决现有技术挑战，未来应关注模型优化和实际应用部署。
...
### [216] [DiffMark: Diffusion-based Robust Watermark Against Deepfakes](https://arxiv.org/abs/2507.01428)
*Chen Sun,Haiyang Sun,Zhiqing Guo,Yunfeng Diao,Liejun Wang,Dan Ma,Gaobo Yang,Keqin Li*

Main category: cs.CV

TL;DR: DiffMark是一种基于扩散模型的新型鲁棒水印框架，通过改进训练和采样方案，结合面部图像和水印条件生成水印图像，并利用交叉信息融合模块和对抗性指导增强水印对Deepfake操作的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: Deepfake技术带来安全和隐私威胁，现有水印方法对Deepfake操作的鲁棒性不足。扩散模型在图像生成中表现出色，可用于水印融合。

Method: 提出DiffMark框架，通过时间步依赖的面部条件权重和交叉信息融合模块（CIF）实现水印与图像的融合，并利用冻结自编码器和Deepfake对抗性指导增强鲁棒性。

Result: 实验证明DiffMark在典型Deepfake操作中表现有效。

Conclusion: DiffMark为Deepfake环境下的水印技术提供了鲁棒解决方案。
...
### [217] [Integrating Traditional and Deep Learning Methods to Detect Tree Crowns in Satellite Images](https://arxiv.org/abs/2507.01502)
*Ozan Durgut,Beril Kallfelz-Sirmacek,Cem Unsalan*
...
Result: 新方法在树冠检测数量和准确性上有所提升，但仍存在改进空间。

Conclusion: 规则化方法有效提升了树冠检测性能，未来可进一步优化。
...
### [218] [TurboReg: TurboClique for Robust and Efficient Point Cloud Registration](https://arxiv.org/abs/2507.01439)
*Shaocheng Yan,Pengcheng Shi,Zhenjun Zhao,Kaixin Wang,Kuang Cao,Ji Wu,Jiayuan Li*

Main category: cs.CV

TL;DR: TurboReg提出了一种快速鲁棒的PCR估计方法，通过轻量级的TurboClique和并行化的PGS算法，显著提高了速度和性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于最大团搜索的方法虽然召回率高，但时间复杂度过高，不适用于实时应用。

Method: 使用TurboClique（3-团）和PGS算法，通过高SC$^2$分数的匹配对引导搜索，实现高效并行化。

Result: 在多个数据集上达到SOTA性能，速度提升显著（如3DMatch+FCGF上快208.22倍）。

Conclusion: TurboReg在保持高召回率的同时，大幅提升了计算效率，适用于实时PCR任务。
...
### [219] [Following the Clues: Experiments on Person Re-ID using Cross-Modal Intelligence](https://arxiv.org/abs/2507.01504)
*Robert Aufschläger,Youssef Shoeb,Azarm Nowzad,Michael Heigl,Fabian Bally,Martin Schramm*
...
<details>
  <summary>Details</summary>
Motivation: 街景数据作为开放数据对自动驾驶和AI研究至关重要，但其中包含的PII信息对行人隐私构成威胁。

Method: 结合大型视觉语言模型、图注意力网络和表示学习，检测文本可描述的PII线索，并优化行人重识别。

Result: 在Market-1501到CUHK03-np的跨数据集实验中表现优异，验证了框架的实用性。

Conclusion: cRID能有效检测语义上有意义的PII，提升行人重识别的性能，代码已开源。
...
### [220] [OoDDINO:A Multi-level Framework for Anomaly Segmentation on Complex Road Scenes](https://arxiv.org/abs/2507.01455)
*Yuxing Liu,Ji Zhang,Zhou Xuchuan,Jingzhong Xiao,Huimin Yang,Jiaxin Zhong*

Main category: cs.CV

TL;DR: OoDDINO是一个新颖的多级异常分割框架，通过粗到细的异常检测策略解决现有像素级方法的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有方法忽视像素间的空间相关性，且全局阈值策略导致误检或漏检。

Method: 结合不确定性引导的异常检测模型和像素级分割模型，采用正交不确定性感知融合策略和自适应双阈值网络。

Result: 在两个基准数据集上的实验验证了框架的优越性和兼容性。
...
### [221] [Autonomous AI Surveillance: Multimodal Deep Learning for Cognitive and Behavioral Monitoring](https://arxiv.org/abs/2507.01590)
*Ameer Hamza,Zuhaib Hussain But,Umar Arif,Samiya,M. Abdullah Asad,Muhammad Naeem*

Main category: cs.CV

TL;DR: 该研究提出了一种多模态课堂监控系统，结合了睡意检测、手机使用追踪和人脸识别，以提高学生注意力评估的精确度。系统采用YOLOv8和LResNet Occ FC等技术，实现了高精度的实时监测，并在实验中表现出色。


<details>
  <summary>Details</summary>
Motivation: 提升课堂监控的精确性和全面性，通过多模态技术实时评估学生的注意力和行为，同时实现自动考勤功能。

Method: 系统整合了YOLOv8模型（用于手机和睡意检测）和LResNet Occ FC（用于人脸识别），并基于RMFD和Roboflow数据集进行训练。通过PHP和ESP32-CAM硬件实现数据捕获和实时监测。

Result: 睡意检测的mAP@50为97.42%，人脸识别的验证准确率为86.45%，手机检测的mAP@50为85.89%。

Conclusion: 该系统通过多模态技术实现了高效的课堂监控和自动考勤，具有较高的准确性和可扩展性，适用于多样化的教育环境。
...
### [222] [Survivability of Backdoor Attacks on Unconstrained Face Recognition Systems](https://arxiv.org/abs/2507.01607)
*Quentin Le Roux,Yannick Teglia,Teddy Furon,Philippe Loubet-Moundi,Eric Bourbao*

Main category: cs.CV

TL;DR: 论文首次系统研究了深度学习人脸识别系统中的后门攻击，展示了两种新型攻击方式，并提出了应对措施。


<details>
  <summary>Details</summary>
Motivation: 深度学习人脸识别广泛应用，但其安全漏洞（尤其是后门攻击）在现实无约束系统中未被充分研究。

Method: 通过探索两种新型后门攻击（人脸生成和关键点偏移），并结合20种系统配置和15种攻击案例进行验证。

Result: 证明了单一后门可绕过系统功能，并提出了应对措施。

Conclusion: 研究填补了文献空白，为相关系统提供了安全建议。
...
### [223] [Representation Entanglement for Generation:Training Diffusion Transformers Is Much Easier Than You Think](https://arxiv.org/abs/2507.01467)
*Ge Wu,Shen Zhang,Ruijing Shi,Shanghua Gao,Zhenyuan Chen,Lei Wang,Zhaowei Chen,Hongcheng Gao,Yao Tang,Jian Yang,Ming-Ming Cheng,Xiang Li*
...
<details>
  <summary>Details</summary>
Motivation: 现有方法（如REPA）在训练扩散模型时利用外部视觉表示，但在推理过程中未能充分利用判别性表示，限制了生成潜力。

Method: REG方法将低层图像潜在变量与预训练基础模型的单个高层类别标记纠缠，直接从纯噪声中生成一致的图像-类别对。

Result: 在ImageNet 256×256上，REG显著加速收敛（训练速度提升63倍和23倍），并在较短时间内超越其他方法的性能。

Conclusion: REG通过语义知识主动引导图像生成，以极低的推理开销实现了高效的生成质量和训练效率提升。
...
### [224] [Prompt Guidance and Human Proximal Perception for HOT Prediction with Regional Joint Loss](https://arxiv.org/abs/2507.01630)
*Yuxiao Wang,Yu Lei,Zhenao Wei,Weiying Xue,Xinyu Jiang,Nan Zhuang,Qi Liu*
...
<details>
  <summary>Details</summary>
Motivation: 当前HOT检测模型局限于单一图像类型，导致过度分割和类别不一致，需要改进。

Method: 提出P3HOT框架，结合语义驱动的提示机制和人类近端感知机制，使用RJLoss和AD-Acc.新指标。

Result: 在HOT-Annotated数据集上，SC-Acc.、mIoU、wIoU和AD-Acc.指标分别提升0.7、2.0、1.6和11.0。

Conclusion: P3HOT在HOT检测任务中表现优异，解决了现有方法的局限性。
...
### [225] [Optimizing Methane Detection On Board Satellites: Speed, Accuracy, and Low-Power Solutions for Resource-Constrained Hardware](https://arxiv.org/abs/2507.01472)
*Jonáš Herec,Vít Růžička,Rado Pitoňák*

Main category: cs.CV

TL;DR: 论文提出了一种高效、低功耗的甲烷检测算法（Mag1c-SAS和CEM），结合机器学习模型（U-Net、LinkNet），显著提升了在资源有限的星载硬件上的检测速度（最高可达100倍和230倍）。同时，提出了三种波段选择策略，优化了处理速度而不牺牲精度。


<details>
  <summary>Details</summary>
Motivation: 甲烷是一种强效温室气体，通过高光谱卫星图像早期检测其泄漏有助于缓解气候变化。现有任务多为手动操作，可能遗漏重要事件，且传统方法计算量大，不适合星载硬件。

Method: 测试了快速目标检测方法（ACE、CEM），并提出Mag1c-SAS算法，结合机器学习模型（U-Net、LinkNet）进行优化。还提出了三种波段选择策略。

Result: Mag1c-SAS和CEM在检测强羽流时表现良好，计算效率高（比原Mag1c快100倍和230倍）。其中一种波段选择策略优于传统方法且使用更少通道。

Conclusion: 研究为星载甲烷检测提供了高效解决方案，代码和数据已开源，为未来低硬件要求的实时检测奠定了基础。
...
### [226] [Active Control Points-based 6DoF Pose Tracking for Industrial Metal Objects](https://arxiv.org/abs/2507.01478)
*Chentao Shen,Ding Pan,Mingyu Mei,Zaixing He,Xinyue Zhao*
...
<details>
  <summary>Details</summary>
Motivation: 工业金属物体的反射特性使其位姿跟踪成为挑战，需要一种更有效的方法。

Method: 使用图像控制点主动生成边缘特征进行优化，并引入最优控制点回归方法提高鲁棒性。

Result: 方法在数据集评估和实际任务中表现有效，适用于实时跟踪。

Conclusion: 该方法为工业金属物体的实时位姿跟踪提供了可行解决方案，代码已开源。
...
### [227] [Depth Anything at Any Condition](https://arxiv.org/abs/2507.01634)
*Boyuan Sun,Modi Jin,Bowen Yin,Qibin Hou*
...
Method: 提出无监督一致性正则化微调范式，并引入空间距离约束以学习块级相对关系。

Result: 实验证明DepthAnything-AC在多样化基准测试中具有零样本能力，包括真实恶劣天气、合成失真和通用场景。

Conclusion: DepthAnything-AC能够有效应对复杂环境条件，提升深度估计的准确性和语义边界清晰度。
...
### [228] [What Really Matters for Robust Multi-Sensor HD Map Construction?](https://arxiv.org/abs/2507.01484)
*Xiaoshuai Hao,Yuting Zhao,Yuheng Ji,Luanyuan Dai,Peng Hao,Dingzhe Li,Shuai Cheng,Rong Yin*

Main category: cs.CV

TL;DR: 该论文提出了一种增强多模态融合方法鲁棒性的策略，用于高精地图构建，通过数据增强、新型多模态融合模块和模态丢弃训练策略，显著提升了基线方法的鲁棒性，并在NuScenes数据集上达到了最先进的性能。
...
Result: 在NuScenes数据集上，实验结果表明所提方法显著提升了基线方法的鲁棒性，并在干净验证集上达到了最先进的性能。
...
### [229] [Autoregressive Image Generation with Linear Complexity: A Spatial-Aware Decay Perspective](https://arxiv.org/abs/2507.01652)
*Yuxin Mao,Zhen Qin,Jinxing Zhou,Hui Deng,Xuyang Shen,Bin Fan,Jing Zhang,Yiran Zhong,Yuchao Dai*

Main category: cs.CV

TL;DR: 提出了一种名为LASAD的新型注意力机制，通过保留2D空间关系解决了线性注意力在图像生成中的性能下降问题，并基于此构建了高效且高质量的图像生成模型LASADGen。
...
Method: 提出线性注意力与空间感知衰减（LASAD）机制，通过基于真实2D空间位置的位置依赖衰减因子保留空间关系，构建LASADGen模型。
...
### [230] [AVC-DPO: Aligned Video Captioning via Direct Preference Optimization](https://arxiv.org/abs/2507.01492)
*Jiyang Tang,Hengyi Li,Yifan Du,Wayne Xin Zhao*

Main category: cs.CV

TL;DR: AVC-DPO是一种后训练框架，通过偏好对齐增强视频MLLM的标题生成能力，重点关注人类偏好的时间和空间信息。


<details>
  <summary>Details</summary>
Motivation: 尽管视频MLLM在视频标题生成任务中取得进展，但根据人类偏好调整标题焦点仍具挑战性。

Method: 设计了针对时间和空间信息的增强提示，通过偏好感知训练和标题对齐优化模型。

Result: 在LOVE@CVPR'25 Workshop Track 1A中表现优异，VDC基准测试中排名第一。

Conclusion: AVC-DPO成功提升了视频MLLM的标题生成能力，满足了人类偏好需求。
...
### [231] [ECCV 2024 W-CODA: 1st Workshop on Multimodal Perception and Comprehension of Corner Cases in Autonomous Driving](https://arxiv.org/abs/2507.01735)
*Kai Chen,Ruiyuan Gao,Lanqing Hong,Hang Xu,Xu Jia,Holger Caesar,Dengxin Dai,Bingbing Liu,Dzmitry Tsishkou,Songcen Xu,Chunjing Xu,Qiang Xu,Huchuan Lu,Dit-Yan Yeung*

Main category: cs.CV

TL;DR: 首届W-CODA研讨会聚焦自动驾驶极端场景，结合多模态感知技术，探讨前沿解决方案。


<details>
  <summary>Details</summary>
Motivation: 探索下一代自动驾驶技术，解决极端场景问题，提升可靠性和智能性。

Method: 邀请5位学术界和工业界专家分享最新进展，组织双轨挑战（场景理解与生成）。

Result: 研讨会汇集了前沿研究成果，推动了自动驾驶技术在极端场景中的应用。
...
### [232] [ReFlex: Text-Guided Editing of Real Images in Rectified Flow via Mid-Step Feature Extraction and Attention Adaptation](https://arxiv.org/abs/2507.01496)
*Jimyeong Kim,Jungwon Park,Yeji Song,Nojun Kwak,Wonjong Rhee*

Main category: cs.CV

TL;DR: 提出了一种基于ReFlow的无需训练、无需掩码的真实图像编辑方法，通过分析中间表示和利用中步潜在特征提升编辑效果。
...
Method: 分析多模态Transformer块的中间表示，提取关键特征；利用中步潜在特征保持结构；调整注意力注入以增强编辑性和文本对齐。

Result: 在两个基准测试中优于九种基线方法，人类评估显示用户偏好显著。

Conclusion: 该方法在真实图像编辑中表现优越，无需额外训练或用户输入掩码。
...
### [233] [Are Vision Transformer Representations Semantically Meaningful? A Case Study in Medical Imaging](https://arxiv.org/abs/2507.01788)
*Montasir Shams,Chashi Mahiul Islam,Shaeke Salman,Phat Tran,Xiuwen Liu*

Main category: cs.CV

TL;DR: 论文揭示了视觉变换器（ViTs）在医学图像分类中的表示缺乏语义意义，且对微小变化敏感，导致分类结果不可靠。


<details>
  <summary>Details</summary>
Motivation: 研究ViTs在医学图像任务中的表现，探究其表示是否具有语义意义及其对微小变化的鲁棒性。

Method: 使用基于投影梯度的算法分析ViTs的表示，评估其对图像微小变化的敏感性。
...
### [234] [Mamba Guided Boundary Prior Matters: A New Perspective for Generalized Polyp Segmentation](https://arxiv.org/abs/2507.01509)
*Tapas K. Dutta,Snehashis Majhi,Deepak Ranjan Nayak,Debesh Jha*

Main category: cs.CV

TL;DR: SAM-MaGuP是一种新型息肉分割方法，通过边界蒸馏模块和1D-2D Mamba适配器解决了现有方法在弱边界分割上的不足，显著提升了分割精度和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 息肉分割在结肠镜图像中对结直肠癌早期诊断至关重要，但现有方法在弱边界分割和泛化能力上表现不足。

Method: 提出SAM-MaGuP，结合边界蒸馏模块和1D-2D Mamba适配器，增强特征学习和全局上下文交互。

Result: 在五个数据集上表现优于现有方法，实现了更高的分割精度和鲁棒性。

Conclusion: SAM-MaGuP通过创新设计为息肉分割领域设定了新标准。
...
### [235] [Exploring Pose-based Sign Language Translation: Ablation Studies and Attention Insights](https://arxiv.org/abs/2507.01532)
*Tomas Zelezny,Jakub Straka,Vaclav Javorek,Ondrej Valach,Marek Hruz,Ivan Gruber*

Main category: cs.CV

TL;DR: 本文探讨了基于姿态的数据预处理技术（归一化、插值和增强）对手语翻译（SLT）性能的影响，使用改进的T5编码器-解码器模型，结果表明这些技术能显著提升模型的鲁棒性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 研究如何通过姿态数据预处理技术提升手语翻译系统的性能，解决连续无注释翻译的挑战。

Method: 采用改进的T5编码器-解码器模型处理姿态表示，并在YouTubeASL和How2Sign数据集上进行消融实验，分析不同预处理策略的影响。

Result: 适当的归一化、插值和增强技术显著提高了翻译准确性和模型泛化能力，同时发现专用寄存器标记能进一步提升性能。

Conclusion: 姿态数据预处理技术对SLT性能至关重要，未来可探索专用寄存器标记的潜力。
...
### [236] [TrackingMiM: Efficient Mamba-in-Mamba Serialization for Real-time UAV Object Tracking](https://arxiv.org/abs/2507.01535)
*Bingxi Liu,Calvin Chen,Junhao Li,Guyang Yu,Haoqian Song,Xuchen Liu,Jinqiang Cui,Hong Zhang*

Main category: cs.CV

TL;DR: 提出了一种基于Mamba模型的TrackingMiM架构，解决了Vision Transformer在无人机跟踪任务中的二次复杂度问题，实现了高效且精确的跟踪。


<details>
  <summary>Details</summary>
Motivation: Vision Transformer在无人机实时跟踪中因二次复杂度问题效率低下，而Mamba模型的计算效率和长序列建模能力为这一问题提供了潜在解决方案。

Method: 提出TrackingMiM，一种Mamba-in-Mamba架构，通过嵌套扫描机制独立处理时空一致的图像块，并利用模板帧作为查询令牌进行跟踪。

Result: 在五个无人机跟踪基准测试中，TrackingMiM实现了最先进的精度和显著更高的速度。

Conclusion: TrackingMiM通过结合Mamba的高效性和嵌套扫描机制，为无人机实时跟踪提供了一种高效且精确的解决方案。
...
### [237] [A Multi-Centric Anthropomorphic 3D CT Phantom-Based Benchmark Dataset for Harmonization](https://arxiv.org/abs/2507.01539)
*Mohammadreza Amirian,Michael Bach,Oscar Jimenez-del-Toro,Christoph Aberle,Roger Schaer,Vincent Andrearczyk,Jean-Félix Maestrati,Maria Martin Asiain,Kyriakos Flouris,Markus Obmann,Clarisse Dromain,Benoît Dufour,Pierre-Alexandre Alois Poletti,Hendrik von Tengg-Kobligk,Rolf Hügli,Martin Kretzschmar,Hatem Alkadhi,Ender Konukoglu,Henning Müller,Bram Stieltjes,Adrien Depeursinge*

Main category: cs.CV

TL;DR: 论文提出一个开源基准数据集，用于促进AI在CT分析中的泛化能力研究，并提供评估方法和基线结果。


<details>
  <summary>Details</summary>
Motivation: AI在医学CT分析中因数据分布变化（如扫描仪制造商、重建技术或剂量差异）导致泛化能力差，需要解决这一问题。

Method: 使用仿人体模型CT扫描数据集（1378个图像系列，13台扫描仪），提供评估图像和特征稳定性及肝脏组织分类的方法和开源代码。

Result: 数据集和方法为开发AI协调技术提供了基准和工具。

Conclusion: 该研究为AI在医学影像中的泛化问题提供了实用解决方案和资源。
...
### [238] [Interpolation-Based Event Visual Data Filtering Algorithms](https://arxiv.org/abs/2507.01557)
*Marcin Kowlaczyk,Tomasz Kryjak*
...
Conclusion: 该方法高效、内存占用低，适合嵌入式设备实现。
...
### [239] [How Well Does GPT-4o Understand Vision? Evaluating Multimodal Foundation Models on Standard Computer Vision Tasks](https://arxiv.org/abs/2507.01955)
*Rahul Ramachandran,Ali Garjani,Roman Bachmann,Andrei Atanov,Oğuzhan Fatih Kar,Amir Zamir*
...
### [240] [A Gift from the Integration of Discriminative and Diffusion-based Generative Learning: Boundary Refinement Remote Sensing Semantic Segmentation](https://arxiv.org/abs/2507.01573)
*Hao Wang,Keyan Hu,Xin Guo,Haifeng Li,Chao Tao*
...
Result: 在五个遥感语义分割数据集上的实验表明，IDGBR能一致地改进不同判别式架构的粗分割结果。

Conclusion: 结合判别式和生成式学习的IDGBR框架有效提升了边界精度，解决了高频和低频特征的平衡问题。
...
### [241] [Locality-aware Parallel Decoding for Efficient Autoregressive Image Generation](https://arxiv.org/abs/2507.01957)
*Zhuoyang Zhang,Luke J. Huang,Chengyue Wu,Shang Yang,Kelly Peng,Yao Lu,Song Han*

Main category: cs.CV

TL;DR: 提出Locality-aware Parallel Decoding (LPD)方法，通过灵活并行自回归建模和局部感知生成顺序，显著加速自回归图像生成，减少生成步骤并降低延迟。


<details>
  <summary>Details</summary>
Motivation: 传统自回归图像生成依赖顺序预测，导致高延迟；现有方法尝试多块预测并行化，但效果有限。

Method: 引入灵活并行自回归建模（支持任意生成顺序和并行度）和局部感知生成顺序（最小化组内依赖，最大化上下文支持）。

Result: 在ImageNet类条件生成中，将生成步骤从256减少到20（256×256分辨率），1024减少到48（512×512分辨率），延迟降低至少3.4倍。
...
### [242] [SketchColour: Channel Concat Guided DiT-based Sketch-to-Colour Pipeline for 2D Animation](https://arxiv.org/abs/2507.01586)
*Bryan Constantine Sadihin,Michael Hua Wang,Shei Pern Chua,Hang Su*
...
Method: 采用扩散变换器（DiT）架构，通过轻量级通道连接适配器和LoRA微调注入草图信息，避免参数和内存膨胀。

Result: 在SAKUGA数据集上表现优于现有方法，训练数据仅为竞争对手的一半，且生成动画时间连贯、伪影少。

Conclusion: SketchColour提供了一种高效、低成本的2D动画色彩化解决方案，具有实际应用潜力。
...
### [243] [Towards Controllable Real Image Denoising with Camera Parameters](https://arxiv.org/abs/2507.01587)
*Youngjin Oh,Junhyeong Kwon,Keuntek Lee,Nam Ik Cho*
...
<details>
  <summary>Details</summary>
Motivation: 现有深度学习方法缺乏根据噪声水平和用户偏好调整去噪强度的灵活性。

Method: 将相机参数转换为控制向量，增强去噪网络的性能。
...
### [244] [DepthSync: Diffusion Guidance-Based Depth Synchronization for Scale- and Geometry-Consistent Video Depth Estimation](https://arxiv.org/abs/2507.01603)
*Yue-Jiang Dong,Wang Zhao,Jiale Xu,Ying Shan,Song-Hai Zhang*
...
Result: 实验表明，DepthSync在多种数据集上显著提升了深度预测的尺度和几何一致性，尤其适用于长视频。
...
### [245] [Perception-Oriented Latent Coding for High-Performance Compressed Domain Semantic Inference](https://arxiv.org/abs/2507.01608)
*Xu Zhang,Ming Lu,Yan Chen,Zhan Ma*
...
Result: POLC在率感知性能上媲美生成式图像编码方法，同时显著提升视觉任务性能，微调开销极低。
...
### [246] [SAILViT: Towards Robust and Generalizable Visual Backbones for MLLMs via Gradual Feature Refinement](https://arxiv.org/abs/2507.01643)
*Weijie Yin,Dingkang Yang,Hongyuan Dong,Zijian Kang,Jiacong Wang,Xiao Liang,Chao Feng,Jiao Ran*

Main category: cs.CV

TL;DR: SAILViT是一种逐步特征学习的ViT，用于提升多模态大语言模型（MLLMs）的性能，解决了参数初始化冲突和模态语义差距问题。


<details>
  <summary>Details</summary>
Motivation: 现有ViTs难以直接与LLMs协同训练，存在参数初始化冲突和模态语义差距，限制了MLLMs在复杂多模态交互中的性能。

Method: 提出SAILViT，通过逐步特征细化实现粗到细的特征对齐和知识注入，支持目标训练需求。

Result: SAILViT在不同参数规模、架构、训练策略和数据规模下表现出强大的鲁棒性和泛化能力，显著提升了MLLMs在OpenCompass基准上的性能。

Conclusion: SAILViT有效解决了ViTs与LLMs协同训练的挑战，为MLLMs的性能提升提供了新方向。
...
### [247] [RobuSTereo: Robust Zero-Shot Stereo Matching under Adverse Weather](https://arxiv.org/abs/2507.01653)
*Yuran Wang,Yingping Liang,Yutao Hu,Ying Fu*
...
Method: 1. 使用扩散模拟生成高质量合成数据；2. 设计结合ConvNet和去噪Transformer的稳健特征编码器。

Result: 实验表明RobuSTereo显著提升了模型在恶劣天气下的鲁棒性和泛化能力。

Conclusion: RobuSTereo通过数据增强和特征提取优化，有效解决了立体匹配在恶劣天气中的挑战。
...
### [248] [SPoT: Subpixel Placement of Tokens in Vision Transformers](https://arxiv.org/abs/2507.01654)
*Martine Hjelkrem-Tan,Marius Aasan,Gabriel Y. Arteaga,Adín Ramírez Rivera*

Main category: cs.CV

TL;DR: 提出了一种名为SPoT的新型tokenization策略，通过连续放置token来突破传统网格限制，显著提升性能并减少推理时所需的token数量。
...
Method: 提出Subpixel Placement of Tokens (SPoT)，利用oracle-guided搜索实现连续token定位。

Result: 通过理想子像素定位，显著减少推理时所需的token数量，同时提升性能。
...
### [249] [What does really matter in image goal navigation?](https://arxiv.org/abs/2507.01667)
*Gianluca Monaci,Philippe Weinzaepfel,Christian Wolf*

Main category: cs.CV

TL;DR: 研究了图像目标导航任务是否可以通过端到端强化学习训练解决，并探讨了架构选择对相对姿态估计的影响。


<details>
  <summary>Details</summary>
Motivation: 探索图像目标导航任务能否通过端到端强化学习高效解决，以验证是否可以通过导航奖励训练相对姿态估计。

Method: 通过大规模研究分析架构选择（如延迟融合、通道堆叠、空间到深度投影和交叉注意力）对导航训练中相对姿态估计的影响。

Result: 发现模拟器设置会影响方法效果，但能力可部分迁移到更真实场景；导航性能与相对姿态估计性能存在相关性。

Conclusion: 端到端强化学习在图像目标导航中具有一定潜力，但需注意模拟器设置的影响，且能力迁移存在限制。
...
### [250] [Facial Emotion Learning with Text-Guided Multiview Fusion via Vision-Language Model for 3D/4D Facial Expression Recognition](https://arxiv.org/abs/2507.01673)
*Muzammil Behzad*
...
### [251] [Component Adaptive Clustering for Generalized Category Discovery](https://arxiv.org/abs/2507.01711)
*Mingfu Yan,Jiancheng Huang,Yifan Liu,Shifeng Chen*

Main category: cs.CV

TL;DR: AdaGCD是一个基于自适应槽注意力（AdaSlot）的聚类对比学习框架，用于在部分标记数据集中动态分类已知和未知类别，无需预定义类别数量。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖预定义类别数量的假设，难以处理真实数据的复杂性。AdaGCD旨在通过自适应机制解决这一问题。

Method: AdaGCD结合自适应槽注意力（AdaSlot），动态确定最优槽数量，并利用对比学习进行聚类。

Result: 在公开和细粒度数据集上的实验验证了AdaGCD的有效性，尤其在利用空间局部信息进行分类发现方面表现优异。

Conclusion: AdaGCD通过动态分配表示容量和自适应机制，显著提升了开放世界场景下的类别发现能力。
...
### [252] [Using Wavelet Domain Fingerprints to Improve Source Camera Identification](https://arxiv.org/abs/2507.01712)
*Xinle Tian,Matthew Nunes,Emiko Dupont,Shaunagh Downing,Freddie Lichtenstein,Matt Burns*
...
<details>
  <summary>Details</summary>
Motivation: 传统小波去噪方法在提取传感器模式噪声（SPN）时需要进行图像反演，过程复杂且效率较低。

Method: 将指纹构建为小波域指纹，直接在小波域中进行比较，避免了反演步骤。

Result: 在真实数据集上的实验表明，该方法不仅检测精度更高，还能显著提升处理速度。

Conclusion: 该方法简化了提取和比较流程，同时提高了性能和效率。
...
### [253] [Soft Self-labeling and Potts Relaxations for Weakly-Supervised Segmentation](https://arxiv.org/abs/2507.01721)
*Zhongwen Zhang,Yuri Boykov*
...
Result: 软自标记方法在标准架构下显著提升了涂鸦标注的训练效果，甚至优于完全像素级监督。
...
### [254] [When Does Pruning Benefit Vision Representations?](https://arxiv.org/abs/2507.01722)
*Enrico Cassano,Riccardo Renzulli,Andrea Bragagnolo,Marco Grangetto*

Main category: cs.CV

TL;DR: 该论文研究了剪枝对视觉模型在可解释性、无监督目标发现和与人类感知对齐三个维度的影响，发现稀疏模型在某些情况下表现更好，但效果高度依赖网络架构和参数规模。


<details>
  <summary>Details</summary>
Motivation: 探讨剪枝如何影响深度视觉模型的可解释性、表示学习及其与人类感知的对齐关系。

Method: 分析不同视觉网络架构在不同稀疏度下对特征归因可解释性方法的影响，评估剪枝是否促进更简洁的结构化表示，并研究剪枝是否增强模型与人类感知的对齐。

Result: 发现稀疏模型在特定情况下（甜点）表现出更高的可解释性、下游泛化能力和人类对齐性，但效果因网络架构和参数规模而异。

Conclusion: 剪枝对视觉模型的影响复杂，需进一步研究其何时及如何优化表示学习。
...
### [255] [HOI-Dyn: Learning Interaction Dynamics for Human-Object Motion Diffusion](https://arxiv.org/abs/2507.01737)
*Lin Wu,Zhixiang Chen,Jianglin Lan*
...
<details>
  <summary>Details</summary>
Motivation: 现有方法独立处理人和物体运动，导致物理上不合理的行为，需改进交互动态建模。

Method: 提出HOI-Dyn框架，采用驱动-响应系统和Transformer模型预测物体响应，引入动态损失优化。
...
### [256] [DeRIS: Decoupling Perception and Cognition for Enhanced Referring Image Segmentation through Loopback Synergy](https://arxiv.org/abs/2507.01738)
*Ming Dai,Wenxuan Cheng,Jiang-jiang Liu,Sen Yang,Wenxiao Cai,Yanpeng Sun,Wankou Yang*

Main category: cs.CV

TL;DR: DeRIS框架将Referring Image Segmentation分解为感知和认知两部分，揭示了当前模型的主要瓶颈在于多模态认知能力不足，并提出Loopback Synergy机制和数据增强方法以提升性能。
...
Method: DeRIS将RIS分解为感知和认知模块，引入Loopback Synergy机制增强模块协同，并提出非参考样本转换数据增强方法。

Result: DeRIS在精确分割和图像-文本理解方面表现优异，且无需特殊架构调整即可适应多参考场景。

Conclusion: DeRIS通过模块化分析和创新机制，显著提升了RIS任务的性能与泛化能力。
...
### [257] [Calibrated Self-supervised Vision Transformers Improve Intracranial Arterial Calcification Segmentation from Clinical CT Head Scans](https://arxiv.org/abs/2507.01744)
*Benjamin Jin,Grant Mair,Joanna M. Wardlaw,Maria del C. Valdés Hernández*

Main category: cs.CV

TL;DR: 本文研究了在3D医学图像分割中使用Vision Transformers（ViTs）的潜力，特别是针对颅内动脉钙化（IAC）的自动量化。通过自监督预训练和微调，ViTs在性能上超越了传统监督学习方法，并提高了对临床场景的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 尽管ViTs在自然图像领域表现出色，但在3D医学图像分割中应用较少。IAC作为神经血管疾病的生物标志物，其自动量化有助于大规模风险评估。本文旨在探索ViTs在IAC分割中的潜力。

Method: 采用掩码自编码器（MAE）框架对ViTs进行自监督预训练，随后在IST-3临床试验的异构数据上进行微调，用于IAC分割。研究了低patch大小和插值上采样对性能的影响。

Result: 自监督ViT在Dice分数上比监督学习的nnU-Net基线高出3.2分；低patch大小和插值上采样对性能至关重要；ViTs提高了对高切片厚度的鲁棒性，并在临床风险分类中提升了46%。

Conclusion: ViTs在3D医学图像分割中具有显著潜力，尤其是在IAC量化任务中。自监督预训练和低patch大小的设计是关键因素，为临床风险分类提供了更可靠的解决方案。
...
### [258] [SSL4SAR: Self-Supervised Learning for Glacier Calving Front Extraction from SAR Imagery](https://arxiv.org/abs/2507.01747)
*Nora Gourmelon,Marcel Dreier,Martin Mayr,Thorsten Seehaus,Dakota Pyles,Matthias Braun,Andreas Maier,Vincent Christlein*
...
<details>
  <summary>Details</summary>
Motivation: 冰川冰量流失加剧，需高精度全年监测以理解前缘消融和崩解过程。现有深度学习模型因领域迁移问题表现不佳。

Method: 提出两种自监督多模态预训练技术，利用新数据集SSL4SAR，并设计混合模型（Swin Transformer编码器+残差CNN解码器）。

Result: 模型在CaFFe基准数据集上平均距离误差为293米，优于之前最佳模型67米；集成模型误差降至75米，接近人类水平（38米）。

Conclusion: 新方法显著提升了冰川前缘季节性变化的监测精度。
...
### [259] [Rethinking Discrete Tokens: Treating Them as Conditions for Continuous Autoregressive Image Synthesis](https://arxiv.org/abs/2507.01756)
*Peng Zheng,Junke Wang,Yi Chang,Yizhou Yu,Rui Ma,Zuxuan Wu*

Main category: cs.CV

TL;DR: DisCon框架通过将离散标记作为条件信号而非生成目标，解决了连续表示建模的优化挑战和量化信息损失问题，显著提升了图像生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有基于自回归的视觉生成模型因量化过程导致信息损失，而连续表示建模又面临密度估计困难和分布外生成风险。
...
### [260] [Boosting Adversarial Transferability Against Defenses via Multi-Scale Transformation](https://arxiv.org/abs/2507.01791)
*Zihong Guo,Chen Wan,Yayin Zheng,Hailing Kuang,Xiaohai Lu*
...
Conclusion: SGP是一种可扩展的输入变换方法，易于与其他对抗攻击结合，有效增强迁移性。
...
### [261] [FreeLoRA: Enabling Training-Free LoRA Fusion for Autoregressive Multi-Subject Personalization](https://arxiv.org/abs/2507.01792)
*Peng Zheng,Ye Wang,Rui Ma,Zuxuan Wu*
...
Method: 采用Full Token Tuning策略训练LoRA模块，并在推理时使用Subject-Aware Inference激活对应模块。
...
### [262] [HCNQA: Enhancing 3D VQA with Hierarchical Concentration Narrowing Supervision](https://arxiv.org/abs/2507.01800)
*Shengli Zhou,Jianuo Zhu,Qilin Huang,Fangjing Wang,Yanfu Zhang,Feng Zheng*

Main category: cs.CV

TL;DR: HCNQA提出了一种分层监督方法，通过逐步聚焦的方式引导3D VQA模型形成合理的推理路径，避免浅层捷径。


<details>
  <summary>Details</summary>
Motivation: 现有3D VQA模型的答案中心监督方法仅关注最终输出，可能导致模型形成浅层推理捷径，且慢思考方法存在欠思考问题。

Method: 采用分层浓度窄化监督方法，模拟人类从广泛区域逐步聚焦到特定对象的思维过程，监督关键检查点。

Result: 实验证明HCNQA能有效确保模型形成合理推理路径并表现更优。

Conclusion: HCNQA通过分层监督解决了3D VQA中推理路径监督不足的问题，提升了模型性能。
...
### [263] [AMD: Adaptive Momentum and Decoupled Contrastive Learning Framework for Robust Long-Tail Trajectory Prediction](https://arxiv.org/abs/2507.01801)
*Bin Rao,Haicheng Liao,Yanchen Guan,Chengyue Wang,Bonan Wang,Jiaxun Zhang,Zhenning Li*
...
<details>
  <summary>Details</summary>
Motivation: 现有研究通常仅依赖基模型的预测误差，忽视了长尾轨迹模式的多样性和不确定性，导致对复杂和危险场景的预测能力不足。
...
### [264] [Modulate and Reconstruct: Learning Hyperspectral Imaging from Misaligned Smartphone Views](https://arxiv.org/abs/2507.01835)
*Daniil Reutsky,Daniil Vladimirov,Yasin Mamedov,Georgy Perevozchikov,Nancy Mehta,Egor Ershov,Radu Timofte*
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖单一RGB图像，导致重建精度受限，因此需要一种更有效的方法。

Method: 使用三摄像头智能手机系统，其中两个镜头配备精选光谱滤镜，结合理论和实证分析，提出MI-HSR框架。

Result: 在新基准测试中，该方法比现有方法提升了30%的光谱估计精度。
...
### [265] [MobileIE: An Extremely Lightweight and Effective ConvNet for Real-Time Image Enhancement on Mobile Devices](https://arxiv.org/abs/2507.01838)
*Hailong Yan,Ao Li,Xiangtao Zhang,Zhe Liu,Zenglin Shi,Ce Zhu,Le Zhang*

Main category: cs.CV

TL;DR: 提出了一种极轻量级的CNN框架（约4K参数），通过重参数化和增量权重优化策略实现高效性，结合特征自变换模块和分层双路径注意力机制，首次在移动设备上实现1,100 FPS的实时图像增强。


<details>
  <summary>Details</summary>
Motivation: 解决深度学习模型在资源受限平台（如移动设备）上部署的高计算和内存需求问题，实现实时图像增强。

Method: 采用极轻量级CNN框架，结合重参数化、增量权重优化、特征自变换模块和分层双路径注意力机制，并使用局部方差加权损失优化性能。

Result: 在多个图像增强任务中，首次实现1,100 FPS的实时推理，同时保持竞争力的图像质量，达到速度和性能的最佳平衡。
...
### [266] [Future Slot Prediction for Unsupervised Object Discovery in Surgical Video](https://arxiv.org/abs/2507.01882)
*Guiqiu Liao,Matjaz Jogan,Marcel Hussing,Edward Zhang,Eric Eaton,Daniel A. Hashimoto*
...
Method: 提出了动态时间槽变换器（DTST）模块，结合时间推理和预测未来最佳槽初始化。
...
### [267] [Self-Reinforcing Prototype Evolution with Dual-Knowledge Cooperation for Semi-Supervised Lifelong Person Re-Identification](https://arxiv.org/abs/2507.01884)
*Kunlun Xu,Fan Zhuo,Jiangmeng Li,Xu Zou,Jiahuan Zhou*

Main category: cs.CV

TL;DR: 论文提出了一种新型框架SPRED，用于解决半监督终身行人重识别问题，通过动态原型引导的伪标签生成和双知识协同净化，显著提升了未标注数据的利用效率。


<details>
  <summary>Details</summary>
Motivation: 现实场景中标注资源有限，现有方法在未标注数据利用时性能下降严重，因此需要一种能有效利用未标注数据的半监督终身学习方法。

Method: 提出SPRED框架，结合动态原型引导的伪标签生成和双知识协同净化，通过循环设计逐步挖掘可靠的伪标签。

Result: 在Semi-LReID基准测试中，SPRED实现了最先进的性能。

Conclusion: SPRED框架通过自增强循环设计，有效提升了半监督终身行人重识别的性能。
...
### [268] [Reasoning to Edit: Hypothetical Instruction-Based Image Editing with Visual Reasoning](https://arxiv.org/abs/2507.01908)
*Qingdong He,Xueqin Chen,Chaoyi Wang,Yanjie Pan,Xiaobin Hu,Zhenye Gan,Yabiao Wang,Chengjie Wang,Xiangtai Li,Jiangning Zhang*

Main category: cs.CV

TL;DR: 论文提出Reason50K数据集和ReasonBrain框架，用于解决复杂隐式假设指令的图像编辑问题，通过多模态大语言模型和细粒度推理线索提取模块提升编辑能力。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以处理需要深层推理的复杂隐式假设指令，且缺乏相关数据集和细粒度细节提取机制。
...
### [269] [Modality Agnostic, patient-specific digital twins modeling temporally varying digestive motion](https://arxiv.org/abs/2507.01909)
*Jorge Tapias Gomez,Nishant Nadkarni,Lando S. Bosma,Jue Jiang,Ergys D. Subashi,William P. Segars,James M. Balter,Mert R Sabuncu,Neelam Tyagi,Harini Veeraraghavan*

Main category: cs.CV

TL;DR: 提出了一种基于数字孪生（DT）的管道，用于评估变形图像配准（DIR）方法在胃肠道（GI）运动中的准确性，并通过定量指标和剂量映射验证其性能。


<details>
  <summary>Details</summary>
Motivation: 临床实施DIR需要基于体素的空间精度指标，如手动标记点，但这对高度移动的GI器官具有挑战性。

Method: 通过半自动化管道生成21个运动阶段的4D序列，模拟GI运动，并使用6种DIR方法进行评估。

Result: 生成的DT模拟了真实的GI运动，运动幅度与真实患者数据相似，并提供了详细的DIR性能指标和剂量映射验证。

Conclusion: 该管道能够严格测试DIR工具在动态复杂区域的性能，为空间和剂量精度提供了详细验证。
...
### [270] [3D Reconstruction and Information Fusion between Dormant and Canopy Seasons in Commercial Orchards Using Deep Learning and Fast GICP](https://arxiv.org/abs/2507.01912)
*Ranjan Sapkota,Zhichao Meng,Martin Churuvija,Xiaoqiang Du,Zenghong Ma,Manoj Karkee*

Main category: cs.CV

TL;DR: 提出了一种多季节信息融合框架，结合休眠期和生长期的RGB-D图像，通过实例分割、3D重建和模型对齐技术，实现果园自动化管理的全年精准操作。


<details>
  <summary>Details</summary>
Motivation: 果园自动化中，生长期茂密的树叶遮挡了树干和树枝结构，限制了机器视觉系统的能力，而休眠期树木落叶后结构更清晰可见。

Method: 使用YOLOv9-Seg进行实例分割，Kinect Fusion进行3D重建，Fast GICP进行模型对齐，融合休眠期和生长期的RGB-D图像数据。

Result: YOLOv9-Seg的MSE为0.0047，分割mAP@50达0.78；Kinect Fusion的3D重建误差小（树干直径RMSE为5.23 mm）；Fast GICP的配准精度高（最小适应分数0.00197）。

Conclusion: 多季节融合框架显著提升了果园自动化操作的精度，解决了生长期遮挡问题，为修剪和疏果等操作提供了更全面的结构信息。
...
### [271] [IC-Custom: Diverse Image Customization via In-Context Learning](https://arxiv.org/abs/2507.01926)
*Yaowei Li,Xiaoyu Li,Zhaoyang Zhang,Yuxuan Bian,Gan Liu,Xinyuan Li,Jiale Xu,Wenbo Hu,Yating Liu,Lingen Li,Jing Cai,Yuexian Zou,Yancheng He,Ying Shan*
...
Method: 提出IC-Custom框架，结合多模态注意力机制和任务导向的注册令牌，使用高质量数据集。
...
### [272] [evMLP: An Efficient Event-Driven MLP Architecture for Vision](https://arxiv.org/abs/2507.01927)
*Zhentan Zheng*

Main category: cs.CV

TL;DR: 论文提出了一种名为evMLP的新方法，通过事件驱动的局部更新机制，选择性处理图像或特征图中的变化区域，显著提升了计算效率。
...
Method: 提出evMLP模型，结合事件驱动局部更新机制，仅处理帧间变化的图像块。
...
### [273] [CI-VID: A Coherent Interleaved Text-Video Dataset](https://arxiv.org/abs/2507.01938)
*Yiming Ju,Jijin Hu,Zhengxiong Luo,Haoge Deng,hanyu Zhao,Li Du,Chengwei Wu,Donglin Hao,Xinlong Wang,Tengfei Pan*

Main category: cs.CV

TL;DR: 论文介绍了CI-VID数据集，用于支持连贯的多场景视频序列生成，超越了现有的孤立文本-视频对数据集。
...
Method: 提出CI-VID数据集，包含34万样本，每个样本为连贯视频片段序列，附带文本描述。设计了多维度基准验证其有效性。

Result: 实验表明，基于CI-VID训练的模型在生成视频序列时显著提升了准确性和内容一致性。

Conclusion: CI-VID数据集支持故事驱动内容的生成，具有高质量和实用价值，并已公开数据和代码。
...
### [274] [LongAnimation: Long Animation Generation with Dynamic Global-Local Memory](https://arxiv.org/abs/2507.01945)
*Nan Chen,Mengqi Huang,Yihao Meng,Zhendong Mao*

Main category: cs.CV

TL;DR: 论文提出了一种名为LongAnimation的新框架，用于解决长动画着色中的长期颜色一致性问题，结合动态全局-局部范式。


<details>
  <summary>Details</summary>
Motivation: 长动画着色在动画产业中成本高昂，现有研究局限于短期着色，缺乏全局信息导致长期颜色不一致。

Method: 提出LongAnimation框架，包括SketchDiT、动态全局-局部记忆模块（DGLM）和颜色一致性奖励机制。

Result: 实验表明，LongAnimation在短（14帧）和长（平均500帧）动画中均能有效保持颜色一致性。

Conclusion: LongAnimation通过动态全局-局部范式成功解决了长期颜色一致性问题，具有实际应用价值。
...
### [275] [Kwai Keye-VL Technical Report](https://arxiv.org/abs/2507.01949)
*Kwai Keye Team,Biao Yang,Bin Wen,Changyi Liu,Chenglong Chu,Chengru Song,Chongling Rao,Chuan Yi,Da Li,Dunju Zang,Fan Yang,Guorui Zhou,Hao Peng,Haojie Ding,Jiaming Huang,Jiangxia Cao,Jiankang Chen,Jingyun Hua,Jin Ouyang,Kaibing Chen,Kaiyu Jiang,Kaiyu Tang,Kun Gai,Shengnan Zhang,Siyang Mao,Sui Huang,Tianke Zhang,Tingting Gao,Wei Chen,Wei Yuan,Xiangyu Wu,Xiao Hu,Xingyu Lu,Yang Zhou,Yi-Fan Zhang,Yiping Yang,Yulong Chen,Zhenhua Wu,Zhenyu Li,Zhixin Ling,Ziming Li,Dehua Ma,Di Xu,Haixuan Gao,Hang Li,Jiawei Guo,Jing Wang,Lejian Ren,Muhao Wei,Qianqian Wang,Qigen Hu,Shiyao Wang,Tao Yu,Xinchen Luo,Yan Li,Yiming Liang,Yuhang Hu,Zeyi Lu,Zhuoran Yang,Zixing Zhang*
...
Method: 采用四阶段预训练和两阶段后训练，包括五种模式的冷启动数据混合和强化学习。

Result: 在公共视频基准测试中达到SOTA，并在通用图像任务中保持竞争力。

Conclusion: Keye-VL在短视频理解上表现优异，并通过新基准KC-MMBench验证了其优势。
...
### [276] [FreeMorph: Tuning-Free Generalized Image Morphing with Diffusion Model](https://arxiv.org/abs/2507.01953)
*Yukang Cao,Chenyang Si,Jinghao Wang,Ziwei Liu*
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖预训练扩散模型的微调，受限于时间和语义/布局差异，而无需调优的方法因多步去噪过程的非线性和预训练模型的偏差而难以保持高质量。

Method: 1) 提出基于输入图像显式指导的自注意力模块修改的球面插值设计；2) 引入步进式变化趋势，混合输入图像的自注意力模块以实现可控过渡。

Result: FreeMorph在图像变形任务中表现优异，速度提升10~50倍，成为新的最佳方法。
...
### [277] [Rethinking the Illusion of Thinking](https://arxiv.org/abs/2507.01231)
...
### [278] [Beyond Black-Box AI: Interpretable Hybrid Systems for Dementia Care](https://arxiv.org/abs/2507.01282)
...
### [279] [AI Agents and Agentic AI-Navigating a Plethora of Concepts for Future Manufacturing](https://arxiv.org/abs/2507.01376)
...
### [280] [A Fuzzy Approach to the Specification, Verification and Validation of Risk-Based Ethical Decision Making Models](https://arxiv.org/abs/2507.01410)
...
### [281] [Pensieve Grader: An AI-Powered, Ready-to-Use Platform for Effortless Handwritten STEM Grading](https://arxiv.org/abs/2507.01431)
...
### [282] [Using multi-agent architecture to mitigate the risk of LLM hallucinations](https://arxiv.org/abs/2507.01446)
...
### [283] [Agent-as-Tool: A Study on the Hierarchical Decision Making with Reinforcement Learning](https://arxiv.org/abs/2507.01489)
*Yanfei Zhang*
...
### [284] [T3DM: Test-Time Training-Guided Distribution Shift Modelling for Temporal Knowledge Graph Reasoning](https://arxiv.org/abs/2507.01597)
...
### [285] [Agent Ideate: A Framework for Product Idea Generation from Patents Using Agentic AI](https://arxiv.org/abs/2507.01717)
...
### [286] [Joint Matching and Pricing for Crowd-shipping with In-store Customers](https://arxiv.org/abs/2507.01749)
...
### [287] [Refining Gelfond Rationality Principle Towards More Comprehensive Foundational Principles for Answer Set Semantics](https://arxiv.org/abs/2507.01833)
...
### [288] [Rethinking the Illusion of Thinking](https://arxiv.org/abs/2507.01231)
*Iñaki Dellibarda Varela,Pablo Romero-Sorozabal,Eduardo Rocon,Manuel Cebrian*
...
Method: 复制并改进了两项争议性基准测试（Towers of Hanoi和River Crossing），引入逐步提示和协作对话方法。

Result: LRMs在复杂任务（如8个圆盘的Towers of Hanoi）中表现有限，但在可解问题（如100+代理对的River Crossing）中表现优异。

Conclusion: LRMs是离散状态空间中的随机搜索器，未来需要在符号化和长程推理方面进行更精细的研究。
...
### [289] [Beyond Black-Box AI: Interpretable Hybrid Systems for Dementia Care](https://arxiv.org/abs/2507.01282)
*Matthew JY Kang,Wenli Yang,Monica R Roberts,Byeong Ho Kang,Charles B Malpas*

Main category: cs.AI

TL;DR: 论文探讨了大型语言模型（LLMs）在医疗诊断中的局限性，特别是在痴呆症诊断和护理中，并提出了结合统计学习和专家知识的混合方法以提高透明度和实用性。


<details>
  <summary>Details</summary>
Motivation: 尽管LLMs在基准测试中表现优异，但在实际临床环境中未能显著改善诊断效果，因此需要探索其局限性并提出改进方向。

Method: 通过范围综述分析AI在临床环境中的局限性，并提出结合统计学习和专家规则知识的混合方法，如PEIRS和ATHENA-CDS。

Result: 研究发现LLMs缺乏透明度和因果推理能力，而混合方法能更好地融入临床工作流程并提供可解释性。
...
### [290] [AI Agents and Agentic AI-Navigating a Plethora of Concepts for Future Manufacturing](https://arxiv.org/abs/2507.01376)
*Yinwang Ren,Yangyang Liu,Tang Ji,Xun Xu*

Main category: cs.AI

TL;DR: 论文探讨了基于生成式AI（GenAI）的AI代理（如LLM-Agents和MLLM-Agents）及Agentic AI在智能制造中的潜力与挑战。


<details>
  <summary>Details</summary>
Motivation: 随着生成式AI的快速发展，AI代理的能力显著提升，但其在智能制造中的定义、边界和应用尚不明确，需要系统性研究。

Method: 研究通过系统回顾AI和AI代理技术的发展，分析LLM-Agents、MLLM-Agents和Agentic AI的核心概念与技术进展，并探讨其在智能制造中的应用与挑战。

Result: 研究发现这些新兴AI范式在信息处理、环境感知和自主决策方面具有潜力，但实际应用仍需明确边界和解决挑战。

Conclusion: 论文为AI代理技术在智能制造中的集成提供了理论基础，并指出了未来研究方向。
...
### [291] [A Fuzzy Approach to the Specification, Verification and Validation of Risk-Based Ethical Decision Making Models](https://arxiv.org/abs/2507.01410)
*Abeer Dyoub,Francesca A. Lisi*
...
<details>
  <summary>Details</summary>
Motivation: 道德领域的复杂性和模糊性使得评估道德机器的表现缺乏明确标准，因此需要一种形式化方法来描述和验证伦理决策模型。

Method: 使用模糊规则描述伦理决策模型，并通过模糊Petri网进行验证和验证。

Result: 通过医疗领域的案例研究，验证了所提方法的可行性和有效性。
...
### [292] [Pensieve Grader: An AI-Powered, Ready-to-Use Platform for Effortless Handwritten STEM Grading](https://arxiv.org/abs/2507.01431)
*Yoonseok Yang,Minjune Kim,Marlon Rondinelli,Keren Shao*
...
Method: 通过LLMs转录和评估学生作业，提供与评分标准对齐的分数、转录和置信度评分，支持从扫描提交到最终反馈的完整流程。

Result: 在20多所机构的真实课程中部署，评分超过30万份学生作业，平均减少65%的评分时间，高置信度预测与教师评分一致率达95.4%。
...
### [293] [Using multi-agent architecture to mitigate the risk of LLM hallucinations](https://arxiv.org/abs/2507.01446)
*Abd Elrahman Amer,Magdi Amer*

Main category: cs.AI

TL;DR: 提出了一种基于多代理系统和模糊逻辑的方法，用于通过SMS处理客户请求，以减少LLM的幻觉风险。
...
### [294] [Agent-as-Tool: A Study on the Hierarchical Decision Making with Reinforcement Learning](https://arxiv.org/abs/2507.01489)
*Yanfei Zhang*

Main category: cs.AI

TL;DR: 提出了一种名为Agent-as-tool的分层框架，将工具调用过程与推理过程分离，显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有研究在工具调用和推理过程中同时决策，且依赖未经处理的工具结果，增加了模型推理负担。

Method: 采用分层框架，分离工具调用与推理过程，由不同代理处理。

Result: 仅需少量强化学习微调（180样本），在Bamboogle任务中表现优异，精确匹配率63.2%，覆盖精确匹配率75.2%。

Conclusion: Agent-as-tool框架有效减轻了模型负担，提升了任务性能。
...
### [295] [T3DM: Test-Time Training-Guided Distribution Shift Modelling for Temporal Knowledge Graph Reasoning](https://arxiv.org/abs/2507.01597)
*Yuehang Si,Zefan Zeng,Jincai Huang,Qing Cheng*
...
<details>
  <summary>Details</summary>
Motivation: 现有TKG推理方法面临训练与测试样本分布偏移建模不足和负采样质量低的问题。
...
### [296] [Agent Ideate: A Framework for Product Idea Generation from Patents Using Agentic AI](https://arxiv.org/abs/2507.01717)
*Gopichand Kanumolu,Ashok Urlana,Charaka Vinayak Kumar,Bala Mallikarjunarao Garlapati*
...
Result: 代理方法在创意质量、相关性和新颖性上表现优于单独使用LLMs。
...
### [297] [Joint Matching and Pricing for Crowd-shipping with In-store Customers](https://arxiv.org/abs/2507.01749)
*Arash Dehghan,Mucahit Cevik,Merve Bodur,Bissan Ghaddar*
...
<details>
  <summary>Details</summary>
Motivation: 针对城市地区最后一公里配送效率的迫切需求，探索利用零售店顾客作为配送员的众包模式。

Method: 采用马尔可夫决策过程（MDP）模型，结合NeurADP进行订单分配和DDQN进行动态定价，实现多目的地路由和不确定性管理。

Result: 实验显示，NeurADP + DDQN策略比固定定价节省6.7%，比短视基线节省18%；灵活配送延迟和多目的地路由进一步降低成本8%和17%。
...
### [298] [Refining Gelfond Rationality Principle Towards More Comprehensive Foundational Principles for Answer Set Semantics](https://arxiv.org/abs/2507.01833)
*Yi-Dong Shen,Thomas Eiter*

Main category: cs.AI

TL;DR: 论文探讨了非单调逻辑编程中的答案集语义，质疑了传统的最小模型属性、约束单调性和基础性是否应作为通用条件，并提出了基于Gelfond答案集原则的改进方法。
...
Method: 方法包括：1) 分析传统条件的局限性；2) 改进Gelfond答案集原则，引入良好支持性和最小性；3) 扩展良好支持性概念；4) 定义新的答案集语义；5) 评估现有语义；6) 计算复杂性分析。

Result: 结果表明传统条件有时过于严格，改进后的原则能更合理地定义答案集语义，并支持更广泛的预期答案集。

Conclusion: 结论是改进的Gelfond答案集原则为答案集语义提供了更合理的通用标准，同时保持了计算可行性。
...
### [299] [Rethinking the Illusion of Thinking](https://arxiv.org/abs/2507.01231)
*Iñaki Dellibarda Varela,Pablo Romero-Sorozabal,Eduardo Rocon,Manuel Cebrian*
...
Method: 复制并改进原研究中的两个争议性基准（河内塔和河流穿越），引入逐步提示和协作对话方法。

Result: 发现LRMs在河内塔任务中因认知限制而失败，而在河流穿越任务中，失败源于测试配置问题，修正后表现优异。

Conclusion: LRMs是随机、强化学习调整的搜索器，其表现复杂且受限于状态空间理解；未来需通过精细实验推动符号推理进步。
...
### [300] [Beyond Black-Box AI: Interpretable Hybrid Systems for Dementia Care](https://arxiv.org/abs/2507.01282)
*Matthew JY Kang,Wenli Yang,Monica R Roberts,Byeong Ho Kang,Charles B Malpas*

Main category: cs.AI

TL;DR: 大型语言模型（LLM）在医学诊断中潜力巨大，但实际临床贡献有限，尤其在痴呆症诊断和护理中。混合方法结合统计学习和专家知识可提升可解释性，未来需注重解释性AI和人类参与。


<details>
  <summary>Details</summary>
Motivation: 探讨AI在临床环境中的实际贡献限制，特别是在痴呆症诊断和护理领域，以推动更有效的AI辅助医疗决策。

Method: 通过范围综述分析AI在临床实践中的局限性，提出结合统计学习和专家知识的混合方法。

Result: 现有AI模型在透明度和因果推理方面存在不足，混合方法如PEIRS和ATHENA-CDS更符合临床工作流程。

Conclusion: 未来AI决策支持应注重解释性和人类参与，衡量标准需包括临床理解和患者结果，而非仅准确性。
...
### [301] [AI Agents and Agentic AI-Navigating a Plethora of Concepts for Future Manufacturing](https://arxiv.org/abs/2507.01376)
*Yinwang Ren,Yangyang Liu,Tang Ji,Xun Xu*
...
Result: 探讨了这些技术在智能制造中的应用潜力及可能面临的挑战。

Conclusion: 研究为AI代理技术在智能制造中的集成与应用提供了系统性视角，并指出了未来研究方向。
...
### [302] [A Fuzzy Approach to the Specification, Verification and Validation of Risk-Based Ethical Decision Making Models](https://arxiv.org/abs/2507.01410)
*Abeer Dyoub,Francesca A. Lisi*

Main category: cs.AI

TL;DR: 提出了一种基于伦理风险评估的模糊规则模型，用于描述和验证道德决策，并通过医疗领域的案例研究进行说明。
...
Result: 提出了一种可验证的道德决策模型，并通过案例研究展示了其应用。

Conclusion: 该方法为道德机器的性能评估提供了可行的技术路径。
...
### [303] [Pensieve Grader: An AI-Powered, Ready-to-Use Platform for Effortless Handwritten STEM Grading](https://arxiv.org/abs/2507.01431)
*Yoonseok Yang,Minjune Kim,Marlon Rondinelli,Keren Shao*
...
Method: 结合LLMs转录和评估学生作业，提供评分、转录和置信度评级，支持从扫描提交到最终反馈的完整流程。

Result: 在20多所机构中部署，评分超过30万份作业，评分时间减少65%，高置信度预测与教师评分一致率达95.4%。

Conclusion: Pensieve有效提升了评分效率，同时保持了高准确性，适用于多学科STEM课程。
...
### [304] [Using multi-agent architecture to mitigate the risk of LLM hallucinations](https://arxiv.org/abs/2507.01446)
*Abd Elrahman Amer,Magdi Amer*

Main category: cs.AI

TL;DR: 提出了一种基于多代理系统和模糊逻辑的解决方案，以减少LLM在客户服务中的幻觉风险。
...
Method: 开发了一个多代理系统，结合LLM代理和模糊逻辑来处理客户短信请求。
...
### [305] [Agent-as-Tool: A Study on the Hierarchical Decision Making with Reinforcement Learning](https://arxiv.org/abs/2507.01489)
*Yanfei Zhang*

Main category: cs.AI

TL;DR: 提出了一种分层框架Agent-as-tool，将工具调用和推理过程分离，显著提升了模型性能。
...
Conclusion: 分层框架有效减轻模型负担，提升推理能力，性能优于现有方法。
...
### [306] [T3DM: Test-Time Training-Guided Distribution Shift Modelling for Temporal Knowledge Graph Reasoning](https://arxiv.org/abs/2507.01597)
*Yuehang Si,Zefan Zeng,Jincai Huang,Qing Cheng*

Main category: cs.AI

TL;DR: 提出了一种新的时间知识图推理方法T3DM，通过建模分布偏移和对抗训练生成高质量负样本，显著提升了推理性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法对训练与测试样本间的事件分布偏移建模不足，且负样本生成质量低。

Method: 采用Test-Time Training-guided Distribution shift Modelling (T3DM)调整模型分布偏移，并设计基于对抗训练的负采样策略。

Result: 实验表明T3DM在多数情况下优于现有基线方法，结果更优且更稳健。

Conclusion: T3DM通过建模分布偏移和改进负采样，显著提升了时间知识图推理的性能和鲁棒性。
...
### [307] [Agent Ideate: A Framework for Product Idea Generation from Patents Using Agentic AI](https://arxiv.org/abs/2507.01717)
*Gopichand Kanumolu,Ashok Urlana,Charaka Vinayak Kumar,Bala Mallikarjunarao Garlapati*
...
<details>
  <summary>Details</summary>
Motivation: 专利蕴含丰富的技术知识，但访问和解读这些信息仍具挑战性，因此需要一种自动化方法来挖掘创新产品概念。
...
### [308] [Joint Matching and Pricing for Crowd-shipping with In-store Customers](https://arxiv.org/abs/2507.01749)
*Arash Dehghan,Mucahit Cevik,Merve Bodur,Bissan Ghaddar*
...
Result: 实验显示，该策略比固定定价策略节省6.7%成本，比短视基准节省约18%。灵活配送和多目的地路由进一步降低成本8%和17%。
...
### [309] [Refining Gelfond Rationality Principle Towards More Comprehensive Foundational Principles for Answer Set Semantics](https://arxiv.org/abs/2507.01833)
*Yi-Dong Shen,Thomas Eiter*

Main category: cs.AI

TL;DR: 论文探讨了非单调逻辑编程中的答案集语义问题，质疑了现有条件的必要性，并提出了基于Gelfond原则的改进方法。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探讨现有答案集语义的强制性条件是否过于严格，以及如何通过改进Gelfond原则来定义更合理的语义。

Method: 方法包括：1）质疑现有条件的必要性；2）改进Gelfond原则为支持性、默认否定最小性和认知否定最小性；3）扩展支持性概念；4）定义新语义；5）评估现有语义；6）分析计算复杂度。

Result: 结果表明现有条件可能过于严格，改进后的Gelfond原则能更合理地定义答案集语义。

Conclusion: 结论是改进的Gelfond原则为答案集语义提供了更灵活和合理的基础，并可用于评估现有语义。
...
### [310] [A LoD of Gaussians: Unified Training and Rendering for Ultra-Large Scale Reconstruction with External Memory](https://arxiv.org/abs/2507.01110)
...
### [311] [Semiautomatic Simplification](https://arxiv.org/abs/2507.01116)
...
### [312] [Multi-Focus Probes for Context-Preserving Network Exploration and Interaction in Immersive Analytics](https://arxiv.org/abs/2507.01140)
...
### [313] [A LoD of Gaussians: Unified Training and Rendering for Ultra-Large Scale Reconstruction with External Memory](https://arxiv.org/abs/2507.01110)
*Felix Windisch,Lukas Radl,Thomas Köhler,Michael Steiner,Dieter Schmalstieg,Markus Steinberger*
...
<details>
  <summary>Details</summary>
Motivation: 解决高斯溅射技术在扩展到大场景时因分区策略导致的边界伪影、训练复杂性和GPU内存限制问题。

Method: 采用层次化高斯与顺序点树的混合数据结构，结合轻量级缓存和视图调度系统，实现动态流式传输和实时渲染。

Result: 实现了无缝多尺度重建和复杂场景的交互式可视化，从广角航拍到精细地面细节。

Conclusion: 该方法为大规模高斯场景的实时渲染提供了一种高效解决方案。
...
### [314] [Semiautomatic Simplification](https://arxiv.org/abs/2507.01116)
*Gong Li,Benjamin Watson*
...
<details>
  <summary>Details</summary>
Motivation: 现有自动简化技术成熟，但无法区分语义重要区域（如面部和肢体）或满足特定用途（如动画）的简化约束。

Method: 用户可调整简化顺序、重新定位顶点并传播至相邻细节层次，以及调整模型表面的分层分区。

Result: 工具能够更好地保留重要区域，并优化简化模型的质量。

Conclusion: semisimp通过用户干预实现了更灵活的模型简化，适用于需要保留语义细节的场景。
...
### [315] [Multi-Focus Probes for Context-Preserving Network Exploration and Interaction in Immersive Analytics](https://arxiv.org/abs/2507.01140)
*Eric Zimmermann,Stefan Bruckner*

Main category: cs.GR

TL;DR: 提出了一种多焦点探针技术，用于沉浸式环境中管理局部与全局视图的切换，支持用户同时操作多个局部子图视图并保持与全局网络的联系。


<details>
  <summary>Details</summary>
Motivation: 解决用户在沉浸式环境中管理局部与全局视图切换的挑战，提升复杂网络数据的交互与编辑效率。

Method: 采用多焦点探针技术，结合视觉和触觉引导机制，支持用户创建多个局部子图视图并保持与全局网络的链接。

Result: 技术能够有效支持用户在多尺度交互中保持上下文，提升网络数据的编辑效率。

Conclusion: 多焦点探针技术为沉浸式网络数据编辑提供了一种有效的解决方案，平衡了局部与全局视图的需求。
...
### [316] [A LoD of Gaussians: Unified Training and Rendering for Ultra-Large Scale Reconstruction with External Memory](https://arxiv.org/abs/2507.01110)
*Felix Windisch,Lukas Radl,Thomas Köhler,Michael Steiner,Dieter Schmalstieg,Markus Steinberger*
...
<details>
  <summary>Details</summary>
Motivation: 解决现有高斯喷绘技术在扩展到大场景时因分区策略导致的边界伪影、训练复杂性和GPU内存限制问题。

Method: 采用混合数据结构（高斯层次结构与顺序点树）和轻量级缓存与视图调度系统，动态流式传输相关高斯数据。

Result: 实现了无缝多尺度重建和复杂场景的交互式可视化，支持从广角航拍到地面细节的实时渲染。
...
### [317] [Semiautomatic Simplification](https://arxiv.org/abs/2507.01116)
*Gong Li,Benjamin Watson*
...
<details>
  <summary>Details</summary>
Motivation: 现有自动简化技术对语义区域和模型用途（如动画）不敏感。

Method: 用户可干预简化过程，调整简化顺序、顶点位置及模型表面分区。

Result: 工具能够保留重要区域并优化简化模型。
...
### [318] [Multi-Focus Probes for Context-Preserving Network Exploration and Interaction in Immersive Analytics](https://arxiv.org/abs/2507.01140)
*Eric Zimmermann,Stefan Bruckner*

Main category: cs.GR

TL;DR: 提出了一种多焦点探针技术，用于沉浸式环境中同时管理局部和全局网络视图，支持多尺度交互和编辑。


<details>
  <summary>Details</summary>
Motivation: 解决用户在沉浸式环境中切换局部和全局网络视图时的挑战。

Method: 采用多焦点探针技术，结合视觉和触觉引导机制，实现多尺度交互。

Result: 技术能够有效支持网络数据的编辑和多尺度交互。

Conclusion: 该技术提升了沉浸式环境中网络数据编辑的可用性和效率。
...
### [319] [MALIBU Benchmark: Multi-Agent LLM Implicit Bias Uncovered](https://arxiv.org/abs/2507.01019)
...
### [320] [Event-based evaluation of abstractive news summarization](https://arxiv.org/abs/2507.01160)
...
### [321] [Matching and Linking Entries in Historical Swedish Encyclopedias](https://arxiv.org/abs/2507.01170)
...
### [322] [MEGA: xLSTM with Multihead Exponential Gated Fusion for Precise Aspect-based Sentiment Analysis](https://arxiv.org/abs/2507.01213)
...
### [323] [The Medium Is Not the Message: Deconfounding Text Embeddings via Linear Concept Erasure](https://arxiv.org/abs/2507.01234)
...
### [324] [GAIus: Combining Genai with Legal Clauses Retrieval for Knowledge-based Assistant](https://arxiv.org/abs/2507.01259)
...
### [325] [Evaluating Large Language Models for Multimodal Simulated Ophthalmic Decision-Making in Diabetic Retinopathy and Glaucoma Screening](https://arxiv.org/abs/2507.01278)
...
### [326] [Rethinking All Evidence: Enhancing Trustworthy Retrieval-Augmented Generation via Conflict-Driven Summarization](https://arxiv.org/abs/2507.01281)
...
### [327] [Frustratingly Simple Retrieval Improves Challenging, Reasoning-Intensive Benchmarks](https://arxiv.org/abs/2507.01297)
...
### [328] [La RoSA: Enhancing LLM Efficiency via Layerwise Rotated Sparse Activation](https://arxiv.org/abs/2507.01299)
...
### [329] [Symbolic or Numerical? Understanding Physics Problem Solving in Reasoning LLMs](https://arxiv.org/abs/2507.01334)
...
### [330] [LEDOM: An Open and Fundamental Reverse Language Model](https://arxiv.org/abs/2507.01335)
...
### [331] [Skywork-Reward-V2: Scaling Preference Data Curation via Human-AI Synergy](https://arxiv.org/abs/2507.01352)
...
### [332] [Clinical NLP with Attention-Based Deep Learning for Multi-Disease Prediction](https://arxiv.org/abs/2507.01437)
...
### [333] [LogitSpec: Accelerating Retrieval-based Speculative Decoding via Next Next Token Speculation](https://arxiv.org/abs/2507.01449)
...
### [334] [Evaluating the Effectiveness of Direct Preference Optimization for Personalizing German Automatic Text Simplifications for Persons with Intellectual Disabilities](https://arxiv.org/abs/2507.01479)
...
### [335] [Efficient Out-of-Scope Detection in Dialogue Systems via Uncertainty-Driven LLM Routing](https://arxiv.org/abs/2507.01541)
...
### [336] [Is External Information Useful for Stance Detection with LLMs?](https://arxiv.org/abs/2507.01543)
...
### [337] [Emotionally Intelligent Task-oriented Dialogue Systems: Architecture, Representation, and Optimisation](https://arxiv.org/abs/2507.01594)
...
### [338] [Chart Question Answering from Real-World Analytical Narratives](https://arxiv.org/abs/2507.01627)
...
### [339] [Confidence and Stability of Global and Pairwise Scores in NLP Evaluation](https://arxiv.org/abs/2507.01633)
...
### [340] [Adapting Language Models to Indonesian Local Languages: An Empirical Study of Language Transferability on Zero-Shot Settings](https://arxiv.org/abs/2507.01645)
*Rifki Afina Putri*
...
### [341] [AdamMeme: Adaptively Probe the Reasoning Capacity of Multimodal Large Language Models on Harmfulness](https://arxiv.org/abs/2507.01702)
...
### [342] [Stereotype Detection as a Catalyst for Enhanced Bias Detection: A Multi-Task Learning Approach](https://arxiv.org/abs/2507.01715)
...
### [343] [LLMs for Legal Subsumption in German Employment Contracts](https://arxiv.org/abs/2507.01734)
...
### [344] [Data interference: emojis, homoglyphs, and issues of data fidelity in corpora and their results](https://arxiv.org/abs/2507.01764)
...
### [345] [MuRating: A High Quality Data Selecting Approach to Multilingual Large Language Model Pretraining](https://arxiv.org/abs/2507.01785)
...
### [346] [Probing Evaluation Awareness of Language Models](https://arxiv.org/abs/2507.01786)
...
### [347] [How Do Vision-Language Models Process Conflicting Information Across Modalities?](https://arxiv.org/abs/2507.01790)
...
### [348] [The Anatomy of Evidence: An Investigation Into Explainable ICD Coding](https://arxiv.org/abs/2507.01802)
...
### [349] [Evaluating Structured Output Robustness of Small Language Models for Open Attribute-Value Extraction from Clinical Notes](https://arxiv.org/abs/2507.01810)
...
### [350] [Low-Perplexity LLM-Generated Sequences and Where To Find Them](https://arxiv.org/abs/2507.01844)
...
### [351] [Eka-Eval : A Comprehensive Evaluation Framework for Large Language Models in Indian Languages](https://arxiv.org/abs/2507.01853)
...
### [352] [DIY-MKG: An LLM-Based Polyglot Language Learning System](https://arxiv.org/abs/2507.01872)
...
### [353] [MiCoTA: Bridging the Learnability Gap with Intermediate CoT and Teacher Assistants](https://arxiv.org/abs/2507.01887)
...
### [354] [High-Layer Attention Pruning with Rescaling](https://arxiv.org/abs/2507.01900)
...
### [355] [AI4Research: A Survey of Artificial Intelligence for Scientific Research](https://arxiv.org/abs/2507.01903)
...
### [356] [Gradient-Adaptive Policy Optimization: Towards Multi-Objective Alignment of Large Language Models](https://arxiv.org/abs/2507.01915)
...
### [357] [Adaptability of ASR Models on Low-Resource Language: A Comparative Study of Whisper and Wav2Vec-BERT on Bangla](https://arxiv.org/abs/2507.01931)
...
### [358] [NaturalThoughts: Selecting and Distilling Reasoning Traces for General Reasoning Tasks](https://arxiv.org/abs/2507.01921)
...
### [359] [Decision-oriented Text Evaluation](https://arxiv.org/abs/2507.01923)
...
### [360] [The Thin Line Between Comprehension and Persuasion in LLMs](https://arxiv.org/abs/2507.01936)
...
### [361] [MALIBU Benchmark: Multi-Agent LLM Implicit Bias Uncovered](https://arxiv.org/abs/2507.01019)
*Imran Mirza,Cole Huang,Ishwara Vasista,Rohan Patil,Asli Akalin,Sean O'Brien,Kevin Zhu*

Main category: cs.CL

TL;DR: MALIBU是一个用于评估多智能体系统中LLM隐含社会偏见的基准测试，通过场景评估和两阶段评分揭示偏见问题。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统可能强化LLM的隐含偏见，需要公平性和公正性的评估工具。

Method: MALIBU通过场景任务和两阶段评分（独立评分和对比评分）评估LLM输出的偏见。

Result: 研究发现偏见缓解可能偏向边缘化群体，而非真正中立，需更细致的策略。

Conclusion: 需要透明、平衡的评估基准和策略来减少多智能体系统中的偏见。
...
### [362] [Event-based evaluation of abstractive news summarization](https://arxiv.org/abs/2507.01160)
*Huiling You,Samia Touileb,Erik Velldal,Lilja Øvrelid*

Main category: cs.CL

TL;DR: 提出了一种基于事件重叠的摘要质量评估方法，用于生成式语言模型的摘要评价。


<details>
  <summary>Details</summary>
Motivation: 现有摘要评估方法依赖人工摘要作为参考，但忽略了事件信息的覆盖。

Method: 通过计算生成摘要、参考摘要和原文之间的事件重叠来评估摘要质量。

Result: 在挪威数据集上实验，验证了该方法能更深入分析摘要中的事件信息。

Conclusion: 该方法为摘要评估提供了新视角，特别关注事件信息的完整性。
...
### [363] [Matching and Linking Entries in Historical Swedish Encyclopedias](https://arxiv.org/abs/2507.01170)
*Simon Börjesson,Erik Ersmark,Pierre Nugues*

Main category: cs.CL

TL;DR: 本文分析了瑞典百科全书《Nordisk familjebok》第一版和第二版的地理条目变化，发现地理焦点从欧洲转向北美、非洲、亚洲、澳大利亚和北欧，反映了第一次世界大战和新势力崛起的影响。


<details>
  <summary>Details</summary>
Motivation: 研究目的是通过比较《Nordisk familjebok》两个版本的地理条目变化，揭示瑞典在19世纪末至20世纪初的知识变迁和社会影响。

Method: 使用数字化文本，通过语义句子嵌入匹配条目，基于Transformer的分类器提取地理条目，并链接到Wikidata进行分析。

Result: 发现地理焦点从欧洲显著转向其他地区，尤其是北美和北欧，反映了历史事件的影响。

Conclusion: 研究证实了《Nordisk familjebok》作为知识载体的动态性，反映了社会和历史变迁。
...
### [364] [MEGA: xLSTM with Multihead Exponential Gated Fusion for Precise Aspect-based Sentiment Analysis](https://arxiv.org/abs/2507.01213)
*Adamu Lawan,Juhua Pu,Haruna Yunusa,Jawad Muhammad,Muhammad Lawan*

Main category: cs.CL

TL;DR: 提出了一种基于xLSTM与多头指数门控融合（MEGA）的新框架，用于提升方面级情感分析（ABSA）的性能和效率。


<details>
  <summary>Details</summary>
Motivation: 现有ABSA方法在计算效率与高性能间难以平衡，xLSTM的潜力尚未被充分挖掘。

Method: 结合双向mLSTM架构与前向及部分翻转后向（PF-mLSTM）流，并引入多头交叉指数门控融合机制（MECGAF）。
...
### [365] [The Medium Is Not the Message: Deconfounding Text Embeddings via Linear Concept Erasure](https://arxiv.org/abs/2507.01234)
*Yu Fan,Yang Tian,Shauli Ravfogel,Mrinmaya Sachan,Elliott Ash,Alexander Hoyle*
...
Conclusion: 去偏算法有效减少了嵌入中的偏差，且未对嵌入质量产生负面影响。
...
### [366] [GAIus: Combining Genai with Legal Clauses Retrieval for Knowledge-based Assistant](https://arxiv.org/abs/2507.01259)
*Michał Matak,Jarosław A. Chudziak*

Main category: cs.CL

TL;DR: 论文探讨了大型语言模型在处理非英语和非中文国家的法律问题时提供准确答案和引用的能力，提出了gAIus架构，通过检索波兰民法典知识提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 研究动机是解决LLM在非主流语言法律任务中的表现问题，并提升其可解释性和准确性。

Method: 提出gAIus架构，采用更可解释、人性化的检索机制，基于波兰民法典构建数据集进行评测。

Result: gAIus显著提升模型性能，使gpt-3.5-turbo-0125提升419%，超越gpt-4o，并将gpt-4o-mini得分从31%提高到86%。

Conclusion: 论文展示了未来研究方向及潜在应用，强调了gAIus在法律信息检索中的价值。
...
### [367] [Evaluating Large Language Models for Multimodal Simulated Ophthalmic Decision-Making in Diabetic Retinopathy and Glaucoma Screening](https://arxiv.org/abs/2507.01278)
*Cindy Lie Tabuse,David Restepo,Carolina Gracitelli,Fernando Korn Malerbi,Caio Regatieri,Luis Filipe Nakayama*

Main category: cs.CL

TL;DR: GPT-4在眼科临床决策模拟中表现中等，适用于简单任务但不适合复杂临床使用。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型（LLM）在眼科临床决策中的潜在应用，尤其是糖尿病视网膜病变（DR）和青光眼筛查。

Method: 使用300张标注的视网膜眼底照片，通过结构化提示描述图像，评估GPT-4的性能，包括添加真实或合成临床数据的影响。

Result: GPT-4在DR筛查中表现中等（准确率82.3%），但在青光眼筛查中表现较差（准确率约78%）。临床数据对结果无显著影响。

Conclusion: GPT-4可模拟基础眼科决策，但缺乏复杂任务的精确性，适用于教育或文档辅助，而非临床使用。
...
### [368] [Rethinking All Evidence: Enhancing Trustworthy Retrieval-Augmented Generation via Conflict-Driven Summarization](https://arxiv.org/abs/2507.01281)
*Juan Chen,Baolong Bi,Wei Zhang,Jingyan Sui,Xiaofei Zhu,Yuanzhuo Wang,Lingrui Mei,Shenghua Liu*

Main category: cs.CL

TL;DR: CARE-RAG框架通过冲突驱动总结和证据优化，提升RAG系统的生成可靠性。


<details>
  <summary>Details</summary>
Motivation: 解决RAG系统中因知识冲突（内部不一致或噪声检索内容）导致的生成不可靠问题。

Method: 提出CARE-RAG框架，包括参数感知证据生成、上下文感知证据优化、冲突驱动总结和QA修复步骤。

Result: 在噪声或冲突证据场景下，CARE-RAG表现优于现有RAG基线。

Conclusion: CARE-RAG通过多源证据的可靠合成，显著提升了RAG系统的可信度。
...
### [369] [Frustratingly Simple Retrieval Improves Challenging, Reasoning-Intensive Benchmarks](https://arxiv.org/abs/2507.01297)
*Xinxi Lyu,Michael Duan,Rulin Shao,Pang Wei Koh,Sewon Min*
...
<details>
  <summary>Details</summary>
Motivation: 现有RAG方法在推理密集型任务中表现有限，主要原因是缺乏与预训练数据广度对齐的高质量数据存储。

Method: 引入CompactDS，通过过滤低质量网页内容并结合内存近似最近邻（ANN）和磁盘精确搜索，实现高效检索。

Result: 在多个基准测试（MMLU、MMLU Pro等）中，CompactDS显著提升了RAG的准确性，相对增益达10%-33%。

Conclusion: CompactDS证明了高质量、多样化数据存储对RAG的重要性，其性能优于复杂代理系统，且简单易用。
...
### [370] [La RoSA: Enhancing LLM Efficiency via Layerwise Rotated Sparse Activation](https://arxiv.org/abs/2507.01299)
*Kai Liu,Bowen Xu,Shaoyu Wu,Xin Chen,Hao Zhou,Yongliang Tao,Lulu Hu*
...
Result: 在LLaMA2-7B上，40%稀疏性下，LaRoSA仅带来0.17困惑度差距，推理速度提升1.30倍，零样本任务准确率差距缩小至0.54%。

Conclusion: LaRoSA在不同规模和类型的LLM中均表现优异，实现了高效且稳定的推理加速。
...
### [371] [Symbolic or Numerical? Understanding Physics Problem Solving in Reasoning LLMs](https://arxiv.org/abs/2507.01334)
*Nifu Dan,Yujun Cai,Yiwei Wang*

Main category: cs.CL

TL;DR: 研究探讨了高级指令调优推理模型（如Deepseek-R1）在解决复杂物理问题上的表现，展示了其在准确性和推理模式上的优势，并指出少样本提示策略可进一步提升性能。
...
Method: 使用Deepseek-R1等推理模型，在SciBench基准测试中评估其表现，并采用少样本提示策略。

Result: 模型在复杂物理问题上达到最先进准确性，并生成独特的符号推导推理模式。少样本提示策略进一步提升了性能。

Conclusion: 推理模型在物理问题解决中表现出色，少样本提示策略为其性能提升提供了潜力。
...
### [372] [LEDOM: An Open and Fundamental Reverse Language Model](https://arxiv.org/abs/2507.01335)
*Xunjian Yin,Sitao Cheng,Yuxi Xie,Xinyu Hu,Li Lin,Xinyi Wang,Liangming Pan,William Yang Wang,Xiaojun Wan*

Main category: cs.CL

TL;DR: LEDOM是首个纯逆向语言模型，通过反向时间顺序处理序列，展示了在通用任务中的潜力，并提出了反向奖励应用，显著提升数学推理任务性能。


<details>
  <summary>Details</summary>
Motivation: 探索逆向语言模型作为基础模型的潜力，并研究其在任务中的独特应用。

Method: 训练了参数为2B和7B的LEDOM模型，通过反向时间顺序处理序列，并引入反向奖励应用优化生成质量。

Result: LEDOM展示了独特的逆向推理能力，反向奖励应用显著提升了数学推理任务的性能。

Conclusion: LEDOM具有广泛的应用潜力，未来将公开模型、代码和数据以促进研究。
...
### [373] [Skywork-Reward-V2: Scaling Preference Data Curation via Human-AI Synergy](https://arxiv.org/abs/2507.01352)
*Chris Yuhao Liu,Liang Zeng,Yuzhen Xiao,Jujie He,Jiacai Liu,Chaojie Wang,Rui Yan,Wei Shen,Fuxiang Zhang,Jiacheng Xu,Yang Liu,Yahui Zhou*

Main category: cs.CL

TL;DR: 论文提出了一种大规模偏好数据集SynPref-40M和奖励模型Skywork-Reward-V2，通过人机协同的两阶段流程提升数据质量，模型在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有奖励模型在评估基准上表现不佳，主要原因是偏好数据集范围狭窄、质量不足。

Method: 设计人机协同的两阶段数据标注流程，利用人类标注质量与AI扩展性，训练了8个不同规模的奖励模型。
...
### [374] [Clinical NLP with Attention-Based Deep Learning for Multi-Disease Prediction](https://arxiv.org/abs/2507.01437)
*Ting Xu,Xiaoxiao Deng,Xiandong Meng,Haifeng Yang,Yan Wu*
...
### [375] [LogitSpec: Accelerating Retrieval-based Speculative Decoding via Next Next Token Speculation](https://arxiv.org/abs/2507.01449)
*Tianyu Liu,Qitan Lv,Hao Li,Xing Gao,Xiao Sun*

Main category: cs.CL

TL;DR: LogitSpec是一种无需训练的检索式推测解码方法，通过利用最后一个token的logit预测下一个和下一个token，显著提高了推测解码的准确性和效率。


<details>
  <summary>Details</summary>
Motivation: 现有检索式推测解码方法难以找到匹配且准确的推测token，限制了其加速效果。

Method: LogitSpec分两步生成推测token：1) 利用最后一个token的logit预测下一个和下一个token；2) 检索相关参考token。

Result: 实验表明，LogitSpec在多种文本生成任务中实现了最高2.61倍的加速和3.28的平均接受token数。

Conclusion: LogitSpec是一种高效、即插即用的推测解码方法，显著提升了LLM推理速度。
...
### [376] [Evaluating the Effectiveness of Direct Preference Optimization for Personalizing German Automatic Text Simplifications for Persons with Intellectual Disabilities](https://arxiv.org/abs/2507.01479)
*Yingqiang Gao,Kaede Johnson,David Froehlich,Luisa Carrer,Sarah Ebling*
...
<details>
  <summary>Details</summary>
Motivation: 现有的LLM-based ATS系统缺乏对目标群体个性化需求的考虑，尤其是在训练过程中未纳入偏好反馈。

Method: 扩展了标准监督微调（SFT）方法，采用DPO技术进行后训练，并结合人类反馈优化模型。

Result: 研究强调了目标群体参与设计个性化AI解决方案的重要性，并展示了DPO在提升ATS个性化方面的有效性。

Conclusion: 这项工作为开发更符合目标群体需求的个性化AI辅助系统提供了方向，强调了目标群体直接参与的价值。
...
### [377] [Efficient Out-of-Scope Detection in Dialogue Systems via Uncertainty-Driven LLM Routing](https://arxiv.org/abs/2507.01541)
*Álvaro Zaera,Diana Nicoleta Popa,Ivan Sekulic,Paolo Rosso*
...
<details>
  <summary>Details</summary>
Motivation: 任务导向对话系统（TODS）需要处理未见过或模糊的查询，OOS检测是确保其鲁棒性的关键挑战。

Method: 1. 对现有TODS中的意图分类器输出进行不确定性估计；2. 对高不确定性实例触发微调的LLM进行最终决策。

Result: 在关键OOS检测基准测试中取得了最优性能，包括从实际部署的TODS中获取的真实OOS数据。
...
### [378] [Is External Information Useful for Stance Detection with LLMs?](https://arxiv.org/abs/2507.01543)
*Quang Minh Nguyen,Taegyoon Kim*
...
<details>
  <summary>Details</summary>
Motivation: 探讨外部信息是否对LLMs在立场检测任务中有帮助，尽管此前研究表明其对BERT类模型有益。

Method: 系统评估了8种LLMs在3个数据集和12个目标上的表现，分析外部信息的影响。

Result: 外部信息通常降低性能，LLMs倾向于与提供的信息立场对齐而非真实立场。

Conclusion: 研究揭示了LLMs在立场检测中的信息偏见风险，与BERT类模型的结果形成对比。
...
### [379] [Emotionally Intelligent Task-oriented Dialogue Systems: Architecture, Representation, and Optimisation](https://arxiv.org/abs/2507.01594)
*Shutong Feng,Hsien-chin Lin,Nurul Lubis,Carel van Niekerk,Michael Heck,Benjamin Ruppik,Renato Vukovic,Milica Gašić*
...
Method: 提出LUSTER系统，结合LLM能力和结构化奖励模型，通过短长期奖励（用户情感和任务成功）优化系统。

Result: 实验表明，结合LLM与结构化奖励模型能提升系统的韧性和情感响应能力。

Conclusion: LUSTER为下一代对话系统提供了一种实用路径，结合了任务成功与情感智能。
...
### [380] [Chart Question Answering from Real-World Analytical Narratives](https://arxiv.org/abs/2507.01627)
*Maeve Hutchinson,Radu Jianu,Aidan Slingsby,Jo Wood,Pranava Madhyastha*

Main category: cs.CL

TL;DR: 新数据集用于图表问答（CQA），基于可视化笔记本构建，包含真实世界的多视图图表和自然语言问题，反映实际推理流程。GPT-4.1在该数据集上准确率为69.3%，显示其挑战性。


<details>
  <summary>Details</summary>
Motivation: 现有CQA基准缺乏真实性和多样性，新数据集旨在填补这一空白，提供更贴近实际应用场景的测试环境。

Method: 从可视化笔记本中构建数据集，包含多视图图表和自然语言问题，模拟真实推理流程。

Result: 在测试中，GPT-4.1的准确率为69.3%，表明现有模型在该数据集上表现仍有提升空间。

Conclusion: 新数据集为CQA任务提供了更具挑战性的基准，揭示了现有模型的局限性，并推动未来研究改进。
...
### [381] [Confidence and Stability of Global and Pairwise Scores in NLP Evaluation](https://arxiv.org/abs/2507.01633)
*Georgii Levtsov,Dmitry Ustalov*
...
<details>
  <summary>Details</summary>
Motivation: 随着指令调优神经语言模型的发展，NLP评估逐渐从全局评分转向成对比较排行榜，本文旨在比较这两种方法的优劣。

Method: 通过合成和真实数据集的计算实验，使用标准全局指标和Bradley-Terry模型进行成对比较。

Result: 全局评分提供更可靠的总体排名，但可能低估某些模型；成对比较能识别低分模型中的强者，但收敛较慢。

Conclusion: 选择评估策略需权衡可靠性和细节识别能力，全局评分适合总体排名，成对比较适合复杂任务。
...
### [382] [Adapting Language Models to Indonesian Local Languages: An Empirical Study of Language Transferability on Zero-Shot Settings](https://arxiv.org/abs/2507.01645)
*Rifki Afina Putri*

Main category: cs.CL

TL;DR: 研究了预训练语言模型在低资源印尼本地语言情感分析任务中的迁移能力，评估了零样本和基于适配器的迁移方法，发现MAD-X显著提升了性能。
...
Method: 使用印尼单语BERT、多语言模型（mBERT、XLM-R）和MAD-X适配器方法，将目标语言分为三类（已见、部分已见、未见）进行评估。

Result: 多语言模型在已见语言上表现最佳，MAD-X显著提升性能；语言模型的先前接触是迁移成功的关键因素。

Conclusion: MAD-X适配器方法有效提升低资源语言性能，语言模型的先前接触是迁移成功的主要因素。
...
### [383] [AdamMeme: Adaptively Probe the Reasoning Capacity of Multimodal Large Language Models on Harmfulness](https://arxiv.org/abs/2507.01702)
*Zixin Chen,Hongzhan Lin,Kaixin Li,Ziyang Luo,Zhen Ye,Guang Chen,Zhiyong Huang,Jing Ma*

Main category: cs.CL

TL;DR: 提出了一种名为AdamMeme的灵活、基于代理的评估框架，用于动态评估多模态大语言模型（mLLMs）对有害模因的理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于静态数据集的评估方法无法适应模因的动态演变，需要一种更灵活、全面的评估框架。

Method: 通过多代理协作，AdamMeme动态更新模因数据，挑战mLLMs的推理能力，揭示其局限性。

Result: 实验表明，AdamMeme能系统性地揭示不同mLLMs的性能差异，并提供细粒度的模型弱点分析。

Conclusion: AdamMeme为评估mLLMs对有害模因的理解提供了一种动态、全面的解决方案。
...
### [384] [Stereotype Detection as a Catalyst for Enhanced Bias Detection: A Multi-Task Learning Approach](https://arxiv.org/abs/2507.01715)
*Aditya Tomar,Rudra Murthy,Pushpak Bhattacharyya*
...
Method: 引入StereoBias数据集，比较编码器模型和基于QLoRA微调的解码器模型，并联合训练偏见与刻板印象检测任务。

Result: 联合训练显著提升偏见检测性能，且解码器模型表现竞争力强。多任务实验证实改进源于偏见与刻板印象的关联。

Conclusion: 利用刻板印象信息可构建更公平有效的AI系统，联合学习是关键。
...
### [385] [LLMs for Legal Subsumption in German Employment Contracts](https://arxiv.org/abs/2507.01734)
*Oliver Wardas,Florian Matthes*

Main category: cs.CL

TL;DR: 论文探讨了NLP在法律文本中的应用，通过LLM和上下文学习评估德国雇佣合同条款的合法性，发现法律背景信息能提升模型性能，但仍不及人类律师。
...
Method: 与法律专家合作扩展数据集，使用LLM和上下文学习评估合同条款的合法性，比较不同法律背景（无背景、全文法律文本、摘要版指南）对模型性能的影响。

Result: 全文法律文本对性能有中等提升，而摘要版指南显著提高了无效条款的召回率和加权F1分数（达80%），但LLM性能仍远低于人类律师。

Conclusion: LLM有潜力辅助律师进行合同合法性审查，但现有方法仍有局限性，需进一步改进。
...
### [386] [Data interference: emojis, homoglyphs, and issues of data fidelity in corpora and their results](https://arxiv.org/abs/2507.01764)
*Matteo Di Cristofaro*

Main category: cs.CL

TL;DR: 研究探讨了分词对语言数据表示和分析结果的影响，强调了预处理表情符号和同形异义词的必要性，并提出了确保数字文本在语料库中准确表示的方法。


<details>
  <summary>Details</summary>
Motivation: 分词是语料库语言学的基础步骤，但表情符号和同形异义词的分词差异会影响数据表示和分析的可靠性。

Method: 研究分析了分词差异对语言数据的影响，并提出了预处理表情符号和同形异义词的方法。

Result: 研究发现，预处理这些元素能提高语料库对源数据的忠实度，支持可靠的语料分析。

Conclusion: 研究强调了理解语言和技术细节对提高语料分析准确性的重要性，对定量和定性研究均有重要意义。
...
### [387] [MuRating: A High Quality Data Selecting Approach to Multilingual Large Language Model Pretraining](https://arxiv.org/abs/2507.01785)
*Zhixun Chen,Ping Guo,Wenhan Han,Yifan Zhang,Binbin Liu,Haobin Lin,Fengze Liu,Yan Zhao,Bingni Zhang,Taifeng Wang,Yin Zheng,Meng Fang*

Main category: cs.CL

TL;DR: MuRating是一个可扩展的多语言数据质量评估框架，通过将英语数据质量信号转移到17种目标语言，提升了大型语言模型的性能。
...
Conclusion: MuRating为多语言数据质量评估提供了有效方法，并指出了未来在翻译保真度和内容多样性方面的改进方向。
...
### [388] [Probing Evaluation Awareness of Language Models](https://arxiv.org/abs/2507.01786)
*Jord Nguyen,Khiem Hoang,Carlo Leonardo Attubato,Felix Hofstätter*

Main category: cs.CL

TL;DR: 研究发现语言模型能区分测试与部署阶段（评估意识），线性探针可分离真实评估与部署提示，现有安全评估被模型视为不真实，需确保评估可信度。


<details>
  <summary>Details</summary>
Motivation: 评估意识可能影响AI治理框架和行业承诺的可靠性，需研究模型如何区分测试与部署。

Method: 使用线性探针分析Llama-3.3-70B-Instruct模型，分离评估与部署提示。

Result: 线性探针能区分评估与部署提示，现有安全评估被模型视为不真实。

Conclusion: 需确保评估可信度，模型内部信息可支持黑盒安全审计，尤其是未来更具欺骗能力的模型。
...
### [389] [How Do Vision-Language Models Process Conflicting Information Across Modalities?](https://arxiv.org/abs/2507.01790)
*Tianze Hua,Tian Yun,Ellie Pavlick*
...
Method: 通过向视觉-语言模型提供不一致输入（如图像与标题不符），观察模型对不同模态的偏好及其内部机制。

Result: 模型倾向于优先处理某一模态，且内部注意力机制可调整模态偏好；发现模态无关的“路由头”可优化性能。
...
### [390] [The Anatomy of Evidence: An Investigation Into Explainable ICD Coding](https://arxiv.org/abs/2507.01802)
*Katharina Beckh,Elisa Studeny,Sujan Sai Gannamaneni,Dario Antweiler,Stefan Rüping*
...
Method: 深入分析MDACE数据集，评估现有可解释医疗编码系统的合理性，提出匹配度量。
...
### [391] [Evaluating Structured Output Robustness of Small Language Models for Open Attribute-Value Extraction from Clinical Notes](https://arxiv.org/abs/2507.01810)
*Nikita Neveditsin,Pawan Lingras,Vijay Mago*

Main category: cs.CL

TL;DR: 比较分析小语言模型生成的临床笔记结构化输出的可解析性，JSON表现最佳。


<details>
  <summary>Details</summary>
Motivation: 评估不同序列化格式（JSON、YAML、XML）在临床笔记属性值提取中的可解析性，为隐私敏感场景提供实用指导。

Method: 比较三种格式的解析性，分析目标提示和大模型的影响，识别常见错误模式。

Result: JSON解析性最高，结构鲁棒性随提示和大模型提升，但对长文档和特定笔记类型下降。

Conclusion: JSON是首选格式，提示设计和模型选择对解析性有显著影响。
...
### [392] [Low-Perplexity LLM-Generated Sequences and Where To Find Them](https://arxiv.org/abs/2507.01844)
*Arthur Wuhrmann,Anastasiia Kucherenko,Andrei Kucharavy*
...
Method: 通过分析低困惑度序列（模型生成的高概率文本片段），开发了一种可靠的提取和溯源方法。

Result: 研究发现大量低困惑度序列无法追溯到训练数据，而匹配的部分则揭示了逐字回忆的范围和性质。
...
### [393] [Eka-Eval : A Comprehensive Evaluation Framework for Large Language Models in Indian Languages](https://arxiv.org/abs/2507.01853)
*Samridhi Raj Sinha,Rajvee Sheth,Abhishek Upperwal,Mayank Singh*

Main category: cs.CL

TL;DR: EKA-EVAL是一个统一且生产就绪的评估框架，整合了35个基准测试（包括10个印度特定数据集），支持分布式推理、量化和多GPU使用，降低了多语言基准测试的门槛。
...
Method: EKA-EVAL整合了35个基准测试，涵盖推理、数学、工具使用、长上下文理解和阅读理解等类别，支持分布式推理、量化和多GPU使用。

Result: EKA-EVAL是首个端到端、可扩展的评估套件，适用于全球和印度LLMs，显著降低了多语言基准测试的门槛。

Conclusion: EKA-EVAL是一个开源框架，旨在建立一个强大的多语言LLM评估生态系统，未来计划扩展到100多个基准测试。
...
### [394] [DIY-MKG: An LLM-Based Polyglot Language Learning System](https://arxiv.org/abs/2507.01872)
*Kenan Tang,Yanhong Li,Yao Qin*

Main category: cs.CL

TL;DR: DIY-MKG是一个开源系统，支持多语言学习，通过构建个性化词汇知识图谱和动态生成测验，解决现有语言学习工具的局限性。
...
Conclusion: DIY-MKG是一个有效的多语言学习工具，具有高可靠性和用户参与度。
...
### [395] [MiCoTA: Bridging the Learnability Gap with Intermediate CoT and Teacher Assistants](https://arxiv.org/abs/2507.01887)
*Dongyi Ding,Tiannan Wang,Chenghao Zhu,Meiling Tao,Yuchen Eleanor Jiang,Wangchunshu Zhou*

Main category: cs.CL

TL;DR: MiCoTA框架通过中间模型和中等长度推理序列，提升小语言模型的长链推理能力。


<details>
  <summary>Details</summary>
Motivation: 解决小语言模型（SLMs）因容量有限而难以学习长链推理的问题（SLMs Learnability Gap）。

Method: 引入MiCoTA框架，利用中间模型作为教师助手和中等长度推理序列，弥合容量和推理长度差距。

Result: 实验表明，MiCoTA显著提升SLMs的推理性能，如Qwen2.5-7B和3B模型在多个基准测试中平均得分提高3.47和3.93。

Conclusion: MiCoTA通过生成更符合SLM分布的数据，为未来SLMs的长链推理数据蒸馏研究提供了新思路。
...
### [396] [High-Layer Attention Pruning with Rescaling](https://arxiv.org/abs/2507.01900)
*Songtao Liu,Peng Liu*
...
Method: 提出了一种策略性剪枝高层注意力头的方法，并引入自适应重缩放参数以校准剪枝后的表示尺度。
...
### [397] [AI4Research: A Survey of Artificial Intelligence for Scientific Research](https://arxiv.org/abs/2507.01903)
*Qiguang Chen,Mingda Yang,Libo Qin,Jinhao Liu,Zheng Yan,Jiannan Guan,Dengyun Peng,Yiyan Ji,Hanjing Li,Mengkang Hu,Yimeng Zhang,Yihao Liang,Yuhang Zhou,Jiaqi Wang,Zhi Chen,Wanxiang Che*

Main category: cs.CL

TL;DR: 本文综述了AI在科研中的应用（AI4Research），提出了系统分类法、指出了研究空白与未来方向，并整理了丰富的资源。


<details>
  <summary>Details</summary>
Motivation: AI（尤其是大语言模型）在科研中的潜力未被充分探索，缺乏全面综述阻碍了该领域的发展。

Method: 提出系统分类法，分析五大主流任务；识别研究空白与未来方向；整理多学科应用、数据与工具资源。

Result: 提供了AI4Research的全面视角，包括分类、未来方向及资源库。

Conclusion: 本文填补了AI4Research综述的空白，为研究社区提供了资源与创新方向。
...
### [398] [Gradient-Adaptive Policy Optimization: Towards Multi-Objective Alignment of Large Language Models](https://arxiv.org/abs/2507.01915)
*Chengao Li,Hanyu Zhang,Yunkun Xu,Hongyan Xue,Xiang Ao,Qing He*

Main category: cs.CL

TL;DR: GAPO是一种新的多目标优化方法，用于对齐大型语言模型与多样化的人类偏好，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 对齐大型语言模型与多样化且可能冲突的人类偏好是一个重要挑战。

Method: 提出GAPO和P-GAPO，采用多梯度下降和自适应梯度缩放来平衡目标间的权衡。

Result: GAPO在Mistral-7B上表现优于现有方法，在帮助性和无害性上均取得更好结果。

Conclusion: GAPO能有效解决多目标对齐问题，并收敛到帕累托最优解。
...
### [399] [Adaptability of ASR Models on Low-Resource Language: A Comparative Study of Whisper and Wav2Vec-BERT on Bangla](https://arxiv.org/abs/2507.01931)
*Md Sazzadul Islam Ridoy,Sumi Akter,Md. Aminur Rahman*
...
Method: 使用Mozilla Common Voice-17和OpenSLR数据集，通过微调和超参数优化评估Whisper和Wav2Vec-BERT的WER、CER等指标。
...
### [400] [NaturalThoughts: Selecting and Distilling Reasoning Traces for General Reasoning Tasks](https://arxiv.org/abs/2507.01921)
*Yang Li,Youssef Emad,Karthik Padthe,Jack Lanchantin,Weizhe Yuan,Thao Nguyen,Jason Weston,Shang-Wen Li,Dong Wang,Ilia Kulikov,Xian Li*

Main category: cs.CL

TL;DR: 通过系统研究教师模型的推理示范对提升学生模型推理能力的影响，发现选择多样化推理策略的困难样本更有效。


<details>
  <summary>Details</summary>
Motivation: 研究教师模型推理示范的类型如何影响学生模型的推理能力提升。

Method: 从教师模型中精选高质量推理轨迹（NaturalThoughts），分析样本效率和可扩展性，并比较随机采样与选择困难样本的效果。

Result: NaturalThoughts在Llama和Qwen模型上表现优于现有数据集，尤其在STEM推理任务中。

Conclusion: 选择多样化推理策略的困难样本能更高效地提升学生模型的推理能力。
...
### [401] [Decision-oriented Text Evaluation](https://arxiv.org/abs/2507.01923)
*Yu-Shiang Huang,Chuan-Ju Wang,Chung-Chi Chen*

Main category: cs.CL

TL;DR: 论文提出了一种基于决策效果的NLG评估框架，通过测量生成文本对人类和LLM决策的影响，发现传统评估指标与实际决策效果相关性较弱。
...
Method: 提出决策导向的评估框架，通过市场摘要文本（如客观晨报和主观收盘分析）测试人类和LLM代理的决策质量。
...
### [402] [The Thin Line Between Comprehension and Persuasion in LLMs](https://arxiv.org/abs/2507.01936)
*Adrian de Wynter,Tangming Yuan*
...
<details>
  <summary>Details</summary>
Motivation: 评估LLMs在辩论中的表现及其对对话结构的理解，以揭示其作为评估工具的局限性。

Method: 通过测试LLMs的辩论能力和对对话结构的理解，分析其表现。

Result: LLMs能进行有说服力的辩论，但无法理解深层对话结构；人类对AI参与更警惕。

Conclusion: LLMs的辩论能力不依赖对内容的理解，实用效果优先于语境建模。
...
### [403] [MALIBU Benchmark: Multi-Agent LLM Implicit Bias Uncovered](https://arxiv.org/abs/2507.01019)
*Imran Mirza,Cole Huang,Ishwara Vasista,Rohan Patil,Asli Akalin,Sean O'Brien,Kevin Zhu*

Main category: cs.CL

TL;DR: MALIBU是一个评估多智能体系统中LLM隐含社会偏见的基准，通过场景评估和两阶段评分揭示偏见问题。


<details>
  <summary>Details</summary>
Motivation: 多智能体系统可能强化LLM的隐含偏见，需公平性评估工具。

Method: MALIBU通过场景任务和两阶段LLM评分（单响应评分和成对比较）评估偏见。

Result: 研究发现偏见缓解可能偏向边缘化群体，需更平衡的策略。

Conclusion: 需透明评估基准和细致公平策略以减少多智能体系统中的偏见。
...
### [404] [Event-based evaluation of abstractive news summarization](https://arxiv.org/abs/2507.01160)
*Huiling You,Samia Touileb,Erik Velldal,Lilja Øvrelid*
...
Method: 提出计算生成摘要、参考摘要和原文之间的事件重叠，使用挪威语数据集进行实验。

Result: 方法能更深入分析摘要中的事件信息。

Conclusion: 事件重叠方法为摘要评估提供了新视角。
...
### [405] [Matching and Linking Entries in Historical Swedish Encyclopedias](https://arxiv.org/abs/2507.01170)
*Simon Börjesson,Erik Ersmark,Pierre Nugues*
...
<details>
  <summary>Details</summary>
Motivation: 研究目的是通过数字化版本分析百科全书条目内容的变化，反映瑞典社会在19世纪末至20世纪初的智力变迁。

Method: 使用语义句子嵌入匹配条目，基于Transformer分类器提取地理条目，并与Wikidata链接，以识别地理趋势。

Result: 发现从第一版到第二版，地理焦点从欧洲显著转向北美、非洲、亚洲、澳大利亚和北欧。

Conclusion: 地理趋势的变化证实了第一次世界大战和新势力崛起的影响，研究方法和数据已开源。
...
### [406] [MEGA: xLSTM with Multihead Exponential Gated Fusion for Precise Aspect-based Sentiment Analysis](https://arxiv.org/abs/2507.01213)
*Adamu Lawan,Juhua Pu,Haruna Yunusa,Jawad Muhammad,Muhammad Lawan*

Main category: cs.CL

TL;DR: 提出了一种基于xLSTM和MEGA的新框架，用于解决ABSA任务中计算效率与性能平衡的问题，实验证明其优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有ABSA方法在计算效率与性能之间难以平衡，xLSTM的潜力在ABSA中尚未被充分挖掘。
...
### [407] [The Medium Is Not the Message: Deconfounding Text Embeddings via Linear Concept Erasure](https://arxiv.org/abs/2507.01234)
*Yu Fan,Yang Tian,Shauli Ravfogel,Mrinmaya Sachan,Elliott Ash,Alexander Hoyle*
...
<details>
  <summary>Details</summary>
Motivation: 文本序列的嵌入相似性度量可能受到无关属性（如来源或语言）的干扰，影响跨语料库应用的效果。

Method: 使用去偏算法从编码器表示中移除混杂因素信息。

Result: 在所有评估的嵌入变体和任务中，文档相似性和聚类指标均显著提升，且不影响分布外基准性能。

Conclusion: 去偏算法有效减少了嵌入偏差，且不影响嵌入的其他性能。
...
### [408] [GAIus: Combining Genai with Legal Clauses Retrieval for Knowledge-based Assistant](https://arxiv.org/abs/2507.01259)
*Michał Matak,Jarosław A. Chudziak*

Main category: cs.CL

TL;DR: 论文探讨了大型语言模型在处理非英语和非中文国家的法律问题时提供答案和引用的能力，提出了名为gAIus的认知LLM架构，其回答基于波兰民法典。提出的检索机制更易解释且效果优于嵌入方法，显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 研究动机是解决大型语言模型在处理非主流语言法律信息时的局限性，尤其是如何提供准确且可解释的法律引用。

Method: 方法包括分析法律信息检索历史，区分判例法和成文法，提出基于波兰民法典的gAIus架构，并设计了一种更易解释的检索机制。

Result: 实验结果显示，gAIus显著提升了模型性能，将gpt-3.5-turbo-0125提升了419%，并超越了gpt-4o的表现。

Conclusion: 结论指出gAIus架构在提升法律任务性能方面具有潜力，并展望了未来研究方向和应用前景。
...
### [409] [Evaluating Large Language Models for Multimodal Simulated Ophthalmic Decision-Making in Diabetic Retinopathy and Glaucoma Screening](https://arxiv.org/abs/2507.01278)
*Cindy Lie Tabuse,David Restepo,Carolina Gracitelli,Fernando Korn Malerbi,Caio Regatieri,Luis Filipe Nakayama*

Main category: cs.CL

TL;DR: GPT-4在眼科中模拟临床决策的能力评估，结果显示其在糖尿病视网膜病变（DR）筛查中表现中等，但在青光眼筛查中表现较差，且添加临床元数据对结果无显著影响。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型（LLMs）在眼科中模拟临床推理的潜力，尤其是在糖尿病视网膜病变和青光眼筛查中的应用。

Method: 使用300张标注的视网膜眼底照片，通过结构化提示描述图像，评估GPT-4在ICDR严重程度评分、DR转诊建议和青光眼杯盘比估计中的表现。

Result: GPT-4在DR筛查中表现中等（准确率82.3%），但在青光眼筛查中表现较差（准确率约78%）。添加元数据未显著影响结果。

Conclusion: GPT-4可从结构化提示中模拟基本眼科决策，但不适合临床使用，可能适用于教育或文档辅助工作。
...
### [410] [Rethinking All Evidence: Enhancing Trustworthy Retrieval-Augmented Generation via Conflict-Driven Summarization](https://arxiv.org/abs/2507.01281)
*Juan Chen,Baolong Bi,Wei Zhang,Jingyan Sui,Xiaofei Zhu,Yuanzhuo Wang,Lingrui Mei,Shenghua Liu*
...
Conclusion: CARE-RAG显著提升了RAG系统的可信度。
...
### [411] [Frustratingly Simple Retrieval Improves Challenging, Reasoning-Intensive Benchmarks](https://arxiv.org/abs/2507.01297)
*Xinxi Lyu,Michael Duan,Rulin Shao,Pang Wei Koh,Sewon Min*
...
Result: CompactDS显著提升了多个推理密集型基准测试的性能（如MMLU提升10%，MATH提升19%），并优于复杂代理系统。
...
### [412] [La RoSA: Enhancing LLM Efficiency via Layerwise Rotated Sparse Activation](https://arxiv.org/abs/2507.01299)
*Kai Liu,Bowen Xu,Shaoyu Wu,Xin Chen,Hao Zhou,Yongliang Tao,Lulu Hu*
...
<details>
  <summary>Details</summary>
Motivation: 现有激活稀疏化方法存在训练耗时或剪枝不稳定问题，LaRoSA旨在提高LLM效率且避免这些限制。

Method: 利用层间正交旋转将输入激活转换为更适合稀疏化的形式，并通过Top-K选择实现稳定稀疏化。

Result: 在LLaMA2-7B上，40%稀疏度下，LaRoSA仅带来0.17困惑度差距，推理速度提升1.30倍，零样本任务准确率差距仅0.54%。

Conclusion: LaRoSA是一种高效、稳定的激活稀疏化方法，适用于多种LLM，性能损失小且加速效果显著。
...
### [413] [Symbolic or Numerical? Understanding Physics Problem Solving in Reasoning LLMs](https://arxiv.org/abs/2507.01334)
*Nifu Dan,Yujun Cai,Yiwei Wang*

Main category: cs.CL

TL;DR: 研究了高级指令调优推理模型（如Deepseek-R1）在解决SciBench物理问题上的表现，发现其不仅能达到最高精度，还能生成独特的符号推导推理模式，且少量提示能进一步提升性能。


<details>
  <summary>Details</summary>
Motivation: 探索大型语言模型在复杂物理推理任务中的表现，尤其是结合指令调优和推理能力的模型。

Method: 使用Deepseek-R1等推理模型，在SciBench物理问题上进行实验评估，并测试少量提示的效果。

Result: 模型在复杂物理问题上达到最高精度，生成符号推导推理模式，少量提示能进一步提升性能。

Conclusion: 推理模型在物理任务中表现出色，少量提示仍有优化空间，未来性能可进一步提升。
...
### [414] [LEDOM: An Open and Fundamental Reverse Language Model](https://arxiv.org/abs/2507.01335)
*Xunjian Yin,Sitao Cheng,Yuxi Xie,Xinyu Hu,Li Lin,Xinyi Wang,Liangming Pan,William Yang Wang,Xiaojun Wan*

Main category: cs.CL

TL;DR: LEDOM是首个纯逆向语言模型，通过反向预测训练，展示了在通用任务中的潜力，并提出了逆向奖励应用以提升数学推理任务性能。
...
Method: 训练了2B和7B参数的逆向语言模型LEDOM，通过反向预测序列，并应用于逆向奖励任务。

Result: LEDOM在数学推理任务中通过逆向奖励显著提升了性能，展示了其独特能力。
...
### [415] [Skywork-Reward-V2: Scaling Preference Data Curation via Human-AI Synergy](https://arxiv.org/abs/2507.01352)
*Chris Yuhao Liu,Liang Zeng,Yuzhen Xiao,Jujie He,Jiacai Liu,Chaojie Wang,Rui Yan,Wei Shen,Fuxiang Zhang,Jiacheng Xu,Yang Liu,Yahui Zhou*
...
<details>
  <summary>Details</summary>
Motivation: 现有奖励模型在捕捉人类偏好方面表现不佳，主要源于偏好数据集的局限性，如范围狭窄、标签合成或质量控制不足。

Method: 设计人机协同的两阶段流程，结合人类标注质量与AI扩展性，构建SynPref-40M数据集，并训练Skywork-Reward-V2系列模型。

Result: Skywork-Reward-V2在多个能力维度（如人类偏好对齐、安全性等）上表现优异，达到当前最佳水平。

Conclusion: 通过高质量数据规模和人机协同优化，Skywork-Reward-V2系列显著提升了奖励模型的性能，展示了现有偏好数据集的潜力。
...
### [416] [Clinical NLP with Attention-Based Deep Learning for Multi-Disease Prediction](https://arxiv.org/abs/2507.01437)
*Ting Xu,Xiaoxiao Deng,Xiandong Meng,Haifeng Yang,Yan Wu*
...
### [417] [LogitSpec: Accelerating Retrieval-based Speculative Decoding via Next Next Token Speculation](https://arxiv.org/abs/2507.01449)
*Tianyu Liu,Qitan Lv,Hao Li,Xing Gao,Xiao Sun*

Main category: cs.CL

TL;DR: LogitSpec是一种无需训练、即插即用的方法，通过利用最后一个token的logit推测后续token，并扩展检索范围，显著提升检索式SD的性能，实现高达2.61倍的加速。


<details>
  <summary>Details</summary>
Motivation: 检索式SD方法依赖匹配范式，但常无法找到准确匹配的draft tokens，限制了性能提升。

Method: LogitSpec分两步生成draft tokens：(1) 利用最后一个token的logit推测下一个token；(2) 检索与推测token相关的参考内容。

Result: 实验表明，LogitSpec在多种文本生成任务中实现最高2.61倍加速，平均每解码步骤接受3.28个token。

Conclusion: LogitSpec有效解决了检索式SD的匹配问题，显著提升性能且易于部署。
...
### [418] [Evaluating the Effectiveness of Direct Preference Optimization for Personalizing German Automatic Text Simplifications for Persons with Intellectual Disabilities](https://arxiv.org/abs/2507.01479)
*Yingqiang Gao,Kaede Johnson,David Froehlich,Luisa Carrer,Sarah Ebling*
...
Method: 扩展了监督微调（SFT）方法，采用DPO技术，结合目标群体的偏好数据进行后训练。
...
### [419] [Efficient Out-of-Scope Detection in Dialogue Systems via Uncertainty-Driven LLM Routing](https://arxiv.org/abs/2507.01541)
*Álvaro Zaera,Diana Nicoleta Popa,Ivan Sekulic,Paolo Rosso*
...
<details>
  <summary>Details</summary>
Motivation: 任务导向对话系统（TODS）中，OOS意图检测对处理未知和模糊查询至关重要，现有方法在效率和性能上存在不足。

Method: 1. 对现有意图分类器输出进行不确定性估计；2. 对高不确定性实例触发微调LLM进行最终决策。

Result: 在OOS检测基准测试中取得最优结果，包括实际部署的TODS数据。

Conclusion: 该方法平衡了计算效率和性能，结合传统方法与LLM，显著提升了OOS检测能力。
...
### [420] [Is External Information Useful for Stance Detection with LLMs?](https://arxiv.org/abs/2507.01543)
*Quang Minh Nguyen,Taegyoon Kim*
...
<details>
  <summary>Details</summary>
Motivation: 探讨外部信息是否对LLM在立场检测任务中有帮助，尽管此前研究表明其对BERT等模型有益。

Method: 系统评估了8种LLM在3个数据集上的表现，分析了外部信息对立场检测的影响。

Result: 外部信息在多数情况下会降低性能，F1分数下降高达27.9%，且LLM倾向于与外部信息的立场而非真实立场对齐。

Conclusion: 研究揭示了LLM在立场检测中对外部信息的敏感性，强调了信息偏差的风险，并指出微调可部分缓解问题。
...
### [421] [Emotionally Intelligent Task-oriented Dialogue Systems: Architecture, Representation, and Optimisation](https://arxiv.org/abs/2507.01594)
*Shutong Feng,Hsien-chin Lin,Nurul Lubis,Carel van Niekerk,Michael Heck,Benjamin Ruppik,Renato Vukovic,Milica Gašić*
...
<details>
  <summary>Details</summary>
Motivation: 任务导向对话系统（ToD）在自然语言交互中存在任务成功、情感理解和信息传递的挑战，现有系统难以兼顾这些需求。

Method: 提出LUSTER系统，结合LLM能力和结构化奖励模型，通过短长期奖励（用户情感和任务成功）优化对话系统。

Result: 实验表明，LUSTER系统在任务成功率和情感响应方面表现更优，为下一代对话系统提供了实用路径。

Conclusion: 结合LLM与强化学习的结构化奖励模型能显著提升ToD系统的鲁棒性和情感智能。
...
### [422] [Chart Question Answering from Real-World Analytical Narratives](https://arxiv.org/abs/2507.01627)
*Maeve Hutchinson,Radu Jianu,Aidan Slingsby,Jo Wood,Pranava Madhyastha*
...
<details>
  <summary>Details</summary>
Motivation: 构建更真实的图表问答数据集，反映实际推理流程，填补现有数据集的不足。

Method: 从可视化笔记本中构建数据集，包含多视图图表和自然语言问题。

Result: GPT-4.1在该数据集上准确率为69.3%，表现显著低于其他基准。

Conclusion: 新数据集更具挑战性，凸显了现有模型在真实图表问答场景中的局限性。
...
### [423] [Confidence and Stability of Global and Pairwise Scores in NLP Evaluation](https://arxiv.org/abs/2507.01633)
*Georgii Levtsov,Dmitry Ustalov*

Main category: cs.CL

TL;DR: 论文比较了全局评分和成对比较在NLP模型评估中的优缺点，发现全局评分更可靠但可能低估某些模型，而成对比较能更好识别低分强模型但收敛较慢。


<details>
  <summary>Details</summary>
Motivation: 随着指令调优神经语言模型的发展，NLP评测从传统全局评分转向成对比较排行榜，需研究两者的优劣以指导评估策略选择。

Method: 通过合成和真实数据集的计算实验，使用标准全局指标和Bradley-Terry模型进行成对比较。

Result: 全局评分提供更可靠的整体排名，但可能低估强模型；成对比较能识别低分强模型，但收敛较慢。

Conclusion: 选择评估策略需权衡可靠性和识别能力，全局评分适合整体排名，成对比较适合特定场景。
...
### [424] [Adapting Language Models to Indonesian Local Languages: An Empirical Study of Language Transferability on Zero-Shot Settings](https://arxiv.org/abs/2507.01645)
*Rifki Afina Putri*

Main category: cs.CL

TL;DR: 研究了预训练语言模型在低资源印尼本地语言情感分析任务中的迁移能力，发现模型性能与语言在预训练中的暴露程度相关，MAD-X方法显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 探索预训练语言模型在低资源印尼本地语言中的迁移能力，尤其是情感分析任务。

Method: 评估了零样本和基于适配器（MAD-X）的迁移方法，使用了印尼单语BERT、多语言模型（mBERT、XLM-R）和MAD-X，并将目标语言分为三类：已见、部分已见和未见。

Result: 多语言模型在已见语言上表现最佳，部分已见次之，未见最差；MAD-X显著提升性能，尤其是已见和部分已见语言。

Conclusion: 模型对语言的预训练暴露程度是迁移成功的关键因素，MAD-X是一种有效的迁移方法。
...
### [425] [AdamMeme: Adaptively Probe the Reasoning Capacity of Multimodal Large Language Models on Harmfulness](https://arxiv.org/abs/2507.01702)
*Zixin Chen,Hongzhan Lin,Kaixin Li,Ziyang Luo,Zhen Ye,Guang Chen,Zhiyong Huang,Jing Ma*

Main category: cs.CL

TL;DR: 提出了一种名为AdamMeme的灵活、基于代理的评估框架，用于动态评估多模态大语言模型（mLLMs）对有害表情包的理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有基于静态数据集的准确性评估方法无法适应动态变化的网络表情包，需要更灵活和全面的评估框架。

Method: 通过多代理协作和迭代更新挑战性样本，动态评估mLLMs对有害表情包的理解能力。

Result: 实验表明，AdamMeme能系统性地揭示不同mLLMs的性能差异，并提供细粒度的模型弱点分析。

Conclusion: AdamMeme为评估mLLMs在动态环境中的表现提供了更全面和适应性的解决方案。
...
### [426] [Stereotype Detection as a Catalyst for Enhanced Bias Detection: A Multi-Task Learning Approach](https://arxiv.org/abs/2507.01715)
*Aditya Tomar,Rudra Murthy,Pushpak Bhattacharyya*

Main category: cs.CL

TL;DR: 论文探讨了语言模型中的偏见和刻板印象问题，提出联合学习偏见和刻板印象检测任务以提升模型性能，并引入StereoBias数据集进行实验。


<details>
  <summary>Details</summary>
Motivation: 偏见和刻板印象在敏感领域（如内容审核和决策）可能造成危害，因此需要更有效的检测方法。

Method: 使用StereoBias数据集，比较编码器模型和微调的解码器模型（QLoRA），并研究联合训练偏见和刻板印象检测的效果。
...
### [427] [LLMs for Legal Subsumption in German Employment Contracts](https://arxiv.org/abs/2507.01734)
*Oliver Wardas,Florian Matthes*
...
<details>
  <summary>Details</summary>
Motivation: 法律工作的文本密集性和资源密集性为NLP研究提供了独特挑战和机会，但现有数据驱动方法因缺乏可解释性和可信度而受限。

Method: 与法律专家合作扩展数据集，利用LLMs和上下文学习评估合同条款合法性，比较不同法律上下文对模型性能的影响。

Result: 完整法律文本略微提升性能，而精简版法律指南显著提高召回率和加权F1分数（80%），但LLMs表现仍远低于人类律师。
...
### [428] [Data interference: emojis, homoglyphs, and issues of data fidelity in corpora and their results](https://arxiv.org/abs/2507.01764)
*Matteo Di Cristofaro*
...
Result: 研究强调了理解语言和技术方面对提高语料分析准确性的重要性，并对定量和定性方法有重要影响。

Conclusion: 分词是语料库语言学中的关键步骤，需结合语言和技术知识以确保分析的准确性和可重复性。
...
### [429] [MuRating: A High Quality Data Selecting Approach to Multilingual Large Language Model Pretraining](https://arxiv.org/abs/2507.01785)
*Zhixun Chen,Ping Guo,Wenhan Han,Yifan Zhang,Binbin Liu,Haobin Lin,Fengze Liu,Yan Zhao,Bingni Zhang,Taifeng Wang,Yin Zheng,Meng Fang*

Main category: cs.CL

TL;DR: MuRating是一个可扩展的多语言数据质量评估框架，通过将英语数据质量信号转移到17种目标语言，显著提升了多语言模型的性能。
...
Result: MuRating在英语和多语言基准测试中均显著提升了模型性能，尤其在知识密集型任务中表现突出。

Conclusion: MuRating为多语言数据质量评估提供了有效解决方案，并指出了未来在翻译保真度和内容平衡方面的研究方向。
...
### [430] [Probing Evaluation Awareness of Language Models](https://arxiv.org/abs/2507.01786)
*Jord Nguyen,Khiem Hoang,Carlo Leonardo Attubato,Felix Hofstätter*

Main category: cs.CL

TL;DR: 研究发现Llama-3.3-70B-Instruct模型能区分测试与部署阶段，线性探针可分离这两种提示，表明模型内部已具备评估意识，且当前安全评估可能显得不真实。


<details>
  <summary>Details</summary>
Motivation: 探讨语言模型评估意识对AI治理框架和行业自愿承诺的潜在影响，确保评估的可靠性。

Method: 使用线性探针分析Llama-3.3-70B-Instruct模型，区分评估与部署提示，并验证安全评估的真实性。

Result: 线性探针能有效分离评估与部署提示，且当前安全评估被模型正确分类为不真实。

Conclusion: 研究强调了确保可信评估和理解模型欺骗能力的重要性，展示了模型内部信息如何支持黑盒安全审计。
...
### [431] [How Do Vision-Language Models Process Conflicting Information Across Modalities?](https://arxiv.org/abs/2507.01790)
*Tianze Hua,Tian Yun,Ellie Pavlick*

Main category: cs.CL

TL;DR: 研究多模态AI模型在输入信息冲突时的行为，发现模型倾向于某一模态，且内部结构和注意力机制可调控这种偏好。
...
Result: 模型倾向于某一模态，且内部结构和特定注意力头可调控偏好；发现模态无关的“路由头”可提升性能。
...
### [432] [The Anatomy of Evidence: An Investigation Into Explainable ICD Coding](https://arxiv.org/abs/2507.01802)
*Katharina Beckh,Elisa Studeny,Sujan Sai Gannamaneni,Dario Antweiler,Stefan Rüping*
...
<details>
  <summary>Details</summary>
Motivation: 自动医疗编码可简化文档和计费流程，但缺乏透明度和评估数据。

Method: 对MDACE数据集进行深入分析，评估现有可解释医疗编码系统的合理性。

Result: 发现真实证据与代码描述部分一致，先进方法与真实证据高度重叠。

Conclusion: 提出了匹配度量标准，并给出了开发与评估可解释医疗编码系统的建议。
...
### [433] [Evaluating Structured Output Robustness of Small Language Models for Open Attribute-Value Extraction from Clinical Notes](https://arxiv.org/abs/2507.01810)
*Nikita Neveditsin,Pawan Lingras,Vijay Mago*

Main category: cs.CL

TL;DR: JSON格式在临床笔记的属性值提取中解析性最佳，而结构化鲁棒性受提示设计和文档长度影响。
...
Method: 评估三种序列化格式的解析性，分析目标提示和大模型的影响，并进行错误模式分析。

Result: JSON解析性最高，结构化鲁棒性随提示设计和模型规模提升，但文档长度和笔记类型会影响表现。

Conclusion: 研究结果为临床环境中选择序列化格式和设计提示提供了实用建议。
...
### [434] [Low-Perplexity LLM-Generated Sequences and Where To Find Them](https://arxiv.org/abs/2507.01844)
*Arthur Wuhrmann,Anastasiia Kucherenko,Andrei Kucharavy*

Main category: cs.CL

TL;DR: 论文提出了一种系统方法，通过分析低困惑度序列来探索LLMs如何利用和复制训练数据，发现部分序列无法映射到训练数据，并量化了匹配序列的分布。
...
Method: 通过分析低困惑度序列，提取并溯源到训练数据中的来源。
...
### [435] [Eka-Eval : A Comprehensive Evaluation Framework for Large Language Models in Indian Languages](https://arxiv.org/abs/2507.01853)
*Samridhi Raj Sinha,Rajvee Sheth,Abhishek Upperwal,Mayank Singh*

Main category: cs.CL

TL;DR: EKA-EVAL是一个统一且可用于生产的评估框架，整合了35个基准测试，包括10个印度语言数据集，支持多语言LLM评估。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型（LLMs）的快速发展，需要超越英语中心化基准的评估框架，以满足语言多样化地区（如印度）的需求。

Method: EKA-EVAL整合了35个基准测试，涵盖推理、数学、工具使用、长上下文理解和阅读理解等类别，支持分布式推理、量化和多GPU使用。

Result: EKA-EVAL是首个端到端、可扩展的评估套件，适用于全球和印度语言LLMs，显著降低了多语言基准测试的门槛。

Conclusion: EKA-EVAL是开源的，旨在建立一个强大的多语言LLM评估生态系统，未来计划扩展到100多个基准测试。
...
### [436] [DIY-MKG: An LLM-Based Polyglot Language Learning System](https://arxiv.org/abs/2507.01872)
*Kenan Tang,Yanhong Li,Yao Qin*
...
### [437] [MiCoTA: Bridging the Learnability Gap with Intermediate CoT and Teacher Assistants](https://arxiv.org/abs/2507.01887)
*Dongyi Ding,Tiannan Wang,Chenghao Zhu,Meiling Tao,Yuchen Eleanor Jiang,Wangchunshu Zhou*

Main category: cs.CL

TL;DR: MiCoTA框架通过中间规模模型作为教师助手，帮助小型语言模型（SLM）学习长链推理任务，显著提升其性能。


<details>
  <summary>Details</summary>
Motivation: 解决小型语言模型（SLM）在长链推理任务中的学习能力不足问题，即“SLMs Learnability Gap”。

Method: 引入MiCoTA框架，利用中间规模模型和中间长度推理序列，弥合SLM的能力和推理长度差距。

Result: 实验显示，经过MiCoTA蒸馏的SLM在多个基准测试中性能显著提升（如Qwen2.5-7B和3B模型分别提升3.47和3.93分）。

Conclusion: MiCoTA为SLM的长链推理数据蒸馏提供了有效方法，并为未来研究奠定了基础。
...
### [438] [High-Layer Attention Pruning with Rescaling](https://arxiv.org/abs/2507.01900)
*Songtao Liu,Peng Liu*

Main category: cs.CL

TL;DR: 提出了一种新的剪枝算法，专注于剪枝模型高层注意力头，并通过自适应重缩放参数校准表示尺度，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 传统剪枝方法不考虑注意力头在模型中的位置，导致性能下降。

Method: 提出剪枝高层注意力头的策略，并引入自适应重缩放参数以校准表示尺度。

Result: 在多种LLM和27个数据集上验证，生成任务表现尤为突出。

Conclusion: 新方法在压缩模型的同时显著提升了性能，特别是在生成任务中。
...
### [439] [AI4Research: A Survey of Artificial Intelligence for Scientific Research](https://arxiv.org/abs/2507.01903)
*Qiguang Chen,Mingda Yang,Libo Qin,Jinhao Liu,Zheng Yan,Jiannan Guan,Dengyun Peng,Yiyan Ji,Hanjing Li,Mengkang Hu,Yimeng Zhang,Yihao Liang,Yuhang Zhou,Jiaqi Wang,Zhi Chen,Wanxiang Che*
...
<details>
  <summary>Details</summary>
Motivation: AI在科研中的潜力巨大，但缺乏全面综述，阻碍了领域发展。本文旨在填补这一空白。

Method: 通过系统分类法对AI4Research任务进行分类，识别研究空白，并整理相关资源。

Result: 提出了AI4Research的系统分类、未来方向和资源库，为研究社区提供支持。

Conclusion: 本文为AI4Research领域提供了全面的视角和资源，有望推动该领域的创新突破。
...
### [440] [Gradient-Adaptive Policy Optimization: Towards Multi-Objective Alignment of Large Language Models](https://arxiv.org/abs/2507.01915)
*Chengao Li,Hanyu Zhang,Yunkun Xu,Hongyan Xue,Xiang Ao,Qing He*

Main category: cs.CL

TL;DR: 论文提出GAPO和P-GAPO方法，通过多目标优化解决LLMs与多样化人类偏好对齐的冲突问题。
...
Method: 提出GAPO（梯度自适应策略优化）和P-GAPO，利用多梯度下降自适应调整目标梯度，实现偏好平衡。
...
### [441] [Adaptability of ASR Models on Low-Resource Language: A Comparative Study of Whisper and Wav2Vec-BERT on Bangla](https://arxiv.org/abs/2507.01931)
*Md Sazzadul Islam Ridoy,Sumi Akter,Md. Aminur Rahman*

Main category: cs.CL

TL;DR: 研究比较了OpenAI的Whisper和Facebook的Wav2Vec-BERT在低资源语言Bangla上的表现，Wav2Vec-BERT在各项指标上表现更优且计算效率更高。


<details>
  <summary>Details</summary>
Motivation: 探索神经网络模型在低资源语言Bangla上的自动语音识别（ASR）性能，以支持这类语言的发展。
...
Conclusion: Wav2Vec-BERT为低资源语言环境下的语音识别系统提供了更优的解决方案。
...
### [442] [NaturalThoughts: Selecting and Distilling Reasoning Traces for General Reasoning Tasks](https://arxiv.org/abs/2507.01921)
*Yang Li,Youssef Emad,Karthik Padthe,Jack Lanchantin,Weizhe Yuan,Thao Nguyen,Jason Weston,Shang-Wen Li,Dong Wang,Ilia Kulikov,Xian Li*

Main category: cs.CL

TL;DR: 通过系统分析教师模型的推理示范对提升学生模型推理能力的影响，研究发现高质量、多样化的推理示例（NaturalThoughts）比随机采样更有效。


<details>
  <summary>Details</summary>
Motivation: 探究教师模型的推理示范中哪些因素对学生模型推理能力的提升最有效。

Method: 从教师模型中精选高质量推理示例（NaturalThoughts），并分析样本效率和可扩展性。

Result: NaturalThoughts在通用STEM推理基准测试中优于现有数据集，且多样化示例更高效。

Conclusion: 精选高质量、多样化的推理示例能显著提升学生模型的推理能力。
...
### [443] [Decision-oriented Text Evaluation](https://arxiv.org/abs/2507.01923)
*Yu-Shiang Huang,Chuan-Ju Wang,Chung-Chi Chen*

Main category: cs.CL

TL;DR: 提出了一种基于决策效果的NLG评估框架，发现传统内在评估指标与决策效果相关性较弱，而人机协作能显著提升决策质量。
...
Method: 通过市场摘要文本（客观晨报和主观收盘分析）测试生成文本对人类和LLM决策的影响，评估交易表现。

Result: 人类和LLM单独依赖摘要时表现不优于随机；但人机协作结合深度分析能显著超越个体表现。

Conclusion: 强调生成文本应评估其促进人机协同决策的能力，传统内在指标存在局限性。
...
### [444] [The Thin Line Between Comprehension and Persuasion in LLMs](https://arxiv.org/abs/2507.01936)
*Adrian de Wynter,Tangming Yuan*

Main category: cs.CL

TL;DR: LLMs擅长维持高水平的对话，但在理解对话深层结构和语境方面存在不足。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs在辩论中的表现及其对对话结构和语境的真实理解能力。

Method: 评估LLMs在辩论中的能力，并测量其对对话结构和语境的认知。

Result: LLMs能维持连贯且有说服力的辩论，但无法展示对深层对话结构的理解。

Conclusion: LLMs作为评估者的局限性源于其语境理解能力的不足，对话效果不依赖对内容的真实理解。
