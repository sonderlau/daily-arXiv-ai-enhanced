...
TL;DR: 该论文分析了影响地球表面绝对温度的主要因素，发现太阳常数最重要，反照率次之，CO2浓度影响最小。


<details>
  <summary>Details</summary>
Motivation: 研究地球表面绝对温度的主要决定因素及其相对重要性，特别关注CO2浓度对有效发射率的影响。

Method: 假设有效发射率e由大气中CO2浓度C决定，分析太阳常数、反照率和CO2浓度对地球表面温度的相对贡献。

Result: 太阳常数是影响地球表面温度最重要的因素，反照率位居第二，CO2浓度的影响相对较小。

Conclusion: 在影响地球绝对表面温度的因素中，太阳常数起主导作用，反照率次之，CO2浓度的影响相对有限。
...
TL;DR: 本文回顾了海洋云增亮(MCB)概念从1990年代作为云微物理过程理论到2015年发展成熟的过程，重点关注创始人John Latham和Stephen Salter的贡献，以及该概念作为缓解地球快速变暖的潜在方法的最新发展。


<details>
  <summary>Details</summary>
Motivation: 开发一种通过增强海洋云层反射率来减缓全球变暖的地球工程技术。

Method: 基于云微物理学原理，通过向海洋云层中注入气溶胶粒子来增加云层反照率，从而提高地球对太阳辐射的反射能力。

Result: MCB从理论概念发展为成熟的地球工程技术方案，并在英国ARIA项目中得到进一步开发。

Conclusion: 海洋云增亮技术具有作为缓解全球变暖潜在方法的可行性，需要进一步研究和开发。
...
TL;DR: 使用耦合深度学习气候模拟器进行太平洋定步实验，发现虽然能准确捕捉大气内部变率，但对ENSO强迫的遥相关响应显著放大，导致极端事件模拟偏差。
...
Method: 采用太平洋定步实验方法，使用耦合深度学习模拟器DLESyM来隔离观测ENSO强迫的大气响应。

Result: 模拟器能真实捕捉大气内部变率，但对ENSO的强迫遥相关响应显著放大，导致高估大气阻塞频率和持续时间，低估峰值强度。

Conclusion: 耦合深度学习气候模型需要像传统数值模型一样进行深入且基于物理的验证，以建立其在物理气候分析中使用的可信度。
...
TL;DR: 该论文展示了将Kaggle竞赛获胜团队的机器学习架构集成到气候模型中，证明多种不同架构都能在低分辨率真实地理环境中实现在线稳定性，这是重要里程碑。


<details>
  <summary>Details</summary>
Motivation: 解决亚网格机器学习参数化在长期气候预测中的在线不稳定性和性能不一致问题，通过社区竞赛推动混合物理-AI气候模拟的发展。

Method: 将Kaggle竞赛获胜团队的机器学习架构耦合到交互式气候模型中，系统评估其在线性能，包括完整的云微物理过程。

Result: 多种不同架构都能实现在线稳定性，所有测试架构显示出相似的离线和在线偏差，多个Kaggle启发架构在某些指标上达到最先进水平。

Conclusion: 众包离线问题本质是改进混合物理-AI气候模拟在线性能的有效途径，在线稳定性在低分辨率真实地理环境中是可复现的里程碑成果。
...
TL;DR: 提出了一种基于生物启发的脉冲神经网络学习规则扩展，将时间驱动更新转换为事件驱动，整合了连续动态、严格局部性等生物特征，实现了百万级神经元的可扩展学习。


<details>
  <summary>Details</summary>
Motivation: 尽管AI技术取得了显著进步，但仍可从生物原理中获益，如循环连接和能量高效机制。受大脑启发，旨在设计计算效率高且可持续的生物启发AI系统。

Method: 扩展了资格传播学习规则，将其从时间驱动转换为事件驱动更新方案，并整合到大规模脉冲神经网络模拟平台中。模型扩展了连续动态、严格局部性、稀疏连接等生物特征。

Result: 在神经形态MNIST等任务上验证了方法的适用性，展示了生物基础约束可以指导计算高效AI算法的设计，实现百万级神经元的可扩展性而不影响学习性能。

Conclusion: 这项工作连接了机器学习和计算神经科学，为可持续的生物启发AI系统铺平道路，同时推进了对类脑学习的理解。
...
TL;DR: 本文提出了一种神经科学启发的拓扑正则化框架，用于增强多模态视觉语言模型对成员推理攻击的隐私保护能力，实验表明该方法能显著降低攻击成功率而不明显影响模型性能。


<details>
  <summary>Details</summary>
Motivation: 随着多模态模型的广泛应用，其训练数据隐私泄露风险日益突出。现有研究主要关注单模态系统的隐私攻击，而多模态模型对隐私攻击的脆弱性以及神经启发的多模态模型是否具有隐私保护能力仍有待探索。

Method: 提出系统性的神经科学启发拓扑正则化框架(tau)，在三种视觉语言模型(BLIP、PaliGemma 2、ViT-GPT2)上应用，使用COCO、CC3M和NoCaps三个基准数据集进行实验，比较基线模型和神经变体模型(tau > 0)的隐私保护能力。

Result: 在BLIP模型和COCO数据集上的实验表明，神经变体VLM的成员推理攻击成功率平均ROC-AUC下降了24%，同时保持了相似的模型效用(MPNet和ROUGE-2指标)。在PaliGemma 2和ViT-GPT2模型以及其他数据集上的进一步验证也证实了结果的稳定性。
...
### [7] [Inferix: A Block-Diffusion based Next-Generation Inference Engine for World Simulation](https://arxiv.org/abs/2511.20714)
...
<details>
  <summary>Details</summary>
Motivation: 世界模型是智能体AI、具身AI和游戏等领域的核心模拟器，能够生成长时、物理真实、交互式的高质量视频。扩展这些模型可以解锁视觉感知、理解和推理的新兴能力，超越当前以LLM为中心的视觉基础模型。

Method: 采用半自回归（块扩散）解码范式，结合扩散和自回归方法的优势，在块内应用扩散生成视频令牌，同时以前一个块为条件，实现更连贯稳定的视频序列。通过重新引入LLM风格的KV缓存管理，克服标准视频扩散的限制。

Result: Inferix实现了高效、可变长度和高质量的视频生成，支持交互式视频流和性能分析，能够准确模拟世界动态。
...
### [8] [Video Object Recognition in Mobile Edge Networks: Local Tracking or Edge Detection?](https://arxiv.org/abs/2511.20716)
*Kun Guo,Yun Shen,Xijun Wang,Chaoqun You,Yun Rui,Tony Q. S. Quek*

Main category: cs.CV

TL;DR: 提出LTED-Ada算法，在移动边缘计算环境下自适应选择本地跟踪或边缘检测，通过深度强化学习优化视频对象识别的准确性和延迟要求。


<details>
  <summary>Details</summary>
Motivation: 资源受限设备（如交通摄像头）的视频对象识别面临挑战，移动边缘计算可将计算密集型检测卸载到边缘服务器，但需要智能决定何时进行边缘检测与本地跟踪。

Method: 提出LTED-Ada算法，基于深度强化学习自适应选择本地跟踪或边缘检测；在多设备场景下使用联邦学习进行协作策略训练。

Result: 通过Raspberry Pi 4B设备和PC边缘服务器的硬件在环实验，证明了LTED-Ada的优越性能。

Conclusion: LTED-Ada算法能有效平衡视频对象识别的准确性和延迟要求，在单设备和多设备场景下均表现优异。
...
### [9] [DeeAD: Dynamic Early Exit of Vision-Language Action for Efficient Autonomous Driving](https://arxiv.org/abs/2511.20720)
*Haibo HU,Lianming Huang,Nan Guan,Chun Jason Xue*

Main category: cs.CV

TL;DR: DeeAD是一个无需训练的动作引导早期退出框架，通过评估中间轨迹的物理可行性来加速视觉语言动作(VLA)模型的推理，减少28%的transformer层使用和29%的延迟。
...
Method: 提出训练免费的早期退出框架，当预测轨迹与轻量级规划先验（如导航或低精度规划）在可容忍偏差（<2m）内对齐时终止推理；引入多跳控制器基于分数变化率自适应跳过冗余层。

Result: 在Bench2Drive基准测试中实现高达28%的transformer层稀疏性和29%的延迟减少，同时保持规划质量和安全性。

Conclusion: DeeAD可以集成到现有VLA模型（如ORION）中而无需重新训练，有效加速VLA规划推理。
...
### [10] [Foundry: Distilling 3D Foundation Models for the Edge](https://arxiv.org/abs/2511.20721)
*Guillaume Letellier,Siddharth Srivastava,Frédéric Jurie,Gaurav Sharma*

Main category: cs.CV

TL;DR: 提出Foundation Model Distillation (FMD)新范式，通过压缩大型自监督学习模型为紧凑高效的代理模型，保留其通用表征能力，并实现Foundry在3D点云上的首个应用。


<details>
  <summary>Details</summary>
Motivation: 大型基础模型虽然功能强大，但计算成本高、模型体积大，难以在边缘设备部署。现有压缩技术会牺牲模型的通用性，需要一种既能压缩模型又能保持其通用表征能力的方法。

Method: 提出FMD框架，训练学生模型学习压缩的SuperTokens来重建教师模型的token级表征，捕获其潜在空间的紧凑基。Foundry是该框架在3D点云上的具体实现。
...
TL;DR: DinoLizer是基于DINOv2的模型，用于定位生成式修复中的篡改区域，通过线性分类头和滑动窗口策略实现高精度检测，在多个数据集上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 当前需要更有效的检测方法来定位生成式修复中的篡改区域，特别是能够区分语义篡改和非语义编辑的方法。

Method: 在预训练的DINOv2模型上添加线性分类头，使用滑动窗口策略处理大尺寸图像，通过后处理优化二进制篡改掩码。

Result: DinoLizer在多个修复数据集上超越最先进的局部篡改检测器，平均IoU提高12%，对后处理操作保持鲁棒性。

Conclusion: DinoLizer展示了Vision Transformers在篡改检测任务中的强大表示能力，并通过消融研究验证了其优越性。
...
<details>
  <summary>Details</summary>
Motivation: 目前缺乏评估视觉语言模型在工具调用界面设计能力的基准，需要了解这些模型在传统设计软件中与设计师协作的潜力。

Method: 创建CANVAS基准，从3.3K个移动UI设计中采样598个工具设计任务，包含设计复制和设计修改两种任务类型，通过上下文工具调用来逐步更新设计。

Result: 领先模型表现出更具策略性的工具调用，提高了设计质量，同时识别出模型的常见错误模式。
...
Method: 开发文本引导语义图像编码器(TIE)，使图像表示能够基于输入文本查询进行条件化生成，减少图像分块数量，提高效率。

Result: 在1B和3B规模下，TIE模型在9个图像到文本基准测试中平均提升1.5和1.3个点，DocVQA和InfoVQA任务提升达6个点，同时仅使用一半图像分块，显著提高推理效率。
...
TL;DR: LongVT是一个端到端的代理框架，通过多模态工具链思维实现长视频推理，利用LMMs的时间定位能力作为视频裁剪工具，采用全局到局部的推理循环来减少幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 现有的大规模多模态模型在处理长视频时容易产生幻觉，因为证据稀疏且时间分散。受人类理解长视频的方式启发（先全局浏览再细查相关片段），需要开发能够有效处理长视频推理的框架。

Method: 提出LongVT框架，利用LMMs的时间定位能力作为原生视频裁剪工具，通过全局到局部的推理循环逐步细化视频帧采样，直到答案基于检索到的视觉证据。采用三阶段训练策略：工具集成的冷启动监督微调、代理强化学习和代理强化微调。

Result: LongVT在四个具有挑战性的长视频理解和推理基准测试中持续优于现有强基线模型。发布了包含247.9K训练样本和1,280个QA对评估基准的VideoSIAH数据集。

Conclusion: LongVT通过多模态工具链思维和全局到局部的推理循环，有效解决了长视频推理中的幻觉问题，在多个基准测试中表现出色，为长视频理解提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 重新审视KRISP模型，解决原模型工业级训练规模大、计算需求高、与大型骨干网络紧密耦合的问题，开发轻量级版本以适应边缘设备部署。

Method: 采用低参数配置，约束在外部知识图谱领域内，防止AI幻觉，仅在该领域内生成输出。通过系统消融研究，包括在合成VQA数据上的概念验证和在DAQUAR数据集上的评估。

Result: 复现模型性能达到原版的75%，能够在智能手机和AR-VR等边缘设备上运行，改善离线视觉推理能力。

Conclusion: 轻量级KRISP复现揭示了原模型未充分覆盖的设计缺陷和实际问题，证明了在资源受限条件下知识增强VQA架构的可行性，为边缘设备上的视觉推理提供了实用解决方案。
...
TL;DR: 本文提出了一个名为"warping"的统一算子来概括现有的动态采样方法，分析了其在深度学习中的理论特性，包括前向与反向传播的不对称性、训练稳定性条件以及离散化效应。


<details>
  <summary>Details</summary>
Motivation: 动态采样机制在计算机视觉模型中已显示出实用性，但缺乏统一的理论分析框架。本文旨在通过统一的算子连接各种动态采样方法，并进行系统的理论分析。

Method: 开发了名为"warping"的广义算子来重构现有架构（如可变形卷积、主动卷积单元、空间变换网络），对算子进行统计建模（IID变量和齐次随机场），分析前向/反向传播不对称性，并引入基于梯度更新的损失景观可视化方法。

Result: 发现动态采样机制代表了一类与传统平移不变卷积算子正交的全新算子类别，确定了确保动态采样网络稳定训练的必要条件，并分析了离散化效应的统计特性。

Conclusion: warping算子为动态采样方法提供了最小化实现和可分析框架，揭示了这类机制独特的理论特性，为理解和改进动态采样网络提供了理论基础。
...
TL;DR: Δ-NeRF是一个模块化残差框架，用于神经辐射场（NeRF）的增量优化，通过残差控制器、不确定性感知门控机制和视图选择策略，实现在不访问过去数据的情况下进行增量训练，同时通过知识蒸馏压缩模型大小。


<details>
  <summary>Details</summary>
Motivation: 现有NeRF框架在引入新视图时需要完全重新训练，限制了在卫星地形分析等数据顺序到达场景中的应用。增量优化方法存在灾难性遗忘问题，且过去数据不可用时难以处理。
...
Conclusion: Δ-NeRF为NeRF的增量优化提供了有效解决方案，在保持性能的同时显著减少训练时间和数据需求，适用于卫星地形分析等顺序数据场景。
...
TL;DR: 提出Split-then-Merge框架，通过将未标注视频分割为前景和背景层并进行自组合，解决生成视频组合控制问题，无需依赖标注数据或人工规则。


<details>
  <summary>Details</summary>
Motivation: 解决生成视频组合中的数据稀缺问题，增强对动态主体与场景交互的控制能力，避免依赖标注数据集或手工规则的限制。

Method: 将未标注视频分割为动态前景和背景层，通过自组合学习主体与场景的交互；采用变换感知训练流程，包含多层融合增强和身份保持损失。

Result: 在定量基准测试和人类/VLLM定性评估中均优于现有最先进方法。

Conclusion: StM框架有效解决了生成视频组合的控制问题，通过自监督学习实现了真实感视频生成，无需依赖标注数据。
...
<details>
  <summary>Details</summary>
Motivation: 构建一个能够精确评估视觉推理能力的合成环境，覆盖多种认知任务类型，为大规模视觉语言模型提供可靠的基准测试。

Method: 使用程序化生成方法创建包含25种任务类型的谜题，包括对称检测、几何变换、空间推理、图表解释和序列预测等，每个谜题都配有可验证的真实解。

Result: 评估显示即使是当前最先进的GPT-5模型在Sphinx基准上仅达到51.1%的准确率，远低于人类表现。使用可验证奖励的强化学习（RLVR）能显著提升模型在这些任务上的准确率。

Conclusion: Sphinx为视觉推理提供了有效的评估基准，RLVR方法在提升多模态推理能力方面显示出巨大潜力。
...
TL;DR: 本文提出了一种无需训练和数据的优化视觉反转（OVI）方法，替代传统扩散模型中计算昂贵的先验网络，通过优化潜在视觉表示来最大化与文本嵌入的相似度，并引入两种约束来提升图像质量。


<details>
  <summary>Details</summary>
Motivation: 扩散模型依赖计算昂贵的先验网络将文本嵌入转换为视觉流形，这些先验需要大量训练数据。本文挑战这种必要性，探索无需训练和数据的替代方案。

Method: 采用优化视觉反转（OVI）方法，从随机伪标记初始化潜在视觉表示，通过迭代优化最大化与文本嵌入的余弦相似度，并引入马哈拉诺比斯距离和最近邻损失两种约束来正则化优化过程。

Result: 在Kandinsky 2.2上的实验表明，OVI可以替代传统先验。分析揭示了当前评估基准的缺陷，约束OVI方法在视觉保真度上优于基线，最近邻方法表现最佳，达到与最先进数据高效先验相当或更高的分数。

Conclusion: OVI作为先验网络的替代方案具有潜力，特别是在数据效率方面，该方法值得进一步研究。代码将在接受后公开。
...
TL;DR: RefTr是一个用于血管树中心线生成的3D图像到图模型，通过循环精炼汇合轨迹来生成中心线图。该模型采用生产者-精炼器架构，在保持高召回率的同时减少了参数量，并在多个数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 在临床应用中，准确检测血管树中心线并保持正确的树拓扑结构至关重要。保持高召回率尤为重要，因为遗漏小分支可能导致致命错误。

Method: 使用基于Transformer解码器的生产者-精炼器架构，生产者提出初始汇合轨迹，精炼器循环精炼这些轨迹形成最终中心线图。汇合轨迹表示能够在精炼完整轨迹时强制保持有效树拓扑。

Result: 在多个公共中心线数据集上，RefTr实现了比之前最先进方法更优的召回率和相当的精度，同时提供更快的推理速度和显著减少的参数数量。

Conclusion: RefTr展示了作为3D医学成像中血管树分析的新最先进框架的潜力，特别是在保持高召回率的同时实现了参数效率。
...
TL;DR: 提出了首个高分辨率(5472×3648px)立体DSLR数据集，包含18000张图像，系统性地变化焦距和光圈，覆盖真实复杂场景，支持深度估计、景深渲染、去模糊等研究。
...
Method: 使用两个相同的相机组件，在9个不同复杂度的场景中，以10种焦距(28-70mm)和5种光圈(f/2.8-f/22)拍摄，共50种光学配置，每个场景2000张图像。

Result: 创建了包含挑战性视觉元素的数据集，如多尺度光学幻觉、反射表面、透明玻璃墙等，展示了当前最先进方法在真实世界光学泛化方面的挑战。

Conclusion: 该数据集支持几何和光学效应的受控分析，为单目/立体深度估计、景深渲染等任务提供基准，并发布了数据集、校准文件和评估代码以支持可重复研究。
...
TL;DR: 提出了首个大规模无监督视觉记忆性数据集，包含82,000多个视频及其描述性回忆数据，用于建模视觉记忆信号，在回忆生成和话到嘴边检索任务上表现优异。
...
Method: 利用Reddit等在线平台的话到嘴边检索查询构建数据集，采用对比训练策略创建多模态话到嘴边检索模型，并对大型视觉语言模型进行微调。

Result: 基于该数据集微调的模型在生成视觉内容开放式记忆描述方面优于GPT-4o等最先进模型，并创建了首个能够执行多模态话到嘴边检索的模型。
...
TL;DR: 提出一种从立体雾天图像序列中同时估计所有雾模型参数的方法，解决了传统顺序估计导致的误差传播问题，并能处理全局不均匀的真实雾况。


<details>
  <summary>Details</summary>
Motivation: 现有方法顺序估计雾模型参数容易导致误差传播，且难以处理真实世界中全局不均匀的雾况。需要一种能同时估计所有参数并适应真实雾况的方法。

Method: 通过求解新的优化问题同时估计所有雾模型参数，假设雾仅局部均匀以处理全局不均匀的真实雾。可作为现有视觉SLAM或里程计系统的附加模块。

Result: 在合成雾数据和真实雾序列SDIRF上的广泛实验表明，该方法在合成数据上产生最准确的估计，在真实雾中适应性更好。

Conclusion: 提出的算法优于现有方法，能有效估计雾参数并适应真实雾况，同时发布了SDIRF数据集和代码以推动雾中视觉感知研究。
...
Method: 提出V^2-SAM框架，包含交叉视角锚点提示生成器（V^2-Anchor）建立几何感知对应，以及交叉视角视觉提示生成器（V^2-Visual）增强外观引导线索，并采用多专家设计和后验循环一致性选择器（PCCS）自适应选择最可靠的专家。

Result: 在Ego-Exo4D、DAVIS-2017和HANDAL-X三个基准测试中均取得了最先进的性能。

Conclusion: V^2-SAM成功将SAM2扩展到跨视角对应任务，通过几何和外观互补的提示生成器以及自适应选择机制，有效解决了跨视角物体对应问题。
...
TL;DR: 提出了Null-Text Test-Time Alignment (Null-TTA)方法，通过优化无分类器引导中的无条件嵌入来对齐扩散模型，防止奖励黑客攻击，在语义空间进行优化。


<details>
  <summary>Details</summary>
Motivation: 现有的测试时对齐方法容易导致目标奖励函数的欠优化或过优化（奖励黑客），需要一种更有效的方法来平衡对齐效果和泛化能力。

Method: 通过优化无分类器引导中的无条件嵌入而非操纵潜在变量或噪声变量，在语义相干流形上进行对齐，防止利用非语义噪声模式来提升奖励。
...
TL;DR: 提出了GaINeR框架，将可训练高斯分布与神经隐式表示结合，实现2D图像的连续表示、可解释几何结构和灵活局部编辑
...
Method: 结合可训练高斯分布和神经网络INR，对给定图像坐标检索K个最近高斯分布，聚合距离加权的嵌入，通过神经网络预测RGB值

Result: 实现了连续图像表示、可解释几何结构和灵活局部编辑，为物理感知和交互式图像处理提供了基础

Conclusion: GaINeR框架成功解决了传统INR在几何结构和编辑能力方面的局限性，为动态和交互式图像应用开辟了新途径
...
TL;DR: 本研究开发了一种深度学习模型，用于减少小脑桥脑角MRI检查中的造影剂剂量，通过从低剂量T1ce图像恢复标准剂量图像质量，实现在10%-30%标准剂量下仍能保持良好的图像质量和诊断性能。


<details>
  <summary>Details</summary>
Motivation: 减少MRI检查中造影剂的使用剂量，降低患者风险和治疗成本，同时保持图像质量和诊断准确性。

Method: 采用多中心回顾性研究，使用前庭神经鞘瘤患者的T1和T1ce图像模拟低剂量T1ce，训练深度学习模型从低剂量图像恢复标准剂量图像质量。

Result: 在10%输入剂量下，DL恢复的T1ce显著改善了分割性能（Dice从0.673提升到0.734），图像质量指标明显提升。10%和30%输入剂量的DL恢复图像均表现出色，后者更具信息性。

Conclusion: 深度学习模型能够显著改善小脑桥脑角低剂量MRI的图像质量，使在10%-30%标准剂量下进行病变检测和诊断表征成为可能。
...
<details>
  <summary>Details</summary>
Motivation: 视频具有天然的时序连贯性，但轻量级模型在捕捉复杂时序动态方面存在困难。通过强制中间表示的平滑变化，可以更好地利用视频的时序一致性。

Method: 使用高斯随机游走模型对连续帧的中间层嵌入变化进行建模，惩罚表示上的突变，促进低加速度的解决方案。

Result: 在Kinetics-600上提升准确率3.8%到6.4%；MoViNets模型家族在各自FLOP约束下将当前最优结果提升3.8%到6.1%；MobileNetV3和MoViNets-Stream在可比内存占用下比先前最优模型提升4.9%到6.4%。

Conclusion: 平滑正则化技术能有效提升轻量级视频识别模型的性能，通过强制时序平滑性更好地捕捉视频动态，在多个基准测试中显著超越现有方法。
...
TL;DR: 提出了一个开放词汇组合解释框架，利用开放词汇语义分割模型生成掩码，从而能够探测神经元对任意概念和数据集的信息编码，解决了传统方法依赖人工标注数据的局限性。


<details>
  <summary>Details</summary>
Motivation: 传统组合解释方法依赖人工标注数据集，限制了其在特定领域和预定义概念之外的适用性。本文旨在开发一个框架，允许用户在视觉领域中探测神经元对任意概念和数据集的信息编码。

Method: 框架包含三个步骤：指定任意概念、使用开放词汇模型生成语义分割掩码、从这些掩码中推导组合解释。通过开放词汇语义分割模型计算开放词汇组合解释。

Result: 与先前方法相比，在定量指标和人类可解释性方面都进行了比较，分析了从人工标注数据转向模型标注数据时解释的差异，并展示了框架在解释灵活性和任务适应性方面的额外能力。

Conclusion: 该框架通过开放词汇语义分割模型，显著扩展了组合解释的适用范围，使其能够处理任意概念和数据集，为理解神经元信息编码提供了更灵活和通用的工具。
...
TL;DR: 提出了UruDendro4数据集，包含102个火炬松样本的横截面图像和年轮标注，支持基于横截面图像的年轮体积建模，并提供了自动年轮检测的性能基准。


<details>
  <summary>Details</summary>
Motivation: 解决木材横截面数据稀缺问题，提供包含不同高度样本的数据集以支持年轮体积建模，并建立自动年轮检测的性能基准。

Method: 创建UruDendro4数据集，包含手动标注的年轮；使用最先进方法（如DeepCS-TRD）进行自动年轮检测性能基准测试；进行消融实验验证参数配置。

Result: DeepCS-TRD方法表现最佳，平均精度0.838，平均召回率0.782，适应兰德误差0.084；包含该数据集的训练提高了模型在年轮检测任务中的泛化能力。

Conclusion: UruDendro4数据集填补了木材横截面数据空白，支持年轮体积建模，为自动年轮检测提供了可靠基准，有助于改进相关算法的性能。
...
<details>
  <summary>Details</summary>
Motivation: 乳腺超声报告生成面临配对图像-报告数据集缺乏和大语言模型产生幻觉风险的问题，需要开发不依赖配对数据的有效方法。
...
TL;DR: 提出了表达性图像合成任务，开发了StickerNet两阶段框架，通过从真实编辑行为中学习来预测贴纸放置参数，强调表达性和用户意图而非真实性。


<details>
  <summary>Details</summary>
Motivation: 传统图像合成研究关注视觉真实性和语义合理性，但现代内容创作中用户更倾向于创造艺术性、趣味性或社交参与性的内容，需要支持风格多样性和宽松放置逻辑的表达性合成。

Method: 提出StickerNet两阶段框架：首先确定合成类型，然后预测不透明度、遮罩、位置和缩放等放置参数。基于在线视觉创作平台的180万次真实编辑行为构建数据集。

Result: 用户研究和定量评估显示StickerNet优于常见基线方法，与人类放置行为高度匹配，证明从真实编辑模式学习的有效性。

Conclusion: 这项工作引入了视觉理解的新方向，强调表达性和用户意图而非真实性，为创意图像编辑提供了更符合实际需求的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 多摄像头交通视频分析面临海量数据处理挑战，传统方法使用VLM将视频转换为文本耗时较长，无法及时生成交通洞察和调查事件。

Method: 采用顺序处理策略，利用摄像头重叠区域，迭代应用不同token限制的VLM，将前一个摄像头的输出作为后续摄像头的提示，并通过对象级相似性检测器智能绕过冗余VLM调用。

Result: 在真实数据集上的实验结果显示，TrafficLens将视频到文本转换时间减少高达4倍，同时保持信息准确性。
...
TL;DR: 提出了一种结合Vision Transformers和同态加密的隐私保护联邦学习框架，用于安全的多机构病理学分类，通过加密CLS令牌实现30倍通信减少，同时防止模型反演攻击。


<details>
  <summary>Details</summary>
Motivation: 医疗机构的协作机器学习需要共享数据以提高诊断准确性，但HIPAA等隐私法规禁止直接共享患者数据。传统联邦学习的模型梯度容易受到重建攻击，可能泄露敏感医疗信息。

Method: 使用Vision Transformers的CLS令牌作为紧凑特征表示，通过CKKS同态加密加密这些令牌后在服务器端安全聚合，实现隐私保护的联邦学习。

Result: 在肺癌病理学分类的三客户端联邦设置中，该方法在未加密域达到96.12%的全局分类准确率，在加密域达到90.02%。加密CLS令牌相比梯度加密减少30倍通信量，仅需326KB数据传输。

Conclusion: 该框架成功结合ViT和同态加密，在保护隐私的同时维持高分类性能，为医疗领域的协作学习提供了可行的隐私保护解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 主流基于扩散模型的免训练风格迁移方法依赖计算密集的反演过程，影响效率且在反演不准确时产生视觉失真。

Method: 使用双整流流并行预测内容和风格轨迹，通过动态中点插值融合两种轨迹的速度场，结合注意力注入指导风格整合。

Result: 实验表明该方法能泛化到多种风格和内容，提供高效且有效的风格迁移流程。

Conclusion: 提出的免反演框架解决了传统方法效率低和视觉失真的问题，实现了鲁棒的风格融合和高保真度输出。
...
TL;DR: 提出了一种无需测试时参考图像的伪装目标检测框架，通过将参考信息蒸馏到类别原型记忆中，在推理时通过查询条件化的原型混合来合成参考向量。


<details>
  <summary>Details</summary>
Motivation: 当前的双分支Ref-COD系统需要在测试时提供参考图像，这限制了部署性、增加了延迟和数据收集负担。

Method: 在训练期间将参考图像蒸馏到类别原型记忆中，维护每个类别的EMA更新原型；在推理时通过查询预测混合权重来生成指导向量；提出双向注意力对齐模块来弥合参考统计量和伪装查询特征之间的表示差距。

Result: 在R2C7K基准测试上的广泛实验表明，该方法与现有最先进方法相比具有竞争力或更优性能。
...
Result: 在四个物理采集数据集上的实验表明，WavePCNet在准确性和鲁棒性方面持续优于现有最先进方法。

Conclusion: WavePCNet通过物理驱动的波前传播补偿，有效提升了在复杂环境条件下对被遮挡物体的感知能力，具有更好的鲁棒性和可解释性。
...
Method: 构建GuardTrace数据集，通过多样化提示策略生成，并通过MLRM和人类投票验证流程进行精炼。提出三阶段渐进式训练方案结合数据精炼过程，使模型能够根据不同风险级别学习细致和上下文相关的安全偏好。

Result: 在涵盖领域内和领域外场景的测试集上，GuardTrace-VL模型在不安全推理检测任务上达到93.1%的F1分数，比之前最强的多模态安全防御方法提高了13.5%。
...
<details>
  <summary>Details</summary>
Motivation: 图像可以看作前景对象在背景上的分层组合，这种分层表示能够独立编辑元素，为内容创作提供更大灵活性。但由于方法和数据有限，单图像分层分解仍然具有挑战性。

Method: 观察到分层分解与图像修复任务之间的强关联，提出通过轻量级微调将基于扩散的修复模型适配到分层分解。引入具有线性注意力复杂度的多模态上下文融合模块来保留潜在空间细节。
...
TL;DR: MERGE是一个多模态实体感知检索增强生成框架，通过构建实体中心的多模态知识库和动态检索机制，解决了新闻图像描述中的信息覆盖不完整、跨模态对齐弱和视觉实体匹配不佳的问题。


<details>
  <summary>Details</summary>
Motivation: 现有新闻图像描述方法存在三个关键挑战：信息覆盖不完整、跨模态对齐弱、视觉实体匹配不佳，需要更有效的多模态知识整合和检索机制。

Method: 构建实体中心多模态知识库(EMKB)，集成文本、视觉和结构化知识；采用多阶段假设-描述策略改善跨模态对齐；通过图像内容引导的动态检索增强视觉实体匹配。

Result: 在GoodNews和NYTimes800k数据集上显著优于现有方法，CIDEr分别提升+6.84和+1.16，命名实体识别F1分数分别提升+4.14和+2.64；在未见过的Visual News数据集上也表现优异。

Conclusion: MERGE框架通过多模态知识整合和动态检索机制，有效提升了新闻图像描述的质量和实体识别能力，展现出强大的鲁棒性和领域适应性。
...
TL;DR: MetaRank是一个元学习框架，用于自动选择最适合特定任务的模型迁移性评估指标，解决了传统方法依赖平均性能而忽略任务特异性的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的模型迁移性评估指标选择往往是临时性的，或者仅基于指标的历史平均性能。然而，我们发现不同指标的有效性高度依赖于具体任务，没有一个单一指标在所有目标数据集上都是最优的。

Method: 将指标选择建模为学习排序问题，使用预训练语言模型将数据集和指标的文本描述编码到共享语义空间，然后训练元预测器学习数据集特征与指标机制之间的复杂关系。

Result: 在11个预训练模型和11个目标数据集上的广泛实验表明，MetaRank方法具有强大的有效性。

Conclusion: MetaRank通过元学习框架实现了任务感知的模型迁移性评估指标自动选择，为实践者提供了先验选择最合适指标的能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有TMVC方法依赖全局密集邻居关系建模视图内依赖，计算成本高且无法直接保证视图间关系一致性，同时缺乏对学习到的多视图邻居结构在类别空间一致性的保证，影响分类结果的可信度。

Method: 引入原型来表示每个视图的邻居结构，简化视图内邻居关系学习，实现视图内和视图间结构的动态对齐，促进跨视图共识的高效一致发现。

Result: 在多个公开多视图数据集上的实验表明，该方法在性能下游和鲁棒性方面与主流TMVC方法相比具有竞争力。

Conclusion: 所提出的基于原型的TMVC框架能够更高效、一致地发现跨视图共识，提高了多视图分类的可信度和性能。
...
Method: 提出CameraMaster框架，通过相机参数嵌入调制相机指令和内容语义，使用交叉注意力注入调制后的指令，并将指令和相机嵌入作为条件信号注入时间嵌入中。

Result: 在78K图像-提示对数据集上的实验显示，CameraMaster对参数变化产生单调且接近线性的响应，支持无缝多参数组合，显著优于现有方法。
...
TL;DR: 提出了CaptionQA基准，通过下游任务效用评估图像描述质量，覆盖4个领域，包含33,027个密集标注的多选题，揭示当前MLLMs在描述效用方面存在显著差距。
...
Method: 构建可扩展的领域相关基准，涵盖自然、文档、电商和具身AI4个领域，包含25个顶级和69个子类别的细粒度分类法，创建33,027个需要视觉信息回答的多选题。

Result: 评估显示图像与其描述效用之间存在显著差距，在传统图像QA基准上表现相近的模型在描述效用上下降高达32%。

Conclusion: CaptionQA基准能有效衡量描述在下游任务中的实用性，揭示了当前MLLMs在生成具有实际效用的描述方面存在明显不足。
...
TL;DR: FlowerDance是一个高效的音乐到舞蹈生成系统，通过结合MeanFlow与物理一致性约束，在少量采样步骤下生成高质量舞蹈动作，并使用BiMamba架构实现高效的非自回归生成。


<details>
  <summary>Details</summary>
Motivation: 现有音乐到舞蹈生成方法的生成效率有限，无法为高保真3D渲染提供足够的计算余量，限制了3D角色在实时应用中的表现力。

Method: 结合MeanFlow与物理一致性约束实现高质量动作生成；使用基于BiMamba的骨干网络和通道级跨模态融合的简单高效架构；支持非自回归生成和动作编辑功能。
...
TL;DR: LungNoduleAgent是一个创新的协作多智能体系统，专门用于分析肺部CT扫描，通过三个模块（结节定位器、放射科医生、医生智能体系统）提高结节描述和恶性分级精度，在多个数据集上表现优于主流模型。
...
Method: 将诊断过程分解为顺序组件：1）结节定位器协调临床检测模型识别结节；2）放射科医生整合局部图像描述技术生成全面CT报告；3）医生智能体系统基于图像和CT报告进行恶性推理，得到病理知识库和多智能体框架支持。
...
<details>
  <summary>Details</summary>
Motivation: 现有的基于学习的图像去模糊方法存在两难选择：基于模型的深度展开方法虽然能强制物理约束，但会产生过度平滑和伪影；生成模型虽然感知质量好，但会因物理约束弱而产生幻觉细节。需要一种能同时保证物理准确性和感知真实性的方法。

Method: 提出了一种新颖框架，通过将强大的生成先验与显式、密集的物理约束相结合。将退化场建模为高维压缩核的密集连续体，捕捉运动和退化模式的微小变化。利用这个丰富的描述符场来调节ControlNet架构，强有力地引导扩散采样过程。

Result: 在具有挑战性的严重模糊场景中，该方法在物理精度和感知真实性之间取得了良好平衡，超越了最先进的基于模型方法和生成基线方法。

Conclusion: 该方法成功调和了基于模型和生成式两种范式，通过将生成先验与密集物理约束相结合，在图像去模糊任务中实现了物理准确性和感知真实性的统一。
...
Result: 实验结果表明MUSE在生成和编辑任务中均优于所有方法，提高了情感准确性和语义多样性，同时在内容、文本提示和情感表达之间保持最佳平衡。

Conclusion: MUSE为情感合成建立了新范式，展示了统一框架在图像情感合成中的有效性。
...
TL;DR: 提出T-NIG模型，通过将时间参数融入正态逆伽马分布来生成阿尔茨海默病的长期预测图像，解决不规则时间间隔数据中疾病特征保持问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在长期AD预测中难以保持疾病相关特征，特别是在处理不规则时间间隔的序列数据时。时间相关分布可以反映疾病特征的变化。

Method: 使用两个时间点的脑图像，通过坐标邻域识别特征，将时间参数融入正态逆伽马分布，利用不确定性估计减少认知和随机不确定性。

Result: T-NIG模型在数据集中的短期和长期预测任务中表现出最先进性能，能够准确预测疾病进展并保持疾病相关特征。

Conclusion: T-NIG模型能有效处理不规则时间分布数据，在保持疾病特征的同时实现准确的长期AD预测。
...
<details>
  <summary>Details</summary>
Motivation: 解决基于扩散的编辑模型在理解复杂用户指令（如组合关系、上下文线索或指代表达）时的困难，避免编辑结果出现语义漂移或无法反映预期变化的问题。
...
TL;DR: 提出了一种新颖的虹膜认证匹配流程，通过3D-CNN学习丰富的时空表示，采用课程学习方式训练，解决了现有方法对旋转、尺度、反射和模糊等变化的鲁棒性问题。


<details>
  <summary>Details</summary>
Motivation: 现有虹膜认证算法在旋转、尺度、反射和模糊等变化下鲁棒性不足，且大多依赖简单的点对点比较，未能有效利用虹膜模式的时空结构。

Method: 将虹膜图像沿一维分割成子图像序列，输入3D-CNN捕获时空特征；采用课程学习方式，结合三元组损失和ArcFace损失进行端到端训练。

Result: 该方法在深度度量域中嵌入了时间依赖性，提高了特征的判别性，能够有效应对旋转、尺度、反射和模糊等挑战。
...
<details>
  <summary>Details</summary>
Motivation: 解决3D重建中长期存在的反射问题，因为视角相关的反射会使外观和几何信息纠缠在一起，影响重建质量。

Method: 采用双分支网络：基于BRDF的反射分支和黏土引导分支，通过合成黏土样图像提供无反射监督信号，联合训练以稳定几何和优化表面法线。

Result: 在合成和真实数据集上的实验显示，在法线精度和网格完整性方面相比现有反射处理方法有显著提升。

Conclusion: 通过"去光泽化"将辐射度转换为中性表示，可以作为学习反射物体几何的有力归纳偏置。
...
### [55] [Scaling Foundation Models for Radar Scene Understanding](https://arxiv.org/abs/2511.21105)
...
Conclusion: RadarFM通过结构化空间语言监督实现了雷达基础模型的开发，为跨任务雷达感知提供了统一框架。
...
### [56] [EM-KD: Distilling Efficient Multimodal Large Language Model with Unbalanced Vision Tokens](https://arxiv.org/abs/2511.21106)
*Ze Feng,Sen Yang,Boqiang Duan,Wankou Yang,Jingdong Wang*

Main category: cs.CV

TL;DR: EM-KD是一种新颖的知识蒸馏范式，通过解决高效MLLMs和普通教师模型之间视觉令牌不平衡的问题，提升高效多模态大语言模型的性能。


<details>
  <summary>Details</summary>
Motivation: 现有高效MLLMs压缩视觉令牌以减少资源消耗，但视觉信息丢失会降低理解能力。先前方法虽然引入知识蒸馏，但忽视了学生和教师模型之间视觉令牌不平衡导致的细粒度视觉理解差异。

Method: 首先使用曼哈顿距离和匈牙利匹配算法在空间维度对齐师生视觉令牌，然后引入两种蒸馏策略：1）视觉-语言亲和度蒸馏（VLAD），最小化亲和矩阵的平滑L1距离；2）视觉语义蒸馏（VSD），使用反向KL散度测量词汇空间中对齐视觉令牌的离散概率分布。

Result: 在多样化基准测试中，EM-KD训练模型在准确性和效率上都大幅优于先前的高效MLLMs。与配备相同视觉令牌匹配策略的先前蒸馏方法相比，EM-KD也实现了更好的性能。

Conclusion: EM-KD通过解决视觉令牌不平衡问题并引入有效的蒸馏策略，显著提升了高效多模态大语言模型的性能，验证了其有效性。
...
### [57] [FaithFusion: Harmonizing Reconstruction and Generation via Pixel-wise Information Gain](https://arxiv.org/abs/2511.21113)
*YuAn Wang,Xiaofan Li,Chi Huang,Wenhao Zhang,Hao Li,Bosheng Wang,Xun Sun,Jun Wang*

Main category: cs.CV

TL;DR: FaithFusion是一个基于像素级期望信息增益的3DGS-扩散模型融合框架，用于可控驾驶场景重建和3D场景生成，解决了几何保真度和视觉外观合成之间的平衡问题。


<details>
  <summary>Details</summary>
Motivation: 在可控驾驶场景重建和3D场景生成中，需要在保持几何保真度的同时，在大视角变化下合成视觉上合理的外观。然而，基于几何的3DGS和基于外观的扩散模型的有效融合面临挑战，缺乏像素级、3D一致的编辑标准往往导致过度修复和几何漂移。

Method: 提出了FaithFusion框架，使用像素级期望信息增益作为统一策略进行时空合成：指导扩散模型作为空间先验来优化高不确定性区域，同时通过像素级权重将编辑结果蒸馏回3DGS。这是一个即插即用的系统，无需额外的先验条件和结构修改。

Result: 在Waymo数据集上的广泛实验表明，该方法在NTA-IoU、NTL-IoU和FID指标上达到了最先进的性能，即使在6米车道偏移时仍能保持107.47的FID分数。

Conclusion: FaithFusion通过像素级期望信息增益有效解决了3DGS和扩散模型融合中的几何保真度和外观合成问题，在驾驶场景重建中实现了优异的性能。
...
### [58] [Deformation-aware Temporal Generation for Early Prediction of Alzheimers Disease](https://arxiv.org/abs/2511.21114)
*Xin Honga,Jie Lin,Minghui Wang*

Main category: cs.CV

TL;DR: 提出了一种名为DATGN的变形感知时序生成网络，用于自动学习脑部MRI图像中的形态变化，以预测阿尔茨海默病的进展。该方法能够处理不完整的时间序列数据，并生成符合疾病进展的未来MRI图像，显著提升了AD分类的准确性。
...
Method: DATGN首先对不完整的MRI时间序列进行插值，然后通过双向时序变形感知模块引导网络生成符合疾病进展的未来MRI图像，实现早期预测。

Result: 在ADNI数据集上的实验显示，DATGN在PSNR和MMSE图像质量指标上表现优异。将DATGN生成的合成数据集成到SVM、CNN和3DCNN分类方法中，AD vs. NC分类准确率提升了6.21%至16%，AD vs. MCI vs. NC分类准确率提升了7.34%至21.25%。

Conclusion: DATGN能够生成符合阿尔茨海默病脑萎缩趋势的MRI图像，有效支持早期疾病预测，并在分类任务中显著提升性能。
...
### [59] [Which Layer Causes Distribution Deviation? Entropy-Guided Adaptive Pruning for Diffusion and Flow Models](https://arxiv.org/abs/2511.21122)
*Changlin Li,Jiawei Zhang,Zeyi Shi,Zongxin Yang,Zhihui Li,Xiaojun Chang*

Main category: cs.CV

TL;DR: EntPruner是一个基于熵引导的自动渐进式剪枝框架，专门针对扩散和流模型，通过条件熵偏差评估模块重要性，实现参数冗余减少和推理加速。


<details>
  <summary>Details</summary>
Motivation: 大规模视觉生成模型在下游任务中存在显著的参数冗余问题，需要一种专门针对生成模型的剪枝方法，既能减少参数又能保持生成多样性和条件保真度。

Method: 提出熵引导剪枝策略，使用数据相关的条件熵偏差(CED)评估模块重要性；采用零样本自适应剪枝框架，自动确定剪枝时机和程度，避免一次性剪枝导致的问题。
...
TL;DR: 提出Ent-Prog框架，通过条件熵膨胀评估模型组件重要性进行优先级训练，结合自适应渐进调度，在保持生成性能的同时实现2.2倍训练加速和2.4倍GPU内存减少。
...
Method: 使用条件熵膨胀评估模型组件重要性进行优先级训练，结合自适应渐进调度动态增加计算复杂度。

Result: 在三个数据集上验证，实现2.2倍训练加速和2.4倍GPU内存减少，且不损失生成性能。

Conclusion: Ent-Prog框架有效解决了扩散模型训练中的效率和内存问题，为人类视频生成提供了实用的训练解决方案。
...
### [61] [CtrlVDiff: Controllable Video Generation via Unified Multimodal Video Diffusion](https://arxiv.org/abs/2511.21129)
...
<details>
  <summary>Details</summary>
Motivation: 传统几何线索（如深度、边缘）在视频编辑中存在局限性，无法充分约束外观、材质和光照，导致物理编辑（如重照明、材质替换）困难且产生时间漂移。需要引入图形学基础的多模态信息来提供互补约束。

Method: 提出CtrlVDiff模型，采用混合模态控制策略（HMCS）融合深度、法线、分割、边缘和图形内在属性（反照率、粗糙度、金属度），并构建MMVideo混合数据集进行训练。

Result: 在理解和生成基准测试中，CtrlVDiff展现出卓越的可控性和保真度，支持分层编辑（重照明、材质调整、对象插入），并在部分模态缺失时保持鲁棒性。

Conclusion: 多模态融合方法显著提升了视频编辑的物理合理性和时间一致性，为可控视频生成提供了有效解决方案。
...
TL;DR: LLaVA-UHD v3提出了渐进式视觉压缩(PVC)方法，通过改进的补丁嵌入和窗口化令牌压缩，在保持性能的同时显著降低计算开销，实现了高效的本地分辨率视觉编码。


<details>
  <summary>Details</summary>
Motivation: 解决全局本地分辨率视觉编码在提升多模态大语言模型能力时带来的计算开销过大的问题。
...
Conclusion: PVC方法能够将预训练的ViT重构为高效架构，在保持通用性的同时显著降低计算开销，为高效MLLM研究提供了有效解决方案。
...
### [63] [DeepRFTv2: Kernel-level Learning for Image Deblurring](https://arxiv.org/abs/2511.21132)
*Xintian Mao,Haofei Song,Yin-Nian Liu,Qingli Li,Yan Wang*

Main category: cs.CV

TL;DR: 提出Fourier Kernel Estimator (FKE)方法，通过在傅里叶空间进行激活操作，将空间域的卷积问题转换为傅里叶空间的乘法问题，使网络能够学习核级模糊过程，无需额外监督。


<details>
  <summary>Details</summary>
Motivation: 当前深度网络仍处于像素级学习阶段，无法让去模糊模型理解模糊的本质。模糊是由清晰图像与模糊核的卷积自然引起的，因此让网络在核级学习模糊过程可以显著提高图像去模糊性能。

Method: 提出FKE方法，将卷积对象从"图像"改为网络提取的"特征"，设计解耦多尺度架构，使用多个分层子Unet和可逆策略，实现更好的多尺度编码和解码。

Result: 实验表明该方法在运动去模糊方面达到最先进的结果，并显示出处理其他核相关问题的潜力。分析显示核估计器能够学习具有物理意义的核。

Conclusion: FKE方法使网络能够以低复杂度学习核级模糊过程，无需额外监督，在图像去模糊任务中表现出色，并具有扩展到其他核相关问题的潜力。
...
Method: GridAR采用网格分区渐进生成方案，在同一画布位置生成多个部分候选，早期修剪不可行候选，固定可行候选作为锚点指导后续解码。同时结合布局指定提示重构策略，通过检查部分视图推断可行布局。

Result: 在N=4时，GridAR比Best-of-N（N=8）在T2I-CompBench++上性能提升14.4%，同时成本降低25.6%。在自回归图像编辑上，PIE-Bench上语义保持能力提升13.9%。
...
TL;DR: 提出了UPA-RFAS框架，针对VLA模型开发通用可迁移的对抗性补丁攻击，能够在未知架构、微调变体和仿真到现实转换中实现跨模型攻击。


<details>
  <summary>Details</summary>
Motivation: 当前VLA模型容易受到对抗攻击，但大多数现有补丁方法过拟合单一模型，在黑盒设置下失效，缺乏通用性和可迁移性。

Method: 结合特征空间目标、鲁棒性增强的两阶段min-max优化、以及VLA特定的注意力劫持和语义错配损失，在共享特征空间中学习单一物理补丁。

Result: 实验表明UPA-RFAS在不同VLA模型、操作任务和物理执行中都能实现一致的跨模型迁移，暴露了基于补丁的实用攻击面。

Conclusion: 该工作为VLA模型的安全防御建立了强基准，揭示了补丁攻击的实际威胁。
...
### [66] [Referring Video Object Segmentation with Cross-Modality Proxy Queries](https://arxiv.org/abs/2511.21139)
*Baoli Sun,Xinzhu Ma,Ning Wang,Zhihui Wang,Zhiyong Wang*

Main category: cs.CV

TL;DR: ProxyFormer是一个新的视频对象分割架构，通过引入代理查询来整合视觉和文本语义，解决现有方法在跨帧依赖性和文本约束集成方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有的基于条件查询的方法存在两个主要问题：(1) 条件查询缺乏帧间依赖性和变化建模，在帧间变化显著时难以准确跟踪目标；(2) 文本约束集成过晚，可能导致视频特征关注非目标对象。

Method: 提出ProxyFormer架构，引入一组代理查询来整合视觉和文本语义，通过多阶段视频特征编码器逐步更新和传播代理查询。采用时空维度解耦的跨模态交互来降低计算成本，并设计联合语义一致性训练策略。

Result: 在四个广泛使用的RVOS基准测试上的综合实验表明，ProxyFormer优于最先进的方法。

Conclusion: ProxyFormer通过代理查询机制有效解决了跨帧依赖性和文本约束集成问题，在视频对象分割任务中表现出优越性能。
...
TL;DR: BotaCLIP是一个轻量级多模态对比学习框架，通过将高分辨率航拍图像与植物样方数据对齐，来适应预训练的地球观测基础模型，从而注入植物学领域知识。
...
Method: 采用对比学习框架，将高分辨率航拍图像与植物样方数据对齐，并使用正则化策略来减轻灾难性遗忘问题。
...
### [68] [TEAR: Temporal-aware Automated Red-teaming for Text-to-Video Models](https://arxiv.org/abs/2511.21145)
*Jiaming He,Guanyu Hou,Hongwei Li,Zhicong Huang,Kangjie Chen,Yi Yu,Wenbo Jiang,Guowen Xu,Tianwei Zhang*

Main category: cs.CV

TL;DR: TEAR是一个针对文本到视频模型安全性的自动化红队框架，通过利用时间动态性生成看似无害但能引发违规视频内容的提示，攻击成功率超过80%。


<details>
  <summary>Details</summary>
Motivation: 现有的安全评估方法主要关注静态图像和文本生成，无法捕捉视频生成中复杂的时间动态性，因此需要专门针对T2V模型时间序列安全风险的评估框架。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要处理两模态对齐，忽略了多模态间的高阶交互，或无法充分保持配对关系，限制了在单模态任务上的有效性。

Method: ConFu扩展传统配对对比目标，增加融合模态对比项，将单个模态及其融合组合嵌入统一表示空间，鼓励模态对与第三模态的联合嵌入。

Result: 在合成和真实多模态基准测试中，ConFu在检索和分类任务上表现出竞争力，支持统一的一对一和两对一检索。

Conclusion: ConFu能够捕捉仅通过配对对齐无法恢复的高阶依赖关系（如XOR类关系），同时保持强配对对应性，为多模态学习提供了有效框架。
...
### [70] [Hybrid SIFT-SNN for Efficient Anomaly Detection of Traffic Flow-Control Infrastructure](https://arxiv.org/abs/2511.21337)
*Munish Rathee,Boris Bačić,Maryam Doborjeh*

Main category: cs.CV

TL;DR: SIFT-SNN框架：一种低延迟神经形态信号处理管道，用于实时检测交通基础设施中的结构异常，结合SIFT空间特征编码和LIF脉冲神经网络，在奥克兰海港大桥数据集上达到92.3%的分类准确率，每帧推理时间9.5毫秒。


<details>
  <summary>Details</summary>
Motivation: 开发实时、低功耗的边缘部署系统，用于交通基础设施的结构安全监测，相比传统CNN方法提供更好的空间特征保持和可解释性。

Method: 集成尺度不变特征变换(SIFT)进行空间特征编码，结合延迟驱动的脉冲转换层和泄漏积分点火(LIF)脉冲神经网络(SNN)进行分类。

Result: 在奥克兰海港大桥数据集上达到92.3%的分类准确率，每帧推理时间9.5毫秒，稀疏脉冲活动率为8.1%，支持实时低功耗边缘部署。

Conclusion: SIFT-SNN框架在结构安全监测中表现出色，具有高准确率和低延迟，但泛化到未见现场条件仍需验证，可作为可移动混凝土屏障等交通流控制基础设施的通用案例研究。
...
### [71] [SurgMLLMBench: A Multimodal Large Language Model Benchmark Dataset for Surgical Scene Understanding](https://arxiv.org/abs/2511.21339)
*Tae-Min Choi,Tae Kyeong Jeong,Garam Kim,Jaemin Lee,Yeongyoon Koh,In Cheul Choi,Jae-Ho Chung,Jong Woong Park,Juyoun Park*

Main category: cs.CV

TL;DR: 提出了SurgMLLMBench，一个统一的多模态基准测试，用于开发和评估交互式多模态LLM在手术场景理解中的应用，包含新收集的MAVIS数据集，整合了像素级器械分割和结构化VQA注释。


<details>
  <summary>Details</summary>
Motivation: 现有手术数据集主要采用VQA格式，存在分类体系异构且缺乏像素级分割支持的问题，限制了评估的一致性和适用性。

Method: 创建统一的SurgMLLMBench基准，整合腹腔镜、机器人辅助和显微手术领域的像素级器械分割掩码和结构化VQA注释，采用统一分类体系。
...
### [72] [AnchorOPT: Towards Optimizing Dynamic Anchors for Adaptive Prompt Learning](https://arxiv.org/abs/2511.21188)
...
<details>
  <summary>Details</summary>
Motivation: 现有提示学习方法使用静态锚点（文本标记）来指导可学习软标记，但这些锚点在值和位置上都缺乏跨任务和阶段自适应的灵活性。

Method: 提出AnchorOPT框架：1）锚点值从任务特定数据中动态学习，而非手工设计；2）锚点与软标记的位置关系通过可学习位置矩阵自适应优化。采用两阶段训练：先学习锚点标记，然后冻结并转移到第二阶段优化软标记和位置矩阵。

Result: 广泛实验表明，仅使用简单的可学习锚点和位置矩阵就能达到或超过一些包含额外可学习模块或正则化技术的方法的性能。

Conclusion: AnchorOPT作为一个即插即用模块，能无缝集成到现有框架中，在不同数据集上带来一致的性能提升。
...
### [73] [Monet: Reasoning in Latent Visual Space Beyond Images and Language](https://arxiv.org/abs/2511.21395)
*Qixun Wang,Yang Shi,Yifei Wang,Yuanxing Zhang,Pengfei Wan,Kun Gai,Xianghua Ying,Yisen Wang*

Main category: cs.CV

TL;DR: Monet是一个训练框架，使多模态大语言模型能够在潜在视觉空间中进行推理，通过生成作为中间视觉思维的连续嵌入。该框架解决了潜在视觉推理中的计算成本和监督不足问题，并提出了VLPO强化学习方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法在视觉推理中缺乏人类般的抽象视觉思维能力，其灵活性受到外部工具的限制。

Method: 采用三阶段蒸馏式监督微调(SFT)管道，构建了包含12.5万条真实世界、图表、OCR和几何思维链的数据集，并提出VLPO强化学习方法。
...
### [74] [Scenes as Tokens: Multi-Scale Normal Distributions Transform Tokenizer for General 3D Vision-Language Understanding](https://arxiv.org/abs/2511.21191)
*Yutao Tang,Cheng Zhao,Gaurav Mittal,Rohith Kukkala,Rama Chellappa,Cheng Peng,Mei Chen*

Main category: cs.CV

TL;DR: NDTokenizer3D是一个通用的3D视觉语言模型，通过多尺度NDT表示和渐进式特征融合，实现了从点云到整体场景标记的有效转换，统一支持多种3D场景理解任务和人类交互。


<details>
  <summary>Details</summary>
Motivation: 当前3D视觉语言模型在场景标记化和跨任务应用方面面临挑战，需要一种能够有效处理高分辨率点云并支持多样化3D理解任务的统一方法。

Method: 采用三阶段场景标记化流程：1) 从原始点云构建多尺度NDT表示；2) 使用多尺度NDT解码器渐进融合跨尺度特征生成场景标记；3) 将解码器重新用于人类交互提示和分割掩码解码。

Result: 在3D参考分割、3D视觉问答和3D密集描述等任务上取得了显著改进，实现了细粒度的通用3D视觉语言理解。

Conclusion: NDTokenizer3D提供了一个紧凑统一的架构，成功连接了语言级推理与3D空间理解，为通用3D场景理解任务提供了有效解决方案。
...
### [75] [Do Reasoning Vision-Language Models Inversely Scale in Test-Time Compute? A Distractor-centric Empirical Analysis](https://arxiv.org/abs/2511.21397)
...
Method: 创建了Idis数据集，系统地在语义、数值和空间维度上变化视觉干扰项，并通过分析推理轨迹中的属性计数来研究干扰项、推理长度和准确率的相互作用。

Result: 视觉干扰项确实导致逆向扩展效应，但不同于文本干扰项，它们会降低准确率而不增加推理长度。这些趋势也适用于已建立的视觉偏见基准测试。

Conclusion: 提出了简单的提示策略来减轻推理模型中的偏见驱动预测，揭示了视觉干扰项对视觉语言模型性能的独特影响机制。
...
TL;DR: 本文提出了一种基于SAM基础模型的遥感变化描述方法，通过提取区域级表示和注入兴趣区域知识来解决现有方法区域意识弱和时间对齐有限的问题。


<details>
  <summary>Details</summary>
Motivation: 现有遥感变化描述方法通常使用CNNs/Transformers提取视觉表示或引入辅助任务，但存在区域意识弱和时间对齐有限的问题。

Method: 使用CNN/Transformer提取全局视觉特征，利用SAM基础模型划分语义和运动级变化区域，构建知识图谱提供兴趣对象信息，通过交叉注意力融合异构信息，最后用Transformer解码器生成自然语言描述。

Result: 在多个广泛使用的基准数据集上实现了最先进的性能。

Conclusion: 提出的基于SAM的方法有效解决了遥感变化描述中的区域意识和时间对齐问题，取得了优异性能。
...
### [77] [You Can Trust Your Clustering Model: A Parameter-free Self-Boosting Plug-in for Deep Clustering](https://arxiv.org/abs/2511.21193)
...
<details>
  <summary>Details</summary>
Motivation: 现有深度聚类方法存在全局特征和局部特征结构之间的差异问题。局部特征在类内样本中表现出强一致性和紧凑性，而全局特征往往呈现交织边界和分离不良的聚类。
...
Conclusion: DCBoost是一种有效的参数自由插件方法，能够利用局部结构线索增强全局特征结构，显著提升深度聚类模型的性能。
...
TL;DR: 提出了一种从工业视频流中自动提取VLA预训练数据的无监督框架，包括运动分词器和基于潜在动作能量的动作分割器，能够发现语义一致的动作基元。


<details>
  <summary>Details</summary>
Motivation: 为了解决工业环境中大量未标记人类演示数据无法有效用于VLA模型预训练的问题，需要自动化地从非结构化工业视频中提取结构化数据。

Method: 首先训练轻量级运动分词器编码运动动态，然后使用基于潜在动作能量指标的无监督动作分割器发现和分割语义一致的动作基元。

Result: 在公共基准和专有电机装配数据集上的评估表明，能够有效分割工作台中人类执行的关键任务，VLM评估确认了发现动作基元的语义一致性。

Conclusion: 这是首个从非结构化工业视频中端到端自动提取和组织VLA预训练数据的全自动系统，为制造业中具身AI集成提供了可扩展解决方案。
...
Method: 使用超图引导的时空事件流补全机制，通过超图连接事件标记并利用上下文信息传递进行补全，可灵活融入RGB标记实现多模态信息补全，然后通过自注意力聚合不同时间步的超图节点信息。

Result: 在单标签和多标签事件分类任务上的大量实验充分验证了所提框架的有效性。

Conclusion: 提出的超图引导事件流补全机制能有效解决事件数据的空间稀疏性问题，并支持多模态特征融合，在事件分类任务上表现出色。
...
### [80] [Towards an Effective Action-Region Tracking Framework for Fine-grained Video Action Recognition](https://arxiv.org/abs/2511.21202)
*Baoli Sun,Yihan Wang,Xinzhu Ma,Zhihui Wang,Kun Lu,Zhiyong Wang*

Main category: cs.CV

TL;DR: 提出ART框架，通过查询-响应机制跟踪局部细节动态，解决细粒度动作识别中难以捕捉细微差异的问题


<details>
  <summary>Details</summary>
Motivation: 当前动作识别方法主要捕获粗粒度运动模式，难以识别随时间演变的局部区域细微差异

Method: 使用区域特定语义激活模块，以文本约束语义作为查询捕获动作相关区域响应，组织成动作轨迹，并设计多级轨迹对比约束

Result: 在广泛使用的动作识别基准测试中表现出优于先前最先进基线的性能

Conclusion: ART框架通过跟踪局部细节动态有效区分相似动作，在细粒度动作识别任务中取得显著效果
...
### [81] [Frequency-Aware Token Reduction for Efficient Vision Transformer](https://arxiv.org/abs/2511.21477)
*Dong-Jae Lee,Jiwan Hur,Jaehyun Choi,Jaemyung Yu,Junmo Kim*

Main category: cs.CV

TL;DR: 提出一种频率感知的token缩减策略，通过将token分为高频和低频两类，选择性保留高频token，将低频token聚合为紧凑的直流token，在提高计算效率的同时缓解秩崩溃问题。


<details>
  <summary>Details</summary>
Motivation: Vision Transformers的二次计算复杂度是主要挑战，现有token缩减方法忽略了自注意力的频率特性，如秩崩溃和过平滑现象。

Method: 将token划分为高频和低频两类，选择性保留高频token，将低频token聚合为紧凑的直流token，保留必要的低频分量。

Result: 通过大量实验和分析，证明该方法在减少计算开销的同时显著提高准确性，并缓解秩崩溃和过平滑问题。

Conclusion: 提出的频率感知token缩减策略有效平衡了计算效率和性能，同时揭示了先前方法的隐含频率特性和局限性。
...
### [82] [From Diffusion to One-Step Generation: A Comparative Study of Flow-Based Models with Application to Image Inpainting](https://arxiv.org/abs/2511.21215)
*Umang Agarwal,Rudraksh Sangore,Sumit Laddha*

Main category: cs.CV

TL;DR: 比较DDPM、CFM和MeanFlow三种生成模型，CFM在50步采样时FID为24.15，显著优于DDPM的402.98；MeanFlow实现单步采样，FID为29.15，推理时间减少50倍。CFM扩展至图像修复，在多种掩码类型下PSNR和SSIM显著提升。


<details>
  <summary>Details</summary>
Motivation: 系统比较三种生成建模范式，探索高效采样方法，并将条件流匹配扩展到图像修复任务。

Method: 使用统一的TinyUNet架构（<150万参数）在CIFAR-10上实现DDPM、CFM和MeanFlow，CFM采用迭代采样，MeanFlow通过建模时间间隔内的平均速度实现单步生成。扩展CFM至图像修复，实现四种掩码类型的掩码引导采样。

Result: CFM在50步采样时FID为24.15，DDPM为402.98；MeanFlow单步采样FID为29.15。图像修复模型在中心掩码上PSNR从4.95提升至8.57 dB（+73%），SSIM从0.289提升至0.418（+45%）。

Conclusion: CFM在生成质量上显著优于DDPM，MeanFlow在保持良好质量的同时大幅减少推理时间。CFM扩展至图像修复任务效果显著，修复感知训练策略有效。
...
### [83] [Merge and Bound: Direct Manipulations on Weights for Class Incremental Learning](https://arxiv.org/abs/2511.21490)
...
<details>
  <summary>Details</summary>
Motivation: 解决类增量学习中的灾难性遗忘问题，通过直接操作模型权重来优化模型性能，同时保持对先前任务的知识。

Method: 使用两种权重合并：任务间权重合并（平均所有先前阶段模型的权重）和任务内权重合并（组合当前阶段内的模型参数）。采用有界更新技术，以最小累积更新优化目标模型并保留先前任务知识。

Result: 在标准CIL基准测试中进行了广泛评估，相比最先进方法表现出优越性能。

Conclusion: M&B方法能够有效减少灾难性遗忘，获得接近旧模型的新模型，且无需修改架构组件或修订学习目标即可集成到现有CIL方法中。
...
### [84] [3-Tracer: A Tri-level Temporal-Aware Framework for Audio Forgery Detection and Localization](https://arxiv.org/abs/2511.21237)
...
### [85] [Multimodal Robust Prompt Distillation for 3D Point Cloud Models](https://arxiv.org/abs/2511.21574)
...
<details>
  <summary>Details</summary>
Motivation: 现有防御方法存在计算开销大和泛化能力差的问题，无法有效应对多样化攻击类型
...
### [86] [FIELDS: Face reconstruction with accurate Inference of Expression using Learning with Direct Supervision](https://arxiv.org/abs/2511.21245)
*Chen Ling,Henglin Shi,Hedvig Kjellström*

Main category: cs.CV

TL;DR: FIELDS是一个3D人脸重建方法，通过结合3D表情参数监督和情感识别分支，从单张图像重建保留细微情感线索的高保真3D人脸模型。
...
Method: 扩展自监督2D图像一致性线索，加入直接3D表情参数监督和辅助情感识别分支。使用来自自发4D面部扫描的真实表情参数指导编码器，强度感知情感损失确保3D表情参数捕捉真实情感内容。

Result: 从单张图像生成情感丰富的3D人脸模型，具有高度真实的表情，显著提高野外面部表情识别性能，同时保持自然度。

Conclusion: 双重监督策略成功弥合2D/3D领域差距，减轻表情强度偏差，产生保留细微情感线索的高保真3D重建结果。
...
### [87] [Qwen3-VL Technical Report](https://arxiv.org/abs/2511.21631)
...
TL;DR: Qwen3-VL是Qwen系列中最强大的视觉语言模型，在多种多模态基准测试中表现优异，支持256K令牌的交错上下文，包含密集和混合专家变体，在文本理解、长上下文理解和多模态推理方面具有三大核心优势。


<details>
  <summary>Details</summary>
Motivation: 开发一个在文本理解、长上下文处理和多模态推理方面都表现卓越的视觉语言模型，以适应现实工作流程中的图像推理、智能决策和多模态代码智能需求。

Method: 采用增强的交错MRoPE进行时空建模，集成DeepStack以利用多级ViT特征加强视觉语言对齐，以及基于文本的时间对齐技术从T-RoPE演进到显式文本时间戳对齐。

Result: 在可比较的令牌预算和延迟约束下，Qwen3-VL在密集和混合专家架构中都实现了卓越性能，在MMMU、MathVista和MathVision等综合评估中表现领先。
...
### [88] [Shift-Equivariant Complex-Valued Convolutional Neural Networks](https://arxiv.org/abs/2511.21250)
*Quentin Gabot,Teck-Yian Lim,Jérémy Fix,Joana Frontera-Pons,Chengfang Ren,Jean-Philippe Ovarlez*

Main category: cs.CV

TL;DR: 本文扩展了可学习多相采样(LPS)方法到复数神经网络，通过添加从复数到实数的投影层，在理论上保证平移等变性和不变性，并在极化合成孔径雷达图像上验证了性能。
...
Method: 将可学习多相采样(LPS)扩展到复数神经网络，添加从复数到实数的投影层，在Gumbel Softmax之前进行投影，确保平移等变性和不变性。

Result: 在多个计算机视觉任务中评估了该方法，包括分类任务中的不变性性质，以及重建和语义分割问题中的等变性性质，使用极化合成孔径雷达图像进行验证。

Conclusion: 扩展的LPS方法成功应用于复数神经网络，在理论上保证了平移等变性和不变性，并在实际计算机视觉任务中展示了良好性能。
...
### [89] [Continual Error Correction on Low-Resource Devices](https://arxiv.org/abs/2511.21652)
...
TL;DR: 提出了一种通过少样本学习在资源受限设备上高效纠正AI分类错误的系统，结合服务器端基础模型训练和设备端原型分类，实现原型更新而非模型重训练的错误纠正。


<details>
  <summary>Details</summary>
Motivation: AI模型在日常设备中的普及面临预测错误降低用户体验的挑战，现有解决方案主要关注错误检测而缺乏高效的纠正机制，特别是在资源受限设备上。

Method: 系统包含两个关键组件：(1)服务器端利用知识蒸馏将基础模型的鲁棒特征表示迁移到设备兼容架构；(2)设备端通过原型适配实现超高效错误纠正，使用原型更新而非模型重训练。

Result: 在Food-101和Flowers-102数据集上，单样本场景下实现超过50%的错误纠正率，同时保持极低的遗忘率（小于0.02%）和可忽略的计算开销。通过Android演示应用验证了系统的实用性。

Conclusion: 该系统为资源受限设备提供了一种实用高效的AI错误纠正方案，通过原型更新机制在保持低计算开销的同时显著提升分类准确性。
...
### [90] [AVFakeBench: A Comprehensive Audio-Video Forgery Detection Benchmark for AV-LMMs](https://arxiv.org/abs/2511.21251)
...
### [91] [Attention-Guided Patch-Wise Sparse Adversarial Attacks on Vision-Language-Action Models](https://arxiv.org/abs/2511.21663)
*Naifu Zhang,Wei Tao,Xi Xiao,Qianpu Sun,Yuxin Zheng,Wentao Mo,Peiqiang Wang,Nan Zhang*

Main category: cs.CV

TL;DR: 提出了ADVLA框架，通过在视觉编码器投影到文本特征空间的特征上直接应用对抗性扰动，有效破坏VLA模型的下游动作预测，具有低幅度、稀疏性和高效性。


<details>
  <summary>Details</summary>
Motivation: 解决现有对抗攻击方法需要昂贵端到端训练且生成明显扰动补丁的问题，开发更高效、隐蔽的攻击方法。

Method: 在视觉编码器投影到文本特征空间的特征上直接施加对抗性扰动，结合注意力引导使扰动集中且稀疏，采用三种策略增强敏感性、强制稀疏性和集中扰动。
...
### [92] [LaGen: Towards Autoregressive LiDAR Scene Generation](https://arxiv.org/abs/2511.21256)
...
Conclusion: LaGen是首个支持长序列LiDAR场景逐帧自回归生成的框架，在生成质量和交互能力方面显著优于现有方法。
...
TL;DR: G²VLM是一个几何基础的视觉语言模型，通过整合3D视觉几何特征来增强空间理解和推理能力，在3D重建和空间理解任务上都表现出色。
...
Method: 提出G²VLM模型，通过多视角图像和视频数据学习3D视觉几何特征，利用上下文学习和交错推理来直接预测3D属性并增强空间推理任务。
...
TL;DR: MatchGS是一个利用3D高斯泼溅技术生成高质量图像匹配训练数据的框架，通过几何校正和2D-3D表示对齐，显著提升了图像匹配器的零样本性能。


<details>
  <summary>Details</summary>
Motivation: 基于学习的图像匹配需要大规模、多样化且几何准确的训练数据，但现有3D高斯泼溅技术存在几何不准确和深度渲染偏差问题，限制了其在数据生成中的应用。

Method: 1) 几何准确的数据生成流水线，精炼3DGS几何以产生精确的对应标签；2) 2D-3D表示对齐策略，将3DGS的显式3D知识注入2D匹配器。

Result: 生成的对应标签将极线误差降低了40倍，在公共基准测试中零样本性能提升高达17.7%。
...
<details>
  <summary>Details</summary>
Motivation: 随着Transformer在单个遥感任务上的出色表现，研究者希望开发一个统一模型来通过多任务学习在多个任务上表现优异。多任务学习方法相比单任务方法具有更好的泛化性、可扩展性和实用性。

Method: 1. 创建数据管理引擎（数据获取、离线处理与集成、在线加载与加权）
2. 提出统一动态分辨率策略处理遥感图像的不同尺度
3. 针对超高分辨率图像引入Zoom-in Chain机制和对应数据集LRS-VQA-Zoom
...
<details>
  <summary>Details</summary>
Motivation: 现有基于Vision Transformer的方法在道路分割中虽然能捕捉全局上下文，但二次复杂度限制了在资源受限平台上的部署效率。Mamba等状态空间模型具有线性时间效率，更适合建模长连续结构。
...
TL;DR: CaliTex是一个解决3D纹理生成中跨视图不一致性的框架，通过几何校准注意力机制显式对齐3D结构，包含部件对齐注意力和条件路由注意力两个模块。


<details>
  <summary>Details</summary>
Motivation: 当前基于扩散模型的3D纹理生成系统存在跨视图不一致问题，即从一个视角看合理的纹理在其他视角无法对齐，这源于注意力模糊问题。

Method: 引入几何校准注意力框架，包含部件对齐注意力（强制语义匹配部件间的空间对齐）和条件路由注意力（通过几何条件路径路由外观信息以保持空间保真度），结合两阶段扩散变换器。

Result: CaliTex能够生成无缝且视图一致的纹理，在实证中优于开源和商业基线模型。
...
<details>
  <summary>Details</summary>
Motivation: VGGT模型在重建大型场景时，由于需要全局注意力计算所有视图的token，导致显著的延迟瓶颈。现有合并方法在不同注意力头中统一合并token，会降低模型的表示能力。
...
TL;DR: 提出一种基于多尺度特征融合的点云法向量估计方法，通过特征聚合和补偿模块实现尺度自适应，在减少参数和运行时间的同时达到最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以在不同数据和几何形状下准确高效地预测点云法向量，主要问题在于难以确定合适的邻域大小。

Method: 使用多尺度特征融合解决邻域大小选择问题，通过特征聚合模块逐步聚合不同尺度的特征，特征补偿模块确保大尺度特征的复用性。

Result: 在合成和真实数据集上均达到最先进性能，且网络参数和运行时间更少。

Conclusion: 基于多尺度特征聚合的近似策略能够实现局部块尺度自适应，提供最优特征描述，显著提升法向量估计的准确性和效率。
...
TL;DR: Endo-G²T是一个针对动态内窥镜场景的几何引导和时间感知训练方案，通过几何先验蒸馏、时间嵌入高斯场和关键帧约束流式处理，解决了内窥镜视频中视角依赖性效应导致的几何漂移问题。


<details>
  <summary>Details</summary>
Motivation: 内窥镜视频存在强烈的视角依赖性效应（如镜面反射、湿润反射和遮挡），纯光度监督会导致几何漂移问题，错误的形状在密集化过程中被强化且难以纠正。
...
Conclusion: 该方法成功解决了动态内窥镜场景中的几何漂移问题，实现了时间一致的几何重建，同时保持了效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型在时空视频定位任务中表现不佳，主要原因是训练目标不对齐和标准视觉编码器中细粒度区域-词语对齐能力弱。

Method: 提出边界框思维链机制，在最终预测前显式推理时空位置；设计包含格式、一致性、时间、空间和思考奖励的多维度强化奖励函数，通过强化微调提供几何感知监督。

Result: 在HCSTVG-v1/v2和VidSTG数据集上评估，STVG-o1在HCSTVG上创造了新的最先进结果，在HCSTVG-v1上m_tIoU比最佳任务特定方法高出7.3%，在VidSTG上与专用模型相当，大幅超越所有现有的基于MLLM的方法。
...
TL;DR: DiverseVAR是一个无需重新训练或微调即可增强文本条件视觉自回归模型多样性的框架，通过文本嵌入噪声注入和尺度旅行精炼技术解决VAR模型多样性不足的问题。


<details>
  <summary>Details</summary>
Motivation: VAR模型在图像生成中虽然质量优秀，但存在严重的多样性不足问题，即使对于简单提示也会生成几乎相同的图像。这一问题在追求图像质量的主流研究中被忽视。

Method: 采用两阶段方法：1）在文本嵌入中注入噪声以增强多样性；2）提出尺度旅行精炼技术，利用多尺度自编码器提取粗尺度标记，在中间阶段恢复生成以保持图像质量。

Result: 实验表明，文本嵌入噪声注入与尺度旅行精炼相结合能显著增强多样性，同时最小化图像质量损失，在多样性-质量权衡中达到新的帕累托前沿。

Conclusion: DiverseVAR框架成功解决了VAR模型的多样性限制，无需额外训练成本，在保持图像质量的同时显著提升了生成多样性。
...
<details>
  <summary>Details</summary>
Motivation: 现有学习方法主要依赖几何特征进行3D重组，但在几何信息不足（如碎片小、侵蚀或对称）时表现不佳，且缺乏防止重叠组装的物理约束。
...
Conclusion: 多模态特征融合能有效解决仅依赖几何特征时的重组困难，在合成和真实文化遗产数据集上均表现出优越性能。
...
Result: 首次在移动设备上实现快速720p图像到视频生成，质量与现有模型相当，在单步条件下每帧720p视频生成时间小于100毫秒，生成速度提升10倍。

Conclusion: MobileI2V成功解决了移动设备上实时视频生成的挑战，为资源受限环境下的高质量视频合成提供了可行方案。
...
Method: 引入交叉注意力机制，使学生特征图的每个像素都能动态考虑教师特征图的所有像素，仅通过额外的损失函数实现非局部知识迁移。
...
TL;DR: 系统研究深度伪造检测中不同设计选择的影响，识别出能持续提升检测性能的关键因素，并在AI-GenBench基准上实现最优性能


<details>
  <summary>Details</summary>
Motivation: 深度伪造检测方法的有效性往往更多依赖于实现细节而非核心设计，这使得难以公平比较检测器并理解真正影响性能的因素
...
TL;DR: 提出了一种新的抗核抗体(ANA)检测框架，通过实例采样器、概率伪标签分发器和自步学习权重系数来处理多实例多标签学习任务，在ANA数据集上取得了显著的性能提升。


<details>
  <summary>Details</summary>
Motivation: ANA检测对于诊断自身免疫性疾病至关重要，但传统手动检测方法缓慢且需要专业训练。虽然机器学习和深度学习已实现自动化，但真实临床环境中的ANA检测面临多实例多标签学习的独特挑战。

Method: 提出包含三个任务特定组件的框架：实例采样器通过建模模式置信度抑制低置信度实例；概率伪标签分发器基于实例可区分性自适应分配标签；自步学习根据经验标签观察调整训练。

Result: 在ANA数据集上，模型相比最佳先前方法实现了+7.0% F1-Macro和+12.6% mAP的提升，在公共医学MIML基准测试中所有关键指标均排名前2，汉明损失和单错误分别降低达18.2%和26.9%。

Conclusion: 该框架克服了传统MIML方法的限制，支持端到端优化，在ANA检测和医学MIML任务中表现出卓越性能，为临床自动化检测提供了有效解决方案。
...
TL;DR: 提出了一种用于构建遥感基础模型的高效集成专家框架，通过分解训练过程为轻量级任务特定专家，解决了当前大模型方法的计算资源需求和可持续性问题。


<details>
  <summary>Details</summary>
Motivation: 当前遥感基础模型主要依赖扩大模型规模和数据集，导致计算资源需求过高且不符合可持续AI原则，限制了可访问性和环境友好性。

Method: 采用集成专家框架，将训练过程分解为轻量级的任务特定ConvNeXtV2专家模型，支持冻结和重用，具有模块化特性。

Result: 该方法在效率、可解释性和可扩展性方面具有显著优势，支持联邦训练、剪枝和持续专家集成，特别适合资源受限的协作环境。

Conclusion: 该框架为构建可扩展且高效的遥感基础模型开辟了新方向，解决了当前大模型方法的资源效率问题。
...
Method: 采用定量指标引导的序列图像生成方法，集成年龄缩放因子来生成年龄特定的MRI图像，并使用年龄缩放像素损失来增强迭代生成效果。

Result: 消融研究表明，定量指标的加入显著提高了MRI图像合成的准确性，结构相似性指数达到0.882的峰值。
...
TL;DR: 提出了PRFL框架，在潜在空间进行偏好优化，解决了视频生成中奖励反馈学习的内存和效率问题。
...
Method: 利用预训练视频生成模型在噪声潜在空间进行奖励建模，提出PRFL框架，在潜在空间进行偏好优化，避免VAE解码，实现全去噪链的高效梯度反向传播。
...
TL;DR: UAVLight是一个用于光照鲁棒3D重建的基准数据集，通过在不同固定时间重复飞行采集数据，提供自然光照变化下的可控真实场景。


<details>
  <summary>Details</summary>
Motivation: 多视角3D重建中的光照不一致性问题严重影响了重建质量，特别是在无人机应用中，现有数据集要么缺乏有意义的光照多样性，要么包含几何和语义变化，无法单独研究光照鲁棒性。

Method: 通过在可重复的地理参考飞行路径上，在一天中的多个固定时间点采集场景数据，在保持几何、校准和视角一致的前提下产生自然光照变化。

Result: 创建了UAVLight基准数据集，提供了在真实室外环境中标准化评估协议，支持开发光照一致、忠实和可重光照的重建方法。

Conclusion: UAVLight为开发在真实室外环境中具有光照鲁棒性的3D重建方法提供了可靠的基础和基准测试平台。
...
Method: 在U-Net地标预测模型的训练中融入2D/3D地标配准，比较基线U-Net、使用姿态估计损失训练的U-Net以及使用姿态估计损失微调的U-Net的性能差异。

Result: 在患者姿态可变的真实手术条件下评估地标检测精度。

Conclusion: 该框架能够处理骨盆透视图像中常见的姿态变化问题，提高地标检测的鲁棒性。
...
TL;DR: Harmony是一个解决音频-视频同步生成挑战的新框架，通过交叉任务协同训练、全局-局部解耦交互模块和同步增强CFG，显著提升了生成质量和同步精度。
...
Conclusion: Harmony通过机制性地强制音频-视频同步，有效解决了联合扩散过程中的根本挑战，为同步音频-视觉内容合成提供了强有力的解决方案。
...
TL;DR: 使用深度学习构建16种口腔病变的多分类器，通过分层数据分割、数据增强和过采样技术解决数据不平衡问题，在口腔癌早期检测中表现出色。


<details>
  <summary>Details</summary>
Motivation: 口腔癌在全球非常常见，但由于口腔内良性、癌前病变和恶性病变的视觉相似性，大多在晚期才被诊断。早期实施计算机辅助诊断系统可显著改善临床结果。

Method: 结合分层数据分割、高级数据增强和过采样技术，构建深度学习多分类器来处理有限且不平衡的数据集。

Result: 实验结果显示83.33%的准确率、89.12%的精确率和77.31%的召回率，优于现有最先进方法，在少数类分类方面表现显著。

Conclusion: 该框架展示了过采样和增强策略在数据不平衡情况下的有效性，是构建可信赖口腔癌早期计算机辅助诊断系统的有前景的第一步。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型在帧级保真度上表现良好，但在运动连贯性、动态性和真实感方面仍存在抖动、重影或不可信动态等问题，标准去噪MSE目标缺乏对时间一致性的直接监督。

Method: 基于3步蒸馏视频扩散模型，训练基于DiT的光学流判别器来区分真实与生成的运动，结合分布匹配正则化器以保持视觉保真度。

Result: 在Wan2.1-T2V-1.3B上的实验显示，MoGAN显著提升运动质量：VBench运动分数比50步教师模型提升+7.3%，比3步DMD模型提升+13.3%；VideoJAM-Bench运动分数分别提升+7.4%和+8.8%，同时保持相当或更好的美学和图像质量分数。人类研究也证实MoGAN在运动质量上更受青睐。
...
<details>
  <summary>Details</summary>
Motivation: SAM模型在自然图像上表现出色，但在遥感图像上由于严重的领域偏移和密集标注稀缺而表现不佳，需要一种仅依赖稀疏点标注的有效适配方法。

Method: 采用Refine-Requery-Reinforce循环：从初始点生成粗伪掩码（Refine），使用自构建的框提示改进分割（Requery），通过跨迭代嵌入对齐减少确认偏差（Reinforce）。

Result: 在WHU、HRSID和NWPU VHR-10三个遥感图像基准数据集上的评估表明，该方法持续超越预训练SAM和最近的点监督分割方法。

Conclusion: 自提示和语义对齐为遥感应用中基础分割模型的可扩展点级适配提供了一条高效路径。
...
<details>
  <summary>Details</summary>
Motivation: 现有的图卷积网络在骨架动作识别中需要大量标注数据，但在实际应用中标注数据往往稀缺，因此需要开发标签高效的模型来减少对标注数据的依赖。

Method: 1. 开发新的获取函数，使用对抗策略选择信息丰富的代表性样本进行标注，平衡代表性、多样性和不确定性；2. 引入双向稳定的GCN架构，改善环境空间和潜在数据空间之间的映射，更好地理解学习到的样本分布。

Result: 在两个具有挑战性的骨架动作识别基准测试上进行了广泛评估，相比先前工作取得了显著改进。

Conclusion: 提出的标签高效GCN模型在减少标注数据需求的同时，在骨架动作识别任务中表现出优越性能，为实际应用提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 长时动作质量评估（如花样滑冰、艺术体操）需要建模长期时间动态，同时保持对上下文混淆因素的鲁棒性。现有方法依赖昂贵标注或单向时间建模，容易受到伪相关性和不稳定长期表示的影响。
...
Conclusion: CaFlow通过结合因果推理和双向时间建模，有效解决了长时动作质量评估中的混淆问题和时间建模挑战，为复杂动作评估提供了鲁棒且连贯的解决方案。
...
TL;DR: Multi-Crit是一个评估多模态模型作为评判者的基准测试，重点关注模型遵循多样化细粒度评估标准的能力。该研究揭示了现有模型在遵循多元标准方面的局限性，并为构建可靠的多模态AI评估系统奠定了基础。


<details>
  <summary>Details</summary>
Motivation: 尽管大型多模态模型越来越多地被用作多模态评估系统的评判者，但它们遵循多样化、细粒度评估标准的能力尚未得到充分探索。需要系统评估这些模型在多元标准下的表现。

Method: 开发Multi-Crit基准测试，通过严格的数据收集流程构建具有多标准人工标注的挑战性响应对。该基准涵盖开放式生成和可验证推理任务，并引入三个新指标来系统评估多元标准遵循能力。

Result: 对25个多模态模型的综合分析显示：1) 专有模型在遵循多元标准方面仍有困难；2) 开源模型在灵活遵循多样标准方面更落后；3) 使用整体判断信号进行批评微调能增强视觉基础，但无法泛化到多元标准级判断。

Conclusion: Multi-Crit为构建可靠和可引导的多模态AI评估奠定了基础，揭示了当前多模态评判者在遵循多元评估标准方面的局限性，为未来改进提供了方向。
...
Method: 使用U-Net进行3D MRI数据分割，结合DenseNet和VGG的混合分类网络，集成多头注意力和空间通道注意力机制，通过预处理步骤处理高维数据。

Result: 在肿瘤分割中获得98%的Dice系数，分类准确率达到99%，优于传统CNN模型和无注意力方法。
...
<details>
  <summary>Details</summary>
Motivation: 探索能否仅通过相机运动轨迹来理解视频内容，而不需要查看像素信息，这是一个看似不可能但具有实际意义的问题。
...
Method: 将多样控制信号编码为单一复合画布图像，采用多任务画布训练策略，在统一学习范式下优化扩散模型以联合理解和整合异构控制。

Result: 在多人组合、姿态控制组合、布局约束生成和多控制生成等挑战性基准测试中，Canvas-to-Image在身份保持和控制遵循方面显著优于最先进方法。

Conclusion: Canvas-to-Image通过统一的画布界面和多任务训练策略，实现了对多种控制信号的集成视觉空间推理，在复杂多控制场景下表现出优异的泛化能力。
...
Method: 提出基于提示的方法，使用LLM根据双曲嵌入的理想标准来转换现有层次结构。

Result: 在16个不同层次结构上的实验表明，LLM重构的层次结构在多个标准嵌入质量指标上都能产生更高质量的双曲嵌入。

Conclusion: LLM引导的层次结构重构不仅提高了双曲嵌入质量，还能提供可解释的重组理由，帮助知识工程师理解重组过程。
...
### [124] [AssurAI: Experience with Constructing Korean Socio-cultural Datasets to Discover Potential Risks of Generative AI](https://arxiv.org/abs/2511.20686)
...
<details>
  <summary>Details</summary>
Motivation: 当前的安全数据集主要是英语中心化的，无法捕捉非英语社会文化背景（如韩语）中的特定风险，且通常仅限于文本模态，需要专门针对韩语多模态的安全评估工具。

Method: 1) 定义35种AI风险因素分类法；2) 构建大规模韩语多模态数据集；3) 采用两阶段构建（专家引导播种+众包扩展）、三重独立标注和迭代专家红队测试的严格质量控制流程。

Result: 试点研究验证了AssurAI在评估最新LLMs安全性方面的有效性，数据集已公开发布以促进更安全可靠的韩语生成AI系统开发。

Conclusion: AssurAI填补了韩语多模态AI安全评估的空白，通过精心设计的风险分类和质量控制流程，为韩语社区提供了有效的安全评估工具。
...
### [126] [EWE: An Agentic Framework for Extreme Weather Analysis](https://arxiv.org/abs/2511.21444)
...
<details>
  <summary>Details</summary>
Motivation: 极端天气事件对全球社会构成日益严重的风险，但传统专家驱动的人工诊断范式存在分析瓶颈，阻碍科学进展。AI在地球科学中的预测应用已有进展，但自动诊断推理这一同等重要的挑战尚未被充分探索。
...
Conclusion: EWE标志着向自动科学发现迈出的一步，具有民主化专业知识和智力资源的潜力，特别对易受极端天气影响的发展中国家具有重要意义。
...
TL;DR: 提出了一个名为"Reasoning With a Star"的太阳物理学推理数据集和基准测试方法，通过多智能体模式提升科学推理能力，发现基于系统工程原理的分解工作流在演绎推理问题上表现优于直接提示。


<details>
  <summary>Details</summary>
Motivation: 解决太阳物理学中科学推理的挑战，包括整合物理假设、保持单位一致性、提供清晰科学格式，而不仅仅是事实回忆。

Method: 构建了基于NASA和UCAR"Living With a Star"暑期学校问题集的数据集，包含问题上下文、推理步骤、答案类型、真实目标等；开发了程序化评分器，使用单位感知数值容差、符号等价性和模式验证；测试了单次基线和四种多智能体模式。

Result: 通过系统工程原理分解工作流在需要演绎推理而非纯归纳回忆的问题上表现优于直接提示方法。

Conclusion: 多智能体协作和系统工程方法能够有效提升科学推理能力，特别是在复杂的演绎推理任务中。
...
TL;DR: 数字孪生技术从NASA航天器模拟发展而来，现正通过整合影像、生物传感器和计算模型，在医疗领域实现从反应性治疗向预测性、预防性和个性化医学的转变。


<details>
  <summary>Details</summary>
Motivation: 推动医疗从反应性治疗向预测性、预防性和个性化医学转变，利用数字孪生技术解决临床诊断、治疗规划和药物开发中的挑战。

Method: 整合医学影像、生物传感器和计算模型，构建患者特定的动态虚拟副本，通过实时数据流进行持续更新，支持双向交互。
...
TL;DR: Paraconsistent-Lib是一个开源的Python库，用于构建PAL2v算法，支持12种经典格区域的分析、分析节点输出和决策输出，简化了算法实现。
...
Result: 实现了稳定的库状态，能够响应用户在GitHub上提出的功能需求和改进建议。

Conclusion: Paraconsistent-Lib是一个活跃开发的开源工具，有效简化了PAL2v算法的实现过程。
...
TL;DR: 本文对多模态思维链(Multimodal-CoT)推理进行了全面分析，评估其在A-OKVQA、OKVQA和ChartQA数据集上的有效性，发现CoT推理效果在不同问题类型间差异显著，常识推理尤其具有挑战性。


<details>
  <summary>Details</summary>
Motivation: 虽然CoT已在多模态场景中取得进展，但其在不同领域的泛化能力仍有待探索。本文旨在评估多模态CoT在需要广泛常识和世界知识的非科学推理任务中的表现。

Method: 采用Zhang等人提出的两阶段框架，将理由生成与答案推理分离，通过门控融合机制将视觉特征与基于T5的语言模型集成，并进行系统消融研究分析视觉特征、理由质量和架构选择的影响。

Result: 视觉集成显著减少了理由生成中的幻觉，但CoT推理效果在不同问题类型间差异很大，常识推理特别具有挑战性。

Conclusion: 本研究为实施多模态推理系统提供了实用见解，并确定了跨领域泛化改进的关键方向。
...
TL;DR: 提出了一个基于大型语言模型代理的智能Wi-Fi框架，使接入点能够通过自然语言对话实时协商自适应协调策略，显著提升密集网络环境下的性能表现。
...
Conclusion: 该框架验证了其作为未来无线网络稳健智能解决方案的潜力，能够实现实时的自适应协调策略协商。
...
TL;DR: OpenApps是一个轻量级开源生态系统，用于评估多模态UI代理在不同应用变体中的可靠性。研究发现代理在固定应用中的表现相对稳定，但在不同应用变体中的成功率波动可达50%以上。
...
Method: 开发了包含6个应用（消息、日历、地图等）的OpenApps生态系统，可配置外观和内容，在单个CPU上运行数千个应用版本，进行了超过10,000次独立评估。

Result: 代理在固定应用中的可靠性相对稳定，但在应用变体中的成功率波动很大（如Kimi-VL-3B从63%降至4%），代理行为（如循环或幻觉操作）也因环境配置而异。

Conclusion: 测量代理在应用变体维度上的可靠性至关重要，OpenApps为这一新维度的评估提供了工具。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常产生错误或过时的内容，如何在终身学习设置中高效准确地更新知识而不需要昂贵的重新训练是一个主要挑战。

Method: 在表示空间进行知识干预，学习抗转述和编辑局部化的模块，将每个更新限制在低维子空间以减少交叉编辑干扰，推理时使用查询自适应路由选择适当模块。

Result: 在LLaMA和Qwen模型的知识编辑基准测试中，RILKE可扩展到大规模数据集，表现出高编辑成功率、强转述泛化能力，并以适度的内存开销保持通用效用。
...
TL;DR: 本文提出了第一个计算保证最优组合解释的框架，通过分解空间对齐因素、设计启发式估计和高效算法，解决了现有方法无法保证最优性的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于beam search的组合解释方法无法提供理论最优性保证，不清楚当前解释与真正最优解的接近程度。

Method: 提出三部分框架：(i)识别影响空间对齐因素的分解方法，(ii)搜索过程中估计对齐度的启发式方法，(iii)首个能在可行时间内计算最优组合解释的算法。

Result: 在计算机视觉和CNN领域，10-40%的beam search解释在涉及重叠概念时是次优的；提出的beam search变体在运行时间上匹配或优于现有方法，同时提供更大的超参数和计算资源灵活性。

Conclusion: 该框架首次实现了保证最优的组合解释计算，揭示了现有方法的局限性，并提供了更高效灵活的替代方案。
...
TL;DR: ENACT是一个评估视觉语言模型是否展现具身认知能力的基准测试，通过世界建模任务来检验模型从自我中心交互中理解环境的能力。


<details>
  <summary>Details</summary>
Motivation: 研究现代视觉语言模型是否在脱离实体的训练方式下仍能表现出具身认知的特征，即智能是否源于感知运动交互而非被动观察。

Method: 设计ENACT基准测试，将具身认知评估转化为基于自我中心交互的世界建模问题，采用视觉问答形式，包含正向世界建模（给定动作重排观察序列）和逆向世界建模（给定观察重排动作序列）两个互补任务。

Result: 实验发现前沿视觉语言模型与人类之间存在性能差距，且随着交互视野延长差距扩大；模型在逆向任务上表现优于正向任务，并表现出人类中心偏见，如偏好右手动作、当相机参数或视角偏离人类视觉时性能下降。

Conclusion: 现代视觉语言模型确实表现出一些具身认知的特征，但与人类相比仍有明显差距，特别是在长视野交互和视角适应性方面存在局限性。
...
TL;DR: Ivy是一个结合符号TMK模型和生成式LLM的AI教练系统，通过结构化约束提升程序技能学习中的解释质量。


<details>
  <summary>Details</summary>
Motivation: 解决LLM在程序技能学习中生成流畅但浅层解释的问题，需要传达步骤背后的因果、目标导向和组合逻辑。

Method: 结合符号Task-Method-Knowledge(TMK)模型与生成式LLM解释层，TMK编码因果转换、目标层次和问题分解，为LLM提供结构化约束。

Result: 与GPT和检索增强GPT基线相比，符号约束显著提升了"如何"和"为什么"问题解释的结构质量。

Conclusion: 展示了可扩展的AI教育方法，通过符号约束增强AI生成解释的教学价值。
...
TL;DR: 提出了ICPO方法，通过内在置信度驱动的群体相对偏好优化来增强大语言模型的推理能力，解决了现有RLVR方法中粗粒度奖励、奖励噪声和低效探索等问题。
...
Method: ICPO方法基于LLM生成不同响应的概率来反映其推理过程的自评估，通过计算偏好优势分数并与可验证奖励结合来指导探索过程。

Result: 在四个通用领域基准和三个数学基准上的综合实验表明，ICPO相比GRPO能稳定提升推理能力。

Conclusion: ICPO方法有效缓解了粗粒度奖励和奖励噪声问题，抑制了过度自信错误，增强了被低估高质量响应的相对优势，防止模型过度拟合特定策略，促进了更彻底的探索。
...
TL;DR: L4M框架结合对抗性LLM代理和SMT求解器验证，将自然语言解释的灵活性与符号验证的严谨性相结合，用于法律裁决。


<details>
  <summary>Details</summary>
Motivation: 现有LLM系统擅长表层文本分析，但缺乏原则性法理学所需的保证，无法同时满足实质理性（结果公平性）和形式理性（遵循明确规则）的要求。

Method: 三阶段流程：1) 法规形式化：将法律条款转换为逻辑公式；2) 双重事实和法规提取：检察官和辩护方对齐的LLM独立提取事实和法规；3) 求解器中心裁决：将双方论点编译为逻辑约束，通过不满足核心触发迭代自我批评，最终由法官LLM生成透明判决。

Result: 在公共基准测试中，该系统超越了GPT-4-mini、DeepSeek-V3、Claude 4等先进LLM以及最先进的法律AI基线，同时提供严谨且可解释的符号化理由。

Conclusion: L4M成功地将自然语言处理的灵活性与符号验证的严谨性相结合，为法律AI系统提供了既具有解释灵活性又具备形式保证的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有开放词汇目标检测方法虽然在多模态数据上预训练，但推理仍局限于固定类别名称，存在多模态训练与单模态推理的差距。文本空间仍有很大探索空间。

Method: 提出OVOD-Agent框架，采用视觉思维链进行可解释推理，将视觉上下文建模为弱马尔可夫决策过程，使用Bandit模块生成探索信号，并结合马尔可夫转移矩阵进行自监督奖励模型优化。

Result: 在COCO和LVIS数据集上的实验表明，OVOD-Agent在各种OVOD骨干网络上都能带来一致改进，特别是在稀有类别上表现突出。

Conclusion: 该框架通过主动视觉推理和自我演化检测机制，有效提升了开放词汇目标检测的性能，证明了所提方法的有效性。
...
Method: 通过抽象Halpern-Pearl因果定义的关键特征，构建一个适用于任何定义了反事实的模型的通用因果定义框架。

Result: 新定义不仅能在更广泛的模型中使用（包括允许回溯的模型），还能处理复杂逻辑公式，并能扩展到解释的定义。

Conclusion: 抽象化因果定义提供了更广泛的应用范围、更强的表达能力，并深化了对因果定义特征的理解。
...
TL;DR: Prune4Web是一个新颖的网页自动化范式，通过将DOM处理从资源密集的LLM读取转变为高效的程序化修剪，解决了复杂网页DOM结构过大的问题。


<details>
  <summary>Details</summary>
Motivation: 现有LLM网页代理在处理复杂真实网页时面临DOM结构过大的挑战（10,000-100,000个token），现有策略要么依赖粗略的DOM截断（可能丢失关键信息），要么使用低效的启发式方法和独立排序模型，无法在精度和可扩展性之间达到最佳平衡。

Method: 提出DOM树修剪编程方法，让LLM生成可执行的Python评分脚本来基于分解子任务的语义线索动态过滤DOM元素。该方法无需LLM读取原始庞大的DOM，而是将遍历和评分委托给轻量级、可解释的程序。同时提出了专门的数据标注管道和两轮对话训练策略，在统一框架内联合优化规划器、程序化过滤器和定位器。

Result: 实现了25倍到50倍的候选元素减少，便于精确的动作定位同时减轻注意力稀释。在低级别定位任务上，准确率从46.8%大幅提升至88.28%。

Conclusion: Prune4Web通过程序化DOM修剪方法，在真实网页自动化中实现了最先进的性能，显著提升了动作定位的准确性和效率。
...
<details>
  <summary>Details</summary>
Motivation: 当前伪布尔求解中最成功的单元传播策略是结合了监视文字方案和计数方法的混合模式，但现有的混合决策方法仍有优化空间。

Method: 引入了新的启发式方法，用于在混合单元传播策略中做出更好的决策选择。

Result: 新启发式方法能够显著超越当前方法，在RoundingSAT求解器中表现出大幅性能提升。

Conclusion: 提出的新启发式方法有效改进了伪布尔求解器中混合单元传播策略的决策质量，为求解器性能带来了显著提升。
...
TL;DR: ChatDRex是一个基于对话的多智能体系统，通过自然语言访问生物医学知识图谱，实现网络药物重定位预测，使非计算机专业的研究人员也能进行复杂的生物信息学分析。


<details>
  <summary>Details</summary>
Motivation: 传统药物重定位预测需要多领域专家协作，但现有工具分散且数据异构，难以整合到统一工作流中。需要开发能让临床专家无需计算机专业知识就能进行复杂分析的工具。

Method: 构建基于NeDRex知识图谱的多智能体系统，包括查询路由、数据检索、算法执行、结果可视化等专门智能体，配备推理模块进行幻觉检测，通过自然语言交互实现网络分析和药物重定位预测。

Result: 开发了ChatDRex系统，实现了自然语言访问生物医学知识图谱，整合了网络分析、药物重定位、功能一致性评估、文献挖掘等多种生物信息学分析功能。
...
TL;DR: MADRA是一个无需训练的多智能体辩论风险评估框架，通过集体推理增强安全意识，同时保持任务性能。该方法在AI2-THOR和VirtualHome上的实验显示，能够拒绝超过90%的不安全任务，同时保持对安全任务的低拒绝率。


<details>
  <summary>Details</summary>
Motivation: 确保具身AI代理在任务规划中的安全性对于现实世界部署至关重要，特别是在家庭环境中，危险指令会带来重大风险。现有方法存在计算成本高或过度拒绝的问题。

Method: 提出MADRA框架，使用多个基于LLM的智能体就给定指令的安全性进行辩论，由关键评估器根据逻辑合理性、风险识别、证据质量和清晰度对响应进行评分。通过迭代审议和共识投票来减少误拒绝。

Result: 在AI2-THOR和VirtualHome上的广泛实验表明，该方法能够拒绝超过90%的不安全任务，同时确保对安全任务的拒绝率较低，在安全性和执行效率方面均优于现有方法。

Conclusion: 这项工作为构建可信赖的具身代理提供了一个可扩展、模型无关的解决方案。
...
TL;DR: 提出了一个层次化空间认知框架，将空间智能分解为5个渐进复杂度级别，并构建了大规模基准SpatialBench来系统评估多模态大语言模型的空间推理能力。
...
Method: 构建了包含15个任务的SpatialBench基准，引入高层次能力导向的度量标准，对大规模MLLM进行广泛实验。

Result: 模型在感知层面表现良好，但在符号推理、因果推断和规划方面仍有限制；人类进行选择性目标导向抽象，而MLLM倾向于过度关注表面细节。
...
<details>
  <summary>Details</summary>
Motivation: 现有验证性能的主要限制在于错误检测能力，因此需要设计更有效的验证方法来提高数学问题的验证准确性。

Method: 悲观验证：为同一证明构建多个并行验证流程，只要其中任何一个报告错误就判定证明不正确。

Result: 该方法在多个数学验证基准测试中显著提升性能，token效率甚至超过扩展长链思维，且多数假阴性实际上源于原始数据集的标注错误。

Conclusion: 悲观验证能有效提高语言模型输出的可靠性，对实现长视野数学任务至关重要，有助于增强语言模型在各种数学任务中的能力。
...
TL;DR: 语言模型在专业领域无法可靠披露AI身份，导致用户无法信任其能力边界。研究发现模型在不同专业领域中的自我透明度存在显著差异，某些模型在推理优化后透明度反而降低。


<details>
  <summary>Details</summary>
Motivation: 在需要专业知识的领域，如果语言模型不能诚实地披露其AI身份，用户可能会错误地信任其能力，从而面临风险。研究旨在评估模型在不同专业角色中的自我透明度表现。

Method: 采用共同花园设计，对16个开放权重模型（4B-671B参数）进行了19,200次试验，评估模型在不同专业角色（如财务顾问、神经外科医生）中的身份披露行为。

Result: 模型在不同专业领域表现出显著不一致：财务顾问角色初始披露率为30.8%，而神经外科医生角色仅为3.5%。模型身份比参数数量更能预测行为表现。推理优化在某些模型中主动抑制了自我透明度。
...
TL;DR: 本文提出了"负责任计算性前瞻"概念，探讨人工智能在负责任前瞻中的作用，强调AI作为辅助工具支持人类决策，而非替代人类判断。


<details>
  <summary>Details</summary>
Motivation: 在技术快速发展和全球挑战日益复杂的时代，负责任前瞻成为政策制定者应对未来不确定性和塑造未来的重要框架。

Method: 建立负责任计算性前瞻的基础原则，并展示一套AI驱动的前瞻工具，结合模拟和情景分析来增强政策制定能力。

Result: AI能够增强政策制定者应对不确定性、评估风险和制定可持续战略的能力，但需要理解社会、环境、经济和政治系统的相互依存关系。

Conclusion: AI应作为负责任、以人为本的前瞻的支持工具，补充而非替代政策制定者的判断，以塑造有韧性和道德健全的未来。
...
<details>
  <summary>Details</summary>
Motivation: 研究LLMs在没有代码执行或其他工具辅助下的规划和状态推理能力，使用经典的8拼图任务进行精确评估。

Method: 测试四个模型在常见提示条件下（零样本、思维链、算法思维）和分层纠正反馈，并使用外部移动验证器提供有效移动。

Result: 反馈提高了某些模型-提示组合的成功率，但成功运行时间长且计算昂贵。即使有移动验证器辅助，所有模型都无法解决任何拼图。
...
<details>
  <summary>Details</summary>
Motivation: 解决AI/ML模型放大人类偏见的问题，并克服系统动力学和结构方程建模因不同基本假设而难以整合的障碍。

Method: 将系统动力学和结构方程建模整合到一个共同的数学框架中，用于生成系统分布、开发方法和比较结果。

Result: 建立了一个能够统一两种方法的数学框架，为数据科学和AI/ML应用提供系统动力学的认识论基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于轨迹的记忆方法存在简洁性偏差，逐渐丢失关键领域知识，且仅记录单模态行为轨迹，无法保留视觉注意力和逻辑推理如何共同贡献于解决方案，这与人类多模态整合的语义记忆不符。

Method: 提出双流记忆框架，分别编码视觉分心模式和逻辑推理错误，采用增长-精炼原则逐步积累和更新多模态语义知识，保持稳定可泛化策略同时避免灾难性遗忘。
