...
Motivation: 脉冲神经网络(SNN)在能效和实时处理方面具有显著优势，特别适合边缘计算和物联网应用。但由于其复杂性和硬件约束的相互影响，设计最优SNN架构面临重大挑战。

Method: 首先概述SNN的操作原理和与传统ANN的关键区别，然后简要介绍ANN的NAS技术现状，重点调查SNN特定的NAS方法，最后从硬件/软件协同设计角度探讨未来研究方向。

Result: 提供了SNNaS领域的全面综述，强调了协同设计方法在释放SNN全部潜力中的重要性，为研究人员和实践者提供了有价值的资源。

Conclusion: 硬件/软件协同设计对于充分发挥神经形态计算的潜力至关重要，本调查旨在为该领域提供整体视角，强调协同设计方法的重要性。
...
Method: 集成视觉语言模型和大语言模型，通过多轮视觉-文本对话进行协作推理，使用对象感知令牌捕获细粒度视觉属性，交互式推理代理动态解释上下文线索。

Result: 在多个公共食物数据集上的实验表明，MultiFoodChat相比现有的无监督和少样本方法具有更高的识别准确率和可解释性。

Conclusion: 该框架展示了作为智能食物质量检测和分析新范式的潜力，能够灵活理解复杂食物场景。
...
<details>
  <summary>Details</summary>
Motivation: 子宫内膜异位症在体内不同位置呈现多种视觉表现，使得其识别非常困难且容易出错，特别是对于非专科医生。该系统旨在为妇科医生提供辅助诊断支持。

Method: 训练一个系统来分割子宫内膜异位症的常见视觉表现——暗色子宫内膜植入物，能够分析腹腔镜手术视频，用多色叠加标注识别出的植入区域，并显示检测摘要以改进视频浏览。

Result: 系统能够成功识别和分割暗色子宫内膜植入物，并在腹腔镜手术视频中进行实时标注和摘要显示。

Conclusion: 该系统为妇科医生在诊断和治疗子宫内膜异位症时提供了有效的辅助工具，通过自动化的视频分析和标注提高了诊断的准确性和效率。
...
<details>
  <summary>Details</summary>
Motivation: 传统视觉模型需要大量领域特定标注数据且难以理解复杂环境上下文，而视觉语言模型在遥感领域的应用尚未充分探索。该研究旨在结合两者的优势来提升遥感图像分析的准确性和上下文感知能力。

Method: 将YOLO目标检测模型与视觉语言模型（LLaVA、ChatGPT、Gemini）集成，在标记和未标记的遥感数据以及退化图像场景中进行评估。

Result: 在飞机检测和计数方面，平均MAE提高了48.46%，在挑战性条件下表现尤为突出；在遥感图像综合理解方面，CLIPScore提高了6.17%。
...
<details>
  <summary>Details</summary>
Motivation: 前列腺癌中的筛状形态是一个预后不良的组织学特征，但存在报告不足和观察者间变异性的问题，需要提高检测的准确性和一致性。

Method: 使用EfficientNetV2-S编码器和多实例学习的深度学习模型，在640个数字化前列腺核心针活检切片上进行训练，并在多个队列中进行内部和外部验证。

Result: 模型在内部验证中AUC为0.97，Cohen's kappa为0.81；在外部验证中AUC为0.90，Cohen's kappa为0.55；在评估者间分析中，模型的表现超过了9位专家病理学家。
...
<details>
  <summary>Details</summary>
Motivation: 现有对抗净化方法主要针对加性扰动设计，在现实世界中常见的非加性扰动（模糊、遮挡、失真）下效果不佳，需要开发能同时处理加性和非加性扰动的净化框架。

Method: 建立对抗图像的生成过程，通过似然最大化方法分离潜在的干净图像和扰动参数，实现对抗净化。

Result: 在GTSRB和CIFAR-10数据集上的实验表明，NAPPure显著提升了图像分类模型对非加性扰动的鲁棒性。

Conclusion: NAPPure框架有效扩展了对抗净化的能力，能够同时处理加性和非加性对抗扰动，为现实世界中的对抗攻击防御提供了更全面的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决大视频语言模型在处理长视频时面临的上下文窗口限制和长期序列信息保留困难的问题，以及传统视频RAG方法中时间依赖关系破坏和无关信息干扰的挑战。
...
Conclusion: Vgent框架通过图结构表示和中间推理机制，有效提升了长视频理解能力，解决了传统RAG方法在视频处理中的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 解决不同场景或生成AI视频之间的复杂同步问题，这些视频存在多样化的主体、背景和非线性时间错位。

Method: 使用预训练模型提取高维嵌入，构建共享紧凑的1D原型序列来锚定关键动作阶段，实现鲁棒对齐。

Result: 在多个数据集上提高了同步准确性、效率和鲁棒性，包括细粒度帧检索和阶段分类任务，是首个解决多生成AI视频同步问题的方法。

Conclusion: TPL为复杂视频同步提供了一种有效解决方案，特别是在处理生成AI视频时表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法面临单视图几何不一致和幻觉问题，以及合成数据训练模型无法捕捉高频细节的局限性，影响了身份保持和真实感。

Method: 引入两个关键贡献：(1) 生成式规范化模块处理多视图为标准化表示；(2) 基于Transformer的模型，在从真实人物圆顶捕捉获得的高斯溅射虚拟形象数据集上训练。采用"捕捉、规范化、溅射"流程。

Result: 该方法从非结构化照片中生成具有高度真实感和稳健身份保持的静态四分之三身体虚拟形象。

Conclusion: 提出的零样本流水线成功解决了现有方法的局限性，实现了超真实且保持身份特征的3D虚拟形象创建。
...
Method: cubic通过CuPy和RAPIDS cuCIM为SciPy和scikit-image API提供设备无关的GPU加速替代方案，自动根据数据位置选择GPU或CPU执行。

Result: 在单个操作基准测试和现有去卷积与分割流程复现中，cubic实现了显著的加速效果，同时保持了算法保真度。
...
<details>
  <summary>Details</summary>
Motivation: 解决视频生成中角色身份保持、精确相机控制和光照适应性的问题，推动视频生成在虚拟制作中的集成
...
Conclusion: 联合建模Big Five和HEXACO能提高多模态人类行为感知能力，为表面人格特质识别提供了有效方法。
...
Method: 使用位平面图像处理细化误差提取，设计位平面引导的噪声图像生成和图像归一化策略，采用最大梯度补丁选择放大噪声信号，提出轻量级分类头结构。

Result: 在GenImage基准上平均准确率达98.9%，提升11.9%，跨生成器泛化能力强，从GAN到Diffusion准确率超98.2%，从Diffusion到GAN准确率超99.2%，处理速度比现有方法快近百倍。

Conclusion: 该方法通过位平面噪声分析实现了高效准确的AI生成图像检测，具有优异的性能和泛化能力，计算成本极低。
...
<details>
  <summary>Details</summary>
Motivation: 随着操纵媒体的兴起，深度伪造技术构成严重威胁。传统检测方法依赖手动设计的音素-视位对齐阈值、基本帧级一致性检查或单模态策略，无法有效识别由GANs、扩散模型和神经渲染等先进生成模型产生的现代深度伪造内容，这些技术生成的完美单帧中仍存在细微时间差异。

Method: 提出PIA多模态框架，整合音素序列、唇部几何数据和先进面部身份嵌入，通过语言、动态面部运动和面部识别线索进行综合分析，识别跨多个互补模态的不一致性。

Result: 该集成方法显著提高了对细微深度伪造篡改的检测能力，能够有效识别传统检测器经常忽略的微小时间差异。

Conclusion: PIA框架通过多模态分析成功解决了传统深度伪造检测方法的局限性，为检测先进生成模型产生的深度伪造内容提供了更有效的解决方案。
...
Method: 提出了事件间隔调制(EIM)方案，通过调制事件之间的间隔来编码信息；对EVS参数进行调优和定制以优化EIM的频率响应；实验确定EIM可用的最大调制阶数；基于获得的参数进行传输实验。

Result: 在室内环境中成功实现了28 kbps（10米距离）和8.4 kbps（50米距离）的传输，为基于事件的OCC系统设定了新的比特率基准。

Conclusion: EIM方案能够有效利用事件视觉传感器的独特特性，显著提高了基于事件的OCC系统的传输性能，证明了其在高速、低延迟通信中的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 解决大规模场景中定位和渲染的计算成本高的问题，现有SCR方法在扩展到大规模场景时受限于单网络容量

Method: 引入门控网络隐式分类和选择子网络，每次推理只激活单个子网络；提出无辅助损失的负载均衡策略(ALF-LB)

Result: 在剑桥测试集上仅需10分钟训练即可获得高质量渲染结果，显著降低计算成本同时保持更高精度

Conclusion: MACE框架为大规模场景应用提供了高效解决方案，在保持精度的同时大幅减少计算成本
...
<details>
  <summary>Details</summary>
Motivation: 现有图像到视频生成模型在人脸身份一致性方面存在困难，特别是当人脸在图像中占比较小且人物表情动作变化较大时。由于人类对身份变化高度敏感，这是一个关键但未被充分探索的挑战。

Method: 提出身份保持奖励引导优化(IPRO)框架，基于强化学习优化扩散模型。使用人脸身份评分器，在采样链的最后步骤反向传播奖励信号，提供更丰富的梯度反馈。还提出新颖的面部评分机制，将真实视频中的面部作为特征池，提供多角度面部信息。加入KL散度正则化来稳定训练。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法如VACE和Phantom在特定主题的多样化场景视频生成方面取得进展，但在动态交互中难以保持多个人物身份的一致性，这对多角色场景至关重要。

Method: 构建了一个基于大规模偏好数据集的视频奖励模型，包含人工标注和合成失真数据，采用针对多人物一致性优化的GRPO变体来改进VACE和Phantom。

Result: 实验表明，Identity-GRPO在人物一致性指标上比基线方法提升了18.9%，并通过消融研究评估了标注质量和设计选择对策略优化的影响。
...
Method: 提出MatchAttention机制，通过BilinearSoftmax实现连续可微的滑动窗口注意力采样，相对位置通过残差连接在层间迭代更新；设计MatchDecoder作为核心组件，并引入门控交叉MatchAttention和一致性约束损失来处理遮挡问题。
...
<details>
  <summary>Details</summary>
Motivation: 开发能够在长距离和高数据速率下实现可靠通信的光学相机通信系统，解决传统方法在恶劣条件下的性能限制。

Method: 使用事件视觉传感器，结合开关键控(OOK)与切换解调技术，并集成数字锁相环(DPLL)来提高解调稳定性。

Result: 在室外实验中，首次实现了在200米距离下60kbps数据速率和400米距离下30kbps数据速率时，误码率(BER)低于10^{-3}的通信性能。

Conclusion: 该方法为光学相机通信系统提供了一种鲁棒的解决方案，在长距离和高数据速率场景下具有实际应用价值。
...
Method: 基于因果推理的框架，利用前门原则设计转换策略，明确包含中介变量和观察到的组织切片，以利用语义特征同时减轻混杂因素的影响

Result: 在CAMELYON17数据集和私有组织病理学数据集上验证，在未见域上获得一致的性能提升，最高达到7%的改进，优于现有基线方法

Conclusion: 因果推理是解决组织病理学图像分析中领域偏移问题的有力工具，具有重要潜力
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型虽然在某些任务上表现优异，但仍容易产生幻觉，过度依赖单一模态或记忆训练数据而缺乏视觉基础。

Method: 训练免费的三层对比解码加水印方法：1) 选择解码层中的成熟层和业余层；2) 使用水印相关问题识别视觉锚定层；3) 应用三层对比解码生成最终输出。
...
<details>
  <summary>Details</summary>
Motivation: 虹膜生物识别系统面临演示攻击威胁，但缺乏足够的训练和评估数据集，因为构建和成像演示攻击存在困难。

Method: 使用多领域图像转换扩散StyleGAN框架，结合扩散模型和GANs的优势，采用多领域架构实现真实眼部图像与不同演示攻击领域之间的转换，并使用针对眼部数据的自适应损失函数。

Result: 在LivDet2020数据集上，1%误检率下的真检测率从93.41%提升到98.72%，显著提高了演示攻击检测系统的性能。

Conclusion: MID-StyleGAN能生成高质量合成眼部图像，有效解决虹膜和眼部生物识别中的数据稀缺问题，为演示攻击检测提供了可扩展的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 主流多模态大语言模型仅通过文本token的下一个词预测进行监督，忽略了视觉中心信息，这限制了模型的视觉分析能力。

Method: 引入可学习的模块化任务查询和视觉对齐层，在多个视觉基础模型的监督下激活特定视觉信号；使用Token网关掩码协调不同视觉基础模型之间的表示冲突。

Result: 大量实验表明，VaCo显著提升了不同MLLMs在各种基准测试上的性能，展示了其在视觉理解方面的卓越能力。

Conclusion: VaCo通过视觉中心激活和协调机制，有效提升了多模态大语言模型的视觉理解能力，为MLLMs的发展提供了新思路。
...
Method: 使用循环一致性关键点作为显著点来增强匹配时的空间一致性约束；提出新型姿态块，将GRU循环单元与变换同步相结合，融合历史和多视角数据。

Result: 在ScanNet和3DMatch数据集上超越了之前的自监督配准方法，甚至优于一些较旧的监督方法；将组件集成到现有方法中也显示出有效性。

Conclusion: 提出的自监督RGB-D配准方法通过循环一致性关键点和新型姿态块设计，在多个基准测试中取得了优越性能，证明了该方法的有效性。
...
Method: SPR通过随机选择图像区域和模型生成的区域描述，引入语义和定位评分来全面评估文本质量和定位质量，并将最佳评分的改进描述与最低评分的初始描述配对进行直接偏好优化。

Result: 在标准引用和定位基准测试上的广泛实验表明，SPR能有效提升多模态大语言模型的空间理解能力，且训练开销极小。

Conclusion: SPR方法通过空间偏好奖励机制，成功增强了多模态大语言模型的细粒度空间理解能力，实现了与视觉输入的更好对齐。
...
Method: 提出了DOS（方向性对象分离）方法，该方法在将文本嵌入传递给文本到图像模型之前修改三种类型的CLIP文本嵌入。
...
<details>
  <summary>Details</summary>
Motivation: 解决脑肿瘤分割中存在的计算开销大和模型鲁棒性评估不足的问题，特别是Mamba模型在多空间轴上的顺序特征计算导致的高计算成本。

Method: 使用双分辨率双向Mamba架构，利用空间填充曲线进行3D到1D特征映射以保持空间局部性，提出门控融合模块自适应整合前后向上下文，并使用量化块离散化特征提升鲁棒性。

Result: 在BraTS2023测试集上，全肿瘤、肿瘤核心和增强肿瘤的Dice系数分别提升0.10%、1.75%和0.93%；在系统性五折评估中，肿瘤核心和增强肿瘤平均Dice增益分别为0.86%和1.45%，同时计算效率提升15倍。
...
<details>
  <summary>Details</summary>
Motivation: 在电子制造中，主板缺陷检测对确保可靠性至关重要。虽然PCB检测已有研究，但组装级全主板检测仍未被充分探索，现有研究主要针对裸板或线路级缺陷。

Method: 使用YOLOv7和Faster R-CNN两种代表性检测器在MiracleFactory主板数据集上进行基准测试，提出置信度-时间投票(CTV Voter)轻量级集成方法，通过可解释规则平衡精度和召回率。

Result: YOLOv7在精度方面表现优异但在召回率上表现不佳，Faster R-CNN则相反。CTV Voter集成方法有效平衡了精度和召回率。在锐度、亮度和方向变化等真实扰动下评估了鲁棒性。

Conclusion: 该研究展示了计算机视觉技术如何从基准结果过渡到实际的组装级主板制造质量保证，并发布了可部署的GUI驱动检测工具，连接了研究评估与操作员可用性。
...
<details>
  <summary>Details</summary>
Motivation: 计算病理学面临处理千兆像素图像的瓶颈和缺乏密集标注的问题，现有方法往往忽略多倍率WSI中的细粒度信息和肿瘤微环境变化。

Method: 采用从易到难的渐进表示学习策略，通过双课程对比多实例学习处理WSI，直接将千兆像素图像转换为预后预测。

Result: 在12种癌症类型（5,954名患者，1254万张切片）上的实验表明，DCMIL优于标准WSI预后模型，能识别细粒度预后关键区域并提供稳健的不确定性估计。

Conclusion: DCMIL不仅能准确预测癌症预后，还能捕获正常与肿瘤组织的形态学差异，具有生成新生物学见解的潜力。
...
Method: 开发了统一的帧内/帧间编码框架，使用单一模型自适应执行编码；提出同时两帧压缩设计，充分利用前后向帧间冗余。

Result: 相比DCVC-RT平均减少10.7%的BD-rate，提供更稳定的每帧比特率和质量，同时保持实时编码/解码性能。

Conclusion: 该框架成功解决了现有NVC方案的局限性，通过统一编码设计和双向压缩实现了显著的性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 基于深度学习的视频目标检测器在安全关键应用中至关重要，但它们容易受到对抗攻击，特别是通用扰动攻击的影响。

Method: 使用核范数正则化促进背景集中的结构化扰动，并采用自适应乐观指数梯度方法进行高效优化。

Result: 该方法在攻击效果上优于低秩投影梯度下降和Frank-Wolfe攻击方法，同时保持高隐蔽性。

Conclusion: 提出的攻击方法在视频目标检测中实现了高效且隐蔽的通用对抗攻击，代码和数据已公开。
...
<details>
  <summary>Details</summary>
Motivation: 无监督深度生成模型可以仅使用健康数据训练，识别与学习到的正常脑结构偏离的异常，避免了监督方法需要大量体素级标注数据和仅限于已知病理的限制。

Method: 采用PRISMA指导的范围综述方法，对49项研究进行综合分析，涵盖脑MRI和CT在肿瘤、中风、多发性硬化等疾病中的应用。

Result: 生成模型在大病灶检测中表现良好，在处理更细微异常方面也取得进展，能够生成可解释的伪健康重建，在标注数据稀缺的情况下特别有价值。

Conclusion: 生成模型为异常检测提供了有前景的方向，支持半监督学习、新型影像生物标志物发现以及统一框架内的疾病内和跨疾病偏差映射。未来工作应优先考虑解剖感知建模、基础模型开发、任务适当的评估指标和严格的临床验证。
...
<details>
  <summary>Details</summary>
Motivation: 多任务图像恢复模型参数量过高导致计算效率低下，需要在不牺牲性能的前提下压缩模型

Method: 使用迭代剪枝策略，通过多轮移除低幅值权重并重置剩余权重到原始初始化，发现高性能稀疏子网络

Result: 在去雨、去雾和去噪任务上，MIR-L仅保留10%参数仍能维持高图像恢复性能

Conclusion: 该方法成功实现了多任务图像恢复模型的高效压缩，为实际应用提供了计算友好的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 放牧活动对农业生产和生物多样性都有重要影响，但目前缺乏可扩展的放牧监测方法。需要开发一种能够利用免费卫星数据来有效指导保护性土地利用合规检查的技术。

Method: 基于Sentinel-2 L2A时间序列数据，使用CNN-LSTM集成模型对多边形定义的田块边界进行二元预测（放牧/未放牧），利用4-10月的多时相反射特征进行训练。

Result: 在五个验证分割上平均F1分数达到77%，对放牧草地的召回率达到90%。在检查资源有限（每年最多检查4%场地）的情况下，优先检查模型预测的非放牧场地，比随机检查能多发现17.2倍的非放牧确认场地。

Conclusion: 粗分辨率的免费卫星数据能够可靠地指导检查资源分配，支持保护性土地利用合规监测。代码和模型已公开。
...
<details>
  <summary>Details</summary>
Motivation: Vision Mamba相比Vision Transformers(ViTs)具有线性而非二次的计算复杂度，且比传统CNN需要更少的可训练参数，这使其在计算和内存效率上更具优势。

Method: 使用Vision Mamba作为骨干网络构建神经网络来预测三维多孔介质的渗透率，并与ViT和CNN模型进行性能比较，同时进行消融研究评估各组件对准确性的影响。

Result: 在实际应用中验证了Vision Mamba相比ViTs和CNNs在三维多孔介质渗透率预测中的优势，包括计算效率和内存效率。

Conclusion: 提出的框架有潜力集成到大型视觉模型中，用Vision Mamba替代ViTs，并公开了源代码以便复现和扩展。
...
<details>
  <summary>Details</summary>
Motivation: 解决文本到图像生成中训练-推理不匹配问题：训练时噪声来自提示特定子空间，推理时来自高斯先验，导致图像与提示对齐不佳

Method: 设计噪声投影器，基于提示嵌入将噪声映射到提示感知对应物；使用VLM获取反馈，蒸馏到奖励模型，通过准直接偏好优化优化投影器

Result: 实验表明，提示感知噪声投影能显著提升不同提示下的文本-图像对齐度

Conclusion: 提出的噪声投影框架无需参考图像或手工先验，推理成本低，能有效改善文本到图像生成的对齐质量
...
<details>
  <summary>Details</summary>
Motivation: 开发一个资源消耗最小但性能优越的文档解析模型，以支持多语言和复杂元素的准确识别。
...
Conclusion: PaddleOCR-VL在性能和效率方面表现出色，非常适合在实际场景中部署使用。
...
<details>
  <summary>Details</summary>
Motivation: 解决牙科放射影像解读中专业人员短缺的问题，克服现有牙科AI系统的局限性，包括单模态限制、任务特定设计和对标注数据的依赖，提升在多样化临床场景中的泛化能力。
...
Conclusion: DentVFM为牙科AI设立了新范式，提供了可扩展、适应性强且标签高效的方法，能够改善智能牙科医疗并解决全球口腔医疗中的关键缺口。
...
<details>
  <summary>Details</summary>
Motivation: 解决医学图像中由于扫描仪和协议差异导致的领域偏移问题，同时提高现有方法的可解释性，这是医学应用中的关键需求。

Method: 使用两个编码器分别提取域不变特征和域特定特征，通过对抗训练和图像重建（将两个特征的重构相加）来确保协调性和信息完整性。

Result: 在图像重建、疾病分类和领域识别任务上达到或优于现有方法，并能可视化域独立脑部特征和域特定成分。

Conclusion: PL-SE-ADA在保持性能的同时显著提高了可解释性，为医学图像分析提供了更可靠的解决方案。
...
TL;DR: 提出了一种无需训练的场景去上下文化方法SDeC，通过抑制文本到图像模型中固有的场景-身份相关性，解决身份漂移问题，实现了更好的身份保持和场景多样性。


<details>
  <summary>Details</summary>
Motivation: 解决文本到图像生成中的身份漂移问题，传统方法需要预先知道所有目标场景，这在现实应用中不切实际。研究发现身份漂移的主要原因是模型训练过程中自然形成的场景-身份相关性。

Method: 提出SDeC方法，通过SVD方向稳定性量化场景-身份相关性，自适应地重新加权相应特征值来抑制身份提示嵌入中的潜在场景-身份相关性。这是一种无需训练、基于提示嵌入编辑的方法。

Result: 实验证明SDeC显著增强了身份保持能力，同时保持了场景多样性，无需预先知道所有目标场景，具有很高的灵活性和通用性。

Conclusion: SDeC通过理论分析和实践验证，提供了一种高效、无需训练的场景去上下文化解决方案，有效解决了文本到图像生成中的身份漂移问题，适用于现实世界应用场景。
...
TL;DR: 提出一个主动AI助手系统，能够在流式视频输入中理解、预测和主动响应事件，具备主动连贯性、及时响应和同步效率三个关键特性。


<details>
  <summary>Details</summary>
Motivation: 开发能够在人类环境中主动运作的AI系统，超越被动观察，实现主动理解、预测和响应事件的能力。

Method: 提出包含数据引擎、多阶段训练策略和主动动态压缩技术的综合技术流程，并引入ESTP-Bench基准和ESTP-F1评估指标。

Result: 所提模型有效解决了关键特性，在多个在线和离线基准测试中优于多个基线方法。

Conclusion: 该研究为实现主动AI助手在流式视频环境中的运作提供了有效的技术框架和评估方法。
...
TL;DR: BalanceGS是一种算法-系统协同设计的3D高斯泼溅训练优化方法，通过平衡密度分配、动态工作负载分配和内存访问优化，在保持重建质量的同时显著提升训练速度。


<details>
  <summary>Details</summary>
Motivation: 传统3D高斯泼溅训练存在三个关键低效问题：高斯密集化中的密度分配不均、高斯投影中的计算负载不平衡以及颜色泼溅中的内存访问碎片化。
...
Method: 采用多分支架构，集成边缘卷积、点注意力和曲率嵌入机制，捕捉局部表面变化和几何特征。

Result: 在nuScenes数据集上，相比最强基线，平均重识别准确率提升约1.97%。
...
Method: 框架包含点描述器（生成关键点的上下文描述）和点定位器（从描述回归像素坐标），使用合成的LlamaPointInPart数据集训练，并通过GRPO优化跨类别泛化能力。
...
TL;DR: 提出了STANCE框架，通过实例线索和密集RoPE技术解决视频生成中物体运动一致性和交互问题，提升时间连贯性。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成在保持物体运动一致性和交互方面存在困难，主要瓶颈包括：运动提示在编码后有效token过少，以及单一优化目标倾向于纹理而非时间一致性。

Method: 1. 引入实例线索：将稀疏的用户可编辑提示转换为密集的2.5D运动场；2. 使用密集RoPE技术：在token空间中保持运动线索的显著性；3. 联合RGB+辅助图预测：分离结构和外观处理。

Result: 该方法减少了深度模糊性，稳定了优化过程，提高了时间连贯性，且无需逐帧轨迹脚本。
...
TL;DR: 提出了一种分层重分类系统，结合SpeciesNet EfficientNetV2-M预测与CLIP嵌入和度量学习，将高级分类标签细化为物种级识别，在动物检测平台上实现了96.5%的准确率。


<details>
  <summary>Details</summary>
Motivation: 现有动物分类模型如SpeciesNet使用保守的汇总策略，导致许多动物仅被标记在高级分类学水平而非物种级别，需要改进以提供更精确的物种识别。

Method: 五阶段流水线：高置信度接受、鸟类覆盖、质心构建、三元组损失度量学习和自适应余弦距离评分，结合SpeciesNet预测与CLIP嵌入。

Result: 在LILA BC沙漠狮子保护数据集的4,018张图像（15,031个检测）上，从"空白"和"动物"标签中恢复了761个鸟类检测，重新分类了456个标记为动物、哺乳动物或空白的检测，准确率达96.5%，64.9%实现了物种级识别。
...
TL;DR: 评估零样本方法在野生动物图像分类中的表现，使用自监督视觉变换器，在Animal Detect平台上开发和测试。比较了不同架构和聚类方法，DINOv2结合UMAP和GMM达到88.6%准确率，1D排序在哺乳动物和鸟类中达到88.2%一致性。
...
Result: 在5个物种测试集上，DINOv2与UMAP和GMM组合达到88.6%准确率（宏F1=0.874），1D排序在哺乳动物和鸟类中达到88.2%一致性，在鱼类中达到95.2%一致性（基于1500张图像）。

Conclusion: 基于这些发现，将连续相似性排序部署到生产环境中，能够快速进行探索性分析，并加速生物多样性监测的手动标注工作流程。
...
TL;DR: 提出Wiki-PRF方法，通过处理、检索和过滤三阶段提升KB-VQA任务性能，结合视觉工具和强化学习训练，在基准数据集上实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 解决KB-VQA任务中多模态查询质量差和检索结果相关性不足的问题，提升视觉语言模型整合外部知识的能力。

Method: 三阶段方法：处理阶段调用视觉工具提取多模态信息；检索阶段整合视觉和文本特征进行知识检索；过滤阶段进行相关性过滤。使用强化学习训练VLM，以答案准确性和格式一致性作为奖励信号。

Result: 在E-VQA和InfoSeek基准数据集上分别提升36.0和42.8分，达到最先进的性能水平。

Conclusion: Wiki-PRF方法通过多阶段处理和强化学习训练，显著提升了KB-VQA任务的答案质量，证明了该方法在整合视觉理解和知识检索方面的有效性。
...
TL;DR: 提出了Shot2Tactic-Caption框架，用于羽毛球视频的语义和时间多尺度字幕生成，能够生成描述单个动作的击球级字幕和捕捉战术执行过程的技术级字幕。
...
Conclusion: Shot2Tactic-Caption框架能够有效生成羽毛球视频的多尺度字幕，特别是能够描述成功执行的战术以及被暂时中断后恢复的战术执行过程。
...
TL;DR: 提出了Efficient Video Sampling (EVS)方法，通过识别和剪枝时间上静态的补丁来减少视频中的令牌冗余，从而降低处理成本并提高推理速度。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在处理密集视频帧序列时面临二次计算成本的问题，长视频往往超出令牌预算，导致严重的上下文限制和延迟问题。

Method: EVS是一种即插即用的方法，通过识别和剪枝时间上静态的补丁（在连续帧间保持不变的空间区域）来减少令牌冗余，无需架构更改或重新训练。

Result: EVS显著减少了令牌数量，同时保持语义保真度，使大型语言模型的首次令牌时间减少高达4倍，且精度损失最小。结合随机剪枝率的上训练，模型对不同程度的压缩具有鲁棒性。

Conclusion: EVS持续改善了效率与精度的权衡，在不牺牲质量的情况下实现了可扩展的视频语言理解。
...
TL;DR: RepTok是一个生成建模框架，使用自监督视觉变换器提取的单个连续潜在token来表示图像，通过微调语义token嵌入和生成解码器，实现高效图像生成。
...
Method: 基于预训练的SSL编码器，仅微调语义token嵌入，配合使用流匹配目标训练的生成解码器，添加余弦相似度损失保持原始SSL空间几何特性。

Result: 在类别条件ImageNet生成上取得竞争性结果，在MS-COCO上以极有限训练预算达到竞争性零样本文本到图像合成性能。

Conclusion: 微调的自监督表示可以作为紧凑有效的潜在空间，用于高效生成建模，单token公式显著减少训练成本。
...
Method: 采用Feynman-Kac引导机制，维护多个粒子轨迹，通过累计top-K概率和熵调度来平衡探索与置信度，使用伪标签驱动的奖励进行引导。

Result: 在ImageNet-C数据集上，SteeringTTA始终优于不进行任何模型更新或使用源数据的基线方法。

Conclusion: SteeringTTA框架有效提升了测试时适应的性能，通过引导机制实现了更好的分布偏移鲁棒性。
...
TL;DR: 提出了一种低成本预训练策略，利用无配对视频剪辑进行上下文学习，使基础视频生成模型具备通用编辑能力，然后通过少量高质量配对数据进行微调。


<details>
  <summary>Details</summary>
Motivation: 基于指令的图像编辑进展迅速，但扩展到视频领域仍未被充分探索，主要由于构建大规模配对视频编辑数据集的成本过高和复杂性。

Method: 基于HunyuanVideoT2V框架，首先在约100万个真实视频剪辑上进行预训练以学习基本编辑概念，然后在少于15万个精选编辑配对上微调以扩展更多编辑任务并提高编辑质量。

Result: 比较实验表明，该方法在指令对齐和视觉保真度方面均优于现有的基于指令的视频编辑方法，编辑指令遵循度提高了12%，编辑质量提高了15%。

Conclusion: 提出的低成本预训练策略有效解决了视频编辑数据稀缺问题，通过无配对视频学习和少量高质量数据微调，实现了高质量的指令驱动视频编辑。
...
<details>
  <summary>Details</summary>
Motivation: MAE预训练在低标签场景表现优异但计算成本高昂，难以在工业环境中实用

Method: 在MAE预训练中，选择性地在编码器部分应用去相关反向传播(DBP)方法，通过减少各层输入相关性来加速收敛

Result: 在ImageNet-1K预训练和ADE20K微调中，DBP-MAE减少21.1%训练时间、21.4%碳排放，分割mIoU提升1.1个百分点；在工业数据上同样有效

Conclusion: DBP能够在大规模ViT预训练中同时减少训练时间、能耗并提升下游性能，具有实际应用价值
...
TL;DR: EuroMineNet是首个基于Sentinel-2多光谱影像的欧洲采矿足迹测绘和监测多时间基准数据集，涵盖2015-2024年133个欧盟采矿点的年度观测和专家验证标注，支持采矿足迹测绘和跨时间变化检测任务。
...
Method: 构建EuroMineNet数据集，使用Sentinel-2多光谱影像，提供133个欧盟采矿点2015-2024年的年度观测和专家验证标注，支持两种任务：多时间采矿足迹测绘和跨时间变化检测。

Result: 对20个最先进的深度学习模型进行基准测试，发现GeoAI方法能有效识别长期环境变化，但在检测对及时缓解至关重要的短期动态方面仍面临挑战。

Conclusion: EuroMineNet通过推进时间一致和可解释的采矿监测，有助于可持续土地利用管理、环境韧性，以及将GeoAI应用于社会和环境的更广泛目标。
...
TL;DR: 提出了WeCKD方法，通过链式知识蒸馏解决传统知识蒸馏中的知识退化、监督效率低和数据依赖问题。
...
Method: 构建弱监督链式知识蒸馏网络，形成渐进式蒸馏链，每个模型从前辈学习并精炼知识后传递给下一模型，每个模型仅需部分数据集训练。

Result: 在四个耳镜成像数据集上超越现有监督方法，在其他两个数据集上验证了跨医学成像模态的泛化能力，相比单骨干网络在有限数据下准确率提升高达23%。

Conclusion: WeCKD通过结构化知识转移减少数据依赖，在有限数据场景下实现有效学习，具有实际应用潜力。
...
TL;DR: VTimeCoT是一个无需训练的视频时序定位和推理框架，通过进度条视觉工具和视觉时序思维链提升多模态大语言模型的视频理解能力。
...
Method: 提出两种新颖的进度条视觉工具：即插即用的进度条集成工具和高效高亮工具，并引入视觉时序思维链过程，实现视频和文本的跨模态推理。
...
TL;DR: 首次系统评估深度视频相机运动分类模型在档案影片材料上的表现，发现Video Swin Transformer在历史二战影像数据集上达到80.25%准确率。


<details>
  <summary>Details</summary>
Motivation: 探索现有相机运动分类方法在历史影像材料上的泛化能力，填补该领域的研究空白。

Method: 总结代表性方法和数据集，评估五种标准视频分类模型在HISTORIAN数据集上的表现。

Result: Video Swin Transformer表现最佳，在有限训练数据下达到80.25%准确率，显示出强大的收敛能力。

Conclusion: 现有模型在低质量视频上具有适应潜力，未来工作应结合多样输入模态和时间架构。
...
TL;DR: 提出一种基于学习图像先验的3D高斯泼溅压缩框架，通过恢复网络建模压缩伪影，结合粗渲染残差作为辅助信息，实现高质量压缩表示。


<details>
  <summary>Details</summary>
Motivation: 现有3DGS压缩方法缺乏学习先验，限制了率失真性能的进一步提升，需要利用学习图像先验来恢复压缩引起的质量退化。

Method: 在初始压缩高斯基础上，构建恢复网络建模图像空间中的压缩伪影，引入粗渲染残差作为辅助信息，通过恢复图像监督优化压缩高斯表示。

Result: 实验验证该框架在率失真性能上优于现有最先进的3DGS压缩方法，在显著减少存储需求的同时提升渲染质量。

Conclusion: 该框架兼容现有高斯压缩方法，通过结合学习先验和辅助信息，实现了紧凑且高质量的3D高斯表示压缩。
...
TL;DR: DEXTER是一个无需训练数据的框架，使用扩散模型和大型语言模型为视觉分类器生成全局文本解释，通过优化文本提示合成激活分类器的图像，并生成描述决策模式和偏见的自然语言报告。


<details>
  <summary>Details</summary>
Motivation: 理解和解释机器学习模型行为对于构建透明可信的AI系统至关重要，但现有方法通常需要访问训练数据或真实标签。
...
Conclusion: DEXTER提供了一个灵活且有效的框架，能够在无需训练数据的情况下揭示视觉分类器的内部机制，为模型可解释性提供了新的途径。
...
TL;DR: 提出了一种基于无条件扩散模型的方法来重建火星表面地形，该方法在重建精度和感知相似性方面优于传统的插值和修复技术。


<details>
  <summary>Details</summary>
Motivation: 火星地形数据中存在大量缺失值，而现有的插值方法在保持几何一致性方面表现不佳。由于火星缺乏像地球那样全面的数据集，无法使用条件方法，因此需要开发专门的无条件深度学习方法。

Method: 使用无条件扩散模型，在12000个来自NASA HiRISE调查的火星高度图增强数据集上进行训练。采用非均匀缩放策略捕捉多尺度地形特征，然后调整到固定的128x128分辨率。

Result: 在1000个样本的评估集上，该方法在重建精度（RMSE提高4-15%）和感知相似性（LPIPS提高29-81%）方面均优于逆距离加权、克里金插值和纳维-斯托克斯算法等传统方法。

Conclusion: 基于无条件扩散模型的方法能够有效重建火星表面地形，为空间探索中的虚拟现实应用提供更可靠的地形数据。
...
TL;DR: 开发了PRISM模型，一种可解释的AI模型，通过整合连续变异谱来表征结直肠癌的形态多样性，在5年总生存期预测中表现优于现有方法。
...
Result: PRISM在5年总生存期预测中表现优异（AUC=0.70，准确率68.37%，HR=3.34），比现有CRC特异性方法高15%，比AI基础模型高约23%准确率，且具有性别无关的稳健性。

Conclusion: PRISM模型成功捕捉了结直肠癌的形态多样性，提供了优越的预后预测性能，并显示出跨临床病理亚组的稳定表现。
...
<details>
  <summary>Details</summary>
Motivation: 现有注意力方法局限于单层或双层特征优化，忽略了多尺度表征间的丰富层间依赖关系，限制了检测尺度变化目标的能力

Method: CFSAM包含三个组件：卷积局部特征提取器、基于Transformer的全局建模单元（捕获跨层交互）、特征融合机制（恢复和增强原始表征）

Result: 在SSD300框架中集成CFSAM，PASCAL VOC上mAP达78.6%（基线75.5%），COCO上mAP达52.1%（基线43.1%），优于现有注意力模块，且加速收敛
...
TL;DR: R-Super 是一种利用医学报告训练AI进行肿瘤分割的方法，大幅减少了对人工绘制肿瘤掩模的需求，在多种肿瘤类型上实现了与基于掩模训练相当甚至更好的性能。


<details>
  <summary>Details</summary>
Motivation: 传统AI肿瘤分割模型需要大量人工绘制的肿瘤掩模，成本高昂且耗时。而临床实践中几乎每个CT扫描都配有描述肿瘤特征的医学报告，这些丰富信息未被充分利用。

Method: R-Super通过训练AI模型使其能够根据医学报告中的描述来分割肿瘤，利用大规模现成的医学报告数据集进行训练。

Result: 在101,654份报告上训练的模型性能与基于723个掩模训练的模型相当。结合报告和掩模训练使敏感性提高13%，特异性提高8%，在7种肿瘤类型中的5种上超越了放射科医生的检测能力。
...
TL;DR: 提出了ImageNet-F基准测试，用于模拟现实世界中混合粒度的图像标注，并开发了free-grain学习方法，通过视觉语言模型和半监督学习提升层次分类性能。


<details>
  <summary>Details</summary>
Motivation: 现有层次图像分类方法假设完整的细粒度标注，但现实世界中的标注粒度因图像质量、标注者专业知识和任务需求而异，需要处理混合粒度的监督信号。

Method: 使用CLIP模拟语义模糊性，构建混合粒度标签；提出free-grain学习框架，结合视觉语言模型生成的伪属性和半监督学习来增强语义和视觉引导。

Result: 提出的方法和强基线在混合监督下显著提升了性能表现。

Conclusion: ImageNet-F基准测试和free-grain学习方法推动了在现实约束下的层次分类研究进展。
...
TL;DR: 本文系统评估了多模态大语言模型在标准人脸识别数据集上的表现，发现虽然MLLMs能捕捉丰富的语义信息，但在零样本应用中仍落后于专用人脸识别模型。


<details>
  <summary>Details</summary>
Motivation: 虽然多模态大语言模型在各种视觉语言任务中表现出色，但其在人脸识别领域的潜力尚未充分探索，特别是开源MLLMs需要在标准基准上与现有人脸识别模型进行公平比较。
...
Conclusion: 该基准为推进基于MLLM的人脸识别奠定了基础，为设计具有更高准确性和泛化能力的下一代模型提供了见解。
...
TL;DR: 提出了MaskCaptioner模型，通过合成字幕扩展LVIS和LV-VIS数据集，实现端到端的密集视频目标检测、跟踪和字幕生成，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 由于密集视频目标字幕任务的复杂性和手动标注的高成本，现有方法采用分离训练策略可能导致次优性能，需要开发端到端的解决方案。
...
Conclusion: 通过合成字幕数据增强和端到端训练，MaskCaptioner能够有效解决密集视频目标字幕任务，显著提升性能。
...
TL;DR: 提出LightQANet框架，通过量化光照特征和自适应提示学习来解决低光图像增强中的纹理恢复、颜色一致性和伪影问题，在多个数据集上达到最先进性能。
...
Method: 提出LightQANet框架，包含光照量化模块(LQM)显式提取和量化光照相关因子，以及光照感知提示模块(LAPM)将光照先验编码为可学习提示来动态指导特征学习。

Result: 在多个低光数据集上的广泛实验表明，该方法在各种挑战性光照场景下都能获得优越的定性和定量结果，达到最先进性能。

Conclusion: 通过结合静态光照建模和动态自适应学习，LightQANet能够实现跨不同光照条件的一致和鲁棒图像质量增强。
...
TL;DR: 提出RealDPO方法，利用真实世界数据作为偏好学习的正样本，通过DPO损失函数提升视频生成模型的运动真实感，显著改善了复杂运动的合成质量。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型在复杂运动合成方面存在局限，生成的运动往往不够自然、流畅且上下文不一致，这限制了其实际应用。

Method: 引入RealDPO对齐范式，使用真实世界视频作为正样本进行偏好学习，通过对比真实视频与错误模型输出实现迭代自校正，并构建RealAction-5K高质量视频数据集支持训练。

Result: 实验表明RealDPO在视频质量、文本对齐和运动真实感方面显著优于现有最先进模型和偏好优化技术。

Conclusion: RealDPO通过真实世界数据驱动的偏好学习有效提升了视频生成模型的运动合成能力，为复杂动作生成提供了新思路。
...
TL;DR: C4D是一个从单目视频恢复4D动态场景的框架，通过结合短期光流和长期点跟踪来扩展现有3D重建方法，能够分离移动物体和静态背景，实现完整的4D重建。


<details>
  <summary>Details</summary>
Motivation: 现有的点图式3D重建方法在处理动态场景时表现不佳，因为移动物体违反了多视角几何约束，导致重建不准确。需要一种能够处理动态场景的4D重建方法。

Method: 提出C4D框架，除了预测点图外，还捕获两种对应关系：短期光流和长期点跟踪。训练动态感知点跟踪器提供移动性信息，估计运动掩码分离移动元素和静态背景。引入动态场景优化目标来恢复每帧3D几何和相机参数。

Result: 实验表明C4D框架实现了完整的4D恢复，在深度估计、相机姿态估计和点跟踪等多个下游任务中表现出强大性能。

Conclusion: C4D通过时间对应关系成功扩展了3D重建到4D，能够有效处理动态场景，为动态场景理解提供了完整的解决方案。
...
TL;DR: 提出了一种基于视觉的微型飞行器集群通信框架，通过运动模式传递信息，使用事件相机捕获和尖峰神经网络解码，实现低功耗可靠通信。


<details>
  <summary>Details</summary>
Motivation: 解决微型飞行器集群在频谱拥塞、干扰和高功耗环境下传统无线电通信的挑战，受蜜蜂摇摆舞启发开发视觉通信方案。

Method: 使用四种运动基元（上下、左右、左-上-右、左-下-右）作为控制符号，通过事件相机被动捕获运动模式，采用事件帧分割模型和轻量级尖峰神经网络进行动作识别和解码。

Result: 实验验证了框架的有效性，实现了准确解码和低功耗，在受限环境中表现出良好的通信性能。

Conclusion: 该视觉通信框架为微型飞行器集群提供了一种节能高效的通信替代方案，特别适用于受约束环境。
...
TL;DR: 提出了WithAnyone模型，通过构建MultiID-2M数据集和对比身份损失，解决了文本到图像生成中的copy-paste问题，在保持身份一致性的同时增强生成多样性。


<details>
  <summary>Details</summary>
Motivation: 现有身份一致生成模型依赖重建训练，导致copy-paste问题——直接复制参考人脸而非在姿态、表情、光照等自然变化中保持身份，限制了生成的可控性和表达能力。

Method: 1) 构建大规模配对数据集MultiID-2M；2) 提出量化copy-paste伪影和身份保真度-变化权衡的基准；3) 引入对比身份损失的训练范式。

Result: WithAnyone模型显著减少copy-paste伪影，提高姿态和表情的可控性，保持强感知质量。用户研究验证了高身份保真度和表达性可控生成。

Conclusion: 该方法有效缓解copy-paste问题，在保持身份相似性的同时实现多样化的可控生成。
...
TL;DR: CoT-PL是一个新的开放词汇目标检测框架，通过引入视觉思维链推理和对比背景学习，在拥挤和遮挡场景中显著提升了新类别的伪标签质量。


<details>
  <summary>Details</summary>
Motivation: 现有的开放词汇目标检测方法过度依赖直接的图像-文本匹配，忽略了语义复杂场景中必要的中间推理步骤，导致在拥挤或遮挡场景下鲁棒性不足。

Method: CoT-PL将目标理解分解为三个可解释步骤：区域感知、零样本类别识别和背景定位，并引入对比背景学习来促进目标与背景的特征解耦。

Result: 在拥挤和遮挡场景中，新类别伪标签质量分别比之前最佳方法提升了103.4%和168.4%；在开放词汇COCO上实现了+7.7 AP50的提升，在LVIS上新类别实现了+2.9 mask AP的提升。

Conclusion: CoT-PL通过结构化视觉思维链推理和对比背景学习的集成管道，在开放词汇目标检测中实现了新的最先进性能，特别在复杂场景下表现出色。
...
Conclusion: Terra通过原生3D表示和架构实现精确多视图一致性，支持单次生成过程从任意视角灵活渲染，实现可探索的世界建模。
...
TL;DR: 本文提出了NEO，一个基于第一性原理构建的原生视觉语言模型家族，能够与顶级模块化模型竞争，仅使用3.9亿图像-文本对即可从零开始高效开发视觉感知能力。


<details>
  <summary>Details</summary>
Motivation: 解决原生视觉语言模型面临的两个关键挑战：明确其与模块化模型的基本约束差异，以及如何使原生VLM研究更加普及和民主化，从而加速该领域发展。

Method: 提出构建原生VLM的三个指导原则：在共享语义空间中有效对齐像素和词表示；无缝整合视觉和语言模块的优势；内在体现支持统一视觉语言编码、对齐和推理的跨模态特性。基于这些原则开发NEO模型家族。

Result: NEO模型在多样化现实场景中能够与顶级模块化模型竞争，仅使用3.9亿图像-文本对就能从零开始高效开发视觉感知，同时在密集单一模型中缓解视觉语言冲突。

Conclusion: NEO为可扩展和强大的原生VLM奠定了基础，配备丰富的可重用组件，促进成本效益高且可扩展的生态系统发展。
...
Conclusion: 该方法作为多视角一致编辑的通用解决方案具有潜力，无需显式3D表示优化。
...
TL;DR: PRTraj是一个新的轨迹表示学习框架，通过环境感知和显式路径选择建模来学习更有效的轨迹表示，在多个下游任务中表现出优越性能。


<details>
  <summary>Details</summary>
Motivation: 现有轨迹表示学习方法将轨迹视为孤立的时空序列，忽略了外部环境和内部路径选择行为对轨迹形成的影响，这限制了表示学习的有效性。
...
Conclusion: PRTraj通过统一环境感知和路径选择建模，显著提升了轨迹表示学习的质量，为下游任务提供了更有效的轨迹嵌入。
...
TL;DR: 提出了一种新的分数位量化方法(SHORT)，通过渐进式降低模型精度从32位到4位，在优化过程中利用分数位来保持生成质量，在多种扩散模型上实现了比标准QAT更低的FiD。
...
Method: 渐进式降低模型精度(从32位到4位)，在优化过程中利用分数位来维持高质量生成。

Result: 在SD3.5-Medium、Sana、PixArt和FLUX.1-schnell等多种扩散模型上实现了改进的质量，比标准QAT降低4-7%的FiD，并成功在三星S25U手机上部署运行。

Conclusion: SHORT方法有效解决了模型量化中的质量保持问题，为在资源受限设备上部署高质量生成模型提供了可行方案。
...
TL;DR: 通过合成数据增强真实数据，创建了大规模腹部肿瘤分割数据集AbdomenAtlas 2.0，在六个器官上显著提升了AI模型性能


<details>
  <summary>Details</summary>
Motivation: 解决肿瘤分割AI训练中缺乏大规模体素级标注数据的问题，真实数据标注成本高且性能提升有限

Method: 结合合成数据和真实数据，创建包含10,135个CT扫描、15,130个肿瘤实例的大规模数据集，由23名放射科专家标注

Result: 相比仅使用真实数据，合成数据能以更少的真实扫描达到相同性能；AbdomenAtlas 2.0在分布内测试中DSC提升7%，分布外测试提升16%

Conclusion: 合成数据可以显著提升数据扩展效率，大规模高质量标注数据集是提升肿瘤分割AI性能的关键
...
TL;DR: QDepth-VLA通过添加深度预测任务增强VLA模型的空间感知能力，使用VQ-VAE编码器预测量化深度图token，在仿真和真实世界任务中表现出色。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型缺乏对3D结构的理解和推理能力，难以完成精细操作任务。

Method: 设计专门的深度专家模块，通过VQ-VAE编码器预测量化深度图潜在token，学习深度感知表示。

Result: 在仿真基准和真实世界任务中展现出强大的空间推理能力和竞争性的操作性能。

Conclusion: QDepth-VLA框架有效提升了VLA模型的空间感知和推理能力，为精细操作任务提供了更好的几何线索理解。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频生成模型在现实场景中表现优异，但在想象力场景中性能显著下降，因为这些场景涉及罕见共现概念和长距离语义关系，超出了训练分布范围。现有测试时缩放方法由于固定的搜索空间和静态奖励设计，难以适应想象力场景的需求。

Method: ImagerySearch通过动态调整推理搜索空间和奖励函数来适应提示中的语义关系。该方法根据提示内容自适应地优化生成过程，确保在挑战性想象力设置中产生更连贯和视觉上合理的视频。

Result: 在LDT-Bench基准测试上，ImagerySearch持续优于强基线视频生成方法和现有测试时缩放方法，在VBench上也取得了有竞争力的改进，证明了其在不同提示类型上的有效性。

Conclusion: ImagerySearch通过动态自适应策略有效解决了想象力场景中视频生成的挑战，LDT-Bench基准测试为未来想象力视频生成研究提供了重要评估工具。
...
<details>
  <summary>Details</summary>
Motivation: 早期检测黑色素瘤对提高生存率至关重要，但现有深度学习方法通常作为黑盒，缺乏对ABCDE等人类可解释特征的量化分析。

Method: 开发深度学习框架，在分类皮肤病变的同时量化ABCD特征评分，并在潜在空间中可视化病变从良性痣到恶性黑色素瘤的演化轨迹。

Result: 在HAM10000数据集上，分类准确率约89%，黑色素瘤AUC为0.96；ABCD特征评估在不对称性、颜色变化和直径预测表现良好，但边界不规则性建模较困难。
...
TL;DR: 该论文提出了一种多模态学习方法，通过预训练专家模型和程序化组合，在无需人工监督的情况下整合多种视觉模态，并开发了高效的小参数模型（<1M）实现与大型模型（~300M）相竞争的性能。
...
Method: 使用预训练专家模型和程序化组合，构建全自动数据流水线处理原始视频数据，采用专门设计的PHG-MAE模型来利用多模态数据，并将模型高效蒸馏为小参数模型。

Result: 开发的小参数模型（<1M）在性能上可与约300M参数的大型模型竞争，成功部署在手持设备和网络摄像头上实现实时语义分割，并在商品硬件上运行。

Conclusion: 通过多模态整合和模型蒸馏技术，可以在保持高性能的同时显著减少模型参数，为在资源受限设备上部署复杂视觉任务提供了可行方案。
...
TL;DR: 提出了Free-Form HOI Generation方法，通过细粒度意图控制生成多样化的手-物体交互，突破了传统固定抓握模式的限制。


<details>
  <summary>Details</summary>
Motivation: 现有手-物体交互生成研究局限于固定抓握模式，基于物理先验如力闭合或通用意图指令，无法捕捉日常交互的多样性。

Method: 构建WildO2数据集，包含4.4k种独特交互；提出TOUCH三阶段框架，基于多级扩散模型，利用显式接触建模进行条件控制，并通过接触一致性和物理约束进行细化。

Result: 实验证明该方法能够生成可控、多样且物理合理的手部交互，能够代表日常活动中的各种交互模式。

Conclusion: 该方法成功扩展了手-物体交互从抓握到自由形式交互（如推、戳、旋转），实现了对细粒度意图的精确控制。
...
TL;DR: BADAS是一个用于自我中心碰撞预测的模型系列，通过重新标注数据集来区分涉及自车的威胁和随机事故，减少误报，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有碰撞预测方法无法区分涉及自车的威胁和不涉及自车的随机事故，导致实际部署中产生过多误报。
...
Conclusion: BADAS通过自我中心评估方法显著提升了碰撞预测的准确性，发布了BADAS-Open模型权重、代码和所有评估数据集的重新标注，以促进自我中心碰撞预测研究。
...
Method: 提出改进的MMDiT块和参考注意力模块，通过丢弃图像→条件的不必要注意力来降低计算成本并稳定控制注入，同时强调参数重用和零初始化线性投影。
...
TL;DR: 提出nlg2choice方法，通过两阶段处理解决多模态大语言模型在细粒度视觉分类中处理数百至数千个选项的挑战，包括分类和检索任务。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注语言任务或5个选项以内的选择题，无法处理细粒度视觉分类中数百至数千个高度相关选项的情况，且难以扩展到检索式问题。

Method: 两阶段方法：先让MLLM回答开放式问题，然后用仅文本约束解码预测最可能选项；在检索设置中使用早停方法提高吞吐量。

Result: 在7个细粒度视觉数据集上的分类和检索评估中表现优于现有方法，且在不同自然语言任务实现方式下性能稳定。

Conclusion: nlg2choice方法能有效处理大规模选项的视觉分类和检索任务，为MLLM在细粒度视觉理解中的应用提供了实用解决方案。
...
Method: 使用MLLM查询不同时刻的对象对视觉输入，生成文本描述作为高级表示，在测试时与正常训练视频的文本描述进行比较来检测异常。

Result: 在基准数据集上的广泛实验表明，该方法不仅能有效检测基于交互的复杂异常，在无交互异常的数据集上也达到了最先进的性能。

Conclusion: 该方法通过MLLM提供可解释性，并能与许多传统VAD方法结合进一步增强其可解释性。
...
TL;DR: 3DScenePrompt是一个视频生成框架，通过双时空条件和3D场景记忆实现精确相机控制和场景一致性，在长视频生成中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常基于单张图像或短视频进行条件生成，难以在长视频生成中保持场景一致性和精确相机控制，特别是在跨越时间边界时动态元素会错误保留。

Method: 采用双时空条件机制：时间相邻帧用于运动连续性，空间相邻内容用于场景一致性。引入3D场景记忆存储从输入视频中提取的静态几何，通过动态SLAM和动态掩码策略分离静态场景和动态元素。

Result: 实验证明该框架在场景一致性、相机可控性和生成质量方面显著优于现有方法，能够保持长距离空间连贯性和精确相机控制，同时不牺牲计算效率或运动真实感。

Conclusion: 3DScenePrompt通过创新的3D场景记忆和双时空条件机制，成功解决了长视频生成中的场景一致性和相机控制问题，为可控视频生成提供了有效解决方案。
...
TL;DR: 提出了一种连续掩码自回归运动变换器，结合DiT结构和多模态融合机制，在文本到动作、语音到手势、音乐到舞蹈等任务中优于现有方法。
...
Method: 1. 视觉操作阶段：在1520万对语料上预训练，包括1000万标题-图表对和520万逐步编辑轨迹，掌握图表生成和编辑能力。2. 策略性视觉辅助推理阶段：在21.9万示例数据集上微调，学习何时以及如何利用视觉辅助进行推理。
...
TL;DR: 提出了一种集成Token-wise Attention的扩散模型，用于降水临近预报，解决了现有方法在可扩展性和长程时空依赖性建模方面的限制。


<details>
  <summary>Details</summary>
Motivation: 降水临近预报面临大气固有的混沌特性和紧密耦合的时空动态挑战。现有扩散模型存在可扩展性问题：潜在空间方法需要单独训练自编码器，增加了复杂性；像素空间方法计算密集且缺乏注意力机制，难以建模长程时空依赖。

Method: 提出Token-wise Attention机制，不仅集成到U-Net扩散模型中，还集成到时空编码器中，动态捕捉多尺度空间交互和时间演化。该方法原生集成注意力而无需高资源成本，消除了单独潜在模块的需求。

Result: 在多个数据集上的广泛实验和视觉评估表明，该方法显著优于最先进方法，在复杂降水预报场景中实现了卓越的局部保真度、泛化能力和鲁棒性。

Conclusion: 所提出的方法通过集成Token-wise Attention，有效解决了降水临近预报中的可扩展性和时空依赖性建模问题，为复杂降水预报提供了更优的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中机器人需要在场景不断变化的情况下从自然语言指令中定位物体，但现有3D视觉定位方法假设需要重建和更新的点云，这导致昂贵的重新扫描成本并阻碍实际部署。

Method: 提出Mem-ChangingGrounder方法，结合跨模态检索和轻量级多视图融合：识别查询隐含的物体类型，检索相关记忆指导行动，在场景中高效探索目标，当先前操作无效时回退，执行目标的多视图扫描，并将多视图扫描的融合证据投影以获得准确的物体边界框。
...
<details>
  <summary>Details</summary>
Motivation: 近距离人-人交互姿态蕴含丰富的交互动态信息，人类能从中直观推断上下文并预测可能的过去和未来动态。受此启发，作者希望利用交互姿态先验来实现通用的交互动画生成。
...
TL;DR: 提出无需配对数据的图像编辑训练新范式，通过扩散模型展开训练并利用视觉语言模型反馈进行端到端优化，在少步设置下达到与监督学习方法相当的性能。


<details>
  <summary>Details</summary>
Motivation: 现有图像编辑模型依赖大规模输入-目标配对数据进行监督微调，但自然配对的训练数据难以大规模获取，现有解决方案会传播预训练模型的伪影问题。

Method: 直接优化少步扩散模型，在训练时展开模型并利用视觉语言模型评估编辑是否遵循指令和保留未改变内容，提供直接梯度进行端到端优化，同时结合分布匹配损失确保视觉保真度。

Result: 在标准基准测试中，无需任何配对数据的情况下，该方法在少步设置下与使用大量监督配对数据训练的图像编辑扩散模型性能相当，且优于基于强化学习的技术如Flow-GRPO。

Conclusion: 该工作证明无需配对数据即可训练高质量图像编辑模型，通过VLM反馈和分布匹配损失实现了与监督方法竞争的性能，解决了数据瓶颈问题。
...
<details>
  <summary>Details</summary>
Motivation: 将强大的进化概念应用于大语言模型领域，基于通用的科学发现方法，解决复杂计算问题。
...
TL;DR: 提出了DOTechnique方法，通过决策一致性而非输出相似性来确定模型有效性，在缺乏明确有效性边界的情况下识别有效区域。


<details>
  <summary>Details</summary>
Motivation: 模型有效性对决策过程至关重要，传统方法依赖预定义有效性框架，但这些框架可能不可用或不充分。

Method: DOTechnique方法通过评估替代模型是否与高保真模型产生等效决策来确定有效性，结合领域约束和符号推理来缩小搜索空间。
...
TL;DR: 该论文提出了一种融合视觉信息（特别是演示文稿幻灯片）的多模态自动语音识别系统，用于科学演示场景，通过数据增强方法解决了数据集缺乏的问题，相比基线模型在词错误率上取得了显著降低。


<details>
  <summary>Details</summary>
Motivation: 现有SOTA ASR系统主要依赖声学信息而忽略了多模态上下文，但视觉信息对于消歧和适应至关重要。特别是在科学演示场景中，演示文稿幻灯片包含重要的领域特定术语信息。

Method: 创建多模态演示基准，开发自动分析领域特定术语转录的方法；通过数据增强方法解决缺乏配套幻灯片数据集的问题；使用增强数据集训练多模态语音模型。

Result: 相比基线模型，训练出的模型在所有词汇上相对减少了约34%的词错误率，在领域特定术语上相对减少了35%的词错误率。

Conclusion: 集成视觉信息（特别是演示文稿幻灯片）可以显著提升ASR系统在科学演示场景中的性能，特别是在处理领域特定术语方面效果显著。
...
TL;DR: 大型语言模型在因果推理任务中表现出系统性因果幻觉，即使在没有足够证据支持的情况下也会推断出因果关系


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型是否会在经典的认知科学范式——列联判断任务中发展出因果幻觉，这种认知偏差被认为是许多社会问题的基础

Method: 构建了1000个医学背景下的零列联场景数据集，在这些场景中可用信息不足以建立变量间的因果关系，然后让LLMs评估潜在原因的有效性

Result: 所有评估的模型都系统地推断出无根据的因果关系，显示出对因果幻觉的强烈易感性

Conclusion: 研究结果支持LLMs只是复制因果语言而没有真正理解的假设，并对其在需要准确因果推理领域的使用提出担忧
...
TL;DR: GammaZero是一个基于动作中心图表示框架的POMDP规划方法，能够在小规模问题上学习后，零样本泛化到更大规模的问题。
...
Method: 将信念状态转换为动作中心图，使用图神经网络从专家演示中学习价值函数和策略，然后用这些启发式指导蒙特卡洛树搜索。

Result: 在标准POMDP基准测试中，GammaZero在相同规模问题上性能与BetaZero相当，且能零样本泛化到训练规模2-4倍大的问题，保持解质量的同时减少搜索需求。
...
TL;DR: 提出一种替代性的AI监管方法：要求大型AI实验室发布小型开放模拟模型，这些模型是从其最大专有模型中蒸馏训练而来的缩小版本，既能确保AI安全又能促进创新。
...
Method: 强制要求大型AI实验室发布小型开放模拟模型，这些模型训练方式类似且从最大专有模型中蒸馏而来，作为公共代理供广泛参与安全验证、可解释性研究和算法透明度研究。

Result: 研究表明基于小型模型开发的安全和可解释性方法能有效泛化到前沿规模系统，显著降低监管负担并加速安全进展。

Conclusion: 这种监管方法成本最小，利用可重用资源，为公共福祉做出重要贡献，同时表明对模型的深入理解能够缓解安全与创新的权衡，实现两者兼得。
...
TL;DR: 提出基于多目标马尔可夫决策过程的共识声明生成框架，通过社会选择理论保证公平性，包含两种方法：随机生成策略保证ex-ante核心稳定性，以及搜索算法最大化平等主义福利。


<details>
  <summary>Details</summary>
Motivation: 现有共识声明生成框架缺乏可证明的公平性保证结构，无法在聚合多样化自由形式意见时提供公平性保障。

Method: 将任务建模为多目标、令牌级马尔可夫决策过程，每个目标对应一个代理的偏好。基于社会选择理论提出两种方法：随机生成策略保证ex-ante核心稳定性，以及搜索算法最大化平等主义福利。
...
TL;DR: 提出了STEMS框架，一种安全约束的多智能体强化学习方法，用于协调建筑能源管理，显著降低能耗成本、碳排放和安全违规。


<details>
  <summary>Details</summary>
Motivation: 解决多建筑能源系统中空间-时间依赖性利用不足、缺乏严格安全保障和系统复杂性的关键挑战。

Method: 结合空间-时间图表示学习（GCN-Transformer融合架构）和安全约束多智能体强化学习（集成控制障碍函数）。

Result: 在真实建筑数据集上，STEMS实现21%成本降低、18%排放减少，安全违规从35.1%降至5.6%，仅0.13的不适比例。

Conclusion: STEMS框架在多建筑能源管理中表现出优越性能，具有强鲁棒性和跨建筑类型的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 当前代理间通信协议碎片化导致语义鸿沟，无法进行严格的系统属性分析，存在架构错位和可利用的协调问题风险。

Method: 开发了两个基础模型：主机代理模型（负责用户交互、任务分解和编排）和任务生命周期模型（详细描述子任务状态转换），并定义了17个主机代理属性和14个任务生命周期属性。

Result: 创建了首个严格基础的、领域无关的框架，能够进行形式化验证、检测协调边界情况和预防死锁及安全漏洞。

Conclusion: 该框架为多AI代理系统的系统分析、设计和部署提供了统一语义基础，确保系统的正确性、可靠性和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 文化遗产面临气候变化加速退化，传统单模态监测方法无法捕捉环境压力与材料退化间的复杂相互作用。

Method: 基于PerceiverIO的轻量多模态架构，采用简化编码器(64D潜在空间)和自适应Barlow Twins损失函数，鼓励模态互补性。

Result: 在斯特拉斯堡大教堂数据上达到76.9%准确率，比标准多模态架构提升43%，比单模态传感器(61.5%)和图像(46.2%)表现更好。

Conclusion: 架构简洁性结合对比正则化可在数据稀缺的遗产监测环境中实现有效的多模态学习，为AI驱动的保护决策支持系统奠定基础。
...
<details>
  <summary>Details</summary>
Motivation: 尽管强化学习研究进展迅速，但在商业视频游戏中的应用仍然缓慢。本文旨在解决游戏AI社区在使用RL驱动NPC时面临的常见挑战，并探索RL与行为树结合的关键交叉点。

Method: 使用AMD Schola插件在虚幻引擎中训练RL代理，在受《最后生还者》启发的复杂3D环境中创建多任务NPC，展示了RL模型与行为树联合训练的详细方法。

Result: 验证了RL与行为树结合方法的可行性，成功展示了各种技能的多任务NPC在复杂3D环境中的表现。

Conclusion: RL与行为树的结合是一个值得进一步探索的关键交叉点，虽然已有研究提出但实际应用仍然罕见，本文证明了该方法的实际可行性。
...
TL;DR: JEDA是一个用于临床订单检索的双编码器系统，能够直接从临床对话中检索规范订单，支持查询和无查询两种模式，无需LLM重写，提供实时、可解释的临床订单检索。


<details>
  <summary>Details</summary>
Motivation: 临床对话混合了明确指令和隐含推理，现有系统依赖LLM重写会带来延迟、不稳定和不透明的问题，阻碍实时订单处理。

Method: 使用PubMedBERT初始化，通过重复安全的对比目标进行微调，采用受限LLM指导将签名订单与补充表述绑定，训练双编码器对齐异构意图表达。

Result: JEDA在实践中取得了显著提升，大幅优于基础编码器和近期开源嵌入器，无查询模式对噪声具有鲁棒性。
...
TL;DR: ARM-FM：通过基础模型自动生成奖励机，利用基础模型的高层推理能力实现强化学习中奖励函数的自动化、组合式设计。


<details>
  <summary>Details</summary>
Motivation: 强化学习算法对奖励函数规范高度敏感，这是限制其广泛应用的核心挑战。需要一种方法来简化奖励函数设计过程。

Method: 使用基础模型从自然语言规范自动生成奖励机，将语言嵌入与奖励机状态关联以实现跨任务泛化。

Result: 在多样化挑战性环境中验证了ARM-FM的有效性，包括零样本泛化能力的证据。

Conclusion: ARM-FM框架通过结合奖励机的结构化形式和基础模型的自然语言理解能力，为强化学习提供了自动化的奖励设计解决方案。
...
TL;DR: 对2019-2024年精准医学中AI实施的文献进行范围综述，识别关键障碍和推动因素，提出基于生态系统的框架来支持可信赖和可持续的实施。
...
Result: 识别了精准医学AI实施的关键障碍和推动因素，强调了塑造真实世界转化的相互依赖关系。

Conclusion: 提出了支持可信赖和可持续AI实施未来方向，强调生态系统方法的重要性。
...
TL;DR: 提出在线骚扰代理基准，包括多轮骚扰对话数据集、多代理模拟、三种越狱方法和混合评估框架。研究发现越狱调优显著提高骚扰攻击成功率，降低拒绝率，并揭示闭源和开源模型的不同升级轨迹。
...
Method: 构建包含多轮骚扰对话数据集、基于重复博弈论的多代理模拟、针对记忆、规划和微调的三种越狱方法，以及混合评估框架。使用LLaMA-3.1-8B-Instruct和Gemini-2.0-flash两个模型进行测试。

Result: 越狱调优使骚扰攻击成功率从57.25-64.19%提升到95.78-96.89%（Llama），从98.46%提升到99.33%（Gemini）。拒绝率降至1-2%。侮辱行为从44.2-50.8%增至84.9-87.8%，谩骂行为从31.5-38.8%增至81.2-85.1%。闭源模型表现出显著脆弱性。

Conclusion: 多轮和理论基础的攻击不仅能以高成功率实施骚扰，还能模拟人类骚扰动态，强调需要开发强大的安全防护措施来保护在线平台安全。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准在评估深度研究系统时存在不足，往往局限于窄领域或提出模糊问题，无法公平比较。需要遵循用户导向、动态性、明确性和多面性四个原则来构建更有效的评估框架。

Method: 构建了包含100个专家策划任务的LiveResearchBench基准，覆盖日常生活、企业和学术领域，每个任务都需要广泛的动态实时网络搜索和综合。同时开发了DeepEval评估套件，包含内容和报告层面的质量评估，整合了四种互补的评估协议。

Result: 使用该框架对17个前沿深度研究系统进行了全面评估，包括单代理网络搜索、单代理深度研究和多代理系统。分析揭示了当前系统的优势、常见失败模式和推进可靠深度研究所需的关键系统组件。

Conclusion: LiveResearchBench和DeepEval为系统评估深度研究能力提供了严格基础，有助于识别当前系统的局限性和未来发展方向。
...
TL;DR: 本文提出Agentic Self-Learning (ASL)框架，通过多角色协同进化的强化学习实现无需人工标注数据的智能体自我学习，发现奖励信号来源和任务数据规模是开放领域智能体训练的关键因素。


<details>
  <summary>Details</summary>
Motivation: 研究如何在不依赖人工标注数据集或预定义规则奖励的情况下，实现基于LLM的智能体的规模化训练，探索开放领域智能体自我学习的关键因素。

Method: 提出ASL框架，包含提示生成器、策略模型和生成式奖励模型三个角色，在共享工具环境和LLM骨干网络中形成任务生成、策略执行和评估的闭环强化学习循环。

Result: ASL实现了持续的多轮性能提升，超越了会平台化或退化的强基线方法，在零标注数据条件下仍能持续改进，表现出优异的样本效率和鲁棒性。
...
TL;DR: 提出了MorphoBench基准测试，用于评估大型模型的推理能力，能够根据模型能力自适应调整问题难度，包含1300多个测试问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM在微调时容易失去预训练的安全对齐，即使使用良性数据或低秩适配也会产生有害响应，需要解决安全行为退化问题。

Method: 使用协方差预处理奇异值分解将预训练权重分解为安全相关和安全无关组件，初始化低秩适配器时冻结安全相关部分；构建空空间投影器限制适配器更新对有害提示的安全输出。

Result: 在多个下游任务实验中，GuardSpace优于现有方法。Llama-2-7B-Chat在GSM8K上微调时，有害分数从14.4%降至3.6%，准确率从26.0%提升至28.0%。

Conclusion: GuardSpace能有效保护LLM在微调过程中的安全对齐，在保持性能的同时显著降低有害响应风险。
...
TL;DR: 提出了Terrarium框架，用于研究基于LLM的多智能体系统中的安全、隐私和安全性问题，通过重构黑板设计创建模块化测试平台。


<details>
  <summary>Details</summary>
Motivation: LLM驱动的多智能体系统能够自动化用户任务，但引入了新的风险，包括错位、恶意方攻击、智能体被攻陷或用户数据被盗。

Method: 重新利用多智能体系统中的黑板设计，创建模块化、可配置的多智能体协作测试平台，识别关键攻击向量如错位、恶意智能体、通信被攻陷和数据投毒。

Result: 实现了三个协作多智能体场景和四种代表性攻击，展示了框架的灵活性。

Conclusion: 通过提供快速原型设计、评估和防御迭代的工具，Terrarium旨在加速可信多智能体系统的进展。
...
<details>
  <summary>Details</summary>
Motivation: 解决多智能体系统中单个错误步骤会级联传播并破坏整个轨迹的问题，需要实时检测和校正机制。

Method: 采用两种互补设计：1) 基于查询和交互历史预测下一步执行嵌入的Next-Execution Reconstruction；2) 学习正常步骤嵌入原型并用于稀疏上下文下稳定重建和异常评分的Prototype-Guided Enhancement。

Result: 在Who&When基准测试中，MASC显著优于所有基线方法，步骤级错误检测AUC-ROC提升高达8.47%，在不同MAS框架中都能带来一致的端到端性能提升。

Conclusion: 元认知监控和针对性校正能够以最小开销缓解错误传播，增强多智能体系统的鲁棒性。
...
TL;DR: 提出了AI4Service新范式，通过Alpha-Service框架实现主动式AI助手，能够从第一人称视角视频中检测服务机会，提供个性化和通用化服务。


<details>
  <summary>Details</summary>
Motivation: 现有AI服务多为被动响应，而真正智能的助手应能预见用户需求并在适当时机主动干预。
...
Conclusion: AI4Service范式为AI从被动工具向主动伴侣的转变提供了可行路径，Alpha-Service框架展示了主动式AI服务的实现潜力。
...
TL;DR: 提出IP-Merging方法，通过识别多模态大语言模型和数学推理大语言模型中的关键推理参数，将其投影到MLLM子空间进行合并，无需调优即可提升MLLM的数学推理能力。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在数学推理能力上落后于纯文本大语言模型，但直接使用模型合并方法会因参数空间不匹配导致性能下降。

Method: IP-Merging方法：1）识别MLLM和Math LLM中的推理相关参数；2）将这些参数投影到MLLM子空间；3）在子空间内合并参数，无需训练调优。

Result: 实验表明IP-Merging能有效提升MLLM的数学推理能力，同时不损害其他能力。

Conclusion: IP-Merging是一种无需调优的方法，能成功将数学推理能力从Math LLM转移到MLLM，解决了参数空间不匹配的问题。
...
TL;DR: Hi-Agent是一种可训练的分层视觉语言代理，通过高层推理模型和低层动作模型的联合优化，在移动设备控制任务中实现了最先进的性能表现。
...
Method: 提出分层架构，将多步决策重新表述为单步子目标序列，并设计了前瞻优势函数，利用低层模型的执行反馈来指导高层优化，解决了长视野任务中的路径爆炸问题。

Result: 在Android-in-the-Wild基准测试中达到87.9%的任务成功率，显著优于基于提示、监督学习和强化学习的现有方法，并在ScreenSpot-v2和AndroidWorld基准上展示了良好的零样本泛化能力。

Conclusion: Hi-Agent通过分层设计和联合优化策略，有效解决了移动设备控制中的泛化问题，在多个基准测试中实现了最先进的性能。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在复杂推理和规划任务中表现不佳，多智能体系统虽然能提供更好的集体推理，但存在推理成本高、延迟长和端到端训练困难等问题。

Method: 提出IMAGINE框架，通过端到端训练将多智能体系统的能力集成到单一模型中，使用Qwen3-8B-Instruct作为基础模型进行训练。

Result: 训练后的单一小规模模型在TravelPlanner基准上达到82.7%的最终通过率，远超DeepSeek-R1-671B的40%，同时保持更小的模型规模。

Conclusion: IMAGINE框架成功将多智能体系统的结构化推理和规划能力集成到单一模型中，显著提升了性能，同时解决了MAS的高成本和训练困难问题。
...
<details>
  <summary>Details</summary>
Motivation: PDDL标准限制公理体中谓词的负出现只能用于直接由动作设置的谓词，而不能用于由公理派生的谓词。然而文献中经常偏离这一限制，只要求公理集是可分层的。本文旨在证明这两种变体实际上可以表达相同的查询。
...
Conclusion: PDDL标准对公理中派生谓词负出现的限制实际上不会减少表达能力，因为通过提出的转换方法，这些负出现可以被消除，从而保持相同的表达能力。
...
TL;DR: Helmsman是一个多智能体系统，通过三个协作阶段自动化联邦学习系统的端到端合成：交互式规划、模块化代码生成和自主评估优化。


<details>
  <summary>Details</summary>
Motivation: 联邦学习系统设计复杂，面临数据异构性和系统约束等多方面挑战，导致解决方案脆弱且定制化，需要一个自动化系统来简化这一过程。

Method: 采用三阶段方法：1）人机交互规划制定研究计划；2）监督智能体团队进行模块化代码生成；3）在沙盒环境中进行自主评估和优化的闭环流程。

Result: 实验表明，该方法生成的解决方案与手工构建的基线方法竞争，甚至更优。

Conclusion: 该工作代表了复杂去中心化AI系统自动化工程的重要进展。
...
TL;DR: JSPLIT是一个基于分类学的框架，用于解决大型语言模型使用MCP工具时提示膨胀问题，通过分层分类和工具选择算法显著减少提示大小并提高工具选择准确性。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统发展，用户期望超越简单文本交互，需要LLM与外部工具交互。但工具数量增加导致提示膨胀，带来高token成本、延迟增加和任务成功率下降的问题。

Method: JSPLIT将工具组织成层次化分类学，根据用户提示和分类结构识别并仅包含最相关工具，包括分类设计、工具选择算法和评估数据集。

Result: JSPLIT显著减少提示大小而不显著影响代理响应能力，在工具数量大幅增加时甚至提高工具选择准确性，有效降低成本同时提高高复杂度环境中的任务成功率。

Conclusion: JSPLIT框架能有效管理提示大小，在工具数量增长时改善代理性能，为复杂代理环境提供成本效益高的解决方案。
...
TL;DR: 神经符号AI（NeSy）结合神经网络和符号推理，但存在推理捷径（RSs）问题——模型可能通过错误地概念化来实现高精度，影响可解释性和可靠性。本文概述了RSs的成因、后果、理论特征及应对方法。


<details>
  <summary>Details</summary>
Motivation: 神经符号AI旨在开发符合先验知识约束的可靠AI系统，但推理捷径问题会损害模型的可解释性和分布外性能，且现有研究分散，需要统一视角来降低研究门槛。

Method: 本文提供了RSs的直观介绍，讨论了其成因和后果，回顾了现有理论特征，并详细阐述了包括缓解和意识策略在内的应对方法。

Result: 通过重新表述高级材料为易消化形式，本文为理解RSs提供了统一视角，有助于开发更可靠的神经符号AI模型。

Conclusion: 本文概述有助于降低研究RSs的门槛，促进可靠神经符号AI和可信AI模型的发展。
...
TL;DR: 本研究探讨了预训练LLM代理能否通过自主生成任务、积累知识和环境交互，实现开放式的自主实体能力。


<details>
  <summary>Details</summary>
Motivation: 随着LLM代理能力增强，研究其是否能从单纯的问题解决工具发展为能够自主规划、设计任务并追求模糊目标的实体。
...
Conclusion: 研究显示了将预训练LLM适应开放式的潜力与当前局限，为未来训练代理管理记忆、有效探索和追求抽象长期目标指明了方向。
...
TL;DR: 本文提出了ColorBench，一个用于评估移动代理在复杂长程任务中性能的图结构基准框架，解决了现有离线静态基准和在线动态测试的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有移动代理评估标准存在缺陷：离线静态基准只能验证单一预设路径，而在线动态测试受限于真实设备的复杂性和不可重现性，两者都无法全面评估代理能力。

Method: 引入图结构基准框架，通过建模真实设备交互中的有限状态，实现动态行为的静态模拟。开发ColorBench基准，包含175个任务（74个单应用，101个跨应用），平均长度超过13步，每个任务包含至少两条正确路径和典型错误路径。

Result: 通过在各种基线模型上评估ColorBench，发现了现有模型的局限性，并基于实验结果提出了改进方向和可行的技术路径。

Conclusion: ColorBench框架能够有效评估移动代理在复杂长程任务中的性能，支持多有效解评估、子任务完成率统计和原子级能力分析，为增强代理在复杂问题上的表现提供了可行方案。
...
TL;DR: 本文提出了Rose-Frame框架，用于诊断人类-AI交互中的认知和认识论漂移，通过三个维度（地图vs领土、直觉vs理性、冲突vs确认）来增强AI部署的透明度和批判意识。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然流畅且情感共鸣，但基于统计预测而非基础推理，存在幻觉风险，可能产生听起来令人信服但缺乏事实有效性的回答。

Method: 引入Rose-Frame三维框架：(i)地图vs领土区分现实表征与现实本身；(ii)直觉vs理性基于双过程理论分离快速情感判断与缓慢反思思考；(iii)冲突vs确认检验想法是通过分歧批判测试还是通过相互验证强化。

Result: Rose-Frame不试图用更多数据或规则修复LLM，而是提供一个反思工具，使模型局限性和用户假设可见，实现更透明和批判性意识的AI部署。

Conclusion: 将对齐重新定义为认知治理：无论是人类还是人工智能的直觉，都必须受到人类理性的治理。只有通过嵌入反思性、可证伪的监督，我们才能将机器的流畅性与人类的理解对齐。
...
TL;DR: 本文对2021-2025年荷兰公共卫生机器学习研究中的算法偏见进行了系统文献综述，开发了RABAT评估工具，发现存在普遍的公平性问题，并提出了ACAR四阶段框架来帮助研究人员在ML生命周期中解决公平性问题。
...
Method: 开发了风险算法偏见评估工具(RABAT)，整合了Cochrane偏见风险、PROBAST和微软负责任AI检查表等成熟框架，应用于35篇同行评审研究。

Result: 分析显示普遍存在差距：虽然数据采样和缺失数据实践有良好记录，但大多数研究忽略了明确的公平框架、亚组分析以及对潜在危害的透明讨论。

Conclusion: 提出了ACAR四阶段公平导向框架，为公共卫生ML从业者提供可操作建议，确保算法创新促进健康公平而非破坏它。
...
<details>
  <summary>Details</summary>
Motivation: 传统检索系统无法有效处理网络威胁情报的自然语言查询与结构化知识图谱之间的语义鸿沟，需要一种能够自动生成可执行推理路径的系统。

Method: 集成路径规划模型预测逻辑关系链，图执行器遍历TITAN本体图谱，在MITRE衍生的双向类型化图谱上进行可逆推理。

Result: 构建了包含88209个样本的TITAN数据集，实证评估显示模型能生成语法有效、语义连贯的可执行推理路径。

Conclusion: TITAN框架成功实现了自然语言威胁查询到结构化知识图谱的可执行推理，为网络威胁情报分析提供了自动化解决方案。
...
TL;DR: NAEL是一个基于主动推理和符号推理的非人类中心主义AI伦理框架，将伦理行为形式化为智能系统在动态多智能体环境中最小化全局期望自由能量的涌现属性。


<details>
  <summary>Details</summary>
Motivation: 解决现有AI伦理模型局限于人类中心主义的问题，让智能体能够在不确定环境中发展上下文敏感、自适应和关系性的伦理行为，无需预设人类道德直觉。

Method: 提出神经符号架构，结合主动推理和符号推理，使智能体能够评估在不确定环境中的行为伦理后果。

Result: 通过伦理资源分配的案例研究展示了NAEL能够动态平衡自我保存、认知学习和集体福利。

Conclusion: NAEL框架为人工智能体提供了一种非人类中心主义的伦理推理方法，能够适应动态环境并发展出上下文敏感的伦理行为。
...
TL;DR: 本文改进了COUP算法配置方法，使其在保持理论保证的同时，实际性能与广泛使用的启发式配置方法相竞争。


<details>
  <summary>Details</summary>
Motivation: COUP是一种功利主义算法配置程序，主要关注理论保证而忽视了实际性能。本文旨在弥补这一差距，使基于理论的功利主义算法配置在实际应用中具有竞争力。

Method: 提出了一系列对COUP的改进措施，这些改进在不降低理论保证的前提下提升了实证性能，并通过实验验证了其效益。

Result: 改进后的COUP算法配置方法在实证性能上显著提升，能够与广泛使用的启发式配置方法竞争，同时保持理论保证。

Conclusion: 通过案例研究展示了如何探索算法选择问题解决方案对效用函数变化的鲁棒性，证明了改进后的COUP方法在实际应用中的有效性。
...
Method: 使用训练样本获取协方差矩阵，进行上下文导向的奇异值分解，在知识感知子空间中将权重分为任务相关和冗余组件，通过剪枝冗余组件净化任务向量。

Result: PAVE作为即插即用方案，可应用于各种基于任务向量的合并方法，在多样化合并方法、任务和模型架构中均显示有效性。

Conclusion: PAVE方法通过知识感知的冗余净化，显著提升了模型合并的性能，是一种通用的任务向量净化方案。
...
TL;DR: CoAST是一个认知对齐的时空大语言模型框架，通过自然语言接口整合世界知识、时空轨迹模式和用户画像，用于下一个兴趣点推荐。
...
Method: CoAST包含两个阶段：(1) 在脱敏用户的时空轨迹数据上继续预训练获取推荐知识；(2) 通过监督微调和强化学习实现认知对齐，使模型判断与人类偏好一致。

Result: 在多个真实数据集上的离线实验和在AMAP App首页"猜你去哪"功能中的在线实验都证明了CoAST的有效性。

Conclusion: CoAST框架成功解决了大语言模型在下一个兴趣点推荐任务中的局限性，通过认知对齐显著提升了推荐性能。
...
TL;DR: 本文分析了强化学习与可验证奖励（RLVR）方法存在的系统性偏向利用而非探索的问题，提出了Simple Pass@K Optimization (SimKO)方法来解决概率过度集中问题。


<details>
  <summary>Details</summary>
Motivation: 现有RLVR方法存在系统性偏向利用而非探索的问题，表现为pass@1性能提升但pass@K（K>1）性能下降。作者希望通过分析训练动态来理解这一问题。

Method: 通过跟踪词汇候选的token级概率分布，发现概率集中效应。提出SimKO方法，采用非对称设计：对已验证正确的响应，提升top-K候选的概率；对已验证错误的响应，对top-1候选施加更强惩罚。

Result: SimKO方法在各种数学和逻辑推理基准测试中，对广泛的K值范围都能获得更高的pass@K性能。

Conclusion: SimKO通过缓解概率过度集中问题有效鼓励了探索，为改进RLVR的探索能力提供了简单有效的方法。
...
TL;DR: Datalake Agent是一个基于LLM的智能代理系统，通过交互式循环选择性获取必要元信息，显著减少NL2SQL任务中的token使用量和处理成本。


<details>
  <summary>Details</summary>
Motivation: 传统NL2SQL方法需要处理大量数据库元信息，导致提示过长、token使用量高和处理成本昂贵。

Method: 采用交互式循环和推理框架，让LLM选择性请求必要信息来解决表问答任务，而不是一次性提供所有元信息。

Result: 在23个数据库和100个表问答任务上的评估显示，token使用量减少高达87%，同时保持竞争力性能。

Conclusion: Datalake Agent能够显著降低NL2SQL任务的成本，同时保持性能，为大规模数据库应用提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决大型语言模型和视觉语言模型在复杂真实世界环境中执行长视距操作任务时，由于常识和推理能力受限而面临的挑战。

Method: 采用两阶段微调框架：监督训练获取基础知识，强化学习解决模型在视觉空间理解和推理方面的不足，设计了考虑长视距性能和动作约束的基于规则的奖励函数。

Result: 在Qwen2.5-VL-3B上训练得到的推理模型，在EmbodiedBench基准上比GPT-4o-mini高出21.33%，比在Qwen2.5-VL-7B上训练的其他工作高出20.33%。

Conclusion: RoboGPT-R1框架通过结合监督训练和强化学习，有效提升了具身智能体在复杂操作任务中的推理能力，证明了该方法的有效性。
...
TL;DR: Instruction Boosting是一种后处理方法，通过增加指令遵循率来提高LLM提示指令的可靠性，最多可提升7个百分点。


<details>
  <summary>Details</summary>
Motivation: 开发者通常通过修改提示来影响LLM行为，但仅添加更多指令无法保证它们被遵循，需要提高指令遵循的可靠性。

Method: 引入Instruction Boosting作为后生成方法，并创建SCALEDIF基准测试，包含最多10条指令的数据样本。

Result: Instruction Boosting将指令遵循率最多提升7个百分点（两条指令）和4个百分点（十条指令）。

Conclusion: 指令数量增加导致性能下降的主要原因是指令间的冲突和紧张关系，开发了定量冲突评分工具来解释性能趋势并为开发者提供反馈。
...
TL;DR: 本文提出了一个紧凑的形式理论，用于描述和衡量由领域先验引导的LLM辅助迭代搜索，通过模糊关系算子表示智能体，引入覆盖生成函数来衡量可达性难度。


<details>
  <summary>Details</summary>
Motivation: 基于LLM的生成-过滤-精炼迭代范式在推理、编程和科学发现中取得进展，但搜索有效性取决于如何将领域先验编码为操作化结构化的假设空间。

Method: 将智能体表示为输入输出的模糊关系算子，约束在固定安全包络内；通过单参数加权所有可达路径得到覆盖生成函数，诱导可达性难度度量；提供可测试推断并通过多数投票实例验证。

Result: 该理论提供了可操作的语言和工具来测量智能体及其搜索空间，为LLM构建的迭代搜索提供了系统化的形式描述。

Conclusion: 提出的形式理论为LLM辅助的迭代搜索提供了工作语言和操作工具，能够系统化地描述和测量这类搜索过程。
...
TL;DR: LabOS是首个将计算推理与物理实验相结合的人工智能共同科学家，通过多模态感知、自进化代理和扩展现实(XR)支持的人机协作，实现AI与科学家在实验室环境中的实时合作。


<details>
  <summary>Details</summary>
Motivation: 现代科学发展需要思想与行动的结合，当前AI主要停留在计算设计阶段，缺乏与物理实验的直接互动能力。LabOS旨在让AI能够参与实际实验过程，理解实验环境并在实时执行中提供协助。

Method: 通过连接多模型AI代理、智能眼镜和人机协作系统，使AI能够看到科学家所见的场景，理解实验背景，并在实时执行中提供帮助。系统集成了多模态感知、自进化代理和XR技术。

Result: 在癌症免疫治疗靶点发现和干细胞工程等应用中，LabOS证明AI能够超越计算设计，直接参与实验过程，将实验室转变为智能协作环境。

Conclusion: LabOS展示了AI从纯粹计算设计向实际参与实验的转变，创造了人机共同进化的智能协作实验室环境，为科学研究提供了新的合作模式。
...
<details>
  <summary>Details</summary>
Motivation: LLM作为自主代理部署时面临上下文窗口有限、状态不同步等限制，导致输出不可靠、行为不可预测、资源使用效率低，特别是在与代码库和文档等结构化知识系统交互时

Method: 引入Gatekeeper协议框架，要求代理先在简约的低保真"潜在状态"表示上进行操作和推理，然后按需策略性地请求高保真上下文，所有交互通过统一的JSON格式进行中介

Result: 该方法显著提高了代理可靠性，通过最小化token消耗改善了计算效率，并实现了与复杂系统的可扩展交互

Conclusion: Gatekeeper协议为在任何结构化知识领域构建更稳健、可预测和基于现实的AI代理创建了基础方法论
...
TL;DR: 提出了一种无需标注数据或模型权重更新的强化学习代理，用于解决企业智能平台中第三方日志的schema映射问题。该代理通过识别模糊字段映射、生成针对性网络搜索查询获取外部证据，并基于置信度奖励迭代优化映射，将准确率从56.4%提升至93.94%。


<details>
  <summary>Details</summary>
Motivation: 企业智能平台需要整合来自多个第三方供应商的日志，但供应商文档在测试时常常不可用、不匹配、格式混乱或不完整，这使得schema映射变得困难。

Method: 使用强化学习代理，在推理过程中：1)识别模糊的字段映射尝试；2)生成针对性的网络搜索查询来收集外部证据；3)应用基于置信度的奖励来迭代优化映射。

Result: 在将Microsoft Defender for Endpoint日志转换为通用schema的实验中，该方法将映射准确率从仅使用LLM的56.4%提升到使用RAG的72.73%，最终经过100次迭代达到93.94%。同时将需要专家审查的低置信度映射减少了85%。

Conclusion: 这种新方法提供了一种基于证据、透明的解决方案，为未来行业问题提供了更稳健、可问责、可扩展、高效、灵活、适应性强和协作性强的解决途径。
...
<details>
  <summary>Details</summary>
Motivation: 生成式验证器虽然性能优秀但计算成本过高，限制了实际应用。需要寻找更高效且预算友好的验证方法。

Method: 采用判别式验证器与自一致性结合的混合方法，在固定计算预算下进行测试时扩展。

Result: 在AIME2025任务上，混合方法比最先进的生成式验证准确率高出15.3%，且计算效率更高。
...
TL;DR: 本文系统研究了多模态抑郁症检测方法，比较了EEG、语音和文本三种模态的特征表示和建模策略，发现三模态组合能显著提升检测性能，预训练嵌入优于手工特征，精心设计的三模态模型达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 现有抑郁症自动检测研究存在范围有限、缺乏系统特征比较、评估协议不一致等问题，需要填补这些空白以推进多模态抑郁症检测研究。

Method: 系统探索EEG、语音和文本的特征表示和建模策略，评估手工特征与预训练嵌入，比较不同神经编码器的有效性，分析单模态、双模态和三模态配置，以及融合策略，并采用一致的受试者独立分割确保稳健可复现的基准测试。

Result: 结果显示：(i) EEG、语音和文本三模态组合增强多模态检测性能；(ii) 预训练嵌入优于手工特征；(iii) 精心设计的三模态模型达到最先进性能水平。

Conclusion: 本研究为未来多模态抑郁症检测研究奠定了基础，证明了多模态方法和预训练特征的有效性。
...
TL;DR: 将康德的《纯粹理性批判》重新解释为反馈稳定性理论，提出复合不稳定性指数(H-Risk)来量化推理系统的过自信问题，并在线性高斯模拟和LLMs中验证了该指标的有效性。
...
Result: 在线性高斯模拟中，高H-Risk预测过自信错误；在LLMs中，脆弱的内部动态与错误校准和幻觉相关，批判式提示对校准和幻觉的影响不一。
...
TL;DR: GroundedPRM是一个基于树搜索和外部工具验证的自动过程监督框架，通过MCTS构建结构化推理路径，使用外部工具验证中间步骤，结合步骤级验证和全局结果评估，仅用10%的数据量就能在ProcessBench上实现26%的相对性能提升。


<details>
  <summary>Details</summary>
Motivation: 现有过程奖励模型(PRMs)面临三大挑战：噪声奖励、低事实保真度以及与步骤级推理目标的不对齐。这些问题源于缺乏可扩展的高质量标注，现有方法依赖昂贵的人工标注、易产生幻觉的LLM自评估或存在信用分配错误的蒙特卡洛估计。

Method: 1) 使用蒙特卡洛树搜索(MCTS)构建结构化推理路径以减少奖励噪声；2) 通过外部工具验证每个中间步骤，提供基于执行的正确性信号；3) 设计混合奖励聚合机制，融合工具验证和MCTS反馈；4) 将奖励信号格式化为增强推理的生成结构。

Result: 仅使用4万个自动标注样本(最佳PRM所用数据的10%)，在ProcessBench上实现了高达26%的相对平均性能提升。在奖励引导的贪婪搜索中，甚至优于使用人工标注监督训练的PRMs。
...
TL;DR: 论文研究了大型语言模型在组合式机器设计中的能力，通过BesiegeField测试平台评估LLMs在空间推理、策略组装和指令遵循方面的表现，并探索了强化学习作为改进途径。
...
Method: 引入BesiegeField测试平台，基于Besiege游戏构建，支持基于部件的构造、物理模拟和奖励驱动评估；使用智能体工作流程对最先进的LLMs进行基准测试；探索强化学习微调作为改进方法。

Result: 当前开源模型在组合式机器设计任务中表现不足，特别是在空间推理、策略组装和指令遵循方面存在挑战。

Conclusion: 在语言、机器设计和物理推理的交叉领域仍存在开放挑战，强化学习微调显示出改进潜力，但需要进一步研究。
...
TL;DR: 提出了统一的多模态虚假内容检测框架UMFDet，能够同时处理人工制作和AI生成的虚假内容，解决了现有方法只能处理单一类型虚假内容的问题。


<details>
  <summary>Details</summary>
Motivation: 现有虚假内容检测方法通常只专注于人工制作或AI生成的单一类型虚假内容，但在真实场景中内容类型未知，限制了这些专门化系统的有效性。

Method: 基于构建的OmniFake数据集，提出UMFDet框架：使用VLM骨干网络，增强类别感知的MoE适配器来捕获类别特定线索，以及归因链式思维机制提供隐式推理指导。

Result: 大量实验表明UMFDet在两种虚假内容类型上都实现了稳健一致的性能，优于专门化基线方法。

Conclusion: UMFDet为真实世界的多模态欺骗检测提供了一个实用解决方案，能够统一处理不同类型的虚假内容。
