...
TL;DR: 该论文分析了影响地球表面绝对温度的主要因素，包括太阳常数、邦德反照率和二氧化碳浓度决定的发射率，并确定了三者的相对重要性。


<details>
  <summary>Details</summary>
Motivation: 研究地球表面温度的决定因素，特别是量化太阳常数、反照率和CO2浓度这三个关键参数对温度影响的相对重要性。

Method: 通过假设有效发射率由大气中CO2浓度决定，建立理论模型来分析这三个因素对地球表面温度的影响程度。

Result: 太阳常数是最重要的影响因素，反照率次之，CO2浓度的影响相对较小。

Conclusion: 在影响地球表面温度的三个主要因素中，太阳常数的作用最为关键，反照率位居第二，而CO2浓度的影响相对较弱。
...
TL;DR: 本文追溯了海洋云增亮(MCB)概念从1990年代作为云微物理过程的理论雏形到2015年发展为完整概念的初始发展历程，重点关注创始人John Latham和Stephen Salter的贡献，以及该概念作为缓解地球快速变暖的潜在方法的最新发展。


<details>
  <summary>Details</summary>
Motivation: 开发一种能够缓解地球快速变暖的潜在地球工程方法，通过人为增强海洋云的反射率来减少太阳辐射吸收。

Method: 基于云微物理学原理，通过向海洋云中注入适当大小的海盐气溶胶来增加云滴浓度和云反照率，从而增强云的冷却效应。

Result: MCB从理论概念逐步发展为具有实际应用前景的地球工程技术，在英国ARIA项目中得到进一步开发。

Conclusion: 海洋云增亮作为一种潜在的气候干预技术，经历了从理论到实践的演进过程，为应对全球变暖提供了新的技术途径。
...
TL;DR: 使用深度学习气候模拟器进行太平洋定步实验，发现虽然能准确捕捉大气内部变率，但对ENSO强迫的遥相关响应显著放大，导致极端事件模拟偏差。
...
Method: 使用耦合深度学习模拟器(DLESyM)进行太平洋定步(PACE)实验，隔离观测ENSO强迫下的大气响应。

Result: 模拟器能真实捕捉大气内部变率，但对ENSO的强迫遥相关响应显著放大，导致大气阻塞频率和持续时间高估，峰值强度低估。

Conclusion: 耦合深度学习气候模型需要像传统数值模型一样进行深入且物理基础扎实的验证，以建立其在物理气候分析中的可信度。
...
TL;DR: 该论文展示了将Kaggle竞赛获胜团队的机器学习架构集成到气候模型中的成果，证明了多种不同架构都能在低分辨率真实地理环境中实现在线稳定性，这是重要的里程碑。


<details>
  <summary>Details</summary>
Motivation: 解决子网格机器学习参数化在长期气候预测中的在线不稳定性和性能不一致问题，推动物理-AI混合气候模拟的发展。

Method: 将Kaggle竞赛获胜团队的机器学习架构与包含完整云微物理的交互式气候模型耦合，系统评估其在线性能。

Result: 多种不同架构都能实现在线稳定性，在特定指标上达到最先进水平，但不同架构对设计选择的响应差异显著。

Conclusion: 众包离线问题的本质是改进物理-AI混合气候模拟在线性能的有效途径，为新一代气候模型的发展提供了重要参考。
...
TL;DR: 该论文提出了一种基于生物启发的脉冲神经网络学习规则扩展，将时间驱动更新转换为事件驱动更新，并整合了连续动态、严格局部性等生物特征，展示了在百万神经元规模下的可扩展性。


<details>
  <summary>Details</summary>
Motivation: 尽管AI技术取得显著进步，但仍可从生物原理中获益，如循环连接和能量高效机制。受大脑启发，旨在开发计算效率高且可持续的生物启发AI系统。

Method: 扩展了资格传播(e-prop)学习规则，将其从时间驱动转换为事件驱动更新方案，整合到大规模脉冲神经网络仿真平台中，并加入了连续动态、严格局部性、稀疏连接等生物特征。

Result: 在神经形态MNIST等任务上验证了方法的适用性，展示了在百万神经元规模下的可扩展性，且不损害学习性能。

Conclusion: 生物基础约束可以为计算效率高的AI算法设计提供信息，这项工作连接了机器学习和计算神经科学，为可持续的生物启发AI系统铺平道路，同时推进对类脑学习的理解。
...
<details>
  <summary>Details</summary>
Motivation: 随着多模态模型的广泛应用，隐私泄露风险日益突出。现有研究主要关注单模态系统的隐私攻击，而多模态模型的隐私脆弱性尚未充分探索。

Method: 引入系统性的神经科学启发的拓扑正则化框架，在BLIP、PaliGemma 2和ViT-GPT2三个视觉语言模型上，使用COCO、CC3M和NoCaps数据集进行实验，比较基线模型和神经变体模型的隐私抵御能力。

Result: 在BLIP模型上，神经变体模型的成员推理攻击成功率平均ROC-AUC下降了24%，同时保持了相似的模型效用（MPNet和ROUGE-2指标）。PaliGemma 2和ViT-GPT2模型在其他数据集上的实验进一步验证了结果的稳定性。

Conclusion: 神经启发的多模态视觉语言模型对隐私攻击具有更强的抵御能力，且不会显著影响模型效用，这为理解多模态模型的隐私风险提供了新视角。
...
TL;DR: 提出Foundation Model Distillation (FMD)新范式，通过训练学生模型学习压缩的SuperTokens来重建教师模型的token级表示，在保持通用表征能力的同时显著减少计算开销。


<details>
  <summary>Details</summary>
Motivation: 基础模型虽然功能强大，但计算成本高、模型庞大，难以在边缘设备上部署。现有压缩技术会牺牲模型的通用性，需要一种既能压缩模型又能保持其通用表征能力的方法。

Method: 提出FMD框架，训练学生模型学习压缩的SuperTokens来重建教师模型的token级表示，捕获其潜在空间的紧凑基础。Foundry是该框架在3D点云上的首个实现。

Result: 单个蒸馏模型在分类、部分分割和少样本场景等多种下游任务中保持强迁移能力，接近完整基础模型的性能，同时使用显著更少的token和FLOPs。

Conclusion: FMD成功压缩了大型自监督学习模型，创建了紧凑、高效且忠实的代理模型，保留了通用表征能力，使其更适合在资源受限的硬件上部署。
...
TL;DR: Inferix是一个专门为世界模型设计的下一代推理引擎，通过优化的半自回归解码过程实现沉浸式世界合成，支持交互式视频流和性能分析，并与LV-Bench基准测试无缝集成。


<details>
  <summary>Details</summary>
Motivation: 世界模型是智能体AI、具身AI和游戏等领域的核心模拟器，能够生成长时、物理真实且交互式的高质量视频。扩展这些模型可以解锁视觉感知、理解和推理的新兴能力，超越当前以LLM为中心的视觉基础模型。

Method: 采用半自回归（块扩散）解码范式，结合扩散和自回归方法的优势，在块内应用扩散生成视频标记，同时以前面块为条件，实现更连贯稳定的视频序列。通过重新引入LLM风格的KV缓存管理，克服标准视频扩散的限制。

Result: Inferix专门针对世界模拟设计，区别于高并发系统（如vLLM或SGLang）和经典视频扩散模型（如xDiTs），支持高效、可变长度和高质量的视频生成。

Conclusion: Inferix作为专门的世界模型推理引擎，通过优化的半自回归解码和交互功能，为世界模型探索提供了新的可能性，希望社区共同推进其发展。
...
TL;DR: 提出LTED-Ada算法，在移动边缘计算环境中自适应选择本地跟踪或边缘检测，通过深度强化学习优化视频对象识别的帧率、准确率和延迟要求。


<details>
  <summary>Details</summary>
Motivation: 资源受限设备（如交通摄像头）的视频对象识别面临挑战，混合方法（本地跟踪+边缘检测）需要智能决策何时执行哪种操作。

Method: 基于深度强化学习的LTED-Ada算法，在单设备场景自适应选择策略；多设备场景通过联邦学习实现协作策略训练。

Result: 在Raspberry Pi 4B设备和PC边缘服务器上的硬件在环实验验证了LTED-Ada的优越性能。

Conclusion: LTED-Ada能有效平衡视频对象识别的准确率、延迟和帧率要求，在移动边缘计算环境中表现优异。
...
TL;DR: DeeAD是一个无需训练的动作引导早期退出框架，通过评估中间轨迹的物理可行性来加速VLA规划模型推理，在保持规划质量和安全性的同时减少28%的Transformer层使用和29%的延迟。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言动作(VLA)模型在自动驾驶中统一了感知、推理和轨迹生成，但由于深度Transformer堆栈导致推理延迟显著。

Method: 提出基于物理可行性的早期退出机制，当预测轨迹与轻量级规划先验(如导航或低精度规划)的偏差小于2米时终止推理；引入多跳控制器自适应跳过冗余层。
...
TL;DR: DinoLizer是基于DINOv2的生成式修复图像篡改区域定位模型，通过线性分类头和滑动窗口策略在14×14补丁分辨率下检测语义篡改区域，在多个数据集上超越现有最优方法。


<details>
  <summary>Details</summary>
Motivation: 解决生成式修复图像中语义篡改区域的精准定位问题，利用DINOv2强大的表示能力来检测生成内容中的篡改痕迹。

Method: 在DINOv2预训练模型基础上添加线性分类头，使用滑动窗口策略处理大尺寸图像，通过后处理优化二值篡改掩码预测。

Result: 在多个修复数据集上平均IoU比次优模型高出12%，对尺寸调整、噪声添加和JPEG压缩等后处理操作保持鲁棒性。

Conclusion: DINOv2在深度伪造定位任务中表现出强大的表示能力，DinoLizer在生成式修复篡改检测方面具有显著优势。
...
TL;DR: CANVAS是一个评估视觉语言模型在工具调用界面设计任务中表现的新基准，包含598个基于工具的设计任务，涵盖设计复制和设计修改两种任务类型。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏评估视觉语言模型在工具调用界面设计方面能力的基准，而理解这种能力对于模型与设计师在常规软件中的协作至关重要。

Method: 创建CANVAS基准，包含598个基于工具的设计任务，从3.3K个移动UI设计中采样，涵盖30个功能类别。任务类型包括设计复制（重现整个UI屏幕）和设计修改（修改现有屏幕的特定部分）。
...
Method: 开发文本引导语义图像编码器(TIE)，在编码过程中引入文本查询作为条件，生成与查询相关的图像表示，并采用文本条件化训练优化编码器。

Result: 在1B和3B规模的9个图像到文本基准测试中，TIE分别平均提升1.5和1.3个点，DocVQA和InfoVQA任务上最高提升6个点；仅使用一半图像tokens即可达到更好性能，显著提升推理效率。

Conclusion: TIE通过文本条件化训练有效优化编码器捕捉关键视觉特征，增强查询相关区域的注意力，提高可解释性和查询特定定位能力。
...
TL;DR: LongVT是一个端到端的智能体框架，通过多模态工具链思维实现长视频推理，利用LMMs的时序定位能力作为视频裁剪工具，采用全局到局部的推理循环来减少幻觉问题。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态模型在处理长视频时容易产生幻觉，因为证据稀疏且时间分散。受人类理解长视频的方式启发（先全局浏览再检查相关片段），需要开发能够进行细粒度视频推理的框架。
...
Conclusion: LongVT框架通过多模态工具链思维有效解决了长视频推理中的幻觉问题，展示了在长视频理解和推理任务上的优越性能，为未来长视频推理研究提供了有价值的资源和基准。
...
TL;DR: 对KRISP模型进行轻量级复现，参数大幅减少但性能达到原模型的75%，揭示了原设计中的缺陷和实际问题，并在资源受限条件下评估知识增强VQA架构的可扩展性和有效性。


<details>
  <summary>Details</summary>
Motivation: 原KRISP模型虽然有效但计算需求大、与大型骨干网络紧密耦合，不适合工业规模训练。本研究从不同角度重新审视KRISP，旨在开发轻量级版本。

Method: 通过系统消融研究进行轻量级复现，包括在合成VQA数据上的概念验证和在DAQUAR数据集上的评估，模型采用低参数设置并受外部知识图谱领域约束。

Result: 复现模型性能达到原模型的75%，揭示了原设计中未充分覆盖的设计缺陷、实际陷阱和隐性问题，同时能防止AI幻觉并仅在特定领域内生成输出。

Conclusion: 轻量级模型参数少，可在智能手机和AR-VR等边缘设备上运行，进一步改善了离线视觉推理能力，展示了在资源受限条件下知识增强VQA架构的可行性。
...
TL;DR: 本文提出了一个名为"warping"的统一算子来概括各种动态采样方法，通过理论分析和实证研究揭示了动态采样网络训练中的前向-反向不对称性，并提供了确保训练稳定性的条件。


<details>
  <summary>Details</summary>
Motivation: 深度学习架构中的动态采样机制在计算机视觉模型中显示出实用性，但这些结构的理论分析尚未统一。本文旨在通过开发一个统一的分析框架来连接各种动态采样方法。

Method: 提出了一个名为"warping"的通用算子，该算子可以重构现有架构（如可变形卷积、主动卷积单元和空间变换网络），并对该算子进行统计分析，建模输入为IID变量和齐次随机场。

Result: 发现了模型训练中前向传播和反向传播之间的独特不对称性，证明这些机制代表了与传统平移不变卷积算子完全不同的正交算子类别，并找到了确保动态采样网络稳定训练的条件。

Conclusion: 通过理论分析和实证研究，建立了动态采样方法的统一分析框架，揭示了其与传统卷积算子的本质差异，并提供了训练稳定性的指导原则。
...
TL;DR: Δ-NeRF是一种模块化残差框架，用于神经辐射场(NeRF)的增量优化，解决了现有方法需要完整重新训练的问题，特别适用于卫星地形分析等数据连续到达的场景。


<details>
  <summary>Details</summary>
Motivation: 现有NeRF框架在引入新视图时需要完整重新训练，限制了在数据连续到达场景中的应用，特别是在卫星地形分析中，区域会随时间重复观测。增量优化方法存在灾难性遗忘问题。

Method: 提出Δ-NeRF框架，包括：(1)残差控制器向冻结的基础NeRF注入逐层修正；(2)不确定性感知门控机制自适应结合基础和优化预测；(3)视图选择策略减少47%训练数据；(4)知识蒸馏将增强模型压缩至原大小20%。

Result: 在卫星图像实验中，Δ-NeRF性能与联合训练相当，训练时间减少30-42%，PSNR比简单微调提升43.5%，在某些指标上甚至超过联合训练。

Conclusion: Δ-NeRF为NeRF的增量优化提供了有效解决方案，在保持性能的同时显著减少训练时间和数据需求，特别适用于连续数据场景。
...
TL;DR: Split-then-Merge (StM) 是一个新颖的生成视频合成框架，通过将未标注视频分割为前景和背景层并进行自组合学习，解决视频生成中的数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 解决生成视频合成中的控制增强问题和数据稀缺问题，传统方法依赖标注数据集或手工规则，而StM利用未标注视频学习动态主体与多样化场景的交互。

Method: 将未标注视频分割为动态前景和背景层，然后进行自组合学习；采用变换感知训练流程，包括多层融合和增强实现affordance感知合成，以及保持前景保真度的身份保持损失。

Result: 实验表明StM在定量基准测试和人类/VLLM定性评估中均优于最先进方法。

Conclusion: StM框架通过创新的分割-合并方法有效学习复杂组合动态，实现了更真实的视频生成，为解决视频生成中的数据稀缺和控制问题提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉推理评估环境往往缺乏精确的可验证性，难以进行大规模数据构建和可靠评估。Sphinx旨在填补这一空白，通过程序化生成具有可验证解法的视觉推理任务。

Method: 使用程序化方法生成包含多种视觉元素（图案、图块、图表、图标、几何原语）的谜题，每个谜题都配有可验证的真实解。涵盖25种任务类型，包括对称检测、几何变换、空间推理、图表解释和序列预测。

Result: 评估显示，即使是当前最先进的GPT-5模型在Sphinx基准上仅达到51.1%的准确率，远低于人类表现。使用可验证奖励的强化学习（RLVR）显著提高了模型在这些任务上的准确率，并在外部视觉推理基准上获得了提升。

Conclusion: Sphinx为视觉推理提供了精确的评估环境，揭示了当前LVLMs在核心认知任务上的局限性。RLVR方法展示了在提升多模态推理能力方面的潜力。
...
TL;DR: 本文提出使用基于优化的视觉反转(OVI)替代传统扩散先验网络，这是一种无需训练和数据的替代方案，通过优化潜在视觉表示来最大化与文本提示嵌入的余弦相似度，并提出了两种正则化约束来提升图像质量。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在文本到图像生成中依赖计算昂贵的扩散先验网络，这些先验需要大量训练数据。本文挑战了训练先验的必要性，寻求更高效的替代方案。

Method: 使用基于优化的视觉反转(OVI)方法，从随机伪标记初始化潜在视觉表示，通过迭代优化最大化与文本嵌入的余弦相似度，并引入马氏距离和最近邻损失两种约束来正则化优化过程。

Result: 在Kandinsky 2.2上的实验表明，OVI可以替代传统先验。分析揭示了当前评估基准的缺陷，约束OVI方法在视觉保真度上优于基线，最近邻方法表现最佳，达到与最先进数据高效先验相当或更高的定量分数。

Conclusion: OVI作为传统先验的可行替代方案值得进一步研究，同时指出了当前评估基准需要改进以更好地反映感知质量。
...
<details>
  <summary>Details</summary>
Motivation: 在临床应用中，准确检测血管树中心线并保持高召回率至关重要，因为遗漏小分支可能导致致命错误。现有方法在保持树状拓扑和召回率方面存在挑战。

Method: 采用Producer-Refiner架构，基于Transformer解码器。Producer提出初始汇合轨迹，Refiner循环精炼这些轨迹形成最终中心线图。使用汇合轨迹表示确保有效树状拓扑，并引入高效的非极大值抑制算法合并重复分支。

Result: 在多个公共中心线数据集上，RefTr实现了优于先前SOTA的召回率和可比精度，同时提供更快的推理速度和显著减少的参数数量（解码器参数减少2.4倍）。
...
<details>
  <summary>Details</summary>
Motivation: 解决深度估计在真实光学条件下的可靠性挑战，弥补缺乏大规模、高保真实立体DSLR数据集的研究限制，缩小合成训练数据与真实相机光学之间的真实感差距。

Method: 使用两个相同的相机组件，在9个不同场景复杂度、光照和背景的场景中，以10个焦距（28-70mm）和5个光圈（f/2.8-f/22）拍摄图像，涵盖50种光学配置，每个场景2000张图像。

Result: 创建了首个高分辨率立体DSLR数据集，支持几何和光学效应的受控分析，包括单目和立体深度估计、浅景深渲染、去模糊、3D场景重建和新视角合成。
...
TL;DR: 提出了首个大规模无监督视觉记忆性数据集，包含82,000个视频和描述性回忆数据，用于建模视觉记忆信号，在回忆生成和舌尖现象检索任务上表现优异。
...
Result: 在数据集上微调的大型视觉语言模型在生成视觉内容开放式记忆描述方面优于GPT-4o等最先进模型，并创建了首个能够执行多模态舌尖现象检索的模型。
...
TL;DR: 提出一种从立体雾天图像序列中同时估计所有雾模型参数的方法，通过解决新型优化问题避免误差传播，并能处理全局不均匀的真实雾况。


<details>
  <summary>Details</summary>
Motivation: 现有方法顺序估计雾参数容易导致误差传播，且假设雾全局均匀无法处理真实世界中不均匀的雾况，需要更鲁棒的参数估计算法。

Method: 提出同时估计所有雾参数的优化算法，假设雾仅局部均匀，可作为现有视觉SLAM或里程计系统的附加模块使用。创建了包含3.4万帧真实雾天道路场景的SDIRF数据集。

Result: 在合成雾数据和真实SDIRF序列上的实验表明，该方法在参数估计精度和真实雾适应能力方面均优于现有方法。

Conclusion: 该方法能有效估计雾参数并适应真实不均匀雾况，为雾天视觉感知研究提供了重要工具和数据集。
...
TL;DR: V^2-SAM是一个统一的跨视图对象对应框架，通过两个互补的提示生成器将SAM2从单视图分割适配到跨视图对应任务，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 跨视图对象对应任务（如ego-exo对象对应）由于视角和外观的剧烈变化而面临重大挑战，现有分割模型如SAM2难以直接应用。

Method: 提出V^2-SAM框架，包含：1）基于DINOv3特征的跨视图锚点提示生成器（V^2-Anchor），建立几何感知对应关系；2）跨视图视觉提示生成器（V^2-Visual），通过新颖的视觉提示匹配器增强外观引导线索；3）多专家设计和后验循环一致性选择器（PCCS）自适应选择最可靠的专家。

Result: 在Ego-Exo4D（ego-exo对象对应）、DAVIS-2017（视频对象跟踪）和HANDAL-X（机器人就绪的跨视图对应）上实现了新的最先进性能。

Conclusion: V^2-SAM通过互补的提示生成器和自适应选择机制，有效解决了跨视图对象对应问题，并在多个基准测试中表现出色。
...
TL;DR: 提出了Null-Text Test-Time Alignment (Null-TTA)方法，通过优化分类器自由引导中的无条件嵌入来实现扩散模型的测试时对齐，避免奖励黑客问题并保持跨奖励泛化能力。
...
Method: 通过优化分类器自由引导中的无条件嵌入而非操作潜在变量或噪声变量，在语义一致的流形上进行对齐。
...
TL;DR: 提出了GaINeR框架，将可训练的高斯分布与神经隐式表示结合，实现具有几何感知能力的连续图像表示，支持局部编辑和物理感知操作。


<details>
  <summary>Details</summary>
Motivation: 传统隐式神经表示缺乏显式几何结构，局部编辑能力有限，难以与物理模拟集成，限制了在动态和交互场景中的应用。

Method: 结合可训练高斯分布和神经网络：对于给定图像坐标，检索K个最近高斯分布，聚合距离加权的嵌入向量，通过神经网络预测RGB值。

Result: 实现了连续图像表示、可解释的几何结构和灵活的局部编辑能力，为物理感知和交互式图像操作提供了基础。

Conclusion: GaINeR框架成功解决了传统INR的局限性，通过几何感知设计扩展了隐式神经表示在动态和交互设置中的应用潜力。
...
TL;DR: 使用深度学习模型来降低小脑桥脑角MRI对比剂剂量，通过训练DL模型从低剂量模拟图像中恢复标准剂量图像质量，在10%-30%标准剂量下仍能保持良好的图像质量和诊断性能。
...
Method: 多中心回顾性研究，使用前庭神经鞘瘤患者的T1和T1ce MRI数据模拟低剂量T1ce，训练深度学习模型从低剂量图像恢复标准剂量图像质量。

Result: 在10%输入剂量下，DL恢复的T1ce显著改善了图像质量指标（SSIM从0.639提升到0.993，PSNR从21.6dB提升到41.4dB）和分割性能（Dice从0.673提升到0.734）。10%和30%剂量下都能获得优秀图像质量。

Conclusion: 深度学习模型能够显著改善小脑桥脑角低剂量MRI的图像质量，使得在仅使用10%-30%标准剂量的情况下仍能进行病变检测和诊断表征。
...
<details>
  <summary>Details</summary>
Motivation: 轻量级视频识别模型在处理复杂时序动态时表现不佳，需要一种能有效利用视频自然时序连贯性的方法。

Method: 使用高斯随机游走建模连续帧中间层嵌入的变化，惩罚表征的突变，促进低加速度的平滑解决方案。

Result: 在Kinetics-600上提升准确率3.8%-6.4%，MoViNets模型家族在各自FLOP约束下将SOTA提升3.8%-6.1%，MobileNetV3和MoViNets-Stream在相似内存占用下提升4.9%-6.4%。

Conclusion: 平滑正则化技术能有效提升轻量级视频识别模型的性能，通过强制时序平滑性更好地捕捉视频的动态特性。
...
<details>
  <summary>Details</summary>
Motivation: 传统组合解释方法依赖人工标注数据集，限制了其适用范围和概念灵活性。本文旨在开发一个能够探测神经元对任意概念和数据集响应的框架。
...
Conclusion: 该框架通过开放词汇语义分割实现了对神经元激活的灵活解释，突破了传统方法的领域限制，为理解深度神经网络中神经元如何编码信息提供了更通用的工具。
...
TL;DR: 提出了UruDendro4数据集，包含102个火炬松横截面图像样本，每个都手动标注了年轮。该数据集支持基于横截面图像的年轮体积建模，并提供了自动年轮检测的性能基准。


<details>
  <summary>Details</summary>
Motivation: 解决木材横截面数据稀缺问题，传统手动测量年轮生长耗时且不精确，需要更准确、自动化的年轮划分方法。

Method: 开发UruDendro4数据集，包含多高度样本支持体积建模；使用DeepCS-TRD等最先进方法进行自动年轮检测性能基准测试；进行消融实验验证参数配置。

Result: DeepCS-TRD方法表现最佳，平均精度0.838，平均召回率0.782，适应兰德误差0.084；包含该数据集的训练提高了模型在年轮检测任务中的泛化能力。

Conclusion: UruDendro4数据集填补了木材横截面数据的空白，支持年轮体积建模，为自动年轮检测提供了可靠的基准，有助于改进林业管理实践。
...
<details>
  <summary>Details</summary>
Motivation: 乳腺超声报告生成面临配对图像-报告数据集缺乏和大语言模型幻觉风险的限制

Method: 使用多任务Swin编码器学习描述符感知的视觉表示，通过双级目标对齐视觉和文本标记，结合标记级交叉熵和余弦相似度对齐损失

Result: 在两个公共乳腺超声数据集上，BUSTR在标准自然语言生成指标和临床效能指标上均有持续改进，特别是对BI-RADS类别和病理学等关键目标
...
<details>
  <summary>Details</summary>
Motivation: 传统图像合成研究专注于视觉真实性和语义合理性，但在实际编辑场景中，用户更倾向于创造艺术性、趣味性或社交参与性的内容，而非保持真实感

Method: 提出StickerNet两阶段框架：首先确定合成类型，然后预测透明度、遮罩、位置和缩放等放置参数。基于180万真实用户编辑行为构建数据集

Result: 用户研究和定量评估显示StickerNet优于常见基线方法，并接近人类放置行为，证明了从真实世界编辑模式学习的有效性

Conclusion: 这项工作引入了视觉理解的新方向，强调表达性和用户意图而非真实性，为创意图像编辑提供了新思路
...
TL;DR: TrafficLens是一个针对多摄像头交通路口的优化算法，通过利用摄像头重叠覆盖区域和智能跳过冗余VLM调用的方式，将视频转文本时间减少4倍，同时保持信息准确性。


<details>
  <summary>Details</summary>
Motivation: 多摄像头交通视频分析面临海量数据处理挑战，传统方法需要将视频数据通过VLM转换为文本再使用LLM分析，这个过程耗时且延迟了交通洞察和事故调查的及时性。
...
TL;DR: 提出结合Vision Transformers和同态加密的隐私保护联邦学习框架，用于安全的跨机构病理学分类，显著减少通信开销并防止模型反演攻击。


<details>
  <summary>Details</summary>
Motivation: 医疗机构的协作机器学习面临隐私法规限制，传统联邦学习的梯度仍易受重建攻击泄露敏感医疗信息。

Method: 使用ViT的CLS令牌作为768维特征表示，采用CKKS同态加密进行安全聚合，在服务器端进行加密推理。

Result: CLS令牌加密比梯度加密减少30倍通信量，防止模型反演攻击，在加密域达到90.02%分类准确率。
...
TL;DR: 提出了一种基于双整流流的免反演风格迁移框架，通过并行预测内容和风格轨迹，使用动态中点插值融合，实现高效且无视觉失真的风格迁移。
...
Result: 实验表明该方法在多种风格和内容上具有良好的泛化能力，实现了高效且高质量的风格迁移。

Conclusion: 该免反演框架为风格迁移提供了有效且高效的解决方案，避免了传统方法中的视觉失真问题。
...
TL;DR: 提出了一种无需测试时参考图像的伪装目标检测框架，通过类别原型记忆和查询条件混合机制来合成参考向量
...
Method: 在训练期间将参考图像提炼到类别原型记忆中，在推理时通过查询条件混合原型来合成参考向量；使用EMA更新的类别原型和双向注意力对齐模块来弥合表示差距
...
TL;DR: 提出WavePCNet网络，通过物理驱动的波前传播补偿来增强对遮挡物体的感知，在低信噪比条件下显著提升定位和分割精度与鲁棒性


<details>
  <summary>Details</summary>
Motivation: 现有基于实值建模或局部卷积的方法无法准确捕捉相干光传播的物理特性，在低信噪比条件下容易收敛到非物理解，影响观测的稳定性和可靠性

Method: WavePCNet集成Tri-Phase波前复传播重投影(TriWCP)来约束相干传播行为，结合动量记忆机制抑制扰动积累，并引入高频跨层补偿增强构建频率选择性通路

Result: 在四个物理采集数据集上的实验表明，WavePCNet在准确性和鲁棒性方面持续优于最先进方法

Conclusion: WavePCNet通过物理驱动的波前传播模拟和补偿机制，有效解决了遮挡物体定位分割中的多重散射和介质扰动问题
...
<details>
  <summary>Details</summary>
Motivation: 现有的多模态安全防护主要只评估输入问题和最终答案，忽略了中间推理过程，导致偏见推断或违反政策的视觉上下文使用等危害未被检测。

Method: 构建GuardTrace数据集，采用多样化提示策略生成，并通过MLRM和人工投票验证流程进行精炼。提出三阶段渐进训练方案结合数据精炼过程，使模型能够根据不同风险级别学习细微和上下文相关的安全偏好。

Result: 在涵盖域内和域外场景的测试集上，GuardTrace-VL模型在不安全推理检测任务中达到93.1%的F1分数，相比之前最强的多模态安全防御方法提升了13.5%。
...
<details>
  <summary>Details</summary>
Motivation: 图像可以看作前景对象在背景上的分层组合，这种分层表示能实现元素的独立编辑，为内容创作提供更大灵活性。但由于方法和数据有限，单图像分层分解仍然具有挑战性。

Method: 观察到分层分解与修复任务之间的强关联，通过轻量级微调将基于扩散的修复模型适配到分层分解。引入具有线性注意力复杂度的多模态上下文融合模块来保留潜在空间细节。
...
<details>
  <summary>Details</summary>
Motivation: 现有新闻图像字幕方法存在三个关键问题：信息覆盖不完整、跨模态对齐弱、视觉实体定位不优。需要开发能够整合文本、视觉和结构化知识的新方法。

Method: MERGE构建实体中心的多模态知识库(EMKB)，采用多阶段假设-字幕策略改善跨模态对齐，通过图像内容引导的动态检索增强视觉实体匹配。

Result: 在GoodNews和NYTimes800k数据集上，MERGE显著优于现有最佳方法，CIDEr分别提升+6.84和+1.16，命名实体识别F1分数分别提升+4.14和+2.64。在未见过的Visual News数据集上表现优异，CIDEr提升+20.17，F1分数提升+6.22。
...
<details>
  <summary>Details</summary>
Motivation: 现有的模型可迁移性评估指标选择方法通常是临时性的或仅基于指标的平均历史表现，但实际中不同指标在不同任务上的效果差异很大，没有一个指标在所有数据集上都最优。

Method: 将指标选择建模为学习排序问题，使用预训练语言模型编码数据集和指标的文本描述，在共享语义空间中嵌入，通过元预测器学习数据集特征与指标机制之间的复杂关系，采用列表优化目标优先正确排序表现最佳的指标。
...
<details>
  <summary>Details</summary>
Motivation: 现有可信多视图分类方法依赖全局密集邻居关系建模视图内依赖，导致计算成本高且无法直接保证视图间关系一致性，同时通过手动分配权重聚合证据，缺乏对学习到的多视图邻居结构在类空间一致性的保证。
...
Conclusion: 所提出的基于原型的可信多视图分类框架能够有效解决现有方法的局限性，实现更高效和可靠的多视图分类决策。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本引导扩散模型在图像编辑中难以实现物理一致的图像修饰和精确参数控制，要么依赖模糊的文本提示，要么需要为每个参数训练单独的头部/权重，限制了可扩展性和多参数组合能力。

Method: 提出CameraMaster框架，显式解耦相机指令，整合指令表示和参数嵌入。使用相机参数嵌入调制相机指令和内容语义，通过交叉注意力将调制后的指令注入内容特征，并将指令和相机嵌入作为条件信号注入时间嵌入，实现统一的层间调制。

Result: 在78K图像-提示对数据集上的实验表明，CameraMaster对参数变化产生单调且接近线性的响应，支持无缝多参数组合，显著优于现有方法。

Conclusion: CameraMaster通过统一的相机感知框架实现了精确的图像修饰控制，解决了现有方法在参数控制精度和可扩展性方面的挑战。
...
TL;DR: 提出了CaptionQA基准，通过下游任务效用评估图像描述质量，涵盖4个领域，包含33,027个密集标注的多选题，揭示当前MLLM生成的描述与图像实际效用之间存在显著差距。


<details>
  <summary>Details</summary>
Motivation: 当前评估方法未能回答一个基本问题：描述能否在实际下游任务中替代图像？需要基于效用的基准来衡量描述质量。

Method: 构建可扩展的领域相关基准，覆盖自然、文档、电商和具身AI4个领域，通过25个顶级类别和69个子类别的细粒度分类体系识别领域特定任务的有用信息。使用LLM仅基于描述回答问题来直接测量描述效用。

Result: 评估显示图像与其描述效用之间存在显著差距，在传统图像QA基准上表现相近的模型，在描述效用上下降高达32%。

Conclusion: CaptionQA基准揭示了当前图像描述生成模型的局限性，为评估和改进描述质量提供了新视角，强调了下游任务效用作为描述质量衡量标准的重要性。
...
TL;DR: FlowerDance是一个高效的从音乐生成舞蹈动作的系统，通过结合MeanFlow和物理一致性约束，在少量采样步骤下生成高质量动作，并使用BiMamba架构实现高效的非自回归生成。
...
Method: 结合MeanFlow与物理一致性约束实现高质量动作生成；使用基于BiMamba的高效模型架构和通道级跨模态融合；支持非自回归生成方式；提供动作编辑功能。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于多模态大语言模型的肺部CT分析方法在准确描述结节形态和融入医学专业知识方面存在局限，影响临床可靠性。协作多智能体系统在医学应用中平衡通用性和精度的潜力尚未充分探索。

Method: 设计三模块系统：Nodule Spotter协调检测模型识别结节；Radiologist整合局部图像描述技术生成CT报告；Doctor Agent System使用图像和CT报告进行恶性推理，基于病理知识库和多智能体框架。
...
TL;DR: 提出了一种新颖的图像去模糊框架，通过将强大的生成先验与显式物理约束相结合，解决了空间变化图像去模糊问题。该方法在严重模糊场景下超越了现有最先进的模型方法和生成基线。


<details>
  <summary>Details</summary>
Motivation: 空间变化图像去模糊仍然是一个病态问题，现有方法存在局限性：基于模型的深度展开方法会产生过度平滑、伪影严重的纹理；生成模型虽然感知质量更好，但由于物理约束较弱会产生幻觉细节。需要一种能够兼顾物理准确性和感知真实性的方法。

Method: 提出了一种新颖框架，通过将强大的生成先验与显式密集物理约束相结合。将退化场建模为高维压缩核的密集连续体，捕捉运动和退化模式的微小变化。利用这个丰富的描述符场来调节ControlNet架构，强烈引导扩散采样过程。

Result: 广泛的实验表明，该方法有效弥合了物理准确性和感知真实性之间的差距，在具有挑战性的严重模糊场景下，超越了最先进的基于模型方法和生成基线。

Conclusion: 该方法通过将生成先验与物理约束相结合，成功解决了空间变化图像去模糊的核心挑战，在保持物理准确性的同时实现了优越的感知质量。
...
<details>
  <summary>Details</summary>
Motivation: 当前图像情感合成方法人为分离生成和编辑任务，导致效率低下且限制了在治疗干预、故事讲述等自然交织任务中的应用。

Method: 采用测试时缩放策略，利用现成情感分类器进行梯度优化，通过语义相似度确定最佳引导时机，使用多情感损失减少固有和相似情感的干扰。

Result: 实验结果显示MUSE在生成和编辑任务中均优于所有方法，提高了情感准确性和语义多样性，同时在内容保持、文本提示遵循和情感表达之间保持最佳平衡。
...
TL;DR: 提出T-NIG模型，通过将时间参数融入正态逆伽马分布来处理阿尔茨海默病预测中不规则时间间隔的脑图像序列，生成长期预测图像并保持疾病相关特征。


<details>
  <summary>Details</summary>
Motivation: 解决长期AD预测中，面对不规则时间间隔的序列数据时难以维持疾病相关特征的问题，利用时间相关分布反映疾病特征变化。

Method: 使用两个不同时间点的脑图像，通过坐标邻域识别特征，在正态逆伽马分布中融入时间参数来理解特征变化，并利用不确定性估计减少认知和随机不确定性。

Result: T-NIG模型在数据集中的短期和长期预测任务上均表现出最先进的性能，能够准确预测疾病进展并保持疾病相关特征。

Conclusion: T-NIG模型能够有效处理不规则时间分布的脑图像数据，在阿尔茨海默病预测中实现准确的长期图像生成和疾病进展预测。
...
### [53] [CLRecogEye : Curriculum Learning towards exploiting convolution features for Dynamic Iris Recognition](https://arxiv.org/abs/2511.21097)
...
Conclusion: 提出的框架通过学习时空表示和课程学习训练，显著提升了虹膜认证系统在复杂条件下的鲁棒性和泛化能力。
...
### [54] [Pygmalion Effect in Vision: Image-to-Clay Translation for Reflective Geometry Reconstruction](https://arxiv.org/abs/2511.21098)
*Gayoung Lee,Junho Kim,Jin-Hwa Kim,Junmo Kim*

Main category: cs.CV

TL;DR: 提出Pygmalion Effect in Vision框架，通过图像到黏土转换来抑制镜面反射线索，同时保持几何一致性，从而改进含复杂反射的多视角图像3D重建。


<details>
  <summary>Details</summary>
Motivation: 解决3D重建中长期存在的反射问题，因为视角相关反射会导致外观和几何信息纠缠在一起，难以准确重建。

Method: 采用双分支网络架构：基于BRDF的反射分支和黏土引导分支，通过合成黏土样图像提供无反射监督信号，联合训练以稳定几何和细化表面法线。
...
TL;DR: 提出了一种名为DATGN的变形感知时序生成网络，用于从脑部MRI图像中自动学习阿尔茨海默病的形态变化，实现早期预测。该方法能处理时序数据缺失问题，生成符合疾病进展的未来MRI图像，显著提升分类准确率。


<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病早期预测对延缓疾病进展至关重要。当前方法主要依赖手动特征提取分析脑部形态变化，存在自动化程度低的问题。时序MRI数据中常见数据缺失，需要有效处理。

Method: DATGN首先对不完整的时序MRI序列进行插值，然后通过双向时序变形感知模块指导网络生成符合疾病进展的未来MRI图像。该方法能自动学习脑部形态变化模式。
...
Conclusion: DATGN能够生成与阿尔茨海默病脑萎缩趋势一致的MRI图像，有效支持疾病早期预测。该方法在图像质量和分类性能方面均表现出色，为阿尔茨海默病的自动化早期诊断提供了有力工具。
...
### [56] [Scaling Foundation Models for Radar Scene Understanding](https://arxiv.org/abs/2511.21105)
...
<details>
  <summary>Details</summary>
Motivation: 雷达传感器在恶劣天气、光照和远距离条件下提供可靠感知，但现有雷达方法碎片化且任务特定，缺乏跨任务迁移能力。基础模型在视觉和语言理解方面取得进展，但与雷达感知的整合仍未被充分探索。
...
Conclusion: RadarFM通过结构化空间语言监督成功构建了雷达基础模型，解决了现有雷达方法的碎片化问题，实现了跨任务的统一表示学习。
...
Method: 提出熵引导剪枝策略，使用条件熵偏差评估模块重要性；采用零样本自适应剪枝框架，动态决定剪枝时机和程度。

Result: 在DiT和SiT模型上实验，实现了最高2.22倍的推理加速，同时在ImageNet和三个下游数据集上保持竞争力的生成质量。

Conclusion: EntPruner有效解决了生成模型在下游任务中的参数冗余问题，在保持性能的同时显著提升推理效率。
...
### [58] [EM-KD: Distilling Efficient Multimodal Large Language Model with Unbalanced Vision Tokens](https://arxiv.org/abs/2511.21106)
*Ze Feng,Sen Yang,Boqiang Duan,Wankou Yang,Jingdong Wang*

Main category: cs.CV

TL;DR: EM-KD是一种新颖的知识蒸馏范式，通过解决高效MLLMs中视觉令牌不平衡的问题，提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有高效MLLMs压缩视觉令牌以减少资源消耗，但视觉信息丢失会降低理解能力。先前方法虽然引入知识蒸馏，但忽略了高效学生模型与原始教师模型之间视觉令牌不平衡导致的细粒度视觉理解差异。

Method: 首先计算教师和学生视觉logits的曼哈顿距离，使用匈牙利匹配算法在空间维度对齐。然后引入两种蒸馏策略：1）视觉-语言亲和力蒸馏（VLAD），计算文本令牌与对齐视觉令牌的亲和矩阵，最小化学生和教师亲和矩阵的平滑L1距离；2）视觉语义蒸馏（VSD），使用反向KL散度度量对齐视觉logits在词汇空间中的离散概率分布。

Result: 在多样化基准测试上的综合评估表明，EM-KD训练模型在准确性和效率上都大幅优于先前的高效MLLMs。与先前蒸馏方法相比，EM-KD也实现了更好的性能。

Conclusion: EM-KD通过解决视觉令牌不平衡问题，有效提升了高效MLLMs的性能，验证了其有效性。
...
TL;DR: 提出Ent-Prog框架，通过条件熵膨胀评估模型组件重要性，结合自适应渐进式训练计划，在保持性能的同时显著降低训练时间和GPU内存消耗。
...
### [60] [FaithFusion: Harmonizing Reconstruction and Generation via Pixel-wise Information Gain](https://arxiv.org/abs/2511.21113)
*YuAn Wang,Xiaofan Li,Chi Huang,Wenhao Zhang,Hao Li,Bosheng Wang,Xun Sun,Jun Wang*

Main category: cs.CV

TL;DR: FaithFusion是一个基于像素级期望信息增益（EIG）的3DGS-扩散模型融合框架，用于可控驾驶场景重建和3D场景生成，在保持几何保真度的同时实现大视角偏移下的视觉合理性。


<details>
  <summary>Details</summary>
Motivation: 在可控驾驶场景重建和3D场景生成中，如何在保持几何保真度的同时，在大视角偏移下合成视觉上合理的场景外观是一个关键挑战。现有的几何驱动3DGS和外观驱动扩散模型融合方法面临像素级、3D一致性编辑标准缺失导致的过度修复和几何漂移问题。

Method: 提出FaithFusion框架，使用像素级期望信息增益（EIG）作为统一策略进行时空一致性合成：EIG作为空间先验指导扩散模型优化高不确定性区域，同时通过像素级权重将编辑结果蒸馏回3DGS。这是一个即插即用系统，无需额外先验条件和结构修改。

Result: 在Waymo数据集上的广泛实验表明，该方法在NTA-IoU、NTL-IoU和FID指标上达到最先进性能，即使在6米车道偏移下仍保持107.47的FID分数。

Conclusion: FaithFusion通过EIG驱动的3DGS-扩散模型融合，有效解决了可控驾驶场景重建中的几何保真度和视觉合理性平衡问题，实现了高质量的时空一致性合成。
...
### [61] [LLaVA-UHD v3: Progressive Visual Compression for Efficient Native-Resolution Encoding in MLLMs](https://arxiv.org/abs/2511.21150)
*Shichu Sun,Yichen Zhang,Haolin Song,Zonghao Guo,Chi Chen,Yidan Zhang,Yuan Yao,Zhiyuan Liu,Maosong Sun*

Main category: cs.CV

TL;DR: LLaVA-UHD v3提出渐进式视觉压缩方法，在保持性能的同时显著降低计算开销，TTFT减少1.9-2.4倍


<details>
  <summary>Details</summary>
Motivation: 解决全局视觉编码在MLLMs中计算开销过大的问题，平衡性能与效率

Method: 渐进式视觉压缩方法，包含精炼补丁嵌入和窗口化令牌压缩两个模块，可集成到标准ViT中

Result: ViT-UHD在保持竞争力的同时，TTFT减少2.4倍；LLaVA-UHD v3性能接近Qwen2-VL，TTFT进一步减少1.9倍

Conclusion: PVC方法能有效重构预训练ViT为高效架构，在保持通用性的同时显著提升效率
...
TL;DR: 提出了UPA-RFAS框架，通过鲁棒特征、注意力和语义学习通用对抗补丁，可在未知架构、微调变体和仿真到现实转移中攻击视觉-语言-动作模型。


<details>
  <summary>Details</summary>
Motivation: 视觉-语言-动作模型容易受到对抗攻击，但通用且可转移的攻击研究不足，现有补丁通常过拟合单一模型，在黑盒设置中失效。

Method: 结合特征空间目标、鲁棒增强的两阶段min-max优化、补丁注意力主导和补丁语义失配损失，在共享特征空间中学习单一物理补丁。

Result: 实验表明UPA-RFAS在不同VLA模型、操作套件和物理执行中一致转移，暴露了基于补丁的实际攻击面。

Conclusion: UPA-RFAS为VLA模型建立了强大的通用对抗补丁攻击基准，为未来防御提供了参考。
...
### [64] [CtrlVDiff: Controllable Video Generation via Unified Multimodal Video Diffusion](https://arxiv.org/abs/2511.21129)
*Dianbing Xi,Jiepeng Wang,Yuanzhi Liang,Xi Qiu,Jialun Liu,Hao Pan,Yuchi Huo,Rui Wang,Haibin Huang,Chi Zhang,Xuelong Li*

Main category: cs.CV

TL;DR: CtrlVDiff是一个统一的扩散框架，通过多模态控制策略融合深度、法线、分割、边缘和图形内在属性，实现视频理解和可控生成，解决了几何线索不足和异构模态融合的挑战。


<details>
  <summary>Details</summary>
Motivation: 传统几何线索（如深度、边缘）只能指定布局但无法约束外观、材质和光照，限制了物理意义的编辑（如重新照明、材质替换）并导致时间漂移。需要图形学基础的多模态来提供互补约束。

Method: 提出CtrlVDiff模型，采用混合模态控制策略（HMCS）路由和融合深度、法线、分割、边缘和图形内在属性（反照率、粗糙度、金属度）特征，从任意子集重新渲染视频。构建MMVideo混合数据集进行训练。

Result: 在理解和生成基准测试中，CtrlVDiff实现了卓越的可控性和保真度，支持分层编辑（重新照明、材质调整、对象插入），超越最先进基线，并在某些模态缺失时保持鲁棒性。

Conclusion: 通过融合图形学基础的多模态，CtrlVDiff统一解决了视频理解和可控生成的双重挑战，实现了精确、可预测的控制和强时间一致性。
...
TL;DR: BotaCLIP是一个轻量级多模态对比学习框架，通过将高分辨率航空影像与植物样方数据对齐，来适配预训练的地球观测基础模型，无需从头训练或高计算成本。


<details>
  <summary>Details</summary>
Motivation: 解决在预训练基础模型中注入领域特定知识而不需要从头训练或高计算成本的挑战，特别是在生物多样性建模等实际应用中。

Method: 引入BotaCLIP框架，通过对比学习将高分辨率航空影像与植物样方数据对齐，采用正则化策略缓解灾难性遗忘问题。
...
### [66] [DeepRFTv2: Kernel-level Learning for Image Deblurring](https://arxiv.org/abs/2511.21132)
*Xintian Mao,Haofei Song,Yin-Nian Liu,Qingli Li,Yan Wang*

Main category: cs.CV

TL;DR: 提出Fourier Kernel Estimator (FKE)，在傅里叶空间中学习模糊核，将空间域卷积问题转换为频域乘法问题，实现核级别的模糊过程学习，显著提升图像去模糊性能。


<details>
  <summary>Details</summary>
Motivation: 当前深度网络仍停留在像素级学习阶段，无法让去模糊模型理解模糊的本质过程。模糊是由清晰图像与模糊核卷积产生的，因此让网络在核级别学习模糊过程可以显著提升性能。

Method: 提出Fourier Kernel Estimator (FKE)，在傅里叶空间进行激活操作，将空间域卷积转换为频域乘法；将卷积对象从图像改为网络提取的特征；设计解耦多尺度架构，使用多个层次子Unet和可逆策略实现高效多尺度编码解码。

Result: 在运动去模糊任务上达到最先进的结果，并展示出处理其他核相关问题的潜力。分析表明核估计器能够学习具有物理意义的模糊核。

Conclusion: FKE方法通过核级别的模糊过程学习，实现了更有效的图像去模糊，且无需额外监督，计算复杂度低，具有广泛的适用性。
...
### [67] [The More, the Merrier: Contrastive Fusion for Higher-Order Multimodal Alignment](https://arxiv.org/abs/2511.21331)
*Stefanos Koutoupis,Michaela Areti Zervou,Konstantinos Kontras,Maarten De Vos,Panagiotis Tsakalides,Grigorios Tsagatakis*

Main category: cs.CV

TL;DR: ConFu是一个多模态表示学习框架，通过对比学习联合嵌入单个模态及其融合组合，既能捕捉高阶依赖关系，又能保持强配对对应性。


<details>
  <summary>Details</summary>
Motivation: 现有方法大多只处理两模态对齐，无法有效捕捉多模态间的高阶交互，同时往往忽略了保持配对关系的重要性。

Method: 扩展传统配对对比目标，增加融合模态对比项，将模态对与第三模态联合嵌入到统一表示空间中。

Result: 在合成和真实多模态基准测试中，ConFu在检索和分类任务上表现出竞争力，支持统一的一对一和一对多检索。

Conclusion: ConFu框架能够有效捕捉高阶依赖关系（如XOR关系），同时保持强配对对应性，为多模态学习提供了统一解决方案。
...
TL;DR: SIFT-SNN框架：一种低延迟神经形态信号处理系统，用于实时检测交通基础设施中的结构异常，结合SIFT特征提取和SNN分类，在奥克兰海港大桥数据集上达到92.3%准确率，每帧推理时间9.5毫秒。


<details>
  <summary>Details</summary>
Motivation: 开发实时、低功耗的边缘计算系统来监测交通基础设施的结构安全，特别是可移动混凝土护栏的异常检测，这种基础设施在全球20多个城市部署。

Method: 集成尺度不变特征变换(SIFT)进行空间特征编码，使用延迟驱动的脉冲转换层和泄漏积分发放(LIF)脉冲神经网络(SNN)进行分类。

Result: 在包含6,000个标记帧的奥克兰海港大桥数据集上达到92.3%的准确率，每帧推理时间9.5毫秒，脉冲活动稀疏度为8.1%。

Conclusion: SIFT-SNN框架实现了实时低功耗的边缘部署，相比传统CNN方法具有更好的空间特征保持性和可解释性，但未见现场条件的泛化能力仍需验证。
...
### [69] [Referring Video Object Segmentation with Cross-Modality Proxy Queries](https://arxiv.org/abs/2511.21139)
*Baoli Sun,Xinzhu Ma,Ning Wang,Zhihui Wang,Zhiyong Wang*

Main category: cs.CV

TL;DR: 提出ProxyFormer，一种新的RVOS架构，通过代理查询整合视觉和文本语义，解决现有方法中跨模态对齐不足和文本约束延迟的问题。


<details>
  <summary>Details</summary>
Motivation: 现有RVOS方法存在两个主要问题：条件查询缺乏帧间依赖性和变化建模，难以在帧间显著变化时准确跟踪目标；文本约束集成过晚，可能导致视频特征关注非目标对象。
...
Conclusion: SurgMLLMBench将作为稳健资源公开发布，支持可复现的评估和交互式手术推理模型的开发。
...
### [71] [TEAR: Temporal-aware Automated Red-teaming for Text-to-Video Models](https://arxiv.org/abs/2511.21145)
*Jiaming He,Guanyu Hou,Hongwei Li,Zhicong Huang,Kangjie Chen,Yi Yu,Wenbo Jiang,Guowen Xu,Tianwei Zhang*

Main category: cs.CV

TL;DR: 提出了TEAR框架，一种针对文本到视频模型安全性的时间感知自动化红队测试方法，能够通过利用时间动态性生成看似无害的文本提示来触发违反策略的视频输出。


<details>
  <summary>Details</summary>
Motivation: 现有的安全性评估方法主要关注静态图像和文本生成，无法捕捉视频生成中复杂的时间动态特性，导致对文本到视频模型的安全性评估不足。

Method: 采用时间感知测试生成器，通过两阶段优化方法（初始生成器训练和时间感知在线偏好学习）来生成文本上无害但能利用时间动态性触发违规视频的提示，并使用精炼模型循环提升提示的隐蔽性和对抗效果。

Result: 在开源和商业文本到视频系统上的广泛实验评估显示，TEAR实现了超过80%的攻击成功率，相比之前最佳结果57%有显著提升。

Conclusion: TEAR框架有效揭示了文本到视频模型在时间动态性方面的安全风险，证明了现有安全性评估方法的不足，并为改进文本到视频模型的安全性提供了重要参考。
...
### [72] [Monet: Reasoning in Latent Visual Space Beyond Images and Language](https://arxiv.org/abs/2511.21395)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在抽象视觉推理方面缺乏人类般的灵活性，受限于外部工具。需要让模型直接在潜在视觉空间中进行推理，提高推理的灵活性和抽象性。

Method: 采用三阶段蒸馏式监督微调流程，解决潜在视觉对齐的高计算成本和潜在嵌入监督不足问题。提出VLPO（视觉潜在策略优化）强化学习方法，将潜在嵌入显式纳入策略梯度更新。构建了包含12.5万条真实世界、图表、OCR和几何推理链的高质量数据集。

Result: Monet-7B模型在真实世界感知和推理基准上表现一致提升，在具有挑战性的抽象视觉推理任务上展现出强大的分布外泛化能力。

Conclusion: Monet框架成功实现了多模态大语言模型在潜在视觉空间中的直接推理，为视觉潜在推理的未来发展提供了重要见解。模型、数据和代码已开源。
...
### [75] [AnchorOPT: Towards Optimizing Dynamic Anchors for Adaptive Prompt Learning](https://arxiv.org/abs/2511.21188)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于CLIP的提示学习方法使用静态锚点（文本标记）来指导可学习软标记，但这些锚点在值和位置上都固定不变，缺乏跨任务和阶段自适应的灵活性。

Method: 提出AnchorOPT框架，在锚点值和位置关系两个维度引入动态性：1) 锚点值从任务特定数据中动态学习而非手工设计；2) 锚点与软标记的位置关系通过可学习位置矩阵自适应优化。采用两阶段训练策略。

Result: 广泛实验表明，仅使用简单的可学习锚点和位置矩阵，就能达到或超过某些包含额外可学习模块或正则化技术的方法的性能。

Conclusion: AnchorOPT作为一个即插即用模块，能无缝集成到现有框架中，在多样化数据集上带来一致的性能提升。
...
### [76] [From Observation to Action: Latent Action-based Primitive Segmentation for VLA Pre-training in Industrial Settings](https://arxiv.org/abs/2511.21428)
*Jiajie Zhang,Sören Schwertfeger,Alexander Kleiner*

Main category: cs.CV

TL;DR: 提出了一种从工业视频流中自动提取VLA预训练数据的无监督框架，包括运动标记化和基于潜在动作能量的动作分割，无需人工标注即可获得结构化训练数据。


<details>
  <summary>Details</summary>
Motivation: 工业环境中存在大量未标记的人类演示视频数据，但缺乏自动化方法来提取这些数据用于VLA模型预训练，限制了具身AI在制造业的应用。

Method: 首先训练轻量级运动标记器编码运动动态，然后使用基于"潜在动作能量"度量的无监督动作分割器发现和分割语义一致的动作基元，输出分割视频片段和对应的潜在动作序列。

Result: 在公开基准和专有电机装配数据集上的评估显示能有效分割工作台中人类执行的关键任务，通过视觉语言模型的聚类和定量评估确认了发现动作基元的语义一致性。
...
### [77] [Scenes as Tokens: Multi-Scale Normal Distributions Transform Tokenizer for General 3D Vision-Language Understanding](https://arxiv.org/abs/2511.21191)
*Yutao Tang,Cheng Zhao,Gaurav Mittal,Rohith Kukkala,Rama Chellappa,Cheng Peng,Mei Chen*

Main category: cs.CV

TL;DR: NDTokenizer3D是一个通用的3D视觉语言模型，通过多尺度NDT表示和渐进式特征融合，将3D场景转换为整体场景标记，支持多种3D理解任务和人类交互。


<details>
  <summary>Details</summary>
Motivation: 现有3D视觉语言模型在将3D场景有效标记化并在多样化3D理解任务中利用这些标记方面仍面临挑战，需要连接语言级推理与3D空间理解。

Method: 采用三阶段场景标记化流程：1)从原始点云构建多尺度NDT表示；2)通过多尺度NDT解码器渐进融合跨尺度特征生成场景标记；3)将解码器重新用作人类交互提示和分割掩码解码的通用接口。

Result: 在3D参考分割、3D视觉问答和3D密集描述任务中取得了显著改进，实现了细粒度的通用3D视觉语言模型。

Conclusion: NDTokenizer3D通过紧凑统一的设计，提供了一个细粒度的通用3D视觉语言模型，成功连接了语言推理与3D空间理解。
...
### [78] [EvRainDrop: HyperGraph-guided Completion for Effective Frame and Event Stream Aggregation](https://arxiv.org/abs/2511.21439)
...
Method: 使用超图引导的时空事件流补全机制，通过超图连接不同时空位置的事件标记，利用上下文信息传递补全稀疏事件；可灵活融合RGB标记作为超图节点，实现多模态信息补全；通过自注意力聚合不同时间步的超图节点信息。

Result: 在单标签和多标签事件分类任务上的大量实验充分验证了所提框架的有效性。

Conclusion: 提出的基于超图的事件流补全框架能够有效解决事件数据的空间稀疏性问题，并支持多模态特征融合，在事件分类任务上表现出色。
...
TL;DR: 提出一种频率感知的令牌缩减策略，通过将令牌分为高频和低频令牌，选择性保留高频令牌并聚合低频令牌为紧凑的直流令牌，在提高计算效率的同时保持性能并缓解秩崩溃问题。


<details>
  <summary>Details</summary>
Motivation: 视觉Transformer在计算复杂度方面存在挑战，现有令牌缩减方法往往忽略了自注意力机制的频率特性，如秩崩溃和过平滑现象。

Method: 将令牌划分为高频令牌和低频令牌，选择性保留高频令牌，将低频令牌聚合成紧凑的直流令牌以保留必要的低频成分。

Result: 通过广泛实验和分析，该方法在减少计算开销的同时显著提高了准确性，并缓解了秩崩溃和过平滑问题。

Conclusion: 该方法有效解决了视觉Transformer的计算效率问题，同时分析了先前方法的隐含频率特性和局限性。
...
### [80] [You Can Trust Your Clustering Model: A Parameter-free Self-Boosting Plug-in for Deep Clustering](https://arxiv.org/abs/2511.21193)
...
<details>
  <summary>Details</summary>
Motivation: 现有深度聚类方法存在全局和局部特征结构不一致的问题——局部结构在类内样本间表现出强一致性和紧凑性，而全局特征往往呈现交织边界和分离不良的聚类。
...
Conclusion: DCBoost是一种有效的参数自由插件，能够通过利用局部结构线索来增强全局特征结构，从而显著提升深度聚类模型的性能。
...
### [81] [Merge and Bound: Direct Manipulations on Weights for Class Incremental Learning](https://arxiv.org/abs/2511.21490)
...
Result: 在标准CIL基准测试中广泛评估，相比最先进方法展现出优越性能。
...
Method: 使用多模态教师（深度投影视觉模型、高性能3D模型、文本编码器）通过置信度门控机制指导学生模型学习轻量级提示，对齐特征嵌入

Result: 在多种白盒和黑盒攻击下显著优于现有防御方法，在干净数据上性能更优

Conclusion: 提出了一种通过高效利用多模态知识构建鲁棒3D视觉系统的新范式
...
### [83] [Towards an Effective Action-Region Tracking Framework for Fine-grained Video Action Recognition](https://arxiv.org/abs/2511.21202)
...
<details>
  <summary>Details</summary>
Motivation: 现有动作识别方法主要捕获粗粒度运动模式，难以识别随时间演变的局部区域细微细节，导致相似动作难以区分。

Method: 使用区域特定语义激活模块，以文本约束的语义作为查询捕获动作相关区域响应；构建动作轨迹，通过多级轨迹对比约束优化空间和时间层面的响应关系；采用任务特定微调机制优化文本语义表示。

Result: 在广泛使用的动作识别基准测试中表现出优于先前最先进基线的性能。

Conclusion: ART框架通过跟踪局部区域动态变化，有效提升了细粒度动作识别的性能，能够区分相似动作的细微差异。
...
### [84] [Qwen3-VL Technical Report](https://arxiv.org/abs/2511.21631)
...
TL;DR: Qwen3-VL是Qwen系列中最强大的视觉语言模型，在多项多模态基准测试中表现优异，支持256K令牌的交错上下文，包含密集和混合专家架构，在文本理解、长上下文理解和多模态推理方面具有领先性能。


<details>
  <summary>Details</summary>
Motivation: 开发更强大的视觉语言模型，解决多模态任务中的文本理解、长上下文处理和跨模态推理挑战，为图像推理、智能决策和多模态代码智能提供基础引擎。

Method: 采用增强的交错MRoPE进行时空建模，集成DeepStack利用多级ViT特征加强视觉语言对齐，使用基于文本的时间对齐方法进行视频处理，提供密集和混合专家两种架构变体。

Result: 在MMMU、MathVista和MathVision等综合评估中取得领先性能，在纯文本理解、长上下文保留检索和多模态推理任务中表现优异，在相同令牌预算和延迟约束下实现更优性能。

Conclusion: Qwen3-VL作为基础引擎，在图像推理、智能决策和多模态代码智能等实际工作流程中具有广泛应用前景，代表了视觉语言模型的重要进展。
...
### [85] [From Diffusion to One-Step Generation: A Comparative Study of Flow-Based Models with Application to Image Inpainting](https://arxiv.org/abs/2511.21215)
...
<details>
  <summary>Details</summary>
Motivation: 比较不同生成建模范式的性能，特别是迭代采样与直接生成方法的差异，并探索条件流匹配在图像修复任务中的应用潜力。
...
Conclusion: CFM在生成质量上显著优于DDPM，MeanFlow在保持较好质量的同时实现了50倍推理加速。CFM在图像修复任务中表现出色，修复感知训练能显著提升修复质量。
...
TL;DR: 提出了一种允许用户通过少样本学习纠正AI错误分类的新系统，结合服务器端基础模型训练和设备端原型分类，实现高效错误纠正而无需模型重训练。


<details>
  <summary>Details</summary>
Motivation: AI模型在日常设备中广泛使用，但预测错误会降低用户体验。现有解决方案主要关注错误检测，很少提供高效的纠正机制，特别是在资源受限的设备上。
...
### [87] [3-Tracer: A Tri-level Temporal-Aware Framework for Audio Forgery Detection and Localization](https://arxiv.org/abs/2511.21237)
...
<details>
  <summary>Details</summary>
Motivation: 部分音频伪造通过选择性修改关键帧而保持整体感知真实性，现有方法仅独立检测单帧伪造，缺乏捕捉不同时间层次异常的分层结构。

Method: T3-Tracer框架包含FA-FAM模块（结合帧级和音频级时序信息检测帧内伪造线索和全局语义不一致）和SMDAM模块（双分支架构联合建模帧特征和帧间差异，识别伪造边界的突变异常）。

Result: 在三个具有挑战性的数据集上进行广泛实验，证明该方法达到了最先进的性能。

Conclusion: T3-Tracer是首个联合分析音频帧、片段和音频层次的框架，能够全面检测伪造痕迹，有效解决了部分音频伪造检测的挑战。
...
### [88] [Attention-Guided Patch-Wise Sparse Adversarial Attacks on Vision-Language-Action Models](https://arxiv.org/abs/2511.21663)
*Naifu Zhang,Wei Tao,Xi Xiao,Qianpu Sun,Yuxin Zheng,Wentao Mo,Peiqiang Wang,Nan Zhang*

Main category: cs.CV

TL;DR: ADVLA是一种针对视觉-语言-动作(VLA)模型的对抗攻击框架，通过在视觉编码器到文本特征空间的投影特征上直接应用对抗扰动，以低幅度约束有效破坏下游动作预测。


<details>
  <summary>Details</summary>
Motivation: 现有对抗攻击方法需要昂贵的端到端训练，且通常生成明显的扰动补丁，存在成本高和扰动明显的问题。
...
### [89] [FIELDS: Face reconstruction with accurate Inference of Expression using Learning with Direct Supervision](https://arxiv.org/abs/2511.21245)
*Chen Ling,Henglin Shi,Hedvig Kjellström*

Main category: cs.CV

TL;DR: FIELDS提出了一种结合2D图像一致性和3D表情参数直接监督的双重监督方法，用于从单张图像重建保留细微情感线索的高保真3D面部模型。
...
Method: 扩展自监督2D图像一致性线索，加入直接3D表情参数监督和辅助情感识别分支。编码器由自发4D面部扫描的真实表情参数引导，使用强度感知情感损失避免表情夸张。

Result: 从单张图像生成情感丰富的面部模型，具有高度真实的表情，显著提升了野外面部表情识别性能，同时保持自然度。

Conclusion: 双重监督策略有效弥合2D/3D领域差距，减轻表情强度偏差，产生保留微妙情感线索的高保真3D重建结果。
...
### [90] [G$^2$VLM: Geometry Grounded Vision Language Model with Unified 3D Reconstruction and Spatial Reasoning](https://arxiv.org/abs/2511.21688)
*Wenbo Hu,Jingli Lin,Yilin Long,Yunlong Ran,Lihan Jiang,Yifan Wang,Chenming Zhu,Runsen Xu,Tai Wang,Jiangmiao Pang*

Main category: cs.CV

TL;DR: G²VLM是一个几何基础视觉语言模型，通过整合3D视觉几何特征来增强空间智能，在3D重建和空间理解任务上表现优异。
...
Result: 在3D重建任务上达到与最先进前馈模型相当的结果，在空间理解和推理任务上取得更好或竞争性的表现。
...
TL;DR: 本文扩展了可学习多相采样(LPS)方法到复数神经网络，通过理论分析和引入新的投影层设计，在计算机视觉任务中实现了平移等变性和不变性。


<details>
  <summary>Details</summary>
Motivation: 传统卷积神经网络缺乏平移等变性和不变性，特别是在下采样和上采样操作中。虽然数据增强可以经验性地学习这些属性，但需要系统性的方法来理论保证这些性质。

Method: 将可学习多相采样(LPS)扩展到复数神经网络，引入了从复数到实数的投影层，并在Gumbel Softmax之前应用该投影层。

Result: 在多个计算机视觉任务中进行了评估，包括分类任务中的不变性特性，以及重建和语义分割问题中的等变性特性，使用了极化合成孔径雷达图像。

Conclusion: 成功将LPS方法扩展到复数神经网络，为计算机视觉任务提供了理论保证的平移等变性和不变性解决方案。
...
Conclusion: AVFakeBench为音视频伪造检测提供了全面的评估基准，揭示了当前方法在复杂伪造场景下的局限性，为未来发展指明了方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有LiDAR数据生成方法仅支持单帧生成，而预测方法需要多帧历史输入且只能确定性预测多帧，缺乏交互性，两者都无法支持长序列交互生成。

Method: 提出LaGen框架，包含场景解耦估计模块增强对象级内容的交互生成能力，以及噪声调制模块减轻长序列生成中的误差累积。

Result: 在基于nuScenes构建的评估协议上，LaGen全面优于最先进的LiDAR生成和预测模型，尤其在后续帧上表现更佳。
...
TL;DR: MatchGS框架通过几何校正的3DGS数据生成和2D-3D表示对齐，显著提升了零样本图像匹配性能，在公开基准上实现了高达17.7%的改进。


<details>
  <summary>Details</summary>
Motivation: 基于学习的图像匹配需要大规模、多样化且几何准确的训练数据，但3D高斯泼溅(3DGS)存在几何不准确性和深度渲染偏差问题，限制了其在对应关系标注中的应用。

Method: 提出MatchGS框架：1）几何保真的数据生成流程，精炼3DGS几何以产生高精度对应标签；2）2D-3D表示对齐策略，将3DGS的显式3D知识注入2D匹配器。

Result: 生成的对应关系将极线误差降低达40倍，支持极端视角变化的监督，仅使用该数据训练的匹配器在公开基准上实现零样本性能显著提升，改进高达17.7%。
...
<details>
  <summary>Details</summary>
Motivation: 随着Transformer在单个遥感任务上取得优异表现，研究人员希望开发一个统一模型来通过多任务学习在多个任务上表现出色。多任务学习方法相比单任务方法具有更好的泛化性、可扩展性和实际应用价值。

Method: 1. 创建数据管理引擎（数据获取、离线处理和集成、在线加载和加权）
...
Conclusion: PathMamba通过结合Mamba的序列建模和Transformer的全局推理，实现了拓扑优越的分割结果，避免了纯注意力模型的高昂扩展成本。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于扩散模型的3D纹理生成系统存在跨视图不一致问题，即从一个视角看纹理合理，但在其他视角下无法对齐。这源于注意力模糊问题，无结构的全注意力机制导致几何混乱和不稳定的外观-结构耦合。

Method: 引入CaliTex框架，包含两个模块：部分对齐注意力（强制语义匹配部分的空间对齐）和条件路由注意力（通过几何条件路径路由外观信息以保持空间保真度）。结合两阶段扩散变换器，使几何一致性成为网络的固有行为。
...
TL;DR: 提出HTTM方法，一种无需训练的三维token合并技术，用于加速VGGT模型在大型场景重建中的推理速度，通过多头粒度合并token来保持特征独特性，实现7倍加速且性能损失可忽略。


<details>
  <summary>Details</summary>
Motivation: VGGT模型在大型场景重建时需要处理长序列输入，其全局注意力机制导致显著的延迟瓶颈，现有token合并方法会损害模型表示能力。

Method: 提出头级时序合并(HTTM)，在多头粒度上合并token，利用头级别的空间局部性和时序对应关系，保持合并后token的独特性。

Result: 在GPU推理中实现最高7倍加速，性能下降可忽略不计。

Conclusion: HTTM是一种有效的训练free加速方法，能够显著提升VGGT在大型场景重建中的推理效率。
...
TL;DR: 提出了一种基于多尺度特征融合的点云法向量估计方法，通过特征聚合和跨尺度补偿来解决不同几何结构下邻域尺寸选择困难的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理不同数据或几何结构时难以确定合适的邻域尺寸，且参数密集，导致法向量估计精度和效率不足。

Method: 使用多尺度特征融合策略，包括特征聚合模块（逐步聚合不同尺度特征并缩小邻域）和特征补偿模块（确保大尺度特征的复用性）。

Result: 在合成和真实数据集上达到最先进性能，网络参数和运行时间更少。

Conclusion: 基于多尺度特征聚合的近似策略能够实现不同局部邻域的尺度自适应，提供最优特征描述。
...
TL;DR: Endo-G²T是一个用于动态内窥镜场景的几何引导和时间感知训练方案，通过几何先验蒸馏、时间嵌入高斯场和关键帧约束流式处理，解决内窥镜视频中的视图依赖效应和几何漂移问题。


<details>
  <summary>Details</summary>
Motivation: 内窥镜视频存在强烈的视图依赖效应（如镜面反射、湿润反射和遮挡），纯光度监督与几何不对齐，导致早期几何漂移，错误的形状在密集化过程中被强化且难以纠正。

Method: 1. 几何引导先验蒸馏：将置信度门控的单目深度转换为监督，使用尺度不变深度和深度梯度损失；2. 时间嵌入高斯场：在XYZT中表示动态，采用转子式旋转参数化；3. 关键帧约束流式处理：在最大点数预算下进行关键帧优化，非关键帧轻量更新。

Result: 在EndoNeRF和StereoMIS-P1数据集上，Endo-G²T在单目重建基线中达到了最先进的结果。

Conclusion: Endo-G²T通过几何引导和时间感知训练，成功解决了内窥镜场景中的几何漂移问题，实现了时间一致的几何重建和高效率处理。
...
<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型具有强大的语言理解能力，但由于训练目标不对齐和标准视觉编码器中细粒度区域-词对齐较弱，它们在时空视频定位任务上表现不佳。

Method: 提出边界框思维链机制，在最终预测前明确推理时空位置；设计包含格式、一致性、时间、空间和思考奖励的多维度强化奖励函数，通过强化微调提供几何感知监督。

Result: 在HCSTVG-v1/v2和VidSTG上评估，STVG-o1在HCSTVG上创造了新的最先进结果，在HCSTVG-v1上比最佳任务特定方法高出7.3% m_tIoU，在VidSTG上与专用模型相当，并大幅超越所有现有的基于MLLM的方法。

Conclusion: 该框架证明了多模态大语言模型可以作为精确时空定位任务的可行且强大的骨干网络，并展示了强大的跨数据集开放词汇泛化能力。
...
TL;DR: DiverseVAR框架通过文本嵌入噪声注入和scale-travel潜在优化技术，在不需重新训练的情况下提升视觉自回归模型的多样性，同时保持图像质量。


<details>
  <summary>Details</summary>
Motivation: VAR模型在图像生成中虽然质量优秀，但存在多样性不足的问题，即使对于简单提示也会产生几乎相同的图像。这个问题在追求图像质量的主流研究中被忽视。

Method: 1. 在文本嵌入中注入噪声以增强多样性；2. 提出scale-travel技术，利用多尺度自编码器提取粗粒度token，在中间阶段恢复生成以保持图像质量。

Result: 实验表明，文本嵌入噪声注入与scale-travel优化相结合，显著提升了多样性，同时最小化了图像质量损失，在多样性-质量权衡中达到了新的帕累托前沿。
...
Conclusion: MobileI2V首次证明了在移动设备上实现实时高质量图像到视频生成的可行性，为移动端视频生成应用开辟了新途径。
...
TL;DR: 提出基于交叉注意力的非局部知识蒸馏(CanKD)，通过交叉注意力机制增强知识迁移过程，让学生特征图的每个像素动态考虑教师特征图的所有像素，实现更全面的像素级关系捕捉。


<details>
  <summary>Details</summary>
Motivation: 传统自注意力蒸馏方法独立对齐师生特征图，无法充分利用教师特征图的全局信息，需要更有效的非局部知识迁移机制来提升特征表示学习。

Method: 引入交叉注意力机制，使学生特征图的每个像素都能动态关注教师特征图的所有像素，仅增加一个额外的损失函数来实现知识蒸馏。
...
<details>
  <summary>Details</summary>
Motivation: 深度伪造检测方法的有效性往往更多依赖于数据预处理、增强策略和优化技术等实现细节，而非核心设计，这使得公平比较检测器变得困难。

Method: 通过系统研究不同设计选择对深度伪造检测模型准确性和泛化能力的影响，重点关注训练、推理和增量更新等方面，分离各个因素的影响。

Result: 实验确定了能持续提升深度伪造检测性能的设计选择，并在AI-GenBench基准测试中实现了最先进的性能。

Conclusion: 建立了架构无关的稳健最佳实践，为未来深度伪造检测系统的设计和开发提供指导。
...
<details>
  <summary>Details</summary>
Motivation: ANA检测对于诊断自身免疫性疾病至关重要，但手动检测缓慢、劳动密集且需要多年训练。虽然机器学习和深度学习实现了自动化，但真实临床环境中的ANA检测涉及多实例多标签学习，面临独特挑战。

Method: 受人类标注逻辑启发，识别一致的ANA子区域并分配聚合标签。使用三个任务特定组件：实例采样器、概率伪标签分配器和自定步调学习率系数。实例采样器通过建模模式置信度抑制低置信度实例，分配器基于实例可区分性自适应分配标签，自定步调学习根据经验标签观察调整训练。

Result: 在一个ANA数据集和三个公共医疗MIML基准测试上进行广泛实验。在ANA数据集上，相比最佳先前方法，F1-Macro提升+7.0%，mAP提升+12.6%，创下新的最先进结果。在公共数据集上所有关键指标排名前2，汉明损失和单错误分别降低达18.2%和26.9%。
...
TL;DR: 提出了一种用于构建遥感基础模型的高效集成专家框架，通过轻量级任务特定的ConvNeXtV2专家模型分解训练过程，解决了现有大模型方法在计算资源和可持续性方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 当前基础模型主要依赖模型规模和数据集大小的扩展，需要大量计算和数据资源，限制了可访问性，且与可持续AI原则相悖。遥感领域需要更高效、环保的解决方案。

Method: 采用集成专家框架，将训练过程分解为轻量级的任务特定ConvNeXtV2专家模型，支持冻结和重用，具有模块化特性，支持联邦训练、剪枝和持续专家集成。

Result: 该方法在效率、可解释性和可扩展性方面具有显著优势，特别适合协作和资源受限的环境。

Conclusion: 该框架为构建可扩展且高效的遥感基础模型设定了新方向，提供了计算资源友好且可持续的替代方案。
...
Result: 消融研究表明定量指标显著提高MRI图像合成准确性，结构相似性指数达到0.882，显示合成图像具有高度相似性。
...
TL;DR: 提出了PRFL框架，在潜在空间进行偏好优化，避免了VAE解码，显著提高了视频生成与人类偏好的对齐效果，同时大幅降低了内存消耗和训练时间。


<details>
  <summary>Details</summary>
Motivation: 现有的视频奖励模型依赖为像素空间输入设计的视觉语言模型，将ReFL优化限制在计算昂贵的VAE解码后的近完成去噪步骤，存在内存开销大、训练时间长的问题，且后期优化缺乏早期监督。

Method: 利用预训练视频生成模型在噪声潜在空间进行奖励建模，提出PRFL框架，在潜在空间进行偏好优化，实现整个去噪链的高效梯度反向传播。

Result: 实验表明PRFL显著改善了与人类偏好的对齐效果，同时相比RGB ReFL实现了内存消耗和训练时间的大幅减少。

Conclusion: 预训练视频生成模型天然适合在噪声潜在空间进行奖励建模，PRFL框架在潜在空间进行偏好优化是更高效的解决方案。
...
TL;DR: UAVLight是一个用于光照鲁棒性3D重建的基准数据集，通过可重复的飞行路径在不同时间点采集数据，提供自然光照变化下的真实户外场景重建评估。


<details>
  <summary>Details</summary>
Motivation: 多视角3D重建中的光照不一致性是一个根本性挑战，现有数据集要么缺乏有意义的光照多样性，要么因几何和语义变化而无法单独研究光照鲁棒性。

Method: 在可重复的地理参考飞行路径上，在一天中多个固定时间点采集场景数据，在保持几何、校准和视点一致性的前提下产生自然光照变化。

Result: 创建了一个受控但真实的基准数据集，提供标准化评估协议，支持在不同光照条件下开发和评估重建方法。

Conclusion: UAVLight为开发在真实户外环境中保持一致性、保真度和可重光照性的重建方法提供了可靠基础。
...
Conclusion: 通过集成姿态估计损失，可以改善在可变患者姿态条件下的地标检测性能。
...
<details>
  <summary>Details</summary>
Motivation: 开源模型在音频-视频对齐方面存在挑战，主要源于联合扩散过程中的三个根本问题：对应漂移、低效的全局注意力机制以及传统CFG的模态内偏见。

Method: 提出Cross-Task Synergy训练范式缓解漂移，设计Global-Local Decoupled Interaction Module实现高效时间对齐，开发Synchronization-Enhanced CFG在推理时显式增强对齐信号。

Result: 广泛实验表明Harmony在生成保真度和音频-视频同步方面显著优于现有方法，建立了新的最先进水平。

Conclusion: Harmony框架通过机制化强制音频-视频同步，有效解决了联合扩散过程中的核心挑战，为同步音频-视觉内容生成提供了创新解决方案。
...
TL;DR: 该研究使用深度学习构建了一个16种口腔病变的多分类器，通过分层数据分割、数据增强和过采样技术解决了数据集有限和不平衡的问题，在口腔癌早期检测方面取得了83.33%的准确率。


<details>
  <summary>Details</summary>
Motivation: 口腔癌在全球非常普遍，但由于口腔内良性、癌前病变和恶性病变在视觉上相似，多数在晚期才被诊断。早期实施计算机辅助诊断系统有望显著改善临床结果。

Method: 结合分层数据分割、高级数据增强和过采样技术，构建深度学习多分类器来分类16种不同的口腔病变。

Result: 实验结果显示模型达到83.33%准确率、89.12%精确率和77.31%召回率，优于现有最先进方法，特别是在少数类分类方面表现突出。

Conclusion: 该框架展示了过采样和增强策略在少数类分类中的有效性，是迈向可信赖的口腔癌早期检测计算机辅助诊断系统的第一步，在临床环境中具有应用前景。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频扩散模型在帧级保真度上表现良好，但在运动连贯性、动态性和真实感方面存在不足，经常产生抖动、重影或不合理的动态效果。标准的去噪MSE目标缺乏对时间一致性的直接监督。

Method: 基于3步蒸馏的视频扩散模型，训练基于DiT的光学流判别器来区分真实与生成的运动，结合分布匹配正则化器来保持视觉保真度。

Result: 在VBench上，MoGAN比50步教师模型提升运动分数7.3%，比3步DMD模型提升13.3%；在VideoJAM-Bench上，分别提升7.4%和8.8%，同时保持相当或更好的美学和图像质量分数。人类研究也证实MoGAN在运动质量上更受偏好。
...
TL;DR: 提出了一种自提示、点监督的框架，使用稀疏点标注将SAM适应到遥感图像，通过Refine-Requery-Reinforce循环逐步提升分割质量。


<details>
  <summary>Details</summary>
Motivation: 解决SAM在遥感图像上因领域偏移和密集标注稀缺导致的性能下降问题。
...
TL;DR: 提出了一种标签高效的图卷积网络模型，通过对抗策略选择信息量大的样本进行标注，并引入双向稳定GCN架构来提高骨架动作识别的性能。
...
TL;DR: CaFlow是一个用于长时序动作质量评估的统一框架，通过因果反事实正则化和双向时序条件流来提升模型对长期动态的建模能力和对上下文混杂因素的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在长时序动作质量评估中面临两个主要挑战：依赖昂贵的标注数据，以及单向时序建模容易受到伪相关性和不稳定长期表示的影响。

Method: 提出CaFlow框架，包含两个核心模块：因果反事实正则化(CCR)模块以自监督方式解耦因果和混杂特征，并通过反事实干预增强因果鲁棒性；BiT-Flow模块通过循环一致性约束建模前向和后向动态，生成更平滑连贯的表示。

Result: 在多个长时序AQA基准测试上的广泛实验表明，CaFlow达到了最先进的性能。

Conclusion: CaFlow通过整合因果反事实解耦和双向时序建模，有效解决了长时序动作质量评估中的挑战，为体育、康复和技能评估等应用提供了更鲁棒的解决方案。
...
TL;DR: Multi-Crit是一个评估多模态模型遵循多样化细粒度标准能力的基准，涵盖开放生成和可验证推理任务，通过系统指标评估模型在多元标准遵循、标准切换灵活性和识别标准冲突方面的表现。


<details>
  <summary>Details</summary>
Motivation: 现有大模型作为多模态评估者时，其遵循多样化细粒度评估标准的能力尚未充分探索，需要系统评估模型在多元标准下的判断能力。

Method: 通过严格的数据筛选流程构建Multi-Crit基准，收集具有多标准人工标注的挑战性响应对，并引入三个新指标系统评估模型能力。

Result: 对25个多模态模型的分析显示：专有模型在开放评估中难以保持多元标准一致性；开源模型在灵活遵循多样化标准方面更落后；基于整体判断信号的批评微调无法泛化到多元标准级判断。

Conclusion: Multi-Crit为构建可靠且可控的多模态AI评估奠定了基础，揭示了当前多模态评估者的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 胶质瘤具有高死亡率，早期准确诊断对治疗干预至关重要，需要开发能够精确分割和分类脑肿瘤的深度学习模型。

Method: 使用U-Net进行3D MRI数据肿瘤分割，结合DenseNet和VGG的混合分类网络，集成多头注意力和空间通道注意力机制，通过预处理步骤处理高维3D MRI数据。

Result: 分割任务Dice系数达到98%，分类准确率达到99%，优于传统CNN模型和无注意力方法，注意力机制增强了临床相关特征的识别。
...
<details>
  <summary>Details</summary>
Motivation: 探索仅凭相机运动轨迹（而非图像像素）能否理解视频内容，验证"如何移动"能否揭示"在做什么"或"观察什么"。

Method: 提出对比学习框架训练CamFormer编码器，将相机姿态轨迹投影到联合嵌入空间，与自然语言对齐。

Result: 相机轨迹是揭示视频内容的显著信息信号，CamFormer嵌入在跨模态对齐、分类和时间分析等任务中表现优异，且对不同相机姿态估计方法具有鲁棒性。
...
TL;DR: Canvas-to-Image是一个统一框架，将文本提示、主题参考、空间布局、姿态约束和布局标注等多种控制信号整合到单一画布界面中，通过多任务训练使扩散模型能够同时理解和集成异构控制，显著提升组合生成的质量和保真度。


<details>
  <summary>Details</summary>
Motivation: 现代扩散模型在生成高质量多样化图像方面表现出色，但在处理高保真度的组合和多模态控制方面仍有困难，特别是当用户需要同时指定文本提示、主题参考、空间排列、姿态约束和布局标注时。

Method: 将多样控制信号编码到单一复合画布图像中，使模型能够直接解释以进行集成视觉空间推理；策划多任务数据集并提出多任务画布训练策略，在统一学习范式下优化扩散模型联合理解和集成异构控制。
...
TL;DR: 本文研究使用大型语言模型自动重构层次结构以优化双曲嵌入质量，实验证明LLM重构的层次结构能显著提升嵌入质量并支持可解释的重组。


<details>
  <summary>Details</summary>
Motivation: 双曲嵌入的质量与输入层次结构密切相关，而现有层次结构往往不符合最优双曲嵌入所需的高分支因子和单继承特性，需要知识工程师手动重组，过程耗时且困难。

Method: 提出基于提示的方法，使用大型语言模型根据双曲嵌入的理想标准自动转换现有层次结构。

Result: 在16个不同层次结构上的实验表明，LLM重构的层次结构在多个标准嵌入质量指标上都能产生更高质量的双曲嵌入。

Conclusion: LLM能够有效自动重构层次结构以优化双曲嵌入，同时提供可解释的重组理由，辅助知识工程师的工作。
...
TL;DR: 提出了首个极端天气智能诊断代理框架EWE，通过知识引导规划、闭环推理和气象工具包，从原始气象数据自动生成多模态可视化并进行诊断分析。


<details>
  <summary>Details</summary>
Motivation: 极端天气事件对全球社会构成日益严重的风险，但当前专家驱动、劳动密集的诊断范式造成了分析瓶颈，阻碍了科学进展。

Method: EWE框架模拟专家工作流程，包括知识引导规划、闭环推理和领域定制的气象工具包，能够从原始气象数据自主生成和解释多模态可视化。

Result: 建立了该新兴领域的首个基准，包含103个高影响事件的精选数据集和新的逐步评估指标。
...
<details>
  <summary>Details</summary>
Motivation: 现有安全数据集主要是英语中心化的，无法捕捉非英语社会文化背景（如韩语）中的特定风险，且通常仅限于文本模态。

Method: 定义了35个AI风险因素分类法，采用两阶段构建（专家引导播种和众包扩展）、三重独立标注和迭代专家红队循环的严格质量控制流程。

Result: 试点研究验证了AssurAI在评估最新LLM安全性方面的有效性，数据集已公开发布。

Conclusion: AssurAI有助于为韩语社区开发更安全可靠的生成AI系统。
...
<details>
  <summary>Details</summary>
Motivation: 解决太阳物理领域大型语言模型推理的挑战，包括物理假设整合、单位一致性和科学格式要求

Method: 构建了基于NASA和UCAR夏季学校问题集的数据集，采用程序化评分器进行验证，比较了单次提示和四种多智能体模式

Result: 通过系统工程原则分解工作流程的方法在演绎推理问题上表现优于直接提示

Conclusion: 多智能体推理模式在复杂科学推理任务中具有优势，特别是在需要演绎推理而非纯归纳记忆的问题上
...
<details>
  <summary>Details</summary>
Motivation: 将数字孪生技术从工业领域引入医疗健康，旨在实现从被动治疗向预测性、预防性和个性化医疗的转变，通过创建患者特异性虚拟模型来改善医疗决策和结果。

Method: 整合医学影像、生物传感器和计算模型，构建动态数据驱动的患者虚拟对应体，通过实时数据流持续更新，支持双向交互，应用于心脏、肿瘤和药物开发等领域。

Result: 代表性应用包括预测心律失常治疗结果的心脏数字孪生、追踪肿瘤进展和优化放疗的肿瘤数字孪生，以及加速药物发现的药理学数字孪生。

Conclusion: 尽管面临互操作性、数据隐私和模型保真度等挑战，但通过可解释AI、联邦学习和统一监管框架等新兴解决方案，数字孪生技术有望推动医疗健康向预测性、预防性和个性化方向发展。
...
<details>
  <summary>Details</summary>
Motivation: 为了提供一个易于使用的通用PAL2v计算库，减少算法实现的复杂性、代码量和错误，支持推理和决策系统的开发。

Method: 设计为通用PAL2v标准计算库，提供三种结果类型：12种经典格区域的矛盾分析、矛盾分析节点输出和决策输出，支持独立或网络形式的算法实现。

Result: 实现了Para-analyzer、ParaExtrCTX、PAL2v Filter、PANnet和PNN等知名PAL2v算法，通过两个示例展示了库的有效性。

Conclusion: Paraconsistent-Lib是一个稳定且持续开发的库，能够响应用户在GitHub上提出的功能需求和改进建议。
...
TL;DR: 该研究对多模态思维链(Multimodal-CoT)在不同领域的泛化能力进行了全面分析，发现在科学推理之外的常识推理任务中效果有限。


<details>
  <summary>Details</summary>
Motivation: 探索多模态思维链方法在科学问答之外的多样化领域（如常识推理）的泛化能力，因为现有方法主要关注科学推理任务。

Method: 采用Zhang等人提出的两阶段框架，将理由生成与答案推理分离，通过门控融合机制整合视觉特征与T5语言模型，在A-OKVQA、OKVQA和ChartQA数据集上进行系统消融研究。

Result: 视觉特征整合显著减少了理由生成中的幻觉，但思维链推理的有效性在不同问题类型间差异很大，常识推理尤其具有挑战性。

Conclusion: 该研究为实施多模态推理系统提供了实用见解，并确定了跨领域泛化能力改进的关键方向。
...
TL;DR: 提出了一个基于大型语言模型代理的智能Wi-Fi框架，使接入点能够通过自然语言对话实时协商自适应协调策略，显著提升密集网络环境下的吞吐量。
...
Result: 综合仿真结果显示，该代理框架成功适应了多样化和动态的网络环境，显著优于最先进的空分复用基线方法。
...
TL;DR: OpenApps是一个轻量级开源生态系统，包含6个可配置外观和内容的应用程序，用于评估多模态UI代理在不同应用变体中的可靠性。研究发现，代理在固定应用中的可靠性相对稳定，但在不同应用变体中的成功率波动可达50%以上。


<details>
  <summary>Details</summary>
Motivation: 当前自主UI代理的评估依赖于固定环境，无法反映代理在遇到应用设计和内容变化时的可靠性。需要开发能够测量代理在不同应用变体中可靠性的评估方法。

Method: 开发OpenApps生态系统，包含6个可配置应用（消息、日历、地图等），只需单个CPU即可运行，能够生成和部署数千个应用版本。进行了超过10,000次独立评估，研究7个领先多模态代理的可靠性。

Result: 研究发现代理在固定应用中的可靠性相对稳定，但在不同应用变体中的成功率波动很大。例如Kimi-VL-3B的平均成功率从63%波动到仅4%。代理行为如循环或幻觉操作也因环境配置而异。

Conclusion: 应用变体是评估UI代理可靠性的重要新维度。OpenApps为研究代理在不同应用环境中的可靠性提供了有效工具。
...
TL;DR: RILKE是一种在表示空间进行干预的知识控制方法，能够在冻结基础权重的情况下实现细粒度知识更新，同时保持模型通用能力。


<details>
  <summary>Details</summary>
Motivation: 解决LLMs产生错误或过时内容的问题，避免昂贵的重新训练，在终身学习设置中实现多知识编辑共存且不相互干扰。

Method: 在表示空间进行干预，学习抗释义和编辑局部化模块，将每个更新限制在低维子空间以减少交叉干扰，推理时通过查询自适应路由器选择适当模块。

Result: 在LLaMA和Qwen模型的知识编辑基准测试中，RILKE可扩展到大规模数据集，展示高编辑成功率、强释义泛化能力，并以适度内存开销保持通用能力。
...
TL;DR: 提出了第一个保证最优组合解释的计算框架，通过分解空间对齐因素、设计启发式估计和高效算法，解决了现有方法无法保证最优性的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于波束搜索的组合解释方法无法提供理论最优性保证，且不清楚当前解释与真正最优解的接近程度。

Method: 提出三部分框架：(i)识别影响空间对齐因素的分解方法，(ii)搜索过程中对齐度的启发式估计，(iii)首个能在可行时间内计算最优组合解释的算法。

Result: 在计算机视觉和卷积神经网络场景中，10-40%的波束搜索解释在涉及重叠概念时是次优的；提出的引导波束搜索变体在保持或改进运行时间的同时提供更好的超参数灵活性。
...
TL;DR: ENACT是一个评估视觉语言模型是否展现具身认知的基准，通过世界建模任务测试模型从自我中心交互中理解环境的能力。


<details>
  <summary>Details</summary>
Motivation: 研究现代视觉语言模型是否在脱离实体训练的情况下仍表现出具身认知的特征，即智能是否源于感知运动交互而非被动观察。

Method: 创建ENACT基准，将具身认知评估转化为基于自我中心交互的世界建模任务，采用部分可观察马尔可夫决策过程框架，包含前向世界建模（给定动作重排观察序列）和逆向世界建模（给定观察重排动作序列）两个互补任务。

Result: 实验显示前沿视觉语言模型与人类之间存在性能差距，且随着交互时间跨度增加而扩大。模型在逆向任务上表现优于前向任务，并表现出以人类为中心的偏见，如偏好右手动作、当相机参数或视角偏离人类视觉时性能下降。

Conclusion: 现代视觉语言模型确实表现出一些具身认知的特征，但与人类相比仍有明显差距，特别是在长时程交互和视角泛化方面存在局限。
...
TL;DR: Ivy是一个AI教练系统，通过结合符号化的任务-方法-知识模型和生成解释层，提供结构化的多步骤解释，解决了LLM生成解释时缺乏因果、目标导向和组合逻辑的问题。
...
Method: Ivy系统结合符号化的任务-方法-知识模型和生成解释层，TMK模型编码因果转换、目标层次和问题分解，在明确的结构边界内指导LLM构建解释。

Result: 与GPT和检索增强GPT基线相比，Ivy在三个推理维度上通过专家和独立标注评估，结果显示符号约束一致提高了对"如何"和"为什么"问题的解释结构质量。
...
TL;DR: 提出了ICPO方法，通过利用LLM生成不同响应的概率来反映其对推理过程的自我评估，结合可验证奖励解决现有RLVR方法中的粗粒度奖励、奖励噪声和低效探索问题。
...
Conclusion: ICPO通过偏好优势分数有效缓解了粗粒度奖励和奖励噪声问题，抑制过度自信错误，增强被低估高质量响应的相对优势，防止模型对特定策略过拟合，促进更彻底的探索。
...
TL;DR: L4M框架结合对抗性LLM代理和SMT求解器证明，将自然语言的解释灵活性与符号验证的严谨性相结合，在法理推理中超越现有LLM和Legal AI系统。


<details>
  <summary>Details</summary>
Motivation: 现有LLM系统擅长表层文本分析，但缺乏原则性法理学所需的保证。法律理性包括实质理性（结果公平性）和形式理性（遵循明确规则），需要将自然语言灵活性与符号验证严谨性结合。

Method: 三阶段流程：1) 法规形式化-将法律条款转换为逻辑公式；2) 双重事实和法规提取-检察官和辩护方对齐的LLM独立映射案例叙述；3) 求解器中心裁决-将双方论点编译为逻辑约束，通过不满足核心触发迭代自我批判，最终由法官LLM生成透明判决。

Result: 在公共基准测试中，该系统超越了GPT-4-mini、DeepSeek-V3、Claude 4等先进LLM以及最先进的Legal AI基线，同时提供严谨且可解释的符号论证。

Conclusion: L4M成功地将LLM的灵活性与符号验证的严谨性相结合，为法律AI系统提供了可解释且可靠的法理推理能力，填补了现有系统的关键空白。
...
TL;DR: 提出了OVOD-Agent框架，将被动类别匹配转变为主动视觉推理和自演进检测，通过Visual-CoT和w-MDP建模视觉上下文转换，结合Bandit模块和奖励模型优化，在COCO和LVIS数据集上取得显著改进。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇目标检测方法虽然在多模态数据集上预训练，但推理仍受限于固定类别名称，存在多模态训练与单模态推理之间的差距。文本表示仍有优化空间。

Method: 提出OVOD-Agent框架，采用Visual-CoT进行可解释的视觉推理，将视觉上下文转换建模为弱马尔可夫决策过程(w-MDP)，使用Bandit模块生成探索信号，结合马尔可夫转移矩阵和Bandit轨迹进行自监督奖励模型优化。

Result: 在COCO和LVIS数据集上的实验表明，OVOD-Agent在各种OVOD骨干网络上均能提供一致改进，特别是在稀有类别上表现突出。

Conclusion: OVOD-Agent框架通过主动视觉推理和自演进检测机制，有效提升了开放词汇目标检测性能，验证了所提方法的有效性。
...
TL;DR: 本文抽象了Halpern-Pearl因果关系定义的关键特征，使其能应用于任何定义了反事实的模型，扩展了因果关系的适用范围。


<details>
  <summary>Details</summary>
Motivation: Halpern-Pearl的因果关系定义局限于因果模型，无法处理包含析取、否定、信念和嵌套反事实的复杂公式。本文旨在抽象该定义，使其能应用于更广泛的模型。

Method: 通过提取Halpern-Pearl因果关系定义的关键特征，构建一个抽象定义框架，使其适用于任何定义了反事实的模型。

Result: 成功开发了一个抽象因果关系定义，能够处理更复杂的逻辑公式，包括析取、否定、信念和嵌套反事实，并能扩展到解释概念。

Conclusion: 抽象化Halpern-Pearl因果关系定义不仅扩展了其应用范围，还深化了对因果关系定义特征的理解，为在其他模型中应用因果关系概念提供了基础。
...
TL;DR: Prune4Web是一种新颖的网页自动化方法，通过程序化修剪DOM树来解决大型DOM结构处理问题，实现25-50倍的元素减少，显著提升动作定位精度。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的网页代理在处理复杂网页时面临DOM结构过大（1万-10万token）的挑战，现有方法要么粗暴截断DOM丢失关键信息，要么使用低效启发式方法，无法在精度和可扩展性之间取得平衡。

Method: 提出DOM树修剪编程范式，让LLM生成可执行的Python评分脚本来动态过滤DOM元素，基于分解子任务的语义线索。该方法将DOM处理从资源密集的LLM读取转向高效的程序化修剪。

Result: 在低级别定位任务上，准确率从46.8%大幅提升至88.28%，实现了25-50倍的候选元素减少，有效减轻注意力稀释问题。

Conclusion: Prune4Web通过统一的框架联合优化规划器、程序化过滤器和定位器，在真实网页自动化中展现出卓越效能，为复杂网页导航提供了可扩展且精确的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前伪布尔求解中最成功的单元传播策略是结合了监视文字方案和计数方法的混合模式，但现有的混合决策方法仍有改进空间。

Method: 提出了新的启发式方法来确定何时在混合单元传播策略中使用监视文字方案或计数方法。

Result: 新启发式方法能够显著优于当前方法，在RoundingSAT求解器中表现出色。

Conclusion: 新的混合决策启发式方法有效提升了伪布尔求解器的性能，证明了改进单元传播策略决策过程的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 传统药物重定位预测需要多领域专家协作，现有工具碎片化且数据异构，难以集成到统一工作流中。ChatDRex旨在解决这些问题，让临床专家无需计算机专业知识也能进行复杂分析。

Method: 基于NeDRex知识图谱构建多智能体系统，包括查询路由、数据检索、算法执行、结果可视化等专门智能体，支持网络分析、功能一致性评估、文献挖掘等生物信息学任务。

Result: 系统实现了自然语言驱动的药物重定位分析，包含幻觉检测机制，能够生成科学假设并探索药物重定位机会。

Conclusion: ChatDRex民主化了生物信息学资源的访问，加速了新型疗法的发现，推进了个性化医疗和转化研究。
...
TL;DR: MADRA是一个无需训练的多智能体辩论风险评估框架，通过集体推理增强AI代理的安全性意识，同时保持任务性能。该框架使用多个基于LLM的智能体辩论指令安全性，结合分层认知协作规划，在AI2-THOR和VirtualHome上实现超过90%的危险任务拒绝率。


<details>
  <summary>Details</summary>
Motivation: 解决现有方法在具身AI代理任务规划中的安全问题，现有方法存在计算成本高或过度拒绝的问题，特别是在家庭环境中危险指令构成重大风险。

Method: 提出MADRA框架：1) 多智能体辩论机制，多个LLM智能体辩论指令安全性；2) 关键评估器对响应进行评分；3) 迭代审议和共识投票；4) 分层认知协作规划框架，整合安全、记忆、规划和自我进化机制。

Result: 在AI2-THOR和VirtualHome上的实验表明，该方法对不安全任务的拒绝率超过90%，同时保持对安全任务的低拒绝率，在安全性和执行效率方面优于现有方法。

Conclusion: 该工作为构建可信赖的具身代理提供了可扩展、模型无关的解决方案，显著减少了误拒绝，同时保持对危险任务的高敏感性。
...
Method: 构建了分层空间认知框架，将空间智能分解为五个认知层次，并开发了SpatialBench基准，包含15个任务，采用统一的能力导向指标进行评估。

Result: 实验发现模型在不同认知层次上表现分层：感知基础能力强，但在符号推理、因果推断和规划方面仍有限制。人类测试显示人类进行选择性、目标导向的抽象，而MLLMs倾向于过度关注表面细节。
...
Result: 该方法在多个数学验证基准上显著提升性能，计算效率甚至超过扩展长链思维，且发现数据集标注错误导致性能被低估。
...
TL;DR: 该研究审计了16个开源模型在专业角色下的AI身份披露能力，发现模型在专业领域存在显著的不一致性披露行为，某些模型在特定领域披露率低至3.5%，存在用户过度信任的风险。


<details>
  <summary>Details</summary>
Motivation: 研究动机是确保语言模型在专业高风险领域能够可靠披露其AI身份，避免用户因无法识别模型能力边界而受到伤害。

Method: 采用common-garden设计，对16个开源模型（4B-671B参数）进行了19,200次试验，使用贝叶斯验证和Rogan-Gladen校正确保结果稳健性。

Result: 模型在不同专业领域披露率差异显著（2.8%-73.6%），金融顾问角色初始披露率30.8%，神经外科医生仅3.5%。模型身份比参数数量更能预测行为，推理优化反而会抑制透明度。
...
TL;DR: 本文提出"负责任计算前瞻"概念，探讨人工智能在负责任前瞻中的作用，强调AI作为支持工具增强而非替代政策制定者判断，以应对21世纪重大挑战。


<details>
  <summary>Details</summary>
Motivation: 在技术快速发展和全球挑战复杂的时代，需要负责任的前瞻框架来帮助政策制定者应对未来不确定性，关注可持续和负责任的未来设计。

Method: 建立负责任计算前瞻的基础原则，提出一套AI驱动的前瞻工具，结合模拟和情景分析，增强政策制定者应对不确定性和评估风险的能力。

Result: AI与模拟、情景分析结合能增强政策制定者处理不确定性、评估风险和制定可持续战略的能力，但需要理解社会、环境、经济和政治系统的相互依赖性。

Conclusion: AI应在负责任、以人为本的前瞻中发挥支持性工具作用，补充而非替代政策制定者判断，推动有韧性和道德健全的未来建设。
...
TL;DR: LLMs在8拼图任务中表现出规划能力不足，即使有外部验证器辅助也无法解决任何谜题，主要问题包括脆弱的内部状态表示和弱启发式规划能力。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs在没有代码执行或其他工具辅助下的规划和状态推理能力，使用8拼图作为评估平台。

Method: 测试四个模型在零样本、思维链、算法思维等提示条件下，并采用分层纠正反馈和外部移动验证器。

Result: 反馈对某些模型-提示组合有改善，但成功运行通常冗长且计算昂贵；即使有外部验证器，所有模型都无法解决任何谜题。
...
TL;DR: 该论文提出了一个将系统动力学和结构方程建模结合的统一数学框架，用于解决AI/ML模型开发中的偏见问题。
...
Result: 建立了一个统一的框架来弥合不同方法论之间的鸿沟，为数据科学和AI/ML应用提供系统动力学的认识论基础。

Conclusion: 该统一框架有助于推进负责任AI/ML的发展，通过整合不同方法论来解决AI系统中的偏见问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于轨迹的记忆方法存在简洁性偏差，逐渐丢失重要领域知识，且仅记录单模态行为轨迹，无法保留视觉注意力和逻辑推理如何共同贡献于解决方案，这与人类的多模态整合语义记忆不匹配。
